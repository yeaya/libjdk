...
void java$transaction$xa$test$PreloadClass() {
}

void java$transaction$xa$test$PreinitClass() {
}

void java$transaction$xa$test$LibEventAction(int32_t eventType, void* eventData) {
	if (eventType == JCPP_LIB_EVENT_TYPE_PRELOAD_CLASS) {
		java$transaction$xa$test$PreloadClass();
	} else if (eventType == JCPP_LIB_EVENT_TYPE_PREINIT_CLASS) {
		java$transaction$xa$test$PreinitClass();
	}
}
