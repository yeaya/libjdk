...
$Class* ObjectTable$$Lambda$lambda$static$0::load$($String* name, bool initialize) {
	...
	$loadClass(ObjectTable$$Lambda$lambda$static$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ObjectTable$$Lambda$lambda$static$0);
	});
	return class$;
}
...
$Target* ObjectTable::getTarget($ObjectEndpoint* oe) {
	...
	$synchronized(ObjectTable::tableLock) {
		return $cast($Target, ObjectTable::objTable->get(oe));
	}
}
...
$Target* ObjectTable::getTarget($Remote* impl) {
	$init(ObjectTable);
	$synchronized(ObjectTable::tableLock) {
		return $cast($Target, ObjectTable::implTable->get($$new($WeakRef, impl)));
	}
}
...
void ObjectTable::putTarget($Target* target) {
	...
	if ($nc($DGCImpl::dgcLog)->isLoggable($Log::VERBOSE)) {
		$DGCImpl::dgcLog->log($Log::VERBOSE, $$str({"add object "_s, oe}));
	}
	$synchronized(ObjectTable::tableLock) {
		if (target->getImpl() != nullptr) {
			if (ObjectTable::objTable->containsKey(oe)) {
				$throwNew($ExportException, "internal error: ObjID already in use"_s);
			} else if (ObjectTable::implTable->containsKey(weakImpl)) {
				$throwNew($ExportException, "object already exported"_s);
			}
			ObjectTable::objTable->put(oe, target);
...
void ObjectTable::removeTarget($Target* target) {
	...
	$init($Log);
	if ($nc($DGCImpl::dgcLog)->isLoggable($Log::VERBOSE)) {
		$DGCImpl::dgcLog->log($Log::VERBOSE, $$str({"remove object "_s, oe}));
	}
	ObjectTable::objTable->remove(oe);
...
void ObjectTable::referenced($ObjID* id, int64_t sequenceNum, $VMID* vmid) {
	...
	$synchronized(ObjectTable::tableLock) {
		...
		$var($Target, target, $cast($Target, ObjectTable::objTable->get(oe)));
		if (target != nullptr) {
...
void ObjectTable::incrementKeepAliveCount() {
	...
	$synchronized(ObjectTable::keepAliveLock) {
		...
		if (ObjectTable::reaper == nullptr) {
			$assignStatic(ObjectTable::reaper, $cast($Thread, $AccessController::doPrivileged($$new($NewThreadAction, $$new($ObjectTable$Reaper), "Reaper"_s, false))));
...
void ObjectTable::decrementKeepAliveCount() {
	...
	$synchronized(ObjectTable::keepAliveLock) {
		...
		if (ObjectTable::keepAliveCount == 0) {
			...
			$AccessController::doPrivileged($$new($ObjectTable$1));
...
$Long* ObjectTable::lambda$static$0() {
	$init(ObjectTable);
	return $Long::getLong("sun.rmi.dgc.server.gcInterval"_s, 3600000);
}

void ObjectTable::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$beforeCallerSensitive();
	ObjectTable::gcInterval = $$sure($Long, $AccessController::doPrivileged($cast($PrivilegedAction, $$new(ObjectTable$$Lambda$lambda$static$0))))->longValue();
...
$Class* ObjectTable::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("sun.rmi.transport.ObjectTable$$Lambda$lambda$static$0")) {
	...
	$loadClass(ObjectTable, name, initialize, &classInfo$$, ObjectTable::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ObjectTable);
	});
	return class$;
}
