...
$OperationArray* DGCImpl_Skel::getOperations() {
	return $cast($OperationArray, DGCImpl_Skel::operations->clone());
}
...
void DGCImpl_Skel::clinit$($Class* clazz) {
	$useLocalObjectStack();
...
$Class* DGCImpl_Skel::load$($String* name, bool initialize) {
	...
	$loadClass(DGCImpl_Skel, name, initialize, &classInfo$$, DGCImpl_Skel::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(DGCImpl_Skel);
	});
	return class$;
}
