...
$String* TCPEndpoint$FQDN::attemptFQDN($InetAddress* localAddr) {
	...
	if ($nc(hostName)->indexOf(u'.') < 0) {
		...
		bool var$0 = (hostName == nullptr) || (hostName->isEmpty());
		if (var$0 || (hostName->indexOf(u'.') < 0)) {
...
void TCPEndpoint$FQDN::getFQDN() {
	...
	$var($Thread, t, $cast($Thread, $AccessController::doPrivileged($$new($NewThreadAction, this, "FQDN Finder"_s, true))));
	$nc(t)->start();
}
...
$Class* TCPEndpoint$FQDN::load$($String* name, bool initialize) {
	...
	$loadClass(TCPEndpoint$FQDN, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TCPEndpoint$FQDN);
	});
	return class$;
}
