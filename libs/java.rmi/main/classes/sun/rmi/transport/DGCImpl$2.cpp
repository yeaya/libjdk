...
$Class* DGCImpl$2$$Lambda$checkInput::load$($String* name, bool initialize) {
	...
	$loadClass(DGCImpl$2$$Lambda$checkInput, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DGCImpl$2$$Lambda$checkInput);
	});
	return class$;
}
...
$Object* DGCImpl$2::run() {
	...
	try {
		...
		try {
			...
			$var($UnicastServerRef, disp, $new($UnicastServerRef, ref, $$new(DGCImpl$2$$Lambda$checkInput)));
			...
			$var($Target, target, $cast($Target, $AccessController::doPrivileged($$new($DGCImpl$2$1, this, disp, stub, dgcID), acceptAcc)));
...
$Class* DGCImpl$2::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("sun.rmi.transport.DGCImpl$2$$Lambda$checkInput")) {
	...
	$loadClass(DGCImpl$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DGCImpl$2);
	});
	return class$;
}
