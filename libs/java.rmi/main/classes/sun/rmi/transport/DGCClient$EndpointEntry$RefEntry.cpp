...
void DGCClient$EndpointEntry$RefEntry::clinit$($Class* clazz) {
	$load($DGCClient);
...
$Class* DGCClient$EndpointEntry$RefEntry::load$($String* name, bool initialize) {
	...
	$loadClass(DGCClient$EndpointEntry$RefEntry, name, initialize, &classInfo$$, DGCClient$EndpointEntry$RefEntry::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(DGCClient$EndpointEntry$RefEntry);
	});
	return class$;
}
