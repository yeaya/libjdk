...
void NewThreadAction::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$beforeCallerSensitive();
	$assignStatic(NewThreadAction::systemThreadGroup, $cast($ThreadGroup, $AccessController::doPrivileged($$new($NewThreadAction$1))));
	$assignStatic(NewThreadAction::userThreadGroup, $cast($ThreadGroup, $AccessController::doPrivileged($$new($NewThreadAction$2))));
}
...
$Class* NewThreadAction::load$($String* name, bool initialize) {
	...
	$loadClass(NewThreadAction, name, initialize, &classInfo$$, NewThreadAction::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(NewThreadAction);
	});
	return class$;
}
