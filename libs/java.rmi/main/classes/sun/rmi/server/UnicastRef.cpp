...
$Class* UnicastRef$$Lambda$lambda$static$0::load$($String* name, bool initialize) {
	...
	$loadClass(UnicastRef$$Lambda$lambda$static$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(UnicastRef$$Lambda$lambda$static$0);
	});
	return class$;
}
...
$Object* UnicastRef::invoke($Remote* obj, $Method* method, $ObjectArray* params, int64_t opnum) {
	...
	if ($nc(UnicastRef::clientRefLog)->isLoggable($Log::VERBOSE)) {
		UnicastRef::clientRefLog->log($Log::VERBOSE, $$str({"method: "_s, method}));
	}
	...
	try {
		try {
			if (UnicastRef::clientRefLog->isLoggable($Log::VERBOSE)) {
				UnicastRef::clientRefLog->log($Log::VERBOSE, $$str({"opnum = "_s, $$str(opnum)}));
			}
			...
			try {
				...
				UnicastRef::clientRefLog->log($Log::BRIEF, "IOException marshalling arguments: "_s, e);
			...
			try {
				try {
					...
					alreadyFreed = true;
					UnicastRef::clientRefLog->log($Log::BRIEF, "free connection (reuse = true)"_s);
					$$nc($nc(this->ref)->getChannel())->free(conn, true);
					$assign(var$5, returnValue);
					return$4 = true;
					goto $finally1;
				} catch ($IOException& e) {
					$cast($StreamRemoteCall, call)->discardPendingRefs();
					UnicastRef::clientRefLog->log($Log::BRIEF, $$str({$(e->getClass()->getName()), " unmarshalling return: "_s}), e);
			...
			if ((call == nullptr) || (!$equals($cast($StreamRemoteCall, call)->getServerException(), e))) {
				reuse = false;
			}
			$throw(e);
		...
		if (!alreadyFreed) {
			if (UnicastRef::clientRefLog->isLoggable($Log::BRIEF)) {
				UnicastRef::clientRefLog->log($Log::BRIEF, $$str({"free connection (reuse = "_s, $$str(reuse), ")"_s}));
			}
...
$RemoteCall* UnicastRef::newCall($RemoteObject* obj, $OperationArray* ops, int32_t opnum, int64_t hash) {
	...
	$var($Connection, conn, $$nc($nc(this->ref)->getChannel())->newConnection());
	try {
		UnicastRef::clientRefLog->log($Log::VERBOSE, "create call context"_s);
		if ($nc(UnicastRef::clientCallLog)->isLoggable($Log::VERBOSE)) {
...
void UnicastRef::logClientCall(Object$* obj, Object$* method) {
	...
	var$0->append("outbound call: "_s);
	var$0->append(this->ref);
	var$0->append(" : "_s);
	var$0->append($($nc($of(obj))->getClass()->getName()));
	var$0->append($($$nc($nc(this->ref)->getObjID())->toString()));
	var$0->append(": "_s);
	var$0->append(method);
	$nc(UnicastRef::clientCallLog)->log($Log::VERBOSE, $$str(var$0));
}
...
bool UnicastRef::remoteEquals($RemoteRef* sub) {
	if ($instanceOf(UnicastRef, sub)) {
		return $nc(this->ref)->remoteEquals($cast(UnicastRef, sub)->ref);
	}
	return false;
}
...
void UnicastRef::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$beforeCallerSensitive();
	$init($Util);
	$assignStatic(UnicastRef::clientRefLog, $Log::getLog("sun.rmi.client.ref"_s, "transport"_s, $Util::logLevel));
	$assignStatic(UnicastRef::clientCallLog, $Log::getLog("sun.rmi.client.call"_s, "RMI"_s, $$sure($Boolean, $AccessController::doPrivileged($cast($PrivilegedAction, $$new(UnicastRef$$Lambda$lambda$static$0))))->booleanValue()));
}
...
$Class* UnicastRef::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("sun.rmi.server.UnicastRef$$Lambda$lambda$static$0")) {
	...
	$loadClass(UnicastRef, name, initialize, &classInfo$$, UnicastRef::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(UnicastRef);
	});
	return class$;
}
