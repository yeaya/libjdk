...
$Registry* LocateRegistry::getRegistry($String* host$renamed, int32_t port, $RMIClientSocketFactory* csf) {
	...
	$var($LiveRef, liveRef, $new($LiveRef, var$0, $$new($TCPEndpoint, host, port, csf, nullptr), false));
	$var($RemoteRef, ref, (csf == nullptr) ? $cast($RemoteRef, $new($UnicastRef, liveRef)) : $cast($RemoteRef, $new($UnicastRef2, liveRef)));
	$load($RegistryImpl);
...
$Class* LocateRegistry::load$($String* name, bool initialize) {
	...
	$loadClass(LocateRegistry, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LocateRegistry);
	});
	return class$;
}
