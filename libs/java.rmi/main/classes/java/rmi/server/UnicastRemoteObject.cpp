...
void UnicastRemoteObject::init$(int32_t port) {
	...
	this->port = port;
	exportObject($cast($Remote, this), port);
}
...
void UnicastRemoteObject::init$(int32_t port, $RMIClientSocketFactory* csf, $RMIServerSocketFactory* ssf) {
	...
	$set(this, csf, csf);
	$set(this, ssf, ssf);
	exportObject($cast($Remote, this), port, csf, ssf);
}
...
void UnicastRemoteObject::reexport() {
	if (this->csf == nullptr && this->ssf == nullptr) {
		exportObject($cast($Remote, this), this->port);
	} else {
		exportObject($cast($Remote, this), this->port, this->csf, this->ssf);
	}
}
...
$Remote* UnicastRemoteObject::exportObject($Remote* obj, $UnicastServerRef* sref) {
	...
	if ($instanceOf(UnicastRemoteObject, obj)) {
		$set($cast(UnicastRemoteObject, obj), ref, $cast($UnicastRef, sref));
	}
...
$Class* UnicastRemoteObject::load$($String* name, bool initialize) {
	...
	$CompoundAttribute exportObjectmethodAnnotations$$[] = {
	...
	$MethodInfo methodInfos$$[] = {
		...
		{"exportObject", "(Ljava/rmi/Remote;)Ljava/rmi/server/RemoteStub;", nullptr, $PUBLIC | $STATIC | $DEPRECATED, $staticMethod(UnicastRemoteObject, exportObject, $RemoteStub*, $Remote*), "java.rmi.RemoteException", nullptr, exportObjectmethodAnnotations$$},
	...
	$loadClass(UnicastRemoteObject, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
