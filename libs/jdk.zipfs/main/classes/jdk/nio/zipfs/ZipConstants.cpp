...
int32_t ZipConstants::SH($bytes* b, int32_t n) {
	...
	return var$0 | ($Byte::toUnsignedInt(b->get(n + 1)) << 8);
}

int64_t ZipConstants::LG($bytes* b, int32_t n) {
	$init(ZipConstants);
	int32_t var$0 = SH(b, n);
	return (var$0 | (SH(b, n + 2) << 16)) & (int64_t)0xffffffff;
}
...
bool ZipConstants::pkSigAt($bytes* b, int32_t n, int32_t b1, int32_t b2) {
	$init(ZipConstants);
	return ((($nc(b)->get(n) == u'P') & ($nc(b)->get(n + 1) == u'K')) & ($nc(b)->get(n + 2) == b1)) & ($nc(b)->get(n + 3) == b2);
}
...
void ZipConstants::clinit$($Class* clazz) {
...
$Class* ZipConstants::load$($String* name, bool initialize) {
	...
	$loadClass(ZipConstants, name, initialize, &classInfo$$, ZipConstants::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ZipConstants);
	});
	return class$;
}
