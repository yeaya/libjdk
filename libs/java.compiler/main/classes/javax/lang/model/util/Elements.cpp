...
$RecordComponentElement* Elements::recordComponentFor($ExecutableElement* accessor) {
	...
	if ($$nc($nc(accessor)->getEnclosingElement())->getKind() == $ElementKind::RECORD) {
		$var($Iterator, i$, $$nc($ElementFilter::recordComponentsIn($($$nc(accessor->getEnclosingElement())->getEnclosedElements())))->iterator());
		for (; $nc(i$)->hasNext();) {
...
$Class* Elements::load$($String* name, bool initialize) {
	...
	$loadClass(Elements, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
