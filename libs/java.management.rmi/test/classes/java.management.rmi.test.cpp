...
void java$management$rmi$test$PreloadClass() {
}

void java$management$rmi$test$PreinitClass() {
}

void java$management$rmi$test$LibEventAction(int32_t eventType, void* eventData) {
	if (eventType == JCPP_LIB_EVENT_TYPE_PRELOAD_CLASS) {
		java$management$rmi$test$PreloadClass();
	} else if (eventType == JCPP_LIB_EVENT_TYPE_PREINIT_CLASS) {
		java$management$rmi$test$PreinitClass();
	}
}
