...
$RMIConnection* RMIServerImpl::doNewClient(Object$* credentials) {
	$useLocalObjectStack();
	bool tracing = RMIServerImpl::logger->traceOn();
	if (tracing) {
		RMIServerImpl::logger->trace("newClient"_s, "making new client"_s);
	}
	...
	if (authenticator == nullptr) {
		bool var$0 = this->env->get("jmx.remote.x.password.file"_s) != nullptr;
		if (var$0 || this->env->get("jmx.remote.x.login.config"_s) != nullptr) {
			$assign(authenticator, $new($JMXPluggableAuthenticator, this->env));
		}
	}
	if (authenticator != nullptr) {
		if (tracing) {
			RMIServerImpl::logger->trace("newClient"_s, $$str({"got authenticator: "_s, $(authenticator->getClass()->getName())}));
		}
		try {
			$assign(subject, authenticator->authenticate(credentials));
		} catch ($SecurityException& e) {
			RMIServerImpl::logger->trace("newClient"_s, $$str({"Authentication failed: "_s, e}));
			$throw(e);
		}
	}
	if (tracing) {
		if (subject != nullptr) {
			RMIServerImpl::logger->trace("newClient"_s, "subject is not null"_s);
		} else {
			RMIServerImpl::logger->trace("newClient"_s, "no subject"_s);
		}
	}
	$var($String, connectionId, makeConnectionId($(getProtocol()), subject));
	if (tracing) {
		RMIServerImpl::logger->trace("newClient"_s, $$str({"making new connection: "_s, connectionId}));
	}
	...
	$synchronized(this->clientList) {
		if (!this->clientList->contains(wr)) {
			$throwNew($IOException, "The connection is refused."_s);
		}
	}
	if (tracing) {
		RMIServerImpl::logger->trace("newClient"_s, $$str({"new connection done: "_s, connectionId}));
	}
	return client;
}
...
void RMIServerImpl::clientClosed($RMIConnection* client) {
	$useLocalObjectStack();
	bool debug = RMIServerImpl::logger->debugOn();
	if (debug) {
		RMIServerImpl::logger->trace("clientClosed"_s, $$str({"client="_s, client}));
	}
	if (client == nullptr) {
	...
	$synchronized(this->clientList) {
		dropDeadReferences();
		{
			$var($Iterator, it, this->clientList->iterator());
			for (; $nc(it)->hasNext();) {
	...
	if (debug) {
		RMIServerImpl::logger->trace("clientClosed"_s, "closing client."_s);
	}
	closeClient(client);
	if (debug) {
		RMIServerImpl::logger->trace("clientClosed"_s, "sending notif"_s);
	}
	$nc(this->connServer)->connectionClosed($($nc(client)->getConnectionId()), "Client connection closed"_s, nullptr);
	if (debug) {
		RMIServerImpl::logger->trace("clientClosed"_s, "done"_s);
	}
}
...
void RMIServerImpl::close() {
	$synchronized(this) {
		...
		bool debug = RMIServerImpl::logger->debugOn();
		if (tracing) {
			RMIServerImpl::logger->trace("close"_s, "closing"_s);
		}
		$var($IOException, ioException, nullptr);
		try {
			if (debug) {
				RMIServerImpl::logger->debug("close"_s, "closing Server"_s);
			}
			closeServer();
		} catch ($IOException& e) {
			if (tracing) {
				RMIServerImpl::logger->trace("close"_s, $$str({"Failed to close server: "_s, e}));
			}
			if (debug) {
				RMIServerImpl::logger->debug("close"_s, e);
			}
			$assign(ioException, e);
		}
		if (debug) {
			RMIServerImpl::logger->debug("close"_s, "closing Clients"_s);
		}
		while (true) {
			$synchronized(this->clientList) {
				if (debug) {
					RMIServerImpl::logger->debug("close"_s, "droping dead references"_s);
				}
				dropDeadReferences();
				if (debug) {
					RMIServerImpl::logger->debug("close"_s, $$str({"client count: "_s, $$str(this->clientList->size())}));
				}
					...
					for (; $nc(it)->hasNext();) {
						...
						if (client != nullptr) {
							try {
								...
								if (tracing) {
									RMIServerImpl::logger->trace("close"_s, $$str({"Failed to close client: "_s, e}));
								}
								if (debug) {
									RMIServerImpl::logger->debug("close"_s, e);
								}
		...
		if (ioException != nullptr) {
			if (tracing) {
				RMIServerImpl::logger->trace("close"_s, "close failed."_s);
			}
			$throw(ioException);
		}
		if (tracing) {
			RMIServerImpl::logger->trace("close"_s, "closed."_s);
		}
	}
}
...
$String* RMIServerImpl::makeConnectionId($String* protocol, $Subject* subject) {
	...
	$synchronized(class$) {
		...
		try {
			...
			RMIServerImpl::logger->trace("makeConnectionId"_s, "getClientHost"_s, e);
		}
		...
		if (RMIServerImpl::logger->traceOn()) {
			RMIServerImpl::logger->trace("newConnectionId"_s, $$str({"connectionId="_s, buf}));
		}
		return buf->toString();
	}
}
...
void RMIServerImpl::clinit$($Class* clazz) {
...
$Class* RMIServerImpl::load$($String* name, bool initialize) {
	...
	$loadClass(RMIServerImpl, name, initialize, &classInfo$$, RMIServerImpl::clinit$, []($Class* clazz) -> $Object* {
