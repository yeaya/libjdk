...
void XmlReaderContentHandler::initMaps() {
	...
	for (i = 0; i < items; ++i) {
		this->propMap->put(this->properties->get(i), $($Integer::valueOf(i)));
	}
	$set(this, colDefMap, $new($HashMap));
	items = $nc(this->colDef)->length;
	for (i = 0; i < items; ++i) {
		this->colDefMap->put(this->colDef->get(i), $($Integer::valueOf(i)));
	}
	$set(this, dataMap, $new($HashMap));
	items = $nc(this->data)->length;
	for (i = 0; i < items; ++i) {
		this->dataMap->put(this->data->get(i), $($Integer::valueOf(i)));
	}
...
void XmlReaderContentHandler::startElement($String* uri, $String* lName, $String* qName, $Attributes* attributes) {
	...
	case XmlReaderContentHandler::PROPERTIES:
		$set(this, tempCommand, ""_s);
		tag = $$sure($Integer, $nc(this->propMap)->get(name))->intValue();
	...
	case XmlReaderContentHandler::METADATA:
		tag = $$sure($Integer, $nc(this->colDefMap)->get(name))->intValue();
	...
	case XmlReaderContentHandler::DATA:
		...
		} else if ($$sure($Integer, this->dataMap->get(name))->intValue() == XmlReaderContentHandler::EmptyStringTag) {
			tag = XmlReaderContentHandler::EmptyStringTag;
		} else {
			tag = $$sure($Integer, this->dataMap->get(name))->intValue();
		}
		...
		try {
			...
			case XmlReaderContentHandler::KeycolsTag:
				if (this->keyCols != nullptr) {
					$var($ints, i, $new($ints, this->keyCols->size()));
					for (int32_t j = 0; j < i->length; ++j) {
						i->set(j, $Integer::parseInt($$cast($String, this->keyCols->elementAt(j))));
					}
			...
			case XmlReaderContentHandler::PropClassTag:
				try {
					...
					$var($String, var$0, $$nc($nc(this->resBundle)->handleGetObject("xmlrch.errmap"_s))->toString());
					$throwNew($SAXException, $($MessageFormat::format(var$0, $$new($ObjectArray, {$(ex->getMessage())}))));
				}
				break;
	...
	case XmlReaderContentHandler::METADATA:
		if ($nc(name)->equals("metadata"_s)) {
			try {
				...
				$var($String, var$1, $$nc($nc(this->resBundle)->handleGetObject("xmlrch.errmetadata"_s))->toString());
				$throwNew($SAXException, $($MessageFormat::format(var$1, $$new($ObjectArray, {$(ex->getMessage())}))));
			}
		} else {
			try {
				if (getNullValue()) {
				...
				$var($String, var$2, $$nc($nc(this->resBundle)->handleGetObject("xmlrch.errmetadata"_s))->toString());
				$throwNew($SAXException, $($MessageFormat::format(var$2, $$new($ObjectArray, {$(ex->getMessage())}))));
			}
			$set(this, metaDataValue, ""_s);
		}
		setTag(-1);
	...
	case XmlReaderContentHandler::DATA:
		...
		case XmlReaderContentHandler::ColTag:
			try {
				...
				$var($String, var$3, $$nc($nc(this->resBundle)->handleGetObject("xmlrch.errinsertval"_s))->toString());
				$throwNew($SAXException, $($MessageFormat::format(var$3, $$new($ObjectArray, {$(ex->getMessage())}))));
			}
			break;
		case XmlReaderContentHandler::RowTag:
			try {
				...
				$var($String, var$4, $$nc($nc(this->resBundle)->handleGetObject("xmlrch.errconstr"_s))->toString());
				$throwNew($SAXException, $($MessageFormat::format(var$4, $$new($ObjectArray, {$(ex->getMessage())}))));
			}
			break;
		case XmlReaderContentHandler::DelTag:
			try {
				...
				$var($String, var$5, $$nc($nc(this->resBundle)->handleGetObject("xmlrch.errdel"_s))->toString());
				$throwNew($SAXException, $($MessageFormat::format(var$5, $$new($ObjectArray, {$(ex->getMessage())}))));
			}
			break;
		case XmlReaderContentHandler::InsTag:
			try {
				...
				$var($String, var$6, $$nc($nc(this->resBundle)->handleGetObject("xmlrch.errinsert"_s))->toString());
				$throwNew($SAXException, $($MessageFormat::format(var$6, $$new($ObjectArray, {$(ex->getMessage())}))));
			}
			break;
		case XmlReaderContentHandler::InsDelTag:
			try {
				...
				$var($String, var$7, $$nc($nc(this->resBundle)->handleGetObject("xmlrch.errinsdel"_s))->toString());
				$throwNew($SAXException, $($MessageFormat::format(var$7, $$new($ObjectArray, {$(ex->getMessage())}))));
			}
			break;
		case XmlReaderContentHandler::UpdTag:
			try {
				...
				$var($String, var$8, $$nc($nc(this->resBundle)->handleGetObject("xmlrch.errupdate"_s))->toString());
				$throwNew($SAXException, $($MessageFormat::format(var$8, $$new($ObjectArray, {$(ex->getMessage())}))));
			}
			break;
		default:
			break;
...
void XmlReaderContentHandler::applyUpdates() {
	...
	if ($nc(this->updates)->size() > 0) {
		try {
			...
			$var($Iterator, i, this->updates->iterator());
			while ($nc(i)->hasNext()) {
			...
			$var($String, var$0, $$nc($nc(this->resBundle)->handleGetObject("xmlrch.errupdrow"_s))->toString());
			$throwNew($SAXException, $($MessageFormat::format(var$0, $$new($ObjectArray, {$(ex->getMessage())}))));
		}
...
void XmlReaderContentHandler::characters($chars* ch, int32_t start, int32_t length) {
	...
	try {
		...
		var$0->append($($$nc($nc(this->resBundle)->handleGetObject("xmlrch.chars"_s))->toString()));
...
void XmlReaderContentHandler::setPropertyValue($String* s) {
	...
	case XmlReaderContentHandler::ConcurrencyTag:
		if (nullValue) {
			$throwNew($SQLException, $($$nc($nc(this->resBundle)->handleGetObject("xmlrch.badvalue"_s))->toString()));
		} else {
	...
	case XmlReaderContentHandler::SyncProviderNameTag:
		if (nullValue) {
			...
			$var($String, str, $nc(s)->substring(0, $nc(s)->indexOf(u'@') + 1));
...
void XmlReaderContentHandler::setMetaDataValue($String* s) {
	...
	case XmlReaderContentHandler::ColumnCountTag:
		...
		this->idx = 0;
		if (nullValue) {
			$throwNew($SQLException, $($$nc($nc(this->resBundle)->handleGetObject("xmlrch.badvalue1"_s))->toString()));
		} else {
			this->md->setColumnCount(getIntegerValue(s));
		}
		break;
...
void XmlReaderContentHandler::setDataValue($chars* ch, int32_t start, int32_t len) {
	...
	case XmlReaderContentHandler::UpdTag:
		...
		this->upd->set(0, $($Integer::valueOf(this->idx)));
		this->upd->set(1, this->tempUpdate);
		$set(this, lastval, $cast($String, this->upd->get(1)));
		break;
	case XmlReaderContentHandler::InsTag:
		break;
	}
...
void XmlReaderContentHandler::insertValue($String* s) {
	...
	case $Types::NUMERIC:
	case $Types::DECIMAL:
		$nc(this->rs)->updateObject(this->idx, $(getBigDecimalValue(s)));
		break;
	case $Types::BINARY:
...
void XmlReaderContentHandler::warning($SAXParseException* err) {
	$useLocalObjectStack();
	$var($String, var$0, $$nc($nc(this->resBundle)->handleGetObject("xmlrch.warning"_s))->toString());
	$nc($System::out)->println($($MessageFormat::format(var$0, $$new($ObjectArray, {
		$($nc(err)->getMessage()),
		$($Integer::valueOf($nc(err)->getLineNumber())),
		$($nc(err)->getSystemId())
	}))));
}
...
$Class* XmlReaderContentHandler::load$($String* name, bool initialize) {
	...
	$loadClass(XmlReaderContentHandler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
