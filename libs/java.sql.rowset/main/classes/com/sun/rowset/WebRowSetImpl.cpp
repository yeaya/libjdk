...
void WebRowSetImpl::init$($Hashtable* env) {
	...
	if (env == nullptr) {
		$throwNew($SQLException, $($$nc($nc(this->resBundle)->handleGetObject("webrowsetimpl.nullhash"_s))->toString()));
	}
	$init($SyncFactory);
...
void WebRowSetImpl::writeXml($Writer* writer) {
	...
	if (this->xmlWriter != nullptr) {
		this->curPosBfrWrite = this->getRow();
		$nc(this->xmlWriter)->writeXML(this, writer);
	} else {
		$throwNew($SQLException, $($$nc($nc(this->resBundle)->handleGetObject("webrowsetimpl.invalidwr"_s))->toString()));
	}
}
...
void WebRowSetImpl::readXml($Reader* reader) {
	...
	try {
		if (reader != nullptr) {
			$nc(this->xmlReader)->readXML(this, reader);
			...
			$throwNew($SQLException, $($$nc($nc(this->resBundle)->handleGetObject("webrowsetimpl.invalidrd"_s))->toString()));
		}
	} catch ($Exception& e) {
...
void WebRowSetImpl::readXml($InputStream* iStream) {
	$useLocalObjectStack();
	if (iStream != nullptr) {
		$nc(this->xmlReader)->readXML(this, iStream);
		if (this->curPosBfrWrite == 0) {
			this->beforeFirst();
		} else {
			this->absolute(this->curPosBfrWrite);
...
void WebRowSetImpl::writeXml($OutputStream* oStream) {
	...
	if (this->xmlWriter != nullptr) {
		this->curPosBfrWrite = this->getRow();
		$nc(this->xmlWriter)->writeXML(this, oStream);
	} else {
		$throwNew($SQLException, $($$nc($nc(this->resBundle)->handleGetObject("webrowsetimpl.invalidwr"_s))->toString()));
	}
}
...
$Class* WebRowSetImpl::load$($String* name, bool initialize) {
	...
	$loadClass(WebRowSetImpl, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
