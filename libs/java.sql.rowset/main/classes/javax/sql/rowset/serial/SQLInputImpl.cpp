...
$Object* SQLInputImpl::getNextAttribute() {
	if (++this->idx >= $nc(this->attrib)->length) {
		...
		this->lastValueWasNull = this->attrib->get(this->idx) == nullptr;
		return this->attrib->get(this->idx);
	}
}
...
$Object* SQLInputImpl::readObject() {
	...
	if ($instanceOf($Struct, attrib)) {
		...
		$Class* c = $cast($Class, $nc(this->map)->get($(s->getSQLTypeName())));
		if (c != nullptr) {
			...
			$var($ObjectArray, attribs, s->getAttributes(this->map));
...
$Class* SQLInputImpl::load$($String* name, bool initialize) {
	...
	$loadClass(SQLInputImpl, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SQLInputImpl);
	});
	return class$;
}
