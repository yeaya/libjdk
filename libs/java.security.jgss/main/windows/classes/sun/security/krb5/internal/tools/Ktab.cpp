...
void Ktab::main($StringArray* args) {
	...
	if (var$0) {
		...
		bool var$2 = var$3 || ($nc(args->get(0))->equalsIgnoreCase("-h"_s));
		bool var$1 = var$2 || ($nc(args->get(0))->equalsIgnoreCase("--help"_s));
...
void Ktab::addEntry() {
	...
	try {
		...
		$System::out->println($$str({"Service key for "_s, this->principal, " is saved in "_s, $($nc(this->table)->tabName())}));
	} catch ($KrbException& e) {
		$nc($System::err)->println($$str({"Failed to add "_s, this->principal, " to keytab."_s}));
		e->printStackTrace();
		$System::exit(-1);
...
void Ktab::listKt() {
	...
	if ((entries != nullptr) && (entries->length > 0)) {
		...
		for (int32_t j = 0; j < column; ++j) {
			$System::out->printf($$str({"%"_s, $$str(width->get(j)), "s "_s}), $$new($ObjectArray, {$nc(output->get(0))->get(j)}));
		}
		$System::out->println();
		for (int32_t j = 0; j < column; ++j) {
		...
		for (int32_t i = 0; i < entries->length; ++i) {
			for (int32_t j = 0; j < column; ++j) {
				$System::out->printf($$str({"%"_s, $$str(width->get(j)), "s "_s}), $$new($ObjectArray, {$nc(output->get(i + 1))->get(j)}));
			}
			$System::out->println();
		}
	} else {
		$System::out->println("0 entry."_s);
	}
}
...
void Ktab::deleteEntry() {
	...
	try {
		...
		if (!this->forced) {
			...
			var$0->append("Are you sure you want to delete service key(s) for "_s);
			var$0->append($(pname->toString()));
			var$0->append(" ("_s);
			var$0->append(this->etype == -1 ? "all etypes"_s : ($$str({"etype="_s, $$str(this->etype)})));
			var$0->append(", "_s);
			var$0->append(this->vDel == -1 ? "all kvno"_s : (this->vDel == -2 ? "old kvno"_s : ($$str({"kvno="_s, $$str(this->vDel)}))));
			var$0->append(") in "_s);
			var$0->append($($nc(this->table)->tabName()));
			var$0->append("? (Y/[N]): "_s);
			$nc($System::out)->print($$str(var$0));
			$System::out->flush();
			$assign(answer, cis->readLine());
			bool var$1 = $nc(answer)->equalsIgnoreCase("Y"_s);
			if (var$1 || answer->equalsIgnoreCase("Yes"_s)) {
				;
			} else {
				$System::exit(0);
			}
...
void Ktab::printHelp() {
	...
	$System::out->println();
	$System::out->println("Available commands:"_s);
	$System::out->println();
	...
	$System::out->println();
	$System::out->println("Common option(s):"_s);
	$System::out->println();
...
$Class* Ktab::load$($String* name, bool initialize) {
	...
	$loadClass(Ktab, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Ktab);
	});
	return class$;
}
