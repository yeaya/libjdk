...
void AcceptSecContextToken::init$($Krb5Context* context, $KrbApReq* apReq) {
	...
	$nc(context)->resetMySequenceNumber($$nc(this->apRep->getSeqNumber())->intValue());
}
...
void AcceptSecContextToken::init$($Krb5Context* context, $Credentials* serviceCreds, $KrbApReq* apReq, $InputStream* is) {
	...
	int32_t peerSeqNumber = (apRepSeqNumber != nullptr ? apRepSeqNumber->intValue() : 0);
...
$Class* AcceptSecContextToken::load$($String* name, bool initialize) {
	...
	$loadClass(AcceptSecContextToken, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AcceptSecContextToken);
	});
	return class$;
}
