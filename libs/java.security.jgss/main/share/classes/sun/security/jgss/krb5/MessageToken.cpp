...
void MessageToken::init$(int32_t tokenId, $Krb5Context* context, $InputStream* is, $MessageProp* prop) {
	...
	try {
		...
		$init($Krb5Token);
		if (!$$nc(this->gssHeader->getOid())->equals($Krb5Token::OID)) {
...
void MessageToken::genSignAndSeqNumber($MessageProp* prop, $bytes* optionalHeader, $bytes* data, int32_t offset, int32_t len, $bytes* optionalTrailer) {
	...
	if (!this->initiator) {
		...
		this->seqNumberData->set(5, (int8_t)255);
		this->seqNumberData->set(6, (int8_t)255);
		this->seqNumberData->set(7, (int8_t)255);
	}
...
bool MessageToken::verifySignAndSeqNumber($bytes* optionalHeader, $bytes* data, int32_t offset, int32_t len, $bytes* optionalTrailer) {
	...
	if ($MessageDigest::isEqual(this->checksum, myChecksum)) {
		...
		int8_t directionByte = 0;
		if (this->initiator) {
			directionByte = (int8_t)255;
		}
		if (($nc(this->seqNumberData)->get(4) == directionByte) && (this->seqNumberData->get(5) == directionByte) && (this->seqNumberData->get(6) == directionByte) && (this->seqNumberData->get(7) == directionByte)) {
			return true;
		}
	}
	return false;
...
$Class* MessageToken::load$($String* name, bool initialize) {
	...
	$loadClass(MessageToken, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MessageToken);
	});
	return class$;
}
