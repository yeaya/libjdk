...
void MessageToken_v2$MessageTokenHeader::init$($MessageToken_v2* this$0, int32_t tokenId, bool conf) {
	...
	this->bytes->set(0, (int8_t)((int32_t)((uint32_t)tokenId >> 8)));
	this->bytes->set(1, (int8_t)(tokenId));
	int32_t flags = 0;
	flags = ((this$0->initiator ? 0 : 1) | ((conf && tokenId != 1028) ? 2 : 0)) | (this$0->have_acceptor_subkey ? 4 : 0);
	this->bytes->set(2, (int8_t)flags);
	this->bytes->set(3, (int8_t)255);
	if (tokenId == 1284) {
		this->bytes->set(4, (int8_t)0);
		this->bytes->set(5, (int8_t)0);
		this->bytes->set(6, (int8_t)0);
		this->bytes->set(7, (int8_t)0);
	} else if (tokenId == 1028) {
		for (int32_t i = 4; i < 8; ++i) {
			this->bytes->set(i, (int8_t)255);
		}
	}
...
void MessageToken_v2$MessageTokenHeader::init$($MessageToken_v2* this$0, $InputStream* is, $MessageProp* prop, int32_t tokId) {
	...
	int32_t flag = $nc(this->bytes)->get(2) & 1;
	...
	int32_t conf_flag = this->bytes->get(2) & 2;
	...
	if (this->tokenId == 1284) {
		if (($nc(this->bytes)->get(3) & 0xff) != 0xff) {
		...
		for (int32_t i = 3; i < 8; ++i) {
			if (($nc(this->bytes)->get(i) & 0xff) != 0xff) {
				$throwNew($GSSException, $GSSException::DEFECTIVE_TOKEN, -1, $$str({$($Krb5Token::getTokenName(this->tokenId)), ":Defective Token Filler!"_s}));
			}
		}
	}
...
$Class* MessageToken_v2$MessageTokenHeader::load$($String* name, bool initialize) {
	...
	$loadClass(MessageToken_v2$MessageTokenHeader, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MessageToken_v2$MessageTokenHeader);
	});
	return class$;
}
