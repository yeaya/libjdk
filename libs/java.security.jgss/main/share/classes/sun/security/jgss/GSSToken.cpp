...
int32_t GSSToken::readInt($InputStream* is) {
	int32_t var$0 = (0xff & $nc(is)->read()) << 8;
	return (var$0 | (0xff & is->read()));
}

int32_t GSSToken::readInt($bytes* src, int32_t pos) {
	return (((0xff & $nc(src)->get(pos)) << 8) | (0xff & $nc(src)->get(pos + 1)));
}
...
$Class* GSSToken::load$($String* name, bool initialize) {
	...
	$loadClass(GSSToken, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(GSSToken);
	});
	return class$;
}
