...
$Object* Krb5AcceptCredential$1::run() {
	$init($GSSCaller);
	return $Krb5Util::getServiceCreds(this->val$caller == $GSSCaller::CALLER_UNKNOWN ? $GSSCaller::CALLER_ACCEPT : this->val$caller, this->val$serverPrinc, this->val$acc);
}
...
$Class* Krb5AcceptCredential$1::load$($String* name, bool initialize) {
	...
	$loadClass(Krb5AcceptCredential$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Krb5AcceptCredential$1);
	});
	return class$;
}
