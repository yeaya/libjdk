...
$bytes* AesSha2DkCrypto::encryptCTS($bytes* baseKey, int32_t usage, $bytes* ivec, $bytes* new_ivec, $bytes* plaintext, int32_t start, int32_t len, bool confounder_exists) {
	...
	try {
		...
		;
		$var($bytes, msg, $Arrays::copyOf(ivec, ivec->length + toBeEncrypted->length));
...
$bytes* AesSha2DkCrypto::decryptCTS($bytes* baseKey, int32_t usage, $bytes* ivec, $bytes* ciphertext, int32_t start, int32_t len, bool confounder_exists) {
	...
	try {
		...
		$assign(Ki, deriveKey(baseKey, usage, (int8_t)85));
		;
		$var($bytes, msg, $Arrays::copyOf(ivec, ivec->length + len - this->hashSize));
		$System::arraycopy(ciphertext, start, msg, ivec->length, len - this->hashSize);
...
void AesSha2DkCrypto::clinit$($Class* clazz) {
...
$Class* AesSha2DkCrypto::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(I)V", nullptr, $PUBLIC, $method(AesSha2DkCrypto, init$, void, int32_t)},
		{"PBKDF2", "([C[BII)[B", nullptr, $PRIVATE | $STATIC, $staticMethod(AesSha2DkCrypto, PBKDF2, $bytes*, $chars*, $bytes*, int32_t, int32_t), "java.security.GeneralSecurityException"},
		{"calculateChecksum", "([BI[BII)[B", nullptr, $PUBLIC, $virtualMethod(AesSha2DkCrypto, calculateChecksum, $bytes*, $bytes*, int32_t, $bytes*, int32_t, int32_t), "java.security.GeneralSecurityException"},
		{"decrypt", "([BI[B[BII)[B", nullptr, $PUBLIC, $virtualMethod(AesSha2DkCrypto, decrypt, $bytes*, $bytes*, int32_t, $bytes*, $bytes*, int32_t, int32_t), "java.security.GeneralSecurityException"},
		{"decryptCTS", "([BI[B[BIIZ)[B", nullptr, $PRIVATE, $method(AesSha2DkCrypto, decryptCTS, $bytes*, $bytes*, int32_t, $bytes*, $bytes*, int32_t, int32_t, bool), "java.security.GeneralSecurityException"},
		{"decryptRaw", "([BI[B[BII)[B", nullptr, $PUBLIC, $virtualMethod(AesSha2DkCrypto, decryptRaw, $bytes*, $bytes*, int32_t, $bytes*, $bytes*, int32_t, int32_t), "java.security.GeneralSecurityException"},
		{"deriveKey", "([BIB)[B", nullptr, $PRIVATE, $method(AesSha2DkCrypto, deriveKey, $bytes*, $bytes*, int32_t, int8_t), "java.security.GeneralSecurityException"},
		{"dr", "([B[B)[B", nullptr, $PROTECTED, $virtualMethod(AesSha2DkCrypto, dr, $bytes*, $bytes*, $bytes*), "java.security.GeneralSecurityException"},
		{"encrypt", "([BI[B[B[BII)[B", nullptr, $PUBLIC, $virtualMethod(AesSha2DkCrypto, encrypt, $bytes*, $bytes*, int32_t, $bytes*, $bytes*, $bytes*, int32_t, int32_t), "java.security.GeneralSecurityException,sun.security.krb5.KrbCryptoException"},
		{"encryptCTS", "([BI[B[B[BIIZ)[B", nullptr, $PRIVATE, $method(AesSha2DkCrypto, encryptCTS, $bytes*, $bytes*, int32_t, $bytes*, $bytes*, $bytes*, int32_t, int32_t, bool), "java.security.GeneralSecurityException,sun.security.krb5.KrbCryptoException"},
		{"encryptRaw", "([BI[B[BII)[B", nullptr, $PUBLIC, $virtualMethod(AesSha2DkCrypto, encryptRaw, $bytes*, $bytes*, int32_t, $bytes*, $bytes*, int32_t, int32_t), "java.security.GeneralSecurityException,sun.security.krb5.KrbCryptoException"},
		{"getChecksumLength", "()I", nullptr, $PUBLIC, $virtualMethod(AesSha2DkCrypto, getChecksumLength, int32_t)},
		{"getCipher", "([B[BI)Ljavax/crypto/Cipher;", nullptr, $PROTECTED, $virtualMethod(AesSha2DkCrypto, getCipher, $Cipher*, $bytes*, $bytes*, int32_t), "java.security.GeneralSecurityException"},
		{"getHmac", "([B[B)[B", nullptr, $PROTECTED, $virtualMethod(AesSha2DkCrypto, getHmac, $bytes*, $bytes*, $bytes*), "java.security.GeneralSecurityException"},
		{"getKeySeedLength", "()I", nullptr, $PROTECTED, $virtualMethod(AesSha2DkCrypto, getKeySeedLength, int32_t)},
		{"randomToKey", "([B)[B", nullptr, $PROTECTED, $virtualMethod(AesSha2DkCrypto, randomToKey, $bytes*, $bytes*)},
		{"readBigEndian", "([BII)I", nullptr, $PUBLIC | $STATIC | $FINAL, $staticMethod(AesSha2DkCrypto, readBigEndian, int32_t, $bytes*, int32_t, int32_t)},
		{"stringToKey", "([CLjava/lang/String;[B)[B", nullptr, $PUBLIC, $virtualMethod(AesSha2DkCrypto, stringToKey, $bytes*, $chars*, $String*, $bytes*), "java.security.GeneralSecurityException"},
		{"stringToKey", "([C[B[B)[B", nullptr, $PRIVATE, $method(AesSha2DkCrypto, stringToKey, $bytes*, $chars*, $bytes*, $bytes*), "java.security.GeneralSecurityException"},
		{}
	};
	$ClassInfo classInfo$$ = {
		$PUBLIC | $ACC_SUPER,
	...
	$loadClass(AesSha2DkCrypto, name, initialize, &classInfo$$, AesSha2DkCrypto::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(AesSha2DkCrypto);
	});
	return class$;
}
