...
void Authenticator::init($DerValue* encoding) {
	...
	bool var$1 = ($nc(encoding)->getTag() & (int8_t)31) != (int8_t)2;
	...
	this->authenticator_vno = $$nc($$nc(subDer->getData())->getBigInteger())->intValue();
	...
	$assign(subDer, $$nc(der->getData())->getDerValue());
	if (($nc(subDer)->getTag() & (int8_t)31) == 4) {
	...
	if ($$nc(der->getData())->available() > 0) {
		if (($$nc(der->getData())->peekByte() & 0x1f) == 7) {
			$assign(subDer, $$nc(der->getData())->getDerValue());
			if (($nc(subDer)->getTag() & (int8_t)31) == (int8_t)7) {
...
$bytes* Authenticator::asn1Encode() {
	...
	if (this->cksum != nullptr) {
		...
		v->addElement($$new($DerValue, var$3, $(this->cksum->asn1Encode())));
	}
	...
	if (this->subKey != nullptr) {
		...
		v->addElement($$new($DerValue, var$6, $(this->subKey->asn1Encode())));
	}
	...
	if (this->authorizationData != nullptr) {
		...
		v->addElement($$new($DerValue, var$8, $(this->authorizationData->asn1Encode())));
	}
