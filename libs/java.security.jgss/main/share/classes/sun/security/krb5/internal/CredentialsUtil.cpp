...
$Credentials* CredentialsUtil::acquireS4U2selfCreds($PrincipalName* client, $Credentials* ccreds) {
	...
	$var($PrincipalName, sname, ccreds->getClient());
	...
	$var($PrincipalName, var$3, ccreds->getClient());
	$init($CredentialsUtil$S4U2Type);
	$var($Credentials, creds, serviceCreds(var$2, ccreds, var$3, sname, nullptr, $$new($PADataArray, {
...
$Credentials* CredentialsUtil::acquireS4U2proxyCreds($String* backend, $Ticket* second, $PrincipalName* client, $Credentials* ccreds) {
	...
	$var($PrincipalName, var$3, ccreds->getClient());
	$var($TicketArray, var$4, $new($TicketArray, {second}));
	$init($CredentialsUtil$S4U2Type);
	$var($Credentials, creds, serviceCreds(var$2, ccreds, var$3, backendPrincipal, var$4, $$new($PADataArray, {$$new($PAData, $Krb5::PA_PAC_OPTIONS, $($$nc($$nc($$new($PaPacOptions)->setResourceBasedConstrainedDelegation(true))->setClaims(true))->asn1Encode()))}), $CredentialsUtil$S4U2Type::PROXY));
	if (!$$nc($nc(creds)->getClient())->equals(client)) {
...
$Credentials* CredentialsUtil::getTGTforRealm($String* localRealm, $String* serviceRealm, $Credentials* ccreds, $booleans* okAsDelegate) {
	...
	for ($assign(cTgt, ccreds), i = 0; i < $nc(realms)->length;) {
		...
		for (k = i + 1; k < realms->length; ++k) {
			if (newTgtRealm->equals(realms->get(k))) {
				break;
			}
		}
		if (k < realms->length) {
...
$Credentials* CredentialsUtil::serviceCreds($PrincipalName* service, $Credentials* ccreds) {
	...
	$init($CredentialsUtil$S4U2Type);
	return serviceCreds(var$0, ccreds, $($nc(ccreds)->getClient()), service, nullptr, nullptr, $CredentialsUtil$S4U2Type::NONE);
}
	...
	while (referrals->size() <= $Config::MAX_REFERRALS) {
		...
		if (isReferral) {
			...
			if (s4u2Type == $CredentialsUtil$S4U2Type::PROXY) {
				...
				if (additionalTickets == nullptr || additionalTickets->length == 0 || credsInOut->get(1) == nullptr) {
			...
			} else if (s4u2Type == $CredentialsUtil$S4U2Type::SELF) {
			...
			$var($String, var$1, refSname->getNameString());
...
$Credentials* CredentialsUtil::serviceCredsSingle($KDCOptions* options, $Credentials* asCreds$renamed, $PrincipalName* cname$renamed, $PrincipalName* clientAlias, $PrincipalName* refSname, $PrincipalName* sname, $CredentialsUtil$S4U2Type* s4u2Type, $TicketArray* additionalTickets, $PADataArray* extraPAs) {
	...
	if (!$nc(serviceRealm)->equals(tgtRealm)) {
		if (CredentialsUtil::DEBUG) {
			...
			$System::out->println($$str({">>> serviceCredsSingle: obtaining credentials from "_s, tgtRealm, " to "_s, serviceRealm}));
		}
...
void CredentialsUtil::clinit$($Class* clazz) {
	$init($Krb5);
...
$Class* CredentialsUtil::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PUBLIC, $method(CredentialsUtil, init$, void)},
		{"acquireS4U2proxyCreds", "(Ljava/lang/String;Lsun/security/krb5/internal/Ticket;Lsun/security/krb5/PrincipalName;Lsun/security/krb5/Credentials;)Lsun/security/krb5/Credentials;", nullptr, $PUBLIC | $STATIC, $staticMethod(CredentialsUtil, acquireS4U2proxyCreds, $Credentials*, $String*, $Ticket*, $PrincipalName*, $Credentials*), "sun.security.krb5.KrbException,java.io.IOException"},
		{"acquireS4U2selfCreds", "(Lsun/security/krb5/PrincipalName;Lsun/security/krb5/Credentials;)Lsun/security/krb5/Credentials;", nullptr, $PUBLIC | $STATIC, $staticMethod(CredentialsUtil, acquireS4U2selfCreds, $Credentials*, $PrincipalName*, $Credentials*), "sun.security.krb5.KrbException,java.io.IOException"},
		{"acquireServiceCreds", "(Ljava/lang/String;Lsun/security/krb5/Credentials;)Lsun/security/krb5/Credentials;", nullptr, $PUBLIC | $STATIC, $staticMethod(CredentialsUtil, acquireServiceCreds, $Credentials*, $String*, $Credentials*), "sun.security.krb5.KrbException,java.io.IOException"},
		{"getTGTforRealm", "(Ljava/lang/String;Ljava/lang/String;Lsun/security/krb5/Credentials;[Z)Lsun/security/krb5/Credentials;", nullptr, $PRIVATE | $STATIC, $staticMethod(CredentialsUtil, getTGTforRealm, $Credentials*, $String*, $String*, $Credentials*, $booleans*), "sun.security.krb5.KrbException"},
		{"handleS4U2ProxyReferral", "(Lsun/security/krb5/Credentials;[Lsun/security/krb5/Credentials;Lsun/security/krb5/PrincipalName;)Ljava/lang/String;", nullptr, $PRIVATE | $STATIC, $staticMethod(CredentialsUtil, handleS4U2ProxyReferral, $String*, $Credentials*, $CredentialsArray*, $PrincipalName*), "sun.security.krb5.KrbException,java.io.IOException"},
		{"handleS4U2SelfReferral", "([Lsun/security/krb5/internal/PAData;Lsun/security/krb5/Credentials;Lsun/security/krb5/Credentials;)V", nullptr, $PRIVATE | $STATIC, $staticMethod(CredentialsUtil, handleS4U2SelfReferral, void, $PADataArray*, $Credentials*, $Credentials*), "sun.security.krb5.Asn1Exception,sun.security.krb5.KrbException,java.io.IOException"},
		{"serviceCreds", "(Lsun/security/krb5/PrincipalName;Lsun/security/krb5/Credentials;)Lsun/security/krb5/Credentials;", nullptr, $PRIVATE | $STATIC, $staticMethod(CredentialsUtil, serviceCreds, $Credentials*, $PrincipalName*, $Credentials*), "sun.security.krb5.KrbException,java.io.IOException"},
		{"serviceCreds", "(Lsun/security/krb5/internal/KDCOptions;Lsun/security/krb5/Credentials;Lsun/security/krb5/PrincipalName;Lsun/security/krb5/PrincipalName;[Lsun/security/krb5/internal/Ticket;[Lsun/security/krb5/internal/PAData;Lsun/security/krb5/internal/CredentialsUtil$S4U2Type;)Lsun/security/krb5/Credentials;", nullptr, $PRIVATE | $STATIC, $staticMethod(CredentialsUtil, serviceCreds, $Credentials*, $KDCOptions*, $Credentials*, $PrincipalName*, $PrincipalName*, $TicketArray*, $PADataArray*, $CredentialsUtil$S4U2Type*), "sun.security.krb5.KrbException,java.io.IOException"},
		{"serviceCredsReferrals", "(Lsun/security/krb5/internal/KDCOptions;Lsun/security/krb5/Credentials;Lsun/security/krb5/PrincipalName;Lsun/security/krb5/PrincipalName;Lsun/security/krb5/internal/CredentialsUtil$S4U2Type;[Lsun/security/krb5/internal/Ticket;[Lsun/security/krb5/internal/PAData;)Lsun/security/krb5/Credentials;", nullptr, $PRIVATE | $STATIC, $staticMethod(CredentialsUtil, serviceCredsReferrals, $Credentials*, $KDCOptions*, $Credentials*, $PrincipalName*, $PrincipalName*, $CredentialsUtil$S4U2Type*, $TicketArray*, $PADataArray*), "sun.security.krb5.KrbException,java.io.IOException"},
		{"serviceCredsSingle", "(Lsun/security/krb5/internal/KDCOptions;Lsun/security/krb5/Credentials;Lsun/security/krb5/PrincipalName;Lsun/security/krb5/PrincipalName;Lsun/security/krb5/PrincipalName;Lsun/security/krb5/PrincipalName;Lsun/security/krb5/internal/CredentialsUtil$S4U2Type;[Lsun/security/krb5/internal/Ticket;[Lsun/security/krb5/internal/PAData;)Lsun/security/krb5/Credentials;", nullptr, $PRIVATE | $STATIC, $staticMethod(CredentialsUtil, serviceCredsSingle, $Credentials*, $KDCOptions*, $Credentials*, $PrincipalName*, $PrincipalName*, $PrincipalName*, $PrincipalName*, $CredentialsUtil$S4U2Type*, $TicketArray*, $PADataArray*), "sun.security.krb5.KrbException,java.io.IOException"},
		{}
	};
	...
	$loadClass(CredentialsUtil, name, initialize, &classInfo$$, CredentialsUtil::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(CredentialsUtil);
	});
	return class$;
}
