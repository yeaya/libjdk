...
void KRBCred::init($DerValue* encoding) {
	$useLocalObjectStack();
	bool var$1 = ($nc(encoding)->getTag() & (int8_t)31) != (int8_t)22;
...
$bytes* KRBCred::asn1Encode() {
	...
	for (int32_t i = 0; i < $nc(this->tickets)->length; ++i) {
		temp->write($($nc(this->tickets->get(i))->asn1Encode()));
	}
...
$Class* KRBCred::load$($String* name, bool initialize) {
	...
	$loadClass(KRBCred, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
