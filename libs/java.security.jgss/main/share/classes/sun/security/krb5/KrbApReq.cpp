...
void KrbApReq::init($APOptions* options, $Credentials* tgs_creds, $Checksum* cksum, $EncryptionKey* subKey, $SeqNumber* seqNumber, $AuthorizationData* authorizationData, int32_t usage) {
	...
	init(options, $nc(tgs_creds)->ticket, $nc(tgs_creds)->key, $nc(tgs_creds)->client, cksum, this->ctime, subKey, seqNumber, authorizationData, usage);
}
...
void KrbApReq::authenticate($Krb5AcceptCredential* cred, $InetAddress* initiator) {
	...
	$set(this, ctime, this->authenticator->ctime);
	this->cusec$ = this->authenticator->cusec;
	$set(this->authenticator, ctime, $nc(this->authenticator->ctime)->withMicroSeconds(this->authenticator->cusec));
	...
	for (int32_t i = 0; i < hash->length; ++i) {
		h->set(2 * i, KrbApReq::hexConst->get((hash->get(i) & 0xff) >> 4));
		h->set(2 * i + 1, KrbApReq::hexConst->get(hash->get(i) & 0x0f));
	}
	$var($String, var$0, $nc($nc(this->authenticator)->cname)->toString());
	$var($String, var$1, $nc($nc($nc(this->apReqMessg)->ticket)->sname)->toString());
	int32_t var$2 = $nc(this->authenticator->ctime)->getSeconds();
	int32_t var$3 = this->authenticator->cusec;
	$var($AuthTimeWithHash, time, $new($AuthTimeWithHash, var$0, var$1, var$2, var$3, alg, $$new($String, h)));
	$nc(KrbApReq::rcache)->checkAndStore($($KerberosTime::now()), time);
	if (initiator != nullptr) {
		$var($HostAddress, sender, $new($HostAddress, initiator));
		if (enc_ticketPart->caddr != nullptr && !enc_ticketPart->caddr->inList(sender)) {
			if (KrbApReq::DEBUG) {
				$var($StringBuilder, var$4, $new($StringBuilder));
				var$4->append(">>> KrbApReq: initiator is "_s);
				var$4->append($(sender->getInetAddress()));
				var$4->append(", but caddr is "_s);
				var$4->append($($Arrays::toString($(enc_ticketPart->caddr->getInetAddresses()))));
				$nc($System::out)->println($$str(var$4));
			}
			$throwNew($KrbApErrException, $Krb5::KRB_AP_ERR_BADADDR);
		}
	}
	$var($KerberosTime, now, $KerberosTime::now());
	bool var$5 = enc_ticketPart->starttime != nullptr && enc_ticketPart->starttime->greaterThanWRTClockSkew(now);
	if (var$5 || $nc(enc_ticketPart->flags)->get($Krb5::TKT_OPTS_INVALID)) {
	...
	$set(this, creds, $new($Credentials, $nc(this->apReqMessg)->ticket, $nc(this->authenticator)->cname, nullptr, $nc($nc(this->apReqMessg)->ticket)->sname, nullptr, enc_ticketPart->key, enc_ticketPart->flags, enc_ticketPart->authtime, enc_ticketPart->starttime, enc_ticketPart->endtime, enc_ticketPart->renewTill, enc_ticketPart->caddr, enc_ticketPart->authorizationData));
	if (KrbApReq::DEBUG) {
...
bool KrbApReq::getMutualAuthRequired() {
	...
	if (this->apReqMessg != nullptr) {
		return $nc(this->apReqMessg->apOptions)->get($Krb5::AP_OPTS_MUTUAL_REQUIRED);
	}
	return false;
}
...
bool KrbApReq::useSessionKey() {
	...
	if (this->apReqMessg != nullptr) {
		return $nc(this->apReqMessg->apOptions)->get($Krb5::AP_OPTS_USE_SESSION_KEY);
	}
	return false;
}
...
void KrbApReq::clinit$($Class* clazz) {
...
$Class* KrbApReq::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(Lsun/security/krb5/Credentials;ZZZLsun/security/krb5/Checksum;)V", nullptr, $PUBLIC, $method(KrbApReq, init$, void, $Credentials*, bool, bool, bool, $Checksum*), "sun.security.krb5.Asn1Exception,sun.security.krb5.KrbCryptoException,sun.security.krb5.KrbException,java.io.IOException"},
		{"<init>", "([BLsun/security/jgss/krb5/Krb5AcceptCredential;Ljava/net/InetAddress;)V", nullptr, $PUBLIC, $method(KrbApReq, init$, void, $bytes*, $Krb5AcceptCredential*, $InetAddress*), "sun.security.krb5.KrbException,java.io.IOException"},
		{"<init>", "(Lsun/security/krb5/internal/APOptions;Lsun/security/krb5/internal/Ticket;Lsun/security/krb5/EncryptionKey;Lsun/security/krb5/PrincipalName;Lsun/security/krb5/Checksum;Lsun/security/krb5/internal/KerberosTime;Lsun/security/krb5/EncryptionKey;Lsun/security/krb5/internal/SeqNumber;Lsun/security/krb5/internal/AuthorizationData;)V", nullptr, 0, $method(KrbApReq, init$, void, $APOptions*, $Ticket*, $EncryptionKey*, $PrincipalName*, $Checksum*, $KerberosTime*, $EncryptionKey*, $SeqNumber*, $AuthorizationData*), "sun.security.krb5.Asn1Exception,java.io.IOException,sun.security.krb5.internal.KdcErrException,sun.security.krb5.KrbCryptoException"},
		{"authenticate", "(Lsun/security/jgss/krb5/Krb5AcceptCredential;Ljava/net/InetAddress;)V", nullptr, $PRIVATE, $method(KrbApReq, authenticate, void, $Krb5AcceptCredential*, $InetAddress*), "sun.security.krb5.KrbException,java.io.IOException"},
		{"checkPermittedEType", "(I)V", nullptr, $PRIVATE | $STATIC, $staticMethod(KrbApReq, checkPermittedEType, void, int32_t), "sun.security.krb5.KrbException"},
		{"createMessage", "(Lsun/security/krb5/internal/APOptions;Lsun/security/krb5/internal/Ticket;Lsun/security/krb5/EncryptionKey;Lsun/security/krb5/PrincipalName;Lsun/security/krb5/Checksum;Lsun/security/krb5/internal/KerberosTime;Lsun/security/krb5/EncryptionKey;Lsun/security/krb5/internal/SeqNumber;Lsun/security/krb5/internal/AuthorizationData;I)V", nullptr, $PRIVATE, $method(KrbApReq, createMessage, void, $APOptions*, $Ticket*, $EncryptionKey*, $PrincipalName*, $Checksum*, $KerberosTime*, $EncryptionKey*, $SeqNumber*, $AuthorizationData*, int32_t), "sun.security.krb5.Asn1Exception,java.io.IOException,sun.security.krb5.internal.KdcErrException,sun.security.krb5.KrbCryptoException"},
		{"cusec", "()I", nullptr, 0, $virtualMethod(KrbApReq, cusec, int32_t)},
		{"decode", "()V", nullptr, 0, $virtualMethod(KrbApReq, decode, void), "sun.security.krb5.KrbException,java.io.IOException"},
		{"decode", "(Lsun/security/util/DerValue;)V", nullptr, 0, $virtualMethod(KrbApReq, decode, void, $DerValue*), "sun.security.krb5.KrbException,java.io.IOException"},
		{"getAPOptions", "()Lsun/security/krb5/internal/APOptions;", nullptr, 0, $virtualMethod(KrbApReq, getAPOptions, $APOptions*), "sun.security.krb5.KrbException,java.io.IOException"},
		{"getChecksum", "()Lsun/security/krb5/Checksum;", nullptr, $PUBLIC, $virtualMethod(KrbApReq, getChecksum, $Checksum*)},
		{"getClient", "()Lsun/security/krb5/PrincipalName;", nullptr, $PUBLIC, $virtualMethod(KrbApReq, getClient, $PrincipalName*)},
		{"getCreds", "()Lsun/security/krb5/Credentials;", nullptr, $PUBLIC, $virtualMethod(KrbApReq, getCreds, $Credentials*)},
		{"getCtime", "()Lsun/security/krb5/internal/KerberosTime;", nullptr, 0, $virtualMethod(KrbApReq, getCtime, $KerberosTime*)},
		{"getMessage", "()[B", nullptr, $PUBLIC, $virtualMethod(KrbApReq, getMessage, $bytes*)},
		{"getMutualAuthRequired", "()Z", nullptr, $PUBLIC, $virtualMethod(KrbApReq, getMutualAuthRequired, bool), "sun.security.krb5.KrbException,java.io.IOException"},
		{"getSeqNumber", "()Ljava/lang/Integer;", nullptr, $PUBLIC, $virtualMethod(KrbApReq, getSeqNumber, $Integer*)},
		{"getSubKey", "()Lsun/security/krb5/EncryptionKey;", nullptr, $PUBLIC, $virtualMethod(KrbApReq, getSubKey, $EncryptionKey*)},
		{"init", "(Lsun/security/krb5/internal/APOptions;Lsun/security/krb5/Credentials;Lsun/security/krb5/Checksum;Lsun/security/krb5/EncryptionKey;Lsun/security/krb5/internal/SeqNumber;Lsun/security/krb5/internal/AuthorizationData;I)V", nullptr, $PRIVATE, $method(KrbApReq, init, void, $APOptions*, $Credentials*, $Checksum*, $EncryptionKey*, $SeqNumber*, $AuthorizationData*, int32_t), "sun.security.krb5.KrbException,java.io.IOException"},
		{"init", "(Lsun/security/krb5/internal/APOptions;Lsun/security/krb5/internal/Ticket;Lsun/security/krb5/EncryptionKey;Lsun/security/krb5/PrincipalName;Lsun/security/krb5/Checksum;Lsun/security/krb5/internal/KerberosTime;Lsun/security/krb5/EncryptionKey;Lsun/security/krb5/internal/SeqNumber;Lsun/security/krb5/internal/AuthorizationData;I)V", nullptr, $PRIVATE, $method(KrbApReq, init, void, $APOptions*, $Ticket*, $EncryptionKey*, $PrincipalName*, $Checksum*, $KerberosTime*, $EncryptionKey*, $SeqNumber*, $AuthorizationData*, int32_t), "sun.security.krb5.Asn1Exception,java.io.IOException,sun.security.krb5.internal.KdcErrException,sun.security.krb5.KrbCryptoException"},
		{"useSessionKey", "()Z", nullptr, 0, $virtualMethod(KrbApReq, useSessionKey, bool), "sun.security.krb5.KrbException,java.io.IOException"},
		{}
	};
	$ClassInfo classInfo$$ = {
		$PUBLIC | $ACC_SUPER,
	...
	$loadClass(KrbApReq, name, initialize, &classInfo$$, KrbApReq::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(KrbApReq);
	});
	return class$;
}
