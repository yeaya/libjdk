...
void Krb5::clinit$($Class* clazz) {
	...
	$assignStatic(Krb5::hexDumper, $new($HexDumpEncoder));
	{
		...
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_NONE)), "No error"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_NAME_EXP)), "Client\'s entry in database expired"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_SERVICE_EXP)), "Server\'s entry in database has expired"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_BAD_PVNO)), "Requested protocol version number not supported"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_C_OLD_MAST_KVNO)), "Client\'s key encrypted in old master key"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_S_OLD_MAST_KVNO)), "Server\'s key encrypted in old master key"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_C_PRINCIPAL_UNKNOWN)), "Client not found in Kerberos database"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_S_PRINCIPAL_UNKNOWN)), "Server not found in Kerberos database"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_PRINCIPAL_NOT_UNIQUE)), "Multiple principal entries in database"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_NULL_KEY)), "The client or server has a null key"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_CANNOT_POSTDATE)), "Ticket not eligible for postdating"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_NEVER_VALID)), "Requested start time is later than end time"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_POLICY)), "KDC policy rejects request"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_BADOPTION)), "KDC cannot accommodate requested option"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_ETYPE_NOSUPP)), "KDC has no support for encryption type"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_SUMTYPE_NOSUPP)), "KDC has no support for checksum type"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_PADATA_TYPE_NOSUPP)), "KDC has no support for padata type"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_TRTYPE_NOSUPP)), "KDC has no support for transited type"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_CLIENT_REVOKED)), "Clients credentials have been revoked"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_SERVICE_REVOKED)), "Credentials for server have been revoked"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_TGT_REVOKED)), "TGT has been revoked"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_CLIENT_NOTYET)), "Client not yet valid - try again later"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_SERVICE_NOTYET)), "Server not yet valid - try again later"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_KEY_EXPIRED)), "Password has expired - change password to reset"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_PREAUTH_FAILED)), "Pre-authentication information was invalid"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KDC_ERR_PREAUTH_REQUIRED)), "Additional pre-authentication required"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_BAD_INTEGRITY)), "Integrity check on decrypted field failed"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_TKT_EXPIRED)), "Ticket expired"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_TKT_NYV)), "Ticket not yet valid"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_REPEAT)), "Request is a replay"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_NOT_US)), "The ticket isn\'t for us"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_BADMATCH)), "Ticket and authenticator don\'t match"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_SKEW)), "Clock skew too great"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_BADADDR)), "Incorrect net address"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_BADVERSION)), "Protocol version mismatch"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_MSG_TYPE)), "Invalid msg type"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_MODIFIED)), "Message stream modified"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_BADORDER)), "Message out of order"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_BADKEYVER)), "Specified version of key is not available"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_NOKEY)), "Service key not available"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_MUT_FAIL)), "Mutual authentication failed"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_BADDIRECTION)), "Incorrect message direction"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_METHOD)), "Alternative authentication method required"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_BADSEQ)), "Incorrect sequence number in message"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_INAPP_CKSUM)), "Inappropriate type of checksum in message"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_ERR_RESPONSE_TOO_BIG)), "Response too big for UDP, retry with TCP"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_ERR_GENERIC)), "Generic error (description in e-text)"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_ERR_FIELD_TOOLONG)), "Field is too long for this implementation"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_NOREALM)), "Realm name not available"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::API_INVALID_ARG)), "Invalid argument"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::BITSTRING_SIZE_INVALID)), "BitString size does not match input byte array"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::BITSTRING_INDEX_OUT_OF_BOUNDS)), "BitString bit index does not fall within size"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::BITSTRING_BAD_LENGTH)), "BitString length is wrong for the expected type"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::REALM_ILLCHAR)), $cstr({'I', 'l', 'l', 'e', 'g', 'a', 'l', ' ', 'c', 'h', 'a', 'r', 'a', 'c', 't', 'e', 'r', ' ', 'i', 'n', ' ', 'r', 'e', 'a', 'l', 'm', ' ', 'n', 'a', 'm', 'e', ';', ' ', 'o', 'n', 'e', ' ', 'o', 'f', ':', ' ', '\'', '/', '\'', ',', ' ', '\'', ':', '\'', ',', ' ', '\'', '\0', '\''}));
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::REALM_NULL)), "Null realm name"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_BAD_TIMEFORMAT)), "Input not in GeneralizedTime format"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_MISSING_FIELD)), "Structure is missing a required field"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_MISPLACED_FIELD)), "Unexpected field number"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_TYPE_MISMATCH)), "Type numbers are inconsistent"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_OVERFLOW)), "Value too large"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_OVERRUN)), "Encoding ended unexpectedly"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_BAD_ID)), "Identifier doesn\'t match expected value"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_BAD_LENGTH)), "Length doesn\'t match expected value"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_BAD_FORMAT)), "Badly-formatted encoding"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_PARSE_ERROR)), "Parse error"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_BAD_CLASS)), "Bad class number"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_BAD_TYPE)), "Bad type number"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_BAD_TAG)), "Bad tag number"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_UNSUPPORTED_TYPE)), "Unsupported ASN.1 type encountered"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::ASN1_CANNOT_ENCODE)), "Encoding failed due to invalid parameter(s)"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_CRYPTO_NOT_SUPPORT)), "Client has no support for crypto type"_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_REQ_OPTIONS)), "Invalid option setting in ticket request."_s);
		Krb5::errMsgList->put($($Integer::valueOf(Krb5::KRB_AP_ERR_GEN_CRED)), "Fail to create credential."_s);
	}
}
...
$Class* Krb5::load$($String* name, bool initialize) {
	...
	$loadClass(Krb5, name, initialize, &classInfo$$, Krb5::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Krb5);
	});
	return class$;
}
