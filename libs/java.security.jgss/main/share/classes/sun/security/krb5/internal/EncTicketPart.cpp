...
void EncTicketPart::init($DerValue* encoding) {
	...
	bool var$1 = ($nc(encoding)->getTag() & (int8_t)31) != (int8_t)3;
	...
	$set(this, flags, $TicketFlags::parse($(der->getData()), (int8_t)0, false));
	$set(this, key, $EncryptionKey::parse($(der->getData()), (int8_t)1, false));
	$var($Realm, crealm, $Realm::parse($(der->getData()), (int8_t)2, false));
	$set(this, cname, $PrincipalName::parse($(der->getData()), (int8_t)3, false, crealm));
	$set(this, transited, $TransitedEncoding::parse($(der->getData()), (int8_t)4, false));
...
$bytes* EncTicketPart::asn1Encode() {
	...
	if (this->starttime != nullptr) {
		...
		bytes->write(var$6, $(this->starttime->asn1Encode()));
	}
...
$Class* EncTicketPart::load$($String* name, bool initialize) {
	...
	$loadClass(EncTicketPart, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(EncTicketPart);
	});
	return class$;
}
