...
$bytes* ECDSAUtils::convertASN1toXMLDSIG($bytes* asn1Bytes, int32_t rawLen) {
	$init(ECDSAUtils);
	if ($nc(asn1Bytes)->length < 8 || asn1Bytes->get(0) != 48) {
		$throwNew($IOException, "Invalid ASN.1 format of ECDSA signature"_s);
	}
	int32_t offset = 0;
	if (asn1Bytes->get(1) > 0) {
		offset = 2;
	...
	if ((asn1Bytes->get(offset - 1) & 0xff) != asn1Bytes->length - offset || (asn1Bytes->get(offset - 1) & 0xff) != 2 + rLength + 2 + sLength || asn1Bytes->get(offset) != 2 || asn1Bytes->get(offset + 2 + rLength) != 2) {
		$throwNew($IOException, "Invalid ASN.1 format of ECDSA signature"_s);
	}
...
$bytes* ECDSAUtils::convertXMLDSIGtoASN1($bytes* xmldsigBytes) {
	...
	$nc(asn1Bytes)->set(0, 48);
	asn1Bytes->set(offset++, (int8_t)len);
	asn1Bytes->set(offset++, 2);
...
$String* ECDSAUtils::getOIDFromPublicKey($ECPublicKey* ecPublicKey) {
	...
	if ($instanceOf($ECFieldFp, ecField)) {
		...
		$assign(field, ecFieldFp->getP());
	} else {
		$var($ECFieldF2m, ecFieldF2m, $cast($ECFieldF2m, ecField));
		$assign(field, $nc(ecFieldF2m)->getReductionPolynomial());
	}
	$var($Iterator, ecCurveDefinitionIterator, ECDSAUtils::ecCurveDefinitions->iterator());
...
$bytes* ECDSAUtils::encodePoint($ECPoint* ecPoint, $EllipticCurve* ellipticCurve) {
	...
	$var($bytes, affineYBytes, stripLeadingZeros($($$nc(ecPoint->getAffineY())->toByteArray())));
	$var($bytes, encodedBytes, $new($bytes, size * 2 + 1));
	encodedBytes->set(0, 4);
	$System::arraycopy(affineXBytes, 0, encodedBytes, size - $nc(affineXBytes)->length + 1, $nc(affineXBytes)->length);
	$System::arraycopy(affineYBytes, 0, encodedBytes, encodedBytes->length - $nc(affineYBytes)->length, $nc(affineYBytes)->length);
	return encodedBytes;
}
...
$Class* ECDSAUtils::load$($String* name, bool initialize) {
	...
	$loadClass(ECDSAUtils, name, initialize, &classInfo$$, ECDSAUtils::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ECDSAUtils);
	});
	return class$;
}
