...
$String* XMLSignatureInputDebugger::getHTMLRepresentation() {
	$useLocalObjectStack();
	if (this->xpathNodeSet == nullptr || this->xpathNodeSet->isEmpty()) {
	...
	try {
		try {
			...
			$assign(var$2, this->writer->toString());
			return$1 = true;
			goto $finally;
...
void XMLSignatureInputDebugger::canonicalizeXPathNodeSet($Node* currentNode) {
	...
	int32_t currentNodeType = $nc(currentNode)->getNodeType();
	{
		...
		case $Node::ATTRIBUTE_NODE:
			$throwNew($XMLSignatureException, "empty"_s, $$new($ObjectArray, {$$str({"An incorrect node was provided for c14n: "_s, $$str(currentNodeType)})}));
		case $Node::DOCUMENT_NODE:
			{
				$nc(this->writer)->write(XMLSignatureInputDebugger::HTMLPrefix);
				{
					$var($Node, currentChild, currentNode->getFirstChild());
					for (; currentChild != nullptr; $assign(currentChild, currentChild->getNextSibling())) {
		...
		case $Node::CDATA_SECTION_NODE:
			{
				...
				outputTextToWriter($(currentNode->getNodeValue()));
				{
					...
					for (;; $assign(nextSibling, $nc(nextSibling)->getNextSibling())) {
						...
						{
							this->outputTextToWriter($($nc(nextSibling)->getNodeValue()));
						}
					}
				}
				$nc(this->writer)->write(XMLSignatureInputDebugger::HTMLIncludeOrExcludeSuffix);
		...
		case $Node::ELEMENT_NODE:
			{
				...
				for (int32_t i = 0; i < attrsLength; ++i) {
					...
					$var($String, var$2, a->getNodeName());
				...
				$nc(this->writer)->write("&gt;"_s);
				this->writer->write(XMLSignatureInputDebugger::HTMLIncludeOrExcludeSuffix);
				{
					$var($Node, currentChild, currentNode->getFirstChild());
					for (; currentChild != nullptr; $assign(currentChild, currentChild->getNextSibling())) {
						this->canonicalizeXPathNodeSet(currentChild);
...
void XMLSignatureInputDebugger::clinit$($Class* clazz) {
...
$Class* XMLSignatureInputDebugger::load$($String* name, bool initialize) {
	...
	$loadClass(XMLSignatureInputDebugger, name, initialize, &classInfo$$, XMLSignatureInputDebugger::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLSignatureInputDebugger);
	});
	return class$;
}
