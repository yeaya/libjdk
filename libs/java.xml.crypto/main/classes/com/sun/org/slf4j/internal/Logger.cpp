...
$Class* Logger$$Lambda$lambda$static$0::load$($String* name, bool initialize) {
	...
	$loadClass(Logger$$Lambda$lambda$static$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Logger$$Lambda$lambda$static$0);
	});
	return class$;
}
...
$Class* Logger$$Lambda$lambda$log0$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(Logger$$Lambda$lambda$log0$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Logger$$Lambda$lambda$log0$1$1);
	});
	return class$;
}
...
void Logger::log0($Level* level, $String* s) {
	...
	if ($nc(this->impl)->isLoggable(level)) {
		$var($StackWalker$StackFrame, sf, $cast($StackWalker$StackFrame, $$sure($Optional, $nc(Logger::WALKER)->walk($$new(Logger$$Lambda$lambda$log0$1$1)))->get()));
		$init($Level);
		$var($Level, var$0, $Level::FINE);
		$var($String, var$1, $nc(sf)->getClassName());
		this->impl->logp(var$0, var$1, $(sf->getMethodName()), s);
	}
}
...
void Logger::log0($Level* level, $String* s, $Throwable* e) {
	...
	if ($nc(this->impl)->isLoggable(level)) {
		...
		$var($Level, var$0, $Level::FINE);
		$var($String, var$1, $nc(sf)->getClassName());
		this->impl->logp(var$0, var$1, $(sf->getMethodName()), s, e);
	}
}
...
void Logger::log0($Level* level, $String* s, $ObjectArray* o) {
	...
	if ($nc(this->impl)->isLoggable(level)) {
		...
		this->impl->logp(var$0, var$1, var$2, $(addIndex(s)), o);
	}
}
...
void Logger::clinit$($Class* clazz) {
	$beforeCallerSensitive();
	$assignStatic(Logger::WALKER, $cast($StackWalker, $AccessController::doPrivileged($cast($PrivilegedAction, $$new(Logger$$Lambda$lambda$static$0)))));
}
...
$Class* Logger::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.org.slf4j.internal.Logger$$Lambda$lambda$static$0")) {
			return Logger$$Lambda$lambda$static$0::load$(name, initialize);
		}
		if (name->equals("com.sun.org.slf4j.internal.Logger$$Lambda$lambda$log0$1$1")) {
