...
$DOMKeyValue$EC$Curve* DOMKeyValue$EC::initializeCurve($String* name, $String* oid, $String* sfield, $String* a, $String* b, $String* x, $String* y, $String* n, int32_t h) {
	...
	$var($BigInteger, var$0, $DOMKeyValue::bigInt(a));
	$var($EllipticCurve, curve, $new($EllipticCurve, field, var$0, $($DOMKeyValue::bigInt(b))));
	$var($BigInteger, var$1, $DOMKeyValue::bigInt(x));
	$var($ECPoint, g, $new($ECPoint, var$1, $($DOMKeyValue::bigInt(y))));
	return $new($DOMKeyValue$EC$Curve, name, oid, curve, g, $($DOMKeyValue::bigInt(n)), h);
}

void DOMKeyValue$EC::init$($ECPublicKey* ecKey) {
	$useLocalObjectStack();
	$DOMKeyValue::init$(ecKey);
...
$String* DOMKeyValue$EC::getCurveOid($ECParameterSpec* params) {
	...
	} else if (matchCurve(params, DOMKeyValue$EC::SECP384R1)) {
		$assign(match, DOMKeyValue$EC::SECP384R1);
	} else if (matchCurve(params, DOMKeyValue$EC::SECP521R1)) {
...
$PublicKey* DOMKeyValue$EC::unmarshalKeyValue($Element* kvtElem) {
	...
	if (var$0 && "http://www.w3.org/2009/xmldsig11#"_s->equals($(curElem->getNamespaceURI()))) {
		$throwNew($UnsupportedOperationException, "ECParameters not supported"_s);
	} else {
		bool var$1 = "NamedCurve"_s->equals($(curElem->getLocalName()));
		if (var$1 && "http://www.w3.org/2009/xmldsig11#"_s->equals($(curElem->getNamespaceURI()))) {
...
$ECParameterSpec* DOMKeyValue$EC::getECParameterSpec($String* oid) {
	...
	} else if (oid->equals($($nc(DOMKeyValue$EC::SECP384R1)->getObjectId()))) {
		return DOMKeyValue$EC::SECP384R1;
	} else if (oid->equals($($nc(DOMKeyValue$EC::SECP521R1)->getObjectId()))) {
...
void DOMKeyValue$EC::clinit$($Class* clazz) {
...
$Class* DOMKeyValue$EC::load$($String* name, bool initialize) {
	...
	$loadClass(DOMKeyValue$EC, name, initialize, &classInfo$$, DOMKeyValue$EC::clinit$, []($Class* clazz) -> $Object* {
