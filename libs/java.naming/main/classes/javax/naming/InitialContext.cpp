...
$Object* InitialContext::lookup($String* name) {
	return $$nc(getURLOrDefaultInitCtx(name))->lookup(name);
}
...
$Object* InitialContext::lookupLink($String* name) {
	return $$nc(getURLOrDefaultInitCtx(name))->lookupLink(name);
}
...
$Object* InitialContext::addToEnvironment($String* propName, Object$* propVal) {
	...
	return $$nc(getDefaultInitCtx())->addToEnvironment(propName, propVal);
}

$Object* InitialContext::removeFromEnvironment($String* propName) {
	$nc(this->myProps)->remove(propName);
	return $$nc(getDefaultInitCtx())->removeFromEnvironment(propName);
}
...
$Class* InitialContext::load$($String* name, bool initialize) {
	...
	$loadClass(InitialContext, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(InitialContext);
	});
	return class$;
}
