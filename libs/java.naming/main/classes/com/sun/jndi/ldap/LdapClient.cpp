...
$LdapResult* LdapClient::authenticate(bool initial, $String* name$renamed, Object$* pw$renamed, int32_t version, $String* authMechanism, $ControlArray* ctls, $Hashtable* env) {
	$synchronized(this) {
		...
		int32_t readTimeout = this->conn->readTimeout;
		...
		try {
			...
			if (var$3 || authMechanism->equalsIgnoreCase("anonymous"_s)) {
				if (!initial || (version == LdapClient::LDAP_VERSION2) || (version == LdapClient::LDAP_VERSION3_VERSION2) || ((ctls != nullptr) && (ctls->length > 0))) {
					try {
						...
						$var($NamingException, ne, $new($CommunicationException, $$str({"anonymous bind failed: "_s, this->conn->host, ":"_s, $$str(this->conn->port)})));
						ne->setRootCause(e);
						$throw(ne);
					}
				} else {
				...
				try {
					try {
						...
						$var($NamingException, ne, $new($CommunicationException, $$str({"simple bind failed: "_s, this->conn->host, ":"_s, $$str(this->conn->port)})));
						ne->setRootCause(e);
						$throw(ne);
					}
				...
				try {
					$assign(res, $LdapSasl::saslBind(this, this->conn, this->conn->host, name, pw, authMechanism, env, ctls));
					if ($nc(res)->status == LdapClient::LDAP_SUCCESS) {
						this->conn->setBound();
					}
				} catch ($IOException& e) {
					$var($NamingException, ne, $new($CommunicationException, $$str({"SASL bind failed: "_s, this->conn->host, ":"_s, $$str(this->conn->port)})));
					ne->setRootCause(e);
					$throw(ne);
				}
			} else {
			...
			if (var$6) {
				bool var$8 = authMechanism->equalsIgnoreCase("none"_s);
				bool var$7 = var$8 || authMechanism->equalsIgnoreCase("anonymous"_s);
				var$6 = var$7 || authMechanism->equalsIgnoreCase("simple"_s);
			}
			if (var$6) {
				$var($bytes, encodedPw, nullptr);
				...
				try {
					try {
						...
						$var($NamingException, ne, $new($CommunicationException, $$str({authMechanism, ":"_s, this->conn->host, ":"_s, $$str(this->conn->port)})));
						ne->setRootCause(e);
						$throw(ne);
					}
...
$LdapResult* LdapClient::ldapBind($String* dn, $bytes* toServer, $ControlArray* bindCtls, $String* auth, bool pauseAfterReceipt) {
	$synchronized(this) {
		$useLocalObjectStack();
		ensureOpen();
		this->conn->abandonOutstandingReqs(nullptr);
		$var($BerEncoder, ber, $new($BerEncoder));
		int32_t curMsgId = this->conn->getMsgId();
		...
		ber->endSeq();
		$var($LdapRequest, req, this->conn->writeRequest(ber, curMsgId, pauseAfterReceipt));
		if (toServer != nullptr) {
			ber->reset();
		}
		$var($BerDecoder, rber, this->conn->readReply(req));
...
bool LdapClient::usingSaslStreams() {
	return ($instanceOf($SaslInputStream, this->conn->inStream));
}
...
$bytes* LdapClient::encodePassword(Object$* pw$renamed, bool v3) {
	...
	if ($instanceOf($String, pw)) {
		if (v3) {
			return $cast($String, pw)->getBytes("UTF8"_s);
		} else {
			return $cast($String, pw)->getBytes("8859_1"_s);
		}
	} else {
...
void LdapClient::forceClose(bool cleanPool) {
	...
	;
	;
	this->conn->cleanup(nullptr, false);
	if (cleanPool) {
		$nc(this->pcb)->removePooledConnection(this);
	}
}
...
void LdapClient::processConnectionClosure() {
	...
	if ($nc(this->unsolicited)->size() > 0) {
		...
		if (this->conn != nullptr) {
			$assign(msg, $str({this->conn->host, ":"_s, $$str(this->conn->port), " connection closed"_s}));
		} else {
...
$LdapResult* LdapClient::search($String* dn, int32_t scope, int32_t deref, int32_t sizeLimit, int32_t timeLimit, bool attrsOnly, $StringArray* attrs, $String* filter, int32_t batchSize, $ControlArray* reqCtls, $Hashtable* binaryAttrs, bool waitFirstReply, int32_t replyQueueCapacity) {
	...
	ber->endSeq();
	$var($LdapRequest, req, this->conn->writeRequest(ber, curMsgId, false, replyQueueCapacity));
	res->msgId = curMsgId;
	res->status = LdapClient::LDAP_SUCCESS;
	if (waitFirstReply) {
...
void LdapClient::clearSearchReply($LdapResult* res, $ControlArray* ctls) {
	if (res != nullptr) {
		$var($LdapRequest, req, this->conn->findRequest(res->msgId));
		if (req == nullptr) {
			return;
		}
...
$LdapResult* LdapClient::getSearchReply(int32_t batchSize, $LdapResult* res, $Hashtable* binaryAttrs) {
	...
	if (($assign(req, this->conn->findRequest($nc(res)->msgId))) == nullptr) {
		return nullptr;
	}
...
$LdapResult* LdapClient::getSearchReply($LdapRequest* req, int32_t batchSize, $LdapResult* res, $Hashtable* binaryAttrs) {
	...
	for (int32_t i = 0; i < batchSize;) {
		$assign(replyBer, this->conn->readReply(req));
...
int32_t LdapClient::parseAttributeValue($BerDecoder* ber, $Attribute* la, bool hasBinaryValues) {
	...
	$var($ints, len, $new($ints, 1));
	if (hasBinaryValues) {
		$nc(la)->add($($nc(ber)->parseOctetString($nc(ber)->peekByte(), len)));
	} else {
...
bool LdapClient::isBinaryValued($String* attrid, $Hashtable* binaryAttrs) {
	...
	bool var$0 = var$1 || LdapClient::defaultBinaryAttrs->containsKey(id);
...
void LdapClient::encodeControls($BerEncoder* ber, $ControlArray* reqCtls) {
	$init(LdapClient);
	$useLocalObjectStack();
	if ((reqCtls == nullptr) || (reqCtls->length == 0)) {
		return;
	}
...
$LdapResult* LdapClient::modify($String* dn, $ints* operations, $AttributeArray* attrs, $ControlArray* reqCtls) {
	...
	for (int32_t i = 0; i < $nc(operations)->length; ++i) {
		...
		if ((operations->get(i) == LdapClient::ADD) && hasNoValue($nc(attrs)->get(i))) {
			$throwNew($InvalidAttributeValueException, $$str({"\'"_s, $($nc(attrs->get(i))->getID()), "\' has no values."_s}));
		} else {
	...
	ber->endSeq();
	$var($LdapRequest, req, this->conn->writeRequest(ber, curMsgId));
...
$LdapResult* LdapClient::add($LdapEntry* entry, $ControlArray* reqCtls) {
	...
	$var($LdapResult, res, $new($LdapResult));
	res->status = LdapClient::LDAP_OPERATIONS_ERROR;
	if (entry == nullptr || entry->DN == nullptr) {
		return res;
	}
	$var($BerEncoder, ber, $new($BerEncoder));
	int32_t curMsgId = this->conn->getMsgId();
	...
	ber->beginSeq($Ber::ASN_SEQUENCE | $Ber::ASN_CONSTRUCTOR);
	$var($NamingEnumeration, enum_, $nc(entry->attributes)->getAll());
	while ($nc(enum_)->hasMore()) {
...
$LdapResult* LdapClient::compare($String* DN, $String* type, $String* value, $ControlArray* reqCtls) {
	...
	$var($bytes, val, this->isLdapv3 ? $nc(value)->getBytes("UTF8"_s) : $nc(value)->getBytes("8859_1"_s));
...
$String* LdapClient::getErrorMessage(int32_t errorCode, $String* errorMessage) {
	...
	if ((errorMessage != nullptr) && (errorMessage->length() != 0)) {
		...
		try {
			if (LdapClient::ldap_error_message->get(errorCode) != nullptr) {
				$assign(message, $str({message, " - "_s, LdapClient::ldap_error_message->get(errorCode), "]"_s}));
			}
...
void LdapClient::processUnsolicited($BerDecoder* ber) {
	...
	try {
		...
		if (LdapClient::DISCONNECT_OID->equals(res->extensionId)) {
			forceClose(this->pooled);
		}
		...
		$synchronized(this->unsolicited) {
			if (this->unsolicited->size() > 0) {
				$assign(first, $cast($LdapCtx, this->unsolicited->elementAt(0)));
...
void LdapClient::notifyUnsolicited(Object$* e) {
	...
	$synchronized(this->unsolicited) {
		$assign(unsolicitedCopy, $new($Vector, $cast($AbstractCollection, this->unsolicited)));
		if ($instanceOf($NamingException, e)) {
			this->unsolicited->setSize(0);
		}
	}
	for (int32_t i = 0; i < unsolicitedCopy->size(); ++i) {
		$$sure($LdapCtx, unsolicitedCopy->elementAt(i))->fireUnsolicited(e);
	}
}

void LdapClient::ensureOpen() {
	if (this->conn == nullptr || !this->conn->useable) {
		if (this->conn != nullptr && this->conn->closureReason != nullptr) {
...
void LdapClient::clinit$($Class* clazz) {
	...
	{
		LdapClient::defaultBinaryAttrs->put("userpassword"_s, $Boolean::TRUE);
		LdapClient::defaultBinaryAttrs->put("javaserializeddata"_s, $Boolean::TRUE);
		LdapClient::defaultBinaryAttrs->put("javaserializedobject"_s, $Boolean::TRUE);
		LdapClient::defaultBinaryAttrs->put("jpegphoto"_s, $Boolean::TRUE);
		LdapClient::defaultBinaryAttrs->put("audio"_s, $Boolean::TRUE);
		LdapClient::defaultBinaryAttrs->put("thumbnailphoto"_s, $Boolean::TRUE);
		LdapClient::defaultBinaryAttrs->put("thumbnaillogo"_s, $Boolean::TRUE);
		LdapClient::defaultBinaryAttrs->put("usercertificate"_s, $Boolean::TRUE);
		LdapClient::defaultBinaryAttrs->put("cacertificate"_s, $Boolean::TRUE);
		LdapClient::defaultBinaryAttrs->put("certificaterevocationlist"_s, $Boolean::TRUE);
		LdapClient::defaultBinaryAttrs->put("authorityrevocationlist"_s, $Boolean::TRUE);
		LdapClient::defaultBinaryAttrs->put("crosscertificatepair"_s, $Boolean::TRUE);
		LdapClient::defaultBinaryAttrs->put("photo"_s, $Boolean::TRUE);
		LdapClient::defaultBinaryAttrs->put("personalsignature"_s, $Boolean::TRUE);
		LdapClient::defaultBinaryAttrs->put("x500uniqueidentifier"_s, $Boolean::TRUE);
	}
...
$Class* LdapClient::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(Ljava/lang/String;ILjava/lang/String;IILjava/io/OutputStream;Lcom/sun/jndi/ldap/pool/PoolCallback;)V", nullptr, 0, $method(LdapClient, init$, void, $String*, int32_t, $String*, int32_t, int32_t, $OutputStream*, $PoolCallback*), "javax.naming.NamingException"},
		{"add", "(Lcom/sun/jndi/ldap/LdapEntry;[Ljavax/naming/ldap/Control;)Lcom/sun/jndi/ldap/LdapResult;", nullptr, 0, $method(LdapClient, add, $LdapResult*, $LdapEntry*, $ControlArray*), "java.io.IOException,javax.naming.NamingException"},
		{"addUnsolicited", "(Lcom/sun/jndi/ldap/LdapCtx;)V", nullptr, 0, $method(LdapClient, addUnsolicited, void, $LdapCtx*)},
		{"authenticate", "(ZLjava/lang/String;Ljava/lang/Object;ILjava/lang/String;[Ljavax/naming/ldap/Control;Ljava/util/Hashtable;)Lcom/sun/jndi/ldap/LdapResult;", "(ZLjava/lang/String;Ljava/lang/Object;ILjava/lang/String;[Ljavax/naming/ldap/Control;Ljava/util/Hashtable<**>;)Lcom/sun/jndi/ldap/LdapResult;", $SYNCHRONIZED, $method(LdapClient, authenticate, $LdapResult*, bool, $String*, Object$*, int32_t, $String*, $ControlArray*, $Hashtable*), "javax.naming.NamingException"},
		{"authenticateCalled", "()Z", nullptr, $SYNCHRONIZED, $method(LdapClient, authenticateCalled, bool)},
		{"clearSearchReply", "(Lcom/sun/jndi/ldap/LdapResult;[Ljavax/naming/ldap/Control;)V", nullptr, 0, $method(LdapClient, clearSearchReply, void, $LdapResult*, $ControlArray*)},
		{"close", "([Ljavax/naming/ldap/Control;Z)V", nullptr, $SYNCHRONIZED, $method(LdapClient, close, void, $ControlArray*, bool)},
		{"closeConnection", "()V", nullptr, $PUBLIC | $SYNCHRONIZED, $virtualMethod(LdapClient, closeConnection, void)},
		{"compare", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;[Ljavax/naming/ldap/Control;)Lcom/sun/jndi/ldap/LdapResult;", nullptr, 0, $method(LdapClient, compare, $LdapResult*, $String*, $String*, $String*, $ControlArray*), "java.io.IOException,javax.naming.NamingException"},
		{"delete", "(Ljava/lang/String;[Ljavax/naming/ldap/Control;)Lcom/sun/jndi/ldap/LdapResult;", nullptr, 0, $method(LdapClient, delete$, $LdapResult*, $String*, $ControlArray*), "java.io.IOException,javax.naming.NamingException"},
		{"encodeAttribute", "(Lcom/sun/jndi/ldap/BerEncoder;Ljavax/naming/directory/Attribute;)V", nullptr, $PRIVATE, $method(LdapClient, encodeAttribute, void, $BerEncoder*, $Attribute*), "java.io.IOException,javax.naming.NamingException"},
		{"encodeControls", "(Lcom/sun/jndi/ldap/BerEncoder;[Ljavax/naming/ldap/Control;)V", nullptr, $STATIC, $staticMethod(LdapClient, encodeControls, void, $BerEncoder*, $ControlArray*), "java.io.IOException"},
		{"encodePassword", "(Ljava/lang/Object;Z)[B", nullptr, $PRIVATE | $STATIC, $staticMethod(LdapClient, encodePassword, $bytes*, Object$*, bool), "java.io.IOException"},
		{"ensureOpen", "()V", nullptr, $PRIVATE, $method(LdapClient, ensureOpen, void), "java.io.IOException"},
		{"extendedOp", "(Ljava/lang/String;[B[Ljavax/naming/ldap/Control;Z)Lcom/sun/jndi/ldap/LdapResult;", nullptr, 0, $method(LdapClient, extendedOp, $LdapResult*, $String*, $bytes*, $ControlArray*, bool), "java.io.IOException,javax.naming.NamingException"},
		{"finalize", "()V", nullptr, $PROTECTED, $virtualMethod(LdapClient, finalize, void)},
		{"forceClose", "(Z)V", nullptr, $PRIVATE, $method(LdapClient, forceClose, void, bool)},
		{"getErrorMessage", "(ILjava/lang/String;)Ljava/lang/String;", nullptr, $STATIC, $staticMethod(LdapClient, getErrorMessage, $String*, int32_t, $String*)},
		{"getInstance", "(ZLjava/lang/String;ILjava/lang/String;IILjava/io/OutputStream;ILjava/lang/String;[Ljavax/naming/ldap/Control;Ljava/lang/String;Ljava/lang/String;Ljava/lang/Object;Ljava/util/Hashtable;)Lcom/sun/jndi/ldap/LdapClient;", "(ZLjava/lang/String;ILjava/lang/String;IILjava/io/OutputStream;ILjava/lang/String;[Ljavax/naming/ldap/Control;Ljava/lang/String;Ljava/lang/String;Ljava/lang/Object;Ljava/util/Hashtable<**>;)Lcom/sun/jndi/ldap/LdapClient;", $STATIC, $staticMethod(LdapClient, getInstance, LdapClient*, bool, $String*, int32_t, $String*, int32_t, int32_t, $OutputStream*, int32_t, $String*, $ControlArray*, $String*, $String*, Object$*, $Hashtable*), "javax.naming.NamingException"},
		{"getSearchReply", "(ILcom/sun/jndi/ldap/LdapResult;Ljava/util/Hashtable;)Lcom/sun/jndi/ldap/LdapResult;", "(ILcom/sun/jndi/ldap/LdapResult;Ljava/util/Hashtable<Ljava/lang/String;Ljava/lang/Boolean;>;)Lcom/sun/jndi/ldap/LdapResult;", 0, $method(LdapClient, getSearchReply, $LdapResult*, int32_t, $LdapResult*, $Hashtable*), "java.io.IOException,javax.naming.NamingException"},
		{"getSearchReply", "(Lcom/sun/jndi/ldap/LdapRequest;ILcom/sun/jndi/ldap/LdapResult;Ljava/util/Hashtable;)Lcom/sun/jndi/ldap/LdapResult;", "(Lcom/sun/jndi/ldap/LdapRequest;ILcom/sun/jndi/ldap/LdapResult;Ljava/util/Hashtable<Ljava/lang/String;Ljava/lang/Boolean;>;)Lcom/sun/jndi/ldap/LdapResult;", $PRIVATE, $method(LdapClient, getSearchReply, $LdapResult*, $LdapRequest*, int32_t, $LdapResult*, $Hashtable*), "java.io.IOException,javax.naming.NamingException"},
		{"hasNoValue", "(Ljavax/naming/directory/Attribute;)Z", nullptr, $PRIVATE | $STATIC, $staticMethod(LdapClient, hasNoValue, bool, $Attribute*), "javax.naming.NamingException"},
		{"incRefCount", "()V", nullptr, $SYNCHRONIZED, $method(LdapClient, incRefCount, void)},
		{"isBinaryValued", "(Ljava/lang/String;Ljava/util/Hashtable;)Z", "(Ljava/lang/String;Ljava/util/Hashtable<Ljava/lang/String;Ljava/lang/Boolean;>;)Z", $PRIVATE, $method(LdapClient, isBinaryValued, bool, $String*, $Hashtable*)},
		{"isUpgradedToStartTls", "()Z", nullptr, 0, $method(LdapClient, isUpgradedToStartTls, bool)},
		{"ldapBind", "(Ljava/lang/String;[B[Ljavax/naming/ldap/Control;Ljava/lang/String;Z)Lcom/sun/jndi/ldap/LdapResult;", nullptr, $PUBLIC | $SYNCHRONIZED, $method(LdapClient, ldapBind, $LdapResult*, $String*, $bytes*, $ControlArray*, $String*, bool), "java.io.IOException,javax.naming.NamingException"},
		{"moddn", "(Ljava/lang/String;Ljava/lang/String;ZLjava/lang/String;[Ljavax/naming/ldap/Control;)Lcom/sun/jndi/ldap/LdapResult;", nullptr, 0, $method(LdapClient, moddn, $LdapResult*, $String*, $String*, bool, $String*, $ControlArray*), "java.io.IOException,javax.naming.NamingException"},
		{"modify", "(Ljava/lang/String;[I[Ljavax/naming/directory/Attribute;[Ljavax/naming/ldap/Control;)Lcom/sun/jndi/ldap/LdapResult;", nullptr, 0, $method(LdapClient, modify, $LdapResult*, $String*, $ints*, $AttributeArray*, $ControlArray*), "java.io.IOException,javax.naming.NamingException"},
		{"notifyUnsolicited", "(Ljava/lang/Object;)V", nullptr, $PRIVATE, $method(LdapClient, notifyUnsolicited, void, Object$*)},
		{"parseAttribute", "(Lcom/sun/jndi/ldap/BerDecoder;Ljava/util/Hashtable;)Ljavax/naming/directory/Attribute;", "(Lcom/sun/jndi/ldap/BerDecoder;Ljava/util/Hashtable<Ljava/lang/String;Ljava/lang/Boolean;>;)Ljavax/naming/directory/Attribute;", $PRIVATE, $method(LdapClient, parseAttribute, $Attribute*, $BerDecoder*, $Hashtable*), "java.io.IOException"},
		{"parseAttributeValue", "(Lcom/sun/jndi/ldap/BerDecoder;Ljavax/naming/directory/Attribute;Z)I", nullptr, $PRIVATE, $method(LdapClient, parseAttributeValue, int32_t, $BerDecoder*, $Attribute*, bool), "java.io.IOException"},
		{"parseControls", "(Lcom/sun/jndi/ldap/BerDecoder;)Ljava/util/Vector;", "(Lcom/sun/jndi/ldap/BerDecoder;)Ljava/util/Vector<Ljavax/naming/ldap/Control;>;", $STATIC, $staticMethod(LdapClient, parseControls, $Vector*, $BerDecoder*), "java.io.IOException"},
		{"parseExtResponse", "(Lcom/sun/jndi/ldap/BerDecoder;Lcom/sun/jndi/ldap/LdapResult;)V", nullptr, $PRIVATE, $method(LdapClient, parseExtResponse, void, $BerDecoder*, $LdapResult*), "java.io.IOException"},
		{"parseResult", "(Lcom/sun/jndi/ldap/BerDecoder;Lcom/sun/jndi/ldap/LdapResult;Z)V", nullptr, $STATIC, $staticMethod(LdapClient, parseResult, void, $BerDecoder*, $LdapResult*, bool), "java.io.IOException"},
		{"processConnectionClosure", "()V", nullptr, 0, $method(LdapClient, processConnectionClosure, void)},
		{"processReply", "(Lcom/sun/jndi/ldap/LdapRequest;Lcom/sun/jndi/ldap/LdapResult;I)Lcom/sun/jndi/ldap/LdapResult;", nullptr, $PRIVATE, $method(LdapClient, processReply, $LdapResult*, $LdapRequest*, $LdapResult*, int32_t), "java.io.IOException,javax.naming.NamingException"},
		{"processUnsolicited", "(Lcom/sun/jndi/ldap/BerDecoder;)V", nullptr, 0, $method(LdapClient, processUnsolicited, void, $BerDecoder*)},
		{"removeUnsolicited", "(Lcom/sun/jndi/ldap/LdapCtx;)V", nullptr, 0, $method(LdapClient, removeUnsolicited, void, $LdapCtx*)},
		{"search", "(Ljava/lang/String;IIIIZ[Ljava/lang/String;Ljava/lang/String;I[Ljavax/naming/ldap/Control;Ljava/util/Hashtable;ZI)Lcom/sun/jndi/ldap/LdapResult;", "(Ljava/lang/String;IIIIZ[Ljava/lang/String;Ljava/lang/String;I[Ljavax/naming/ldap/Control;Ljava/util/Hashtable<Ljava/lang/String;Ljava/lang/Boolean;>;ZI)Lcom/sun/jndi/ldap/LdapResult;", 0, $method(LdapClient, search, $LdapResult*, $String*, int32_t, int32_t, int32_t, int32_t, bool, $StringArray*, $String*, int32_t, $ControlArray*, $Hashtable*, bool, int32_t), "java.io.IOException,javax.naming.NamingException"},
		{"usingSaslStreams", "()Z", nullptr, 0, $method(LdapClient, usingSaslStreams, bool)},
		{}
	};
	$ClassInfo classInfo$$ = {
	...
	$loadClass(LdapClient, name, initialize, &classInfo$$, LdapClient::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(LdapClient);
	});
	return class$;
}
