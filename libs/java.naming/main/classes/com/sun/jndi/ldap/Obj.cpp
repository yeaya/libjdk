...
$Attributes* Obj::encodeObject(char16_t separator, Object$* obj, $Attributes* attrs$renamed, $Attribute* objectClass, bool cloned) {
	...
	if (!var$0) {
		bool var$1 = objectClass->size() == 1;
		var$0 = var$1 && objectClass->contains("top"_s);
	}
	bool structural = (var$0);
	if (structural) {
		objectClass->add(Obj::JAVA_OBJECT_CLASSES->get(Obj::STRUCTURAL));
	}
	if ($instanceOf($Referenceable, obj)) {
		objectClass->add(Obj::JAVA_OBJECT_CLASSES->get(Obj::BASE_OBJECT));
		objectClass->add(Obj::JAVA_OBJECT_CLASSES->get(Obj::REF_OBJECT));
		if (!cloned) {
			$assign(attrs, $cast($Attributes, $nc(attrs)->clone()));
		}
		$nc(attrs)->put(objectClass);
		return (encodeReference(separator, $($cast($Referenceable, obj)->getReference()), attrs, obj));
		...
		objectClass->add(Obj::JAVA_OBJECT_CLASSES->get(Obj::BASE_OBJECT));
		bool var$2 = objectClass->contains(Obj::JAVA_OBJECT_CLASSES->get(Obj::MAR_OBJECT));
		if (!(var$2 || objectClass->contains(Obj::JAVA_OBJECT_CLASSES_LOWER->get(Obj::MAR_OBJECT)))) {
			objectClass->add(Obj::JAVA_OBJECT_CLASSES->get(Obj::SER_OBJECT));
		}
		if (!cloned) {
			$assign(attrs, $cast($Attributes, $nc(attrs)->clone()));
		}
		$nc(attrs)->put(objectClass);
		attrs->put($$new($BasicAttribute, Obj::JAVA_ATTRIBUTES->get(Obj::SERIALIZED_DATA), $(serializeObject(obj))));
		if (attrs->get(Obj::JAVA_ATTRIBUTES->get(Obj::CLASSNAME)) == nullptr) {
			attrs->put(Obj::JAVA_ATTRIBUTES->get(Obj::CLASSNAME), $($of(obj)->getClass()->getName()));
		}
		if (attrs->get(Obj::JAVA_ATTRIBUTES->get(Obj::TYPENAME)) == nullptr) {
			$var($Attribute, tAttr, $LdapCtxFactory::createTypeNameAttr($of(obj)->getClass()));
			if (tAttr != nullptr) {
				attrs->put(tAttr);
			}
		}
...
$StringArray* Obj::getCodebases($Attribute* codebaseAttr) {
	...
	if (codebaseAttr == nullptr) {
		...
		$var($StringTokenizer, parser, $new($StringTokenizer, $$cast($String, codebaseAttr->get())));
...
$Object* Obj::decodeObject($Attributes* attrs) {
	...
	$var($StringArray, codebases, getCodebases($($nc(attrs)->get(Obj::JAVA_ATTRIBUTES->get(Obj::CODEBASE)))));
	try {
		if (($assign(attr, attrs->get(Obj::JAVA_ATTRIBUTES->get(Obj::SERIALIZED_DATA)))) != nullptr) {
			...
			return deserializeObject($$cast($bytes, $nc(attr)->get()), cl);
		} else if (($assign(attr, attrs->get(Obj::JAVA_ATTRIBUTES->get(Obj::REMOTE_LOC)))) != nullptr) {
			$var($String, var$0, $cast($String, $$nc(attrs->get(Obj::JAVA_ATTRIBUTES->get(Obj::CLASSNAME)))->get()));
			return decodeRmiObject(var$0, $$cast($String, $nc(attr)->get()), codebases);
		}
		$assign(attr, attrs->get(Obj::JAVA_ATTRIBUTES->get(Obj::OBJECT_CLASS)));
		bool var$1 = attr != nullptr;
		if (var$1) {
			bool var$2 = attr->contains(Obj::JAVA_OBJECT_CLASSES->get(Obj::REF_OBJECT));
			var$1 = var$2 || attr->contains(Obj::JAVA_OBJECT_CLASSES_LOWER->get(Obj::REF_OBJECT));
		}
		if (var$1) {
...
$Attributes* Obj::encodeReference(char16_t separator, $Reference* ref, $Attributes* attrs, Object$* orig) {
	...
	if (($assign(s, $nc(ref)->getClassName())) != nullptr) {
		$nc(attrs)->put($$new($BasicAttribute, Obj::JAVA_ATTRIBUTES->get(Obj::CLASSNAME), s));
	}
	if (($assign(s, ref->getFactoryClassName())) != nullptr) {
		$nc(attrs)->put($$new($BasicAttribute, Obj::JAVA_ATTRIBUTES->get(Obj::FACTORY), s));
	}
	if (($assign(s, ref->getFactoryClassLocation())) != nullptr) {
		$nc(attrs)->put($$new($BasicAttribute, Obj::JAVA_ATTRIBUTES->get(Obj::CODEBASE), s));
	}
	if (orig != nullptr && $nc(attrs)->get(Obj::JAVA_ATTRIBUTES->get(Obj::TYPENAME)) != nullptr) {
		$var($Attribute, tAttr, $LdapCtxFactory::createTypeNameAttr($of(orig)->getClass()));
		if (tAttr != nullptr) {
			attrs->put(tAttr);
		}
	}
	int32_t count = ref->size();
	if (count > 0) {
		$var($Attribute, refAttr, $new($BasicAttribute, Obj::JAVA_ATTRIBUTES->get(Obj::REF_ADDR)));
		...
		for (int32_t i = 0; i < count; ++i) {
			...
			if ($instanceOf($StringRefAddr, refAddr)) {
				...
				var$0->append(""_s);
				var$0->append(separator);
				var$0->append(i);
				var$0->append(separator);
				var$0->append($(refAddr->getType()));
				var$0->append(separator);
				var$0->append($(refAddr->getContent()));
				refAttr->add($$str(var$0));
			} else {
				if (encoder == nullptr) {
				...
				var$1->append(""_s);
				var$1->append(separator);
				var$1->append(i);
				var$1->append(separator);
				var$1->append($($nc(refAddr)->getType()));
				var$1->append(separator);
				var$1->append(separator);
				var$1->append($($nc(encoder)->encodeToString($(serializeObject(refAddr)))));
				refAttr->add($$str(var$1));
			}
		}
...
$Reference* Obj::decodeReference($Attributes* attrs, $StringArray* codebases) {
	...
	if (($assign(attr, $nc(attrs)->get(Obj::JAVA_ATTRIBUTES->get(Obj::CLASSNAME)))) != nullptr) {
		$assign(className, $cast($String, $nc(attr)->get()));
	} else {
		$throwNew($InvalidAttributesException, $$str({Obj::JAVA_ATTRIBUTES->get(Obj::CLASSNAME), " attribute is required"_s}));
	}
	if (($assign(attr, attrs->get(Obj::JAVA_ATTRIBUTES->get(Obj::FACTORY)))) != nullptr) {
		$assign(factory, $cast($String, $nc(attr)->get()));
	}
	$var($Reference, ref, $new($Reference, className, factory, (codebases != nullptr ? codebases->get(0) : ($String*)nullptr)));
	if (($assign(attr, attrs->get(Obj::JAVA_ATTRIBUTES->get(Obj::REF_ADDR)))) != nullptr) {
		...
		{
			$var($NamingEnumeration, vals, attr->getAll());
			for (; $nc(vals)->hasMore();) {
				$assign(val, $cast($String, vals->next()));
				if ($nc(val)->length() == 0) {
					$throwNew($InvalidAttributeValueException, $$str({"malformed "_s, Obj::JAVA_ATTRIBUTES->get(Obj::REF_ADDR), " attribute - empty attribute value"_s}));
				}
				separator = val->charAt(0);
				start = 1;
				if ((sep = val->indexOf(separator, start)) < 0) {
					$throwNew($InvalidAttributeValueException, $$str({"malformed "_s, Obj::JAVA_ATTRIBUTES->get(Obj::REF_ADDR), " attribute - separator \'"_s, $$str(separator), "\'not found"_s}));
				}
				if (($assign(posnStr, val->substring(start, sep))) == nullptr) {
					$throwNew($InvalidAttributeValueException, $$str({"malformed "_s, Obj::JAVA_ATTRIBUTES->get(Obj::REF_ADDR), " attribute - empty RefAddr position"_s}));
				}
				try {
					posn = $Integer::parseInt(posnStr);
				} catch ($NumberFormatException& nfe) {
					$throwNew($InvalidAttributeValueException, $$str({"malformed "_s, Obj::JAVA_ATTRIBUTES->get(Obj::REF_ADDR), " attribute - RefAddr position not an integer"_s}));
				}
				start = sep + 1;
				if ((sep = val->indexOf(separator, start)) < 0) {
					$throwNew($InvalidAttributeValueException, $$str({"malformed "_s, Obj::JAVA_ATTRIBUTES->get(Obj::REF_ADDR), " attribute - RefAddr type not found"_s}));
				}
				if (($assign(type, val->substring(start, sep))) == nullptr) {
					$throwNew($InvalidAttributeValueException, $$str({"malformed "_s, Obj::JAVA_ATTRIBUTES->get(Obj::REF_ADDR), " attribute - empty RefAddr type"_s}));
				}
				start = sep + 1;
				if (start == val->length()) {
...
$Object* Obj::deserializeObject($bytes* obj, $ClassLoader* cl) {
	...
	try {
		...
		try {
			$var($ObjectInputStream, deserial, cl == nullptr ? $new($ObjectInputStream, bytes) : $cast($ObjectInputStream, $new($Obj$LoaderInputStream, bytes, cl)));
			$var($Throwable, var$0, nullptr);
...
$Attributes* Obj::determineBindAttrs(char16_t separator, Object$* obj$renamed, $Attributes* attrs$renamed, bool cloned, $Name* name, $Context* ctx, $Hashtable* env) {
	...
	if (attrs == nullptr || attrs->size() == 0) {
		...
		cloned = true;
		$assign(objectClass, $new($BasicAttribute, "objectClass"_s, "top"_s));
	} else {
		...
		if (objectClass == nullptr) {
			...
			$assign(objectClass, $cast($Attribute, objectClass->clone()));
		}
	}
...
$Class* Obj::load$($String* name, bool initialize) {
	$CompoundAttribute REMOTE_LOCfieldAnnotations$$[] = {
	...
	$FieldInfo fieldInfos$$[] = {
		...
		{"REMOTE_LOC", "I", nullptr, $PRIVATE | $STATIC | $FINAL | $DEPRECATED, $constField(Obj, REMOTE_LOC), REMOTE_LOCfieldAnnotations$$},
	...
	$loadClass(Obj, name, initialize, &classInfo$$, Obj::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Obj);
	});
	return class$;
}
