...
$Hashtable* ResourceManager::getInitialEnvironment($Hashtable* env$renamed) {
	...
	for (int32_t i = 0; i < $nc(props)->length; ++i) {
		...
		if (val == nullptr) {
			$assign(val, (jndiSysProps != nullptr) ? jndiSysProps->get(i) : ResourceManager::helper->getJndiProperty(i));
		}
		if (val != nullptr) {
			(env)->put(props->get(i), val);
		}
	}
...
$String* ResourceManager::getProperty($String* propName, $Hashtable* env, $Context* ctx, bool concat) {
	$init(ResourceManager);
	$useLocalObjectStack();
	$var($String, val1, (env != nullptr) ? $cast($String, env->get(propName)) : ($String*)nullptr);
...
$FactoryEnumeration* ResourceManager::getFactories($String* propName, $Hashtable* env, $Context* ctx) {
	...
	$synchronized(ResourceManager::factoryCache) {
		$assign(perLoaderCache, $cast($Map, ResourceManager::factoryCache->get(loader)));
		if (perLoaderCache == nullptr) {
			$assign(perLoaderCache, $new($HashMap, 11));
			ResourceManager::factoryCache->put(loader, perLoaderCache);
		}
	}
	$synchronized(perLoaderCache) {
		$var($List, factories, $cast($List, perLoaderCache->get(facProp)));
		if (factories != nullptr) {
			...
			while (parser->hasMoreTokens()) {
				try {
					...
					$Class* c = ResourceManager::helper->loadClass(className, loader);
...
$Object* ResourceManager::getFactory($String* propName, $Hashtable* env, $Context* ctx, $String* classSuffix, $String* defaultPkgPrefix) {
	...
	$synchronized(ResourceManager::urlFactoryCache) {
		$assign(perLoaderCache, $cast($Map, ResourceManager::urlFactoryCache->get(loader)));
		if (perLoaderCache == nullptr) {
			$assign(perLoaderCache, $new($HashMap, 11));
			ResourceManager::urlFactoryCache->put(loader, perLoaderCache);
		}
	}
	$synchronized(perLoaderCache) {
		$var($Object, factory, nullptr);
		$var($WeakReference, factoryRef, $cast($WeakReference, perLoaderCache->get(key)));
		...
		while (factory == nullptr && parser->hasMoreTokens()) {
			...
			try {
				$var($Object, tmp, $nc(ResourceManager::helper->loadClass(className, loader))->newInstance());
				$assign(factory, tmp);
...
$Hashtable* ResourceManager::getProviderResource(Object$* obj) {
	...
	$synchronized(ResourceManager::propertiesCache) {
		...
		$var($Hashtable, props, $cast($Hashtable, ResourceManager::propertiesCache->get(c)));
		if (props != nullptr) {
			return props;
		}
		...
		if (istream != nullptr) {
			try {
				$cast($Properties, props)->load(istream);
			} catch ($IOException& e) {
		...
		ResourceManager::propertiesCache->put(c, props);
		return props;
	}
}
...
$Hashtable* ResourceManager::getApplicationResources() {
	...
	$synchronized(ResourceManager::propertiesCache) {
		$var($Hashtable, result, $cast($Hashtable, ResourceManager::propertiesCache->get(cl)));
		if (result != nullptr) {
			return result;
		}
		try {
			$var($NamingEnumeration, resources, ResourceManager::helper->getResources(cl, ResourceManager::APP_RESOURCE_FILE_NAME));
			...
			try {
				...
				while ($nc(resources)->hasMore()) {
					$$sure($InputStream, resources->next())->close();
				}
			}
			if (var$0 != nullptr) {
				$throw(var$0);
			}
			$var($InputStream, istream, ResourceManager::helper->getJavaHomeConfStream(ResourceManager::JRE_CONF_PROPERTY_FILE_NAME));
			if (istream != nullptr) {
		...
		ResourceManager::propertiesCache->put(cl, result);
		return result;
	}
}
...
bool ResourceManager::isListProperty($String* prop$renamed) {
	...
	for (int32_t i = 0; i < ResourceManager::listProperties->length; ++i) {
		if (prop == ResourceManager::listProperties->get(i)) {
			return true;
		}
	}
	return false;
...
$Class* ResourceManager::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PRIVATE, $method(ResourceManager, init$, void)},
		{"getApplicationResources", "()Ljava/util/Hashtable;", "()Ljava/util/Hashtable<-Ljava/lang/String;Ljava/lang/Object;>;", $PRIVATE | $STATIC, $staticMethod(ResourceManager, getApplicationResources, $Hashtable*), "javax.naming.NamingException"},
		{"getFactories", "(Ljava/lang/String;Ljava/util/Hashtable;Ljavax/naming/Context;)Lcom/sun/naming/internal/FactoryEnumeration;", "(Ljava/lang/String;Ljava/util/Hashtable<**>;Ljavax/naming/Context;)Lcom/sun/naming/internal/FactoryEnumeration;", $PUBLIC | $STATIC, $staticMethod(ResourceManager, getFactories, $FactoryEnumeration*, $String*, $Hashtable*, $Context*), "javax.naming.NamingException"},
		{"getFactory", "(Ljava/lang/String;Ljava/util/Hashtable;Ljavax/naming/Context;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/Object;", "(Ljava/lang/String;Ljava/util/Hashtable<**>;Ljavax/naming/Context;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/Object;", $PUBLIC | $STATIC, $staticMethod(ResourceManager, getFactory, $Object*, $String*, $Hashtable*, $Context*, $String*, $String*), "javax.naming.NamingException"},
		{"getInitialEnvironment", "(Ljava/util/Hashtable;)Ljava/util/Hashtable;", "(Ljava/util/Hashtable<**>;)Ljava/util/Hashtable<**>;", $PUBLIC | $STATIC, $staticMethod(ResourceManager, getInitialEnvironment, $Hashtable*, $Hashtable*), "javax.naming.NamingException"},
		{"getProperty", "(Ljava/lang/String;Ljava/util/Hashtable;Ljavax/naming/Context;Z)Ljava/lang/String;", "(Ljava/lang/String;Ljava/util/Hashtable<**>;Ljavax/naming/Context;Z)Ljava/lang/String;", $PUBLIC | $STATIC, $staticMethod(ResourceManager, getProperty, $String*, $String*, $Hashtable*, $Context*, bool), "javax.naming.NamingException"},
		{"getProviderResource", "(Ljava/lang/Object;)Ljava/util/Hashtable;", "(Ljava/lang/Object;)Ljava/util/Hashtable<-Ljava/lang/String;Ljava/lang/Object;>;", $PRIVATE | $STATIC, $staticMethod(ResourceManager, getProviderResource, $Hashtable*, Object$*), "javax.naming.NamingException"},
		{"isListProperty", "(Ljava/lang/String;)Z", nullptr, $PRIVATE | $STATIC, $staticMethod(ResourceManager, isListProperty, bool, $String*)},
		{"mergeTables", "(Ljava/util/Hashtable;Ljava/util/Hashtable;)V", "(Ljava/util/Hashtable<-Ljava/lang/String;Ljava/lang/Object;>;Ljava/util/Hashtable<-Ljava/lang/String;Ljava/lang/Object;>;)V", $PRIVATE | $STATIC, $staticMethod(ResourceManager, mergeTables, void, $Hashtable*, $Hashtable*)},
		{}
	};
	$ClassInfo classInfo$$ = {
