...
SchemaFactory* SchemaFactory::newInstance($String* schemaLanguage, $String* factoryClassName, $ClassLoader* classLoader) {
	...
	if ($nc(f)->isSchemaLanguageSupported(schemaLanguage)) {
		...
		$throwNew($IllegalArgumentException, $$str({"Factory "_s, $(f->getClass()->getName()), " does not implement the schema language specified by: "_s, schemaLanguage}));
	}
}
...
$Schema* SchemaFactory::newSchema($URL* schema) {
	$useLocalObjectStack();
	return newSchema($$new($StreamSource, $($nc(schema)->toExternalForm())));
}
...
$Class* SchemaFactory::load$($String* name, bool initialize) {
	...
	$loadClass(SchemaFactory, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SchemaFactory);
	});
	return class$;
}
