...
$Class* CatalogFeatures$$Lambda$lambda$setProperties$0::load$($String* name, bool initialize) {
	...
	$loadClass(CatalogFeatures$$Lambda$lambda$setProperties$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CatalogFeatures$$Lambda$lambda$setProperties$0);
	});
	return class$;
}
...
void CatalogFeatures::setProperties($CatalogFeatures$Builder* builder) {
	$useLocalObjectStack();
	$$nc($$nc($nc($nc(builder)->values)->entrySet())->stream())->forEach($$new(CatalogFeatures$$Lambda$lambda$setProperties$0, this));
}
...
void CatalogFeatures::setProperty($CatalogFeatures$Feature* feature, $CatalogFeatures$State* state, $String* value) {
	...
	if (value != nullptr && value->length() != 0) {
		...
		if ($nc(this->states)->get(index) == nullptr || $nc(state)->compareTo(this->states->get(index)) >= 0) {
...
void CatalogFeatures::clinit$($Class* clazz) {
...
$Class* CatalogFeatures::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("javax.xml.catalog.CatalogFeatures$$Lambda$lambda$setProperties$0")) {
	...
	$loadClass(CatalogFeatures, name, initialize, &classInfo$$, CatalogFeatures::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(CatalogFeatures);
	});
	return class$;
}
