...
void Util::validateUriSyntax($URI* uri) {
	...
	if (!$nc(uri)->isAbsolute()) {
		$CatalogMessages::reportIAE($CatalogMessages::ERR_URI_NOTABSOLUTE, $$new($ObjectArray, {uri}), nullptr);
	}
	try {
		uri->toURL();
	} catch ($MalformedURLException& ex) {
		$CatalogMessages::reportIAE($CatalogMessages::ERR_URI_NOTVALIDURL, $$new($ObjectArray, {uri}), nullptr);
	}
}
...
bool Util::isFileUri($URI* uri) {
	$init(Util);
	$useLocalObjectStack();
	bool var$0 = Util::SCHEME_FILE->equals($($nc(uri)->getScheme()));
	if (var$0 || Util::SCHEME_JAR->equals($(uri->getScheme()))) {
		return true;
	}
	return false;
}
		...
		if (nullptr != uri->getScheme()) {
			...
			}
			{
				...
				case 1:
					...
					if (openJarFile) {
						$var($String, jarFile, tempUri->substring(Util::SCHEME_JARFILE->length(), pos));
