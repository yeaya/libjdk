...
void CatalogMessages::reportNPEOnNull($String* name, Object$* value) {
	...
	$useLocalObjectStack();
	if (value == nullptr) {
		$throwNew($NullPointerException, $(formatMessage(CatalogMessages::ERR_NULL_ARGUMENT, $$new($ObjectArray, {name}))));
	}
}
...
void CatalogMessages::clinit$($Class* clazz) {
...
$Class* CatalogMessages::load$($String* name, bool initialize) {
	...
	$loadClass(CatalogMessages, name, initialize, &classInfo$$, CatalogMessages::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(CatalogMessages);
	});
	return class$;
}
