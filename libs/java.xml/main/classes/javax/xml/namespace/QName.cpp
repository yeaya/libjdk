...
bool QName::equals(Object$* objectToTest) {
	...
	return var$0 && $nc(this->namespaceURI)->equals(qName->namespaceURI);
}
...
QName* QName::valueOf($String* qNameAsString) {
	...
	if (qNameAsString->charAt(0) != u'{') {
		$init($XMLConstants);
		return $new(QName, $XMLConstants::NULL_NS_URI, qNameAsString, $XMLConstants::DEFAULT_NS_PREFIX);
	}
	$init($XMLConstants);
	if (qNameAsString->startsWith($$str({"{"_s, $XMLConstants::NULL_NS_URI, "}"_s}))) {
	...
	int32_t endOfNamespaceURI = qNameAsString->indexOf(u'}');
