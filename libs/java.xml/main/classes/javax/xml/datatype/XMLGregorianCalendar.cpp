...
int32_t XMLGregorianCalendar::getMillisecond() {
	...
	return $($$nc(getFractionalSecond())->movePointRight(3))->intValue();
}
...
$Class* XMLGregorianCalendar::load$($String* name, bool initialize) {
	...
	$loadClass(XMLGregorianCalendar, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XMLGregorianCalendar);
	});
	return class$;
}
