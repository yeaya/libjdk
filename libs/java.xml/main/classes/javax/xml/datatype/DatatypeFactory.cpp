...
$Duration* DatatypeFactory::newDurationDayTime(bool isPositive, $BigInteger* day, $BigInteger* hour, $BigInteger* minute, $BigInteger* second) {
	return newDuration(isPositive, nullptr, nullptr, day, hour, minute, (second != nullptr) ? $$new($BigDecimal, second) : ($BigDecimal*)nullptr);
}

$Duration* DatatypeFactory::newDurationDayTime(bool isPositive, int32_t day, int32_t hour, int32_t minute, int32_t second) {
	$useLocalObjectStack();
	$var($BigInteger, var$0, $BigInteger::valueOf((int64_t)day));
	$var($BigInteger, var$1, $BigInteger::valueOf((int64_t)hour));
	$var($BigInteger, var$2, $BigInteger::valueOf((int64_t)minute));
	return newDurationDayTime(isPositive, var$0, var$1, var$2, $($BigInteger::valueOf((int64_t)second)));
}
...
$Duration* DatatypeFactory::newDurationYearMonth(int64_t durationInMilliseconds) {
	...
	$var($BigInteger, years, $cast($BigInteger, fullDuration->getField($DatatypeConstants::YEARS)));
	if (years == nullptr) {
		$init($BigInteger);
...
$Duration* DatatypeFactory::newDurationYearMonth(bool isPositive, $BigInteger* year, $BigInteger* month) {
	return newDuration(isPositive, year, month, nullptr, nullptr, nullptr, nullptr);
}

$Duration* DatatypeFactory::newDurationYearMonth(bool isPositive, int32_t year, int32_t month) {
	$useLocalObjectStack();
	$var($BigInteger, var$0, $BigInteger::valueOf((int64_t)year));
	return newDurationYearMonth(isPositive, var$0, $($BigInteger::valueOf((int64_t)month)));
}
...
$XMLGregorianCalendar* DatatypeFactory::newXMLGregorianCalendar(int32_t year, int32_t month, int32_t day, int32_t hour, int32_t minute, int32_t second, int32_t millisecond, int32_t timezone) {
	...
	if (millisecond != $DatatypeConstants::FIELD_UNDEFINED) {
		...
		$assign(realMillisecond, $($BigDecimal::valueOf((int64_t)millisecond))->movePointLeft(3));
	}
...
$XMLGregorianCalendar* DatatypeFactory::newXMLGregorianCalendarTime(int32_t hours, int32_t minutes, int32_t seconds, $BigDecimal* fractionalSecond, int32_t timezone) {
	return newXMLGregorianCalendar(nullptr, $DatatypeConstants::FIELD_UNDEFINED, $DatatypeConstants::FIELD_UNDEFINED, hours, minutes, seconds, fractionalSecond, timezone);
}
...
$XMLGregorianCalendar* DatatypeFactory::newXMLGregorianCalendarTime(int32_t hours, int32_t minutes, int32_t seconds, int32_t milliseconds, int32_t timezone) {
	...
	if (milliseconds != $DatatypeConstants::FIELD_UNDEFINED) {
		...
		$assign(realMilliseconds, $($BigDecimal::valueOf((int64_t)milliseconds))->movePointLeft(3));
	}
	return newXMLGregorianCalendarTime(hours, minutes, seconds, realMilliseconds, timezone);
}

void DatatypeFactory::clinit$($Class* clazz) {
...
$Class* DatatypeFactory::load$($String* name, bool initialize) {
	...
	$loadClass(DatatypeFactory, name, initialize, &classInfo$$, DatatypeFactory::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(DatatypeFactory);
	});
	return class$;
}
