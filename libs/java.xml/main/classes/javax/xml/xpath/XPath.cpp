...
$Object* XPath::evaluateExpression($String* expression, Object$* item, $Class* type) {
	$useLocalObjectStack();
	return $nc(type)->cast($(evaluate(expression, item, $($XPathEvaluationResult$XPathResultType::getQNameType(type)))));
}
...
$Object* XPath::evaluateExpression($String* expression, $InputSource* source, $Class* type) {
	$useLocalObjectStack();
	return $nc(type)->cast($(evaluate(expression, source, $($XPathEvaluationResult$XPathResultType::getQNameType(type)))));
}
