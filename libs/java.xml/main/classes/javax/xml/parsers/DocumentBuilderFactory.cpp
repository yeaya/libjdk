...
void DocumentBuilderFactory::clinit$($Class* clazz) {
...
$Class* DocumentBuilderFactory::load$($String* name, bool initialize) {
	...
	$loadClass(DocumentBuilderFactory, name, initialize, &classInfo$$, DocumentBuilderFactory::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(DocumentBuilderFactory);
	});
	return class$;
}
