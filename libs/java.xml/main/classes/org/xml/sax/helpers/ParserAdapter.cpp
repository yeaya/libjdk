...
void ParserAdapter::startElement($String* qName, $AttributeList* qAtts) {
	...
	for (int32_t i = 0; i < length; ++i) {
		...
		try {
			...
			$nc(this->atts)->addAttribute($nc(attName)->get(0), $nc(attName)->get(1), $nc(attName)->get(2), type, value);
	...
	if (this->contentHandler != nullptr) {
		...
		$nc(this->contentHandler)->startElement($nc(name)->get(0), $nc(name)->get(1), $nc(name)->get(2), this->atts);
	}
}
...
void ParserAdapter::endElement($String* qName) {
	...
	if (!this->namespaces) {
		if (this->contentHandler != nullptr) {
			this->contentHandler->endElement(""_s, ""_s, $($nc(qName)->intern()));
		}
		return;
	}
	$var($StringArray, names, processName(qName, false, false));
	if (this->contentHandler != nullptr) {
		this->contentHandler->endElement($nc(names)->get(0), $nc(names)->get(1), $nc(names)->get(2));
...
void ParserAdapter::clinit$($Class* clazz) {
...
$Class* ParserAdapter::load$($String* name, bool initialize) {
	...
	$loadClass(ParserAdapter, name, initialize, &classInfo$$, ParserAdapter::clinit$, []($Class* clazz) -> $Object* {
