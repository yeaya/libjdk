	...
	case $XMLEvent::START_DOCUMENT:
		{
			...
			try {
				...
				if ($XMLStreamWriterBase::class$->isAssignableFrom($nc(this->fStreamWriter)->getClass())) {
					...
					$cast($XMLStreamWriterBase, this->fStreamWriter)->writeStartDocument(var$0, var$1, var$2, startDocument->standaloneSet());
				} else {
					$var($String, var$3, startDocument->getCharacterEncodingScheme());
					this->fStreamWriter->writeStartDocument(var$3, $(startDocument->getVersion()));
				}
...
$Class* XMLEventWriterImpl::load$($String* name, bool initialize) {
	...
	$loadClass(XMLEventWriterImpl, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XMLEventWriterImpl);
	});
	return class$;
}
