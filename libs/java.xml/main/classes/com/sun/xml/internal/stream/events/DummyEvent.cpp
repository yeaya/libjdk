...
void DummyEvent::init$() {
	$set(this, fLocation, $cast($Location, DummyEvent::nowhere));
}
...
void DummyEvent::clinit$($Class* clazz) {
...
$Class* DummyEvent::load$($String* name, bool initialize) {
	...
	$loadClass(DummyEvent, name, initialize, &classInfo$$, DummyEvent::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(DummyEvent);
	});
	return class$;
}
