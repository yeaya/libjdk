...
int32_t DTDGrammar::getAttributeDeclIndex(int32_t elementDeclIndex, $String* attributeDeclName) {
	...
	while (attDefIndex != -1) {
		...
		if ($nc($nc(this->fAttributeDecl)->name)->rawname == attributeDeclName || $nc(attributeDeclName)->equals(this->fAttributeDecl->name->rawname)) {
			return attDefIndex;
		}
...
bool DTDGrammar::getElementDecl(int32_t elementDeclIndex, $XMLElementDecl* elementDecl) {
	...
	$nc(elementDecl->simpleType)->defaultType = -1;
...
void DTDGrammar::ensureElementDeclCapacity(int32_t chunk) {
	...
	if (chunk >= $nc(this->fElementDeclName)->length) {
		$set(this, fElementDeclName, resize(this->fElementDeclName, this->fElementDeclName->length * 2));
...
void DTDGrammar::ensureAttributeDeclCapacity(int32_t chunk) {
	...
	if (chunk >= $nc(this->fAttributeDeclName)->length) {
		$set(this, fAttributeDeclName, resize(this->fAttributeDeclName, this->fAttributeDeclName->length * 2));
