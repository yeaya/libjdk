...
$BufferAllocator* ThreadLocalBufferAllocator::getBufferAllocator() {
	...
	$var($SoftReference, sr, $cast($SoftReference, ThreadLocalBufferAllocator::TL->get()));
	if (sr != nullptr) {
...
void ThreadLocalBufferAllocator::clinit$($Class* clazz) {
...
$Class* ThreadLocalBufferAllocator::load$($String* name, bool initialize) {
	...
	$loadClass(ThreadLocalBufferAllocator, name, initialize, &classInfo$$, ThreadLocalBufferAllocator::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ThreadLocalBufferAllocator);
	});
	return class$;
}
