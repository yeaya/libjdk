...
$Class* DTMDefaultBaseTraversers$PrecedingTraverser::load$($String* name, bool initialize) {
	...
	$loadClass(DTMDefaultBaseTraversers$PrecedingTraverser, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DTMDefaultBaseTraversers$PrecedingTraverser);
	});
	return class$;
}
