		...
		if ((nullptr != source) && $instanceOf($DOMSource, source)) {
			...
			if (isSAXSource || isStreamSource) {
				...
				try {
					...
					addDTM(dtm, dtmPos, 0);
					bool haveXercesParser = (nullptr != reader) && ($$nc(reader->getClass()->getName())->equals("com.sun.org.apache.xerces.internal.parsers.SAXParser"_s));
					if (haveXercesParser) {
						incremental = true;
					}
					if (this->m_incremental && incremental) {
						...
						if (nullptr == $nc(reader)->getErrorHandler()) {
						...
						$nc(reader)->setContentHandler(dtm);
						reader->setDTDHandler(dtm);
...
int32_t DTMManagerDefault::getDTMHandleFromNode($Node* node) {
	$synchronized(this) {
		...
		if ($instanceOf($DTMNodeProxy, node)) {
			...
			$var($Node, root, node);
			$var($Node, p, ($nc(root)->getNodeType() == $Node::ATTRIBUTE_NODE) ? $cast($Node, $cast($Attr, root)->getOwnerElement()) : root->getParentNode());
			for (; p != nullptr; $assign(p, p->getParentNode())) {
				$assign(root, p);
			}
			...
			if ($instanceOf($DOM2DTMdefaultNamespaceDeclarationNode, node)) {
				handle = $nc(dtm)->getHandleOfNode($($cast($Attr, node)->getOwnerElement()));
				$var($String, var$0, node->getNamespaceURI());
				handle = dtm->getAttributeNode(handle, var$0, $(node->getLocalName()));
			} else {
...
$XMLReader* DTMManagerDefault::getXMLReader($Source* inputSource) {
	$synchronized(this) {
		$useLocalObjectStack();
		try {
			$var($XMLReader, reader, ($instanceOf($SAXSource, inputSource)) ? $cast($SAXSource, inputSource)->getXMLReader() : ($XMLReader*)nullptr);
			if (nullptr == reader) {
...
int32_t DTMManagerDefault::getDTMIdentity($DTM* dtm) {
	$synchronized(this) {
		...
		if ($instanceOf($DTMDefaultBase, dtm)) {
			...
			if ($equals(dtmdb->getManager(), this)) {
		...
		for (int32_t i = 0; i < n; ++i) {
			$var($DTM, tdtm, this->m_dtms->get(i));
...
bool DTMManagerDefault::release($DTM* dtm, bool shouldHardDelete) {
	$synchronized(this) {
		...
		if ($instanceOf($SAX2DTM, dtm)) {
			$cast($SAX2DTM, dtm)->clearCoRoutine();
		}
		if ($instanceOf($DTMDefaultBase, dtm)) {
			$var($SuballocatedIntVector, ids, $cast($DTMDefaultBase, dtm)->getDTMIDs());
...
$Class* DTMManagerDefault::load$($String* name, bool initialize) {
	...
	$loadClass(DTMManagerDefault, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DTMManagerDefault);
	});
	return class$;
}
