...
void XMLEntityManager::addExternalEntity($String* name, $String* publicId, $String* literalSystemId, $String* baseSystemId$renamed) {
	...
	if (!$nc(this->fEntities)->containsKey(name)) {
		if (baseSystemId == nullptr) {
			...
			if (size == 0 && this->fCurrentEntity != nullptr && this->fCurrentEntity->entityLocation != nullptr) {
			...
			for (int32_t i = size - 1; i >= 0; --i) {
				...
				if ($nc(externalEntity)->entityLocation != nullptr && externalEntity->entityLocation->getExpandedSystemId() != nullptr) {
					$assign(baseSystemId, externalEntity->entityLocation->getExpandedSystemId());
					break;
				}
			}
		}
		$var($Entity, entity, $new($Entity$ExternalEntity, name, $$cast($XMLResourceIdentifierImpl, $new($XMLEntityDescriptionImpl, name, publicId, literalSystemId, baseSystemId, $(expandSystemId(literalSystemId, baseSystemId, false)))), nullptr, this->fInExternalSubset));
...
void XMLEntityManager::addUnparsedEntity($String* name, $String* publicId, $String* systemId, $String* baseSystemId, $String* notation) {
	$useLocalObjectStack();
	if (!$nc(this->fEntities)->containsKey(name)) {
		$var($Entity$ExternalEntity, entity, $new($Entity$ExternalEntity, name, $$cast($XMLResourceIdentifierImpl, $new($XMLEntityDescriptionImpl, name, publicId, systemId, baseSystemId, nullptr)), notation, this->fInExternalSubset));
		this->fEntities->put(name, entity);
	} else if (this->fWarnDuplicateEntityDef) {
		$init($XMLMessageFormatter);
		$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "MSG_DUPLICATE_ENTITY_DEFINITION"_s, $$new($ObjectArray, {name}), $XMLErrorReporter::SEVERITY_WARNING);
...
$String* XMLEntityManager::setupCurrentEntity(bool reference, $String* name, $XMLInputSource* xmlInputSource, bool literal, bool isExternal) {
	...
	if (reader == nullptr) {
			...
			if (!($instanceOf($HttpURLConnection, connect))) {
				...
				if ($instanceOf($HTTPInputSource, xmlInputSource)) {
					...
					while ($nc(propIter)->hasNext()) {
						...
						urlConnection->setRequestProperty(var$0, $$cast($String, entry->getValue()));
					}
				...
				$assign(stream, connect->getInputStream());
				if (followRedirects) {
		...
		if (encoding == nullptr) {
			...
			if (count == 4) {
				...
				if (info->hasBOM) {
					if ($nc($XMLEntityManager$EncodingInfo::STR_UTF8)->equals(readerEncoding)) {
						stream->skip(3);
					} else if ($nc($XMLEntityManager$EncodingInfo::STR_UTF16)->equals(readerEncoding)) {
						stream->skip(2);
					}
				}
			...
			} else if ($nc($XMLEntityManager$EncodingInfo::STR_UTF16)->equals(encoding)) {
			...
			} else if ($nc($XMLEntityManager$EncodingInfo::STR_UCS4)->equals(encoding)) {
				$var($ints, b4, $new($ints, 4));
				int32_t count = 0;
				for (; count < 4; ++count) {
					b4->set(count, rewindableStream->readAndBuffer());
			...
			} else if ($nc($XMLEntityManager$EncodingInfo::STR_UCS2)->equals(encoding)) {
				$var($ints, b4, $new($ints, 4));
				int32_t count = 0;
				for (; count < 4; ++count) {
					b4->set(count, rewindableStream->readAndBuffer());
...
$StaxXMLInputSource* XMLEntityManager::resolveEntityAsPerStax($XMLResourceIdentifier* resourceIdentifier) {
	...
	if (baseSystemId == nullptr && this->fCurrentEntity != nullptr && this->fCurrentEntity->entityLocation != nullptr) {
		$assign(baseSystemId, this->fCurrentEntity->entityLocation->getExpandedSystemId());
		if (baseSystemId != nullptr) {
			needExpand = true;
		}
	...
	if (this->fStaxEntityResolver != nullptr) {
		$assign(staxInputSource, this->fStaxEntityResolver->resolveEntity(ri));
		if (staxInputSource != nullptr) {
			this->fISCreatedByResolver = true;
		}
	}
	if (this->fEntityResolver != nullptr) {
		$assign(xmlInputSource, this->fEntityResolver->resolveEntity(ri));
	...
	if (staxInputSource == nullptr && this->fUseCatalog) {
		...
		if (this->fCatalogFile != nullptr) {
			try {
				...
				$init($XMLMessageFormatter);
				$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "CatalogException"_s, $$new($ObjectArray, {$($SecuritySupport::sanitizePath(this->fCatalogFile))}), $XMLErrorReporter::SEVERITY_FATAL_ERROR, e);
			}
		}
	}
	if (staxInputSource == nullptr) {
...
void XMLEntityManager::startEntity(bool isGE, $String* entityName, bool literal) {
	...
	if (external) {
		...
		$assign(extLitSysId, externalEntity->entityLocation != nullptr ? externalEntity->entityLocation->getLiteralSystemId() : ($String*)nullptr);
		$assign(extBaseSysId, externalEntity->entityLocation != nullptr ? externalEntity->entityLocation->getBaseSystemId() : ($String*)nullptr);
		...
		if (unparsed || (general && !this->fExternalGeneralEntities) || (parameter && !this->fExternalParameterEntities) || !this->fSupportDTD || !this->fSupportExternalEntities) {
			if (this->fEntityHandler != nullptr) {
				$nc(this->fResourceIdentifier)->clear();
				$var($String, encoding, nullptr);
				this->fResourceIdentifier->setValues((externalEntity->entityLocation != nullptr ? $(externalEntity->entityLocation->getPublicId()) : ($String*)nullptr), extLitSysId, extBaseSysId, expandedSystemId);
				$nc(this->fEntityAugs)->removeAllItems();
				$init($Constants);
				this->fEntityAugs->putItem($Constants::ENTITY_SKIPPED, $Boolean::TRUE);
				$nc(this->fEntityHandler)->startEntity(entityName, this->fResourceIdentifier, encoding, this->fEntityAugs);
	...
	for (int32_t i = size; i >= 0; --i) {
		$var($Entity, activeEntity, i == size ? $cast($Entity, this->fCurrentEntity) : $cast($Entity, $nc(this->fEntityStack)->get(i)));
		if ($nc(activeEntity)->name == entityName) {
			...
			$init($XMLMessageFormatter);
			$nc(this->fErrorReporter)->reportError($(this->getEntityScanner()), $XMLMessageFormatter::XML_DOMAIN, "RecursiveReference"_s, $$new($ObjectArray, {
				entityName,
				path
			...
			if (this->fEntityHandler != nullptr) {
				...
				$var($String, encoding, nullptr);
				if (external) {
					this->fResourceIdentifier->setValues(($nc(externalEntity)->entityLocation != nullptr ? $(externalEntity->entityLocation->getPublicId()) : ($String*)nullptr), extLitSysId, extBaseSysId, expandedSystemId);
				}
				$nc(this->fEntityAugs)->removeAllItems();
				$init($Constants);
				this->fEntityAugs->putItem($Constants::ENTITY_SKIPPED, $Boolean::TRUE);
	...
	if (external) {
		...
		if (!this->fISCreatedByResolver) {
			if (this->fLoadExternalDTD) {
				...
				if (accessError != nullptr) {
					$var($XMLLocator, var$0, this->getEntityScanner());
					$init($XMLMessageFormatter);
					...
					$nc(this->fErrorReporter)->reportError(var$0, var$1, var$2, $$new($ObjectArray, {
						$($SecuritySupport::sanitizePath(expandedSystemId)),
						accessError
					}), $XMLErrorReporter::SEVERITY_FATAL_ERROR);
				}
			}
		...
		$assign(xmlInputSource, $new($XMLInputSource, nullptr, nullptr, nullptr, reader, nullptr));
	}
...
void XMLEntityManager::startEntity(bool isGE, $String* name, $XMLInputSource* xmlInputSource, bool literal, bool isExternal) {
	...
	if (this->fSecurityManager != nullptr && this->fSecurityManager->isOverLimit(this->entityExpansionIndex, this->fLimitAnalyzer)) {
		this->fSecurityManager->debugPrint(this->fLimitAnalyzer);
		$init($XMLMessageFormatter);
		$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "EntityExpansionLimit"_s, $$new($ObjectArray, {$($nc(this->fSecurityManager)->getLimitValueByIndex(this->entityExpansionIndex))}), $XMLErrorReporter::SEVERITY_FATAL_ERROR);
		this->fEntityExpansionCount = 0;
	}
	if (this->fEntityHandler != nullptr) {
		this->fEntityHandler->startEntity(name, this->fResourceIdentifier, encoding, nullptr);
	}
}
...
$Entity$ScannedEntity* XMLEntityManager::getTopLevelEntity() {
	return $cast($Entity$ScannedEntity, ($nc(this->fEntityStack)->empty() ? ($Entity*)nullptr : $cast($Entity, this->fEntityStack->get(0))));
}
...
void XMLEntityManager::closeReaders() {
	...
	while (!$nc(this->fReaderStack)->isEmpty()) {
		try {
			$$sure($Reader, this->fReaderStack->pop())->close();
		} catch ($IOException& e) {
		}
	}
}
...
void XMLEntityManager::endEntity() {
	;
	$var($Entity$ScannedEntity, entity, $nc(this->fEntityStack)->size() > 0 ? $cast($Entity$ScannedEntity, this->fEntityStack->pop()) : ($Entity$ScannedEntity*)nullptr);
	if (this->fCurrentEntity != nullptr) {
		try {
			if (this->fLimitAnalyzer != nullptr) {
...
void XMLEntityManager::reset($PropertyManager* propertyManager) {
	...
	this->fSupportDTD = $$sure($Boolean, propertyManager->getProperty($XMLInputFactory::SUPPORT_DTD))->booleanValue();
	this->fReplaceEntityReferences = $$sure($Boolean, propertyManager->getProperty($XMLInputFactory::IS_REPLACING_ENTITY_REFERENCES))->booleanValue();
	this->fSupportExternalEntities = $$sure($Boolean, propertyManager->getProperty($XMLInputFactory::IS_SUPPORTING_EXTERNAL_ENTITIES))->booleanValue();
	...
	$init($XMLConstants);
	this->fUseCatalog = $$sure($Boolean, propertyManager->getProperty($XMLConstants::USE_CATALOG))->booleanValue();
	$init($JdkXmlUtils);
...
$StringArray* XMLEntityManager::getRecognizedFeatures() {
	return $cast($StringArray, XMLEntityManager::RECOGNIZED_FEATURES->clone());
}
...
void XMLEntityManager::setProperty($String* propertyId, Object$* value) {
	...
	if (propertyId->equals(XMLEntityManager::XML_SECURITY_PROPERTY_MANAGER)) {
...
$StringArray* XMLEntityManager::getRecognizedProperties() {
	return $cast($StringArray, XMLEntityManager::RECOGNIZED_PROPERTIES->clone());
}

$Boolean* XMLEntityManager::getFeatureDefault($String* featureId) {
	for (int32_t i = 0; i < XMLEntityManager::RECOGNIZED_FEATURES->length; ++i) {
		if ($nc(XMLEntityManager::RECOGNIZED_FEATURES->get(i))->equals(featureId)) {
			return XMLEntityManager::FEATURE_DEFAULTS->get(i);
		}
	}
	return nullptr;
}
...
$Object* XMLEntityManager::getPropertyDefault($String* propertyId) {
	for (int32_t i = 0; i < XMLEntityManager::RECOGNIZED_PROPERTIES->length; ++i) {
		if ($nc(XMLEntityManager::RECOGNIZED_PROPERTIES->get(i))->equals(propertyId)) {
			return XMLEntityManager::PROPERTY_DEFAULTS->get(i);
		}
	}
	return nullptr;
}
		...
		if (i < len) {
			...
			for (i = 0; i < len; ++i) {
				...
				if (b < 0) {
					...
					buffer->append(XMLEntityManager::gHexChs->get(ch & 0x0f));
				} else if ($nc(XMLEntityManager::gNeedEscaping)->get(b)) {
					buffer->append(u'%');
...
$OutputStream* XMLEntityManager::createOutputStream($String* uri) {
	...
	if (var$0) {
		bool var$1 = host == nullptr || host->length() == 0;
		var$0 = var$1 || host->equals("localhost"_s);
	}
	if (var$0) {
...
$String* XMLEntityManager::getPathWithoutEscapes($String* origPath) {
	...
	if (var$0 && origPath->indexOf(u'%') != -1) {
		...
		for (int32_t i = 1; i < size; ++i) {
			...
			result->append((char16_t)$($Integer::valueOf($($nc(token)->substring(0, 2)), 16))->intValue());
...
$String* XMLEntityManager::expandSystemId($String* systemId, $String* baseSystemId, bool strict) {
	...
	if (strict) {
		...
		$var($URI, base, nullptr);
		if (baseSystemId == nullptr || baseSystemId->length() == 0) {
...
$XMLEntityManager$EncodingInfo* XMLEntityManager::getEncodingInfo($bytes* b4, int32_t count) {
	...
	int32_t b0 = $nc(b4)->get(0) & 0xff;
	int32_t b1 = b4->get(1) & 0xff;
	if (b0 == 254 && b1 == 255) {
		$init($XMLEntityManager$EncodingInfo);
...
$Reader* XMLEntityManager::createReader($InputStream* inputStream, $String* encoding$renamed, $Boolean* isBigEndian) {
	...
	$var($Locale, l, $nc(this->fErrorReporter)->getLocale());
	{
		...
		case 4:
			if (isBigEndian != nullptr) {
				...
				$nc(this->fErrorReporter)->reportError($(this->getEntityScanner()), $XMLMessageFormatter::XML_DOMAIN, "EncodingByteOrderUnsupported"_s, $$new($ObjectArray, {encoding}), $XMLErrorReporter::SEVERITY_FATAL_ERROR);
			}
			break;
		case 5:
			if (isBigEndian != nullptr) {
	...
	if (!validIANA || (this->fAllowJavaEncodings && !validJava)) {
		$nc(this->fErrorReporter)->reportError($(this->getEntityScanner()), $XMLMessageFormatter::XML_DOMAIN, "EncodingDeclInvalid"_s, $$new($ObjectArray, {encoding}), $XMLErrorReporter::SEVERITY_FATAL_ERROR);
...
$String* XMLEntityManager::getPublicId() {
	return (this->fCurrentEntity != nullptr && this->fCurrentEntity->entityLocation != nullptr) ? this->fCurrentEntity->entityLocation->getPublicId() : ($String*)nullptr;
}

$String* XMLEntityManager::getExpandedSystemId() {
	$useLocalObjectStack();
	if (this->fCurrentEntity != nullptr) {
		if (this->fCurrentEntity->entityLocation != nullptr && this->fCurrentEntity->entityLocation->getExpandedSystemId() != nullptr) {
			return $nc(this->fCurrentEntity)->entityLocation->getExpandedSystemId();
		} else {
			int32_t size = $nc(this->fEntityStack)->size();
			for (int32_t i = size - 1; i >= 0; --i) {
				$var($Entity$ScannedEntity, externalEntity, $cast($Entity$ScannedEntity, $nc(this->fEntityStack)->get(i)));
...
$String* XMLEntityManager::getLiteralSystemId() {
	$useLocalObjectStack();
	if (this->fCurrentEntity != nullptr) {
		if (this->fCurrentEntity->entityLocation != nullptr && this->fCurrentEntity->entityLocation->getLiteralSystemId() != nullptr) {
			return $nc(this->fCurrentEntity)->entityLocation->getLiteralSystemId();
		} else {
			int32_t size = $nc(this->fEntityStack)->size();
			for (int32_t i = size - 1; i >= 0; --i) {
				$var($Entity$ScannedEntity, externalEntity, $cast($Entity$ScannedEntity, $nc(this->fEntityStack)->get(i)));
				if ($nc(externalEntity)->entityLocation != nullptr && externalEntity->entityLocation->getLiteralSystemId() != nullptr) {
...
void XMLEntityManager::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$init($Constants);
	...
	}));
	{
		for (int32_t i = 0; i <= 31; ++i) {
			XMLEntityManager::gNeedEscaping->set(i, true);
			XMLEntityManager::gAfterEscaping1->set(i, XMLEntityManager::gHexChs->get(i >> 4));
			XMLEntityManager::gAfterEscaping2->set(i, XMLEntityManager::gHexChs->get(i & 0x0f));
		}
		XMLEntityManager::gNeedEscaping->set(127, true);
		XMLEntityManager::gAfterEscaping1->set(127, u'7');
		XMLEntityManager::gAfterEscaping2->set(127, u'F');
		...
		for (int32_t i = 0; i < len; ++i) {
			ch = escChs->get(i);
			XMLEntityManager::gNeedEscaping->set(ch, true);
			XMLEntityManager::gAfterEscaping1->set(ch, XMLEntityManager::gHexChs->get(ch >> 4));
			XMLEntityManager::gAfterEscaping2->set(ch, XMLEntityManager::gHexChs->get(ch & 0x0f));
		}
	}
}
...
$Class* XMLEntityManager::load$($String* name, bool initialize) {
	...
	$loadClass(XMLEntityManager, name, initialize, &classInfo$$, XMLEntityManager::clinit$, []($Class* clazz) -> $Object* {
