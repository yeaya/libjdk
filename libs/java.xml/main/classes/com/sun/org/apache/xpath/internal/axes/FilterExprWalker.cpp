...
void FilterExprWalker::setRoot(int32_t root) {
	...
	$var($XPathContext, var$0, $nc(this->m_lpi)->getXPathContext());
	$var($PrefixResolver, var$1, this->m_lpi->getPrefixResolver());
	$set(this, m_exprObj, $FilterExprIteratorSimple::executeFilterExpr(root, var$0, var$1, this->m_lpi->getIsTopLevel(), this->m_lpi->m_stackFrame, this->m_expr));
}

$Object* FilterExprWalker::clone() {
	$var(FilterExprWalker, clone, $cast(FilterExprWalker, $AxesWalker::clone()));
	if (nullptr != this->m_exprObj) {
		$set($nc(clone), m_exprObj, $cast($XNodeSet, this->m_exprObj->clone()));
	}
	return $of(clone);
}
...
int32_t FilterExprWalker::getNextNode() {
	if (nullptr != this->m_exprObj) {
		int32_t next = this->m_exprObj->nextNode();
		return next;
	} else {
		return $DTM::NULL;
	}
...
$Class* FilterExprWalker::load$($String* name, bool initialize) {
	...
	$loadClass(FilterExprWalker, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
