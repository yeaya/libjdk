	...
	case XPath$NodeTest::QNAME:
		{
			if ($nc($nc(this->name)->prefix)->length() != 0) {
				if (this->name->uri != nullptr) {
					return $str({this->name->prefix, $$str(u':'), this->name->localpart});
				}
				return $str({"{"_s, this->name->uri, $$str(u'}'), this->name->prefix, $$str(u':'), this->name->localpart});
			}
	...
	case XPath$NodeTest::NAMESPACE:
		{
			if ($nc($nc(this->name)->prefix)->length() != 0) {
				if (this->name->uri != nullptr) {
					return $str({this->name->prefix, ":*"_s});
				}
				return $str({"{"_s, this->name->uri, $$str(u'}'), this->name->prefix, ":*"_s});
			}
			return "???:*"_s;
		}
...
$Class* XPath$NodeTest::load$($String* name, bool initialize) {
	...
	$loadClass(XPath$NodeTest, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XPath$NodeTest);
	});
	return class$;
}
