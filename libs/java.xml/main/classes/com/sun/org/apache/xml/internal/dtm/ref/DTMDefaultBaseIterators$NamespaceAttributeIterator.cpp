...
$Class* DTMDefaultBaseIterators$NamespaceAttributeIterator::load$($String* name, bool initialize) {
	...
	$loadClass(DTMDefaultBaseIterators$NamespaceAttributeIterator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DTMDefaultBaseIterators$NamespaceAttributeIterator);
	});
	return class$;
}
