...
void OutlineableChunkStart::clinit$($Class* clazz) {
...
$Class* OutlineableChunkStart::load$($String* name, bool initialize) {
	...
	$loadClass(OutlineableChunkStart, name, initialize, &classInfo$$, OutlineableChunkStart::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(OutlineableChunkStart);
	});
	return class$;
}
