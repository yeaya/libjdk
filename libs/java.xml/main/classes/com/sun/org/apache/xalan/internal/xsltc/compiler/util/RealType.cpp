...
void RealType::translateTo($ClassGenerator* classGen, $MethodGenerator* methodGen, $StringType* type) {
	...
	$init($Constants);
	$nc(il)->append($$new($INVOKESTATIC, $nc(cpg)->addMethodref($Constants::BASIS_LIBRARY_CLASS, "realToString"_s, $$str({"(D)"_s, $Constants::STRING_SIG}))));
}
...
void RealType::translateTo($ClassGenerator* classGen, $MethodGenerator* methodGen, $IntType* type) {
	...
	$var($InstructionList, il, $nc(methodGen)->getInstructionList());
	$init($Constants);
	$nc(il)->append($$new($INVOKESTATIC, $nc(cpg)->addMethodref($Constants::BASIS_LIBRARY_CLASS, "realToInt"_s, "(D)I"_s)));
}
...
void RealType::translateTo($ClassGenerator* classGen, $MethodGenerator* methodGen, $ReferenceType* type) {
	...
	$var($InstructionList, il, $nc(methodGen)->getInstructionList());
	$init($Constants);
	$nc(il)->append($$new($NEW, $nc(cpg)->addClass($Constants::DOUBLE_CLASS)));
	il->append($Constants::DUP_X2);
	il->append($Constants::DUP_X2);
	il->append($Constants::POP);
	il->append($$new($INVOKESPECIAL, cpg->addMethodref($Constants::DOUBLE_CLASS, "<init>"_s, "(D)V"_s)));
}
...
void RealType::translateTo($ClassGenerator* classGen, $MethodGenerator* methodGen, $Class* clazz) {
	...
	if (clazz == $Character::TYPE) {
		...
		$init($Constants);
		$nc(il)->append($Constants::D2L);
	} else if (clazz == $Float::TYPE) {
		$init($Constants);
		$nc(il)->append($Constants::D2F);
...
void RealType::translateFrom($ClassGenerator* classGen, $MethodGenerator* methodGen, $Class* clazz) {
	...
	if (clazz == $Character::TYPE || clazz == $Byte::TYPE || clazz == $Short::TYPE || clazz == $Integer::TYPE) {
		...
		$init($Constants);
		$nc(il)->append($Constants::L2D);
	} else if (clazz == $Float::TYPE) {
		$init($Constants);
		$nc(il)->append($Constants::F2D);
	} else if (clazz == $Double::TYPE) {
		$init($Constants);
		$nc(il)->append($Constants::NOP);
	} else {
		...
		$var($String, var$0, $ErrorMsg::DATA_CONVERSION_ERR);
		$var($Object, var$1, toString());
		$var($ErrorMsg, err, $new($ErrorMsg, var$0, var$1, $($nc(clazz)->getName())));
		$$nc($nc(classGen)->getParser())->reportError($Constants::FATAL, err);
	}
}
...
void RealType::translateUnBox($ClassGenerator* classGen, $MethodGenerator* methodGen) {
	...
	$var($InstructionList, il, $nc(methodGen)->getInstructionList());
	$init($Constants);
	$nc(il)->append($$new($CHECKCAST, $nc(cpg)->addClass($Constants::DOUBLE_CLASS)));
...
$Class* RealType::load$($String* name, bool initialize) {
	...
	$loadClass(RealType, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(RealType);
	});
	return class$;
}
