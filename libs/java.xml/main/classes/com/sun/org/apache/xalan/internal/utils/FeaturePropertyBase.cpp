...
void FeaturePropertyBase::setValue($Enum* property, $FeaturePropertyBase$State* state, $String* value) {
	if ($nc(state)->compareTo($nc(this->states)->get($nc(property)->ordinal())) >= 0) {
		$nc(this->values)->set(property->ordinal(), value);
...
int32_t FeaturePropertyBase::getIndex($Class* property, $String* propertyName) {
	$useLocalObjectStack();
	$var($EnumArray, arr$, $cast($EnumArray, $nc(property)->getEnumConstants()));
...
$Class* FeaturePropertyBase::load$($String* name, bool initialize) {
	...
	$loadClass(FeaturePropertyBase, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FeaturePropertyBase);
	});
	return class$;
}
