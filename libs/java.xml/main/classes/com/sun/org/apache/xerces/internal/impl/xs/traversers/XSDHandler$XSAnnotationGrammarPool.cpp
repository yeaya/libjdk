	...
	if (grammarType == $XMLGrammarDescription::XML_SCHEMA) {
		if (this->fInitialGrammarSet == nullptr) {
			if (this->fGrammarBucket == nullptr) {
				...
				$set(this, fInitialGrammarSet, $new($GrammarArray, {$SchemaGrammar$Schema4Annotations::INSTANCE}));
			} else {
				$var($SchemaGrammarArray, schemaGrammars, this->fGrammarBucket->getGrammars());
				for (int32_t i = 0; i < $nc(schemaGrammars)->length; ++i) {
					...
					if ($nc($SchemaSymbols::URI_SCHEMAFORSCHEMA)->equals($($nc(schemaGrammars->get(i))->getTargetNamespace()))) {
						$set(this, fInitialGrammarSet, $cast($GrammarArray, schemaGrammars));
						return this->fInitialGrammarSet;
					}
				}
				$var($GrammarArray, grammars, $new($GrammarArray, schemaGrammars->length + 1));
...
$Grammar* XSDHandler$XSAnnotationGrammarPool::retrieveGrammar($XMLGrammarDescription* desc) {
	...
	if ($nc(desc)->getGrammarType() == $XMLGrammarDescription::XML_SCHEMA) {
		$var($String, tns, $cast($XMLSchemaDescription, desc)->getTargetNamespace());
		if (this->fGrammarBucket != nullptr) {
			$var($Grammar, grammar, this->fGrammarBucket->getGrammar(tns));
			if (grammar != nullptr) {
				return grammar;
			}
		}
...
$Class* XSDHandler$XSAnnotationGrammarPool::load$($String* name, bool initialize) {
	...
	$loadClass(XSDHandler$XSAnnotationGrammarPool, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XSDHandler$XSAnnotationGrammarPool);
	});
	return class$;
}
