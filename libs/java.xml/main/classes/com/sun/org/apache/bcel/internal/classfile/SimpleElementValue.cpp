...
$String* SimpleElementValue::stringifyValue() {
	...
	int32_t _type = $ElementValue::getType();
	{
		...
		case $ElementValue::PRIMITIVE_LONG:
			$assign(j, $cast($ConstantLong, $nc(cpool)->getConstant(getIndex(), $Const::CONSTANT_Long)));
			return $Long::toString($nc(j)->getBytes());
		case $ElementValue::PRIMITIVE_DOUBLE:
			$assign(d, $cast($ConstantDouble, $nc(cpool)->getConstant(getIndex(), $Const::CONSTANT_Double)));
			return $Double::toString($nc(d)->getBytes());
		case $ElementValue::PRIMITIVE_FLOAT:
			$assign(f, $cast($ConstantFloat, $nc(cpool)->getConstant(getIndex(), $Const::CONSTANT_Float)));
			return $Float::toString($nc(f)->getBytes());
		case $ElementValue::PRIMITIVE_SHORT:
			$assign(s, $cast($ConstantInteger, $nc(cpool)->getConstant(getIndex(), $Const::CONSTANT_Integer)));
			return $Integer::toString($nc(s)->getBytes());
		case $ElementValue::PRIMITIVE_BYTE:
			$assign(b, $cast($ConstantInteger, $nc(cpool)->getConstant(getIndex(), $Const::CONSTANT_Integer)));
			return $Integer::toString($nc(b)->getBytes());
		case $ElementValue::PRIMITIVE_CHAR:
			$assign(ch, $cast($ConstantInteger, $nc(cpool)->getConstant(getIndex(), $Const::CONSTANT_Integer)));
			return $String::valueOf((char16_t)$nc(ch)->getBytes());
		case $ElementValue::PRIMITIVE_BOOLEAN:
			$assign(bo, $cast($ConstantInteger, $nc(cpool)->getConstant(getIndex(), $Const::CONSTANT_Integer)));
			if ($nc(bo)->getBytes() == 0) {
				return "false"_s;
			}
			return "true"_s;
		case $ElementValue::STRING:
			$assign(cu8, $cast($ConstantUtf8, $nc(cpool)->getConstant(getIndex(), $Const::CONSTANT_Utf8)));
...
$Class* SimpleElementValue::load$($String* name, bool initialize) {
	...
	$loadClass(SimpleElementValue, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SimpleElementValue);
	});
	return class$;
}
