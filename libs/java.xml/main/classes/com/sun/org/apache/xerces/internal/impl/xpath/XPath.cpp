...
$Class* XPath$$Lambda$lambda$toString$0::load$($String* name, bool initialize) {
	...
	$loadClass(XPath$$Lambda$lambda$toString$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XPath$$Lambda$lambda$toString$0);
	});
	return class$;
}
...
$XPath$LocationPathArray* XPath::getLocationPaths() {
	...
	for (int32_t i = 0; i < this->fLocationPaths->length; ++i) {
		ret->set(i, $$cast($XPath$LocationPath, $nc(this->fLocationPaths->get(i))->clone()));
	}
	return ret;
}
...
$String* XPath::toString() {
	...
	$var($String, s, $cast($String, $$nc($$nc($nc(l)->stream())->map($$new(XPath$$Lambda$lambda$toString$0)))->collect($($Collectors::joining("|"_s)))));
	return s;
}
...
$XPath$LocationPath* XPath::buildLocationPath($ArrayList* stepsVector) {
	...
	$assign(steps, $cast($XPath$StepArray, stepsVector->toArray(steps)));
	stepsVector->clear();
...
$XPath$LocationPathArray* XPath::parseExpression($NamespaceContext* context) {
	...
	check(!expectingStep);
	locationPathsVector->add($(buildLocationPath(stepsVector)));
	return $cast($XPath$LocationPathArray, locationPathsVector->toArray($$new($XPath$LocationPathArray, locationPathsVector->size())));
}
...
void XPath::main($StringArray* argv) {
	...
	for (int32_t i = 0; i < $nc(argv)->length; ++i) {
		...
		try {
			...
			$System::out->println($$str({"expanded xpath: \""_s, $(xpath->toString()), $$str(u'\"')}));
		} catch ($XPathException& e) {
			$System::out->println($$str({"error: "_s, $(e->getMessage())}));
		}
	}
}
...
$Class* XPath::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.org.apache.xerces.internal.impl.xpath.XPath$$Lambda$lambda$toString$0")) {
	...
	$loadClass(XPath, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XPath);
	});
	return class$;
}
