...
void StAXStream2SAX::handlePI() {
	...
	try {
		...
		$nc(this->_sax)->processingInstruction(var$0, $(this->staxStreamReader->getPIData()));
	} catch ($SAXException& e) {
		$throwNew($XMLStreamException, e);
	}
}
...
void StAXStream2SAX::handleEndElement() {
	...
	try {
		...
		$plusAssign(qname, $(qName->getLocalPart()));
		$var($String, var$1, qName->getNamespaceURI());
		$nc(this->_sax)->endElement(var$1, $(qName->getLocalPart()), qname);
		int32_t nsCount = this->staxStreamReader->getNamespaceCount();
		for (int32_t i = nsCount - 1; i >= 0; --i) {
			$var($String, prefix, this->staxStreamReader->getNamespacePrefix(i));
			if (prefix == nullptr) {
				$assign(prefix, ""_s);
			}
...
void StAXStream2SAX::handleStartElement() {
	...
	try {
		...
		for (int32_t i = 0; i < nsCount; ++i) {
			...
			$var($String, uri, this->staxStreamReader->getNamespaceURI(i));
			if (uri == nullptr && $nc(prefix)->isEmpty()) {
				$assign(uri, ""_s);
			}
			$nc(this->_sax)->startPrefixMapping(prefix, uri);
		}
		$var($QName, qName, this->staxStreamReader->getName());
...
$Attributes* StAXStream2SAX::getAttributes() {
	...
	for (int32_t i = 0; i < this->staxStreamReader->getAttributeCount(); ++i) {
		$var($String, uri, this->staxStreamReader->getAttributeNamespace(i));
		if (uri == nullptr) {
			$assign(uri, ""_s);
		}
		$var($String, localName, this->staxStreamReader->getAttributeLocalName(i));
		$var($String, prefix, this->staxStreamReader->getAttributePrefix(i));
		...
		$var($String, type, this->staxStreamReader->getAttributeType(i));
		$var($String, value, this->staxStreamReader->getAttributeValue(i));
...
$Class* StAXStream2SAX::load$($String* name, bool initialize) {
	...
	$loadClass(StAXStream2SAX, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
