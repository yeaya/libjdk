...
$Type* AbsolutePathPattern::typeCheck($SymbolTable* stable) {
	$init($Type);
	return this->_left == nullptr ? $Type::Root : this->_left->typeCheck(stable);
}
...
$StepPattern* AbsolutePathPattern::getKernelPattern() {
	return this->_left != nullptr ? this->_left->getKernelPattern() : ($StepPattern*)nullptr;
}
...
void AbsolutePathPattern::translate($ClassGenerator* classGen, $MethodGenerator* methodGen) {
	...
	if (this->_left != nullptr) {
		if ($instanceOf($StepPattern, this->_left)) {
			...
			$nc(local)->setStart($(il->append($$new($ISTORE, $nc(local)->getIndex()))));
	...
	il->append($$new($PUSH, cpg, (int32_t)$DTM::DOCUMENT_NODE));
	$var($BranchHandle, skip, il->append($$new($IF_ICMPEQ, nullptr)));
	...
	if (this->_left != nullptr) {
		...
		$nc(this->_falseList)->append(this->_left->_falseList);
	}
}

$String* AbsolutePathPattern::toString() {
	return $str({"absolutePathPattern("_s, (this->_left != nullptr ? $(this->_left->toString()) : ")"_s)});
}
...
$Class* AbsolutePathPattern::load$($String* name, bool initialize) {
	...
	$loadClass(AbsolutePathPattern, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AbsolutePathPattern);
	});
	return class$;
}
