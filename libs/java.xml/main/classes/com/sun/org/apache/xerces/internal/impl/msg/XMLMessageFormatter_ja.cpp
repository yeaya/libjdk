...
void XMLMessageFormatter_ja::clinit$($Class* clazz) {
...
$Class* XMLMessageFormatter_ja::load$($String* name, bool initialize) {
	...
	$loadClass(XMLMessageFormatter_ja, name, initialize, &classInfo$$, XMLMessageFormatter_ja::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLMessageFormatter_ja);
	});
	return class$;
}
