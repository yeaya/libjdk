...
void XMLDocumentFragmentScannerImpl$ElementStack::init$($XMLDocumentFragmentScannerImpl* this$0) {
	...
	for (int32_t i = 0; i < this->fElements->length; ++i) {
		this->fElements->set(i, $$new($QName));
	}
}
...
$QName* XMLDocumentFragmentScannerImpl$ElementStack::pushElement($QName* element) {
	...
	if (this->fDepth == $nc(this->fElements)->length) {
		$var($QNameArray, array, $new($QNameArray, this->fElements->length * 2));
...
$Class* XMLDocumentFragmentScannerImpl$ElementStack::load$($String* name, bool initialize) {
	...
	$loadClass(XMLDocumentFragmentScannerImpl$ElementStack, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XMLDocumentFragmentScannerImpl$ElementStack);
	});
	return class$;
}
