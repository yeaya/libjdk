...
$ObjectArray2* SerializerMessages_ko::getContents() {
	...
	$var($ObjectArray2, contents, $new($ObjectArray2, {
		$$new($ObjectArray, {
			$MsgKey::BAD_MSGKEY,
			u"메시지 키 \'\'{0}\'\'이(가) 메시지 클래스 \'\'{1}\'\'에 없습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::BAD_MSGFORMAT,
			u"메시지 클래스 \'\'{1}\'\'에서 \'\'{0}\'\' 메시지의 형식이 잘못되었습니다."_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"경고: 출력 문서의 버전이 \'\'{0}\'\'이(가) 되도록 요청했습니다. 이 버전의 XML은 지원되지 않습니다. 출력 문서의 버전은 \'\'1.0\'\'이 됩니다."_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"SerializerFactory에 전달된 Properties 객체에 \'\'{0}\'\' 속성이 없습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_ENCODING_NOT_SUPPORTED,
			u"경고: 인코딩 \'\'{0}\'\'은(는) Java 런타임에 지원되지 않습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_FEATURE_NOT_FOUND,
			u"\'\'{0}\'\' 매개변수를 인식할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_FEATURE_NOT_SUPPORTED,
			u"\'\'{0}\'\' 매개변수가 인식되었지만 요청된 값을 설정할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_STRING_TOO_LONG,
			u"결과 문자열이 너무 커서 DOMString에 맞지 않음: \'\'{0}\'\'."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_TYPE_MISMATCH_ERR,
			u"이 매개변수 이름에 대한 값 유형이 필요한 값 유형과 호환되지 않습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_NO_OUTPUT_SPECIFIED,
			u"데이터를 쓸 출력 대상이 널입니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_UNSUPPORTED_ENCODING,
			u"지원되지 않는 인코딩이 발견되었습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_UNABLE_TO_SERIALIZE_NODE,
			u"노드를 직렬화할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_CDATA_SECTIONS_SPLIT,
			u"CDATA 섹션에 하나 이상의 종료 표시자 \']]>\'가 있습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_WARNING_WF_NOT_CHECKED,
			u"Well-Formedness 검사기의 인스턴스를 생성할 수 없습니다. well-formed 매개변수가 true로 설정되었지만 well-formedness 검사를 수행할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_WF_INVALID_CHARACTER,
			u"\'\'{0}\'\' 노드에 부적합한 XML 문자가 포함되어 있습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_WF_INVALID_CHARACTER_IN_COMMENT,
			u"주석에서 부적합한 XML 문자(유니코드: 0x{0})가 발견되었습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_WF_INVALID_CHARACTER_IN_PI,
			u"부적합한 XML 문자(유니코드: 0x{0})가 instructiondata 처리에서 발견되었습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_WF_INVALID_CHARACTER_IN_CDATA,
			u"부적합한 XML 문자(유니코드: 0x{0})가 CDATASection의 콘텐츠에서 발견되었습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_WF_INVALID_CHARACTER_IN_TEXT,
			u"부적합한 XML 문자(유니코드: 0x{0})가 노드의 문자 데이터 콘텐츠에서 발견되었습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_WF_INVALID_CHARACTER_IN_NODE_NAME,
			u"부적합한 XML 문자가 이름이 \'\'{1}\'\'인 {0} 노드에서 발견되었습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_WF_DASH_IN_COMMENT,
			u"주석에서는 \"--\" 문자열이 허용되지 않습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_WF_LT_IN_ATTVAL,
			u"요소 유형 \"{0}\"과(와) 연관된 \"{1}\" 속성의 값에는 \'\'<\'\' 문자가 포함되지 않아야 합니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_WF_REF_TO_UNPARSED_ENT,
			u"구문이 분석되지 않은 엔티티 참조 \"&{0};\"은(는) 허용되지 않습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_WF_REF_TO_EXTERNAL_ENT,
			u"속성값에서는 외부 엔티티 참조 \"&{0};\"이 허용되지 않습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_NS_PREFIX_CANNOT_BE_BOUND,
			u"\"{0}\" 접두어를 \"{1}\" 네임스페이스에 바인드할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_NULL_LOCAL_ELEMENT_NAME,
			u"\"{0}\" 요소의 로컬 이름이 널입니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_NULL_LOCAL_ATTR_NAME,
			u"\"{0}\" 속성의 로컬 이름이 널입니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_ELEM_UNBOUND_PREFIX_IN_ENTREF,
			u"엔티티 노드 \"{0}\"의 대체 텍스트에 바인드되지 않은 접두어 \"{2}\"을(를) 사용하는 요소 노드 \"{1}\"이(가) 포함되어 있습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_ATTR_UNBOUND_PREFIX_IN_ENTREF,
			u"엔티티 노드 \"{0}\"의 대체 텍스트에 바인드되지 않은 접두어 \"{2}\"을(를) 사용하는 속성 노드 \"{1}\"이(가) 포함되어 있습니다."_s
		}),
		$$new($ObjectArray, {
			$MsgKey::ER_WRITING_INTERNAL_SUBSET,
			u"내부 부분 집합을 쓰는 중 오류가 발생했습니다."_s
		})
	}));
	return contents;
}
...
$Class* SerializerMessages_ko::load$($String* name, bool initialize) {
	...
	$loadClass(SerializerMessages_ko, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SerializerMessages_ko);
	});
	return class$;
}
