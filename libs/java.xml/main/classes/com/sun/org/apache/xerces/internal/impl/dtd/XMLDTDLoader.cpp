...
$StringArray* XMLDTDLoader::getRecognizedFeatures() {
	return $cast($StringArray, XMLDTDLoader::LOADER_RECOGNIZED_FEATURES->clone());
}
...
void XMLDTDLoader::setFeature($String* featureId, bool state) {
	...
	} else if (featureId->equals($XMLDTDProcessor::WARN_ON_DUPLICATE_ATTDEF)) {
		this->fWarnDuplicateAttdef = state;
	} else if (featureId->equals($XMLDTDProcessor::WARN_ON_UNDECLARED_ELEMDEF)) {
		this->fWarnOnUndeclaredElemdef = state;
	} else if (featureId->equals($XMLDTDProcessor::NOTIFY_CHAR_REFS)) {
...
$StringArray* XMLDTDLoader::getRecognizedProperties() {
	return $cast($StringArray, XMLDTDLoader::LOADER_RECOGNIZED_PROPERTIES->clone());
}
...
$Object* XMLDTDLoader::getProperty($String* propertyId) {
	...
	if ($nc(propertyId)->equals($XMLDTDProcessor::SYMBOL_TABLE)) {
	...
	} else if (propertyId->equals($XMLDTDProcessor::ERROR_REPORTER)) {
		...
		return $of(getLocale());
	} else if (propertyId->equals($XMLDTDProcessor::GRAMMAR_POOL)) {
		return this->fGrammarPool;
	} else if (propertyId->equals($XMLDTDProcessor::DTD_VALIDATOR)) {
...
$Grammar* XMLDTDLoader::loadGrammar($XMLInputSource* source) {
	...
	$var($String, var$1, source->getPublicId());
	...
	if (this->fDTDGrammar != nullptr && this->fGrammarPool != nullptr) {
		this->fGrammarPool->cacheGrammars($XMLDTDDescription::XML_DTD, $$new($GrammarArray, {this->fDTDGrammar}));
	}
		...
		try {
			try {
				if (internalSubset != nullptr) {
					...
					$var($XMLInputSource, is, $new($XMLInputSource, nullptr, baseSystemId, nullptr, $$new($StringReader, $(buffer->toString())), nullptr));
				...
				if (systemId != nullptr) {
					...
					$var($XMLInputSource, source, $nc(this->fEntityManager)->resolveEntity($cast($XMLResourceIdentifierImpl, desc)));
					$nc(this->fDTDScanner)->setInputSource(source);
					$nc(this->fDTDScanner)->scanDTDExternalSubset(true);
				}
			} catch ($EOFException& e) {
...
void XMLDTDLoader::clinit$($Class* clazz) {
	$init($Constants);
...
$Class* XMLDTDLoader::load$($String* name, bool initialize) {
	...
	$loadClass(XMLDTDLoader, name, initialize, &classInfo$$, XMLDTDLoader::clinit$, []($Class* clazz) -> $Object* {
