...
$Object* XPathResultImpl::value() {
	...
	try {
		return getValue(this->resultObject, this->type$);
...
$Object* XPathResultImpl::getValue($XObject* resultObject, $Class* type) {
	...
	int32_t resultType = classToInternalType(type);
	{
		...
		switch (resultType) {
		case $XObject::CLASS_BOOLEAN:
			return type->cast($($Boolean::valueOf($nc(resultObject)->bool$())));
		case $XObject::CLASS_NUMBER:
			if ($Double::class$->isAssignableFrom(type)) {
				return type->cast($($Double::valueOf($nc(resultObject)->num())));
			} else if ($Integer::class$->isAssignableFrom(type)) {
				return type->cast($($Integer::valueOf($cast(int32_t, $nc(resultObject)->num()))));
			} else if ($Long::class$->isAssignableFrom(type)) {
				return type->cast($($Long::valueOf($cast(int64_t, $nc(resultObject)->num()))));
			}
			break;
		case $XObject::CLASS_STRING:
			return type->cast($($nc(resultObject)->str()));
		case $XObject::CLASS_NODESET:
			$load($Node);
		...
		case $XObject::CLASS_RTREEFRAG:
			$assign(ni, $nc(resultObject)->nodeset());
			return type->cast($($nc(ni)->nextNode()));
		}
	}
	return nullptr;
}
...
$Class* XPathResultImpl::load$($String* name, bool initialize) {
	...
	$loadClass(XPathResultImpl, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XPathResultImpl);
	});
	return class$;
}
