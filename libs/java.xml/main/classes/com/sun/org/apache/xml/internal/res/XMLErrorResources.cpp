...
void XMLErrorResources::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(XMLErrorResources::contents, $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			XMLErrorResources::ER_FUNCTION_NOT_SUPPORTED,
			"Function not supported!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_CANNOT_OVERWRITE_CAUSE,
			"Cannot overwrite cause"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_NO_DEFAULT_IMPL,
			"No default implementation found "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_CHUNKEDINTARRAY_NOT_SUPPORTED,
			"ChunkedIntArray({0}) not currently supported"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_OFFSET_BIGGER_THAN_SLOT,
			"Offset bigger than slot"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_COROUTINE_NOT_AVAIL,
			"Coroutine not available, id={0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_COROUTINE_CO_EXIT,
			"CoroutineManager received co_exit() request"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_COJOINROUTINESET_FAILED,
			"co_joinCoroutineSet() failed"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_COROUTINE_PARAM,
			"Coroutine parameter error ({0})"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_PARSER_DOTERMINATE_ANSWERS,
			"\nUNEXPECTED: Parser doTerminate answers {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_NO_PARSE_CALL_WHILE_PARSING,
			"parse may not be called while parsing"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_TYPED_ITERATOR_AXIS_NOT_IMPLEMENTED,
			"Error: typed iterator for axis  {0} not implemented"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_ITERATOR_AXIS_NOT_IMPLEMENTED,
			"Error: iterator for axis {0} not implemented "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_ITERATOR_CLONE_NOT_SUPPORTED,
			"Iterator clone not supported"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_UNKNOWN_AXIS_TYPE,
			"Unknown axis traversal type: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_AXIS_NOT_SUPPORTED,
			"Axis traverser not supported: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_NO_DTMIDS_AVAIL,
			"No more DTM IDs are available"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			"Node must be non-null for getDTMHandleFromNode"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_COULD_NOT_RESOLVE_NODE,
			"Could not resolve the node to a handle"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_STARTPARSE_WHILE_PARSING,
			"startParse may not be called while parsing"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_STARTPARSE_NEEDS_SAXPARSER,
			"startParse needs a non-null SAXParser"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_COULD_NOT_INIT_PARSER,
			"could not initialize parser with"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_EXCEPTION_CREATING_POOL,
			"exception creating new instance for pool"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			XMLErrorResources::ER_SCHEME_REQUIRED,
			"Scheme is required!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_NO_SCHEME_IN_URI,
			"No scheme found in URI: {0}"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			XMLErrorResources::ER_INVALID_PORT,
		...
		$$new($ObjectArray, {
			XMLErrorResources::ER_PARSER_IN_USE,
			"Parser is already in use"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_CANNOT_CHANGE_WHILE_PARSING,
			"Cannot change {0} {1} while parsing"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_SELF_CAUSATION_NOT_PERMITTED,
			"Self-causation not permitted"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			"Method not yet supported "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_INCRSAXSRCFILTER_NOT_RESTARTABLE,
			"IncrementalSAXSource_Filter not currently restartable"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_XMLRDR_NOT_BEFORE_STARTPARSE,
			"XMLReader not before startParse request"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			"ListingErrorHandler created with null PrintWriter!"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			XMLErrorResources::ER_LOCATION_UNKNOWN,
			"Location of error unknown"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			"createDocument() not supported in XPathContext!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_CHILD_HAS_NO_OWNER_DOCUMENT,
			"Attribute child does not have an owner document!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_CHILD_HAS_NO_OWNER_DOCUMENT_ELEMENT,
			"Attribute child does not have an owner document element!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_CANT_OUTPUT_TEXT_BEFORE_DOC,
			"Warning: can\'t output text before document element!  Ignoring..."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_CANT_HAVE_MORE_THAN_ONE_ROOT,
			"Can\'t have more than one root on a DOM!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_ARG_LOCALNAME_NULL,
			"Argument \'localName\' is null"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_ARG_LOCALNAME_INVALID,
			"Localname in QNAME should be a valid NCName"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_ARG_PREFIX_INVALID,
			"Prefix in QNAME should be a valid NCName"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_NAME_CANT_START_WITH_COLON,
			"Name cannot start with a colon"_s
		}),
		$$new($ObjectArray, {
			"BAD_CODE"_s,
		...
		$$new($ObjectArray, {
			XMLErrorResources::ER_BUFFER_SIZE_LESSTHAN_ZERO,
			"Buffer size <=0"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			XMLErrorResources::ER_ILLEGAL_ATTRIBUTE_POSITION,
		...
		$$new($ObjectArray, {
			XMLErrorResources::ER_STRAY_ATTRIBUTE,
			"Attribute \'\'{0}\'\' outside of element."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources::ER_STRAY_NAMESPACE,
...
$Class* XMLErrorResources::load$($String* name, bool initialize) {
	...
	$loadClass(XMLErrorResources, name, initialize, &classInfo$$, XMLErrorResources::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLErrorResources);
	});
	return class$;
}
