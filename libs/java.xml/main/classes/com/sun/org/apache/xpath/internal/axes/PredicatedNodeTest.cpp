...
$Object* PredicatedNodeTest::clone() {
	...
	if ((nullptr != this->m_proximityPositions) && (this->m_proximityPositions == $nc(clone)->m_proximityPositions)) {
		$set(clone, m_proximityPositions, $new($ints, this->m_proximityPositions->length));
	...
	if (pos > 0) {
		...
		if (nullptr != this->m_predicates) {
			for (int32_t i = 0; i < $nc(this->m_predicates)->length; ++i) {
				$nc(this->m_predicates->get(i))->exprSetParent(this);
			}
		}
	}
}
...
bool PredicatedNodeTest::deepEquals($Expression* expr) {
	...
	if (nullptr != this->m_predicates) {
		int32_t n = this->m_predicates->length;
		if ((nullptr == $nc(pnt)->m_predicates) || (pnt->m_predicates->length != n)) {
			return false;
		}
		for (int32_t i = 0; i < n; ++i) {
			if (!$nc($nc(this->m_predicates)->get(i))->deepEquals($nc(pnt->m_predicates)->get(i))) {
				return false;
			}
		}
...
$Class* PredicatedNodeTest::load$($String* name, bool initialize) {
	...
	$loadClass(PredicatedNodeTest, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
