...
void XMLErrorResources_ko::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(XMLErrorResources_ko::contents, $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_FUNCTION_NOT_SUPPORTED,
			u"함수가 지원되지 않습니다!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_CANNOT_OVERWRITE_CAUSE,
			u"원인을 겹쳐 쓸 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_NO_DEFAULT_IMPL,
			u"기본 구현을 찾을 수 없습니다. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_CHUNKEDINTARRAY_NOT_SUPPORTED,
			u"ChunkedIntArray({0})는 현재 지원되지 않습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_OFFSET_BIGGER_THAN_SLOT,
			u"오프셋이 슬롯보다 큽니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_COROUTINE_NOT_AVAIL,
			u"Coroutine을 사용할 수 없습니다. ID={0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_COROUTINE_CO_EXIT,
			u"CoroutineManager가 co_exit() 요청을 수신했습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_COJOINROUTINESET_FAILED,
			u"co_joinCoroutineSet()를 실패했습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_COROUTINE_PARAM,
			u"Coroutine 매개변수 오류({0})"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_PARSER_DOTERMINATE_ANSWERS,
			u"\n예상치 않은 오류: 구문 분석기 doTerminate가 {0}에 응답합니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_NO_PARSE_CALL_WHILE_PARSING,
			u"구문 분석 중 parse를 호출할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_TYPED_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"오류: {0} 축에 대해 입력된 이터레이터가 구현되지 않았습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"오류: {0} 축에 대한 이터레이터가 구현되지 않았습니다. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_ITERATOR_CLONE_NOT_SUPPORTED,
			u"이터레이터 복제는 지원되지 않습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_UNKNOWN_AXIS_TYPE,
			u"알 수 없는 축 순회 유형: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_AXIS_NOT_SUPPORTED,
			u"축 순환기가 지원되지 않음: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_NO_DTMIDS_AVAIL,
			u"더 이상 사용 가능한 DTM ID가 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_NOT_SUPPORTED,
			u"지원되지 않음: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_NODE_NON_NULL,
			u"노드는 getDTMHandleFromNode에 대해 널이 아니어야 합니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_COULD_NOT_RESOLVE_NODE,
			u"노드를 핸들로 분석할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_STARTPARSE_WHILE_PARSING,
			u"구문 분석 중 startParse를 호출할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_STARTPARSE_NEEDS_SAXPARSER,
			u"startParse에는 널이 아닌 SAXParser가 필요합니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_COULD_NOT_INIT_PARSER,
			u"구문 분석기를 초기화할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_EXCEPTION_CREATING_POOL,
			u"풀에 대한 새 인스턴스를 생성하는 중 예외사항이 발생했습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_PATH_CONTAINS_INVALID_ESCAPE_SEQUENCE,
			u"경로에 부적합한 이스케이프 시퀀스가 포함되어 있습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_SCHEME_REQUIRED,
			u"체계가 필요합니다!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_NO_SCHEME_IN_URI,
			u"URI에서 체계를 찾을 수 없음: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_NO_SCHEME_INURI,
			u"URI에서 체계를 찾을 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_PATH_INVALID_CHAR,
			u"경로에 부적합한 문자가 포함됨: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_SCHEME_FROM_NULL_STRING,
			u"널 문자열에서 체계를 설정할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_SCHEME_NOT_CONFORMANT,
			u"체계가 일치하지 않습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_HOST_ADDRESS_NOT_WELLFORMED,
			u"호스트가 완전한 주소가 아닙니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_PORT_WHEN_HOST_NULL,
			u"호스트가 널일 경우 포트를 설정할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_INVALID_PORT,
			u"포트 번호가 부적합합니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_FRAG_FOR_GENERIC_URI,
			u"일반 URI에 대해서만 부분을 설정할 수 있습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_FRAG_WHEN_PATH_NULL,
			u"경로가 널일 경우 부분을 설정할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_FRAG_INVALID_CHAR,
			u"부분에 부적합한 문자가 포함되어 있습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_PARSER_IN_USE,
			u"구문 분석기가 이미 사용되고 있습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_CANNOT_CHANGE_WHILE_PARSING,
			u"구문 분석 중 {0} {1}을(를) 변경할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_SELF_CAUSATION_NOT_PERMITTED,
			u"자체 인과 관계는 허용되지 않습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_NO_USERINFO_IF_NO_HOST,
			u"호스트를 지정하지 않은 경우에는 Userinfo를 지정할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_NO_PORT_IF_NO_HOST,
			u"호스트를 지정하지 않은 경우에는 포트를 지정할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_NO_QUERY_STRING_IN_PATH,
			u"경로 및 질의 문자열에 질의 문자열을 지정할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_NO_FRAGMENT_STRING_IN_PATH,
			u"경로와 부분에 모두 부분을 지정할 수는 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_CANNOT_INIT_URI_EMPTY_PARMS,
			u"빈 매개변수로 URI를 초기화할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_METHOD_NOT_SUPPORTED,
			u"메소드가 아직 지원되지 않습니다. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_INCRSAXSRCFILTER_NOT_RESTARTABLE,
			u"현재 IncrementalSAXSource_Filter를 재시작할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_XMLRDR_NOT_BEFORE_STARTPARSE,
			u"startParse 요청 전에 XMLReader가 실행되지 않았습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_AXIS_TRAVERSER_NOT_SUPPORTED,
			u"축 순환기가 지원되지 않음: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_ERRORHANDLER_CREATED_WITH_NULL_PRINTWRITER,
			u"널 PrintWriter로 ListingErrorHandler가 생성되었습니다!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_SYSTEMID_UNKNOWN,
			u"SystemId를 알 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_LOCATION_UNKNOWN,
			u"오류 위치를 알 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_PREFIX_MUST_RESOLVE,
			u"접두어는 네임스페이스로 분석되어야 함: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_CREATEDOCUMENT_NOT_SUPPORTED,
			u"XPathContext에서는 createDocument()가 지원되지 않습니다!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_CHILD_HAS_NO_OWNER_DOCUMENT,
			u"속성 하위에 소유자 문서가 없습니다!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_CHILD_HAS_NO_OWNER_DOCUMENT_ELEMENT,
			u"속성 하위에 소유자 문서 요소가 없습니다!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_CANT_OUTPUT_TEXT_BEFORE_DOC,
			u"경고: 문서 요소 앞에 텍스트를 출력할 수 없습니다! 무시하는 중..."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_CANT_HAVE_MORE_THAN_ONE_ROOT,
			u"DOM에서 루트를 두 개 이상 사용할 수 없습니다!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_ARG_LOCALNAME_NULL,
			u"\'localName\' 인수가 널입니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_ARG_LOCALNAME_INVALID,
			u"QNAME의 Localname은 적합한 NCName이어야 합니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_ARG_PREFIX_INVALID,
			u"QNAME의 접두어는 적합한 NCName이어야 합니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_NAME_CANT_START_WITH_COLON,
			u"이름은 콜론으로 시작할 수 없습니다."_s
		}),
		$$new($ObjectArray, {
			"BAD_CODE"_s,
			u"createMessage에 대한 매개변수가 범위를 벗어났습니다."_s
		}),
		$$new($ObjectArray, {
			"FORMAT_FAILED"_s,
			u"messageFormat 호출 중 예외사항이 발생했습니다."_s
		}),
		$$new($ObjectArray, {
			"line"_s,
			u"행 번호"_s
		...
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_SERIALIZER_NOT_CONTENTHANDLER,
			u"Serializer 클래스 \'\'{0}\'\'이(가) org.xml.sax.ContentHandler를 구현하지 않았습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_RESOURCE_COULD_NOT_FIND,
			u"[{0}] 리소스를 찾을 수 없습니다.\n {1}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_RESOURCE_COULD_NOT_LOAD,
			u"[{0}] 리소스가 다음을 로드할 수 없음: {1} \n {2} \t {3}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_BUFFER_SIZE_LESSTHAN_ZERO,
			u"버퍼 크기 <=0"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_INVALID_UTF16_SURROGATE,
			u"부적합한 UTF-16 대리 요소가 감지됨: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_OIERROR,
			u"IO 오류"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_ILLEGAL_ATTRIBUTE_POSITION,
			u"하위 노드가 생성된 후 또는 요소가 생성되기 전에 {0} 속성을 추가할 수 없습니다. 속성이 무시됩니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_NAMESPACE_PREFIX,
			u"\'\'{0}\'\' 접두어에 대한 네임스페이스가 선언되지 않았습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_STRAY_ATTRIBUTE,
			u"\'\'{0}\'\' 속성이 요소에 포함되어 있지 않습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_STRAY_NAMESPACE,
			u"네임스페이스 선언 \'\'{0}\'\'=\'\'{1}\'\'이(가) 요소에 포함되어 있지 않습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_COULD_NOT_LOAD_RESOURCE,
			u"{0}을(를) 로드할 수 없습니다. CLASSPATH를 확인하십시오. 현재 기본값만 사용하는 중입니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_ILLEGAL_CHARACTER,
			u"{1}의 지정된 출력 인코딩에서 표시되지 않는 정수 값 {0}의 문자를 출력하려고 시도했습니다."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ko::ER_COULD_NOT_LOAD_METHOD_PROPERTY,
			u"출력 메소드 \'\'{1}\'\'에 대한 속성 파일 \'\'{0}\'\'을(를) 로드할 수 없습니다. CLASSPATH를 확인하십시오."_s
		})
	}));
}
...
$Class* XMLErrorResources_ko::load$($String* name, bool initialize) {
	...
	$loadClass(XMLErrorResources_ko, name, initialize, &classInfo$$, XMLErrorResources_ko::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLErrorResources_ko);
	});
	return class$;
}
