...
void DOMResultBuilder::setDOMResult($DOMResult* result) {
	...
	if (result != nullptr) {
		...
		$set(this, fDocument, ($nc(this->fTarget)->getNodeType() == $Node::DOCUMENT_NODE) ? $cast($Document, this->fTarget) : this->fTarget->getOwnerDocument());
...
void DOMResultBuilder::doctypeDecl($DocumentType* node) {
	...
	if (this->fDocumentImpl != nullptr) {
		...
		$var($DocumentType, docType, this->fDocumentImpl->createDocumentType(var$0, var$1, $(node->getSystemId())));
		...
		for (int32_t i = 0; i < length; ++i) {
			...
			$nc(newNotation)->setPublicId($(oldNotation->getPublicId()));
			newNotation->setSystemId($(oldNotation->getSystemId()));
			$nc(newMap)->setNamedItem($cast($NodeImpl, newNotation));
		}
		append(docType);
	}
}
...
void DOMResultBuilder::startElement($QName* element, $XMLAttributes* attributes, $Augmentations* augs) {
	...
	if (this->fDocumentImpl == nullptr) {
		$assign(elem, $nc(this->fDocument)->createElementNS($nc(element)->uri, $nc(element)->rawname));
		for (int32_t i = 0; i < attrCount; ++i) {
			attributes->getName(i, this->fAttributeQName);
			$nc(elem)->setAttributeNS(this->fAttributeQName->uri, this->fAttributeQName->rawname, $(attributes->getValue(i)));
		}
	} else {
		$assign(elem, this->fDocumentImpl->createElementNS($nc(element)->uri, $nc(element)->rawname, $nc(element)->localpart));
		for (int32_t i = 0; i < attrCount; ++i) {
			attributes->getName(i, this->fAttributeQName);
			$var($AttrImpl, attr, $cast($AttrImpl, $nc(this->fDocumentImpl)->createAttributeNS(this->fAttributeQName->uri, this->fAttributeQName->rawname, this->fAttributeQName->localpart)));
			...
			if (attrPSVI != nullptr) {
				if (this->fStorePSVI) {
					$cast($PSVIAttrNSImpl, attr)->setPSVI(attrPSVI);
				}
				...
				if (type == nullptr) {
					...
					if (type != nullptr) {
						attr->setType(type);
						if ($cast($XSSimpleType, type)->isIDType()) {
...
void DOMResultBuilder::endDocument($Augmentations* augs) {
	...
	if (this->fNextSibling == nullptr) {
		$var($Iterator, i$, this->fTargetChildren->iterator());
		for (; $nc(i$)->hasNext();) {
...
void DOMResultBuilder::append($Node* node) {
	if (this->fCurrentNode != nullptr) {
		...
		if ((var$0 & ($sl(1, $nc(node)->getNodeType()))) == 0) {
...
void DOMResultBuilder::clinit$($Class* clazz) {
	{
		$assignStatic(DOMResultBuilder::kidOK, $new($ints, 13));
		DOMResultBuilder::kidOK->set($Node::DOCUMENT_NODE, (($sl(1, $Node::ELEMENT_NODE) | $sl(1, $Node::PROCESSING_INSTRUCTION_NODE)) | $sl(1, $Node::COMMENT_NODE)) | $sl(1, $Node::DOCUMENT_TYPE_NODE));
		DOMResultBuilder::kidOK->set($Node::DOCUMENT_FRAGMENT_NODE, DOMResultBuilder::kidOK->set($Node::ENTITY_NODE, DOMResultBuilder::kidOK->set($Node::ENTITY_REFERENCE_NODE, DOMResultBuilder::kidOK->set($Node::ELEMENT_NODE, (((($sl(1, $Node::ELEMENT_NODE) | $sl(1, $Node::PROCESSING_INSTRUCTION_NODE)) | $sl(1, $Node::COMMENT_NODE)) | $sl(1, $Node::TEXT_NODE)) | $sl(1, $Node::CDATA_SECTION_NODE)) | $sl(1, $Node::ENTITY_REFERENCE_NODE)))));
		DOMResultBuilder::kidOK->set($Node::ATTRIBUTE_NODE, $sl(1, $Node::TEXT_NODE) | $sl(1, $Node::ENTITY_REFERENCE_NODE));
		DOMResultBuilder::kidOK->set($Node::DOCUMENT_TYPE_NODE, 0);
		DOMResultBuilder::kidOK->set($Node::PROCESSING_INSTRUCTION_NODE, 0);
		DOMResultBuilder::kidOK->set($Node::COMMENT_NODE, 0);
		DOMResultBuilder::kidOK->set($Node::TEXT_NODE, 0);
		DOMResultBuilder::kidOK->set($Node::CDATA_SECTION_NODE, 0);
		DOMResultBuilder::kidOK->set($Node::NOTATION_NODE, 0);
	}
}
...
$Class* DOMResultBuilder::load$($String* name, bool initialize) {
	...
	$loadClass(DOMResultBuilder, name, initialize, &classInfo$$, DOMResultBuilder::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(DOMResultBuilder);
	});
	return class$;
}
