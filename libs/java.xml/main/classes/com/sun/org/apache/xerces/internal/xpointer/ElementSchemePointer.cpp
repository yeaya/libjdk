...
void ElementSchemePointer::parseXPointer($String* xpointer) {
	...
	if (!success) {
		reportError("InvalidElementSchemeXPointer"_s, $$new($ObjectArray, {xpointer}));
	}
...
bool ElementSchemePointer::matchChildSequence($QName* element, int32_t event) {
	if (this->fCurrentChildDepth >= $nc(this->fCurrentChildSequence)->length) {
		$var($ints, tmpCurrentChildSequence, $new($ints, this->fCurrentChildSequence->length));
	...
	if (this->fIsResolveElement) {
		...
		if (event == $XPointerPart::EVENT_ELEMENT_START) {
			this->fCurrentChildSequence->set(this->fCurrentChildDepth, this->fCurrentChildPosition);
			...
			--this->fCurrentChildDepth;
			this->fCurrentChildPosition = this->fCurrentChildSequence->get(this->fCurrentChildDepth) + 1;
...
bool ElementSchemePointer::checkMatch() {
	if (!this->fIsShortHand) {
		if ($nc(this->fChildSequence)->length <= this->fCurrentChildDepth + 1) {
			for (int32_t i = 0; i < this->fChildSequence->length; ++i) {
				if (this->fChildSequence->get(i) != $nc(this->fCurrentChildSequence)->get(i)) {
					return false;
				}
			}
		} else {
		...
		for (int32_t i = 0; i < this->fChildSequence->length; ++i) {
			...
			if (this->fChildSequence->get(i) != this->fCurrentChildSequence->get(i + 1)) {
				return false;
			}
		}
	} else {
...
$Class* ElementSchemePointer::load$($String* name, bool initialize) {
	...
	$loadClass(ElementSchemePointer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ElementSchemePointer);
	});
	return class$;
}
