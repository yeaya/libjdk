...
$XMLGregorianCalendar* YearDV::getXMLGregorianCalendar($AbstractDateTimeDV$DateTimeData* date) {
	...
	return $nc($AbstractDateTimeDV::datatypeFactory)->newXMLGregorianCalendar($nc(date)->unNormYear, $DatatypeConstants::FIELD_UNDEFINED, $DatatypeConstants::FIELD_UNDEFINED, $DatatypeConstants::FIELD_UNDEFINED, $DatatypeConstants::FIELD_UNDEFINED, $DatatypeConstants::FIELD_UNDEFINED, $DatatypeConstants::FIELD_UNDEFINED, $nc(date)->hasTimeZone() ? date->timezoneHr * 60 + date->timezoneMin : $DatatypeConstants::FIELD_UNDEFINED);
}
...
$Class* YearDV::load$($String* name, bool initialize) {
	...
	$loadClass(YearDV, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(YearDV);
	});
	return class$;
}
