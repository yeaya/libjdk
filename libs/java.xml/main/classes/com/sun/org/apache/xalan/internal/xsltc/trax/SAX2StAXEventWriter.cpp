...
void SAX2StAXEventWriter::writeStartDocument() {
	...
	try {
		$nc(this->writer)->add($($nc(this->eventFactory)->createStartDocument(this->encoding, this->xmlVersion)));
...
void SAX2StAXEventWriter::endDocument() {
	...
	$nc(this->eventFactory)->setLocation($(getCurrentLocation()));
	try {
		$nc(this->writer)->add($($nc(this->eventFactory)->createEndDocument()));
	} catch ($XMLStreamException& e) {
		$throwNew($SAXException, e);
	}
...
void SAX2StAXEventWriter::startElement($String* uri, $String* localName, $String* qName, $Attributes* attributes) {
	...
	try {
		try {
			...
			$var($String, var$2, qname->get(1));
			$var($Iterator, var$3, $nc(events->get(1))->iterator());
			$nc(this->writer)->add($($nc(this->eventFactory)->createStartElement(var$1, uri, var$2, var$3, $($nc(events->get(0))->iterator()))));
		} catch ($XMLStreamException& e) {
			$throwNew($SAXException, e);
		}
...
void SAX2StAXEventWriter::endElement($String* uri, $String* localName, $String* qName) {
	...
	try {
		$nc(this->writer)->add($($nc(this->eventFactory)->createEndElement(qname->get(0), uri, qname->get(1), nsIter)));
	} catch ($XMLStreamException& e) {
		$throwNew($SAXException, e);
	}
}
...
void SAX2StAXEventWriter::comment($chars* ch, int32_t start, int32_t length) {
	...
	$nc(this->eventFactory)->setLocation($(getCurrentLocation()));
	try {
		$nc(this->writer)->add($($nc(this->eventFactory)->createComment($$new($String, ch, start, length))));
	} catch ($XMLStreamException& e) {
		$throwNew($SAXException, e);
	}
}
...
void SAX2StAXEventWriter::characters($chars* ch, int32_t start, int32_t length) {
	...
	try {
		if (!this->isCDATA) {
			$nc(this->eventFactory)->setLocation($(getCurrentLocation()));
			$nc(this->writer)->add($($nc(this->eventFactory)->createCharacters($$new($String, ch, start, length))));
		}
	} catch ($XMLStreamException& e) {
		$throwNew($SAXException, e);
	}
...
void SAX2StAXEventWriter::processingInstruction($String* target, $String* data) {
	...
	try {
		$nc(this->writer)->add($($nc(this->eventFactory)->createProcessingInstruction(target, data)));
	} catch ($XMLStreamException& e) {
		$throwNew($SAXException, e);
	}
}
...
void SAX2StAXEventWriter::endCDATA() {
	...
	$nc(this->eventFactory)->setLocation($(getCurrentLocation()));
	try {
		$nc(this->writer)->add($($nc(this->eventFactory)->createCData($($nc(this->CDATABuffer)->toString()))));
	} catch ($XMLStreamException& e) {
		$throwNew($SAXException, e);
	}
...
void SAX2StAXEventWriter::createStartEvents($Attributes* attributes, $CollectionArray* events) {
	...
	if (this->namespaces != nullptr) {
		int32_t nDecls = this->namespaces->size();
		for (int32_t i = 0; i < nDecls; ++i) {
			$var($String, prefix, $cast($String, $nc(this->namespaces)->get(i++)));
			$var($String, uri, $cast($String, this->namespaces->get(i)));
	...
	$init($Collections);
	$nc(events)->set(0, (nsMap == nullptr ? $cast($Collection, $Collections::EMPTY_LIST) : $(nsMap->values())));
	events->set(1, (attrs == nullptr ? $Collections::EMPTY_LIST : attrs));
}
...
$Class* SAX2StAXEventWriter::load$($String* name, bool initialize) {
	...
	$loadClass(SAX2StAXEventWriter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
