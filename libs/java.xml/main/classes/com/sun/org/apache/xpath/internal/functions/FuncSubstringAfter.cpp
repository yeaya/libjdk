...
$XObject* FuncSubstringAfter::execute($XPathContext* xctxt) {
	...
	$init($XString);
	return (-1 == index) ? $XString::EMPTYSTRING : $cast($XString, s1->substring(index + $nc(s2)->length()));
}
...
$Class* FuncSubstringAfter::load$($String* name, bool initialize) {
	...
	$loadClass(FuncSubstringAfter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
