...
void XMLErrorResources_ja::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(XMLErrorResources_ja::contents, $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_FUNCTION_NOT_SUPPORTED,
			u"関数がサポートされていません。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_CANNOT_OVERWRITE_CAUSE,
			u"原因を上書きできません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_NO_DEFAULT_IMPL,
			u"デフォルト実装が見つかりません "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_CHUNKEDINTARRAY_NOT_SUPPORTED,
			u"ChunkedIntArray({0})は現在サポートされていません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_OFFSET_BIGGER_THAN_SLOT,
			u"オフセットがスロットよりも大きいです"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_COROUTINE_NOT_AVAIL,
			u"コルーチンを使用できません。id={0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_COROUTINE_CO_EXIT,
			u"CoroutineManagerがco_exit()リクエストを受け取りました"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_COJOINROUTINESET_FAILED,
			u"co_joinCoroutineSet()が失敗しました"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_COROUTINE_PARAM,
			u"コルーチン・パラメータのエラー({0})"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_PARSER_DOTERMINATE_ANSWERS,
			u"\n不明: パーサーdoTerminateの応答は{0}です"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_NO_PARSE_CALL_WHILE_PARSING,
			u"解析は構文解析中に呼び出すことができません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_TYPED_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"エラー: 軸{0}の型指定されたイテレータが実装されていません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"エラー: 軸{0}のイテレータが実装されていません "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_ITERATOR_CLONE_NOT_SUPPORTED,
			u"イテレータのクローンはサポートされていません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_UNKNOWN_AXIS_TYPE,
			u"不明な軸トラバース・タイプです: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_AXIS_NOT_SUPPORTED,
			u"軸トラバーサ機能はサポートされていません: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_NO_DTMIDS_AVAIL,
			u"DTM IDはこれ以上使用できません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_NOT_SUPPORTED,
			u"サポートされていません: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_NODE_NON_NULL,
			u"ノードはgetDTMHandleFromNodeについて非nullである必要があります"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_COULD_NOT_RESOLVE_NODE,
			u"ノードをハンドルに解決できませんでした"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_STARTPARSE_WHILE_PARSING,
			u"startParseは構文解析中に呼び出すことはできません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_STARTPARSE_NEEDS_SAXPARSER,
			u"startParseには非nullのSAXParserが必要です"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_COULD_NOT_INIT_PARSER,
			u"次の理由でパーサーを初期化できませんでした: "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_EXCEPTION_CREATING_POOL,
			u"プール用の新規インスタンスの作成中に発生した例外"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_PATH_CONTAINS_INVALID_ESCAPE_SEQUENCE,
			u"パスに無効なエスケープ・シーケンスが含まれています"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_SCHEME_REQUIRED,
			u"スキームが必要です。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_NO_SCHEME_IN_URI,
			u"スキームがURIに見つかりません: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_NO_SCHEME_INURI,
			u"スキームがURIに見つかりません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_PATH_INVALID_CHAR,
			u"パスに無効な文字が含まれています: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_SCHEME_FROM_NULL_STRING,
			u"null文字列からはスキームを設定できません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_SCHEME_NOT_CONFORMANT,
			u"スキームが整合していません。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_HOST_ADDRESS_NOT_WELLFORMED,
			u"ホストは整形式のアドレスではありません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_PORT_WHEN_HOST_NULL,
			u"ホストがnullの場合はポートを設定できません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_INVALID_PORT,
			u"無効なポート番号"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_FRAG_FOR_GENERIC_URI,
			u"汎用URIのフラグメントのみ設定できます"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_FRAG_WHEN_PATH_NULL,
			u"パスがnullの場合はフラグメントを設定できません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_FRAG_INVALID_CHAR,
			u"フラグメントに無効文字が含まれています"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_PARSER_IN_USE,
			u"パーサーはすでに使用中です"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_CANNOT_CHANGE_WHILE_PARSING,
			u"解析中に{0} {1}を変更できません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_SELF_CAUSATION_NOT_PERMITTED,
			u"自己原因は許可されません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_NO_USERINFO_IF_NO_HOST,
			u"ホストが指定されていない場合はUserinfoを指定できません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_NO_PORT_IF_NO_HOST,
			u"ホストが指定されていない場合はポートを指定できません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_NO_QUERY_STRING_IN_PATH,
			u"問合せ文字列はパスおよび問合せ文字列内に指定できません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_NO_FRAGMENT_STRING_IN_PATH,
			u"フラグメントはパスとフラグメントの両方に指定できません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_CANNOT_INIT_URI_EMPTY_PARMS,
			u"URIは空のパラメータを使用して初期化できません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_METHOD_NOT_SUPPORTED,
			u"メソッドはまだサポートされていません "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_INCRSAXSRCFILTER_NOT_RESTARTABLE,
			u"IncrementalSAXSource_Filterは現在は再起動可能ではありません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_XMLRDR_NOT_BEFORE_STARTPARSE,
			u"XMLReaderはstartParseリクエストより前にできません"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_AXIS_TRAVERSER_NOT_SUPPORTED,
			u"軸トラバーサ機能はサポートされていません: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_ERRORHANDLER_CREATED_WITH_NULL_PRINTWRITER,
			u"null PrintWriterによってListingErrorHandlerが作成されました。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_SYSTEMID_UNKNOWN,
			u"不明なSystemId"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_LOCATION_UNKNOWN,
			u"エラーの場所が不明です"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_PREFIX_MUST_RESOLVE,
			u"接頭辞はネームスペースに解決される必要があります: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_CREATEDOCUMENT_NOT_SUPPORTED,
			u"createDocument()はXPathContextでサポートされていません。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_CHILD_HAS_NO_OWNER_DOCUMENT,
			u"属性の子に所有者ドキュメントがありません。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_CHILD_HAS_NO_OWNER_DOCUMENT_ELEMENT,
			u"属性の子に所有者ドキュメント要素がありません。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_CANT_OUTPUT_TEXT_BEFORE_DOC,
			u"警告: ドキュメント要素の前にテキストを出力できません。  無視します..."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_CANT_HAVE_MORE_THAN_ONE_ROOT,
			u"DOMに複数のルートを持つことはできません。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_ARG_LOCALNAME_NULL,
			u"引数\'localName\'はnullです"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_ARG_LOCALNAME_INVALID,
			u"QNAMEのLocalnameは有効なNCNameである必要があります"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_ARG_PREFIX_INVALID,
			u"QNAMEの接頭辞は有効なNCNameである必要があります"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_NAME_CANT_START_WITH_COLON,
			u"名前の先頭をコロンにすることはできません"_s
		}),
		$$new($ObjectArray, {
			"BAD_CODE"_s,
			u"createMessageのパラメータが範囲外です"_s
		}),
		$$new($ObjectArray, {
			"FORMAT_FAILED"_s,
			u"messageFormatの呼出し中に例外がスローされました"_s
		}),
		$$new($ObjectArray, {
			"line"_s,
			u"行番号"_s
		...
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_SERIALIZER_NOT_CONTENTHANDLER,
			u"シリアライザ・クラス\'\'{0}\'\'はorg.xml.sax.ContentHandlerを実装しません。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_RESOURCE_COULD_NOT_FIND,
			u"リソース[ {0} ]は見つかりませんでした。\n {1}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_RESOURCE_COULD_NOT_LOAD,
			u"リソース[ {0} ]をロードできませんでした: {1} \n {2} \t {3}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_BUFFER_SIZE_LESSTHAN_ZERO,
			u"バッファ・サイズ<=0"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_INVALID_UTF16_SURROGATE,
			u"無効なUTF-16サロゲートが検出されました: {0}。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_OIERROR,
			u"IOエラー"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_ILLEGAL_ATTRIBUTE_POSITION,
			u"子ノードの後または要素が生成される前に属性{0}を追加できません。属性は無視されます。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_NAMESPACE_PREFIX,
			u"接頭辞\'\'{0}\'\'のネームスペースが宣言されていません。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_STRAY_ATTRIBUTE,
			u"属性\'\'{0}\'\'が要素の外側にあります。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_STRAY_NAMESPACE,
			u"ネームスペース宣言\'\'{0}\'\'=\'\'{1}\'\'が要素の外側にあります。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_COULD_NOT_LOAD_RESOURCE,
			u"\'\'{0}\'\'をロードできませんでした(CLASSPATHを確認してください)。現在は単にデフォルトを使用しています"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_ILLEGAL_CHARACTER,
			u"{1}の指定された出力エンコーディングで示されない整数値{0}の文字を出力しようとしました。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ja::ER_COULD_NOT_LOAD_METHOD_PROPERTY,
...
$Class* XMLErrorResources_ja::load$($String* name, bool initialize) {
	...
	$loadClass(XMLErrorResources_ja, name, initialize, &classInfo$$, XMLErrorResources_ja::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLErrorResources_ja);
	});
	return class$;
}
