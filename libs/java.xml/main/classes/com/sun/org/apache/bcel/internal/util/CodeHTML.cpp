...
void CodeHTML::init$($String* dir, $String* class_name, $MethodArray* methods, $ConstantPool* constant_pool, $ConstantHTML* constant_html) {
	...
	$set(this, file, $new($PrintWriter, $$new($FileOutputStream, $$str({dir, class_name, "_code.html"_s}))));
	this->file->println("<HTML><BODY BGCOLOR=\"#C0C0C0\">"_s);
	for (int32_t i = 0; i < $nc(methods)->length; ++i) {
		writeMethod(methods->get(i), i);
	}
	this->file->println("</BODY></HTML>"_s);
	this->file->close();
}
...
$String* CodeHTML::codeToHTML($ByteSequence* bytes, int32_t method_number) {
	...
	}
	{
		...
		case $Const::TABLESWITCH:
			{
				...
				for (int32_t i = 0; i < jump_table->length; ++i) {
		...
		case $Const::LOOKUPSWITCH:
			...
			buf->append("<TH>default</TH></TR>\n<TR>"_s);
			for (int32_t i = 0; i < npairs; ++i) {
				buf->append("<TD><A HREF=\"#code"_s)->append(method_number)->append("@"_s)->append(jump_table->get(i))->append("\">"_s)->append(jump_table->get(i))->append("</A></TD>"_s);
			}
			buf->append("<TD><A HREF=\"#code"_s)->append(method_number)->append("@"_s)->append(default_offset)->append("\">"_s)->append(default_offset)->append("</A></TD></TR>\n</TABLE>\n"_s);
			break;
		case $Const::GOTO:
		...
		case $Const::PUTSTATIC:
			...
			$assign(name, this->constantPool->getConstantString(class_index, $Const::CONSTANT_Class));
			$assign(name, $Utility::compactClassName(name, false));
			index = c1->getNameAndTypeIndex();
			$assign(field_name, this->constantPool->constantToString(index, $Const::CONSTANT_NameAndType));
		...
		case $Const::INVOKEDYNAMIC:
			...
			$assign(c2, $cast($ConstantNameAndType, this->constantPool->getConstant(index, $Const::CONSTANT_NameAndType)));
		...
		$var($CodeExceptionArray, ce, code->getExceptionTable());
		{
			...
			for (int32_t len$ = $nc(arr$)->length, i$ = 0; i$ < len$; ++i$) {
				$var($CodeException, cex, arr$->get(i$));
				{
					...
					$nc(this->gotoSet)->set(cex->getEndPC());
					$nc(this->gotoSet)->set(cex->getHandlerPC());
				}
			}
		}
		$var($AttributeArray, attributes, code->getAttributes());
		{
			...
			for (; i$ < len$; ++i$) {
				...
				if ($nc(attribute)->getTag() == $Const::ATTR_LOCAL_VARIABLE_TABLE) {
					$var($LocalVariableArray, vars, $cast($LocalVariableTable, attribute)->getLocalVariableTable());
					{
...
void CodeHTML::writeMethod($Method* method, int32_t method_number) {
	...
	this->file->print($$str({"<P><B><FONT COLOR=\"#FF0000\">"_s, access, "</FONT>&nbsp;<A NAME=method"_s, $$str(method_number), ">"_s, $($Class2HTML::referenceType(type)), "</A>&nbsp<A HREF=\""_s, this->className, "_methods.html#method"_s, $$str(method_number), "\" TARGET=Methods>"_s, html_name, "</A>("_s}));
	for (int32_t i = 0; i < $nc(args)->length; ++i) {
		this->file->print($($Class2HTML::referenceType(args->get(i))));
		if (i < args->length - 1) {
			this->file->print(",&nbsp;"_s);
		}
	}
	this->file->println(")</B></P>"_s);
	$var($Code, c, nullptr);
	...
	if ($nc(attributes)->length > 0) {
		...
		for (int32_t i = 0; i < attributes->length; ++i) {
			...
			if (tag != $Const::ATTR_UNKNOWN) {
				this->file->print($$str({"<LI><A HREF=\""_s, this->className, "_attributes.html#method"_s, $$str(method_number), "@"_s, $$str(i), "\" TARGET=Attributes>"_s, $($Const::getAttributeName(tag)), "</A></LI>\n"_s}));
			} else {
				this->file->print($$str({"<LI>"_s, attributes->get(i), "</LI>"_s}));
			}
			if (tag == $Const::ATTR_CODE) {
				...
				for (int32_t j = 0; j < $nc(attributes2)->length; ++j) {
					...
					this->file->print($$str({"<LI><A HREF=\""_s, this->className, "_attributes.html#method"_s, $$str(method_number), "@"_s, $$str(i), "@"_s, $$str(j), "\" TARGET=Attributes>"_s, $($Const::getAttributeName(tag)), "</A></LI>\n"_s}));
				}
				this->file->print("</UL>"_s);
			}
		}
		this->file->println("</UL>"_s);
	}
	if (code != nullptr) {
		{
			...
			try {
				try {
					...
					findGotos(stream, c);
					stream->reset();
					this->file->println("<TABLE BORDER=0><TR><TH ALIGN=LEFT>Byte<BR>offset</TH><TH ALIGN=LEFT>Instruction</TH><TH ALIGN=LEFT>Argument</TH>"_s);
					for (; stream->available() > 0;) {
						...
						this->file->println($$str({"<TR VALIGN=TOP><TD>"_s, anchor2, "</TD><TD>"_s, anchor, str, "</TR>"_s}));
					}
				} catch ($Throwable& t$) {
					try {
						stream->close();
		...
		this->file->println("<TR><TD> </A></TD></TR>"_s);
		this->file->println("</TABLE>"_s);
	}
}

void CodeHTML::clinit$($Class* clazz) {
	CodeHTML::wide = false;
}
...
$Class* CodeHTML::load$($String* name, bool initialize) {
	...
	$loadClass(CodeHTML, name, initialize, &classInfo$$, CodeHTML::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(CodeHTML);
	});
	return class$;
}
