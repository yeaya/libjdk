...
void XMLErrorResources_cs::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(XMLErrorResources_cs::_contents, $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_FUNCTION_NOT_SUPPORTED,
			u"Nepodporovaná funkce!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_CANNOT_OVERWRITE_CAUSE,
			u"Příčinu nelze přepsat"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_NO_DEFAULT_IMPL,
			u"Nebyla nalezena výchozí implementace. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_CHUNKEDINTARRAY_NOT_SUPPORTED,
			u"Funkce ChunkedIntArray({0}) není aktuálně podporována."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_OFFSET_BIGGER_THAN_SLOT,
			u"Offset je větší než slot."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_COROUTINE_NOT_AVAIL,
			u"Společná rutina není k dispozici, id={0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_COROUTINE_CO_EXIT,
			u"Funkce CoroutineManager obdržela požadavek co_exit()"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_COJOINROUTINESET_FAILED,
			"Selhala funkce co_joinCoroutineSet()"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_COROUTINE_PARAM,
			u"Chyba parametru společné rutiny ({0})"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_PARSER_DOTERMINATE_ANSWERS,
			u"\nNeočekávané: odpovědi funkce analyzátoru doTerminate {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_NO_PARSE_CALL_WHILE_PARSING,
			u"během analýzy nelze volat analyzátor"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_TYPED_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"Chyba: zadaný iterátor osy {0} není implementován"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"Chyba: zadaný iterátor osy {0} není implementován "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_ITERATOR_CLONE_NOT_SUPPORTED,
			u"Nepodporovaný klon iterátoru."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_UNKNOWN_AXIS_TYPE,
			u"Neznámý typ osy průchodu: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_AXIS_NOT_SUPPORTED,
			u"Nepodporovaná osa průchodu: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_NO_DTMIDS_AVAIL,
			u"Žádná další ID DTM nejsou k dispozici"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_NOT_SUPPORTED,
			u"Nepodporováno: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_NODE_NON_NULL,
			u"Uzel použitý ve funkci getDTMHandleFromNode musí mít hodnotu not-null"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_COULD_NOT_RESOLVE_NODE,
			u"Uzel nelze přeložit do manipulátoru"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_STARTPARSE_WHILE_PARSING,
			u"Během analýzy nelze volat funkci startParse."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_STARTPARSE_NEEDS_SAXPARSER,
			u"Funkce startParse vyžaduje SAXParser s hodnotou not-null."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_COULD_NOT_INIT_PARSER,
			u"nelze inicializovat analyzátor s: "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_EXCEPTION_CREATING_POOL,
			u"výjimka při vytváření nové instance společné oblasti"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_PATH_CONTAINS_INVALID_ESCAPE_SEQUENCE,
			"Cesta obsahuje neplatnou escape sekvenci"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_SCHEME_REQUIRED,
			u"Je vyžadováno schéma!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_NO_SCHEME_IN_URI,
			u"V URI nebylo nalezeno žádné schéma: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_NO_SCHEME_INURI,
			u"V URI nebylo nalezeno žádné schéma"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_PATH_INVALID_CHAR,
			u"Cesta obsahuje neplatný znak: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_SCHEME_FROM_NULL_STRING,
			u"Nelze nastavit schéma řetězce s hodnotou null."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_SCHEME_NOT_CONFORMANT,
			u"Schéma nevyhovuje."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_HOST_ADDRESS_NOT_WELLFORMED,
			u"Adresa hostitele má nesprávný formát."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_PORT_WHEN_HOST_NULL,
			u"Má-li hostitel hodnotu null, nelze nastavit port."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_INVALID_PORT,
			u"Neplatné číslo portu."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_FRAG_FOR_GENERIC_URI,
			u"Fragment lze nastavit jen u generického URI."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_FRAG_WHEN_PATH_NULL,
			u"Má-li cesta hodnotu null, nelze nastavit fragment."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_FRAG_INVALID_CHAR,
			u"Fragment obsahuje neplatný znak."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_PARSER_IN_USE,
			u"Analyzátor se již používá."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_CANNOT_CHANGE_WHILE_PARSING,
			u"Během analýzy nelze měnit {0} {1}."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_SELF_CAUSATION_NOT_PERMITTED,
			u"Způsobení sama sebe (self-causation) není povoleno"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_NO_USERINFO_IF_NO_HOST,
			u"Není-li určen hostitel, nelze zadat údaje o uživateli."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_NO_PORT_IF_NO_HOST,
			u"Není-li určen hostitel, nelze zadat port."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_NO_QUERY_STRING_IN_PATH,
			u"V řetězci cesty a dotazu nelze zadat řetězec dotazu."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_NO_FRAGMENT_STRING_IN_PATH,
			u"Fragment nelze určit zároveň v cestě i ve fragmentu."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_CANNOT_INIT_URI_EMPTY_PARMS,
			u"URI nelze inicializovat s prázdnými parametry."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_METHOD_NOT_SUPPORTED,
			u"Prozatím nepodporovaná metoda. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_INCRSAXSRCFILTER_NOT_RESTARTABLE,
			u"Filtr IncrementalSAXSource_Filter nelze aktuálně znovu spustit."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_XMLRDR_NOT_BEFORE_STARTPARSE,
			u"Před požadavkem startParse není XMLReader."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_AXIS_TRAVERSER_NOT_SUPPORTED,
			u"Nepodporovaná osa průchodu: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_ERRORHANDLER_CREATED_WITH_NULL_PRINTWRITER,
			u"Prvek ListingErrorHandler byl vytvořen s funkcí PrintWriter s hodnotou null!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_SYSTEMID_UNKNOWN,
			u"Neznámý identifikátor SystemId"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_LOCATION_UNKNOWN,
			u"Chyba se vyskytla na neznámém místě"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_PREFIX_MUST_RESOLVE,
			u"Předponu musí být možno přeložit do oboru názvů: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_CREATEDOCUMENT_NOT_SUPPORTED,
			"Funkce XPathContext nepodporuje funkci createDocument()!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_CHILD_HAS_NO_OWNER_DOCUMENT,
			u"Potomek atributu nemá dokument vlastníka!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_CHILD_HAS_NO_OWNER_DOCUMENT_ELEMENT,
			u"Potomek atributu nemá prvek dokumentu vlastníka!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_CANT_OUTPUT_TEXT_BEFORE_DOC,
			u"Varování: výstup textu nemůže předcházet prvku dokumentu! Ignorováno..."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_CANT_HAVE_MORE_THAN_ONE_ROOT,
			u"DOM nemůže mít několik kořenů!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_ARG_LOCALNAME_NULL,
			u"Argument \'localName\' má hodnotu null"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_ARG_LOCALNAME_INVALID,
			u"Hodnota Localname ve funkci QNAME by měla být platným prvkem NCName"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_ARG_PREFIX_INVALID,
			u"Předpona ve funkci QNAME by měla být platným prvkem NCName"_s
		}),
		$$new($ObjectArray, {
			"BAD_CODE"_s,
			"Parametr funkce createMessage je mimo limit"_s
		}),
		$$new($ObjectArray, {
			"FORMAT_FAILED"_s,
			u"Při volání funkce messageFormat došlo k výjimce"_s
		}),
		$$new($ObjectArray, {
			"line"_s,
			u"Řádek #"_s
		...
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_SERIALIZER_NOT_CONTENTHANDLER,
			u"Třída serializace \'\'{0}\'\' neimplementuje org.xml.sax.ContentHandler."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_RESOURCE_COULD_NOT_FIND,
			u"Nelze najít zdroj [ {0} ].\n {1}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_RESOURCE_COULD_NOT_LOAD,
			u"Nelze zavést zdroj [ {0} ]: {1} \n {2} \t {3}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_BUFFER_SIZE_LESSTHAN_ZERO,
			u"Velikost vyrovnávací paměti <=0"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_INVALID_UTF16_SURROGATE,
			u"Byla zjištěna neplatná náhrada UTF-16: {0} ?"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_OIERROR,
			u"Chyba vstupu/výstupu"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_ILLEGAL_ATTRIBUTE_POSITION,
			u"Nelze přidat atribut {0} po uzlech potomků ani před tím, než je vytvořen prvek. Atribut bude ignorován."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_NAMESPACE_PREFIX,
			u"Obor názvů pro předponu \'\'{0}\'\' nebyl deklarován."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_STRAY_ATTRIBUTE,
			u"Atribut \'\'{0}\'\' je vně prvku."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_STRAY_NAMESPACE,
			u"Deklarace oboru názvů \'\'{0}\'\'=\'\'{1}\'\' je vně prvku."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_COULD_NOT_LOAD_RESOURCE,
			u"Nelze zavést \'\'{0}\'\' (zkontrolujte proměnnou CLASSPATH), proto se používají pouze výchozí hodnoty"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_cs::ER_COULD_NOT_LOAD_METHOD_PROPERTY,
...
$Class* XMLErrorResources_cs::load$($String* name, bool initialize) {
	...
	$loadClass(XMLErrorResources_cs, name, initialize, &classInfo$$, XMLErrorResources_cs::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLErrorResources_cs);
	});
	return class$;
}
