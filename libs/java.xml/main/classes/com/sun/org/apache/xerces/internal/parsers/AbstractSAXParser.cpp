...
void AbstractSAXParser::startDocument($XMLLocator* locator, $String* encoding, $NamespaceContext* namespaceContext, $Augmentations* augs) {
	...
	try {
		if (this->fDocumentHandler != nullptr) {
			if (locator != nullptr) {
				this->fDocumentHandler->setDocumentLocator($$new($AbstractSAXParser$LocatorProxy, this, locator));
			}
			$nc(this->fDocumentHandler)->startDocument();
		}
		if (this->fContentHandler != nullptr) {
			if (locator != nullptr) {
				this->fContentHandler->setDocumentLocator($$new($AbstractSAXParser$LocatorProxy, this, locator));
			}
...
void AbstractSAXParser::xmlDecl($String* version, $String* encoding, $String* standalone, $Augmentations* augs) {
	...
	if (this->fContentHandler != nullptr) {
		try {
			this->fContentHandler->declaration(version, encoding, standalone);
		} catch ($SAXException& e) {
			$throwNew($XNIException, e);
		}
	}
			...
			if (this->fNamespaces) {
				...
				if (!this->fNamespacePrefixes) {
					for (int32_t i = len - 1; i >= 0; --i) {
						...
						$init($XMLSymbols);
						if (($nc(this->fQName)->prefix == $XMLSymbols::PREFIX_XMLNS) || (this->fQName->rawname == $XMLSymbols::PREFIX_XMLNS)) {
					...
					for (int32_t i = len - 1; i >= 0; --i) {
						...
						$init($XMLSymbols);
						if (($nc(this->fQName)->prefix == $XMLSymbols::PREFIX_XMLNS) || (this->fQName->rawname == $XMLSymbols::PREFIX_XMLNS)) {
							$set(this->fQName, prefix, ""_s);
							$set(this->fQName, uri, ""_s);
							$set(this->fQName, localpart, ""_s);
...
void AbstractSAXParser::ignorableWhitespace($XMLString* text, $Augmentations* augs) {
	try {
		if (this->fDocumentHandler != nullptr) {
			this->fDocumentHandler->ignorableWhitespace($nc(text)->ch, $nc(text)->offset, $nc(text)->length);
		}
		if (this->fContentHandler != nullptr) {
...
void AbstractSAXParser::endElement($QName* element, $Augmentations* augs) {
	...
	try {
		if (this->fDocumentHandler != nullptr) {
			this->fDocumentHandler->endElement($nc(element)->rawname);
		}
		if (this->fContentHandler != nullptr) {
			$set(this, fAugmentations, augs);
			$var($String, uri, $nc(element)->uri != nullptr ? element->uri : ""_s);
...
void AbstractSAXParser::comment($XMLString* text, $Augmentations* augs) {
	try {
		if (this->fLexicalHandler != nullptr) {
			this->fLexicalHandler->comment($nc(text)->ch, 0, $nc(text)->length);
		}
	} catch ($SAXException& e) {
		$throwNew($XNIException, e);
	}
...
void AbstractSAXParser::processingInstruction($String* target, $XMLString* data, $Augmentations* augs) {
	...
	try {
		if (this->fDocumentHandler != nullptr) {
			this->fDocumentHandler->processingInstruction(target, $($nc(data)->toString()));
		}
		if (this->fContentHandler != nullptr) {
...
void AbstractSAXParser::attributeDecl($String* elementName, $String* attributeName, $String* type$renamed, $StringArray* enumeration, $String* defaultType, $XMLString* defaultValue, $XMLString* nonNormalizedDefaultValue, $Augmentations* augs) {
	...
	try {
		if (this->fDeclHandler != nullptr) {
			...
			if (var$0 || type->equals("ENUMERATION"_s)) {
			...
			$var($String, value, (defaultValue == nullptr) ? ($String*)nullptr : defaultValue->toString());
...
void AbstractSAXParser::internalEntityDecl($String* name, $XMLString* text, $XMLString* nonNormalizedText, $Augmentations* augs) {
	try {
		if (this->fDeclHandler != nullptr) {
			this->fDeclHandler->internalEntityDecl(name, $($nc(text)->toString()));
		}
	} catch ($SAXException& e) {
		$throwNew($XNIException, e);
	}
	...
	try {
		...
		if ($nc(featureId)->startsWith($Constants::SAX_FEATURE_PREFIX)) {
			...
			if (var$3 && featureId->endsWith($Constants::STRING_INTERNING_FEATURE)) {
				if (!state) {
					$throwNew($SAXNotSupportedException, $($SAXMessageFormatter::formatMessage($($nc(this->fConfiguration)->getLocale()), "false-not-supported"_s, $$new($ObjectArray, {featureId}))));
				}
				return;
			}
			...
			if (var$6 && featureId->endsWith($Constants::UNICODE_NORMALIZATION_CHECKING_FEATURE)) {
				if (state) {
					$throwNew($SAXNotSupportedException, $($SAXMessageFormatter::formatMessage($($nc(this->fConfiguration)->getLocale()), "true-not-supported"_s, $$new($ObjectArray, {featureId}))));
				}
				return;
			}
			...
			if (var$9) {
				$throwNew($SAXNotSupportedException, $($SAXMessageFormatter::formatMessage($($nc(this->fConfiguration)->getLocale()), "feature-read-only"_s, $$new($ObjectArray, {featureId}))));
			}
		} else {
			$init($XMLConstants);
		...
		if ($nc(propertyId)->startsWith($Constants::SAX_PROPERTY_PREFIX)) {
			...
			if (var$1 && propertyId->endsWith($Constants::LEXICAL_HANDLER_PROPERTY)) {
				try {
					...
					$throwNew($SAXNotSupportedException, $($SAXMessageFormatter::formatMessage($($nc(this->fConfiguration)->getLocale()), "incompatible-class"_s, $$new($ObjectArray, {
						propertyId,
						"org.xml.sax.ext.LexicalHandler"_s
					}))));
				}
				return;
			}
			...
			if (var$2 && propertyId->endsWith($Constants::DECLARATION_HANDLER_PROPERTY)) {
				try {
					...
					$throwNew($SAXNotSupportedException, $($SAXMessageFormatter::formatMessage($($nc(this->fConfiguration)->getLocale()), "incompatible-class"_s, $$new($ObjectArray, {
						propertyId,
						"org.xml.sax.ext.DeclHandler"_s
					}))));
				}
				return;
			}
			...
			if (var$3) {
				$throwNew($SAXNotSupportedException, $($SAXMessageFormatter::formatMessage($($nc(this->fConfiguration)->getLocale()), "property-read-only"_s, $$new($ObjectArray, {propertyId}))));
			}
		}
...
void AbstractSAXParser::setDeclHandler($DeclHandler* handler) {
	...
	if (this->fParseInProgress) {
		$throwNew($SAXNotSupportedException, $($SAXMessageFormatter::formatMessage($($nc(this->fConfiguration)->getLocale()), "property-not-parsing-supported"_s, $$new($ObjectArray, {"http://xml.org/sax/properties/declaration-handler"_s}))));
	}
...
void AbstractSAXParser::setLexicalHandler($LexicalHandler* handler) {
	$useLocalObjectStack();
	if (this->fParseInProgress) {
		$throwNew($SAXNotSupportedException, $($SAXMessageFormatter::formatMessage($($nc(this->fConfiguration)->getLocale()), "property-not-parsing-supported"_s, $$new($ObjectArray, {"http://xml.org/sax/properties/lexical-handler"_s}))));
	}
...
$ElementPSVI* AbstractSAXParser::getElementPSVI() {
	$init($Constants);
	return (this->fAugmentations != nullptr) ? $cast($ElementPSVI, this->fAugmentations->getItem($Constants::ELEMENT_PSVI)) : ($ElementPSVI*)nullptr;
}

$AttributePSVI* AbstractSAXParser::getAttributePSVI(int32_t index) {
	$init($Constants);
	return $cast($AttributePSVI, $$nc($nc(this->fAttributesProxy->fAttributes)->getAugmentations(index))->getItem($Constants::ATTRIBUTE_PSVI));
}

$AttributePSVI* AbstractSAXParser::getAttributePSVIByName($String* uri, $String* localname) {
	$init($Constants);
	return $cast($AttributePSVI, $$nc($nc(this->fAttributesProxy->fAttributes)->getAugmentations(uri, localname))->getItem($Constants::ATTRIBUTE_PSVI));
}

void AbstractSAXParser::clinit$($Class* clazz) {
	$init($Constants);
...
$Class* AbstractSAXParser::load$($String* name, bool initialize) {
	...
	$loadClass(AbstractSAXParser, name, initialize, &classInfo$$, AbstractSAXParser::clinit$, []($Class* clazz) -> $Object* {
