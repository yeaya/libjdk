...
void WalkerFactory::diagnoseIterator($String* name, int32_t analysis, $Compiler* compiler) {
	...
	var$0->append($($nc(compiler)->toString()));
	var$0->append(", "_s);
	var$0->append(name);
	var$0->append(", "_s);
	var$0->append($($Integer::toBinaryString(analysis)));
	var$0->append(", "_s);
	var$0->append($(getAnalysisString(analysis)));
...
$DTMIterator* WalkerFactory::newDTMIterator($Compiler* compiler, int32_t opPos, bool isTopLevel) {
	...
	if ($instanceOf($LocPathIterator, iter)) {
		$cast($LocPathIterator, iter)->setIsTopLevel(isTopLevel);
	}
	return iter;
}
...
int32_t WalkerFactory::getAxisFromStep($Compiler* compiler, int32_t stepOpCodePos) {
	...
	$throwNew($RuntimeException, $($XSLMessages::createXPATHMessage($XPATHErrorResources::ER_NULL_ERROR_HANDLER, $$new($ObjectArray, {$($Integer::toString(stepType))}))));
}
...
int32_t WalkerFactory::analyze($Compiler* compiler, int32_t stepOpCodePos, int32_t stepIndex) {
	...
	analysisResult |= (stepCount & WalkerFactory::BITS_COUNT);
		...
		for (; nullptr != pat; $assign(pat, $nc(pat)->getRelativePathPattern())) {
			...
			if (whatToShow == $DTMFilter::SHOW_ATTRIBUTE || whatToShow == $DTMFilter::SHOW_NAMESPACE) {
				...
				if (isDownwardAxisOfMany(axis)) {
					$var($String, var$0, pat->getNamespace());
					$var($StepPattern, attrPat, $new($StepPattern, whatToShow, var$0, $(pat->getLocalName()), newAxis, 0));
...
$StepPattern* WalkerFactory::createDefaultStepPattern($Compiler* compiler, int32_t opPos, $MatchPatternIterator* mpi, int32_t analysis, $StepPattern* tail, $StepPattern* head) {
	...
	if (nullptr == ai) {
		...
		$var($String, var$0, compiler->getStepNS(opPos));
		$assign(ai, $new($StepPattern, whatToShow, var$0, $(compiler->getStepLocalName(opPos)), axis, predicateAxis));
	}
	;
...
$AxesWalker* WalkerFactory::createDefaultWalker($Compiler* compiler, int32_t opPos, $WalkingIterator* lpi, int32_t analysis) {
	...
	int32_t stepType = $nc(compiler)->getOp(opPos);
	bool simpleInit = false;
	int32_t totalNumberWalkers = (analysis & WalkerFactory::BITS_COUNT);
	bool prevIsOneStepDown = true;
	switch (stepType) {
	case $OpCodes::OP_VARIABLE:
	case $OpCodes::OP_EXTFUNCTION:
	...
	if (simpleInit) {
		...
		if ((0 == (whatToShow & ((($DTMFilter::SHOW_ATTRIBUTE | $DTMFilter::SHOW_NAMESPACE) | $DTMFilter::SHOW_ELEMENT) | $DTMFilter::SHOW_PROCESSING_INSTRUCTION))) || (whatToShow == $DTMFilter::SHOW_ALL)) {
			...
			$var($String, var$0, compiler->getStepNS(opPos));
			$nc(ai)->initNodeTest(whatToShow, var$0, $(compiler->getStepLocalName(opPos)));
		}
	}
	return ai;
}
...
$String* WalkerFactory::getAnalysisString(int32_t analysis) {
	...
	if ((analysis & WalkerFactory::BIT_NODETEST_ANY) != 0) {
		buf->append("NTANY|"_s);
	}
	if ((analysis & WalkerFactory::BIT_PREDICATE) != 0) {
		buf->append("PRED|"_s);
	}
	if ((analysis & WalkerFactory::BIT_ANCESTOR) != 0) {
		buf->append("ANC|"_s);
	}
	if ((analysis & WalkerFactory::BIT_ANCESTOR_OR_SELF) != 0) {
		buf->append("ANCOS|"_s);
	}
	if ((analysis & WalkerFactory::BIT_ATTRIBUTE) != 0) {
		buf->append("ATTR|"_s);
	}
	if ((analysis & WalkerFactory::BIT_CHILD) != 0) {
		buf->append("CH|"_s);
	}
	if ((analysis & WalkerFactory::BIT_DESCENDANT) != 0) {
		buf->append("DESC|"_s);
	}
	if ((analysis & WalkerFactory::BIT_DESCENDANT_OR_SELF) != 0) {
		buf->append("DESCOS|"_s);
	}
	if ((analysis & WalkerFactory::BIT_FOLLOWING) != 0) {
		buf->append("FOL|"_s);
	}
	if ((analysis & WalkerFactory::BIT_FOLLOWING_SIBLING) != 0) {
		buf->append("FOLS|"_s);
	}
	if ((analysis & WalkerFactory::BIT_NAMESPACE) != 0) {
		buf->append("NS|"_s);
	}
	if ((analysis & WalkerFactory::BIT_PARENT) != 0) {
		buf->append("P|"_s);
	}
	if ((analysis & WalkerFactory::BIT_PRECEDING) != 0) {
		buf->append("PREC|"_s);
	}
	if ((analysis & WalkerFactory::BIT_PRECEDING_SIBLING) != 0) {
		buf->append("PRECS|"_s);
	}
	if ((analysis & WalkerFactory::BIT_SELF) != 0) {
		buf->append(".|"_s);
	}
	if ((analysis & WalkerFactory::BIT_FILTER) != 0) {
		buf->append("FLT|"_s);
	}
	if ((analysis & WalkerFactory::BIT_ROOT) != 0) {
		buf->append("R|"_s);
	}
	return buf->toString();
}
...
bool WalkerFactory::hasPredicate(int32_t analysis) {
	return (0 != (analysis & WalkerFactory::BIT_PREDICATE));
}

bool WalkerFactory::isWild(int32_t analysis) {
	return (0 != (analysis & WalkerFactory::BIT_NODETEST_ANY));
}
...
bool WalkerFactory::walksAttributes(int32_t analysis) {
	return (0 != (analysis & WalkerFactory::BIT_ATTRIBUTE));
}

bool WalkerFactory::walksNamespaces(int32_t analysis) {
	return (0 != (analysis & WalkerFactory::BIT_NAMESPACE));
}

bool WalkerFactory::walksChildren(int32_t analysis) {
	return (0 != (analysis & WalkerFactory::BIT_CHILD));
}
...
bool WalkerFactory::canCrissCross(int32_t analysis) {
	if (walksSelfOnly(analysis)) {
		...
		bool var$0 = walksDownOnly(analysis);
		if (var$0 && !canSkipSubtrees(analysis)) {
			return false;
			...
			bool var$1 = walksSubtree(analysis);
			if (var$1) {
				bool var$3 = walksSideways(analysis);
				bool var$2 = var$3 || walksUp(analysis);
				var$1 = var$2 || canSkipSubtrees(analysis);
			}
			if (var$1) {
				return true;
			} else {
...
bool WalkerFactory::isOneStep(int32_t analysis) {
	return (analysis & WalkerFactory::BITS_COUNT) == 1;
}

int32_t WalkerFactory::getStepCount(int32_t analysis) {
	return (analysis & WalkerFactory::BITS_COUNT);
}
...
$Class* WalkerFactory::load$($String* name, bool initialize) {
	...
	$loadClass(WalkerFactory, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(WalkerFactory);
	});
	return class$;
}
