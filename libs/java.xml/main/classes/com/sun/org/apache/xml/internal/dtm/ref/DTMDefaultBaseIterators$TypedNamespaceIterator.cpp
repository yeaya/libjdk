...
$Class* DTMDefaultBaseIterators$TypedNamespaceIterator::load$($String* name, bool initialize) {
	...
	$loadClass(DTMDefaultBaseIterators$TypedNamespaceIterator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DTMDefaultBaseIterators$TypedNamespaceIterator);
	});
	return class$;
}
