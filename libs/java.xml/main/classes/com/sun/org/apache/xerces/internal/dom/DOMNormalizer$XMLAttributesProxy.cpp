...
int32_t DOMNormalizer$XMLAttributesProxy::addAttribute($QName* qname, $String* attrType, $String* attrValue) {
	$useLocalObjectStack();
	int32_t index = $nc(this->fElement)->getXercesAttribute($nc(qname)->uri, $nc(qname)->localpart);
	if (index < 0) {
		$var($AttrImpl, attr, $cast($AttrImpl, $$sure($CoreDocumentImpl, $nc(this->fElement)->getOwnerDocument())->createAttributeNS(qname->uri, qname->rawname, qname->localpart)));
...
int32_t DOMNormalizer$XMLAttributesProxy::getLength() {
	return (this->fAttributes != nullptr) ? this->fAttributes->getLength() : 0;
}
...
void DOMNormalizer$XMLAttributesProxy::getName(int32_t attrIndex, $QName* attrName) {
	if (this->fAttributes != nullptr) {
		this->this$0->updateQName($$cast($Node, this->fAttributes->getItem(attrIndex)), attrName);
	}
}
...
$String* DOMNormalizer$XMLAttributesProxy::getPrefix(int32_t index) {
	$useLocalObjectStack();
	if (this->fAttributes != nullptr) {
		$var($Node, node, $cast($Node, this->fAttributes->getItem(index)));
...
void DOMNormalizer$XMLAttributesProxy::setValue(int32_t attrIndex, $String* attrValue) {
	if (this->fAttributes != nullptr) {
		$var($AttrImpl, attr, $cast($AttrImpl, this->fAttributes->getItem(attrIndex)));
...
$String* DOMNormalizer$XMLAttributesProxy::getValue(int32_t index) {
	return (this->fAttributes != nullptr) ? $$nc(this->fAttributes->item(index))->getNodeValue() : ""_s;
}
...
$String* DOMNormalizer$XMLAttributesProxy::getValue($String* uri, $String* localName) {
	if (this->fAttributes != nullptr) {
		$var($Node, node, this->fAttributes->getNamedItemNS(uri, localName));
		return (node != nullptr) ? node->getNodeValue() : ($String*)nullptr;
	}
	return nullptr;
}
...
bool DOMNormalizer$XMLAttributesProxy::isSpecified(int32_t attrIndex) {
	return $$sure($Attr, $nc(this->fAttributes)->getItem(attrIndex))->getSpecified();
}
...
$Class* DOMNormalizer$XMLAttributesProxy::load$($String* name, bool initialize) {
	...
	$loadClass(DOMNormalizer$XMLAttributesProxy, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DOMNormalizer$XMLAttributesProxy);
	});
	return class$;
}
