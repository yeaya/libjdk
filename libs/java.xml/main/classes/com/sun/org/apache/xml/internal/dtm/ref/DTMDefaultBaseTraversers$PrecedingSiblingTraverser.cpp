...
$Class* DTMDefaultBaseTraversers$PrecedingSiblingTraverser::load$($String* name, bool initialize) {
	...
	$loadClass(DTMDefaultBaseTraversers$PrecedingSiblingTraverser, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DTMDefaultBaseTraversers$PrecedingSiblingTraverser);
	});
	return class$;
}
