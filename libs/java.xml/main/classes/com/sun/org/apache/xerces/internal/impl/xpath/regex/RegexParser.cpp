...
$Token* RegexParser::parse($String* regex, int32_t options) {
	...
	if (this->references != nullptr) {
		for (int32_t i = 0; i < $nc(this->references)->size(); ++i) {
			$var($RegexParser$ReferencePosition, position, $cast($RegexParser$ReferencePosition, this->references->get(i)));
...
void RegexParser::next() {
	...
	if (this->context == RegexParser::S_INBRACKETS) {
		...
		case u'\\':
			...
			break;
		case u'-':
			if (this->offset < this->regexlen && this->regex->charAt(this->offset) == u'[') {
				++this->offset;
...
$Token* RegexParser::processCondition() {
	...
	if (u'1' <= ch && ch <= u'9') {
		...
		++this->offset;
		if (this->regex->charAt(this->offset) != u')') {
...
$Token* RegexParser::processBacksolidus_c() {
	int32_t ch2 = 0;
	if (this->offset >= this->regexlen || ((ch2 = $nc(this->regex)->charAt(this->offset++)) & 0xffe0) != 0x40) {
	...
	case RegexParser::T_CHAR:
		if (this->chardata == u'{' && this->offset < this->regexlen) {
			...
			if ((ch = $nc(this->regex)->charAt(off++)) >= u'0' && ch <= u'9') {
				min = ch - u'0';
				while (off < this->regexlen && (ch = this->regex->charAt(off++)) >= u'0' && ch <= u'9') {
					min = min * 10 + ch - u'0';
					if (min < 0) {
...
$RangeToken* RegexParser::processBacksolidus_pP(int32_t c) {
	...
	int32_t nameend = $nc(this->regex)->indexOf(u'}', namestart);
	if (nameend < 0) {
		$throw($(this->ex("parser.atom.3"_s, this->offset)));
	}
	$var($String, pname, this->regex->substring(namestart, nameend));
...
$RangeToken* RegexParser::parseCharacterClass(bool useNrange) {
	...
	while ((type = this->read()) != RegexParser::T_EOF) {
		...
		if (type == RegexParser::T_BACKSOLIDUS) {
			...
			int32_t nameend = $nc(this->regex)->indexOf(u':', this->offset);
			if (nameend < 0) {
			...
			$var($String, name, this->regex->substring(this->offset, nameend));
...
$Token* RegexParser::getTokenForShorthand(int32_t ch) {
	...
	case u'd':
		$init($Token);
		$assign(tok, this->isSet($RegularExpression::USE_UNICODE_CATEGORY) ? $cast($Token, $Token::getRange("Nd"_s, true)) : $Token::token_0to9);
		break;
	case u'D':
		$init($Token);
		$assign(tok, this->isSet($RegularExpression::USE_UNICODE_CATEGORY) ? $cast($Token, $Token::getRange("Nd"_s, false)) : $Token::token_not_0to9);
		break;
	case u'w':
		$init($Token);
		$assign(tok, this->isSet($RegularExpression::USE_UNICODE_CATEGORY) ? $cast($Token, $Token::getRange("IsWord"_s, true)) : $Token::token_wordchars);
		break;
	case u'W':
		$init($Token);
		$assign(tok, this->isSet($RegularExpression::USE_UNICODE_CATEGORY) ? $cast($Token, $Token::getRange("IsWord"_s, false)) : $Token::token_not_wordchars);
		break;
	case u's':
		$init($Token);
		$assign(tok, this->isSet($RegularExpression::USE_UNICODE_CATEGORY) ? $cast($Token, $Token::getRange("IsSpace"_s, true)) : $Token::token_spaces);
		break;
	case u'S':
		$init($Token);
		$assign(tok, this->isSet($RegularExpression::USE_UNICODE_CATEGORY) ? $cast($Token, $Token::getRange("IsSpace"_s, false)) : $Token::token_not_spaces);
		break;
	default:
...
$Class* RegexParser::load$($String* name, bool initialize) {
	...
	$loadClass(RegexParser, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(RegexParser);
	});
	return class$;
}
