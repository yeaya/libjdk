...
void StAXEvent2SAX::bridge() {
	...
	try {
		...
		if (event->getEventType() == $XMLStreamConstants::START_DOCUMENT) {
			...
			$set(this, version, $cast($StartDocument, event)->getVersion());
			if ($cast($StartDocument, event)->encodingSet()) {
				$set(this, encoding, $cast($StartDocument, event)->getCharacterEncodingScheme());
			}
			$assign(event, this->staxEventReader->nextEvent());
			$assign(event, this->staxEventReader->nextEvent());
		}
...
void StAXEvent2SAX::handleEndElement($EndElement* event) {
	...
	try {
		...
		$nc(this->_sax)->endElement(var$1, $(qName->getLocalPart()), qname);
		{
			...
			for (; $nc(i)->hasNext();) {
				$var($String, prefix, $$sure($Namespace, i->next())->getPrefix());
				if (prefix == nullptr) {
					$assign(prefix, ""_s);
				}
...
$Attributes* StAXEvent2SAX::getAttributes($StartElement* event) {
	...
	{
		$var($Iterator, i, event->getAttributes());
		for (; $nc(i)->hasNext();) {
...
$Class* StAXEvent2SAX::load$($String* name, bool initialize) {
	...
	$loadClass(StAXEvent2SAX, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
