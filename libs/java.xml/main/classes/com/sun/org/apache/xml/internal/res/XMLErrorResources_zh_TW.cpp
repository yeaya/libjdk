...
void XMLErrorResources_zh_TW::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(XMLErrorResources_zh_TW::contents, $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_FUNCTION_NOT_SUPPORTED,
			u"不支援函數！"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_CANNOT_OVERWRITE_CAUSE,
			u"無法覆寫原因"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_NO_DEFAULT_IMPL,
			u"找不到預設的實行"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_CHUNKEDINTARRAY_NOT_SUPPORTED,
			u"目前不支援 ChunkedIntArray({0})"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_OFFSET_BIGGER_THAN_SLOT,
			u"位移大於位置"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_COROUTINE_NOT_AVAIL,
			u"沒有可用的共同常式，id={0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_COROUTINE_CO_EXIT,
			u"CoroutineManager 收到 co_exit() 要求"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_COJOINROUTINESET_FAILED,
			u"co_joinCoroutineSet() 失敗"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_COROUTINE_PARAM,
			u"共同常式參數錯誤 ({0})"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_PARSER_DOTERMINATE_ANSWERS,
			u"\n未預期: 剖析器 doTerminate 答覆 {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_NO_PARSE_CALL_WHILE_PARSING,
			u"剖析時可能未呼叫 parse"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_TYPED_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"錯誤: 未實行軸 {0} 的類型重複程式"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"錯誤: 未實行軸 {0} 的重複程式"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_ITERATOR_CLONE_NOT_SUPPORTED,
			u"不支援重複程式複製"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_UNKNOWN_AXIS_TYPE,
			u"不明的軸周遊類型: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_AXIS_NOT_SUPPORTED,
			u"不支援軸周遊程式: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_NO_DTMIDS_AVAIL,
			u"不再有可用的 DTM ID"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_NOT_SUPPORTED,
			u"不支援: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_NODE_NON_NULL,
			u"節點必須是非空值的 getDTMHandleFromNode"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_COULD_NOT_RESOLVE_NODE,
			u"無法解析節點為控制代碼"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_STARTPARSE_WHILE_PARSING,
			u"剖析時可能未呼叫 startParse"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_STARTPARSE_NEEDS_SAXPARSER,
			u"startParse 需要非空值 SAXParser"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_COULD_NOT_INIT_PARSER,
			u"無法起始剖析器"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_EXCEPTION_CREATING_POOL,
			u"建立集區的新執行處理時發生異常狀況"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_PATH_CONTAINS_INVALID_ESCAPE_SEQUENCE,
			u"路徑包含無效的遁離序列"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_SCHEME_REQUIRED,
			u"配置是必要項目！"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_NO_SCHEME_IN_URI,
			u"在 URI 中找不到配置: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_NO_SCHEME_INURI,
			u"在 URI 找不到配置"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_PATH_INVALID_CHAR,
			u"路徑包含無效的字元: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_SCHEME_FROM_NULL_STRING,
			u"無法從空值字串設定配置"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_SCHEME_NOT_CONFORMANT,
			u"配置不一致。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_HOST_ADDRESS_NOT_WELLFORMED,
			u"主機沒有完整的位址"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_PORT_WHEN_HOST_NULL,
			u"主機為空值時，無法設定連接埠"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_INVALID_PORT,
			u"無效的連接埠號碼"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_FRAG_FOR_GENERIC_URI,
			u"只能對一般 URI 設定片段"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_FRAG_WHEN_PATH_NULL,
			u"路徑為空值時，無法設定片段"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_FRAG_INVALID_CHAR,
			u"片段包含無效的字元"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_PARSER_IN_USE,
			u"剖析器使用中"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_CANNOT_CHANGE_WHILE_PARSING,
			u"剖析時無法變更 {0} {1}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_SELF_CAUSATION_NOT_PERMITTED,
			u"不允許自行引發"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_NO_USERINFO_IF_NO_HOST,
			u"如果沒有指定主機，不可指定 Userinfo"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_NO_PORT_IF_NO_HOST,
			u"如果沒有指定主機，不可指定連接埠"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_NO_QUERY_STRING_IN_PATH,
			u"在路徑及查詢字串中不可指定查詢字串"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_NO_FRAGMENT_STRING_IN_PATH,
			u"路徑和片段不能同時指定片段"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_CANNOT_INIT_URI_EMPTY_PARMS,
			u"無法以空白參數起始設定 URI"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_METHOD_NOT_SUPPORTED,
			u"尚不支援方法"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_INCRSAXSRCFILTER_NOT_RESTARTABLE,
			u"IncrementalSAXSource_Filter 目前無法重新啟動"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_XMLRDR_NOT_BEFORE_STARTPARSE,
			u"XMLReader 不能在 startParse 要求之前"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_AXIS_TRAVERSER_NOT_SUPPORTED,
			u"不支援軸周遊程式: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_ERRORHANDLER_CREATED_WITH_NULL_PRINTWRITER,
			u"使用空值 PrintWriter 建立 ListingErrorHandler！"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_SYSTEMID_UNKNOWN,
			u"不明的 SystemId"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_LOCATION_UNKNOWN,
			u"不明的錯誤位置"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_PREFIX_MUST_RESOLVE,
			u"前置碼必須解析為命名空間: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_CREATEDOCUMENT_NOT_SUPPORTED,
			u"XPathContext 中不支援 createDocument()！"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_CHILD_HAS_NO_OWNER_DOCUMENT,
			u"屬性子項不具有擁有者文件！"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_CHILD_HAS_NO_OWNER_DOCUMENT_ELEMENT,
			u"屬性子項不具有擁有者文件元素！"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_CANT_OUTPUT_TEXT_BEFORE_DOC,
			u"警告: 無法在文件元素之前輸出文字！正在忽略..."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_CANT_HAVE_MORE_THAN_ONE_ROOT,
			u"DOM 的根不能超過一個！"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_ARG_LOCALNAME_NULL,
			u"引數 \'localName\' 為空值"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_ARG_LOCALNAME_INVALID,
			u"QNAME 中的 Localname 應為有效的 NCName"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_ARG_PREFIX_INVALID,
			u"QNAME 中的前置碼應為有效的 NCName"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_NAME_CANT_START_WITH_COLON,
			u"名稱不能以冒號為開頭"_s
		}),
		$$new($ObjectArray, {
			"BAD_CODE"_s,
			u"createMessage 的參數超出範圍"_s
		}),
		$$new($ObjectArray, {
			"FORMAT_FAILED"_s,
			u"messageFormat 呼叫期間發生異常狀況"_s
		}),
		$$new($ObjectArray, {
			"line"_s,
			u"行號"_s
		...
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_SERIALIZER_NOT_CONTENTHANDLER,
			u"serializer 類別 \'\'{0}\'\' 不實行 org.xml.sax.ContentHandler。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_RESOURCE_COULD_NOT_FIND,
			u"找不到資源 [ {0} ]。\n{1}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_RESOURCE_COULD_NOT_LOAD,
			u"無法載入資源 [ {0} ]: {1} \n {2} \t {3}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_BUFFER_SIZE_LESSTHAN_ZERO,
			u"緩衝區大小 <=0"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_INVALID_UTF16_SURROGATE,
			u"偵測到無效的 UTF-16 代理: {0}？"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_OIERROR,
			u"IO 錯誤"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_ILLEGAL_ATTRIBUTE_POSITION,
			u"在產生子項節點之後，或在產生元素之前，不可新增屬性 {0}。屬性會被忽略。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_NAMESPACE_PREFIX,
			u"字首 \'\'{0}\'\' 的命名空間尚未宣告。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_STRAY_ATTRIBUTE,
			u"屬性 \'\'{0}\'\' 在元素之外。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_STRAY_NAMESPACE,
			u"命名空間宣告 \'\'{0}\'\'=\'\'{1}\'\' 超出元素外。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_COULD_NOT_LOAD_RESOURCE,
			u"無法載入 \'\'{0}\'\' (檢查 CLASSPATH)，目前只使用預設值"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_ILLEGAL_CHARACTER,
			u"嘗試輸出整數值 {0} 的字元，但是它不是以指定的 {1} 輸出編碼呈現。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_TW::ER_COULD_NOT_LOAD_METHOD_PROPERTY,
			u"無法載入輸出方法 \'\'{1}\'\' 的屬性檔 \'\'{0}\'\' (檢查 CLASSPATH)"_s
		})
	}));
}
...
$Class* XMLErrorResources_zh_TW::load$($String* name, bool initialize) {
	...
	$loadClass(XMLErrorResources_zh_TW, name, initialize, &classInfo$$, XMLErrorResources_zh_TW::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLErrorResources_zh_TW);
	});
	return class$;
}
