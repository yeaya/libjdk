...
$XObject* FuncSystemProperty::execute($XPathContext* xctxt) {
	...
	if (indexOfNSSep > 0) {
		...
		if (var$0 || namespace$->equals("http://www.w3.org/1999/XSL/Transform"_s)) {
			$assign(result, xsltInfo->getProperty(propName));
			if (nullptr == result) {
				$init($XPATHErrorResources);
				warn(xctxt, $XPATHErrorResources::WG_PROPERTY_NOT_SUPPORTED, $$new($ObjectArray, {fullName}));
				$init($XString);
			...
			try {
				...
				warn(xctxt, $XPATHErrorResources::WG_SECURITY_EXCEPTION, $$new($ObjectArray, {fullName}));
				$init($XString);
				return $XString::EMPTYSTRING;
			}
		}
	...
	bool var$1 = $nc(propName)->equals("version"_s);
...
void FuncSystemProperty::clinit$($Class* clazz) {
...
$Class* FuncSystemProperty::load$($String* name, bool initialize) {
	...
	$loadClass(FuncSystemProperty, name, initialize, &classInfo$$, FuncSystemProperty::clinit$, []($Class* clazz) -> $Object* {
