...
int32_t DurationImpl::calcSignum(bool isPositive) {
	bool var$4 = this->years == nullptr || this->years->signum() == 0;
	bool var$3 = var$4 && (this->months == nullptr || this->months->signum() == 0);
	bool var$2 = var$3 && (this->days == nullptr || this->days->signum() == 0);
	bool var$1 = var$2 && (this->hours == nullptr || this->hours->signum() == 0);
	bool var$0 = var$1 && (this->minutes == nullptr || this->minutes->signum() == 0);
	if (var$0 && (this->seconds == nullptr || this->seconds->signum() == 0)) {
		return 0;
	}
	if (isPositive) {
		return 1;
...
void DurationImpl::testNonNegative($BigInteger* n, $DatatypeConstants$Field* f) {
	...
	if (n != nullptr && n->signum() < 0) {
		$throwNew($IllegalArgumentException, $($DatatypeMessageFormatter::formatMessage(nullptr, "NegativeField"_s, $$new($ObjectArray, {$($nc(f)->toString())}))));
	}
}
...
void DurationImpl::init$(bool isPositive, int32_t years, int32_t months, int32_t days, int32_t hours, int32_t minutes, int32_t seconds) {
	...
	$var($BigInteger, var$1, wrap(months));
	$var($BigInteger, var$2, wrap(days));
	$var($BigInteger, var$3, wrap(hours));
	$var($BigInteger, var$4, wrap(minutes));
	DurationImpl::init$(isPositive, var$0, var$1, var$2, var$3, var$4, seconds != $DatatypeConstants::FIELD_UNDEFINED ? $$new($BigDecimal, $($String::valueOf(seconds))) : ($BigDecimal*)nullptr);
}
...
$String* DurationImpl::parsePiece($String* whole, $ints* idx) {
	...
	++(*idx)[0];
	return whole->substring(start, idx->get(0));
}
...
void DurationImpl::organizeParts($String* whole, $StringArray* parts, $ints* partsIndex, int32_t len, $String* tokens) {
	...
	for (int32_t i = len - 1; i >= 0; --i) {
		...
		int32_t nidx = tokens->lastIndexOf($nc(parts->get(i))->charAt($nc(parts->get(i))->length() - 1), idx - 1);
		if (nidx == -1) {
			$throwNew($IllegalArgumentException, whole);
		}
		...
		idx = nidx;
		parts->set(idx, parts->get(i));
		$nc(partsIndex)->set(idx, $nc(partsIndex)->get(i));
	}
	for (--idx; idx >= 0; --idx) {
...
$BigInteger* DurationImpl::parseBigInteger($String* whole, $String* part$renamed, int32_t index) {
	...
	$assign(part, $nc(part)->substring(0, $nc(part)->length() - 1));
...
void DurationImpl::checkMaxValue($Number* field, $DatatypeConstants$Field* fieldType) {
	...
	if (fieldValue != nullptr && fieldValue->compareTo(DurationImpl::MaxIntAsBigInt) == 1) {
		$throwNew($UnsupportedOperationException, $($DatatypeMessageFormatter::formatMessage(nullptr, "TooLarge"_s, $$new($ObjectArray, {
			$$str({$($of(this)->getClass()->getName()), "#compare(Duration duration)"_s, fieldType}),
			$($nc($of(field))->toString())
		}))));
	}
}
...
int32_t DurationImpl::compareDates($Duration* duration1, $Duration* duration2) {
	...
	$var($XMLGregorianCalendar, tempA, $cast($XMLGregorianCalendar, $nc(DurationImpl::TEST_POINTS->get(0))->clone()));
	$var($XMLGregorianCalendar, tempB, $cast($XMLGregorianCalendar, $nc(DurationImpl::TEST_POINTS->get(0))->clone()));
	...
	$assign(tempA, $cast($XMLGregorianCalendar, $nc(DurationImpl::TEST_POINTS->get(1))->clone()));
	$assign(tempB, $cast($XMLGregorianCalendar, $nc(DurationImpl::TEST_POINTS->get(1))->clone()));
	$nc(tempA)->add(duration1);
	$nc(tempB)->add(duration2);
	...
	$assign(tempA, $cast($XMLGregorianCalendar, $nc(DurationImpl::TEST_POINTS->get(2))->clone()));
	$assign(tempB, $cast($XMLGregorianCalendar, $nc(DurationImpl::TEST_POINTS->get(2))->clone()));
	$nc(tempA)->add(duration1);
	$nc(tempB)->add(duration2);
	resultB = tempA->compare(tempB);
	resultA = compareResults(resultA, resultB);
	...
	$assign(tempA, $cast($XMLGregorianCalendar, $nc(DurationImpl::TEST_POINTS->get(3))->clone()));
	$assign(tempB, $cast($XMLGregorianCalendar, $nc(DurationImpl::TEST_POINTS->get(3))->clone()));
	$nc(tempA)->add(duration1);
	$nc(tempB)->add(duration2);
	resultB = tempA->compare(tempB);
	resultA = compareResults(resultA, resultB);
...
int32_t DurationImpl::hashCode() {
	$var($Calendar, cal, $nc(DurationImpl::TEST_POINTS->get(0))->toGregorianCalendar());
	this->addTo(cal);
...
$String* DurationImpl::toString() {
	...
	if (this->hours != nullptr || this->minutes != nullptr || this->seconds != nullptr) {
		...
		if (this->hours != nullptr) {
			buf->append(this->hours)->append(u'H');
		}
		if (this->minutes != nullptr) {
			buf->append(this->minutes)->append(u'M');
		}
...
bool DurationImpl::isSet($DatatypeConstants$Field* field) {
	...
	if (field == nullptr) {
		...
		$throwNew($NullPointerException, $($DatatypeMessageFormatter::formatMessage(nullptr, "FieldCannotBeNull"_s, $$new($ObjectArray, {methodName}))));
	}
	$init($DatatypeConstants);
...
$Duration* DurationImpl::normalizeWith($Calendar* startTimeInstant) {
	...
	return $new(DurationImpl, var$1, nullptr, nullptr, var$2, var$3, var$4, $$cast($BigDecimal, getField($DatatypeConstants::SECONDS)));
}
...
$Duration* DurationImpl::multiply($BigDecimal* factor$renamed) {
	...
	for (int32_t i = 0; i < 5; ++i) {
		$var($BigDecimal, bd, getFieldAsBigDecimal(DurationImpl::FIELDS->get(i)));
		$assign(bd, $($nc(bd)->multiply(factor))->add(carry));
		$init($RoundingMode);
		...
		if (i == 1) {
			...
			$assign(carry, bd->multiply(DurationImpl::FACTORS->get(i)));
		}
	}
	if (this->seconds != nullptr) {
		buf->set(5, $($(this->seconds->multiply(factor))->add(carry)));
	} else {
		buf->set(5, carry);
	}
...
$Duration* DurationImpl::add($Duration* rhs) {
	...
	$var($BigInteger, var$3, $cast($BigInteger, rhs->getField($DatatypeConstants::MONTHS)));
	...
	$var($BigInteger, var$5, $cast($BigInteger, rhs->getField($DatatypeConstants::DAYS)));
	...
	$var($BigInteger, var$7, $cast($BigInteger, rhs->getField($DatatypeConstants::HOURS)));
	...
	$var($BigInteger, var$9, $cast($BigInteger, rhs->getField($DatatypeConstants::MINUTES)));
	...
	$var($BigDecimal, var$11, $cast($BigDecimal, rhs->getField($DatatypeConstants::SECONDS)));
	...
	$var($BigInteger, var$13, toBigInteger(var$14, var$15 && rhs->getField($DatatypeConstants::YEARS) == nullptr));
	$var($BigDecimal, var$17, sanitize(buf->get(1), s));
	bool var$18 = lhs->getField($DatatypeConstants::MONTHS) == nullptr;
	$var($BigInteger, var$16, toBigInteger(var$17, var$18 && rhs->getField($DatatypeConstants::MONTHS) == nullptr));
	$var($BigDecimal, var$20, sanitize(buf->get(2), s));
	bool var$21 = lhs->getField($DatatypeConstants::DAYS) == nullptr;
	$var($BigInteger, var$19, toBigInteger(var$20, var$21 && rhs->getField($DatatypeConstants::DAYS) == nullptr));
	$var($BigDecimal, var$23, sanitize(buf->get(3), s));
	bool var$24 = lhs->getField($DatatypeConstants::HOURS) == nullptr;
	$var($BigInteger, var$22, toBigInteger(var$23, var$24 && rhs->getField($DatatypeConstants::HOURS) == nullptr));
	$var($BigDecimal, var$26, sanitize(buf->get(4), s));
	bool var$27 = lhs->getField($DatatypeConstants::MINUTES) == nullptr;
	$var($BigInteger, var$25, toBigInteger(var$26, var$27 && rhs->getField($DatatypeConstants::MINUTES) == nullptr));
	bool var$29 = $nc(buf->get(5))->signum() == 0;
	bool var$28 = var$29 && lhs->getField($DatatypeConstants::SECONDS) == nullptr;
	return $new(DurationImpl, var$12, var$13, var$16, var$19, var$22, var$25, (var$28 && rhs->getField($DatatypeConstants::SECONDS) == nullptr) ? ($BigDecimal*)nullptr : $(sanitize(buf->get(5), s)));
}
	...
	do {
		...
		for (int32_t i = start; i < end; ++i) {
			if (s * $nc($nc(buf)->get(i))->signum() < 0) {
				touched = true;
				$init($RoundingMode);
				$var($BigDecimal, borrow, $($nc(buf->get(i))->abs())->divide(DurationImpl::FACTORS->get(i - 1), 0, $RoundingMode::UP));
				if ($nc(buf->get(i))->signum() > 0) {
					$assign(borrow, borrow->negate());
				}
				buf->set(i - 1, $($nc(buf->get(i - 1))->subtract(borrow)));
				buf->set(i, $($nc(buf->get(i))->add($(borrow->multiply(DurationImpl::FACTORS->get(i - 1))))));
			}
			if ($nc(buf->get(i))->signum() != 0) {
				s = $nc(buf->get(i))->signum();
			}
...
void DurationImpl::addTo($Calendar* calendar) {
	...
	if (this->seconds != nullptr) {
		$init($RoundingMode);
		$var($BigDecimal, fraction, this->seconds->subtract($(this->seconds->setScale(0, $RoundingMode::DOWN))));
		int32_t millisec = $(fraction->movePointRight(3))->intValue();
...
void DurationImpl::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$init($DatatypeConstants);
...
$Class* DurationImpl::load$($String* name, bool initialize) {
	...
	$loadClass(DurationImpl, name, initialize, &classInfo$$, DurationImpl::clinit$, []($Class* clazz) -> $Object* {
