...
void RegularExpression$Context::resetCommon(int32_t nofclosures) {
	...
	if (this->closureContexts == nullptr || this->closureContexts->length != nofclosures) {
	...
	for (int32_t i = 0; i < nofclosures; ++i) {
		if ($nc(this->closureContexts)->get(i) == nullptr) {
			this->closureContexts->set(i, $$new($RegularExpression$ClosureContext));
		} else {
			$nc(this->closureContexts->get(i))->reset();
		}
	}
}
...
$Class* RegularExpression$Context::load$($String* name, bool initialize) {
	...
	$loadClass(RegularExpression$Context, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(RegularExpression$Context);
	});
	return class$;
}
