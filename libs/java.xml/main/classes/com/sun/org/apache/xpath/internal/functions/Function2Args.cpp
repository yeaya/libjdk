...
void Function2Args::callArgVisitors($XPathVisitor* visitor) {
	...
	if (nullptr != this->m_arg1) {
		this->m_arg1->callVisitors($$new($Function2Args$Arg1Owner, this), visitor);
	}
}
...
bool Function2Args::deepEquals($Expression* expr) {
	...
	if (nullptr != this->m_arg1) {
		...
		if (!this->m_arg1->deepEquals($cast(Function2Args, expr)->m_arg1)) {
			return false;
		}
...
$Class* Function2Args::load$($String* name, bool initialize) {
	...
	$loadClass(Function2Args, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
