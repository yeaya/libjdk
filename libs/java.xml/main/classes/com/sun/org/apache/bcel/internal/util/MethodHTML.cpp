...
void MethodHTML::init$($String* dir, $String* class_name, $MethodArray* methods, $FieldArray* fields, $ConstantHTML* constant_html, $AttributeHTML* attribute_html) {
	...
	$set(this, file, $new($PrintWriter, $$new($FileOutputStream, $$str({dir, class_name, "_methods.html"_s}))));
	this->file->println("<HTML><BODY BGCOLOR=\"#C0C0C0\"><TABLE BORDER=0>"_s);
	this->file->println("<TR><TH ALIGN=LEFT>Access&nbsp;flags</TH><TH ALIGN=LEFT>Type</TH><TH ALIGN=LEFT>Field&nbsp;name</TH></TR>"_s);
	{
	...
	this->file->println("<TABLE BORDER=0><TR><TH ALIGN=LEFT>Access&nbsp;flags</TH><TH ALIGN=LEFT>Return&nbsp;type</TH><TH ALIGN=LEFT>Method&nbsp;name</TH><TH ALIGN=LEFT>Arguments</TH></TR>"_s);
...
void MethodHTML::writeField($Field* field) {
	...
	for (int32_t i = 0; i < attributes->length; ++i) {
		if ($nc(attributes->get(i))->getTag() == $Const::ATTR_CONSTANT_VALUE) {
			...
			this->file->print($$str({"<TD>= <A HREF=\""_s, this->className, "_attributes.html#"_s, name, "@"_s, $$str(i), "\" TARGET=\"Attributes\">"_s, str, "</TD>\n"_s}));
			break;
		}
	}
	this->file->println("</TR>"_s);
}
...
void MethodHTML::writeMethod($Method* method, int32_t method_number) {
	...
	this->file->print($$str({"<TR VALIGN=TOP><TD><FONT COLOR=\"#FF0000\"><A NAME=method"_s, $$str(method_number), ">"_s, access, "</A></FONT></TD>"_s}));
	this->file->print($$str({"<TD>"_s, $($Class2HTML::referenceType(type)), "</TD><TD><A HREF="_s, this->className, "_code.html#method"_s, $$str(method_number), " TARGET=Code>"_s, html_name, "</A></TD>\n<TD>("_s}));
	...
	for (int32_t i = 0; i < $nc(attributes)->length; ++i) {
		...
		if (tag == $Const::ATTR_EXCEPTIONS) {
			this->file->print("<TR VALIGN=TOP><TD COLSPAN=2></TD><TH ALIGN=LEFT>throws</TH><TD>"_s);
			...
			for (int32_t j = 0; j < $nc(exceptions)->length; ++j) {
				this->file->print($($nc(this->constantHtml)->referenceConstant(exceptions->get(j))));
			...
			for (int32_t j = 0; j < $nc(c_a)->length; ++j) {
				this->attribute_html->writeAttribute(c_a->get(j), $$str({"method"_s, $$str(method_number), "@"_s, $$str(i), "@"_s, $$str(j)}), method_number);
			}
		}
	}
}
...
$Class* MethodHTML::load$($String* name, bool initialize) {
	...
	$loadClass(MethodHTML, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MethodHTML);
	});
	return class$;
}
