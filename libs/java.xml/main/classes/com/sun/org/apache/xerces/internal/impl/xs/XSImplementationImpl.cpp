...
bool XSImplementationImpl::hasFeature($String* feature, $String* version) {
	...
	bool var$0 = var$1 && (version == nullptr || version->equals("1.0"_s));
...
$XSLoader* XSImplementationImpl::createXSLoader($StringList* versions) {
	...
	for (int32_t i = 0; i < $nc(versions)->getLength(); ++i) {
		if (!$$nc(versions->item(i))->equals("1.0"_s)) {
			...
			$var($String, msg, $DOMMessageFormatter::formatMessage($DOMMessageFormatter::DOM_DOMAIN, "FEATURE_NOT_SUPPORTED"_s, $$new($ObjectArray, {$(versions->item(i))})));
...
void XSImplementationImpl::clinit$($Class* clazz) {
...
$Class* XSImplementationImpl::load$($String* name, bool initialize) {
	...
	$loadClass(XSImplementationImpl, name, initialize, &classInfo$$, XSImplementationImpl::clinit$, []($Class* clazz) -> $Object* {
