...
void XMLVersionDetector::reset($XMLComponentManager* componentManager) {
	...
	for (int32_t i = 14; i < this->fExpectedVersionString->length; ++i) {
		this->fExpectedVersionString->set(i, u' ');
	}
}
...
int16_t XMLVersionDetector::determineDocVersion($XMLInputSource* inputSource) {
	...
	try {
		...
		this->fExpectedVersionString->set(14, (char16_t)quoteChar);
		for (int32_t versionPos = 0; versionPos < XMLVersionDetector::XML11_VERSION->length; ++versionPos) {
			this->fExpectedVersionString->set(15 + versionPos, (char16_t)scanner->scanChar(nullptr));
		}
		this->fExpectedVersionString->set(18, (char16_t)scanner->scanChar(nullptr));
		fixupCurrentEntity(this->fEntityManager, this->fExpectedVersionString, 19);
		int32_t matched = 0;
		for (; matched < XMLVersionDetector::XML11_VERSION->length; ++matched) {
			if (this->fExpectedVersionString->get(15 + matched) != XMLVersionDetector::XML11_VERSION->get(matched)) {
				break;
			}
		}
		scanner->detectingVersion = false;
		...
		$nc(this->fErrorReporter)->reportError(var$0, var$1, $(e->getArguments()), $XMLErrorReporter::SEVERITY_FATAL_ERROR, e);
		scanner->detectingVersion = false;
		return $Constants::XML_VERSION_1_0;
...
void XMLVersionDetector::fixupCurrentEntity($XMLEntityManager* manager, $chars* scannedChars, int32_t length) {
	...
	if ($nc(currentEntity)->count - $nc(currentEntity)->position + length > $nc($nc(currentEntity)->ch)->length) {
...
void XMLVersionDetector::clinit$($Class* clazz) {
	$init($Constants);
...
$Class* XMLVersionDetector::load$($String* name, bool initialize) {
	...
	$loadClass(XMLVersionDetector, name, initialize, &classInfo$$, XMLVersionDetector::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLVersionDetector);
	});
	return class$;
}
