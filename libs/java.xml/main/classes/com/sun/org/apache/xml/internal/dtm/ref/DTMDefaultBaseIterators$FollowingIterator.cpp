...
$Class* DTMDefaultBaseIterators$FollowingIterator::load$($String* name, bool initialize) {
	...
	$loadClass(DTMDefaultBaseIterators$FollowingIterator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DTMDefaultBaseIterators$FollowingIterator);
	});
	return class$;
}
