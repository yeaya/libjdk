...
$String* SimpleResultTreeImpl::getDocumentURI(int32_t node) {
	...
	var$0->append("simple_rtf"_s);
	var$0->append(SimpleResultTreeImpl::_documentURIIndex++);
	return $str(var$0);
}
...
void SimpleResultTreeImpl::characters($String* str) {
	if (this->_size >= $nc(this->_textArray)->length) {
		$var($StringArray, newTextArray, $new($StringArray, this->_textArray->length * 2));
...
void SimpleResultTreeImpl::clinit$($Class* clazz) {
...
$Class* SimpleResultTreeImpl::load$($String* name, bool initialize) {
	...
	$loadClass(SimpleResultTreeImpl, name, initialize, &classInfo$$, SimpleResultTreeImpl::clinit$, []($Class* clazz) -> $Object* {
