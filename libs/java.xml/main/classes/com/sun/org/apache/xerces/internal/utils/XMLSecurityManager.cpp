...
void XMLSecurityManager::setLimit(int32_t index, $JdkProperty$State* state, Object$* value) {
	if (index == XMLSecurityManager::indexEntityCountInfo) {
		...
		if ($instanceOf($Integer, value)) {
			temp = $cast($Integer, value)->intValue();
		} else {
...
void XMLSecurityManager::setLimit(int32_t index, $JdkProperty$State* state, int32_t value) {
	...
	} else if ($nc(state)->compareTo($nc(this->states)->get(index)) >= 0) {
...
bool XMLSecurityManager::isOverLimit(int32_t index, $XMLLimitAnalyzer* limitAnalyzer) {
	...
	if (var$0 || index == $XMLSecurityManager$Limit::MAX_NAME_LIMIT->ordinal()) {
		return ($nc(limitAnalyzer)->getTotalValue(index) > this->values->get(index));
	} else {
		return ($nc(limitAnalyzer)->getValue(index) > this->values->get(index));
	}
}
	...
	for (; i$ < len$; ++i$) {
		...
		if (!getSystemProperty(limit, $($nc(limit)->systemProperty()))) {
			...
			for (int32_t len$ = arr$->length, i$ = 0; i$ < len$; ++i$) {
				...
				{
					$var($String, oldName, $nc(nameMap)->getOldName($(limit->systemProperty())));
					if (oldName != nullptr) {
...
XMLSecurityManager* XMLSecurityManager::convert(Object$* value, XMLSecurityManager* securityManager$renamed) {
	...
	if ($instanceOf(XMLSecurityManager, value)) {
		...
		if ($instanceOf($SecurityManager, value)) {
			...
			$init($XMLSecurityManager$Limit);
			$init($JdkProperty$State);
			$nc(securityManager)->setLimit($XMLSecurityManager$Limit::MAX_OCCUR_NODE_LIMIT, $JdkProperty$State::APIPROPERTY, origSM->getMaxOccurNodeLimit());
			securityManager->setLimit($XMLSecurityManager$Limit::ENTITY_EXPANSION_LIMIT, $JdkProperty$State::APIPROPERTY, origSM->getEntityExpansionLimit());
			securityManager->setLimit($XMLSecurityManager$Limit::ELEMENT_ATTRIBUTE_LIMIT, $JdkProperty$State::APIPROPERTY, origSM->getElementAttrLimit());
		}
		return securityManager;
	}
}

void XMLSecurityManager::clinit$($Class* clazz) {
...
$Class* XMLSecurityManager::load$($String* name, bool initialize) {
	...
	$loadClass(XMLSecurityManager, name, initialize, &classInfo$$, XMLSecurityManager::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLSecurityManager);
	});
	return class$;
}
