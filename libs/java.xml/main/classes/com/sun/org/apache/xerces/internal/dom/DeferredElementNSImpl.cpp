...
void DeferredElementNSImpl::synchronizeData() {
	...
	if (index < 0) {
		...
		$set(this, localName, this->name->substring(index + 1));
	}
	...
	if (attrIndex != -1) {
		...
		do {
			...
			if (var$0) {
				...
				if (!var$1) {
					bool var$3 = attr->getNamespaceURI() != nullptr;
					$init($NamespaceContext);
					bool var$2 = var$3 && attr->getNamespaceURI() != $NamespaceContext::XMLNS_URI;
					var$1 = var$2 && $$nc(attr->getName())->indexOf(u':') < 0;
				}
				var$0 = var$1;
			}
			if (var$0) {
				seenSchemaDefault = true;
				$nc(attrs)->setNamedItemNS($cast($NodeImpl, attr));
			} else {
				$nc(attrs)->setNamedItem($cast($NodeImpl, attr));
			}
...
$Class* DeferredElementNSImpl::load$($String* name, bool initialize) {
	...
	$loadClass(DeferredElementNSImpl, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
