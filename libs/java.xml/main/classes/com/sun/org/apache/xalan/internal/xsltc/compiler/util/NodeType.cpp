...
void NodeType::translateTo($ClassGenerator* classGen, $MethodGenerator* methodGen, $StringType* type) {
	...
	$var($InstructionList, il, $nc(methodGen)->getInstructionList());
	{
		...
		case $NodeTest::PI:
			...
			$init($Constants);
			il->append($Constants::SWAP);
			index = $nc(cpg)->addInterfaceMethodref($Constants::DOM_INTF, $Constants::GET_NODE_VALUE, $Constants::GET_NODE_VALUE_SIG);
			il->append($$new($INVOKEINTERFACE, index, 2));
			break;
		default:
			{
				...
				$var($String, var$0, $ErrorMsg::DATA_CONVERSION_ERR);
				$var($Object, var$1, toString());
				$assign(err, $new($ErrorMsg, var$0, var$1, $($nc(type)->toString())));
...
void NodeType::translateTo($ClassGenerator* classGen, $MethodGenerator* methodGen, $NodeSetType* type) {
	...
	$var($InstructionList, il, $nc(methodGen)->getInstructionList());
	$init($Constants);
	$nc(il)->append($$new($NEW, $nc(cpg)->addClass($Constants::SINGLETON_ITERATOR)));
	il->append($Constants::DUP_X1);
	il->append($Constants::SWAP);
	int32_t init = cpg->addMethodref($Constants::SINGLETON_ITERATOR, "<init>"_s, $$str({"("_s, $Constants::NODE_SIG, ")V"_s}));
...
void NodeType::translateTo($ClassGenerator* classGen, $MethodGenerator* methodGen, $ReferenceType* type) {
	...
	$var($InstructionList, il, $nc(methodGen)->getInstructionList());
	$init($Constants);
	$nc(il)->append($$new($NEW, $nc(cpg)->addClass($Constants::RUNTIME_NODE_CLASS)));
	il->append($Constants::DUP_X1);
	il->append($Constants::SWAP);
	il->append($$new($PUSH, cpg, this->_type));
	il->append($$new($INVOKESPECIAL, cpg->addMethodref($Constants::RUNTIME_NODE_CLASS, "<init>"_s, "(II)V"_s)));
}
...
void NodeType::translateTo($ClassGenerator* classGen, $MethodGenerator* methodGen, $Class* clazz) {
	...
	$init($Constants);
	il->append($Constants::SWAP);
	bool var$0 = className->equals("org.w3c.dom.Node"_s);
...
void NodeType::translateUnBox($ClassGenerator* classGen, $MethodGenerator* methodGen) {
	...
	$var($InstructionList, il, $nc(methodGen)->getInstructionList());
	$init($Constants);
	$nc(il)->append($$new($CHECKCAST, $nc(cpg)->addClass($Constants::RUNTIME_NODE_CLASS)));
	il->append($$new($GETFIELD, cpg->addFieldref($Constants::RUNTIME_NODE_CLASS, $Constants::NODE_FIELD, $Constants::NODE_FIELD_SIG)));
}
...
$Class* NodeType::load$($String* name, bool initialize) {
	...
	$loadClass(NodeType, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(NodeType);
	});
	return class$;
}
