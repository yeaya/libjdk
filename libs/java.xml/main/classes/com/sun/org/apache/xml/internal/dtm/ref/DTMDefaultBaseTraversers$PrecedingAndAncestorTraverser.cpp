...
$Class* DTMDefaultBaseTraversers$PrecedingAndAncestorTraverser::load$($String* name, bool initialize) {
	...
	$loadClass(DTMDefaultBaseTraversers$PrecedingAndAncestorTraverser, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DTMDefaultBaseTraversers$PrecedingAndAncestorTraverser);
	});
	return class$;
}
