...
$String* ListDV$ListData::toString() {
	$synchronized(this) {
		...
		if (this->canonical == nullptr) {
			...
			if (len > 0) {
				buf->append($($nc(this->data->get(0))->toString()));
			}
			for (int32_t i = 1; i < len; ++i) {
				buf->append(u' ');
				buf->append($($nc(this->data->get(i))->toString()));
			}
...
bool ListDV$ListData::equals(Object$* obj) {
	...
	for (int32_t i = 0; i < count; ++i) {
		if (!$nc(this->data->get(i))->equals(odata->get(i))) {
			return false;
		}
	}
	return true;
...
int32_t ListDV$ListData::hashCode() {
	...
	for (int32_t i = 0; i < $nc(this->data)->length; ++i) {
		hash ^= $nc(this->data->get(i))->hashCode();
	}
	return hash;
}
...
bool ListDV$ListData::contains(Object$* item) {
	for (int32_t i = 0; i < $nc(this->data)->length; ++i) {
		if ($equals(item, this->data->get(i))) {
			return true;
		}
	}
	return false;
...
$Class* ListDV$ListData::load$($String* name, bool initialize) {
	...
	$loadClass(ListDV$ListData, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
