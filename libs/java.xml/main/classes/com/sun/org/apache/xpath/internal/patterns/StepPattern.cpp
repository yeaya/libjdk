...
void StepPattern::fixupVariables($List* vars, int32_t globalsSize) {
	...
	if (nullptr != this->m_predicates) {
		for (int32_t i = 0; i < $nc(this->m_predicates)->length; ++i) {
			$nc(this->m_predicates->get(i))->fixupVariables(vars, globalsSize);
		}
	}
...
int32_t StepPattern::getPredicateCount() {
	return (nullptr == this->m_predicates) ? 0 : this->m_predicates->length;
}
	...
	$var($StringBuffer, buf, $new($StringBuffer));
	{
		...
		for (; pat != nullptr; $assign(pat, pat->m_relativePathPattern)) {
			...
			if (nullptr != pat->m_predicates) {
				for (int32_t i = 0; i < pat->m_predicates->length; ++i) {
					buf->append("["_s);
					buf->append(pat->m_predicates->get(i));
					buf->append("]"_s);
				}
			}
		}
...
void StepPattern::callSubtreeVisitors($XPathVisitor* visitor) {
	$useLocalObjectStack();
	if (nullptr != this->m_predicates) {
		int32_t n = this->m_predicates->length;
		for (int32_t i = 0; i < n; ++i) {
...
bool StepPattern::deepEquals($Expression* expr) {
	...
	if (nullptr != this->m_predicates) {
		int32_t n = this->m_predicates->length;
		if ((nullptr == $nc(sp)->m_predicates) || (sp->m_predicates->length != n)) {
			return false;
		}
		for (int32_t i = 0; i < n; ++i) {
			if (!$nc($nc(this->m_predicates)->get(i))->deepEquals($nc(sp->m_predicates)->get(i))) {
				return false;
			}
		}
	...
	if (nullptr != this->m_relativePathPattern) {
		if (!this->m_relativePathPattern->deepEquals($nc(sp)->m_relativePathPattern)) {
			return false;
		}
