...
bool Type::equals(Object$* o) {
	if ($instanceOf(Type, o)) {
		...
		return (this->type == t->type) && $nc(this->signature)->equals(t->signature);
	}
	return false;
}
...
Type* Type::getType($String* signature) {
	...
	if (type <= $Const::T_VOID) {
		...
		$var(Type, t, getType($($nc(signature)->substring(dim))));
		...
		return $ObjectType::getInstance($(parsedSignature->replace(u'/', u'.')));
	}
}
...
Type* Type::getReturnType($String* signature) {
	...
	$useLocalObjectStack();
	try {
		int32_t index = $nc(signature)->lastIndexOf(u')') + 1;
...
int32_t Type::getTypeSize($String* signature) {
	...
	if (type <= $Const::T_VOID) {
		...
		int32_t consumed = Type::consumed(getTypeSize($($nc(signature)->substring(dim))));
