...
$Type* KeyCall::typeCheck($SymbolTable* stable) {
	...
	if (this->_name != nullptr) {
		$var($Type, nameType, this->_name->typeCheck(stable));
	...
	if (this->_valueType != $Type::NodeSet && this->_valueType != $Type::Reference && this->_valueType != $Type::String) {
		...
		$set(this, _valueType, this->_value->typeCheck(stable));
	}
	addParentDependency();
	return returnType;
}
...
$Class* KeyCall::load$($String* name, bool initialize) {
	...
	$loadClass(KeyCall, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(KeyCall);
	});
	return class$;
}
