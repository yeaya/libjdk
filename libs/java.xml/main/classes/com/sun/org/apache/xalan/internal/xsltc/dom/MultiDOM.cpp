...
int32_t MultiDOM::addDOMAdapter($DOMAdapter* adapter, bool indexByURI) {
	...
	if ($instanceOf($DTMDefaultBase, dom)) {
		...
		domNo = $usr(simpleRTF->getDocument(), $DTMManager::IDENT_DTM_NODE_BITS);
	}
	if (domNo >= this->_size) {
		...
		$set(this, _adapters, $cast($DOMArray, newArray));
	}
	this->_free = domNo + 1;
	if (dtmSize == 1) {
	...
	if ($instanceOf($AdaptiveResultTreeImpl, dom)) {
		...
		$var($DOM, nestedDom, adaptiveRTF->getNestedDOM());
		if (nestedDom != nullptr) {
			$var($StringArray, var$0, adapter->getNamesArray());
			$var($StringArray, var$1, adapter->getUrisArray());
			$var($ints, var$2, adapter->getTypesArray());
			$var($DOMAdapter, newAdapter, $new($DOMAdapter, nestedDom, var$0, var$1, var$2, $(adapter->getNamespaceArray())));
...
$DOM* MultiDOM::getDOMAdapter($String* uri) {
	...
	if (domIdx == nullptr) {
		...
		return ($nc(this->_adapters)->get(domIdx->intValue()));
	}
}
...
void MultiDOM::setFilter($StripFilter* filter) {
	for (int32_t dom = 0; dom < this->_free; ++dom) {
		if ($nc(this->_adapters)->get(dom) != nullptr) {
			$nc(this->_adapters->get(dom))->setFilter(filter);
		}
	}
}
...
bool MultiDOM::isMatchingAdapterEntry($DOM* entry, $DOMAdapter* adapter) {
	...
	if (!var$0) {
		...
		if (var$1) {
			var$1 = $cast($AdaptiveResultTreeImpl, dom)->getNestedDOM() == $cast($DOMAdapter, entry)->getDOMImpl();
		}
		var$0 = var$1;
	}
	return var$0;
...
void MultiDOM::removeDOMAdapter($DOMAdapter* adapter) {
	...
	$var($DOM, dom, adapter->getDOMImpl());
	if ($instanceOf($DTMDefaultBase, dom)) {
		$var($SuballocatedIntVector, ids, $cast($DTMDefaultBase, dom)->getDTMIDs());
		...
		if ((id > 0) && (id < $nc(this->_adapters)->length) && isMatchingAdapterEntry(this->_adapters->get(id), adapter)) {
			$nc(this->_adapters)->set(id, nullptr);
		} else {
			bool found = false;
			for (int32_t i = 0; i < $nc(this->_adapters)->length; ++i) {
				if (isMatchingAdapterEntry(this->_adapters->get(id), adapter)) {
...
$Class* MultiDOM::load$($String* name, bool initialize) {
	...
	$loadClass(MultiDOM, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MultiDOM);
	});
	return class$;
}
