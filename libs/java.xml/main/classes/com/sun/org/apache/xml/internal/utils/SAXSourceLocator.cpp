...
$String* SAXSourceLocator::getPublicId() {
	return (nullptr == this->m_locator) ? $LocatorImpl::getPublicId() : this->m_locator->getPublicId();
}

$String* SAXSourceLocator::getSystemId() {
	return (nullptr == this->m_locator) ? $LocatorImpl::getSystemId() : this->m_locator->getSystemId();
}

int32_t SAXSourceLocator::getLineNumber() {
	return (nullptr == this->m_locator) ? $LocatorImpl::getLineNumber() : this->m_locator->getLineNumber();
}

int32_t SAXSourceLocator::getColumnNumber() {
	return (nullptr == this->m_locator) ? $LocatorImpl::getColumnNumber() : this->m_locator->getColumnNumber();
}
...
$Class* SAXSourceLocator::load$($String* name, bool initialize) {
	...
	$loadClass(SAXSourceLocator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
