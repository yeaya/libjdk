...
void XMLErrorResources_zh_CN::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(XMLErrorResources_zh_CN::contents, $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_FUNCTION_NOT_SUPPORTED,
			u"不支持该函数!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_CANNOT_OVERWRITE_CAUSE,
			u"无法覆盖原因"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_NO_DEFAULT_IMPL,
			u"找不到默认实现 "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_CHUNKEDINTARRAY_NOT_SUPPORTED,
			u"当前不支持 ChunkedIntArray({0})"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_OFFSET_BIGGER_THAN_SLOT,
			u"偏移量大于插槽"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_COROUTINE_NOT_AVAIL,
			u"Coroutine 不可用, id={0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_COROUTINE_CO_EXIT,
			u"CoroutineManager 收到 co_exit() 请求"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_COJOINROUTINESET_FAILED,
			u"co_joinCoroutineSet() 失败"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_COROUTINE_PARAM,
			u"Coroutine 参数错误 ({0})"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_PARSER_DOTERMINATE_ANSWERS,
			u"\n意外: 解析器对答复{0}执行 doTerminate"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_NO_PARSE_CALL_WHILE_PARSING,
			u"无法在解析时调用 parse"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_TYPED_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"错误: 未实现轴{0}的类型化迭代器"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"错误: 未实现轴{0}的迭代器 "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_ITERATOR_CLONE_NOT_SUPPORTED,
			u"不支持克隆迭代器"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_UNKNOWN_AXIS_TYPE,
			u"轴遍历类型未知: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_AXIS_NOT_SUPPORTED,
			u"不支持轴遍历程序: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_NO_DTMIDS_AVAIL,
			u"无法使用更多 DTM ID"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_NOT_SUPPORTED,
			u"不支持: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_NODE_NON_NULL,
			u"getDTMHandleFromNode 的节点必须为非空值"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_COULD_NOT_RESOLVE_NODE,
			u"无法将节点解析为句柄"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_STARTPARSE_WHILE_PARSING,
			u"无法在解析时调用 startParse"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_STARTPARSE_NEEDS_SAXPARSER,
			u"startParse 需要非空 SAXParser"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_COULD_NOT_INIT_PARSER,
			u"无法使用以下对象初始化解析器"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_EXCEPTION_CREATING_POOL,
			u"为池创建新实例时出现异常错误"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_PATH_CONTAINS_INVALID_ESCAPE_SEQUENCE,
			u"路径包含无效的转义序列"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_SCHEME_REQUIRED,
			u"方案是必需的!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_NO_SCHEME_IN_URI,
			u"在 URI 中找不到方案: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_NO_SCHEME_INURI,
			u"在 URI 中找不到方案"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_PATH_INVALID_CHAR,
			u"路径包含无效的字符: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_SCHEME_FROM_NULL_STRING,
			u"无法从空字符串设置方案"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_SCHEME_NOT_CONFORMANT,
			u"方案不一致。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_HOST_ADDRESS_NOT_WELLFORMED,
			u"主机不是格式良好的地址"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_PORT_WHEN_HOST_NULL,
			u"主机为空时, 无法设置端口"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_INVALID_PORT,
			u"无效的端口号"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_FRAG_FOR_GENERIC_URI,
			u"只能为一般 URI 设置片段"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_FRAG_WHEN_PATH_NULL,
			u"路径为空时, 无法设置片段"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_FRAG_INVALID_CHAR,
			u"片段包含无效的字符"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_PARSER_IN_USE,
			u"解析器已在使用"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_CANNOT_CHANGE_WHILE_PARSING,
			u"无法在解析时更改{0} {1}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_SELF_CAUSATION_NOT_PERMITTED,
			u"不允许使用自因"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_NO_USERINFO_IF_NO_HOST,
			u"如果没有指定主机, 则不可以指定 Userinfo"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_NO_PORT_IF_NO_HOST,
			u"如果没有指定主机, 则不可以指定端口"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_NO_QUERY_STRING_IN_PATH,
			u"路径和查询字符串中不能指定查询字符串"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_NO_FRAGMENT_STRING_IN_PATH,
			u"路径和片段中都无法指定片段"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_CANNOT_INIT_URI_EMPTY_PARMS,
			u"无法以空参数初始化 URI"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_METHOD_NOT_SUPPORTED,
			u"尚不支持该方法 "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_INCRSAXSRCFILTER_NOT_RESTARTABLE,
			u"当前无法重新启动 IncrementalSAXSource_Filter"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_XMLRDR_NOT_BEFORE_STARTPARSE,
			u"XMLReader 不在 startParse 请求之前"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_AXIS_TRAVERSER_NOT_SUPPORTED,
			u"不支持轴遍历程序: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_ERRORHANDLER_CREATED_WITH_NULL_PRINTWRITER,
			u"使用空 PrintWriter 创建了 ListingErrorHandler!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_SYSTEMID_UNKNOWN,
			u"SystemId 未知"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_LOCATION_UNKNOWN,
			u"错误所在的位置未知"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_PREFIX_MUST_RESOLVE,
			u"前缀必须解析为名称空间: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_CREATEDOCUMENT_NOT_SUPPORTED,
			u"XPathContext 中不支持 createDocument()!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_CHILD_HAS_NO_OWNER_DOCUMENT,
			u"属性子级没有所有者文档!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_CHILD_HAS_NO_OWNER_DOCUMENT_ELEMENT,
			u"属性子级没有所有者文档元素!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_CANT_OUTPUT_TEXT_BEFORE_DOC,
			u"警告: 无法输出文档元素之前的文本! 将忽略..."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_CANT_HAVE_MORE_THAN_ONE_ROOT,
			u"DOM 上不能有多个根!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_ARG_LOCALNAME_NULL,
			u"参数 \'localName\' 为空值"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_ARG_LOCALNAME_INVALID,
			u"QNAME 中的本地名称应为有效 NCName"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_ARG_PREFIX_INVALID,
			u"QNAME 中的前缀应为有效 NCName"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_NAME_CANT_START_WITH_COLON,
			u"名称不能以冒号开头"_s
		}),
		$$new($ObjectArray, {
			"BAD_CODE"_s,
			u"createMessage 的参数超出范围"_s
		}),
		$$new($ObjectArray, {
			"FORMAT_FAILED"_s,
			u"调用 messageFormat 时抛出异常错误"_s
		}),
		$$new($ObjectArray, {
			"line"_s,
			u"行号"_s
		...
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_SERIALIZER_NOT_CONTENTHANDLER,
			u"串行器类 \'\'{0}\'\' 不实现 org.xml.sax.ContentHandler。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_RESOURCE_COULD_NOT_FIND,
			u"找不到资源 [ {0} ]。\n {1}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_RESOURCE_COULD_NOT_LOAD,
			u"资源 [ {0} ] 无法加载: {1} \n {2} \t {3}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_BUFFER_SIZE_LESSTHAN_ZERO,
			u"缓冲区大小 <=0"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_INVALID_UTF16_SURROGATE,
			u"检测到无效的 UTF-16 代理: {0}?"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_OIERROR,
			u"IO 错误"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_ILLEGAL_ATTRIBUTE_POSITION,
			u"在生成子节点之后或在生成元素之前无法添加属性 {0}。将忽略属性。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_NAMESPACE_PREFIX,
			u"没有说明名称空间前缀 \'\'{0}\'\'。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_STRAY_ATTRIBUTE,
			u"属性 \'\'{0}\'\' 在元素外部。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_STRAY_NAMESPACE,
			u"名称空间声明 \'\'{0}\'\'=\'\'{1}\'\' 在元素外部。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_COULD_NOT_LOAD_RESOURCE,
			u"无法加载 \'\'{0}\'\' (检查 CLASSPATH), 现在只使用默认值"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_ILLEGAL_CHARACTER,
			u"尝试输出未以{1}的指定输出编码表示的整数值 {0} 的字符。"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_zh_CN::ER_COULD_NOT_LOAD_METHOD_PROPERTY,
...
$Class* XMLErrorResources_zh_CN::load$($String* name, bool initialize) {
	...
	$loadClass(XMLErrorResources_zh_CN, name, initialize, &classInfo$$, XMLErrorResources_zh_CN::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLErrorResources_zh_CN);
	});
	return class$;
}
