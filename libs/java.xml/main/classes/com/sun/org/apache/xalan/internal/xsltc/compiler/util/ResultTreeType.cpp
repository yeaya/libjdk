...
void ResultTreeType::translateTo($ClassGenerator* classGen, $MethodGenerator* methodGen, $StringType* type) {
	...
	if (this->_methodName == nullptr) {
		...
		$nc(il)->append($$new($INVOKEINTERFACE, index, 1));
	} else {
		...
		il->append($$new($GETFIELD, $nc(cpg)->addFieldref(className, "_dom"_s, $Constants::DOM_INTF_SIG)));
		int32_t index = cpg->addMethodref($Constants::STRING_VALUE_HANDLER, "<init>"_s, "()V"_s);
		...
		$nc(handler)->setStart($(il->append($$new($ASTORE, $nc(handler)->getIndex()))));
...
void ResultTreeType::translateTo($ClassGenerator* classGen, $MethodGenerator* methodGen, $ReferenceType* type) {
	...
	if (this->_methodName == nullptr) {
		...
		$nc(newDom)->setStart($(il->append($$new($ASTORE, $nc(newDom)->getIndex()))));
		...
		$nc(domBuilder)->setStart($(il->append($$new($ASTORE, $nc(domBuilder)->getIndex()))));
...
void ResultTreeType::translateTo($ClassGenerator* classGen, $MethodGenerator* methodGen, $NodeSetType* type) {
	...
	il->append($$new($GETFIELD, $nc(cpg)->addFieldref($Constants::TRANSLET_CLASS, $Constants::NAMES_INDEX, $Constants::NAMES_INDEX_SIG)));
	il->append($(classGen->loadTranslet()));
	...
	int32_t mapping = cpg->addInterfaceMethodref($Constants::DOM_INTF, "setupMapping"_s, $$str({"(["_s, $Constants::STRING_SIG, "["_s, $Constants::STRING_SIG, "[I["_s, $Constants::STRING_SIG, ")V"_s}));
...
void ResultTreeType::translateTo($ClassGenerator* classGen, $MethodGenerator* methodGen, $Class* clazz) {
	...
	if ($nc(className)->equals("org.w3c.dom.Node"_s)) {
		...
		$nc(il)->append($$new($INVOKEINTERFACE, index, 2));
...
$Class* ResultTreeType::load$($String* name, bool initialize) {
	...
	$loadClass(ResultTreeType, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ResultTreeType);
	});
	return class$;
}
