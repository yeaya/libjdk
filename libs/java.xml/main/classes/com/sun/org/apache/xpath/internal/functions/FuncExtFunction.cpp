...
void FuncExtFunction::fixupVariables($List* vars, int32_t globalsSize) {
	...
	if (nullptr != this->m_argVec) {
		int32_t nArgs = this->m_argVec->size();
...
$Expression* FuncExtFunction::getArg(int32_t n) {
	if (n >= 0 && n < $nc(this->m_argVec)->size()) {
		return $cast($Expression, this->m_argVec->get(n));
	} else {
		return nullptr;
	}
}
...
$XObject* FuncExtFunction::execute($XPathContext* xctxt) {
	...
	if ($nc(xctxt)->isSecureProcessing()) {
		...
		$throwNew($TransformerException, $($XPATHMessages::createXPATHMessage($XPATHErrorResources::ER_EXTENSION_FUNCTION_CANNOT_BE_INVOKED, $$new($ObjectArray, {$(toString())}))));
	}
	...
	$var($ExtensionsProvider, extProvider, $cast($ExtensionsProvider, xctxt->getOwnerObject()));
...
void FuncExtFunction::callArgVisitors($XPathVisitor* visitor) {
	...
	for (int32_t i = 0; i < $nc(this->m_argVec)->size(); ++i) {
		$var($Expression, exp, $cast($Expression, this->m_argVec->get(i)));
...
$String* FuncExtFunction::toString() {
	if (this->m_namespace != nullptr && this->m_namespace->length() > 0) {
...
$Class* FuncExtFunction::load$($String* name, bool initialize) {
	...
	$loadClass(FuncExtFunction, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
