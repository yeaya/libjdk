...
void XMLDTDValidator::init$() {
	...
	for (int32_t i = 0; i < this->fElementQNamePartsStack->length; ++i) {
		this->fElementQNamePartsStack->set(i, $$new($QName));
	}
...
$StringArray* XMLDTDValidator::getRecognizedFeatures() {
	return $cast($StringArray, XMLDTDValidator::RECOGNIZED_FEATURES->clone());
}
...
$StringArray* XMLDTDValidator::getRecognizedProperties() {
	return $cast($StringArray, XMLDTDValidator::RECOGNIZED_PROPERTIES->clone());
}
...
$Boolean* XMLDTDValidator::getFeatureDefault($String* featureId) {
	for (int32_t i = 0; i < XMLDTDValidator::RECOGNIZED_FEATURES->length; ++i) {
		if ($nc(XMLDTDValidator::RECOGNIZED_FEATURES->get(i))->equals(featureId)) {
			return XMLDTDValidator::FEATURE_DEFAULTS->get(i);
		}
	}
	return nullptr;
}
...
$Object* XMLDTDValidator::getPropertyDefault($String* propertyId) {
	for (int32_t i = 0; i < XMLDTDValidator::RECOGNIZED_PROPERTIES->length; ++i) {
		if ($nc(XMLDTDValidator::RECOGNIZED_PROPERTIES->get(i))->equals(propertyId)) {
			return XMLDTDValidator::PROPERTY_DEFAULTS->get(i);
		}
	}
	return nullptr;
}
...
void XMLDTDValidator::startDocument($XMLLocator* locator, $String* encoding, $NamespaceContext* namespaceContext, $Augmentations* augs) {
	if (this->fGrammarPool != nullptr) {
		...
		$var($GrammarArray, grammars, this->fGrammarPool->retrieveInitialGrammarSet($XMLGrammarDescription::XML_DTD));
		int32_t length = (grammars != nullptr) ? grammars->length : 0;
		for (int32_t i = 0; i < length; ++i) {
			$nc(this->fGrammarBucket)->putGrammar($cast($DTDGrammar, $nc(grammars)->get(i)));
		}
	}
...
void XMLDTDValidator::doctypeDecl($String* rootElement, $String* publicId, $String* systemId, $Augmentations* augs) {
	...
	this->fRootElement->setValues(nullptr, rootElement, rootElement, nullptr);
	...
	if (this->fDTDGrammar == nullptr) {
		if (this->fGrammarPool != nullptr && (systemId != nullptr || publicId != nullptr)) {
			$set(this, fDTDGrammar, $cast($DTDGrammar, this->fGrammarPool->retrieveGrammar(grammarDesc)));
		}
	}
	if (this->fDTDGrammar == nullptr) {
		...
		if (this->fInElementContent) {
			...
			if (augs != nullptr && $equals(augs->getItem($Constants::CHAR_REF_PROBABLE_WS), $Boolean::TRUE)) {
				...
				$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "MSG_CONTENT_INVALID_SPECIFIED"_s, $$new($ObjectArray, {
					...
					$($nc(this->fDTDGrammar)->getContentSpecAsString(this->fElementDepth)),
					"character reference"_s
...
void XMLDTDValidator::comment($XMLString* text, $Augmentations* augs) {
	if (this->fPerformValidation && this->fElementDepth >= 0 && this->fDTDGrammar != nullptr) {
		...
		if (this->fTempElementDecl->type == $XMLElementDecl::TYPE_EMPTY) {
			$init($XMLMessageFormatter);
			$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "MSG_CONTENT_INVALID_SPECIFIED"_s, $$new($ObjectArray, {
				this->fCurrentElement->rawname,
				"EMPTY"_s,
...
void XMLDTDValidator::addDTDDefaultAttrsAndValidate($QName* elementName, int32_t elementIndex, $XMLAttributes* attributes) {
	...
	while (attlistIndex != -1) {
		...
		;
		$var($String, attPrefix, $nc(this->fTempAttDecl->name)->prefix);
		...
		int32_t attDefaultType = $nc(this->fTempAttDecl->simpleType)->defaultType;
	...
	for (int32_t i = 0; i < attrCount; ++i) {
			...
			if ($nc(this->fGrammarBucket)->getStandalone()) {
				...
				if (nonNormalizedValue != nullptr) {
					...
					if (entityName != nullptr) {
						$init($XMLMessageFormatter);
						$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "MSG_REFERENCE_TO_EXTERNALLY_DECLARED_ENTITY_WHEN_STANDALONE"_s, $$new($ObjectArray, {entityName}), $XMLErrorReporter::SEVERITY_ERROR);
					}
				}
			}
		}
		...
		while (position != -1) {
			...
			if ($nc(this->fTempAttDecl->name)->rawname == attrRawName) {
				declared = true;
				break;
			}
		...
		if ($nc(this->fTempAttDecl->simpleType)->defaultType == $XMLSimpleType::DEFAULT_TYPE_FIXED) {
			$var($String, defaultValue, this->fTempAttDecl->simpleType->defaultValue);
		...
		if (this->fTempAttDecl->simpleType->type == $XMLSimpleType::TYPE_ENTITY || this->fTempAttDecl->simpleType->type == $XMLSimpleType::TYPE_ENUMERATION || this->fTempAttDecl->simpleType->type == $XMLSimpleType::TYPE_ID || this->fTempAttDecl->simpleType->type == $XMLSimpleType::TYPE_IDREF || this->fTempAttDecl->simpleType->type == $XMLSimpleType::TYPE_NMTOKEN || this->fTempAttDecl->simpleType->type == $XMLSimpleType::TYPE_NOTATION) {
...
$String* XMLDTDValidator::getExternalEntityRefInAttrValue($String* nonNormalizedValue) {
	...
	int32_t ampIndex = nonNormalizedValue->indexOf(u'&');
	while (ampIndex != -1) {
		if (ampIndex + 1 < valLength && nonNormalizedValue->charAt(ampIndex + 1) != u'#') {
			int32_t semicolonIndex = nonNormalizedValue->indexOf(u';', ampIndex + 1);
			...
			if (entIndex > -1) {
				...
				if (this->fEntityDecl->inExternal || ($assign(entityName, getExternalEntityRefInAttrValue(this->fEntityDecl->value))) != nullptr) {
					return entityName;
				}
			}
		}
		ampIndex = nonNormalizedValue->indexOf(u'&', ampIndex + 1);
	}
	return nullptr;
}
...
void XMLDTDValidator::validateDTDattribute($QName* element, $String* attValue, $XMLAttributeDecl* attributeDecl) {
	...
	case $XMLSimpleType::TYPE_ENTITY:
		{
			bool isAlistAttribute = attributeDecl->simpleType->list;
			try {
				if (isAlistAttribute) {
	...
	case $XMLSimpleType::TYPE_ENUMERATION:
		{
			bool found = false;
			$var($StringArray, enumVals, attributeDecl->simpleType->enumeration);
			if (enumVals == nullptr) {
				found = false;
			} else {
	...
	case $XMLSimpleType::TYPE_IDREF:
		{
			...
			try {
				...
				if (isAlistAttribute) {
					$init($XMLMessageFormatter);
					$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "IDREFSInvalid"_s, $$new($ObjectArray, {attValue}), $XMLErrorReporter::SEVERITY_ERROR);
				} else {
					$init($XMLMessageFormatter);
	...
	case $XMLSimpleType::TYPE_NMTOKEN:
		{
			...
			try {
				...
				if (isAlistAttribute) {
					$init($XMLMessageFormatter);
					$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "NMTOKENSInvalid"_s, $$new($ObjectArray, {attValue}), $XMLErrorReporter::SEVERITY_ERROR);
				} else {
					$init($XMLMessageFormatter);
					$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "NMTOKENInvalid"_s, $$new($ObjectArray, {attValue}), $XMLErrorReporter::SEVERITY_ERROR);
				}
			}
			break;
		}
...
void XMLDTDValidator::rootElementSpecified($QName* rootElement) {
	$useLocalObjectStack();
	if (this->fPerformValidation) {
		$var($String, root1, this->fRootElement->rawname);
		$var($String, root2, $nc(rootElement)->rawname);
		if (root1 == nullptr || !root1->equals(root2)) {
			$init($XMLMessageFormatter);
...
int32_t XMLDTDValidator::checkContent(int32_t elementIndex, $QNameArray* children, int32_t childOffset, int32_t childCount) {
	...
	if (contentType == $XMLElementDecl::TYPE_EMPTY) {
		...
		$assign(cmElem, this->fTempElementDecl->contentModelValidator);
...
void XMLDTDValidator::charDataInContent() {
	...
	if ($nc(this->fElementChildren)->length <= this->fElementChildrenLength) {
		$var($QNameArray, newarray, $new($QNameArray, this->fElementChildren->length * 2));
		$System::arraycopy(this->fElementChildren, 0, newarray, 0, this->fElementChildren->length);
		$set(this, fElementChildren, newarray);
	}
	$var($QName, qname, this->fElementChildren->get(this->fElementChildrenLength));
	if (qname == nullptr) {
		for (int32_t i = this->fElementChildrenLength; i < this->fElementChildren->length; ++i) {
			this->fElementChildren->set(i, $$new($QName));
		}
		$assign(qname, this->fElementChildren->get(this->fElementChildrenLength));
	}
	$nc(qname)->clear();
	...
	case $XMLSimpleType::TYPE_ENUMERATION:
		{
			...
			for (int32_t i = 0; i < $nc(attrDecl->simpleType->enumeration)->length; ++i) {
				totalLength += $nc(attrDecl->simpleType->enumeration->get(i))->length() + 1;
			}
			$var($StringBuilder, buffer, $new($StringBuilder, totalLength));
			buffer->append(u'(');
			for (int32_t i = 0; i < attrDecl->simpleType->enumeration->length; ++i) {
				if (i > 0) {
					buffer->append(u'|');
				}
...
void XMLDTDValidator::ensureStackCapacity(int32_t newElementDepth) {
	...
	if (newElementDepth == $nc(this->fElementQNamePartsStack)->length) {
		...
		$var($QName, qname, this->fElementQNamePartsStack->get(newElementDepth));
		if (qname == nullptr) {
			for (int32_t i = newElementDepth; i < this->fElementQNamePartsStack->length; ++i) {
				this->fElementQNamePartsStack->set(i, $$new($QName));
			}
		}
...
bool XMLDTDValidator::handleStartElement($QName* element, $XMLAttributes* attributes, $Augmentations* augs) {
	...
	if (this->fDTDGrammar == nullptr) {
		...
		if (this->fPerformValidation) {
			$init($XMLMessageFormatter);
			$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "MSG_GRAMMAR_NOT_FOUND"_s, $$new($ObjectArray, {$nc(element)->rawname}), $XMLErrorReporter::SEVERITY_ERROR);
		}
		...
		if (this->fCurrentContentSpecType == -1 && this->fPerformValidation) {
			$init($XMLMessageFormatter);
			$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "MSG_ELEMENT_NOT_DECLARED"_s, $$new($ObjectArray, {$nc(element)->rawname}), $XMLErrorReporter::SEVERITY_ERROR);
		}
	...
	if (this->fPerformValidation) {
		if ($nc(this->fElementChildrenOffsetStack)->length <= this->fElementDepth) {
			$var($ints, newarray, $new($ints, this->fElementChildrenOffsetStack->length * 2));
		...
		this->fElementChildrenOffsetStack->set(this->fElementDepth, this->fElementChildrenLength);
		if ($nc(this->fElementChildren)->length <= this->fElementChildrenLength) {
...
void XMLDTDValidator::handleEndElement($QName* element, $Augmentations* augs, bool isEmpty) {
			...
			if (result != -1) {
				...
				if (this->fTempElementDecl->type == $XMLElementDecl::TYPE_EMPTY) {
					...
					$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, messageKey, $$new($ObjectArray, {
						$nc(element)->rawname,
						$($nc(this->fDTDGrammar)->getContentSpecAsString(elementIndex))
					}), $XMLErrorReporter::SEVERITY_ERROR);
				}
			}
		}
	...
	if (this->fElementDepth < 0) {
		...
		this->fInElementContent = false;
		if (this->fPerformValidation) {
			$var($Iterator, invIdRefs, this->fValidationState->checkIDRefID());
	...
	this->fCurrentElement->setValues($nc(this->fElementQNamePartsStack)->get(this->fElementDepth));
...
void XMLDTDValidator::clinit$($Class* clazz) {
	$init($Constants);
...
$Class* XMLDTDValidator::load$($String* name, bool initialize) {
	...
	$loadClass(XMLDTDValidator, name, initialize, &classInfo$$, XMLDTDValidator::clinit$, []($Class* clazz) -> $Object* {
