...
void ParameterAnnotationEntry::init$($DataInput* input, $ConstantPool* constant_pool) {
	...
	for (int32_t i = 0; i < annotation_table_length; ++i) {
		this->annotationTable->set(i, $($AnnotationEntry::read(input, constant_pool, false)));
	}
}
...
void ParameterAnnotationEntry::dump($DataOutputStream* dos) {
	$useLocalObjectStack();
	$nc(dos)->writeShort(this->annotationTable->length);
	{
...
$ParameterAnnotationEntryArray* ParameterAnnotationEntry::createParameterAnnotationEntries($AttributeArray* attrs) {
	...
	return $cast($ParameterAnnotationEntryArray, accumulatedAnnotations->toArray($$new($ParameterAnnotationEntryArray, accumulatedAnnotations->size())));
}
...
$Class* ParameterAnnotationEntry::load$($String* name, bool initialize) {
	...
	$loadClass(ParameterAnnotationEntry, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ParameterAnnotationEntry);
	});
	return class$;
}
