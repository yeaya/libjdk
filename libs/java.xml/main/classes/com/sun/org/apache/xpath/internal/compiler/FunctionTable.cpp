...
$Integer* FunctionTable::getFunctionID($String* key) {
	...
	if (nullptr == id) {
		$assign(id, $cast($Integer, FunctionTable::m_functionID->get(key)));
	}
	return id;
}
...
int32_t FunctionTable::installFunction($String* name, $Class* func) {
	...
	if (func != nullptr && !$Function::class$->isAssignableFrom(func)) {
		...
		var$0->append($(func->getName()));
		var$0->append(" cannot be cast to "_s);
		var$0->append($($Function::class$->getName()));
...
bool FunctionTable::functionAvailable($String* methName) {
	$var($Integer, tblEntry, $cast($Integer, FunctionTable::m_functionID->get(methName)));
	if (nullptr != tblEntry) {
		return true;
	} else {
...
void FunctionTable::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$assignStatic(FunctionTable::m_functionID, $new($HashMap));
	{
		...
		$load($FuncCurrent);
		FunctionTable::m_functions->set(FunctionTable::FUNC_CURRENT, $FuncCurrent::class$);
		$load($FuncLast);
		FunctionTable::m_functions->set(FunctionTable::FUNC_LAST, $FuncLast::class$);
		$load($FuncPosition);
		FunctionTable::m_functions->set(FunctionTable::FUNC_POSITION, $FuncPosition::class$);
		$load($FuncCount);
		FunctionTable::m_functions->set(FunctionTable::FUNC_COUNT, $FuncCount::class$);
		$load($FuncId);
		FunctionTable::m_functions->set(FunctionTable::FUNC_ID, $FuncId::class$);
		$load($FuncLocalPart);
		FunctionTable::m_functions->set(FunctionTable::FUNC_LOCAL_PART, $FuncLocalPart::class$);
		$load($FuncNamespace);
		FunctionTable::m_functions->set(FunctionTable::FUNC_NAMESPACE, $FuncNamespace::class$);
		$load($FuncQname);
		FunctionTable::m_functions->set(FunctionTable::FUNC_QNAME, $FuncQname::class$);
		$load($FuncGenerateId);
		FunctionTable::m_functions->set(FunctionTable::FUNC_GENERATE_ID, $FuncGenerateId::class$);
		$load($FuncNot);
		FunctionTable::m_functions->set(FunctionTable::FUNC_NOT, $FuncNot::class$);
		$load($FuncTrue);
		FunctionTable::m_functions->set(FunctionTable::FUNC_TRUE, $FuncTrue::class$);
		$load($FuncFalse);
		FunctionTable::m_functions->set(FunctionTable::FUNC_FALSE, $FuncFalse::class$);
		$load($FuncBoolean);
		FunctionTable::m_functions->set(FunctionTable::FUNC_BOOLEAN, $FuncBoolean::class$);
		$load($FuncLang);
		FunctionTable::m_functions->set(FunctionTable::FUNC_LANG, $FuncLang::class$);
		$load($FuncNumber);
		FunctionTable::m_functions->set(FunctionTable::FUNC_NUMBER, $FuncNumber::class$);
		$load($FuncFloor);
		FunctionTable::m_functions->set(FunctionTable::FUNC_FLOOR, $FuncFloor::class$);
		$load($FuncCeiling);
		FunctionTable::m_functions->set(FunctionTable::FUNC_CEILING, $FuncCeiling::class$);
		$load($FuncRound);
		FunctionTable::m_functions->set(FunctionTable::FUNC_ROUND, $FuncRound::class$);
		$load($FuncSum);
		FunctionTable::m_functions->set(FunctionTable::FUNC_SUM, $FuncSum::class$);
		$load($FuncString);
		FunctionTable::m_functions->set(FunctionTable::FUNC_STRING, $FuncString::class$);
		$load($FuncStartsWith);
		FunctionTable::m_functions->set(FunctionTable::FUNC_STARTS_WITH, $FuncStartsWith::class$);
		$load($FuncContains);
		FunctionTable::m_functions->set(FunctionTable::FUNC_CONTAINS, $FuncContains::class$);
		$load($FuncSubstringBefore);
		FunctionTable::m_functions->set(FunctionTable::FUNC_SUBSTRING_BEFORE, $FuncSubstringBefore::class$);
		$load($FuncSubstringAfter);
		FunctionTable::m_functions->set(FunctionTable::FUNC_SUBSTRING_AFTER, $FuncSubstringAfter::class$);
		$load($FuncNormalizeSpace);
		FunctionTable::m_functions->set(FunctionTable::FUNC_NORMALIZE_SPACE, $FuncNormalizeSpace::class$);
		$load($FuncTranslate);
		FunctionTable::m_functions->set(FunctionTable::FUNC_TRANSLATE, $FuncTranslate::class$);
		$load($FuncConcat);
		FunctionTable::m_functions->set(FunctionTable::FUNC_CONCAT, $FuncConcat::class$);
		$load($FuncSystemProperty);
		FunctionTable::m_functions->set(FunctionTable::FUNC_SYSTEM_PROPERTY, $FuncSystemProperty::class$);
		$load($FuncExtFunctionAvailable);
		FunctionTable::m_functions->set(FunctionTable::FUNC_EXT_FUNCTION_AVAILABLE, $FuncExtFunctionAvailable::class$);
		$load($FuncExtElementAvailable);
		FunctionTable::m_functions->set(FunctionTable::FUNC_EXT_ELEM_AVAILABLE, $FuncExtElementAvailable::class$);
		$load($FuncSubstring);
		FunctionTable::m_functions->set(FunctionTable::FUNC_SUBSTRING, $FuncSubstring::class$);
		$load($FuncStringLength);
		FunctionTable::m_functions->set(FunctionTable::FUNC_STRING_LENGTH, $FuncStringLength::class$);
		$load($FuncDoclocation);
		FunctionTable::m_functions->set(FunctionTable::FUNC_DOCLOCATION, $FuncDoclocation::class$);
		$load($FuncUnparsedEntityURI);
		FunctionTable::m_functions->set(FunctionTable::FUNC_UNPARSED_ENTITY_URI, $FuncUnparsedEntityURI::class$);
		$load($FuncHere);
		FunctionTable::m_functions->set(FunctionTable::FUNC_HERE, $FuncHere::class$);
	}
	{
		$init($Keywords);
		FunctionTable::m_functionID->put($Keywords::FUNC_CURRENT_STRING, $($Integer::valueOf(FunctionTable::FUNC_CURRENT)));
		FunctionTable::m_functionID->put($Keywords::FUNC_LAST_STRING, $($Integer::valueOf(FunctionTable::FUNC_LAST)));
		FunctionTable::m_functionID->put($Keywords::FUNC_POSITION_STRING, $($Integer::valueOf(FunctionTable::FUNC_POSITION)));
		FunctionTable::m_functionID->put($Keywords::FUNC_COUNT_STRING, $($Integer::valueOf(FunctionTable::FUNC_COUNT)));
		FunctionTable::m_functionID->put($Keywords::FUNC_ID_STRING, $($Integer::valueOf(FunctionTable::FUNC_ID)));
		FunctionTable::m_functionID->put($Keywords::FUNC_KEY_STRING, $($Integer::valueOf(FunctionTable::FUNC_KEY)));
		FunctionTable::m_functionID->put($Keywords::FUNC_LOCAL_PART_STRING, $($Integer::valueOf(FunctionTable::FUNC_LOCAL_PART)));
		FunctionTable::m_functionID->put($Keywords::FUNC_NAMESPACE_STRING, $($Integer::valueOf(FunctionTable::FUNC_NAMESPACE)));
		FunctionTable::m_functionID->put($Keywords::FUNC_NAME_STRING, $($Integer::valueOf(FunctionTable::FUNC_QNAME)));
		FunctionTable::m_functionID->put($Keywords::FUNC_GENERATE_ID_STRING, $($Integer::valueOf(FunctionTable::FUNC_GENERATE_ID)));
		FunctionTable::m_functionID->put($Keywords::FUNC_NOT_STRING, $($Integer::valueOf(FunctionTable::FUNC_NOT)));
		FunctionTable::m_functionID->put($Keywords::FUNC_TRUE_STRING, $($Integer::valueOf(FunctionTable::FUNC_TRUE)));
		FunctionTable::m_functionID->put($Keywords::FUNC_FALSE_STRING, $($Integer::valueOf(FunctionTable::FUNC_FALSE)));
		FunctionTable::m_functionID->put($Keywords::FUNC_BOOLEAN_STRING, $($Integer::valueOf(FunctionTable::FUNC_BOOLEAN)));
		FunctionTable::m_functionID->put($Keywords::FUNC_LANG_STRING, $($Integer::valueOf(FunctionTable::FUNC_LANG)));
		FunctionTable::m_functionID->put($Keywords::FUNC_NUMBER_STRING, $($Integer::valueOf(FunctionTable::FUNC_NUMBER)));
		FunctionTable::m_functionID->put($Keywords::FUNC_FLOOR_STRING, $($Integer::valueOf(FunctionTable::FUNC_FLOOR)));
		FunctionTable::m_functionID->put($Keywords::FUNC_CEILING_STRING, $($Integer::valueOf(FunctionTable::FUNC_CEILING)));
		FunctionTable::m_functionID->put($Keywords::FUNC_ROUND_STRING, $($Integer::valueOf(FunctionTable::FUNC_ROUND)));
		FunctionTable::m_functionID->put($Keywords::FUNC_SUM_STRING, $($Integer::valueOf(FunctionTable::FUNC_SUM)));
		FunctionTable::m_functionID->put($Keywords::FUNC_STRING_STRING, $($Integer::valueOf(FunctionTable::FUNC_STRING)));
		FunctionTable::m_functionID->put($Keywords::FUNC_STARTS_WITH_STRING, $($Integer::valueOf(FunctionTable::FUNC_STARTS_WITH)));
		FunctionTable::m_functionID->put($Keywords::FUNC_CONTAINS_STRING, $($Integer::valueOf(FunctionTable::FUNC_CONTAINS)));
		FunctionTable::m_functionID->put($Keywords::FUNC_SUBSTRING_BEFORE_STRING, $($Integer::valueOf(FunctionTable::FUNC_SUBSTRING_BEFORE)));
		FunctionTable::m_functionID->put($Keywords::FUNC_SUBSTRING_AFTER_STRING, $($Integer::valueOf(FunctionTable::FUNC_SUBSTRING_AFTER)));
		FunctionTable::m_functionID->put($Keywords::FUNC_NORMALIZE_SPACE_STRING, $($Integer::valueOf(FunctionTable::FUNC_NORMALIZE_SPACE)));
		FunctionTable::m_functionID->put($Keywords::FUNC_TRANSLATE_STRING, $($Integer::valueOf(FunctionTable::FUNC_TRANSLATE)));
		FunctionTable::m_functionID->put($Keywords::FUNC_CONCAT_STRING, $($Integer::valueOf(FunctionTable::FUNC_CONCAT)));
		FunctionTable::m_functionID->put($Keywords::FUNC_SYSTEM_PROPERTY_STRING, $($Integer::valueOf(FunctionTable::FUNC_SYSTEM_PROPERTY)));
		FunctionTable::m_functionID->put($Keywords::FUNC_EXT_FUNCTION_AVAILABLE_STRING, $($Integer::valueOf(FunctionTable::FUNC_EXT_FUNCTION_AVAILABLE)));
		FunctionTable::m_functionID->put($Keywords::FUNC_EXT_ELEM_AVAILABLE_STRING, $($Integer::valueOf(FunctionTable::FUNC_EXT_ELEM_AVAILABLE)));
		FunctionTable::m_functionID->put($Keywords::FUNC_SUBSTRING_STRING, $($Integer::valueOf(FunctionTable::FUNC_SUBSTRING)));
		FunctionTable::m_functionID->put($Keywords::FUNC_STRING_LENGTH_STRING, $($Integer::valueOf(FunctionTable::FUNC_STRING_LENGTH)));
		FunctionTable::m_functionID->put($Keywords::FUNC_UNPARSED_ENTITY_URI_STRING, $($Integer::valueOf(FunctionTable::FUNC_UNPARSED_ENTITY_URI)));
		FunctionTable::m_functionID->put($Keywords::FUNC_DOCLOCATION_STRING, $($Integer::valueOf(FunctionTable::FUNC_DOCLOCATION)));
		FunctionTable::m_functionID->put($Keywords::FUNC_HERE_STRING, $($Integer::valueOf(FunctionTable::FUNC_HERE)));
	}
}
...
$Class* FunctionTable::load$($String* name, bool initialize) {
	...
	$loadClass(FunctionTable, name, initialize, &classInfo$$, FunctionTable::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(FunctionTable);
	});
	return class$;
}
