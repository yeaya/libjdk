...
void DecimalFormatting::translate($ClassGenerator* classGen, $MethodGenerator* methodGen) {
	...
	il->append($$new($NEW, cpg->addClass(DecimalFormatting::DFS_CLASS)));
	il->append($Constants::DUP);
	il->append($$new($GETSTATIC, cpg->addFieldref($Constants::LOCALE_CLASS, "US"_s, $Constants::LOCALE_SIG)));
	il->append($$new($INVOKESPECIAL, init));
	$var($String, tmp, getAttribute("NaN"_s));
	if ((tmp == nullptr) || (tmp->equals($Constants::EMPTYSTRING))) {
		int32_t nan = cpg->addMethodref(DecimalFormatting::DFS_CLASS, "setNaN"_s, "(Ljava/lang/String;)V"_s);
		il->append($Constants::DUP);
		il->append($$new($PUSH, cpg, "NaN"_s));
	...
	if ((tmp == nullptr) || (tmp->equals($Constants::EMPTYSTRING))) {
		...
		il->append($Constants::DUP);
		il->append($$new($PUSH, cpg, "Infinity"_s));
	...
	for (int32_t i = 0; i < nAttributes; ++i) {
		...
		if (valid) {
			il->append($Constants::DUP);
			il->append($$new($PUSH, cpg, (int32_t)$nc(value)->charAt(0)));
			il->append($$new($INVOKEVIRTUAL, method));
		}
	}
...
void DecimalFormatting::clinit$($Class* clazz) {
...
$Class* DecimalFormatting::load$($String* name, bool initialize) {
	...
	$loadClass(DecimalFormatting, name, initialize, &classInfo$$, DecimalFormatting::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(DecimalFormatting);
	});
	return class$;
}
