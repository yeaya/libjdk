...
void XMLSchemaValidatorComponentManager::init$($XSGrammarPoolContainer* grammarContainer) {
	...
	this->fComponents->put(XMLSchemaValidatorComponentManager::ENTITY_MANAGER, this->fEntityManager);
	$set(this, fErrorReporter, $new($XMLErrorReporter));
	this->fComponents->put(XMLSchemaValidatorComponentManager::ERROR_REPORTER, this->fErrorReporter);
	$set(this, fNamespaceContext, $new($NamespaceSupport));
	this->fComponents->put(XMLSchemaValidatorComponentManager::NAMESPACE_CONTEXT, this->fNamespaceContext);
	$set(this, fSchemaValidator, $new($XMLSchemaValidator));
	this->fComponents->put(XMLSchemaValidatorComponentManager::SCHEMA_VALIDATOR, this->fSchemaValidator);
	$set(this, fValidationManager, $new($ValidationManager));
	this->fComponents->put(XMLSchemaValidatorComponentManager::VALIDATION_MANAGER, this->fValidationManager);
	this->fComponents->put(XMLSchemaValidatorComponentManager::ENTITY_RESOLVER, nullptr);
	this->fComponents->put(XMLSchemaValidatorComponentManager::ERROR_HANDLER, nullptr);
	this->fComponents->put(XMLSchemaValidatorComponentManager::SYMBOL_TABLE, $$new($SymbolTable));
	this->fComponents->put(XMLSchemaValidatorComponentManager::XMLGRAMMAR_POOL, $($nc(grammarContainer)->getGrammarPool()));
	...
	this->fFeatures->put(XMLSchemaValidatorComponentManager::NORMALIZE_DATA, $Boolean::FALSE);
	this->fFeatures->put(XMLSchemaValidatorComponentManager::SCHEMA_ELEMENT_DEFAULT, $Boolean::FALSE);
	this->fFeatures->put(XMLSchemaValidatorComponentManager::SCHEMA_AUGMENT_PSVI, $Boolean::TRUE);
	this->fFeatures->put($XMLConstants::USE_CATALOG, $(grammarContainer->getFeature($XMLConstants::USE_CATALOG)));
	...
	this->fFeatures->put(XMLSchemaValidatorComponentManager::ID_IDREF_CHECKING, $Boolean::TRUE);
	this->fFeatures->put(XMLSchemaValidatorComponentManager::IDENTITY_CONSTRAINT_CHECKING, $Boolean::TRUE);
	this->fFeatures->put(XMLSchemaValidatorComponentManager::UNPARSED_ENTITY_CHECKING, $Boolean::TRUE);
...
$FeatureState* XMLSchemaValidatorComponentManager::getFeatureState($String* featureId) {
	...
	if ($nc($ParserConfigurationSettings::PARSER_SETTINGS)->equals(featureId)) {
		...
		bool var$0 = XMLSchemaValidatorComponentManager::VALIDATION->equals(featureId);
		if (var$0 || XMLSchemaValidatorComponentManager::SCHEMA_VALIDATION->equals(featureId)) {
			return $FeatureState::is(true);
		} else if (XMLSchemaValidatorComponentManager::USE_GRAMMAR_POOL_ONLY->equals(featureId)) {
			...
			} else if (XMLSchemaValidatorComponentManager::SCHEMA_ELEMENT_DEFAULT->equals(featureId)) {
				return $FeatureState::is(true);
			}
		}
	}
...
void XMLSchemaValidatorComponentManager::setFeature($String* featureId, bool value) {
	...
	if ($nc($ParserConfigurationSettings::PARSER_SETTINGS)->equals(featureId)) {
		...
		bool var$0 = value == false;
		if (var$0) {
			bool var$1 = XMLSchemaValidatorComponentManager::VALIDATION->equals(featureId);
			var$0 = var$1 || XMLSchemaValidatorComponentManager::SCHEMA_VALIDATION->equals(featureId);
		}
		if (var$0) {
			$init($Status);
			$throwNew($XMLConfigurationException, $Status::NOT_SUPPORTED, featureId);
		} else if (XMLSchemaValidatorComponentManager::USE_GRAMMAR_POOL_ONLY->equals(featureId) && value != this->fUseGrammarPoolOnly) {
			$init($Status);
			$throwNew($XMLConfigurationException, $Status::NOT_SUPPORTED, featureId);
		}
	}
...
$PropertyState* XMLSchemaValidatorComponentManager::getPropertyState($String* propertyId) {
	$useLocalObjectStack();
	if (XMLSchemaValidatorComponentManager::LOCALE->equals(propertyId)) {
		return $PropertyState::is($(getLocale()));
	}
	$var($Object, component, this->fComponents->get(propertyId));
	if (component != nullptr) {
		return $PropertyState::is(component);
	} else if (this->fComponents->containsKey(propertyId)) {
...
void XMLSchemaValidatorComponentManager::setProperty($String* propertyId, Object$* value) {
	bool var$5 = XMLSchemaValidatorComponentManager::ENTITY_MANAGER->equals(propertyId);
	bool var$4 = var$5 || XMLSchemaValidatorComponentManager::ERROR_REPORTER->equals(propertyId);
	bool var$3 = var$4 || XMLSchemaValidatorComponentManager::NAMESPACE_CONTEXT->equals(propertyId);
	bool var$2 = var$3 || XMLSchemaValidatorComponentManager::SCHEMA_VALIDATOR->equals(propertyId);
	bool var$1 = var$2 || XMLSchemaValidatorComponentManager::SYMBOL_TABLE->equals(propertyId);
	bool var$0 = var$1 || XMLSchemaValidatorComponentManager::VALIDATION_MANAGER->equals(propertyId);
	if (var$0 || XMLSchemaValidatorComponentManager::XMLGRAMMAR_POOL->equals(propertyId)) {
		$init($Status);
	...
	bool var$7 = XMLSchemaValidatorComponentManager::ENTITY_RESOLVER->equals(propertyId);
	bool var$6 = var$7 || XMLSchemaValidatorComponentManager::ERROR_HANDLER->equals(propertyId);
	if (var$6 || XMLSchemaValidatorComponentManager::SECURITY_MANAGER->equals(propertyId)) {
		this->fComponents->put(propertyId, value);
		return;
	} else if (XMLSchemaValidatorComponentManager::LOCALE->equals(propertyId)) {
		setLocale($cast($Locale, value));
		this->fComponents->put(propertyId, value);
		return;
	}
	$init($JdkProperty$State);
	if (this->fInitSecurityManager == nullptr || !this->fInitSecurityManager->setLimit(propertyId, $JdkProperty$State::APIPROPERTY, value)) {
		$init($XMLSecurityPropertyManager$State);
		if (this->fSecurityPropertyMgr == nullptr || !this->fSecurityPropertyMgr->setValue(propertyId, $XMLSecurityPropertyManager$State::APIPROPERTY, value)) {
			if (!this->fInitProperties->containsKey(propertyId)) {
				this->fInitProperties->put(propertyId, $($ParserConfigurationSettings::getProperty(propertyId)));
			}
...
void XMLSchemaValidatorComponentManager::reset() {
	...
	$nc(this->fEntityManager)->reset(this);
...
void XMLSchemaValidatorComponentManager::setErrorHandler($ErrorHandler* errorHandler) {
	...
	setProperty(XMLSchemaValidatorComponentManager::ERROR_HANDLER, (errorHandler != nullptr) ? $$new($ErrorHandlerWrapper, errorHandler) : $$new($ErrorHandlerWrapper, $($DraconianErrorHandler::getInstance())));
}
...
void XMLSchemaValidatorComponentManager::restoreInitialState() {
	...
	this->fComponents->put(XMLSchemaValidatorComponentManager::ERROR_HANDLER, nullptr);
	setLocale(nullptr);
	this->fComponents->put(XMLSchemaValidatorComponentManager::LOCALE, nullptr);
	$nc(this->fInitSecurityManager)->setSecureProcessing(true);
	this->fComponents->put(XMLSchemaValidatorComponentManager::SECURITY_MANAGER, this->fInitSecurityManager);
	setLocale(nullptr);
	this->fComponents->put(XMLSchemaValidatorComponentManager::LOCALE, nullptr);
	if (!this->fInitFeatures->isEmpty()) {
		{
			$var($Iterator, i$, $$nc(this->fInitFeatures->entrySet())->iterator());
			for (; $nc(i$)->hasNext();) {
				$var($Map$Entry, entry, $cast($Map$Entry, i$->next()));
				{
					$var($String, name, $cast($String, $nc(entry)->getKey()));
					bool value = $$sure($Boolean, entry->getValue())->booleanValue();
	...
	if (!this->fInitProperties->isEmpty()) {
		{
			$var($Iterator, i$, $$nc(this->fInitProperties->entrySet())->iterator());
			for (; $nc(i$)->hasNext();) {
				$var($Map$Entry, entry, $cast($Map$Entry, i$->next()));
				{
					$var($String, name, $cast($String, $nc(entry)->getKey()));
...
void XMLSchemaValidatorComponentManager::clinit$($Class* clazz) {
	$init($Constants);
...
$Class* XMLSchemaValidatorComponentManager::load$($String* name, bool initialize) {
	...
	$loadClass(XMLSchemaValidatorComponentManager, name, initialize, &classInfo$$, XMLSchemaValidatorComponentManager::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLSchemaValidatorComponentManager);
	});
	return class$;
}
