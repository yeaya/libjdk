...
$Class* DTMDefaultBaseTraversers$DescendantFromRootTraverser::load$($String* name, bool initialize) {
	...
	$loadClass(DTMDefaultBaseTraversers$DescendantFromRootTraverser, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DTMDefaultBaseTraversers$DescendantFromRootTraverser);
	});
	return class$;
}
