...
void XMLErrorResources_it::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(XMLErrorResources_it::contents, $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_FUNCTION_NOT_SUPPORTED,
			"Funzione non supportata."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_CANNOT_OVERWRITE_CAUSE,
			"Impossibile sovrascrivere la causa"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_NO_DEFAULT_IMPL,
			"Nessuna implementazione predefinita trovata "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_CHUNKEDINTARRAY_NOT_SUPPORTED,
			"ChunkedIntArray({0}) non supportato al momento"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_OFFSET_BIGGER_THAN_SLOT,
			u"Offset più grande dello slot"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_COROUTINE_NOT_AVAIL,
			"Co-routine non disponibile, ID={0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_COROUTINE_CO_EXIT,
			"CoroutineManager ha ricevuto una richiesta co_exit()"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_COJOINROUTINESET_FAILED,
			"co_joinCoroutineSet() non riuscito"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_COROUTINE_PARAM,
			"Errore del parametro di co-routine ({0})"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_PARSER_DOTERMINATE_ANSWERS,
			"\nIMPREVISTO: risposte doTerminate del parser {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_NO_PARSE_CALL_WHILE_PARSING,
			u"impossibile richiamare parse mentre è in corso un\'analisi"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_TYPED_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"Errore: l\'\'iteratore con tipo per l\'\'asse {0} non è implementato"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"Errore: l\'\'iteratore per l\'\'asse {0} non è implementato "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_ITERATOR_CLONE_NOT_SUPPORTED,
			"Duplicazione dell\'iteratore non supportata"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_UNKNOWN_AXIS_TYPE,
			"Tipo di asse trasversale sconosciuto: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_AXIS_NOT_SUPPORTED,
			"Asse trasversale non supportato: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_NO_DTMIDS_AVAIL,
			"Non sono disponibili altri ID DTM"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_NOT_SUPPORTED,
			"Non supportato: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_NODE_NON_NULL,
			"Il nodo deve essere non nullo per getDTMHandleFromNode"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_COULD_NOT_RESOLVE_NODE,
			"Impossibile risolvere il nodo in un handle"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_STARTPARSE_WHILE_PARSING,
			u"impossibile richiamare startParse mentre è in corso un\'analisi"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_STARTPARSE_NEEDS_SAXPARSER,
			"startParse richiede un valore non nullo per SAXParser"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_COULD_NOT_INIT_PARSER,
			"impossibile inizializzare il parser con"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_EXCEPTION_CREATING_POOL,
			"eccezione durante la creazione di una nuova istanza per il pool"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_PATH_CONTAINS_INVALID_ESCAPE_SEQUENCE,
			"Il percorso contiene sequenza di escape non valida"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_SCHEME_REQUIRED,
			u"Lo schema è obbligatorio."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_NO_SCHEME_IN_URI,
			"Nessuno schema trovato nell\'\'URI: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_NO_SCHEME_INURI,
			"Nessuno schema trovato nell\'URI"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_PATH_INVALID_CHAR,
			"Il percorso contiene un carattere non valido: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_SCHEME_FROM_NULL_STRING,
			"Impossibile impostare lo schema da una stringa nulla"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_SCHEME_NOT_CONFORMANT,
			u"Lo schema non è conforme."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_HOST_ADDRESS_NOT_WELLFORMED,
			u"Host non è un indirizzo corretto"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_PORT_WHEN_HOST_NULL,
			u"La porta non può essere impostata se l\'host è nullo"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_INVALID_PORT,
			"Numero di porta non valido"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_FRAG_FOR_GENERIC_URI,
			u"Il frammento può essere impostato solo per un URI generico"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_FRAG_WHEN_PATH_NULL,
			u"Il frammento non può essere impostato se il percorso è nullo"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_FRAG_INVALID_CHAR,
			"Il frammento contiene un carattere non valido"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_PARSER_IN_USE,
			u"Parser già in uso"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_CANNOT_CHANGE_WHILE_PARSING,
			"Impossibile modificare {0} {1} durante l\'\'analisi"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_SELF_CAUSATION_NOT_PERMITTED,
			"Creazione automatica della causa non consentita"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_NO_USERINFO_IF_NO_HOST,
			u"Userinfo non può essere specificato se l\'host non è specificato"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_NO_PORT_IF_NO_HOST,
			u"La porta non può essere specificata se l\'host non è specificato"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_NO_QUERY_STRING_IN_PATH,
			u"La stringa di query non può essere specificata nella stringa di percorso e query."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_NO_FRAGMENT_STRING_IN_PATH,
			u"Il frammento non può essere specificato sia nel percorso che nel frammento"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_CANNOT_INIT_URI_EMPTY_PARMS,
			"Impossibile inizializzare l\'URI con i parametri vuoti"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_METHOD_NOT_SUPPORTED,
			"Metodo attualmente non supportato "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_INCRSAXSRCFILTER_NOT_RESTARTABLE,
			"IncrementalSAXSource_Filter attualmente non riavviabile"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_XMLRDR_NOT_BEFORE_STARTPARSE,
			"XMLReader non si trova prima della richiesta startParse"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_AXIS_TRAVERSER_NOT_SUPPORTED,
			"Asse trasversale non supportato: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_ERRORHANDLER_CREATED_WITH_NULL_PRINTWRITER,
			"ListingErrorHandler creato con PrintWriter nullo."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_SYSTEMID_UNKNOWN,
			"SystemId sconosciuto"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_LOCATION_UNKNOWN,
			"Posizione sconosciuta dell\'errore"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_PREFIX_MUST_RESOLVE,
			"Il prefisso deve essere risolto in uno spazio di nomi: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_CREATEDOCUMENT_NOT_SUPPORTED,
			"createDocument() non supportato in XPathContext"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_CHILD_HAS_NO_OWNER_DOCUMENT,
			"L\'elemento figlio dell\'attributo non dispone di un documento proprietario."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_CHILD_HAS_NO_OWNER_DOCUMENT_ELEMENT,
			"L\'elemento figlio dell\'attributo non dispone di un elemento di documento proprietario."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_CANT_OUTPUT_TEXT_BEFORE_DOC,
			"Avvertenza: impossibile creare l\'output del testo prima dell\'elemento del documento. Operazione ignorata..."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_CANT_HAVE_MORE_THAN_ONE_ROOT,
			u"Non possono esistere più radici in un DOM."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_ARG_LOCALNAME_NULL,
			u"L\'argomento \'localName\' è nullo"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_ARG_LOCALNAME_INVALID,
			"Localname in QNAME deve essere un NCName valido"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_ARG_PREFIX_INVALID,
			"Il prefisso in QNAME deve essere un NCName valido"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_NAME_CANT_START_WITH_COLON,
			u"Il nome non può iniziare con i due punti"_s
		}),
		$$new($ObjectArray, {
			"BAD_CODE"_s,
		...
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_SERIALIZER_NOT_CONTENTHANDLER,
			"La classe serializzatore \'\'{0}\'\' non implementa org.xml.sax.ContentHandler."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_RESOURCE_COULD_NOT_FIND,
			"Risorsa [ {0} ] non trovata.\n {1}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_RESOURCE_COULD_NOT_LOAD,
			"Impossibile caricare la risorsa [ {0} ]: {1} \n {2} \t {3}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_BUFFER_SIZE_LESSTHAN_ZERO,
			"Dimensione buffer <=0"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_INVALID_UTF16_SURROGATE,
			"Rilevato surrogato UTF-16 non valido: {0}?"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_OIERROR,
			"Errore IO"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_ILLEGAL_ATTRIBUTE_POSITION,
		...
		$$new($ObjectArray, {
			XMLErrorResources_it::ER_COULD_NOT_LOAD_METHOD_PROPERTY,
...
$Class* XMLErrorResources_it::load$($String* name, bool initialize) {
	...
	$loadClass(XMLErrorResources_it, name, initialize, &classInfo$$, XMLErrorResources_it::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLErrorResources_it);
	});
	return class$;
}
