...
$Class* LiteralElement$$Lambda$lambda$getNamespaceScope$0::load$($String* name, bool initialize) {
	...
	$loadClass(LiteralElement$$Lambda$lambda$getNamespaceScope$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LiteralElement$$Lambda$lambda$getNamespaceScope$0);
	});
	return class$;
}
...
$String* LiteralElement::accessedNamespace($String* prefix) {
	if (this->_literalElemParent != nullptr) {
		$var($String, result, this->_literalElemParent->accessedNamespace(prefix));
		if (result != nullptr) {
			return result;
		}
	}
	return this->_accessedPrefixes != nullptr ? $cast($String, this->_accessedPrefixes->get(prefix)) : ($String*)nullptr;
}
...
void LiteralElement::registerNamespace($String* prefix$renamed, $String* uri, $SymbolTable* stable, bool declared) {
	...
	if (this->_literalElemParent != nullptr) {
		$var($String, parentUri, this->_literalElemParent->accessedNamespace(prefix));
	...
	if (this->_accessedPrefixes == nullptr) {
		...
		$var($String, old, $cast($String, this->_accessedPrefixes->get(prefix)));
		if (old != nullptr) {
			if (old->equals(uri)) {
				return;
			} else {
...
$Type* LiteralElement::typeCheck($SymbolTable* stable) {
	$useLocalObjectStack();
	if (this->_attributeElements != nullptr) {
		$var($Iterator, i$, this->_attributeElements->iterator());
		for (; $nc(i$)->hasNext();) {
...
$Set* LiteralElement::getNamespaceScope($SyntaxTreeNode* node$renamed) {
	...
	while (node != nullptr) {
		...
		if (mapping != nullptr) {
			$$nc($$nc(mapping->entrySet())->stream())->forEach($$new(LiteralElement$$Lambda$lambda$getNamespaceScope$0, all));
		}
	...
	for (int32_t i = 0; i < count; ++i) {
		...
		if (qname->equals($(parser->getUseAttributeSets()))) {
			if (!$Util::isValidQNames(val)) {
				$init($ErrorMsg);
				$var($ErrorMsg, err, $new($ErrorMsg, $ErrorMsg::INVALID_QNAME_ERR, val, this));
...
void LiteralElement::translate($ClassGenerator* classGen, $MethodGenerator* methodGen) {
	...
	if (this->_accessedPrefixes != nullptr) {
		$var($Iterator, i$, $$nc(this->_accessedPrefixes->entrySet())->iterator());
		for (; $nc(i$)->hasNext();) {
			$var($Map$Entry, entry, $cast($Map$Entry, i$->next()));
			{
				$var($String, prefix, $cast($String, $nc(entry)->getKey()));
...
bool LiteralElement::checkAttributesUnique() {
	...
	if (this->_attributeElements != nullptr) {
		int32_t numAttrs = this->_attributeElements->size();
		...
		for (int32_t i = 0; i < numAttrs; ++i) {
			...
			if ($instanceOf($UseAttributeSets, node)) {
				...
				$var($AttributeValue, attrName, xslAttr->getName());
				if ($instanceOf($AttributeValueTemplate, attrName)) {
					...
					$var($String, name, simpleAttr->toString());
					if (name != nullptr && $nc(attrsTable)->get(name) != nullptr) {
						return false;
					} else if (name != nullptr) {
						$nc(attrsTable)->put(name, xslAttr);
					}
				}
			}
		}
	...
	$var($List, contents, $nc(node)->getContents());
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			if ($instanceOf($Text, child)) {
				...
				$var($List, chooseContents, child->getContents());
				{
...
$Class* LiteralElement::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.org.apache.xalan.internal.xsltc.compiler.LiteralElement$$Lambda$lambda$getNamespaceScope$0")) {
	...
	$loadClass(LiteralElement, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LiteralElement);
	});
	return class$;
}
