...
$Class* ToStream$CharacterBuffer$GenericCharacters::load$($String* name, bool initialize) {
	...
	$loadClass(ToStream$CharacterBuffer$GenericCharacters, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ToStream$CharacterBuffer$GenericCharacters);
	});
	return class$;
}
