...
void TreeWalker::dispatachChars($Node* node) {
	...
	if ($instanceOf($DOM2DTM$CharacterNodeHandler, this->m_contentHandler)) {
		$cast($DOM2DTM$CharacterNodeHandler, this->m_contentHandler)->characters(node);
	} else {
...
void TreeWalker::startNode($Node* node) {
	...
	if ($instanceOf($NodeConsumer, this->m_contentHandler)) {
		$cast($NodeConsumer, this->m_contentHandler)->setOriginatingNode(node);
	}
	if ($instanceOf($Locator, node)) {
		$var($Locator, loc, $cast($Locator, node));
		$nc(this->m_locator)->setColumnNumber(loc->getColumnNumber());
		$nc(this->m_locator)->setLineNumber(loc->getLineNumber());
		$nc(this->m_locator)->setPublicId($(loc->getPublicId()));
		$nc(this->m_locator)->setSystemId($(loc->getSystemId()));
	} else {
	...
	}
	{
		...
		case $Node::ELEMENT_NODE:
			{
				$assign(atts, $cast($Element, node)->getAttributes());
