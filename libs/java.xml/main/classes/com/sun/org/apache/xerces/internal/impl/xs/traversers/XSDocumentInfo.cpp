...
void XSDocumentInfo::initNamespaceSupport($Element* schemaRoot) {
	...
	while (true) {
		...
		}
		{
			...
			$var($NamedNodeMap, map, $nc(eparent)->getAttributes());
			int32_t length = (map != nullptr) ? map->getLength() : 0;
			for (int32_t i = 0; i < length; ++i) {
				$var($Attr, attr, $cast($Attr, $nc(map)->item(i)));
...
bool XSDocumentInfo::isAllowedNS($String* namespace$) {
	return $nc(this->fImportedNS)->contains(namespace$ == nullptr ? ""_s : namespace$);
}
...
$Class* XSDocumentInfo::load$($String* name, bool initialize) {
	...
	$loadClass(XSDocumentInfo, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XSDocumentInfo);
	});
	return class$;
}
