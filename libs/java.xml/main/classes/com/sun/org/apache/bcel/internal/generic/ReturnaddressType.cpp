...
int32_t ReturnaddressType::hashCode() {
	...
	return $nc(this->returnTarget)->hashCode();
}
...
bool ReturnaddressType::equals(Object$* rat) {
	...
	if (this->returnTarget == nullptr || $nc(that)->returnTarget == nullptr) {
		return $nc(that)->returnTarget == this->returnTarget;
	}
	return $nc($nc(that)->returnTarget)->equals(this->returnTarget);
}
...
void ReturnaddressType::clinit$($Class* clazz) {
...
$Class* ReturnaddressType::load$($String* name, bool initialize) {
	...
	$loadClass(ReturnaddressType, name, initialize, &classInfo$$, ReturnaddressType::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ReturnaddressType);
	});
	return class$;
}
