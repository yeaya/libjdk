...
void Lexer::tokenize($String* pat, $List* targetStrings) {
	...
	$set(this->m_compiler, m_opMap, $new($OpMapVector, $OpMap::MAXTOKENQUEUESIZE * 5, $OpMap::BLOCKTOKENQUEUESIZE * 5, $OpMap::MAPINDEX_LENGTH));
	...
	if (startSubstring != -1) {
		isNum = false;
		isStartOfPat = mapPatternElemPos(nesting, isStartOfPat, isAttrName);
		if ((-1 != posOfNSSep) || ((this->m_namespaceContext != nullptr) && (this->m_namespaceContext->handlesNullPrefixes()))) {
...
bool Lexer::mapPatternElemPos(int32_t nesting, bool isStart, bool isAttrName) {
	if (0 == nesting) {
		if (this->m_patternMapSize >= $nc(this->m_patternMap)->length) {
			...
			int32_t len = this->m_patternMap->length;
			$set(this, m_patternMap, $new($ints, this->m_patternMapSize + 100));
			$System::arraycopy(patternMap, 0, this->m_patternMap, 0, len);
		}
		if (!isStart) {
			(*this->m_patternMap)[this->m_patternMapSize - 1] -= Lexer::TARGETEXTRA;
		}
		this->m_patternMap->set(this->m_patternMapSize, ($nc(this->m_compiler)->getTokenQueueSize() - (isAttrName ? 1 : 0)) + Lexer::TARGETEXTRA);
...
void Lexer::resetTokenMark(int32_t mark) {
	...
	if (this->m_processor->m_queueMark < qsz) {
		$set(this->m_processor, m_token, $cast($String, $$nc($nc(this->m_compiler)->getTokenQueue())->elementAt(this->m_processor->m_queueMark++)));
...
int32_t Lexer::getKeywordToken($String* key) {
	...
	try {
		...
		tok = (nullptr != itok) ? itok->intValue() : 0;
...
int32_t Lexer::mapNSTokens($String* pat, int32_t startSubstring, int32_t posOfNSSep, int32_t posOfScan) {
	...
	if (var$0 && !prefix->equals("xmlns"_s)) {
		try {
			if (prefix->length() > 0) {
				$assign(uName, this->m_namespaceContext->getNamespaceForPrefix(prefix));
			} else {
