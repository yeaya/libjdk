...
$String* ModuleRequires::toString() {
	$useLocalObjectStack();
	return $str({"requires("_s, $$str(this->requiresIndex), ", "_s, $($String::format("%04x"_s, $$new($ObjectArray, {$($Integer::valueOf(this->requiresFlags))}))), ", "_s, $$str(this->requiresVersionIndex), ")"_s});
}
...
$String* ModuleRequires::toString($ConstantPool* constant_pool) {
	...
	buf->append(", "_s)->append($($String::format("%04x"_s, $$new($ObjectArray, {$($Integer::valueOf(this->requiresFlags))}))));
...
$Class* ModuleRequires::load$($String* name, bool initialize) {
	...
	$loadClass(ModuleRequires, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
