...
$String* XPath$Step::toString() {
	...
	if (this->axis->type == $XPath$Axis::ATTRIBUTE) {
		return $str({"@"_s, $($nc(this->nodeTest)->toString())});
	}
	if (this->axis->type == $XPath$Axis::CHILD) {
		return $nc(this->nodeTest)->toString();
	}
	if (this->axis->type == $XPath$Axis::DESCENDANT) {
		return "//"_s;
	}
	return $str({"??? ("_s, $$str(this->axis->type), $$str(u')')});
}
...
$Class* XPath$Step::load$($String* name, bool initialize) {
	...
	$loadClass(XPath$Step, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XPath$Step);
	});
	return class$;
}
