...
void JAXPValidatorComponent::init$($ValidatorHandler* validatorHandler) {
	...
	this->validator->setErrorHandler($$new($JAXPValidatorComponent$1, this));
	this->validator->setResourceResolver($$new($JAXPValidatorComponent$2, this));
}
...
void JAXPValidatorComponent::characters($XMLString* text, $Augmentations* augs) {
	$set(this, fCurrentAug, augs);
	this->xni2sax->characters(text, nullptr);
}

void JAXPValidatorComponent::ignorableWhitespace($XMLString* text, $Augmentations* augs) {
	$set(this, fCurrentAug, augs);
	this->xni2sax->ignorableWhitespace(text, nullptr);
}
...
void JAXPValidatorComponent::updateAttributes($Attributes* atts) {
	...
	for (int32_t i = 0; i < len; ++i) {
		...
		if (j == -1) {
			...
			int32_t idx = $nc(aqn)->indexOf(u':');
			if (idx < 0) {
				$assign(prefix, nullptr);
			} else {
				$assign(prefix, symbolize($(aqn->substring(0, idx))));
			}
			$var($String, var$1, symbolize($(atts->getLocalName(i))));
			$var($String, var$2, symbolize(aqn));
			$var($QName, var$0, $new($QName, prefix, var$1, var$2, $(symbolize($(atts->getURI(i))))));
...
void JAXPValidatorComponent::clinit$($Class* clazz) {
	$init($Constants);
...
$Class* JAXPValidatorComponent::load$($String* name, bool initialize) {
	...
	$loadClass(JAXPValidatorComponent, name, initialize, &classInfo$$, JAXPValidatorComponent::clinit$, []($Class* clazz) -> $Object* {
