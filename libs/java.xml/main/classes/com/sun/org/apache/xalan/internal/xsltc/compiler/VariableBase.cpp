...
void VariableBase::translateValue($ClassGenerator* classGen, $MethodGenerator* methodGen) {
	...
	if (this->_select != nullptr) {
		...
		if ($instanceOf($NodeSetType, $($nc(this->_select)->getType()))) {
			...
			$nc(il)->append($$new($NEW, cpg->addClass($Constants::CACHED_NODE_LIST_ITERATOR_CLASS)));
...
$Class* VariableBase::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		...
		{"getType", "()Lcom/sun/org/apache/xalan/internal/xsltc/compiler/util/Type;", nullptr, $PUBLIC, $virtualMethod(VariableBase, getType, $1Type*)},
	...
	$loadClass(VariableBase, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(VariableBase);
	});
	return class$;
}
