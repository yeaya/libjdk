...
$DTMAxisIterator* DTMDefaultBaseIterators::getTypedAxisIterator(int32_t axis, int32_t type) {
	...
	$var($DTMAxisIterator, iterator, nullptr);
	{
		...
		default:
			...
			$throwNew($DTMException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_TYPED_ITERATOR_AXIS_NOT_IMPLEMENTED, $$new($ObjectArray, {$($Axis::getNames(axis))}))));
		}
	}
	return (iterator);
}
...
$DTMAxisIterator* DTMDefaultBaseIterators::getAxisIterator(int32_t axis) {
	...
	default:
		$init($XMLErrorResources);
		$throwNew($DTMException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_ITERATOR_AXIS_NOT_IMPLEMENTED, $$new($ObjectArray, {$($Axis::getNames(axis))}))));
	}
	return (iterator);
}
...
$Class* DTMDefaultBaseIterators::load$($String* name, bool initialize) {
	...
	$loadClass(DTMDefaultBaseIterators, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DTMDefaultBaseIterators);
	});
	return class$;
}
