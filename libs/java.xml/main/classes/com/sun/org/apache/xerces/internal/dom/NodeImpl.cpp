...
int16_t NodeImpl::compareTreePosition($Node* other) {
	...
	if (thisAncestorType == $Node::ATTRIBUTE_NODE) {
		$assign(thisNode, $cast($AttrImpl, thisAncestor)->getOwnerElement());
	}
	if (otherAncestorType == $Node::ATTRIBUTE_NODE) {
		$assign(otherNode, $cast($AttrImpl, otherAncestor)->getOwnerElement());
	}
	...
	}
	{
		...
		for (; current != nullptr; $assign(current, current->getNextSibling())) {
...
int16_t NodeImpl::compareDocumentPosition($Node* other) {
	...
	if (thisOwnerDoc != otherOwnerDoc && thisOwnerDoc != nullptr && otherOwnerDoc != nullptr) {
		int32_t otherDocNum = $cast($CoreDocumentImpl, otherOwnerDoc)->getNodeNumber();
		int32_t thisDocNum = $cast($CoreDocumentImpl, thisOwnerDoc)->getNodeNumber();
			...
			case $Node::ENTITY_NODE:
				{
					if (thisAncestorType != otherAncestorType) {
						...
						if ($$sure($NamedNodeMapImpl, $nc(container)->getNotations())->precedes(otherAncestor, thisAncestor)) {
					...
					} else if ($$sure($NamedNodeMapImpl, $nc(container)->getEntities())->precedes(otherAncestor, thisAncestor)) {
						return ((int16_t)($Node::DOCUMENT_POSITION_PRECEDING | $Node::DOCUMENT_POSITION_IMPLEMENTATION_SPECIFIC));
					} else {
						return ((int16_t)($Node::DOCUMENT_POSITION_FOLLOWING | $Node::DOCUMENT_POSITION_IMPLEMENTATION_SPECIFIC));
					}
	...
	case $Node::ATTRIBUTE_NODE:
		{
			...
			if (otherAncestorType == $Node::ATTRIBUTE_NODE) {
				$assign(otherNode, $cast($AttrImpl, otherAncestor)->getOwnerElement());
				if (otherNode == thisNode) {
					if ($$sure($NamedNodeMapImpl, $nc(thisNode)->getAttributes())->precedes(other, this)) {
						return ((int16_t)($Node::DOCUMENT_POSITION_PRECEDING | $Node::DOCUMENT_POSITION_IMPLEMENTATION_SPECIFIC));
					} else {
						return ((int16_t)($Node::DOCUMENT_POSITION_FOLLOWING | $Node::DOCUMENT_POSITION_IMPLEMENTATION_SPECIFIC));
					}
...
bool NodeImpl::isDefaultNamespace($String* namespaceURI) {
	...
	case $Node::DOCUMENT_NODE:
		{
			$var($Element, docElement, $cast($Document, this)->getDocumentElement());
...
$Object* NodeImpl::setUserData($String* key, Object$* data, $UserDataHandler* handler) {
	return $$nc(ownerDocument())->setUserData(this, key, data, handler);
}

$Object* NodeImpl::getUserData($String* key) {
	return $$nc(ownerDocument())->getUserData(this, key);
}
...
$Object* NodeImpl::getUserData() {
	return $$nc(ownerDocument())->getUserData(this);
}
...
bool NodeImpl::isReadOnly() {
	return (this->flags & NodeImpl::READONLY) != 0;
}

void NodeImpl::isReadOnly(bool value) {
	this->flags = (int16_t)(value ? this->flags | NodeImpl::READONLY : this->flags & ~NodeImpl::READONLY);
}

bool NodeImpl::needsSyncData() {
	return (this->flags & NodeImpl::SYNCDATA) != 0;
}

void NodeImpl::needsSyncData(bool value) {
	this->flags = (int16_t)(value ? this->flags | NodeImpl::SYNCDATA : this->flags & ~NodeImpl::SYNCDATA);
}

bool NodeImpl::needsSyncChildren() {
	return (this->flags & NodeImpl::SYNCCHILDREN) != 0;
}

void NodeImpl::needsSyncChildren(bool value) {
	this->flags = (int16_t)(value ? this->flags | NodeImpl::SYNCCHILDREN : this->flags & ~NodeImpl::SYNCCHILDREN);
}

bool NodeImpl::isOwned() {
	return (this->flags & NodeImpl::OWNED) != 0;
}

void NodeImpl::isOwned(bool value) {
	this->flags = (int16_t)(value ? this->flags | NodeImpl::OWNED : this->flags & ~NodeImpl::OWNED);
}

bool NodeImpl::isFirstChild() {
	return (this->flags & NodeImpl::FIRSTCHILD) != 0;
}

void NodeImpl::isFirstChild(bool value) {
	this->flags = (int16_t)(value ? this->flags | NodeImpl::FIRSTCHILD : this->flags & ~NodeImpl::FIRSTCHILD);
}

bool NodeImpl::isSpecified() {
	return (this->flags & NodeImpl::SPECIFIED) != 0;
}

void NodeImpl::isSpecified(bool value) {
	this->flags = (int16_t)(value ? this->flags | NodeImpl::SPECIFIED : this->flags & ~NodeImpl::SPECIFIED);
}

bool NodeImpl::internalIsIgnorableWhitespace() {
	return (this->flags & NodeImpl::IGNORABLEWS) != 0;
}

void NodeImpl::isIgnorableWhitespace(bool value) {
	this->flags = (int16_t)(value ? this->flags | NodeImpl::IGNORABLEWS : this->flags & ~NodeImpl::IGNORABLEWS);
}

bool NodeImpl::hasStringValue() {
	return (this->flags & NodeImpl::HASSTRING) != 0;
}

void NodeImpl::hasStringValue(bool value) {
	this->flags = (int16_t)(value ? this->flags | NodeImpl::HASSTRING : this->flags & ~NodeImpl::HASSTRING);
}

bool NodeImpl::isNormalized() {
	return (this->flags & NodeImpl::NORMALIZED) != 0;
}
...
void NodeImpl::isNormalized(bool value) {
	...
	this->flags = (int16_t)(value ? this->flags | NodeImpl::NORMALIZED : this->flags & ~NodeImpl::NORMALIZED);
}

bool NodeImpl::isIdAttribute() {
	return (this->flags & NodeImpl::ID) != 0;
}

void NodeImpl::isIdAttribute(bool value) {
	this->flags = (int16_t)(value ? this->flags | NodeImpl::ID : this->flags & ~NodeImpl::ID);
}
...
$String* NodeImpl::toString() {
	...
	var$0->append("["_s);
	var$0->append($(getNodeName()));
	var$0->append(": "_s);
	var$0->append($(getNodeValue()));
	var$0->append("]"_s);
	return $str(var$0);
}
...
$Class* NodeImpl::load$($String* name, bool initialize) {
	...
	$CompoundAttribute compareTreePositionmethodAnnotations$$[] = {
	...
	$MethodInfo methodInfos$$[] = {
		...
		{"compareTreePosition", "(Lorg/w3c/dom/Node;)S", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(NodeImpl, compareTreePosition, int16_t, $Node*), nullptr, nullptr, compareTreePositionmethodAnnotations$$},
