...
void SchemaNamespaceSupport::setEffectiveContext($StringArray* namespaceDecls) {
	if (namespaceDecls == nullptr || namespaceDecls->length == 0) {
		return;
	}
	pushContext();
...
$Class* SchemaNamespaceSupport::load$($String* name, bool initialize) {
	...
	$loadClass(SchemaNamespaceSupport, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SchemaNamespaceSupport);
	});
	return class$;
}
