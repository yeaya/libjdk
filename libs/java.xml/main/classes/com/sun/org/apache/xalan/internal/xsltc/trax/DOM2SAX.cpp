...
void DOM2SAX::parse() {
	if (this->_dom != nullptr) {
		bool isIncomplete = (this->_dom->getNodeType() != $Node::DOCUMENT_NODE);
		if (isIncomplete) {
...
void DOM2SAX::parse($Node* node) {
	...
	}
	{
		...
		case $Node::PROCESSING_INSTRUCTION_NODE:
			{
				$var($String, var$6, node->getNodeName());
				$nc(this->_sax)->processingInstruction(var$6, $(node->getNodeValue()));
				break;
			}
		case $Node::TEXT_NODE:
			{
				$assign(data, node->getNodeValue());
				$var($chars, var$7, $nc(data)->toCharArray());
				$nc(this->_sax)->characters(var$7, 0, data->length());
				break;
			}
		}
	}
...
void DOM2SAX::clinit$($Class* clazz) {
...
$Class* DOM2SAX::load$($String* name, bool initialize) {
	...
	$loadClass(DOM2SAX, name, initialize, &classInfo$$, DOM2SAX::clinit$, []($Class* clazz) -> $Object* {
