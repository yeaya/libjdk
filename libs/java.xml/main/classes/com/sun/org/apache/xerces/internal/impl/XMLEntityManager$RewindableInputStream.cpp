...
int32_t XMLEntityManager$RewindableInputStream::readAndBuffer() {
	...
	this->fData->set(this->fLength++, (int8_t)b);
	++this->fOffset;
	return b & 0xff;
}
...
int32_t XMLEntityManager$RewindableInputStream::read() {
	if (this->fOffset < this->fLength) {
		return $nc(this->fData)->get(this->fOffset++) & 0xff;
	}
...
int32_t XMLEntityManager$RewindableInputStream::read($bytes* b, int32_t off, int32_t len) {
	...
	if (bytesLeft == 0) {
		...
		if ($nc(this->this$0->fCurrentEntity)->mayReadChunks || !this->this$0->fCurrentEntity->xmlDeclChunkRead) {
...
$Class* XMLEntityManager$RewindableInputStream::load$($String* name, bool initialize) {
	...
	$loadClass(XMLEntityManager$RewindableInputStream, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XMLEntityManager$RewindableInputStream);
	});
	return class$;
}
