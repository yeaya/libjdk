...
void Output::mergeOutput(Output* previous) {
	...
	if ($nc(previous)->hasAttribute("cdata-section-elements"_s)) {
		...
		var$0->append($(previous->getAttribute("cdata-section-elements"_s)));
		var$0->append(u' ');
		var$0->append($(getAttribute("cdata-section-elements"_s)));
		addAttribute("cdata-section-elements"_s, $$str(var$0));
	}
...
void Output::parseContents($Parser* parser) {
	...
	if (this->_method != nullptr) {
		$set(this, _method, this->_method->toLowerCase());
		bool var$2 = this->_method->equals("xml"_s);
		bool var$1 = var$2 || (this->_method->equals("html"_s));
		bool var$0 = var$1 || (this->_method->equals("text"_s));
		if (!var$0) {
			bool var$3 = $XML11Char::isXML11ValidQName(this->_method);
			var$0 = var$3 && (this->_method->indexOf(":"_s) > 0);
		}
		if (var$0) {
			$init($OutputKeys);
	...
	if ($nc(this->_encoding)->equals($Constants::EMPTYSTRING)) {
		...
		try {
			...
			$var($OutputStreamWriter, writer, $new($OutputStreamWriter, $System::out, canonicalEncoding));
		} catch ($UnsupportedEncodingException& e) {
			$init($ErrorMsg);
			$var($ErrorMsg, msg, $new($ErrorMsg, $ErrorMsg::UNSUPPORTED_ENCODING, this->_encoding, this));
	...
	if ($nc(this->_cdata)->equals($Constants::EMPTYSTRING)) {
		...
		while (tokens->hasMoreTokens()) {
			...
			if (!$XML11Char::isXML11ValidQName(qname)) {
				$init($ErrorMsg);
				$var($ErrorMsg, err, $new($ErrorMsg, $ErrorMsg::INVALID_QNAME_ERR, qname, this));
	...
	} else if (this->_method != nullptr && this->_method->equals("html"_s)) {
		this->_indent = true;
	}
	...
	if (this->_method != nullptr) {
		if (this->_method->equals("html"_s)) {
		...
		} else if (this->_method->equals("text"_s)) {
			if (this->_mediaType == nullptr) {
...
void Output::translate($ClassGenerator* classGen, $MethodGenerator* methodGen) {
	...
	if ((this->_version != nullptr) && (!this->_version->equals(Output::XML_VERSION))) {
		$init($Constants);
	...
	if (this->_indentamount != nullptr && !this->_indentamount->equals($Constants::EMPTYSTRING)) {
...
void Output::clinit$($Class* clazz) {
...
$Class* Output::load$($String* name, bool initialize) {
	...
	$loadClass(Output, name, initialize, &classInfo$$, Output::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Output);
	});
	return class$;
}
