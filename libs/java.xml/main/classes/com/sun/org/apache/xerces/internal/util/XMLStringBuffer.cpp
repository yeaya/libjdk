...
void XMLStringBuffer::append(char16_t c) {
	if (this->length + 1 > $nc(this->ch)->length) {
		int32_t newLength = this->ch->length * 2;
		if (newLength < this->ch->length + XMLStringBuffer::DEFAULT_SIZE) {
			newLength = this->ch->length + XMLStringBuffer::DEFAULT_SIZE;
		}
		$var($chars, tmp, $new($chars, newLength));
		$System::arraycopy(this->ch, 0, tmp, 0, this->length);
		$set(this, ch, tmp);
	}
	this->ch->set(this->length, c);
	++this->length;
}
...
void XMLStringBuffer::append($String* s) {
	...
	if (this->length + length > $nc(this->ch)->length) {
		int32_t newLength = this->ch->length * 2;
		if (newLength < this->ch->length + length + XMLStringBuffer::DEFAULT_SIZE) {
			newLength = this->ch->length + length + XMLStringBuffer::DEFAULT_SIZE;
		}
...
void XMLStringBuffer::append($XMLString* s) {
	append($nc(s)->ch, $nc(s)->offset, $nc(s)->length);
}
...
$Class* XMLStringBuffer::load$($String* name, bool initialize) {
	...
	$loadClass(XMLStringBuffer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XMLStringBuffer);
	});
	return class$;
}
