...
void ConstantHTML::init$($String* dir, $String* class_name, $String* class_package, $MethodArray* methods, $ConstantPool* constant_pool) {
	...
	$set(this, file, $new($PrintWriter, $$new($FileOutputStream, $$str({dir, class_name, "_cp.html"_s}))));
	$set(this, constantRef, $new($StringArray, $nc(this->constants)->length));
	this->constantRef->set(0, "&lt;unknown&gt;"_s);
	this->file->println("<HTML><BODY BGCOLOR=\"#C0C0C0\"><TABLE BORDER=0>"_s);
	for (int32_t i = 1; i < this->constants->length; ++i) {
		if (i % 2 == 0) {
...
$String* ConstantHTML::referenceConstant(int32_t index) {
	return this->constantRef->get(index);
}
...
void ConstantHTML::writeConstant(int32_t index) {
	...
	this->file->println($$str({"<H4> <A NAME=cp"_s, $$str(index), ">"_s, $$str(index), "</A> "_s, $($Const::getConstantName(tag)), "</H4>"_s}));
	{
		...
		case $Const::CONSTANT_Methodref:
			...
			$assign(method_class, this->constantPool->constantToString(class_index, $Const::CONSTANT_Class));
			...
			$assign(c2, $cast($ConstantNameAndType, this->constantPool->getConstant(name_index, $Const::CONSTANT_NameAndType)));
			$assign(signature, this->constantPool->constantToString($nc(c2)->getSignatureIndex(), $Const::CONSTANT_Utf8));
			...
			this->constantRef->set(index, $$str({ret_type, "&nbsp;<A HREF=\""_s, this->className, "_cp.html#cp"_s, $$str(class_index), "\" TARGET=Constants>"_s, short_method_class, "</A>.<A HREF=\""_s, this->className, "_cp.html#cp"_s, $$str(index), "\" TARGET=ConstantPool>"_s, html_method_name, "</A>&nbsp;"_s, arg_types}));
			this->file->println($$str({"<P><TT>"_s, ret_type, "&nbsp;"_s, ref, arg_types, "&nbsp;</TT>\n<UL><LI><A HREF=\"#cp"_s, $$str(class_index), "\">Class index("_s, $$str(class_index), ")</A>\n<LI><A HREF=\"#cp"_s, $$str(name_index), "\">NameAndType index("_s, $$str(name_index), ")</A></UL>"_s}));
			break;
		case $Const::CONSTANT_Fieldref:
			...
			$assign(field_class, this->constantPool->constantToString(class_index, $Const::CONSTANT_Class));
			...
			$assign(field_name, this->constantPool->constantToString(name_index, $Const::CONSTANT_NameAndType));
			...
			this->constantRef->set(index, $$str({"<A HREF=\""_s, this->className, "_cp.html#cp"_s, $$str(class_index), "\" TARGET=Constants>"_s, short_field_class, "</A>.<A HREF=\""_s, this->className, "_cp.html#cp"_s, $$str(index), "\" TARGET=ConstantPool>"_s, field_name, "</A>"_s}));
			this->file->println($$str({"<P><TT>"_s, ref, "</TT><BR>\n<UL><LI><A HREF=\"#cp"_s, $$str(class_index), "\">Class("_s, $$str(class_index), ")</A><BR>\n<LI><A HREF=\"#cp"_s, $$str(name_index), "\">NameAndType("_s, $$str(name_index), ")</A></UL>"_s}));
			break;
		case $Const::CONSTANT_Class:
			...
			$assign(class_name2, this->constantPool->constantToString(index, tag));
			...
			this->constantRef->set(index, $$str({"<A HREF=\""_s, this->className, "_cp.html#cp"_s, $$str(index), "\" TARGET=ConstantPool>"_s, short_class_name, "</A>"_s}));
			this->file->println($$str({"<P><TT>"_s, ref, "</TT><UL><LI><A HREF=\"#cp"_s, $$str(name_index), "\">Name index("_s, $$str(name_index), ")</A></UL>\n"_s}));
			break;
		case $Const::CONSTANT_String:
			...
			$assign(str, $Class2HTML::toHTML($(this->constantPool->constantToString(index, tag))));
			this->file->println($$str({"<P><TT>"_s, str, "</TT><UL><LI><A HREF=\"#cp"_s, $$str(name_index), "\">Name index("_s, $$str(name_index), ")</A></UL>\n"_s}));
			break;
		case $Const::CONSTANT_NameAndType:
			...
			signature_index = c6->getSignatureIndex();
			this->file->println($$str({"<P><TT>"_s, $($Class2HTML::toHTML($(this->constantPool->constantToString(index, tag)))), "</TT><UL><LI><A HREF=\"#cp"_s, $$str(name_index), "\">Name index("_s, $$str(name_index), ")</A>\n<LI><A HREF=\"#cp"_s, $$str(signature_index), "\">Signature index("_s, $$str(signature_index), ")</A></UL>\n"_s}));
			break;
		default:
			this->file->println($$str({"<P><TT>"_s, $($Class2HTML::toHTML($($nc(this->constantPool)->constantToString(index, tag)))), "</TT>\n"_s}));
		}
	}
}
...
int32_t ConstantHTML::getMethodNumber($String* str) {
	...
	for (int32_t i = 0; i < $nc(this->methods)->length; ++i) {
		...
		var$0->append($($nc(this->methods->get(i))->getName()));
		var$0->append($($nc(this->methods->get(i))->getSignature()));
		$var($String, cmp, $str(var$0));
		if (cmp->equals(str)) {
			return i;
		}
	}
...
$Class* ConstantHTML::load$($String* name, bool initialize) {
	...
	$loadClass(ConstantHTML, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ConstantHTML);
	});
	return class$;
}
