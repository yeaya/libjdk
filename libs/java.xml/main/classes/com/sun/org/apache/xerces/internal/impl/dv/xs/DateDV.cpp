...
$XMLGregorianCalendar* DateDV::getXMLGregorianCalendar($AbstractDateTimeDV$DateTimeData* date) {
	...
	return $nc($AbstractDateTimeDV::datatypeFactory)->newXMLGregorianCalendar($nc(date)->unNormYear, $nc(date)->unNormMonth, $nc(date)->unNormDay, $DatatypeConstants::FIELD_UNDEFINED, $DatatypeConstants::FIELD_UNDEFINED, $DatatypeConstants::FIELD_UNDEFINED, $DatatypeConstants::FIELD_UNDEFINED, $nc(date)->hasTimeZone() ? (date->timezoneHr * 60 + date->timezoneMin) : $DatatypeConstants::FIELD_UNDEFINED);
}
...
$Class* DateDV::load$($String* name, bool initialize) {
	...
	$loadClass(DateDV, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DateDV);
	});
	return class$;
}
