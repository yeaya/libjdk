...
Attribute* Attribute::readAttribute($DataInput* file, $ConstantPool* constant_pool) {
	...
	for (int8_t i = 0; i < $Const::KNOWN_ATTRIBUTES; ++i) {
	...
	}
	{
		...
		case $Const::ATTR_UNKNOWN:
			$assign(r, Attribute::readers->get(name));
			if ($instanceOf($UnknownAttributeReader, r)) {
				return $cast($UnknownAttributeReader, r)->createAttribute(name_index, length, file, constant_pool);
			}
...
Attribute* Attribute::readAttribute($DataInputStream* file, $ConstantPool* constant_pool) {
	$init(Attribute);
	return readAttribute($cast($DataInput, file), constant_pool);
}
...
void Attribute::clinit$($Class* clazz) {
...
$Class* Attribute::load$($String* name, bool initialize) {
	...
	$loadClass(Attribute, name, initialize, &classInfo$$, Attribute::clinit$, []($Class* clazz) -> $Object* {
