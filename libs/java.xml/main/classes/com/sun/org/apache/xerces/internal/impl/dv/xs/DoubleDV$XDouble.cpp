			...
			if (this->value == $Double::POSITIVE_INFINITY) {
				...
				if (this->canonical->indexOf(u'E') == -1) {
					...
					if (this->value >= 1 || this->value <= -1) {
						...
						chars->set(edp, u'.');
						for (int32_t i = nzp + 1, j = edp + 1; i < len; ++i, ++j) {
...
$Class* DoubleDV$XDouble::load$($String* name, bool initialize) {
	...
	$loadClass(DoubleDV$XDouble, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DoubleDV$XDouble);
	});
	return class$;
}
