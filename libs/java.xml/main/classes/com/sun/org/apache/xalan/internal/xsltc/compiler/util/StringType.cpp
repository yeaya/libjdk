...
void StringType::translateTo($ClassGenerator* classGen, $MethodGenerator* methodGen, $RealType* type) {
	...
	$var($InstructionList, il, $nc(methodGen)->getInstructionList());
	$init($Constants);
	$nc(il)->append($$new($INVOKESTATIC, $nc(cpg)->addMethodref($Constants::BASIS_LIBRARY_CLASS, $Constants::STRING_TO_REAL, $Constants::STRING_TO_REAL_SIG)));
}
...
$FlowList* StringType::translateToDesynthesized($ClassGenerator* classGen, $MethodGenerator* methodGen, $BooleanType* type) {
	...
	$var($InstructionList, il, $nc(methodGen)->getInstructionList());
	$init($Constants);
	$nc(il)->append($$new($INVOKEVIRTUAL, $nc(cpg)->addMethodref($Constants::STRING_CLASS, "length"_s, "()I"_s)));
	return $new($FlowList, $(il->append($$new($IFEQ, nullptr))));
}
...
void StringType::translateFrom($ClassGenerator* classGen, $MethodGenerator* methodGen, $Class* clazz) {
	...
	if ($$nc($nc(clazz)->getName())->equals("java.lang.String"_s)) {
		...
		$var($BranchHandle, ifNonNull, il->append($$new($IFNONNULL, nullptr)));
...
$Class* StringType::load$($String* name, bool initialize) {
	...
	$loadClass(StringType, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(StringType);
	});
	return class$;
}
