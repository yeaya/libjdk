...
void StAXSchemaParser::parse($XMLEventReader* input) {
	...
	if (currentEvent != nullptr) {
		...
		this->fLocationWrapper->setLocation(nullptr);
		this->fNamespaceContext->setNamespaceContext(nullptr);
			...
			case $XMLStreamConstants::CHARACTERS:
				{
					...
					this->fTempString->setValues(var$6, var$7, input->getTextLength());
			...
			case $XMLStreamConstants::SPACE:
				{
					...
					this->fTempString->setValues(var$8, var$9, input->getTextLength());
			...
			case $XMLStreamConstants::CDATA:
				{
					...
					this->fTempString->setValues(var$10, var$11, input->getTextLength());
					$nc(this->fSchemaDOMParser)->characters(this->fTempString, nullptr);
					$nc(this->fSchemaDOMParser)->endCDATA(nullptr);
					break;
				}
...
void StAXSchemaParser::sendCharactersToSchemaParser($String* str, bool whitespace) {
	if (str != nullptr) {
		...
		int32_t remainder = length & StAXSchemaParser::CHUNK_MASK;
		if (remainder > 0) {
		...
		while (i < length) {
			...
			this->fTempString->setValues(this->fCharBuffer, 0, StAXSchemaParser::CHUNK_SIZE);
			if (whitespace) {
				$nc(this->fSchemaDOMParser)->ignorableWhitespace(this->fTempString, nullptr);
			} else {
				$nc(this->fSchemaDOMParser)->characters(this->fTempString, nullptr);
...
void StAXSchemaParser::fillXMLAttributes($StartElement* event) {
	...
	while ($nc(attrs)->hasNext()) {
		...
		$var($String, type, attr->getDTDType());
		int32_t idx = this->fAttributes->getLength();
		$init($XMLSymbols);
		this->fAttributes->addAttributeNS(this->fAttributeQName, (type != nullptr) ? type : $XMLSymbols::fCDATASymbol, $(attr->getValue()));
...
void StAXSchemaParser::fillXMLAttributes($XMLStreamReader* input) {
	...
	for (int32_t i = 0; i < len; ++i) {
		...
		$init($XMLSymbols);
		this->fAttributes->addAttributeNS(this->fAttributeQName, (type != nullptr) ? type : $XMLSymbols::fCDATASymbol, $(input->getAttributeValue(i)));
...
void StAXSchemaParser::addNamespaceDeclarations() {
	...
	$var($Iterator, iter, this->fDeclaredPrefixes->iterator());
	while ($nc(iter)->hasNext()) {
		$assign(nsPrefix, $cast($String, iter->next()));
		$assign(nsURI, this->fNamespaceContext->getURI(nsPrefix));
		...
		this->fAttributeQName->setValues(prefix, localpart, rawname, $1NamespaceContext::XMLNS_URI);
		$init($XMLSymbols);
...
void StAXSchemaParser::fillDeclaredPrefixes($Iterator* namespaces) {
	...
	while ($nc(namespaces)->hasNext()) {
		...
		this->fDeclaredPrefixes->add(prefix != nullptr ? prefix : ""_s);
	}
}
...
void StAXSchemaParser::fillQName($QName* toFill, $1QName* toCopy) {
	$useLocalObjectStack();
	$var($String, var$0, $nc(toCopy)->getNamespaceURI());
	$var($String, var$1, toCopy->getLocalPart());
	fillQName(toFill, var$0, var$1, $(toCopy->getPrefix()));
}
...
void StAXSchemaParser::fillQName($QName* toFill, $String* uri$renamed, $String* localpart$renamed, $String* prefix$renamed) {
	...
	if (prefix != $XMLSymbols::EMPTY_STRING) {
		...
		this->fStringBuffer->append(u':');
		this->fStringBuffer->append(localpart);
		$assign(raw, $nc(this->fSymbolTable)->addSymbol(this->fStringBuffer->ch, this->fStringBuffer->offset, this->fStringBuffer->length));
	}
...
$Class* StAXSchemaParser::load$($String* name, bool initialize) {
	...
	$loadClass(StAXSchemaParser, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(StAXSchemaParser);
	});
	return class$;
}
