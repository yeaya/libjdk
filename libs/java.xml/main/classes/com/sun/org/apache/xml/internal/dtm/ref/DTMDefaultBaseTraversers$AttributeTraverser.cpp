...
$Class* DTMDefaultBaseTraversers$AttributeTraverser::load$($String* name, bool initialize) {
	...
	$loadClass(DTMDefaultBaseTraversers$AttributeTraverser, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DTMDefaultBaseTraversers$AttributeTraverser);
	});
	return class$;
}
