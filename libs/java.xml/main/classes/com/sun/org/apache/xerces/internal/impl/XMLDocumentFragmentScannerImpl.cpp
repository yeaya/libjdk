...
void XMLDocumentFragmentScannerImpl::reset($PropertyManager* propertyManager) {
	...
	this->fNamespaces = $$sure($Boolean, $nc(propertyManager)->getProperty($XMLInputFactory::IS_NAMESPACE_AWARE))->booleanValue();
	this->fNotifyBuiltInRefs = false;
	...
	this->fSupportExternalEntities = $nc(bo)->booleanValue();
	$init($Constants);
...
$StringArray* XMLDocumentFragmentScannerImpl::getRecognizedFeatures() {
	return $cast($StringArray, XMLDocumentFragmentScannerImpl::RECOGNIZED_FEATURES->clone());
}
...
$StringArray* XMLDocumentFragmentScannerImpl::getRecognizedProperties() {
	return $cast($StringArray, XMLDocumentFragmentScannerImpl::RECOGNIZED_PROPERTIES->clone());
}
...
void XMLDocumentFragmentScannerImpl::setProperty($String* propertyId, Object$* value) {
	...
	if (propertyId->equals(XMLDocumentFragmentScannerImpl::XML_SECURITY_PROPERTY_MANAGER)) {
...
$Boolean* XMLDocumentFragmentScannerImpl::getFeatureDefault($String* featureId) {
	for (int32_t i = 0; i < XMLDocumentFragmentScannerImpl::RECOGNIZED_FEATURES->length; ++i) {
		if ($nc(XMLDocumentFragmentScannerImpl::RECOGNIZED_FEATURES->get(i))->equals(featureId)) {
			return XMLDocumentFragmentScannerImpl::FEATURE_DEFAULTS->get(i);
		}
	}
	return nullptr;
}
...
$Object* XMLDocumentFragmentScannerImpl::getPropertyDefault($String* propertyId) {
	for (int32_t i = 0; i < XMLDocumentFragmentScannerImpl::RECOGNIZED_PROPERTIES->length; ++i) {
		if ($nc(XMLDocumentFragmentScannerImpl::RECOGNIZED_PROPERTIES->get(i))->equals(propertyId)) {
			return XMLDocumentFragmentScannerImpl::PROPERTY_DEFAULTS->get(i);
		}
	}
	return nullptr;
}
...
void XMLDocumentFragmentScannerImpl::startEntity($String* name, $XMLResourceIdentifier* identifier, $String* encoding, $Augmentations* augs) {
	...
	if (this->fEntityDepth == $nc(this->fEntityStack)->length) {
		$var($ints, entityarray, $new($ints, this->fEntityStack->length * 2));
		$System::arraycopy(this->fEntityStack, 0, entityarray, 0, this->fEntityStack->length);
		$set(this, fEntityStack, entityarray);
	}
	this->fEntityStack->set(this->fEntityDepth, this->fMarkupDepth);
	...
	if (this->fStandalone && $nc(this->fEntityStore)->isEntityDeclInExternalSubset(name)) {
		reportFatalError("MSG_REFERENCE_TO_EXTERNALLY_DECLARED_ENTITY_WHEN_STANDALONE"_s, $$new($ObjectArray, {name}));
	}
...
void XMLDocumentFragmentScannerImpl::scanXMLDeclOrTextDecl(bool scanningTextDecl) {
	...
	--this->fMarkupDepth;
	$var($String, version, this->fStrings->get(0));
	$var($String, encoding, this->fStrings->get(1));
	$var($String, standalone, this->fStrings->get(2));
	$set(this, fDeclaredEncoding, encoding);
	this->fStandaloneSet = standalone != nullptr;
	this->fStandalone = this->fStandaloneSet && $nc(standalone)->equals("yes"_s);
	$nc(this->fEntityManager)->setStandalone(this->fStandalone);
	if (this->fDocumentHandler != nullptr) {
		if (scanningTextDecl) {
			this->fDocumentHandler->textDecl(version, encoding, nullptr);
		} else {
...
int16_t XMLDocumentFragmentScannerImpl::storePointerForADepth(int16_t elementPointer) {
	...
	for (int16_t i = 0; i < XMLDocumentFragmentScannerImpl::MAX_POINTER_AT_A_DEPTH; ++i) {
		if (canStore(depth, i)) {
...
bool XMLDocumentFragmentScannerImpl::skipElement(int16_t column) {
	...
	for (int16_t i = column; i < XMLDocumentFragmentScannerImpl::MAX_POINTER_AT_A_DEPTH; ++i) {
		...
		if ($nc(this->fElementArray)->get(pointer) != nullptr && skipFromTheBuffer(this->fElementArray->get(pointer))) {
			;
			this->fLastPointerLocation = pointer;
...
bool XMLDocumentFragmentScannerImpl::seekCloseOfStartTag() {
	...
	if (c == u'>') {
		...
		if (!$nc(this->fEntityScanner)->skipChar(u'>', $XMLScanner$NameType::ELEMENTEND)) {
			reportFatalError("ElementUnterminated"_s, $$new($ObjectArray, {$nc(this->fElementQName)->rawname}));
		}
		...
		if (!$nc(this->fEntityScanner)->scanData("]]>"_s, contentBuffer, this->fChunkSize)) {
			...
			if (c != -1 && isInvalidLiteral(c)) {
				if ($XMLChar::isHighSurrogate(c)) {
					...
					reportFatalError("InvalidCharInCDSect"_s, $$new($ObjectArray, {$($Integer::toString(c, 16))}));
					$nc(this->fEntityScanner)->scanChar(nullptr);
				}
			} else {
				this->fInCData = true;
...
int32_t XMLDocumentFragmentScannerImpl::scanEndElement() {
	...
	if (!$nc(this->fEntityScanner)->skipString(endElementName->rawname)) {
		reportFatalError("ETagRequired"_s, $$new($ObjectArray, {rawname}));
	}
	$nc(this->fEntityScanner)->skipSpaces();
	$init($XMLScanner$NameType);
	if (!$nc(this->fEntityScanner)->skipChar(u'>', $XMLScanner$NameType::ELEMENTEND)) {
		reportFatalError("ETagUnterminated"_s, $$new($ObjectArray, {rawname}));
	}
	--this->fMarkupDepth;
	--this->fMarkupDepth;
	if (this->fMarkupDepth < $nc(this->fEntityStack)->get(this->fEntityDepth - 1)) {
		reportFatalError("ElementEntityMismatch"_s, $$new($ObjectArray, {rawname}));
	}
	if (this->fDocumentHandler != nullptr) {
		this->fDocumentHandler->endElement(endElementName, nullptr);
	}
	if (this->dtdGrammarUtil != nullptr) {
	...
	if (ch != -1) {
		if (this->fDocumentHandler != nullptr) {
			if (this->fNotifyCharRefs) {
				this->fDocumentHandler->startGeneralEntity(this->fCharRefLiteral, nullptr, nullptr, nullptr);
			}
...
void XMLDocumentFragmentScannerImpl::scanEntityReference($XMLStringBuffer* content) {
	...
	if (!$nc(this->fEntityScanner)->skipChar(u';', $XMLScanner$NameType::REFERENCE)) {
		reportFatalError("SemicolonRequiredInReference"_s, $$new($ObjectArray, {name}));
	}
	if ($nc(this->fEntityStore)->isUnparsedEntity(name)) {
		reportFatalError("ReferenceToUnparsedEntity"_s, $$new($ObjectArray, {name}));
	}
	--this->fMarkupDepth;
	...
	if (!$nc(this->fEntityStore)->isDeclaredEntity(name)) {
		if (!this->fSupportDTD && this->fReplaceEntityReferences) {
			reportFatalError("EntityNotDeclared"_s, $$new($ObjectArray, {name}));
			return;
		}
		if (this->fHasExternalDTD && !this->fStandalone) {
			if (this->fValidation) {
				$init($XMLMessageFormatter);
				$nc(this->fErrorReporter)->reportError(this->fEntityScanner, $XMLMessageFormatter::XML_DOMAIN, "EntityNotDeclared"_s, $$new($ObjectArray, {name}), $XMLErrorReporter::SEVERITY_ERROR);
			}
		} else {
			reportFatalError("EntityNotDeclared"_s, $$new($ObjectArray, {name}));
		}
...
void XMLDocumentFragmentScannerImpl::checkDepth($String* elementName) {
	...
	if ($nc(this->fSecurityManager)->isOverLimit($XMLSecurityManager$Limit::MAX_ELEMENT_DEPTH_LIMIT, this->fLimitAnalyzer)) {
		...
		reportFatalError("MaxElementDepthLimit"_s, $$new($ObjectArray, {
			elementName,
			$($Integer::valueOf($nc(this->fLimitAnalyzer)->getTotalValue($XMLSecurityManager$Limit::MAX_ELEMENT_DEPTH_LIMIT))),
			$($Integer::valueOf($nc(this->fSecurityManager)->getLimit($XMLSecurityManager$Limit::MAX_ELEMENT_DEPTH_LIMIT))),
			"maxElementDepth"_s
		}));
	}
}
...
void XMLDocumentFragmentScannerImpl::handleCharacter(char16_t c, $String* entity, $XMLStringBuffer* content) {
	...
	$nc(content)->append(c);
	if (this->fDocumentHandler != nullptr) {
		this->fSingleChar->set(0, c);
		if (this->fNotifyBuiltInRefs) {
			this->fDocumentHandler->startGeneralEntity(entity, nullptr, nullptr, nullptr);
		}
...
void XMLDocumentFragmentScannerImpl::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$init($Constants);
...
$Class* XMLDocumentFragmentScannerImpl::load$($String* name, bool initialize) {
	...
	$loadClass(XMLDocumentFragmentScannerImpl, name, initialize, &classInfo$$, XMLDocumentFragmentScannerImpl::clinit$, []($Class* clazz) -> $Object* {
