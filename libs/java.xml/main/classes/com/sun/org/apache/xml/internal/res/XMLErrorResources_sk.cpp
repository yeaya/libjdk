...
void XMLErrorResources_sk::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(XMLErrorResources_sk::_contents, $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_FUNCTION_NOT_SUPPORTED,
			u"Funkcia nie je podporovaná!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_CANNOT_OVERWRITE_CAUSE,
			u"Nie je možné prepísať príčinu"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_NO_DEFAULT_IMPL,
			u"Nebola nájdená žiadna predvolená implementácia "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_CHUNKEDINTARRAY_NOT_SUPPORTED,
			u"ChunkedIntArray({0}) nie je momentálne podporovaný"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_OFFSET_BIGGER_THAN_SLOT,
			u"Offset väčší, než zásuvka"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_COROUTINE_NOT_AVAIL,
			u"Korutina nie je dostupná, id={0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_COROUTINE_CO_EXIT,
			u"CoroutineManager obdržal požiadavku co_exit()"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_COJOINROUTINESET_FAILED,
			"zlyhal co_joinCoroutineSet()"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_COROUTINE_PARAM,
			"Chyba parametra korutiny ({0})"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_PARSER_DOTERMINATE_ANSWERS,
			u"\nNEOČAKÁVANÉ: Analyzátor doTerminate odpovedá {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_NO_PARSE_CALL_WHILE_PARSING,
			u"syntaktický analyzátor nemôže byť volaný počas vykonávania analýzy"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_TYPED_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"Chyba: napísaný iterátor pre os {0} nie je implementovaný"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"Chyba: iterátor pre os {0} nie je implementovaný "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_ITERATOR_CLONE_NOT_SUPPORTED,
			u"Klon iterátora nie je podporovaný"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_UNKNOWN_AXIS_TYPE,
			u"Neznámy typ pretínania osí: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_AXIS_NOT_SUPPORTED,
			u"Pretínanie osí nie je podporované: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_NO_DTMIDS_AVAIL,
			u"Žiadne ďalšie DTM ID nie sú dostupné"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_NOT_SUPPORTED,
			u"Nie je podporované: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_NODE_NON_NULL,
			u"Pre getDTMHandleFromNode musí byť uzol nenulový"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_COULD_NOT_RESOLVE_NODE,
			u"Nebolo možné určiť uzol na spracovanie"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_STARTPARSE_WHILE_PARSING,
			u"startParse nemôže byť volaný počas vykonávania analýzy"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_STARTPARSE_NEEDS_SAXPARSER,
			u"startParse potrebuje nenulový SAXParser"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_COULD_NOT_INIT_PARSER,
			u"nebolo možné inicializovať syntaktický analyzátor pomocou"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_EXCEPTION_CREATING_POOL,
			u"výnimka vytvárania novej inštancie oblasti"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_PATH_CONTAINS_INVALID_ESCAPE_SEQUENCE,
			u"Cesta obsahuje neplatnú únikovú sekvenciu"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_SCHEME_REQUIRED,
			u"Je požadovaná schéma!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_NO_SCHEME_IN_URI,
			u"V URI sa nenašla žiadna schéma: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_NO_SCHEME_INURI,
			u"V URI nebola nájdená žiadna schéma"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_PATH_INVALID_CHAR,
			u"Cesta obsahuje neplatný znak: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_SCHEME_FROM_NULL_STRING,
			u"Nie je možné stanoviť schému z nulového reťazca"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_SCHEME_NOT_CONFORMANT,
			u"Nezhodná schéma."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_HOST_ADDRESS_NOT_WELLFORMED,
			u"Hostiteľ nie je správne formátovaná adresa"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_PORT_WHEN_HOST_NULL,
			u"Nemôže byť stanovený port, ak je hostiteľ null"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_INVALID_PORT,
			u"Neplatné číslo portu"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_FRAG_FOR_GENERIC_URI,
			u"Fragment môže byť stanovený len pre všeobecné URI"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_FRAG_WHEN_PATH_NULL,
			u"Ak je cesta nulová, nemôže byť stanovený fragment"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_FRAG_INVALID_CHAR,
			u"Fragment obsahuje neplatný znak"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_PARSER_IN_USE,
			u"Syntaktický analyzátor je už používaný"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_CANNOT_CHANGE_WHILE_PARSING,
			u"Nie je možné zmeniť {0} {1} počas vykonávania analýzy"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_SELF_CAUSATION_NOT_PERMITTED,
			u"Samozapríčinenie nie je povolené"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_NO_USERINFO_IF_NO_HOST,
			u"Ak nebol zadaný hostiteľ, možno nebolo zadané userinfo"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_NO_PORT_IF_NO_HOST,
			u"Ak nebol zadaný hostiteľ, možno nebol zadaný port"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_NO_QUERY_STRING_IN_PATH,
			u"Reťazec dotazu nemôže byť zadaný v ceste a reťazci dotazu"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_NO_FRAGMENT_STRING_IN_PATH,
			u"Fragment nemôže byť zadaný v ceste, ani vo fragmente"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_CANNOT_INIT_URI_EMPTY_PARMS,
			u"Nie je možné inicializovať URI s prázdnymi parametrami"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_METHOD_NOT_SUPPORTED,
			u"Metóda ešte nie je podporovaná "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_INCRSAXSRCFILTER_NOT_RESTARTABLE,
			u"IncrementalSAXSource_Filter nie je momentálne reštartovateľný"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_XMLRDR_NOT_BEFORE_STARTPARSE,
			u"XMLReader nepredchádza požiadavke na startParse"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_AXIS_TRAVERSER_NOT_SUPPORTED,
			u"Pretínanie osí nie je podporované: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_ERRORHANDLER_CREATED_WITH_NULL_PRINTWRITER,
			u"ListingErrorHandler vytvorený s nulovým PrintWriter!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_SYSTEMID_UNKNOWN,
			u"Neznáme SystemId"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_LOCATION_UNKNOWN,
			u"Neznáme miesto výskytu chyby"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_PREFIX_MUST_RESOLVE,
			u"Predpona sa musí rozlíšiť do názvového priestoru: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_CREATEDOCUMENT_NOT_SUPPORTED,
			u"createDocument() nie je podporované XPathContext!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_CHILD_HAS_NO_OWNER_DOCUMENT,
			u"Potomok atribútu nemá dokument vlastníka!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_CHILD_HAS_NO_OWNER_DOCUMENT_ELEMENT,
			u"Potomok atribútu nemá súčasť dokumentu vlastníka!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_CANT_OUTPUT_TEXT_BEFORE_DOC,
			u"Upozornenie: nemožno vypustiť text pred elementom dokumentu!  Ignorovanie..."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_CANT_HAVE_MORE_THAN_ONE_ROOT,
			u"Nie je možné mať viac, než jeden koreň DOM!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_ARG_LOCALNAME_NULL,
			"Argument \'localName\' je null"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_ARG_LOCALNAME_INVALID,
			u"Lokálny názov v QNAME by mal byť platným NCName"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_ARG_PREFIX_INVALID,
			u"Predpona v QNAME by mala byť platným NCName"_s
		}),
		$$new($ObjectArray, {
			"BAD_CODE"_s,
			u"Parameter na createMessage bol mimo ohraničenia"_s
		}),
		$$new($ObjectArray, {
			"FORMAT_FAILED"_s,
			u"Výnimka počas volania messageFormat"_s
		}),
		$$new($ObjectArray, {
			"line"_s,
			"Riadok #"_s
		...
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_SERIALIZER_NOT_CONTENTHANDLER,
			u"Trieda serializátora \'\'{0}\'\' neimplementuje org.xml.sax.ContentHandler."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_RESOURCE_COULD_NOT_FIND,
			u"Prostriedok [ {0} ] nemohol byť nájdený.\n {1}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_RESOURCE_COULD_NOT_LOAD,
			u"Prostriedok [ {0} ] sa nedal načítať: {1} \n {2} \t {3}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_BUFFER_SIZE_LESSTHAN_ZERO,
			u"Veľkosť vyrovnávacej pamäte <=0"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_INVALID_UTF16_SURROGATE,
			u"Bolo zistené neplatné nahradenie UTF-16: {0} ?"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_OIERROR,
			"chyba IO"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_ILLEGAL_ATTRIBUTE_POSITION,
			u"Nie je možné pridať atribút {0} po uzloch potomka alebo pred vytvorením elementu.  Atribút bude ignorovaný."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_NAMESPACE_PREFIX,
			u"Názvový priestor pre predponu \'\'{0}\'\' nebol deklarovaný."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_STRAY_ATTRIBUTE,
			u"Atribút \'\'{0}\'\' je mimo elementu."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_STRAY_NAMESPACE,
			u"Deklarácia názvového priestoru \'\'{0}\'\'=\'\'{1}\'\' je mimo elementu."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_COULD_NOT_LOAD_RESOURCE,
			u"Nedalo sa načítať \'\'{0}\'\' (skontrolujte CLASSPATH), používajú sa predvolené hodnoty"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_sk::ER_COULD_NOT_LOAD_METHOD_PROPERTY,
			u"Nedal sa načítať súbor vlastností \'\'{0}\'\' pre výstupnú metódu \'\'{1}\'\' (skontrolujte CLASSPATH)"_s
		})
	}));
}
...
$Class* XMLErrorResources_sk::load$($String* name, bool initialize) {
	...
	$loadClass(XMLErrorResources_sk, name, initialize, &classInfo$$, XMLErrorResources_sk::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLErrorResources_sk);
	});
	return class$;
}
