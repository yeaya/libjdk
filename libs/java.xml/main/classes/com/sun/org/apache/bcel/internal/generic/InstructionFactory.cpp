...
$InvokeInstruction* InstructionFactory::createInvoke($InstructionFactory$MethodObject* m, int16_t kind) {
	return createInvoke($nc(m)->class_name, $nc(m)->name, $nc(m)->result_type, $nc(m)->arg_types, kind);
}

bool InstructionFactory::isString($Type* type) {
	$init(InstructionFactory);
	return ($instanceOf($ObjectType, type)) && $$nc($cast($ObjectType, type)->getClassName())->equals("java.lang.String"_s);
}

$Instruction* InstructionFactory::createAppend($Type* type) {
	int8_t t = $nc(type)->getType();
	if (isString(type)) {
		return createInvoke(InstructionFactory::append_mos->get(0), $Const::INVOKEVIRTUAL);
	}
	switch (t) {
	...
	case $Const::T_INT:
	case $Const::T_LONG:
		return createInvoke(InstructionFactory::append_mos->get(t), $Const::INVOKEVIRTUAL);
	case $Const::T_ARRAY:
	case $Const::T_OBJECT:
		return createInvoke(InstructionFactory::append_mos->get(1), $Const::INVOKEVIRTUAL);
	default:
...
$Instruction* InstructionFactory::createCast($Type* src_type, $Type* dest_type) {
	...
	if (($instanceOf($BasicType, src_type)) && ($instanceOf($BasicType, dest_type))) {
		int8_t dest = dest_type->getType();
		int8_t src = src_type->getType();
		if (dest == $Const::T_LONG && (src == $Const::T_CHAR || src == $Const::T_BYTE || src == $Const::T_SHORT)) {
			src = $Const::T_INT;
		}
		$var($String, name, $str({"com.sun.org.apache.bcel.internal.generic."_s, InstructionFactory::short_names->get(src - $Const::T_CHAR), "2"_s, InstructionFactory::short_names->get(dest - $Const::T_CHAR)}));
		...
		return $new($CHECKCAST, $nc(this->cp)->addClass($($cast($ObjectType, dest_type)->getClassName())));
	} else {
...
void InstructionFactory::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(InstructionFactory::append_mos, $new($InstructionFactory$MethodObjectArray, {
		$$new($InstructionFactory$MethodObject, "java.lang.StringBuffer"_s, "append"_s, $Type::STRINGBUFFER, $$new($TypeArray, {$Type::STRING})),
		$$new($InstructionFactory$MethodObject, "java.lang.StringBuffer"_s, "append"_s, $Type::STRINGBUFFER, $$new($TypeArray, {$Type::OBJECT})),
		nullptr,
		nullptr,
		$$new($InstructionFactory$MethodObject, "java.lang.StringBuffer"_s, "append"_s, $Type::STRINGBUFFER, $$new($TypeArray, {$Type::BOOLEAN})),
		$$new($InstructionFactory$MethodObject, "java.lang.StringBuffer"_s, "append"_s, $Type::STRINGBUFFER, $$new($TypeArray, {$Type::CHAR})),
		$$new($InstructionFactory$MethodObject, "java.lang.StringBuffer"_s, "append"_s, $Type::STRINGBUFFER, $$new($TypeArray, {$Type::FLOAT})),
		$$new($InstructionFactory$MethodObject, "java.lang.StringBuffer"_s, "append"_s, $Type::STRINGBUFFER, $$new($TypeArray, {$Type::DOUBLE})),
		$$new($InstructionFactory$MethodObject, "java.lang.StringBuffer"_s, "append"_s, $Type::STRINGBUFFER, $$new($TypeArray, {$Type::INT})),
		$$new($InstructionFactory$MethodObject, "java.lang.StringBuffer"_s, "append"_s, $Type::STRINGBUFFER, $$new($TypeArray, {$Type::INT})),
		$$new($InstructionFactory$MethodObject, "java.lang.StringBuffer"_s, "append"_s, $Type::STRINGBUFFER, $$new($TypeArray, {$Type::INT})),
		$$new($InstructionFactory$MethodObject, "java.lang.StringBuffer"_s, "append"_s, $Type::STRINGBUFFER, $$new($TypeArray, {$Type::LONG}))
	}));
}
...
$Class* InstructionFactory::load$($String* name, bool initialize) {
	...
	$loadClass(InstructionFactory, name, initialize, &classInfo$$, InstructionFactory::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(InstructionFactory);
	});
	return class$;
}
