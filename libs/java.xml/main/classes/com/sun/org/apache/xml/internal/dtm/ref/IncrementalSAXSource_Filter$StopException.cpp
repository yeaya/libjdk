...
$Class* IncrementalSAXSource_Filter$StopException::load$($String* name, bool initialize) {
	...
	$loadClass(IncrementalSAXSource_Filter$StopException, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(IncrementalSAXSource_Filter$StopException);
	});
	return class$;
}
