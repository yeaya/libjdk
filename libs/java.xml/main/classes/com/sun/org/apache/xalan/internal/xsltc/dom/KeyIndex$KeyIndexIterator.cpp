...
$IntegerArray* KeyIndex$KeyIndexIterator::lookupNodes(int32_t root, $String* keyValue) {
	...
	if (!this->_isKeyIterator) {
		...
		while (values->hasMoreElements()) {
			...
			if (nodes == nullptr && this->this$0->_enhancedDOM != nullptr && this->this$0->_enhancedDOM->hasDOMSource()) {
...
$DTMAxisIterator* KeyIndex$KeyIndexIterator::setStartNode(int32_t node) {
	...
	if (this->_keyValueIterator != nullptr) {
		$set(this, _keyValueIterator, this->_keyValueIterator->setStartNode(node));
	}
	init();
...
void KeyIndex$KeyIndexIterator::init() {
	...
	if (this->_keyValueIterator == nullptr) {
		...
		$var($DTMAxisIterator, keyValues, this->_keyValueIterator->reset());
...
int32_t KeyIndex$KeyIndexIterator::getLast() {
	return (this->_nodes != nullptr) ? this->_nodes->cardinality() : $MultiValuedNodeHeapIterator::getLast();
}
...
$Class* KeyIndex$KeyIndexIterator::load$($String* name, bool initialize) {
	...
	$loadClass(KeyIndex$KeyIndexIterator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(KeyIndex$KeyIndexIterator);
	});
	return class$;
}
