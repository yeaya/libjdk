...
bool XPointerHandler$Scanner::scanExpr($SymbolTable* symbolTable, $XPointerHandler$Tokens* tokens, $String* data, int32_t currentOffset, int32_t endOffset) {
	...
	while (true) {
		...
		int8_t chartype = (ch >= 128) ? XPointerHandler$Scanner::CHARTYPE_NONASCII : this->fASCIICharMap->get(ch);
		switch (chartype) {
		...
		case XPointerHandler$Scanner::CHARTYPE_WHITESPACE:
			if (openParen == 0) {
				...
				if (currentOffset == nameOffset) {
					this->this$0->reportError("InvalidShortHandPointer"_s, $$new($ObjectArray, {data}));
					return false;
				}
				if (currentOffset < endOffset) {
					ch = data->charAt(currentOffset);
				...
				if (currentOffset == dataOffset) {
					this->this$0->reportError("InvalidSchemeDataInXPointer"_s, $$new($ObjectArray, {data}));
					return false;
				}
				if (currentOffset < endOffset) {
					ch = data->charAt(currentOffset);
...
int32_t XPointerHandler$Scanner::scanNCName($String* data, int32_t endOffset, int32_t currentOffset) {
	...
	if (ch >= 128) {
		...
		int8_t chartype = this->fASCIICharMap->get(ch);
...
int32_t XPointerHandler$Scanner::scanData($String* data, $StringBuffer* schemeData, int32_t endOffset, int32_t currentOffset) {
	while (true) {
		...
		if (chartype == XPointerHandler$Scanner::CHARTYPE_OPEN_PAREN) {
			...
			ch = data->charAt(currentOffset);
			chartype = (ch >= 128) ? XPointerHandler$Scanner::CHARTYPE_NONASCII : this->fASCIICharMap->get(ch);
...
$Class* XPointerHandler$Scanner::load$($String* name, bool initialize) {
	...
	$loadClass(XPointerHandler$Scanner, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XPointerHandler$Scanner);
	});
	return class$;
}
