...
void JAXPConstants::clinit$($Class* clazz) {
...
$Class* JAXPConstants::load$($String* name, bool initialize) {
	...
	$loadClass(JAXPConstants, name, initialize, &classInfo$$, JAXPConstants::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(JAXPConstants);
	});
	return class$;
}
