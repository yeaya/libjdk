...
void XPath::setExpression($Expression* exp) {
	if (nullptr != this->m_mainExp) {
		$nc(exp)->exprSetParent($(this->m_mainExp->exprGetParent()));
	}
...
void XPath::init$($String* exprString, $SourceLocator* locator, $PrefixResolver* prefixResolver, int32_t type, $ErrorListener* errorListener$renamed) {
	...
	if (XPath::SELECT == type) {
		...
		$throwNew($RuntimeException, $($XSLMessages::createXPATHMessage($XPATHErrorResources::ER_CANNOT_DEAL_XPATH_TYPE, $$new($ObjectArray, {$($Integer::toString(type))}))));
	}
	...
	try {
		try {
			...
			while ($instanceOf($WrappedRuntimeException, e)) {
				$assign(e, $cast($WrappedRuntimeException, e)->getException());
			}
...
void XPath::error($XPathContext* xctxt, int32_t sourceNode, $String* msg, $ObjectArray* args) {
	...
	if (nullptr != ehandler) {
		...
		var$0->append($($nc(slocator)->getSystemId()));
		var$0->append("; line "_s);
		var$0->append(slocator->getLineNumber());
		var$0->append("; column "_s);
		var$0->append(slocator->getColumnNumber());
...
$Class* XPath::load$($String* name, bool initialize) {
	...
	$loadClass(XPath, name, initialize, &classInfo$$, XPath::clinit$, []($Class* clazz) -> $Object* {
