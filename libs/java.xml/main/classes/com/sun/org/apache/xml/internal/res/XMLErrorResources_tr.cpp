...
void XMLErrorResources_tr::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(XMLErrorResources_tr::_contents, $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_FUNCTION_NOT_SUPPORTED,
			u"İşlev desteklenmiyor!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_CANNOT_OVERWRITE_CAUSE,
			u"Nedenin üzerine yazılamaz"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_NO_DEFAULT_IMPL,
			u"Varsayılan uygulama bulunamadı"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_CHUNKEDINTARRAY_NOT_SUPPORTED,
			u"ChunkedIntArray({0}) şu an desteklenmiyor"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_OFFSET_BIGGER_THAN_SLOT,
			u"Göreli konum yuvadan büyük"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_COROUTINE_NOT_AVAIL,
			u"Coroutine kullanılamıyor, id={0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_COROUTINE_CO_EXIT,
			u"CoroutineManager co_exit() isteği aldı"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_COJOINROUTINESET_FAILED,
			u"co_joinCoroutineSet() başarısız oldu"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_COROUTINE_PARAM,
			u"Coroutine değiştirgesi hatası ({0})"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_PARSER_DOTERMINATE_ANSWERS,
			u"\nBEKLENMEYEN: Parser doTerminate yanıtı {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_NO_PARSE_CALL_WHILE_PARSING,
			u"Ayrıştırma sırasında parse çağrılamaz"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_TYPED_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"Hata: {0} ekseni için tip atanmış yineleyici gerçekleştirilmedi"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_ITERATOR_AXIS_NOT_IMPLEMENTED,
			u"Hata: {0} ekseni için yineleyici gerçekleştirilmedi"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_ITERATOR_CLONE_NOT_SUPPORTED,
			u"Yineleyici eşkopyası desteklenmiyor"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_UNKNOWN_AXIS_TYPE,
			u"Bilinmeyen eksen dolaşma tipi: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_AXIS_NOT_SUPPORTED,
			u"Eksen dolaşıcı desteklenmiyor: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_NO_DTMIDS_AVAIL,
			u"Kullanılabilecek başka DTM tanıtıcısı yok"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_NOT_SUPPORTED,
			"Desteklenmiyor: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_NODE_NON_NULL,
			u"getDTMHandleFromNode için düğüm boş değerli olmamalıdır"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_COULD_NOT_RESOLVE_NODE,
			u"Düğüm tanıtıcı değere çözülemedi"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_STARTPARSE_WHILE_PARSING,
			u"Ayrıştırma sırasında startParse çağrılamaz"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_STARTPARSE_NEEDS_SAXPARSER,
			u"startParse için boş değerli olmayan SAXParser gerekiyor"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_COULD_NOT_INIT_PARSER,
			u"Ayrıştırıcı bununla kullanıma hazırlanamadı"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_EXCEPTION_CREATING_POOL,
			u"Havuz için yeni örnek yaratılırken kural dışı durum"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_PATH_CONTAINS_INVALID_ESCAPE_SEQUENCE,
			u"Yol geçersiz kaçış dizisi içeriyor"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_SCHEME_REQUIRED,
			u"Şema gerekli!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_NO_SCHEME_IN_URI,
			u"URI içinde şema bulunamadı: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_NO_SCHEME_INURI,
			u"URI içinde şema bulunamadı"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_PATH_INVALID_CHAR,
			u"Yol geçersiz karakter içeriyor: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_SCHEME_FROM_NULL_STRING,
			u"Boş değerli dizgiden şema tanımlanamaz"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_SCHEME_NOT_CONFORMANT,
			u"Şema uyumlu değil."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_HOST_ADDRESS_NOT_WELLFORMED,
			u"Anasistem doğru biçimli bir adres değil"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_PORT_WHEN_HOST_NULL,
			u"Anasistem boş değerliyken kapı tanımlanamaz"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_INVALID_PORT,
			u"Kapı numarası geçersiz"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_FRAG_FOR_GENERIC_URI,
			u"Parça yalnızca soysal URI için tanımlanabilir"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_FRAG_WHEN_PATH_NULL,
			u"Yol boş değerliyken parça tanımlanamaz"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_FRAG_INVALID_CHAR,
			u"Parça geçersiz karakter içeriyor"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_PARSER_IN_USE,
			u"Ayrıştırıcı kullanımda"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_CANNOT_CHANGE_WHILE_PARSING,
			u"Ayrıştırma sırasında {0} {1} değiştirilemez"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_SELF_CAUSATION_NOT_PERMITTED,
			u"Öznedenselliğe izin verilmez"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_NO_USERINFO_IF_NO_HOST,
			u"Anasistem belirtilmediyse kullanıcı bilgisi belirtilemez"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_NO_PORT_IF_NO_HOST,
			u"Anasistem belirtilmediyse kapı belirtilemez"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_NO_QUERY_STRING_IN_PATH,
			"Yol ve sorgu dizgisinde sorgu dizgisi belirtilemez"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_NO_FRAGMENT_STRING_IN_PATH,
			u"Parça hem yolda, hem de parçada belirtilemez"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_CANNOT_INIT_URI_EMPTY_PARMS,
			u"Boş değiştirgelerle URI kullanıma hazırlanamaz"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_METHOD_NOT_SUPPORTED,
			u"Yöntem henüz desteklenmiyor"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_INCRSAXSRCFILTER_NOT_RESTARTABLE,
			u"IncrementalSAXSource_Filter şu an yeniden başlatılabilir durumda değil"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_XMLRDR_NOT_BEFORE_STARTPARSE,
			u"XMLReader, startParse isteğinden önce olmaz"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_AXIS_TRAVERSER_NOT_SUPPORTED,
			u"Eksen dolaşıcı desteklenmiyor: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_ERRORHANDLER_CREATED_WITH_NULL_PRINTWRITER,
			u"ListingErrorHandler boş değerli PrintWriter ile yaratıldı!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_SYSTEMID_UNKNOWN,
			"SystemId bilinmiyor"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_LOCATION_UNKNOWN,
			"Hata yeri bilinmiyor"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_PREFIX_MUST_RESOLVE,
			u"Önek bir ad alanına çözülmelidir: {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_CREATEDOCUMENT_NOT_SUPPORTED,
			u"XPathContext içinde createDocument() desteklenmiyor!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_CHILD_HAS_NO_OWNER_DOCUMENT,
			u"Özniteliğin alt öğesinin iye belgesi yok!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_CHILD_HAS_NO_OWNER_DOCUMENT_ELEMENT,
			u"Özniteliğin alt öğesinin iye belge öğesi yok!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_CANT_OUTPUT_TEXT_BEFORE_DOC,
			u"Uyarı: Belge öğesinden önce metin çıkışı olamaz!  Yoksayılıyor..."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_CANT_HAVE_MORE_THAN_ONE_ROOT,
			u"DOM üzerinde birden fazla kök olamaz!"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_ARG_LOCALNAME_NULL,
			u"\'localName\' bağımsız değiştirgesi boş değerli"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_ARG_LOCALNAME_INVALID,
			u"QNAME içindeki yerel ad (localname) geçerli bir NCName olmalıdır"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_ARG_PREFIX_INVALID,
			u"QNAME içindeki önek geçerli bir NCName olmalıdır"_s
		}),
		$$new($ObjectArray, {
			"BAD_CODE"_s,
			u"createMessage için kullanılan değiştirge sınırların dışında"_s
		}),
		$$new($ObjectArray, {
			"FORMAT_FAILED"_s,
			u"messageFormat çağrısı sırasında kural dışı durum yayınlandı"_s
		}),
		$$new($ObjectArray, {
			"line"_s,
			u"Satır #"_s
		...
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_SERIALIZER_NOT_CONTENTHANDLER,
			u"Diziselleştirici sınıfı \'\'{0}\'\' org.xml.sax.ContentHandler işlevini uygulamıyor."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_RESOURCE_COULD_NOT_FIND,
			u"Kaynak [ {0} ] bulunamadı.\n {1}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_RESOURCE_COULD_NOT_LOAD,
			u"Kaynak [ {0} ] yükleyemedi: {1} \n {2} \t {3}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_BUFFER_SIZE_LESSTHAN_ZERO,
			u"Arabellek büyüklüğü <=0"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_INVALID_UTF16_SURROGATE,
			u"UTF-16 yerine kullanılan değer geçersiz: {0} ?"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_OIERROR,
			u"GÇ hatası"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_ILLEGAL_ATTRIBUTE_POSITION,
			u"Alt düğümlerden sonra ya da bir öğe üretilmeden önce {0} özniteliği eklenemez. Öznitelik yoksayılacak."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_NAMESPACE_PREFIX,
			u"\'\'{0}\'\' önekine ilişkin ad alanı bildirilmedi."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_STRAY_ATTRIBUTE,
			u"\'\'{0}\'\' özniteliği öğenin dışında."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_STRAY_NAMESPACE,
			u"\'\'{0}\'\'=\'\'{1}\'\' ad alanı bildirimi öğenin dışında."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_COULD_NOT_LOAD_RESOURCE,
			u"\'\'{0}\'\' yüklenemedi (CLASSPATH değişkeninizi inceleyin), yalnızca varsayılanlar kullanılıyor"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_tr::ER_COULD_NOT_LOAD_METHOD_PROPERTY,
			u"\'\'{1}\'\' çıkış yöntemi için \'\'{0}\'\' özellik dosyası yüklenemedi (CLASSPATH değişkenini inceleyin)"_s
		})
	}));
}
...
$Class* XMLErrorResources_tr::load$($String* name, bool initialize) {
	...
	$loadClass(XMLErrorResources_tr, name, initialize, &classInfo$$, XMLErrorResources_tr::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLErrorResources_tr);
	});
	return class$;
}
