...
void SuballocatedIntVector::addElement(int32_t value) {
	...
	if (indexRelativeToCache >= 0 && indexRelativeToCache < this->m_blocksize) {
		...
		int32_t offset = this->m_firstFree & this->m_MASK;
		...
		$var($ints, block, this->m_map->get(index));
		if (nullptr == block) {
			$assign(block, this->m_map->set(index, $$new($ints, this->m_blocksize)));
		}
...
void SuballocatedIntVector::removeElementAt(int32_t at) {
	...
	if (at < this->m_firstFree) {
		...
		while (index <= maxindex) {
			...
			if (index < maxindex) {
				$var($ints, next, this->m_map->get(index + 1));
				if (next != nullptr) {
...
int32_t SuballocatedIntVector::elementAt(int32_t i) {
	...
	return $nc($nc(this->m_map)->get($usr(i, this->m_SHIFT)))->get(i & this->m_MASK);
}
...
int32_t SuballocatedIntVector::indexOf(int32_t elem, int32_t index) {
	...
	int32_t boffset = index & this->m_MASK;
	int32_t maxindex = $usr(this->m_firstFree, this->m_SHIFT);
	...
	int32_t maxoffset = this->m_firstFree & this->m_MASK;
...
int32_t SuballocatedIntVector::lastIndexOf(int32_t elem) {
	$useLocalObjectStack();
	int32_t boffset = this->m_firstFree & this->m_MASK;
...
$Class* SuballocatedIntVector::load$($String* name, bool initialize) {
	...
	$loadClass(SuballocatedIntVector, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SuballocatedIntVector);
	});
	return class$;
}
