...
$DTMAxisTraverser* DTMDefaultBaseTraversers::getAxisTraverser(int32_t axis) {
	...
	if (nullptr == this->m_traversers) {
		...
		$assign(traverser, this->m_traversers->get(axis));
	...
	default:
		...
		$throwNew($DTMException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_UNKNOWN_AXIS_TYPE, $$new($ObjectArray, {$($Integer::toString(axis))}))));
	}
	if (nullptr == traverser) {
		$init($XMLErrorResources);
		$throwNew($DTMException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_AXIS_TRAVERSER_NOT_SUPPORTED, $$new($ObjectArray, {$($Axis::getNames(axis))}))));
	}
...
$Class* DTMDefaultBaseTraversers::load$($String* name, bool initialize) {
	...
	$loadClass(DTMDefaultBaseTraversers, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DTMDefaultBaseTraversers);
	});
	return class$;
}
