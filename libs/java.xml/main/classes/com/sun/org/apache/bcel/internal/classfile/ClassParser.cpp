...
void ClassParser::init$($InputStream* inputStream, $String* fileName) {
	...
	this->isZip = var$0 || clazz->startsWith("java.util.jar."_s);
...
void ClassParser::readClassInfo() {
	...
	if ((this->accessFlags & $Const::ACC_INTERFACE) != 0) {
		this->accessFlags |= $Const::ACC_ABSTRACT;
	}
	if (((this->accessFlags & $Const::ACC_ABSTRACT) != 0) && ((this->accessFlags & $Const::ACC_FINAL) != 0)) {
...
void ClassParser::readConstantPool() {
	$set(this, constantPool, $new($ConstantPool, this->dataInputStream));
}
...
void ClassParser::readFields() {
	...
	for (int32_t i = 0; i < fields_count; ++i) {
		this->fields->set(i, $$new($Field, this->dataInputStream, this->constantPool));
	}
}
...
void ClassParser::readInterfaces() {
	...
	for (int32_t i = 0; i < interfaces_count; ++i) {
		this->interfaces->set(i, this->dataInputStream->readUnsignedShort());
	}
}
...
void ClassParser::readMethods() {
	...
	for (int32_t i = 0; i < methods_count; ++i) {
		this->methods->set(i, $$new($Method, this->dataInputStream, this->constantPool));
	}
}
...
$Class* ClassParser::load$($String* name, bool initialize) {
	...
	$loadClass(ClassParser, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ClassParser);
	});
	return class$;
}
