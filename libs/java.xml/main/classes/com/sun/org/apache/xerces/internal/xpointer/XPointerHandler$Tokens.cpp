...
void XPointerHandler$Tokens::init$($XPointerHandler* this$0, $SymbolTable* symbolTable) {
	...
	this->fTokenNames->put($($Integer::valueOf(XPointerHandler$Tokens::XPTRTOKEN_OPEN_PAREN)), "XPTRTOKEN_OPEN_PAREN"_s);
	this->fTokenNames->put($($Integer::valueOf(XPointerHandler$Tokens::XPTRTOKEN_CLOSE_PAREN)), "XPTRTOKEN_CLOSE_PAREN"_s);
	this->fTokenNames->put($($Integer::valueOf(XPointerHandler$Tokens::XPTRTOKEN_SHORTHAND)), "XPTRTOKEN_SHORTHAND"_s);
	this->fTokenNames->put($($Integer::valueOf(XPointerHandler$Tokens::XPTRTOKEN_SCHEMENAME)), "XPTRTOKEN_SCHEMENAME"_s);
	this->fTokenNames->put($($Integer::valueOf(XPointerHandler$Tokens::XPTRTOKEN_SCHEMEDATA)), "XPTRTOKEN_SCHEMEDATA"_s);
}
...
$Class* XPointerHandler$Tokens::load$($String* name, bool initialize) {
	...
	$loadClass(XPointerHandler$Tokens, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XPointerHandler$Tokens);
	});
	return class$;
}
