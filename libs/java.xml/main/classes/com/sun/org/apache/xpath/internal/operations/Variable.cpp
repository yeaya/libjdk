...
void Variable::fixupVariables($List* vars, int32_t globalsSize) {
	...
	$var($String, msg, $XSLMessages::createXPATHMessage($XPATHErrorResources::ER_COULD_NOT_FIND_VAR, $$new($ObjectArray, {$($nc(this->m_qname)->toString())})));
	$var($TransformerException, te, $new($TransformerException, msg, this));
...
$XObject* Variable::execute($XPathContext* xctxt, bool destructiveOK) {
	...
	if (nullptr == result) {
		$init($XPATHErrorResources);
		warn(xctxt, $XPATHErrorResources::WG_ILLEGAL_VARIABLE_REFERENCE, $$new($ObjectArray, {$($nc(this->m_qname)->getLocalPart())}));
...
void Variable::clinit$($Class* clazz) {
...
$Class* Variable::load$($String* name, bool initialize) {
	...
	$loadClass(Variable, name, initialize, &classInfo$$, Variable::clinit$, []($Class* clazz) -> $Object* {
