...
$StringArray* XMLNamespaceBinder::getRecognizedFeatures() {
	return $cast($StringArray, XMLNamespaceBinder::RECOGNIZED_FEATURES->clone());
}
...
$StringArray* XMLNamespaceBinder::getRecognizedProperties() {
	return $cast($StringArray, XMLNamespaceBinder::RECOGNIZED_PROPERTIES->clone());
}
...
$Boolean* XMLNamespaceBinder::getFeatureDefault($String* featureId) {
	for (int32_t i = 0; i < XMLNamespaceBinder::RECOGNIZED_FEATURES->length; ++i) {
		if ($nc(XMLNamespaceBinder::RECOGNIZED_FEATURES->get(i))->equals(featureId)) {
			return XMLNamespaceBinder::FEATURE_DEFAULTS->get(i);
		}
	}
	return nullptr;
}
...
$Object* XMLNamespaceBinder::getPropertyDefault($String* propertyId) {
	for (int32_t i = 0; i < XMLNamespaceBinder::RECOGNIZED_PROPERTIES->length; ++i) {
		if ($nc(XMLNamespaceBinder::RECOGNIZED_PROPERTIES->get(i))->equals(propertyId)) {
			return XMLNamespaceBinder::PROPERTY_DEFAULTS->get(i);
		}
	}
	return nullptr;
}
...
void XMLNamespaceBinder::handleStartElement($QName* element, $XMLAttributes* attributes, $Augmentations* augs, bool isEmpty) {
	...
	if ($nc(element)->prefix == $XMLSymbols::PREFIX_XMLNS) {
		...
		$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XMLNS_DOMAIN, "ElementXMLNSPrefix"_s, $$new($ObjectArray, {element->rawname}), $XMLErrorReporter::SEVERITY_FATAL_ERROR);
	}
	...
	for (int32_t i = 0; i < length; ++i) {
		...
		if (prefix == $XMLSymbols::PREFIX_XMLNS || prefix == $XMLSymbols::EMPTY_STRING && localpart == $XMLSymbols::PREFIX_XMLNS) {
			...
			if (prefix == $XMLSymbols::PREFIX_XMLNS && localpart == $XMLSymbols::PREFIX_XMLNS) {
				$init($XMLMessageFormatter);
				$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XMLNS_DOMAIN, "CantBindXMLNS"_s, $$new($ObjectArray, {$(attributes->getQName(i))}), $XMLErrorReporter::SEVERITY_FATAL_ERROR);
			}
			...
			if (localpart == $XMLSymbols::PREFIX_XML) {
				if (uri != $NamespaceContext::XML_URI) {
					$init($XMLMessageFormatter);
					$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XMLNS_DOMAIN, "CantBindXML"_s, $$new($ObjectArray, {$(attributes->getQName(i))}), $XMLErrorReporter::SEVERITY_FATAL_ERROR);
				}
			...
			if (prefixBoundToNullURI(uri, localpart)) {
				$init($XMLMessageFormatter);
				$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XMLNS_DOMAIN, "EmptyPrefixedAttName"_s, $$new($ObjectArray, {$(attributes->getQName(i))}), $XMLErrorReporter::SEVERITY_FATAL_ERROR);
				continue;
			}
			$nc(this->fNamespaceContext)->declarePrefix(prefix, $nc(uri)->length() != 0 ? uri : ($String*)nullptr);
		}
	}
	$var($String, prefix, element->prefix != nullptr ? element->prefix : $XMLSymbols::EMPTY_STRING);
	...
	for (int32_t i = 0; i < length; ++i) {
		...
		$var($String, arawname, this->fAttributeQName->rawname);
		if (arawname == $XMLSymbols::PREFIX_XMLNS) {
			$set(this->fAttributeQName, uri, $nc(this->fNamespaceContext)->getURI($XMLSymbols::PREFIX_XMLNS));
...
void XMLNamespaceBinder::handleEndElement($QName* element, $Augmentations* augs, bool isEmpty) {
	$init($XMLSymbols);
	$var($String, eprefix, $nc(element)->prefix != nullptr ? element->prefix : $XMLSymbols::EMPTY_STRING);
...
void XMLNamespaceBinder::clinit$($Class* clazz) {
	$init($Constants);
	...
	$assignStatic(XMLNamespaceBinder::FEATURE_DEFAULTS, $new($BooleanArray, {nullptr}));
...
$Class* XMLNamespaceBinder::load$($String* name, bool initialize) {
	...
	$loadClass(XMLNamespaceBinder, name, initialize, &classInfo$$, XMLNamespaceBinder::clinit$, []($Class* clazz) -> $Object* {
