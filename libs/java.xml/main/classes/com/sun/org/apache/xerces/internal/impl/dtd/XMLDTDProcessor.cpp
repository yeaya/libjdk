...
void XMLDTDProcessor::reset($XMLComponentManager* componentManager) {
	...
	if (this->fValidator != nullptr) {
		$set(this, fGrammarBucket, this->fValidator->getGrammarBucket());
	} else {
...
$StringArray* XMLDTDProcessor::getRecognizedFeatures() {
	return $cast($StringArray, XMLDTDProcessor::RECOGNIZED_FEATURES->clone());
}
...
$StringArray* XMLDTDProcessor::getRecognizedProperties() {
	return $cast($StringArray, XMLDTDProcessor::RECOGNIZED_PROPERTIES->clone());
}
...
$Boolean* XMLDTDProcessor::getFeatureDefault($String* featureId) {
	for (int32_t i = 0; i < XMLDTDProcessor::RECOGNIZED_FEATURES->length; ++i) {
		if ($nc(XMLDTDProcessor::RECOGNIZED_FEATURES->get(i))->equals(featureId)) {
			return XMLDTDProcessor::FEATURE_DEFAULTS->get(i);
		}
	}
	return nullptr;
}
...
$Object* XMLDTDProcessor::getPropertyDefault($String* propertyId) {
	for (int32_t i = 0; i < XMLDTDProcessor::RECOGNIZED_PROPERTIES->length; ++i) {
		if ($nc(XMLDTDProcessor::RECOGNIZED_PROPERTIES->get(i))->equals(propertyId)) {
			return XMLDTDProcessor::PROPERTY_DEFAULTS->get(i);
		}
	}
	return nullptr;
}
...
void XMLDTDProcessor::checkStandaloneEntityRef($String* name, $DTDGrammar* grammar, $XMLEntityDecl* tempEntityDecl, $XMLErrorReporter* errorReporter) {
	...
	if (entIndex > -1) {
		...
		if ($nc(tempEntityDecl)->inExternal) {
			...
			$nc(errorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "MSG_REFERENCE_TO_EXTERNALLY_DECLARED_ENTITY_WHEN_STANDALONE"_s, $$new($ObjectArray, {name}), $XMLErrorReporter::SEVERITY_ERROR);
		}
	}
}
...
void XMLDTDProcessor::elementDecl($String* name, $String* contentModel, $Augmentations* augs) {
	if (this->fValidation) {
		if (this->fDTDElementDecls->contains(name)) {
			$init($XMLMessageFormatter);
			$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "MSG_ELEMENT_ALREADY_DECLARED"_s, $$new($ObjectArray, {name}), $XMLErrorReporter::SEVERITY_ERROR);
		} else {
	...
	if (this->fValidation) {
		...
		if (type == $XMLSymbols::fIDSymbol) {
			if (defaultValue != nullptr && defaultValue->length != 0) {
				if (defaultType == nullptr || !(defaultType == $XMLSymbols::fIMPLIEDSymbol || defaultType == $XMLSymbols::fREQUIREDSymbol)) {
					$init($XMLMessageFormatter);
					$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "IDDefaultTypeInvalid"_s, $$new($ObjectArray, {attributeName}), $XMLErrorReporter::SEVERITY_ERROR);
				}
			}
			if (!$nc(this->fTableOfIDAttributeNames)->containsKey(elementName)) {
				...
				$var($String, previousIDAttributeName, $cast($String, this->fTableOfIDAttributeNames->get(elementName)));
				$init($XMLMessageFormatter);
		...
		if (type == $XMLSymbols::fNOTATIONSymbol) {
			...
			if ($nc(this->fTableOfNOTATIONAttributeNames)->containsKey(elementName) == false) {
				...
				$var($String, previousNOTATIONAttributeName, $cast($String, this->fTableOfNOTATIONAttributeNames->get(elementName)));
				$init($XMLMessageFormatter);
		...
		if (defaultValue != nullptr && (defaultType == nullptr || (defaultType != nullptr && defaultType == $XMLSymbols::fFIXEDSymbol))) {
			$var($String, value, defaultValue->toString());
...
void XMLDTDProcessor::notationDecl($String* name, $XMLResourceIdentifier* identifier, $Augmentations* augs) {
	...
	if (this->fValidation) {
		...
		if ($nc(grammar)->getNotationDeclIndex(name) != -1) {
			$init($XMLMessageFormatter);
			$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "UniqueNotationName"_s, $$new($ObjectArray, {name}), $XMLErrorReporter::SEVERITY_ERROR);
		}
	}
	if (this->fDTDGrammar != nullptr) {
...
void XMLDTDProcessor::endDTD($Augmentations* augs) {
	...
	if (this->fDTDGrammar != nullptr) {
		...
		if (this->fGrammarPool != nullptr) {
			...
			this->fGrammarPool->cacheGrammars($XMLGrammarDescription::XML_DTD, $$new($GrammarArray, {this->fDTDGrammar}));
		}
	}
	if (this->fValidation) {
		$var($DTDGrammar, grammar, this->fDTDGrammar != nullptr ? this->fDTDGrammar : $nc(this->fGrammarBucket)->getActiveGrammar());
		{
			$var($Iterator, i$, $$nc(this->fNDataDeclNotations->entrySet())->iterator());
			for (; $nc(i$)->hasNext();) {
...
bool XMLDTDProcessor::normalizeDefaultAttrValue($XMLString* value) {
	...
	for (int32_t i = value->offset; i < end; ++i) {
		if ($nc(value->ch)->get(i) == u' ') {
			if (!skipSpace) {
				value->ch->set(current++, u' ');
				skipSpace = true;
			} else {
			}
		} else {
			if (current != i) {
				value->ch->set(current, value->ch->get(i));
			}
			++current;
			skipSpace = false;
		}
	...
	if ($nc(contentSpec)->type == $XMLContentSpec::CONTENTSPECNODE_LEAF) {
		...
		if (value != nullptr && grammar->getElementDeclIndex(value) == -1) {
			...
			$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "UndeclaredElementInContentSpec"_s, $$new($ObjectArray, {
				$nc($(grammar->getElementDeclName(elementIndex)))->rawname,
				value
			}), $XMLErrorReporter::SEVERITY_WARNING);
		}
...
void XMLDTDProcessor::clinit$($Class* clazz) {
	$init($Constants);
...
$Class* XMLDTDProcessor::load$($String* name, bool initialize) {
	...
	$loadClass(XMLDTDProcessor, name, initialize, &classInfo$$, XMLDTDProcessor::clinit$, []($Class* clazz) -> $Object* {
