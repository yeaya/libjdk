...
void DOMResultAugmentor::setDOMResult($DOMResult* result) {
	...
	if (result != nullptr) {
		...
		$set(this, fDocument, ($nc(target)->getNodeType() == $Node::DOCUMENT_NODE) ? $cast($Document, target) : target->getOwnerDocument());
...
bool DOMResultAugmentor::processAttributePSVI($AttrImpl* attr, $AttributePSVI* attrPSVI) {
	...
	if (type == nullptr) {
		...
		if (type != nullptr) {
			...
			return $cast($XSSimpleType, type)->isIDType();
		}
	} else {
		$nc(attr)->setType(type);
		return $cast($XSSimpleType, type)->isIDType();
...
$Class* DOMResultAugmentor::load$($String* name, bool initialize) {
	...
	$loadClass(DOMResultAugmentor, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DOMResultAugmentor);
	});
	return class$;
}
