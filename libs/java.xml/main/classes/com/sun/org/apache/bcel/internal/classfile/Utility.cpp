...
int32_t Utility::unwrap($ThreadLocal* tl) {
	$init(Utility);
	return $$sure($Integer, $nc(tl)->get())->intValue();
}
...
$String* Utility::classOrInterface(int32_t access_flags) {
	$init(Utility);
	return ((access_flags & $Const::ACC_INTERFACE) != 0) ? "interface"_s : "class"_s;
}
...
$String* Utility::codeToString($ByteSequence* bytes, $ConstantPool* constant_pool, bool verbose) {
	...
	}
	{
		...
		case $Const::INVOKEVIRTUAL:
			index = bytes->readUnsignedShort();
			buf->append("\t"_s)->append($($nc(constant_pool)->constantToString(index, $Const::CONSTANT_Methodref)))->append(verbose ? $$str({" ("_s, $$str(index), ")"_s}) : ""_s);
			break;
		case $Const::INVOKEINTERFACE:
			index = bytes->readUnsignedShort();
			nargs = bytes->readUnsignedByte();
			buf->append("\t"_s)->append($($nc(constant_pool)->constantToString(index, $Const::CONSTANT_InterfaceMethodref)))->append(verbose ? $$str({" ("_s, $$str(index), ")\t"_s}) : ""_s)->append(nargs)->append("\t"_s)->append(bytes->readUnsignedByte());
			break;
		case $Const::INVOKEDYNAMIC:
			index = bytes->readUnsignedShort();
			buf->append("\t"_s)->append($($nc(constant_pool)->constantToString(index, $Const::CONSTANT_InvokeDynamic)))->append(verbose ? $$str({" ("_s, $$str(index), ")\t"_s}) : ""_s)->append(bytes->readUnsignedByte())->append(bytes->readUnsignedByte());
			break;
		case $Const::LDC_W:
		case $Const::LDC2_W:
			index = bytes->readUnsignedShort();
			buf->append("\t\t"_s)->append($($nc(constant_pool)->constantToString(index, $$nc($nc(constant_pool)->getConstant(index))->getTag())))->append(verbose ? $$str({" ("_s, $$str(index), ")"_s}) : ""_s);
			break;
		case $Const::LDC:
		...
		case $Const::ANEWARRAY:
			index = bytes->readUnsignedShort();
			buf->append("\t\t<"_s)->append($(compactClassName($($nc(constant_pool)->getConstantString(index, $Const::CONSTANT_Class)), false)))->append(">"_s)->append(verbose ? $$str({" ("_s, $$str(index), ")"_s}) : ""_s);
			break;
		case $Const::MULTIANEWARRAY:
			{
				index = bytes->readUnsignedShort();
				int32_t dimensions = bytes->readUnsignedByte();
				buf->append("\t<"_s)->append($(compactClassName($($nc(constant_pool)->getConstantString(index, $Const::CONSTANT_Class)), false)))->append(">\t"_s)->append(dimensions)->append(verbose ? $$str({" ("_s, $$str(index), ")"_s}) : ""_s);
			}
			break;
		case $Const::IINC:
			if (Utility::wide) {
...
$String* Utility::compactClassName($String* str$renamed, $String* prefix, bool chopit) {
	...
	if (chopit) {
		...
		if (var$0 && ($(str->substring(len))->indexOf(u'.') == -1)) {
...
$StringArray* Utility::methodSignatureArgumentTypes($String* signature, bool chopit) {
	...
	int32_t index = 0;
	try {
		index = $nc(signature)->indexOf(u'(') + 1;
		if (index <= 0) {
	...
	return $cast($StringArray, vec->toArray($$new($StringArray, vec->size())));
}
...
$String* Utility::methodSignatureReturnType($String* signature, bool chopit) {
	...
	try {
		index = $nc(signature)->lastIndexOf(u')') + 1;
		if (index <= 0) {
			$throwNew($ClassFormatException, $$str({"Invalid method signature: "_s, signature}));
		}
...
$String* Utility::methodSignatureToString($String* signature, $String* name, $String* access, bool chopit, $LocalVariableTable* vars) {
	...
	var$1->append(access);
	var$1->append((access->length() > 0) ? " "_s : ""_s);
	var$1->append(type);
	var$1->append(" "_s);
	var$1->append(name);
	var$1->append($(buf->toString()));
	return $str(var$1);
}
...
$String* Utility::signatureToString($String* signature, bool chopit) {
	...
	$assign(type, typeSignatureToString($(signature->substring(index)), chopit));
	index += unwrap(Utility::consumed_chars);
	bool var$0 = $nc(typeParams)->length() == 0;
	if (var$0 && (index == signature->length())) {
		return type;
	}
...
$String* Utility::typeParamTypeToString($String* signature, bool chopit) {
	$init(Utility);
	$useLocalObjectStack();
	int32_t index = $nc(signature)->indexOf(u':');
	if (index <= 0) {
...
$String* Utility::typeSignatureToString($String* signature, bool chopit) {
	...
	try {
		...
		case u'T':
			{
				int32_t index = signature->indexOf(u';');
				if (index < 0) {
		...
		case u'L':
			{
				int32_t fromIndex = signature->indexOf(u'<');
				if (fromIndex < 0) {
					fromIndex = 0;
				} else {
					fromIndex = signature->indexOf(u'>', fromIndex);
					if (fromIndex < 0) {
						$throwNew($ClassFormatException, $$str({"Invalid signature: "_s, signature}));
					}
				}
				int32_t index = signature->indexOf(u';', fromIndex);
				if (index < 0) {
					$throwNew($ClassFormatException, $$str({"Invalid signature: "_s, signature}));
				}
				int32_t bracketIndex = $(signature->substring(0, index))->indexOf(u'<');
				if (bracketIndex < 0) {
					wrap(Utility::consumed_chars, index + 1);
					return compactClassName($(signature->substring(1, index)), chopit);
				}
				fromIndex = signature->indexOf(u';');
				if (fromIndex < 0) {
					$throwNew($ClassFormatException, $$str({"Invalid signature: "_s, signature}));
				}
...
int8_t Utility::typeOfMethodSignature($String* signature) {
	...
	try {
		...
		index = signature->lastIndexOf(u')') + 1;
...
int16_t Utility::searchOpcode($String* name$renamed) {
	...
	$init($Const);
	for (int16_t i = 0; i < $Const::OPCODE_NAMES_LENGTH; ++i) {
...
void Utility::clinit$($Class* clazz) {
	...
	$assignStatic(Utility::MAP_CHAR, $new($ints, 256));
	{
		...
		for (int32_t i = u'A'; i <= u'Z'; ++i) {
			Utility::CHAR_MAP->set(j, i);
			Utility::MAP_CHAR->set(i, j);
			++j;
		}
		...
		Utility::CHAR_MAP->set(j, u'$');
		Utility::MAP_CHAR->set(u'$', j);
		++j;
		Utility::CHAR_MAP->set(j, u'_');
		Utility::MAP_CHAR->set(u'_', j);
	}
}
