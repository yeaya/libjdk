...
void LocalVariableTable::init$(int32_t name_index, int32_t length, $DataInput* input, $ConstantPool* constant_pool) {
	...
	for (int32_t i = 0; i < local_variable_table_length; ++i) {
		this->localVariableTable->set(i, $$new($LocalVariable, input, constant_pool));
	}
}
...
$String* LocalVariableTable::toString() {
	...
	for (int32_t i = 0; i < $nc(this->localVariableTable)->length; ++i) {
		buf->append(this->localVariableTable->get(i));
...
$Attribute* LocalVariableTable::copy($ConstantPool* _constant_pool) {
	...
	for (int32_t i = 0; i < $nc(this->localVariableTable)->length; ++i) {
		$nc(c->localVariableTable)->set(i, $($nc(this->localVariableTable->get(i))->copy()));
	}
...
int32_t LocalVariableTable::getTableLength() {
	return this->localVariableTable == nullptr ? 0 : this->localVariableTable->length;
}
...
$Class* LocalVariableTable::load$($String* name, bool initialize) {
	...
	$CompoundAttribute getLocalVariablemethodAnnotations$$[] = {
	...
	$MethodInfo methodInfos$$[] = {
		...
		{"getLocalVariable", "(I)Lcom/sun/org/apache/bcel/internal/classfile/LocalVariable;", nullptr, $PUBLIC | $FINAL | $DEPRECATED, $method(LocalVariableTable, getLocalVariable, $LocalVariable*, int32_t), nullptr, nullptr, getLocalVariablemethodAnnotations$$},
	...
	$loadClass(LocalVariableTable, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
