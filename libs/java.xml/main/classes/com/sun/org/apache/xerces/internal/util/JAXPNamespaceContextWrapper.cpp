...
$String* JAXPNamespaceContextWrapper::getURI($String* prefix) {
	if (this->fNamespaceContext != nullptr) {
		$var($String, uri, this->fNamespaceContext->getNamespaceURI(prefix));
		$init($XMLConstants);
		if (uri != nullptr && !$nc($XMLConstants::NULL_NS_URI)->equals(uri)) {
			return (this->fSymbolTable != nullptr) ? this->fSymbolTable->addSymbol(uri) : uri->intern();
		}
	}
	return nullptr;
}
...
$String* JAXPNamespaceContextWrapper::getPrefix($String* uri$renamed) {
	...
	if (this->fNamespaceContext != nullptr) {
		...
		$var($String, prefix, this->fNamespaceContext->getPrefix(uri));
		if (prefix == nullptr) {
			$init($XMLConstants);
			$assign(prefix, $XMLConstants::DEFAULT_NS_PREFIX);
		}
		return (this->fSymbolTable != nullptr) ? this->fSymbolTable->addSymbol(prefix) : $nc(prefix)->intern();
	}
	return nullptr;
}

$Enumeration* JAXPNamespaceContextWrapper::getAllPrefixes() {
	return $Collections::enumeration($$cast($AbstractCollection, $new($TreeSet, $cast($AbstractCollection, this->fAllPrefixes))));
}
...
void JAXPNamespaceContextWrapper::pushContext() {
	...
	this->fContext->set(++this->fCurrentContext, this->fAllPrefixes->size());
...
void JAXPNamespaceContextWrapper::popContext() {
	this->fAllPrefixes->setSize($nc(this->fContext)->get(this->fCurrentContext--));
}
...
int32_t JAXPNamespaceContextWrapper::getDeclaredPrefixCount() {
	return (this->fPrefixes != nullptr) ? this->fPrefixes->size() : 0;
}
...
$Class* JAXPNamespaceContextWrapper::load$($String* name, bool initialize) {
	...
	$loadClass(JAXPNamespaceContextWrapper, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JAXPNamespaceContextWrapper);
	});
	return class$;
}
