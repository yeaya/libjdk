		...
		case ElementValue::ENUM_CONSTANT:
			{
				...
				return $new($EnumElementValue, ElementValue::ENUM_CONSTANT, var$0, input->readUnsignedShort(), cpool);
			}
...
$Class* ElementValue::load$($String* name, bool initialize) {
	...
	$loadClass(ElementValue, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ElementValue);
	});
	return class$;
}
