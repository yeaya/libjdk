...
void DocumentImpl::removeNodeIterator($NodeIterator* nodeIterator) {
	...
	$nc(this->iterators)->remove(nodeIterator);
}
...
void DocumentImpl::removeRange($Range* range) {
	...
	$nc(this->ranges)->remove(range);
}

void DocumentImpl::replacedText($NodeImpl* node) {
	$useLocalObjectStack();
	if (this->ranges != nullptr) {
		int32_t size = this->ranges->size();
		for (int32_t i = 0; i != size; ++i) {
			$$sure($RangeImpl, $nc(this->ranges)->get(i))->receiveReplacedText(node);
		}
	}
}
...
void DocumentImpl::deletedText($NodeImpl* node, int32_t offset, int32_t count) {
	...
	if (this->ranges != nullptr) {
		int32_t size = this->ranges->size();
		for (int32_t i = 0; i != size; ++i) {
			$$sure($RangeImpl, $nc(this->ranges)->get(i))->receiveDeletedText(node, offset, count);
		}
	}
}
...
void DocumentImpl::insertedText($NodeImpl* node, int32_t offset, int32_t count) {
	...
	if (this->ranges != nullptr) {
		int32_t size = this->ranges->size();
		for (int32_t i = 0; i != size; ++i) {
			$$sure($RangeImpl, $nc(this->ranges)->get(i))->receiveInsertedText(node, offset, count);
		}
	}
}
...
void DocumentImpl::splitData($Node* node, $Node* newNode, int32_t offset) {
	...
	if (this->ranges != nullptr) {
		int32_t size = this->ranges->size();
		for (int32_t i = 0; i != size; ++i) {
			$$sure($RangeImpl, $nc(this->ranges)->get(i))->receiveSplitData(node, newNode, offset);
		}
	}
}
...
$Event* DocumentImpl::createEvent($String* type) {
	...
	bool var$1 = type->equalsIgnoreCase("MutationEvents"_s);
	if (var$1 || "MutationEvent"_s->equals(type)) {
		return $cast($EventImpl, $new($MutationEventImpl));
	} else {
		$init($DOMMessageFormatter);
		$var($String, msg, $DOMMessageFormatter::formatMessage($DOMMessageFormatter::DOM_DOMAIN, "NOT_SUPPORTED_ERR"_s, nullptr));
		$throwNew($DOMException, $DOMException::NOT_SUPPORTED_ERR, msg);
...
void DocumentImpl::addEventListener($NodeImpl* node, $String* type, $EventListener* listener, bool useCapture) {
	$useLocalObjectStack();
	if (type == nullptr || type->equals(""_s) || listener == nullptr) {
		return;
	}
...
bool DocumentImpl::dispatchEvent($NodeImpl* node, $Event* event) {
	...
	if (!$nc(evt)->initialized || evt->type == nullptr || evt->type->equals(""_s)) {
		$init($DOMMessageFormatter);
	...
	$var($LCount, lc, $LCount::lookup($(evt->getType())));
	if ($nc(lc)->total == 0) {
	...
	if (enclosingAttr != nullptr) {
		...
		if ($nc(lc)->total > 0) {
			if (owner != nullptr) {
				...
				$var($String, var$1, enclosingAttr->getNodeValue());
				me->initMutationEvent(var$0, true, false, $cast($NodeImpl, enclosingAttr), oldvalue, var$1, $(enclosingAttr->getNodeName()), change);
				owner->dispatchEvent($cast($EventImpl, me));
			}
		}
	}
	$init($MutationEventImpl);
	...
	if ($nc(lc)->total > 0) {
		...
		while (true) {
			...
			if (type == $Node::ATTRIBUTE_NODE) {
				...
				$set(retval, oldvalue, retval->node->getNodeValue());
...
void DocumentImpl::insertedNode($NodeImpl* node, $NodeImpl* newInternal, bool replace) {
	...
	if (this->mutationEvents) {
		...
		if ($nc(lc)->total > 0) {
			...
			dispatchEvent(newInternal, $cast($EventImpl, me));
		}
		$assign(lc, $LCount::lookup($MutationEventImpl::DOM_NODE_INSERTED_INTO_DOCUMENT));
		if ($nc(lc)->total > 0) {
			$var($NodeImpl, eventAncestor, node);
			if (this->savedEnclosingAttr != nullptr) {
				$assign(eventAncestor, $cast($NodeImpl, $nc(this->savedEnclosingAttr->node)->getOwnerElement()));
			}
			if (eventAncestor != nullptr) {
				...
				while (p != nullptr) {
					...
					if (p->getNodeType() == $Node::ATTRIBUTE_NODE) {
						$assign(p, $cast($NodeImpl, $cast($AttrImpl, p)->getOwnerElement()));
					} else {
				...
				if (eventAncestor->getNodeType() == $Node::DOCUMENT_NODE) {
					...
					dispatchEventToSubtree(newInternal, $cast($EventImpl, me));
				}
			}
		}
		if (!replace) {
	...
	if (this->ranges != nullptr) {
		int32_t size = this->ranges->size();
		for (int32_t i = 0; i != size; ++i) {
			$$sure($RangeImpl, $nc(this->ranges)->get(i))->insertedNodeFromDOM(newInternal);
		}
	}
}
...
void DocumentImpl::removingNode($NodeImpl* node, $NodeImpl* oldChild, bool replace) {
	$useLocalObjectStack();
	if (this->iterators != nullptr) {
		int32_t size = this->iterators->size();
		for (int32_t i = 0; i != size; ++i) {
			$$sure($NodeIteratorImpl, $nc(this->iterators)->get(i))->removeNode(oldChild);
		}
	}
	if (this->ranges != nullptr) {
		int32_t size = this->ranges->size();
		for (int32_t i = 0; i != size; ++i) {
			$$sure($RangeImpl, $nc(this->ranges)->get(i))->removeNode(oldChild);
		}
	}
	if (this->mutationEvents) {
		if (!replace) {
		...
		if ($nc(lc)->total > 0) {
			...
			dispatchEvent(oldChild, $cast($EventImpl, me));
		}
			...
			if (eventAncestor != nullptr) {
				{
					...
					for (; p != nullptr; $assign(p, p->parentNode())) {
						$assign(eventAncestor, p);
					}
				}
				if ($nc(eventAncestor)->getNodeType() == $Node::DOCUMENT_NODE) {
					...
					dispatchEventToSubtree(oldChild, $cast($EventImpl, me));
				}
			}
		}
	}
...
void DocumentImpl::setAttrNode($AttrImpl* attr, $AttrImpl* previous) {
	if (this->mutationEvents) {
		if (previous == nullptr) {
			...
			dispatchAggregateEvents($nc(attr)->ownerNode, attr, $(previous->getNodeValue()), $MutationEvent::MODIFICATION);
		}
	}
}
...
void DocumentImpl::removedAttrNode($AttrImpl* attr, $NodeImpl* oldOwner, $String* name) {
	...
	if (this->mutationEvents) {
		...
		if ($nc(lc)->total > 0) {
			$var($MutationEventImpl, me, $new($MutationEventImpl));
			me->initMutationEvent($MutationEventImpl::DOM_ATTR_MODIFIED, true, false, $cast($NodeImpl, attr), $($nc(attr)->getNodeValue()), nullptr, name, $MutationEvent::REMOVAL);
			dispatchEvent(oldOwner, $cast($EventImpl, me));
		}
...
void DocumentImpl::writeObject($ObjectOutputStream* out) {
	$useLocalObjectStack();
	$var($Vector, it, (this->iterators == nullptr) ? ($Vector*)nullptr : $new($Vector, this->iterators));
	$var($Vector, r, (this->ranges == nullptr) ? ($Vector*)nullptr : $new($Vector, this->ranges));
	$var($Hashtable, el, nullptr);
	if (this->eventListeners != nullptr) {
		$assign(el, $new($Hashtable));
		{
			$var($Iterator, i$, $$nc(this->eventListeners->entrySet())->iterator());
			for (; $nc(i$)->hasNext();) {
	...
	$nc(pf)->put("iterators"_s, it);
	pf->put("ranges"_s, r);
	pf->put("eventListeners"_s, el);
...
void DocumentImpl::readObject($ObjectInputStream* in) {
	...
	$var($Vector, it, $cast($Vector, $nc(gf)->get("iterators"_s, nullptr)));
	$var($Vector, r, $cast($Vector, gf->get("ranges"_s, nullptr)));
	$var($Hashtable, el, $cast($Hashtable, gf->get("eventListeners"_s, nullptr)));
	this->mutationEvents = gf->get("mutationEvents"_s, false);
	if (it != nullptr) {
		$set(this, iterators, $new($ArrayList, $cast($AbstractCollection, it)));
	}
	if (r != nullptr) {
		$set(this, ranges, $new($ArrayList, $cast($AbstractCollection, r)));
	}
	if (el != nullptr) {
		$set(this, eventListeners, $new($HashMap));
		{
			...
			for (; $nc(i$)->hasNext();) {
				...
				{
					$var($Object, var$0, $cast($NodeImpl, $nc(e)->getKey()));
					this->eventListeners->put(var$0, $$new($ArrayList, $$cast($Collection, e->getValue())));
				}
			}
		}
	}
...
void DocumentImpl::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$load($Vector);
	$load($Hashtable);
...
$Class* DocumentImpl::load$($String* name, bool initialize) {
	...
	$loadClass(DocumentImpl, name, initialize, &classInfo$$, DocumentImpl::clinit$, []($Class* clazz) -> $Object* {
