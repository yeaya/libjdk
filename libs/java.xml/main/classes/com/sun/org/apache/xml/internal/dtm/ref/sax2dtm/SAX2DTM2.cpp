...
int32_t SAX2DTM2::_exptype2(int32_t identity) {
	if (identity < this->m_blocksize) {
		...
		return $nc($nc(this->m_exptype_map)->get($usr(identity, this->m_SHIFT)))->get(identity & this->m_MASK);
	}
}
...
int32_t SAX2DTM2::_nextsib2(int32_t identity) {
	if (identity < this->m_blocksize) {
		...
		return $nc($nc(this->m_nextsib_map)->get($usr(identity, this->m_SHIFT)))->get(identity & this->m_MASK);
	}
}
...
int32_t SAX2DTM2::_firstch2(int32_t identity) {
	if (identity < this->m_blocksize) {
		...
		return $nc($nc(this->m_firstch_map)->get($usr(identity, this->m_SHIFT)))->get(identity & this->m_MASK);
	}
}
...
int32_t SAX2DTM2::_parent2(int32_t identity) {
	if (identity < this->m_blocksize) {
		...
		return $nc($nc(this->m_parent_map)->get($usr(identity, this->m_SHIFT)))->get(identity & this->m_MASK);
	}
}
...
int32_t SAX2DTM2::_type2(int32_t identity) {
	...
	if (identity < this->m_blocksize) {
		...
		eType = $nc($nc(this->m_exptype_map)->get($usr(identity, this->m_SHIFT)))->get(identity & this->m_MASK);
	}
	if ($DTM::NULL != eType) {
...
int32_t SAX2DTM2::getExpandedTypeID2(int32_t nodeHandle) {
	...
	if (nodeID != $DTM::NULL) {
		if (nodeID < this->m_blocksize) {
			...
			return $nc($nc(this->m_exptype_map)->get($usr(nodeID, this->m_SHIFT)))->get(nodeID & this->m_MASK);
		}
	} else {
		return $DTM::NULL;
	}
...
void SAX2DTM2::startElement($String* uri, $String* localName$renamed, $String* qName, $Attributes* attributes) {
	...
	charactersFlush();
	bool var$0 = uri == nullptr || uri->isEmpty();
	if (var$0 && (localName == nullptr || localName->isEmpty())) {
	...
	for (int32_t i = 0; i < n; ++i) {
		...
		if (attrUri == nullptr || attrUri->isEmpty()) {
			if (attrLocalName == nullptr || attrLocalName->isEmpty()) {
				int32_t colon = $nc(attrQName)->lastIndexOf(u':');
				$assign(attrLocalName, (colon > -1) ? attrQName->substring(colon + 1) : attrQName);
			} else {
				int32_t colon = attrLocalName->lastIndexOf(u':');
...
$XMLString* SAX2DTM2::getStringValue(int32_t nodeHandle) {
	...
	if (type == $DTM::ELEMENT_NODE || type == $DTM::DOCUMENT_NODE) {
			...
			do {
				...
				if (type == $DTM::TEXT_NODE || type == $DTM::CDATA_SECTION_NODE) {
					...
					if (dataIndex >= 0) {
						...
						length += dataIndex & SAX2DTM2::TEXT_LENGTH_MAX;
					} else {
						if (-1 == offset) {
		...
		if (dataIndex >= 0) {
			if (this->m_xstrf != nullptr) {
				return this->m_xstrf->newstr(this->m_chars, $usr(dataIndex, SAX2DTM2::TEXT_LENGTH_BITS), dataIndex & SAX2DTM2::TEXT_LENGTH_MAX);
			} else {
				return $new($XMLStringDefault, $($nc(this->m_chars)->getString($usr(dataIndex, SAX2DTM2::TEXT_LENGTH_BITS), dataIndex & SAX2DTM2::TEXT_LENGTH_MAX)));
			}
		} else if (this->m_xstrf != nullptr) {
			$var($FastStringBuffer, var$0, this->m_chars);
			int32_t var$1 = $nc(this->m_data)->elementAt(-dataIndex);
			return this->m_xstrf->newstr(var$0, var$1, this->m_data->elementAt(-dataIndex + 1));
		} else {
			int32_t var$2 = $nc(this->m_data)->elementAt(-dataIndex);
			return $new($XMLStringDefault, $($nc(this->m_chars)->getString(var$2, this->m_data->elementAt(-dataIndex + 1))));
		}
	} else {
		int32_t dataIndex = $nc(this->m_dataOrQName)->elementAt(identity);
		if (dataIndex < 0) {
		...
		if (this->m_xstrf != nullptr) {
			return this->m_xstrf->newstr($$cast($String, $nc(this->m_values)->get(dataIndex)));
		} else {
...
$String* SAX2DTM2::getStringValueX(int32_t nodeHandle) {
	...
	if (type == $DTM::ELEMENT_NODE || type == $DTM::DOCUMENT_NODE) {
		...
		int32_t dataIndex = $nc(this->m_dataOrQName)->elementAt(identity);
		if (dataIndex >= 0) {
			return $nc(this->m_chars)->getString($usr(dataIndex, SAX2DTM2::TEXT_LENGTH_BITS), dataIndex & SAX2DTM2::TEXT_LENGTH_MAX);
		} else {
			int32_t var$0 = $nc(this->m_data)->elementAt(-dataIndex);
			return $nc(this->m_chars)->getString(var$0, this->m_data->elementAt(-dataIndex + 1));
		}
	} else {
		int32_t dataIndex = $nc(this->m_dataOrQName)->elementAt(identity);
		if (dataIndex < 0) {
...
$String* SAX2DTM2::getStringValue() {
	...
	if (var$0 && (_nextsib2(child) == $DTM::NULL)) {
		...
		if (dataIndex >= 0) {
			...
			int32_t var$1 = $nc(this->m_data)->elementAt(-dataIndex);
			return $nc(this->m_chars)->getString(var$1, this->m_data->elementAt(-dataIndex + 1));
		}
	} else {
...
void SAX2DTM2::dispatchCharactersEvents(int32_t nodeHandle, $ContentHandler* ch, bool normalize) {
	...
	if (type == $DTM::ELEMENT_NODE || type == $DTM::DOCUMENT_NODE) {
		...
		if (dataIndex >= 0) {
			if (normalize) {
				$nc(this->m_chars)->sendNormalizedSAXcharacters(ch, $usr(dataIndex, SAX2DTM2::TEXT_LENGTH_BITS), dataIndex & SAX2DTM2::TEXT_LENGTH_MAX);
			} else {
				$nc(this->m_chars)->sendSAXcharacters(ch, $usr(dataIndex, SAX2DTM2::TEXT_LENGTH_BITS), dataIndex & SAX2DTM2::TEXT_LENGTH_MAX);
			}
		} else if (normalize) {
			int32_t var$0 = $nc(this->m_data)->elementAt(-dataIndex);
			$nc(this->m_chars)->sendNormalizedSAXcharacters(ch, var$0, this->m_data->elementAt(-dataIndex + 1));
		} else {
			int32_t var$1 = $nc(this->m_data)->elementAt(-dataIndex);
			$nc(this->m_chars)->sendSAXcharacters(ch, var$1, this->m_data->elementAt(-dataIndex + 1));
		}
	} else {
		int32_t dataIndex = $nc(this->m_dataOrQName)->elementAt(identity);
		if (dataIndex < 0) {
		...
		if (normalize) {
			$var($chars, var$2, $nc(str)->toCharArray());
			$FastStringBuffer::sendNormalizedSAXcharacters(var$2, 0, str->length(), ch);
		} else {
			$var($chars, var$3, $nc(str)->toCharArray());
			$nc(ch)->characters(var$3, 0, str->length());
		}
	}
}
...
void SAX2DTM2::copyTextNode(int32_t nodeID, $SerializationHandler* handler) {
	if (nodeID != $DTM::NULL) {
		...
		if (dataIndex >= 0) {
			$nc(this->m_chars)->sendSAXcharacters(handler, $usr(dataIndex, SAX2DTM2::TEXT_LENGTH_BITS), dataIndex & SAX2DTM2::TEXT_LENGTH_MAX);
		} else {
			int32_t var$0 = $nc(this->m_data)->elementAt(-dataIndex);
			$nc(this->m_chars)->sendSAXcharacters(handler, var$0, this->m_data->elementAt(-dataIndex + 1));
		}
	}
}
...
$String* SAX2DTM2::copyElement(int32_t nodeID, int32_t exptype, $SerializationHandler* handler) {
	...
	if ($nc(uri)->length() == 0) {
		...
		int32_t prefixIndex = $nc(qName)->indexOf(u':');
		$var($String, prefix, nullptr);
		if (prefixIndex > 0) {
...
void SAX2DTM2::copyNS(int32_t nodeID, $SerializationHandler* handler, bool inScope) {
	$useLocalObjectStack();
	bool var$0 = this->m_namespaceDeclSetElements != nullptr && this->m_namespaceDeclSetElements->size() == 1 && this->m_namespaceDeclSets != nullptr;
	if (var$0 && $$sure($SuballocatedIntVector, this->m_namespaceDeclSets->get(0))->size() == 1) {
		return;
	}
	...
	if (inScope) {
		...
		if (nsContext == nullptr || nsContext->size() < 1) {
			return;
		} else {
...
void SAX2DTM2::copyAttribute(int32_t nodeID, int32_t exptype, $SerializationHandler* handler) {
	...
	if (dataIndex <= 0) {
		...
		int32_t colonIndex = $nc(qname)->indexOf(u':');
		if (colonIndex > 0) {
...
void SAX2DTM2::clinit$($Class* clazz) {
...
$Class* SAX2DTM2::load$($String* name, bool initialize) {
	...
	$loadClass(SAX2DTM2, name, initialize, &classInfo$$, SAX2DTM2::clinit$, []($Class* clazz) -> $Object* {
