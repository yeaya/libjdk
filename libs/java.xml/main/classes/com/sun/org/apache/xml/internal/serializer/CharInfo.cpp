		...
		try {
			try {
				...
				while (line != nullptr) {
					...
					int32_t index = $nc(line)->indexOf(u' ');
					if (index > 1) {
...
CharInfo* CharInfo::getCharInfo($String* entitiesFileName, $String* method) {
	...
	if ($nc(entitiesFileName)->indexOf(u':') < 0) {
...
int32_t CharInfo::bit(int32_t i) {
	$init(CharInfo);
	int32_t ret = ($sl(1, i & CharInfo::LOW_ORDER_BITMASK));
	return ret;
}
...
void CharInfo::set(int32_t i) {
	...
	(*$nc(this->array_of_bits))[j] |= ($sl(1, i & CharInfo::LOW_ORDER_BITMASK));
}
...
bool CharInfo::get(int32_t i) {
	...
	if (j < this->firstWordNotUsed) {
		in_the_set = ($nc(this->array_of_bits)->get(j) & ($sl(1, i & CharInfo::LOW_ORDER_BITMASK))) != 0;
	}
	return in_the_set;
}
...
void CharInfo::clinit$($Class* clazz) {
...
$Class* CharInfo::load$($String* name, bool initialize) {
	...
	$loadClass(CharInfo, name, initialize, &classInfo$$, CharInfo::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(CharInfo);
	});
	return class$;
}
