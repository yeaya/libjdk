...
bool XSConstraints::checkSimpleDerivation($XSSimpleType* derived, $XSSimpleType* base$renamed, int16_t block) {
	...
	if ((block & $XSConstants::DERIVATION_RESTRICTION) != 0 || ($$nc($nc(derived)->getBaseType())->getFinal() & $XSConstants::DERIVATION_RESTRICTION) != 0) {
		return false;
	}
...
bool XSConstraints::checkComplexDerivation($XSComplexTypeDecl* derived, $XSTypeDefinition* base$renamed, int16_t block) {
	...
	if (($nc(derived)->fDerivedBy & block) != 0) {
		return false;
	}
	$var($XSTypeDefinition, directBase, derived->fBaseType);
	if (directBase == base) {
		return true;
	}
	$init($SchemaGrammar);
...
$Object* XSConstraints::ElementDefaultValidImmediate($XSTypeDefinition* type, $String* value, $ValidationContext* context, $ValidatedInfo* vinfo) {
	...
	if ($nc(type)->getTypeCategory() == $XSTypeDefinition::SIMPLE_TYPE) {
		...
		if (ctype->fContentType == $XSComplexTypeDecl::CONTENTTYPE_SIMPLE) {
			...
			if (!$$sure($XSParticleDecl, ctype->getParticle())->emptiable()) {
				return nullptr;
			}
		} else {
			return nullptr;
...
void XSConstraints::reportSchemaError($XMLErrorReporter* errorReporter, $SimpleLocator* loc, $String* key, $ObjectArray* args) {
	...
	if (loc != nullptr) {
		...
		$nc(errorReporter)->reportError(loc, $XSMessageFormatter::SCHEMA_DOMAIN, key, args, $XMLErrorReporter::SEVERITY_ERROR);
	} else {
		$init($XSMessageFormatter);
...
void XSConstraints::fullSchemaChecking($XSGrammarBucket* grammarBucket, $SubstitutionGroupHandler* SGHandler, $CMBuilder* cmBuilder, $XMLErrorReporter* errorReporter) {
		...
		for (int32_t i = 0; i < $nc(redefinedGroups)->length;) {
			...
			if (baseMG == nullptr) {
				...
				try {
					...
					reportSchemaError(errorReporter, rgLocators->get(i / 2 - 1), "src-redefine.6.2.2"_s, $$new($ObjectArray, {
						derivedGrp->fName,
						key
					}));
				}
	...
	for (int32_t i = grammars->length - 1, j = 0; i >= 0; --i) {
		keepType = 0;
		...
		for (j = 0; j < $nc(types)->length; ++j) {
			if (!fullChecked) {
				if ($nc(types->get(j))->fParticle != nullptr) {
					...
					try {
						...
						$var($SimpleLocator, var$0, $nc(ctLocators)->get(j));
						$var($String, var$1, e->getKey());
						reportSchemaError(errorReporter, var$0, var$1, $(e->getArgs()));
					}
				}
			}
			if ($nc(types->get(j))->fBaseType != nullptr && !$equals($nc(types->get(j))->fBaseType, $SchemaGrammar::fAnyType) && $nc(types->get(j))->fDerivedBy == $XSConstants::DERIVATION_RESTRICTION && ($instanceOf($XSComplexTypeDecl, $nc(types->get(j))->fBaseType))) {
				...
				$var($XSParticleDecl, baseParticle, $cast($XSComplexTypeDecl, $nc(types->get(j))->fBaseType)->fParticle);
				if (derivedParticle == nullptr) {
					if (baseParticle != nullptr && !baseParticle->emptiable()) {
						reportSchemaError(errorReporter, $nc(ctLocators)->get(j), "derivation-ok-restriction.5.3.2"_s, $$new($ObjectArray, {
							$nc(types->get(j))->fName,
							$($nc($nc(types->get(j))->fBaseType)->getName())
						}));
					}
					...
					try {
						...
						$var($SimpleLocator, var$2, $nc(ctLocators)->get(j));
						$var($String, var$3, e->getKey());
						reportSchemaError(errorReporter, var$2, var$3, $(e->getArgs()));
						reportSchemaError(errorReporter, ctLocators->get(j), "derivation-ok-restriction.5.4.2"_s, $$new($ObjectArray, {$nc(types->get(j))->fName}));
					}
				} else {
					reportSchemaError(errorReporter, $nc(ctLocators)->get(j), "derivation-ok-restriction.5.4.2"_s, $$new($ObjectArray, {$nc(types->get(j))->fName}));
				}
			}
			...
			if (cm != nullptr) {
				try {
					...
					reportSchemaError(errorReporter, var$4, var$5, $(e->getArgs()));
				}
			}
...
void XSConstraints::findElemInTable($XSComplexTypeDecl* type, $XSElementDecl* elem, $SymbolHash* elemDeclHash) {
	$init(XSConstraints);
	$useLocalObjectStack();
	$var($String, name, $str({$nc(elem)->fName, ","_s, $nc(elem)->fTargetNamespace}));
...
bool XSConstraints::particleValidRestriction($XSParticleDecl* dParticle$renamed, $SubstitutionGroupHandler* dSGHandler$renamed, $XSParticleDecl* bParticle$renamed, $SubstitutionGroupHandler* bSGHandler$renamed, bool checkWCOccurrence) {
	...
	if (var$0 && !$nc(bParticle)->emptiable()) {
		...
		bool var$1 = !dParticle->isEmpty();
		if (var$1 && $nc(bParticle)->isEmpty()) {
	...
	case $XSParticleDecl::PARTICLE_ELEMENT:
		{
			...
			default:
				{
					$throwNew($XMLSchemaException, "Internal-Error"_s, $$new($ObjectArray, {"in particleValidRestriction"_s}));
				}
			}
		}
	case $XSParticleDecl::PARTICLE_WILDCARD:
		{
			...
			case $XSParticleDecl::PARTICLE_ELEMENT:
				{
					$throwNew($XMLSchemaException, "cos-particle-restrict.2"_s, $$new($ObjectArray, {"any:choice,sequence,all,elt"_s}));
				}
			default:
				{
					$throwNew($XMLSchemaException, "Internal-Error"_s, $$new($ObjectArray, {"in particleValidRestriction"_s}));
	...
	case $XSModelGroupImpl::MODELGROUP_ALL:
		{
			...
			case $XSParticleDecl::PARTICLE_ELEMENT:
				{
					$throwNew($XMLSchemaException, "cos-particle-restrict.2"_s, $$new($ObjectArray, {"all:choice,sequence,elt"_s}));
				}
			default:
				{
					$throwNew($XMLSchemaException, "Internal-Error"_s, $$new($ObjectArray, {"in particleValidRestriction"_s}));
	...
	case $XSModelGroupImpl::MODELGROUP_CHOICE:
		{
			...
			case $XSParticleDecl::PARTICLE_ELEMENT:
				{
					$throwNew($XMLSchemaException, "cos-particle-restrict.2"_s, $$new($ObjectArray, {"choice:all,sequence,elt"_s}));
				}
			default:
				{
					$throwNew($XMLSchemaException, "Internal-Error"_s, $$new($ObjectArray, {"in particleValidRestriction"_s}));
	...
	case $XSModelGroupImpl::MODELGROUP_SEQUENCE:
		{
			...
			case $XSParticleDecl::PARTICLE_ELEMENT:
				{
					$throwNew($XMLSchemaException, "cos-particle-restrict.2"_s, $$new($ObjectArray, {"seq:elt"_s}));
				}
			default:
				{
					$throwNew($XMLSchemaException, "Internal-Error"_s, $$new($ObjectArray, {"in particleValidRestriction"_s}));
...
$XSParticleDecl* XSConstraints::getNonUnaryGroup($XSParticleDecl* p) {
	$init(XSConstraints);
	if ($nc(p)->fType == $XSParticleDecl::PARTICLE_ELEMENT || p->fType == $XSParticleDecl::PARTICLE_WILDCARD) {
		return p;
	}
	if (p->fMinOccurs == 1 && p->fMaxOccurs == 1 && p->fValue != nullptr && $cast($XSModelGroupImpl, p->fValue)->fParticleCount == 1) {
		return getNonUnaryGroup($nc($cast($XSModelGroupImpl, p->fValue)->fParticles)->get(0));
	} else {
		return p;
	}
}
...
void XSConstraints::checkNameAndTypeOK($XSElementDecl* dElement, int32_t dMin, int32_t dMax, $XSElementDecl* bElement, int32_t bMin, int32_t bMax) {
	$init(XSConstraints);
	$useLocalObjectStack();
	if ($nc(dElement)->fName != $nc(bElement)->fName || dElement->fTargetNamespace != bElement->fTargetNamespace) {
	...
	bool var$0 = !bElement->getNillable();
	if (var$0 && dElement->getNillable()) {
		$throwNew($XMLSchemaException, "rcase-NameAndTypeOK.2"_s, $$new($ObjectArray, {dElement->fName}));
	}
	if (!checkOccurrenceRange(dMin, dMax, bMin, bMax)) {
		$throwNew($XMLSchemaException, "rcase-NameAndTypeOK.3"_s, $$new($ObjectArray, {
			...
			dMax == $SchemaSymbols::OCCURRENCE_UNBOUNDED ? "unbounded"_s : $($Integer::toString(dMax)),
			$($Integer::toString(bMin)),
			bMax == $SchemaSymbols::OCCURRENCE_UNBOUNDED ? "unbounded"_s : $($Integer::toString(bMax))
		}));
	}
	if (bElement->getConstraintType() == $XSConstants::VC_FIXED) {
		if (dElement->getConstraintType() != $XSConstants::VC_FIXED) {
			$throwNew($XMLSchemaException, "rcase-NameAndTypeOK.4.a"_s, $$new($ObjectArray, {
				dElement->fName,
				$($nc(bElement->fDefault)->stringValue())
			}));
		}
		bool isSimple = false;
		if ($nc(dElement->fType)->getTypeCategory() == $XSTypeDefinition::SIMPLE_TYPE || $cast($XSComplexTypeDecl, dElement->fType)->fContentType == $XSComplexTypeDecl::CONTENTTYPE_SIMPLE) {
			isSimple = true;
		}
		bool var$1 = !isSimple && !$nc($nc(bElement->fDefault)->normalizedValue)->equals($nc(dElement->fDefault)->normalizedValue);
		if (var$1 || isSimple && !$nc($nc(bElement->fDefault)->actualValue)->equals($nc(dElement->fDefault)->actualValue)) {
	...
	int32_t blockSet1 = dElement->fBlock;
	int32_t blockSet2 = bElement->fBlock;
	if (((blockSet1 & blockSet2) != blockSet2) || (blockSet1 == $XSConstants::DERIVATION_NONE && blockSet2 != $XSConstants::DERIVATION_NONE)) {
		$throwNew($XMLSchemaException, "rcase-NameAndTypeOK.6"_s, $$new($ObjectArray, {dElement->fName}));
	}
	if (!checkTypeDerivationOk(dElement->fType, bElement->fType, (int16_t)(($XSConstants::DERIVATION_EXTENSION | $XSConstants::DERIVATION_LIST) | $XSConstants::DERIVATION_UNION))) {
		$throwNew($XMLSchemaException, "rcase-NameAndTypeOK.7"_s, $$new($ObjectArray, {
			dElement->fName,
			$($nc(dElement->fType)->getName()),
			$($nc(bElement->fType)->getName())
		}));
	}
}
...
void XSConstraints::checkNSCompat($XSElementDecl* elem, int32_t min1, int32_t max1, $XSWildcardDecl* wildcard, int32_t min2, int32_t max2, bool checkWCOccurrence) {
	...
	if (checkWCOccurrence && !checkOccurrenceRange(min1, max1, min2, max2)) {
		$throwNew($XMLSchemaException, "rcase-NSCompat.2"_s, $$new($ObjectArray, {
			...
			max1 == $SchemaSymbols::OCCURRENCE_UNBOUNDED ? "unbounded"_s : $($Integer::toString(max1)),
			$($Integer::toString(min2)),
			max2 == $SchemaSymbols::OCCURRENCE_UNBOUNDED ? "unbounded"_s : $($Integer::toString(max2))
		}));
	}
...
void XSConstraints::checkNSSubset($XSWildcardDecl* dWildcard, int32_t min1, int32_t max1, $XSWildcardDecl* bWildcard, int32_t min2, int32_t max2) {
	...
	if (dWildcard->weakerProcessContents(bWildcard)) {
		$throwNew($XMLSchemaException, "rcase-NSSubset.3"_s, $$new($ObjectArray, {
			...
			$($nc(bWildcard)->getProcessContentsAsString())
		}));
	}
}
...
bool XSConstraints::overlapUPA($XSElementDecl* element1, $XSElementDecl* element2, $SubstitutionGroupHandler* sgHandler) {
	...
	for (int32_t i = $nc(subGroup)->length - 1; i >= 0; --i) {
		if ($nc(subGroup->get(i))->fName == element2->fName && $nc(subGroup->get(i))->fTargetNamespace == element2->fTargetNamespace) {
			return true;
		}
	}
	$assign(subGroup, sgHandler->getSubstitutionGroup(element2));
	for (int32_t i = $nc(subGroup)->length - 1; i >= 0; --i) {
		if ($nc(subGroup->get(i))->fName == element1->fName && $nc(subGroup->get(i))->fTargetNamespace == element1->fTargetNamespace) {
			return true;
		}
	}
	return false;
...
bool XSConstraints::overlapUPA($XSElementDecl* element, $XSWildcardDecl* wildcard, $SubstitutionGroupHandler* sgHandler) {
	...
	for (int32_t i = $nc(subGroup)->length - 1; i >= 0; --i) {
		if (wildcard->allowNamespace($nc(subGroup->get(i))->fTargetNamespace)) {
			return true;
		}
	}
	return false;
...
bool XSConstraints::overlapUPA($XSWildcardDecl* wildcard1, $XSWildcardDecl* wildcard2) {
	$init(XSConstraints);
	$var($XSWildcardDecl, intersect, $nc(wildcard1)->performIntersectionWith(wildcard2, $nc(wildcard1)->fProcessContents));
	if (intersect == nullptr || intersect->fType != $XSWildcardDecl::NSCONSTRAINT_LIST || $nc(intersect->fNamespaceList)->length != 0) {
		return true;
	}
	return false;
}
...
void XSConstraints::clinit$($Class* clazz) {
	$init($SchemaGrammar);
	$init($SchemaSymbols);
...
$Class* XSConstraints::load$($String* name, bool initialize) {
	...
	$loadClass(XSConstraints, name, initialize, &classInfo$$, XSConstraints::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XSConstraints);
	});
	return class$;
}
