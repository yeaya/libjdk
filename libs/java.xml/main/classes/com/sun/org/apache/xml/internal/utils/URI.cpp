...
void URI::init$($String* p_scheme, $String* p_userinfo, $String* p_host, int32_t p_port, $String* p_path, $String* p_queryString, $String* p_fragment) {
	...
	if (p_scheme == nullptr || $(p_scheme->trim())->length() == 0) {
		...
		$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_SCHEME_REQUIRED, nullptr)));
	}
	if (p_host == nullptr) {
		if (p_userinfo != nullptr) {
			$init($XMLErrorResources);
			$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_NO_USERINFO_IF_NO_HOST, nullptr)));
		}
		if (p_port != -1) {
			$init($XMLErrorResources);
			$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_NO_PORT_IF_NO_HOST, nullptr)));
		}
	}
	if (p_path != nullptr) {
		if (p_path->indexOf(u'?') != -1 && p_queryString != nullptr) {
			$init($XMLErrorResources);
			$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_NO_QUERY_STRING_IN_PATH, nullptr)));
		}
		if (p_path->indexOf(u'#') != -1 && p_fragment != nullptr) {
			$init($XMLErrorResources);
			$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_NO_FRAGMENT_STRING_IN_PATH, nullptr)));
		}
	}
	setScheme(p_scheme);
	setHost(p_host);
...
void URI::initialize(URI* p_base$renamed, $String* p_uriSpec) {
	$useLocalObjectStack();
	$var(URI, p_base, p_base$renamed);
	if (p_base == nullptr && (p_uriSpec == nullptr || $(p_uriSpec->trim())->length() == 0)) {
		$init($XMLErrorResources);
		$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_CANNOT_INIT_URI_EMPTY_PARMS, nullptr)));
	}
	if (p_uriSpec == nullptr || $(p_uriSpec->trim())->length() == 0) {
		initialize(p_base);
		return;
	}
	...
	int32_t index = 0;
	int32_t colonIndex = uriSpec->indexOf(u':');
	if (colonIndex < 0) {
		if (p_base == nullptr) {
			$init($XMLErrorResources);
			$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_NO_SCHEME_IN_URI, $$new($ObjectArray, {uriSpec}))));
		}
	} else {
		initializeScheme(uriSpec);
		$assign(uriSpec, uriSpec->substring(colonIndex + 1));
		if (this->m_scheme != nullptr && p_base != nullptr) {
			bool var$1 = uriSpec->startsWith("/"_s);
			bool var$0 = var$1 || !this->m_scheme->equals(p_base->m_scheme);
			if (var$0 || !$$nc(p_base->getSchemeSpecificPart())->startsWith("/"_s)) {
				$assign(p_base, nullptr);
			}
		}
	...
	if (p_base != nullptr) {
		...
		bool var$2 = $nc(this->m_path)->length() > 0;
		if (var$2 && this->m_path->startsWith("/"_s)) {
			return;
		}
		$var($String, path, ""_s);
...
void URI::initializeScheme($String* p_uriSpec) {
	...
	if (scheme->length() == 0) {
		$init($XMLErrorResources);
		$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_NO_SCHEME_INURI, nullptr)));
	} else {
		setScheme(scheme);
	}
}
	...
	while (index < end) {
		...
		if (testChar == u'%') {
			...
			if (var$0 || !isHex(p_uriSpec->charAt(index + 2))) {
				$init($XMLErrorResources);
				$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_PATH_CONTAINS_INVALID_ESCAPE_SEQUENCE, nullptr)));
			}
		} else {
			...
			if (var$1 && !isUnreservedCharacter(testChar)) {
				if (u'\\' != testChar) {
					$init($XMLErrorResources);
					$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_PATH_INVALID_CHAR, $$new($ObjectArray, {$($String::valueOf(testChar))}))));
				}
			}
		}
		++index;
	...
	if (testChar == u'?') {
		...
		while (index < end) {
			...
			if (testChar == u'%') {
				...
				bool var$3 = !isReservedCharacter(testChar);
				if (var$3 && !isUnreservedCharacter(testChar)) {
...
void URI::setScheme($String* p_scheme) {
	...
	if (p_scheme == nullptr) {
		$init($XMLErrorResources);
		$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_SCHEME_FROM_NULL_STRING, nullptr)));
	}
	if (!isConformantSchemeName(p_scheme)) {
		$init($XMLErrorResources);
		$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_SCHEME_NOT_CONFORMANT, nullptr)));
	}
	...
	if (p_userinfo == nullptr) {
		...
		while (index < end) {
			...
			if (testChar == u'%') {
				...
				bool var$1 = !isUnreservedCharacter(testChar);
				if (var$1 && URI::USERINFO_CHARACTERS->indexOf(testChar) == -1) {
...
void URI::setHost($String* p_host) {
	$useLocalObjectStack();
	if (p_host == nullptr || $(p_host->trim())->length() == 0) {
		$set(this, m_host, p_host);
		$set(this, m_userinfo, nullptr);
		this->m_port = -1;
	} else if (!isWellFormedAddress(p_host)) {
		$init($XMLErrorResources);
		$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_HOST_ADDRESS_NOT_WELLFORMED, nullptr)));
	}
	$set(this, m_host, p_host);
}
...
void URI::setPort(int32_t p_port) {
	...
	if (p_port >= 0 && p_port <= 0x0000ffff) {
		if (this->m_host == nullptr) {
			$init($XMLErrorResources);
			$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_PORT_WHEN_HOST_NULL, nullptr)));
		}
	} else if (p_port != -1) {
		$init($XMLErrorResources);
		$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_INVALID_PORT, nullptr)));
	}
	this->m_port = p_port;
}
...
void URI::appendPath($String* p_addToPath) {
	...
	if (!isURIString(p_addToPath)) {
		$init($XMLErrorResources);
		$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_PATH_INVALID_CHAR, $$new($ObjectArray, {p_addToPath}))));
	}
...
void URI::setFragment($String* p_fragment) {
	...
	if (p_fragment == nullptr) {
		...
		$init($XMLErrorResources);
		$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_FRAG_FOR_GENERIC_URI, nullptr)));
	} else if (getPath() == nullptr) {
		$init($XMLErrorResources);
		$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_FRAG_WHEN_PATH_NULL, nullptr)));
	} else if (!isURIString(p_fragment)) {
		$init($XMLErrorResources);
		$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_FRAG_INVALID_CHAR, nullptr)));
	} else {
...
bool URI::isConformantSchemeName($String* p_scheme) {
	...
	char16_t testChar = 0;
	for (int32_t i = 1; i < p_scheme->length(); ++i) {
		testChar = p_scheme->charAt(i);
		bool var$0 = !isAlphanum(testChar);
		if (var$0 && URI::SCHEME_CHARACTERS->indexOf(testChar) == -1) {
			return false;
		}
	}
	return true;
...
bool URI::isReservedCharacter(char16_t p_char) {
	$init(URI);
	return URI::RESERVED_CHARACTERS->indexOf(p_char) != -1;
}

bool URI::isUnreservedCharacter(char16_t p_char) {
	$init(URI);
	bool var$0 = isAlphanum(p_char);
	return (var$0 || URI::MARK_CHARACTERS->indexOf(p_char) != -1);
}
...
$Class* URI::load$($String* name, bool initialize) {
	$FieldInfo fieldInfos$$[] = {
		{"serialVersionUID", "J", nullptr, $PRIVATE | $STATIC | $FINAL, $constField(URI, serialVersionUID)},
	...
	$MethodInfo methodInfos$$[] = {
		...
		{"<init>", "(Lcom/sun/org/apache/xml/internal/utils/URI;)V", nullptr, $PUBLIC, $method(URI, init$, void, URI*)},
		{"<init>", "(Ljava/lang/String;)V", nullptr, $PUBLIC, $method(URI, init$, void, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		{"<init>", "(Lcom/sun/org/apache/xml/internal/utils/URI;Ljava/lang/String;)V", nullptr, $PUBLIC, $method(URI, init$, void, URI*, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		{"<init>", "(Ljava/lang/String;Ljava/lang/String;)V", nullptr, $PUBLIC, $method(URI, init$, void, $String*, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		{"<init>", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V", nullptr, $PUBLIC, $method(URI, init$, void, $String*, $String*, $String*, $String*, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		{"<init>", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;ILjava/lang/String;Ljava/lang/String;Ljava/lang/String;)V", nullptr, $PUBLIC, $method(URI, init$, void, $String*, $String*, $String*, int32_t, $String*, $String*, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		{"appendPath", "(Ljava/lang/String;)V", nullptr, $PUBLIC, $virtualMethod(URI, appendPath, void, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		...
		{"initialize", "(Lcom/sun/org/apache/xml/internal/utils/URI;)V", nullptr, $PRIVATE, $method(URI, initialize, void, URI*)},
		{"initialize", "(Lcom/sun/org/apache/xml/internal/utils/URI;Ljava/lang/String;)V", nullptr, $PRIVATE, $method(URI, initialize, void, URI*, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		{"initializeAuthority", "(Ljava/lang/String;)V", nullptr, $PRIVATE, $method(URI, initializeAuthority, void, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		{"initializePath", "(Ljava/lang/String;)V", nullptr, $PRIVATE, $method(URI, initializePath, void, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		{"initializeScheme", "(Ljava/lang/String;)V", nullptr, $PRIVATE, $method(URI, initializeScheme, void, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		...
		{"setFragment", "(Ljava/lang/String;)V", nullptr, $PUBLIC, $virtualMethod(URI, setFragment, void, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		{"setHost", "(Ljava/lang/String;)V", nullptr, $PUBLIC, $virtualMethod(URI, setHost, void, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		{"setPath", "(Ljava/lang/String;)V", nullptr, $PUBLIC, $virtualMethod(URI, setPath, void, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		{"setPort", "(I)V", nullptr, $PUBLIC, $virtualMethod(URI, setPort, void, int32_t), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		{"setQueryString", "(Ljava/lang/String;)V", nullptr, $PUBLIC, $virtualMethod(URI, setQueryString, void, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		{"setScheme", "(Ljava/lang/String;)V", nullptr, $PUBLIC, $virtualMethod(URI, setScheme, void, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
		{"setUserinfo", "(Ljava/lang/String;)V", nullptr, $PUBLIC, $virtualMethod(URI, setUserinfo, void, $String*), "com.sun.org.apache.xml.internal.utils.URI$MalformedURIException"},
