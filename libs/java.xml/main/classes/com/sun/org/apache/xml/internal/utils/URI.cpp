...
void URI::initialize(URI* p_base$renamed, $String* p_uriSpec) {
	...
	if (colonIndex < 0) {
		if (p_base == nullptr) {
			$init($XMLErrorResources);
			$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_NO_SCHEME_IN_URI, $$new($ObjectArray, {$of(uriSpec)}))));
		}
	} else {
		...
		if (this->m_scheme != nullptr && p_base != nullptr) {
			...
			bool var$0 = var$1 || !$nc(this->m_scheme)->equals(p_base->m_scheme);
	...
	if (p_base != nullptr) {
		...
		if (var$2 && $nc(this->m_path)->startsWith("/"_s)) {
			return;
		}
		$var($String, path, ""_s);
	...
	while (index < end) {
		...
		if (testChar == u'%') {
			...
			if (var$2 && !isUnreservedCharacter(testChar)) {
				if (u'\\' != testChar) {
					$init($XMLErrorResources);
					$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_PATH_INVALID_CHAR, $$new($ObjectArray, {$($of($String::valueOf(testChar)))}))));
				}
			}
		}
		++index;
...
void URI::appendPath($String* p_addToPath) {
	...
	if (!isURIString(p_addToPath)) {
		$init($XMLErrorResources);
		$throwNew($URI$MalformedURIException, $($XMLMessages::createXMLMessage($XMLErrorResources::ER_PATH_INVALID_CHAR, $$new($ObjectArray, {$of(p_addToPath)}))));
	}
