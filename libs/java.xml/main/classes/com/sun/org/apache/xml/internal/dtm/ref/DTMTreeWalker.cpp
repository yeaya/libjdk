		...
		case $DTM::ELEMENT_NODE:
			{
				...
				$assign(ns, dtm->getNamespaceURI(node));
				if (nullptr == ns) {
					$assign(ns, ""_s);
				}
	...
	case $DTM::ELEMENT_NODE:
		{
			...
			$var($String, var$0, this->m_dtm->getLocalName(node));
			$nc(this->m_contentHandler)->endElement(ns, var$0, $(this->m_dtm->getNodeName(node)));
...
$Class* DTMTreeWalker::load$($String* name, bool initialize) {
	...
	$loadClass(DTMTreeWalker, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DTMTreeWalker);
	});
	return class$;
}
