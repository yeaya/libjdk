...
void ApplyTemplates::translate($ClassGenerator* classGen, $MethodGenerator* methodGen) {
	...
	if ((this->_type != nullptr) && ($instanceOf($ResultTreeType, this->_type))) {
		if (sortObjects->size() > 0) {
			$init($ErrorMsg);
			$var($ErrorMsg, err, $new($ErrorMsg, $ErrorMsg::RESULT_TREE_SORT_ERR, this));
	...
	il->append($$new($INVOKEVIRTUAL, applyTemplates));
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			if ($instanceOf($WithParam, child)) {
				$cast($WithParam, child)->releaseResultTree(classGen, methodGen);
			}
		}
	}
	bool var$1 = stylesheet->hasLocalParams();
...
$Class* ApplyTemplates::load$($String* name, bool initialize) {
	...
	$loadClass(ApplyTemplates, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ApplyTemplates);
	});
	return class$;
}
