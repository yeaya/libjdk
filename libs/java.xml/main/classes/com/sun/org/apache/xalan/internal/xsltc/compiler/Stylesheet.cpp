...
$Class* Stylesheet$$Lambda$lambda$processModes$0::load$($String* name, bool initialize) {
	...
	$loadClass(Stylesheet$$Lambda$lambda$processModes$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Stylesheet$$Lambda$lambda$processModes$0);
	});
	return class$;
}
...
$Class* Stylesheet$$Lambda$lambda$compileModes$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(Stylesheet$$Lambda$lambda$compileModes$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Stylesheet$$Lambda$lambda$compileModes$1$1);
	});
	return class$;
}
...
void Stylesheet::checkOutputMethod() {
	if (this->_lastOutputElement != nullptr) {
		$var($String, method, this->_lastOutputElement->getOutputMethod());
		if (method != nullptr) {
...
void Stylesheet::setImportPrecedence(int32_t precedence) {
	...
	while ($nc(elements)->hasNext()) {
		...
		if ($instanceOf($Include, child)) {
			$var(Stylesheet, included, $cast($Include, child)->getIncludedStylesheet());
	...
	if (this->_importedFrom != nullptr) {
		...
		if (this->_includedFrom->getImportPrecedence() != precedence) {
...
int32_t Stylesheet::getMinimumDescendantPrecedence() {
	...
	if (this->_minimumDescendantPrecedence == -1) {
		...
		int32_t inclImpCount = (this->_includedStylesheets != nullptr) ? this->_includedStylesheets->size() : 0;
		for (int32_t i = 0; i < inclImpCount; ++i) {
			int32_t prec = $$sure(Stylesheet, $nc(this->_includedStylesheets)->get(i))->getMinimumDescendantPrecedence();
			if (prec < min) {
				min = prec;
			}
		}
...
bool Stylesheet::checkForLoop($String* systemId) {
	if (this->_systemId != nullptr && this->_systemId->equals(systemId)) {
		return true;
	}
...
bool Stylesheet::isExtension($String* uri) {
	return (this->_extensions->get(uri) != nullptr);
}
...
void Stylesheet::parseContents($Parser* parser) {
	...
	if (sheet != nullptr) {
		$init($ErrorMsg);
		$var($ErrorMsg, err, $new($ErrorMsg, $ErrorMsg::MULTIPLE_STYLESHEET_ERR, this));
...
void Stylesheet::parseOwnChildren($Parser* parser) {
	...
	for (int32_t i = 0; i < count; ++i) {
		...
		if (!this->_templateInlining && ($instanceOf($Template, child))) {
			...
			$var($String, name, $str({"template$dot$"_s, $$str(template$->getPosition())}));
...
void Stylesheet::processModes() {
	...
	$$nc($$nc(this->_modes->values())->stream())->forEach($$new(Stylesheet$$Lambda$lambda$processModes$0, this));
}

void Stylesheet::compileModes($ClassGenerator* classGen) {
	$useLocalObjectStack();
	$nc(this->_defaultMode)->compileApplyTemplates(classGen);
	$$nc($$nc(this->_modes->values())->stream())->forEach($$new(Stylesheet$$Lambda$lambda$compileModes$1$1, classGen));
}
...
$Mode* Stylesheet::getMode($QName* modeName) {
	...
	if (modeName == nullptr) {
		...
		$var($Mode, mode, $cast($Mode, this->_modes->get($(modeName->getStringRep()))));
		if (mode == nullptr) {
			$var($String, suffix, $Integer::toString(this->_nextModeSerial++));
			$var($Object, var$0, modeName->getStringRep());
			this->_modes->put(var$0, $assign(mode, $new($Mode, modeName, this, suffix)));
		}
		return mode;
	}
}
...
void Stylesheet::addDOMField($ClassGenerator* classGen) {
	$useLocalObjectStack();
	$init($Constants);
	$var($1Type, var$0, $Util::getJCRefType($Constants::DOM_INTF_SIG));
	$var($String, var$1, $Constants::DOM_FIELD);
	$var($FieldGen, fgen, $new($FieldGen, $Constants::ACC_PUBLIC, var$0, var$1, $($nc(classGen)->getConstantPool())));
	classGen->addField($(fgen->getField()));
}

void Stylesheet::addStaticField($ClassGenerator* classGen, $String* type, $String* name) {
	$useLocalObjectStack();
	$var($1Type, var$0, $Util::getJCRefType(type));
	$var($FieldGen, fgen, $new($FieldGen, $Constants::ACC_PROTECTED | $Constants::ACC_STATIC, var$0, name, $($nc(classGen)->getConstantPool())));
	classGen->addField($(fgen->getField()));
}
...
void Stylesheet::translate() {
	...
	while ($nc(elements)->hasNext()) {
		...
		if ($instanceOf($Template, element)) {
			...
			$$nc(getMode($(template$->getModeName())))->addTemplate(template$);
		} else if ($instanceOf($AttributeSet, element)) {
			$cast($AttributeSet, element)->translate(classGen, nullptr);
...
void Stylesheet::compileStaticInitializer($ClassGenerator* classGen) {
	...
	for (int32_t i = 0; i < size; ++i) {
		...
		if ((index = $nc(encodedName)->lastIndexOf(u':')) > -1) {
			urisArray->set(i, $(encodedName->substring(0, index)));
		}
		index = index + 1;
		if (encodedName->charAt(index) == u'@') {
	...
	int32_t namesArrayRef = cpg->addFieldref(this->_className, $Constants::STATIC_NAMES_ARRAY_FIELD, $Constants::NAMES_INDEX_SIG);
	...
	for (int32_t i = 0; i < size; ++i) {
		...
		il->append($Constants::IASTORE);
	}
	$var($List, namespaces, $$nc(getXSLTC())->getNamespaceIndex());
	staticConst->markChunkStart();
	il->append($$new($PUSH, cpg, $nc(namespaces)->size()));
	il->append($$new($ANEWARRAY, cpg->addClass($Constants::STRING)));
	...
	staticConst->markChunkEnd();
	for (int32_t i = 0; i < namespaces->size(); ++i) {
	...
	for (int32_t i = 0; i < charDataCount; ++i) {
		staticConst->markChunkStart();
		il->append($$new($PUSH, cpg, $($$nc(getXSLTC())->getCharacterData(i))));
		il->append($$new($INVOKEVIRTUAL, toCharArray));
		il->append($$new($PUTSTATIC, cpg->addFieldref(this->_className, $$str({$Constants::STATIC_CHAR_DATA_FIELD, $$str(i)}), $Constants::STATIC_CHAR_DATA_FIELD_SIG)));
		staticConst->markChunkEnd();
	}
...
void Stylesheet::compileConstructor($ClassGenerator* classGen, $Output* output) {
	...
	$init($Constants);
	il->append($$new($INVOKESPECIAL, $nc(cpg)->addMethodref($Constants::TRANSLET_CLASS, "<init>"_s, "()V"_s)));
	constructor->markChunkStart();
	il->append($(classGen->loadTranslet()));
	il->append($$new($GETSTATIC, cpg->addFieldref(this->_className, $Constants::STATIC_NAMES_ARRAY_FIELD, $Constants::NAMES_INDEX_SIG)));
	il->append($$new($PUTFIELD, cpg->addFieldref($Constants::TRANSLET_CLASS, $Constants::NAMES_INDEX, $Constants::NAMES_INDEX_SIG)));
	il->append($(classGen->loadTranslet()));
	il->append($$new($GETSTATIC, cpg->addFieldref(this->_className, $Constants::STATIC_URIS_ARRAY_FIELD, $Constants::URIS_INDEX_SIG)));
	il->append($$new($PUTFIELD, cpg->addFieldref($Constants::TRANSLET_CLASS, $Constants::URIS_INDEX, $Constants::URIS_INDEX_SIG)));
	...
	constructor->markChunkStart();
	il->append($(classGen->loadTranslet()));
	il->append($$new($GETSTATIC, cpg->addFieldref(this->_className, $Constants::STATIC_NAMESPACE_ARRAY_FIELD, $Constants::NAMESPACE_INDEX_SIG)));
	il->append($$new($PUTFIELD, cpg->addFieldref($Constants::TRANSLET_CLASS, $Constants::NAMESPACE_INDEX, $Constants::NAMESPACE_INDEX_SIG)));
	constructor->markChunkEnd();
	constructor->markChunkStart();
	il->append($(classGen->loadTranslet()));
	il->append($$new($PUSH, cpg, $AbstractTranslet::CURRENT_TRANSLET_VERSION));
	il->append($$new($PUTFIELD, cpg->addFieldref($Constants::TRANSLET_CLASS, $Constants::TRANSLET_VERSION_INDEX, $Constants::TRANSLET_VERSION_INDEX_SIG)));
	constructor->markChunkEnd();
	if (this->_hasIdCall) {
		constructor->markChunkStart();
		il->append($(classGen->loadTranslet()));
...
$String* Stylesheet::compileTopLevel($ClassGenerator* classGen) {
	...
	$nc(current)->setStart($(il->append($$new($ISTORE, $nc(current)->getIndex()))));
	$var($List, varDepElements, $new($ArrayList, this->_globals));
	$var($Iterator, elements, this->elements());
	while ($nc(elements)->hasNext()) {
		$var($SyntaxTreeNode, element, $cast($SyntaxTreeNode, elements->next()));
	...
	while ($nc(elements)->hasNext()) {
		...
		if ($instanceOf($DecimalFormatting, element)) {
			$cast($DecimalFormatting, element)->translate(classGen, toplevel);
		} else if ($instanceOf($Whitespace, element)) {
			whitespaceRules->addAll($($cast($Whitespace, element)->getRules()));
		}
	}
...
$List* Stylesheet::resolveDependencies($List* input) {
	...
	while ($nc(input)->size() > 0) {
		...
		if (!changed) {
			$init($ErrorMsg);
			$var($ErrorMsg, err, $new($ErrorMsg, $ErrorMsg::CIRCULAR_VARIABLE_ERR, $($of(input)->toString()), this));
...
void Stylesheet::compileTransform($ClassGenerator* classGen) {
	...
	il->append($$new($INVOKEVIRTUAL, cpg->addMethodref($Constants::TRANSLET_CLASS, "makeDOMAdapter"_s, $$str({"("_s, $Constants::DOM_INTF_SIG, ")"_s, $Constants::DOM_ADAPTER_SIG}))));
	if (isMultiDocument()) {
...
void Stylesheet::peepHoleOptimization($MethodGenerator* methodGen) {
	...
	$var($InstructionFinder, find, $new($InstructionFinder, il));
	{
		...
		for (; $nc(iter)->hasNext();) {
			...
			try {
				$nc(il)->delete$($nc(match)->get(0), $nc(match)->get(1));
...
$Class* Stylesheet::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.org.apache.xalan.internal.xsltc.compiler.Stylesheet$$Lambda$lambda$processModes$0")) {
			return Stylesheet$$Lambda$lambda$processModes$0::load$(name, initialize);
		}
		if (name->equals("com.sun.org.apache.xalan.internal.xsltc.compiler.Stylesheet$$Lambda$lambda$compileModes$1$1")) {
	...
	$loadClass(Stylesheet, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Stylesheet);
	});
	return class$;
}
