...
$Class* KeyIndex$KeyIndexIterator$KeyIndexHeapNode::load$($String* name, bool initialize) {
	...
	$loadClass(KeyIndex$KeyIndexIterator$KeyIndexHeapNode, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(KeyIndex$KeyIndexIterator$KeyIndexHeapNode);
	});
	return class$;
}
