...
void UnionPathExpr::setParser($Parser* parser) {
	...
	$set(this, _components, $cast($ExpressionArray, components->toArray($$new($ExpressionArray, size))));
	for (int32_t i = 0; i < size; ++i) {
		...
		if ($instanceOf($Step, $nc(this->_components)->get(i))) {
			$var($Step, step, $cast($Step, this->_components->get(i)));
...
void UnionPathExpr::flatten($List* components) {
	...
	if (this->_rest != nullptr) {
		if ($instanceOf(UnionPathExpr, this->_rest)) {
			$cast(UnionPathExpr, this->_rest)->flatten(components);
		} else {
...
void UnionPathExpr::translate($ClassGenerator* classGen, $MethodGenerator* methodGen) {
	...
	$nc(il)->append($$new($NEW, cpg->addClass($Constants::UNION_ITERATOR_CLASS)));
...
$Class* UnionPathExpr::load$($String* name, bool initialize) {
	...
	$loadClass(UnionPathExpr, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(UnionPathExpr);
	});
	return class$;
}
