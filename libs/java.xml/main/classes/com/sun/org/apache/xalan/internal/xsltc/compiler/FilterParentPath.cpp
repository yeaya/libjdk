...
void FilterParentPath::translate($ClassGenerator* classGen, $MethodGenerator* methodGen) {
	...
	$nc(filterTemp)->setStart($($nc(il)->append($$new($ASTORE, $nc(filterTemp)->getIndex()))));
	...
	$nc(pathTemp)->setStart($(il->append($$new($ASTORE, $nc(pathTemp)->getIndex()))));
...
$Class* FilterParentPath::load$($String* name, bool initialize) {
	...
	$loadClass(FilterParentPath, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FilterParentPath);
	});
	return class$;
}
