...
bool XSDDescription::equals(Object$* descObj) {
	...
	if (this->fNamespace != nullptr) {
		return this->fNamespace->equals($($nc(desc)->getTargetNamespace()));
	} else {
...
int32_t XSDDescription::hashCode() {
	return (this->fNamespace == nullptr) ? 0 : this->fNamespace->hashCode();
}
...
$Class* XSDDescription::load$($String* name, bool initialize) {
	...
	$loadClass(XSDDescription, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
