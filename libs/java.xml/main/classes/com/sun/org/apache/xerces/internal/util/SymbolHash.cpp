...
int32_t SymbolHash::getValues($ObjectArray* elements, int32_t from) {
	$useLocalObjectStack();
	for (int32_t i = 0, j = 0; i < this->fTableSize && j < this->fNum; ++i) {
...
$ObjectArray* SymbolHash::getEntries() {
	...
	for (int32_t i = 0, j = 0; i < this->fTableSize && j < this->fNum << 1; ++i) {
		$var($SymbolHash$Entry, entry, $nc(this->fBuckets)->get(i));
		for (; entry != nullptr; $assign(entry, entry->next)) {
...
SymbolHash* SymbolHash::makeClone() {
	...
	$set(newTable, fHashMultipliers, this->fHashMultipliers != nullptr ? $cast($ints, this->fHashMultipliers->clone()) : ($ints*)nullptr);
	for (int32_t i = 0; i < this->fTableSize; ++i) {
		if ($nc(this->fBuckets)->get(i) != nullptr) {
			$nc(newTable->fBuckets)->set(i, $($nc(this->fBuckets->get(i))->makeClone()));
		}
	}
	return newTable;
}
...
int32_t SymbolHash::hash(Object$* key) {
	if (this->fHashMultipliers == nullptr || !($instanceOf($String, key))) {
		return $nc($of(key))->hashCode() & 0x7fffffff;
	}
...
int32_t SymbolHash::hash0($String* symbol) {
	...
	for (int32_t i = 0; i < length; ++i) {
		code = code * $nc(multipliers)->get(i & SymbolHash::MULTIPLIERS_MASK) + symbol->charAt(i);
	}
...
$Class* SymbolHash::load$($String* name, bool initialize) {
	...
	$loadClass(SymbolHash, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SymbolHash);
	});
	return class$;
}
