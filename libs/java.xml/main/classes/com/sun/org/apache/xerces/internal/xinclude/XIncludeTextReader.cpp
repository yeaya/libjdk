...
$Reader* XIncludeTextReader::getReader($XMLInputSource* source) {
	...
	if ($nc(source)->getCharacterStream() != nullptr) {
		...
		if (source->getByteStream() != nullptr) {
			...
			if ($instanceOf($HttpURLConnection, urlCon) && $instanceOf($HTTPInputSource, source)) {
				...
				while ($nc(propIter)->hasNext()) {
					...
					urlConnection->setRequestProperty(var$1, $$cast($String, entry->getValue()));
				}
			...
			$var($String, rawContentType, urlCon->getContentType());
			int32_t index = (rawContentType != nullptr) ? rawContentType->indexOf(u';') : -1;
			$var($String, contentType, nullptr);
			$var($String, charset, nullptr);
			if (index != -1) {
				$assign(contentType, $($nc(rawContentType)->substring(0, index))->trim());
		...
		if (javaEncoding == nullptr) {
			...
			$throwNew($IOException, $($nc(aFormatter)->formatMessage(aLocale, "EncodingDeclInvalid"_s, $$new($ObjectArray, {encoding}))));
...
$Reader* XIncludeTextReader::createUTF8Reader($InputStream* stream) {
	$useLocalObjectStack();
	int32_t var$0 = $nc($nc(this->fTempString)->ch)->length;
	$init($XMLMessageFormatter);
	$var($MessageFormatter, var$1, $nc(this->fErrorReporter)->getMessageFormatter($XMLMessageFormatter::XML_DOMAIN));
	return $new($UTF8Reader, stream, var$0, var$1, $(this->fErrorReporter->getLocale()));
}

$Reader* XIncludeTextReader::createUTF16Reader($InputStream* stream, bool isBigEndian) {
	$useLocalObjectStack();
	int32_t var$0 = $nc($nc(this->fTempString)->ch)->length << 1;
	$init($XMLMessageFormatter);
	$var($MessageFormatter, var$1, $nc(this->fErrorReporter)->getMessageFormatter($XMLMessageFormatter::XML_DOMAIN));
	return $new($UTF16Reader, stream, var$0, isBigEndian, var$1, $(this->fErrorReporter->getLocale()));
}
...
$Reader* XIncludeTextReader::createASCIIReader($InputStream* stream) {
	...
	$init($XMLMessageFormatter);
	$var($MessageFormatter, var$1, $nc(this->fErrorReporter)->getMessageFormatter($XMLMessageFormatter::XML_DOMAIN));
	return $new($ASCIIReader, stream, var$0, var$1, $(this->fErrorReporter->getLocale()));
}
...
$String* XIncludeTextReader::consumeBOM($InputStream* stream, $String* encoding) {
	...
	if ($nc(encoding)->equals("UTF-8"_s)) {
		...
		if (count == 3) {
			int32_t b0 = b->get(0) & 0xff;
			int32_t b1 = b->get(1) & 0xff;
			int32_t b2 = b->get(2) & 0xff;
...
void XIncludeTextReader::parse() {
	...
	while (readSize != -1) {
			...
			if (!isValid(ch)) {
				if ($XMLChar::isHighSurrogate(ch)) {
					...
					if ($XMLChar::isLowSurrogate(ch2)) {
						...
						if (!isValid(sup)) {
							$init($XMLMessageFormatter);
							$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "InvalidCharInContent"_s, $$new($ObjectArray, {$($Integer::toString(sup, 16))}), $XMLErrorReporter::SEVERITY_FATAL_ERROR);
						}
					} else {
						$init($XMLMessageFormatter);
						$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "InvalidCharInContent"_s, $$new($ObjectArray, {$($Integer::toString(ch2, 16))}), $XMLErrorReporter::SEVERITY_FATAL_ERROR);
					}
				} else {
					$init($XMLMessageFormatter);
					$nc(this->fErrorReporter)->reportError($XMLMessageFormatter::XML_DOMAIN, "InvalidCharInContent"_s, $$new($ObjectArray, {$($Integer::toString(ch, 16))}), $XMLErrorReporter::SEVERITY_FATAL_ERROR);
				}
			}
		}
		if (this->fHandler != nullptr && readSize > 0) {
			...
			this->fHandler->characters(this->fTempString, $(this->fHandler->modifyAugmentations(nullptr, true)));
		}
...
void XIncludeTextReader::setBufferSize(int32_t bufferSize) {
	if ($nc($nc(this->fTempString)->ch)->length != ++bufferSize) {
		$set(this->fTempString, ch, $new($chars, bufferSize));
	}
}
...
$Class* XIncludeTextReader::load$($String* name, bool initialize) {
	...
	$loadClass(XIncludeTextReader, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XIncludeTextReader);
	});
	return class$;
}
