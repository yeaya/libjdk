	...
	case 66:
		{
			...
			if ($instanceOf($Step, rlp) && $nc(this->parser)->isElementAxis($cast($Step, rlp)->getAxis())) {
	...
	case 70:
		{
			...
			if ($instanceOf($Step, rlp) && $cast($Step, rlp)->isAbbreviatedDot()) {
				$assign(result, step);
	...
	case 75:
		{
			...
			if ((axis == $Axis::CHILD) && (type != $NodeTest::ATTRIBUTE)) {
				...
				} else if ($instanceOf($Step, rlp) && $cast($Step, rlp)->isAbbreviatedDot()) {
	...
	case 103:
		{
			...
			int32_t index = $nc(string)->lastIndexOf(u':');
			if (index > 0) {
				$var($String, prefix, string->substring(0, index));
				$assign(namespace$, $nc($nc(this->parser)->_symbolTable)->lookupNamespace(prefix));
			}
			$var($Expression, result, (namespace$ == nullptr) ? $new($LiteralExpr, string) : $new($LiteralExpr, string, namespace$));
	...
	case 104:
		{
			...
			if (var$106 || num->longValue() > $Integer::MAX_VALUE) {
	...
	case 108:
		{
			...
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("number"_s)) {
				$assign(result, $new($NumberCall, fname, $XPathParser::EmptyArgs));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("string"_s)) {
				$assign(result, $new($StringCall, fname, $XPathParser::EmptyArgs));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("concat"_s)) {
				$assign(result, $new($ConcatCall, fname, $XPathParser::EmptyArgs));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("true"_s)) {
				$assign(result, $new($BooleanExpr, true));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("false"_s)) {
				$assign(result, $new($BooleanExpr, false));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("name"_s)) {
				$assign(result, $new($NameCall, fname));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("generate-id"_s)) {
				$assign(result, $new($GenerateIdCall, fname, $XPathParser::EmptyArgs));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("string-length"_s)) {
				$assign(result, $new($StringLengthCall, fname, $XPathParser::EmptyArgs));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("position"_s)) {
				$assign(result, $new($PositionCall, fname));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("last"_s)) {
				$assign(result, $new($LastCall, fname));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("local-name"_s)) {
				$assign(result, $new($LocalNameCall, fname));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("namespace-uri"_s)) {
	...
	case 109:
		{
			...
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("document"_s)) {
				this->parser->setMultiDocument(true);
				$assign(result, $new($DocumentCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("string"_s)) {
				$assign(result, $new($StringCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("boolean"_s)) {
				$assign(result, $new($BooleanCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("name"_s)) {
				$assign(result, $new($NameCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("generate-id"_s)) {
				$assign(result, $new($GenerateIdCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("not"_s)) {
				$assign(result, $new($NotCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("format-number"_s)) {
				$assign(result, $new($FormatNumberCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("unparsed-entity-uri"_s)) {
				$assign(result, $new($UnparsedEntityUriCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("key"_s)) {
				$assign(result, $new($KeyCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("id"_s)) {
				$assign(result, $new($KeyCall, fname, argl));
				this->parser->setHasIdCall(true);
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("ceiling"_s)) {
				$assign(result, $new($CeilingCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("round"_s)) {
				$assign(result, $new($RoundCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("floor"_s)) {
				$assign(result, $new($FloorCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("contains"_s)) {
				$assign(result, $new($ContainsCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("string-length"_s)) {
				$assign(result, $new($StringLengthCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("starts-with"_s)) {
				$assign(result, $new($StartsWithCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("function-available"_s)) {
				$assign(result, $new($FunctionAvailableCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("element-available"_s)) {
				$assign(result, $new($ElementAvailableCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("local-name"_s)) {
				$assign(result, $new($LocalNameCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("lang"_s)) {
				$assign(result, $new($LangCall, fname, argl));
			} else if (fname == this->parser->getQNameIgnoreDefaultNs("namespace-uri"_s)) {
				$assign(result, $new($NamespaceUriCall, fname, argl));
			} else {
				$init($Constants);
				if (fname == this->parser->getQName($Constants::TRANSLET_URI, "xsltc"_s, "cast"_s)) {
					$assign(result, $new($CastCall, fname, argl));
				} else {
					bool var$112 = $$nc($nc(fname)->getLocalPart())->equals("nodeset"_s);
					if (var$112 || $$nc(fname->getLocalPart())->equals("node-set"_s)) {
			...
			int32_t var$113 = $nc($$cast($Symbol, parser_stack->get(parser_top - 3)))->left;
			$assign(parser_result, $new($Symbol, 16, var$113, $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->right, result));
		}
		return parser_result;
	case 110:
		{
			...
			$var($List, temp, $new($ArrayList));
			temp->add(arg);
			int32_t var$114 = $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->left;
			$assign(parser_result, $new($Symbol, 36, var$114, $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->right, temp));
		}
		return parser_result;
	case 111:
		{
			...
			$nc(argl)->add(0, arg);
			int32_t var$115 = $nc($$cast($Symbol, parser_stack->get(parser_top - 2)))->left;
			$assign(parser_result, $new($Symbol, 36, var$115, $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->right, argl));
		}
		return parser_result;
	case 112:
		{
			$var($QName, fname, $cast($QName, $nc($$cast($Symbol, $nc(parser_stack)->get(parser_top - 0)))->value));
			int32_t var$116 = $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->left;
			$assign(parser_result, $new($Symbol, 38, var$116, $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->right, fname));
		}
		return parser_result;
	case 113:
		{
			...
			$assign(parser_result, $new($Symbol, 39, var$117, $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->right, vname));
		}
		return parser_result;
	case 114:
		{
			...
			$assign(parser_result, $new($Symbol, 3, var$118, $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->right, ex));
		}
		return parser_result;
	case 115:
		{
			...
			$assign(parser_result, $new($Symbol, 25, var$119, $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->right, nt));
		}
		return parser_result;
	case 116:
		{
			$var($Object, result, $Integer::valueOf($NodeTest::ANODE));
			int32_t var$120 = $nc($$cast($Symbol, $nc(parser_stack)->get(parser_top - 0)))->left;
			$assign(parser_result, $new($Symbol, 25, var$120, $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->right, result));
		}
		return parser_result;
	case 117:
		{
	...
	case 119:
		{
			...
			$var($Expression, var$123, $new($NameCall, name));
			$var($Expression, exp, $new($EqualityExpr, $Operators::EQ, var$123, $$new($LiteralExpr, l)));
			...
			int32_t var$124 = $nc($$cast($Symbol, parser_stack->get(parser_top - 3)))->left;
			$assign(parser_result, $new($Symbol, 25, var$124, $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->right, result));
		}
		return parser_result;
	case 120:
		{
			$var($Object, result, $Integer::valueOf($NodeTest::PI));
			int32_t var$125 = $nc($$cast($Symbol, $nc(parser_stack)->get(parser_top - 0)))->left;
	...
	case 121:
		{
			...
			$assign(parser_result, $new($Symbol, 26, var$126, $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->right, result));
		}
		return parser_result;
	case 122:
		{
			$var($QName, qn, $cast($QName, $nc($$cast($Symbol, $nc(parser_stack)->get(parser_top - 0)))->value));
			int32_t var$127 = $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->left;
			$assign(parser_result, $new($Symbol, 26, var$127, $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->right, qn));
		}
		return parser_result;
	case 123:
		{
			...
			$assign(parser_result, $new($Symbol, 37, var$128, $nc($$cast($Symbol, parser_stack->get(parser_top - 0)))->right, result));
		}
		return parser_result;
	case 124:
		{
			$var($QName, result, $nc(this->parser)->getQNameIgnoreDefaultNs("div"_s));
			int32_t var$129 = $nc($$cast($Symbol, $nc(parser_stack)->get(parser_top - 0)))->left;
...
$Class* parser_actions::load$($String* name, bool initialize) {
	...
	$loadClass(parser_actions, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(parser_actions);
	});
	return class$;
}
