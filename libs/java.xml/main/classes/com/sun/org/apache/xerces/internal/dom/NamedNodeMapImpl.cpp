...
int32_t NamedNodeMapImpl::getLength() {
	return (this->nodes != nullptr) ? this->nodes->size() : 0;
}

$Node* NamedNodeMapImpl::item(int32_t index) {
	return (this->nodes != nullptr && index < this->nodes->size()) ? $cast($Node, this->nodes->get(index)) : ($Node*)nullptr;
}
...
void NamedNodeMapImpl::setReadOnly(bool readOnly, bool deep) {
	...
	if (deep && this->nodes != nullptr) {
		for (int32_t i = this->nodes->size() - 1; i >= 0; --i) {
			$$sure($NodeImpl, $nc(this->nodes)->get(i))->setReadOnly(readOnly, deep);
		}
	}
}
...
void NamedNodeMapImpl::setOwnerDocument($CoreDocumentImpl* doc) {
	...
	if (this->nodes != nullptr) {
		...
		for (int32_t i = 0; i < size; ++i) {
			$$sure($NodeImpl, item(i))->setOwnerDocument(doc);
		}
	}
}

bool NamedNodeMapImpl::isReadOnly() {
	return (this->flags & NamedNodeMapImpl::READONLY) != 0;
}

void NamedNodeMapImpl::isReadOnly(bool value) {
	this->flags = (int16_t)(value ? this->flags | NamedNodeMapImpl::READONLY : this->flags & ~NamedNodeMapImpl::READONLY);
}

bool NamedNodeMapImpl::changed() {
	return (this->flags & NamedNodeMapImpl::CHANGED) != 0;
}

void NamedNodeMapImpl::changed(bool value) {
	this->flags = (int16_t)(value ? this->flags | NamedNodeMapImpl::CHANGED : this->flags & ~NamedNodeMapImpl::CHANGED);
}

bool NamedNodeMapImpl::hasDefaults() {
	return (this->flags & NamedNodeMapImpl::HASDEFAULTS) != 0;
}

void NamedNodeMapImpl::hasDefaults(bool value) {
	this->flags = (int16_t)(value ? this->flags | NamedNodeMapImpl::HASDEFAULTS : this->flags & ~NamedNodeMapImpl::HASDEFAULTS);
}
...
int32_t NamedNodeMapImpl::findNamePoint($String* name, int32_t start) {
	...
	if (this->nodes != nullptr) {
		int32_t first = start;
		int32_t last = this->nodes->size() - 1;
		while (first <= last) {
			i = (first + last) / 2;
			int32_t test = $nc(name)->compareTo($(($$sure($Node, $nc(this->nodes)->get(i)))->getNodeName()));
			if (test == 0) {
				return i;
			} else if (test < 0) {
				last = i - 1;
	...
	for (int32_t i = 0; i < size; ++i) {
		...
		if (namespaceURI == nullptr) {
			...
			if (var$0) {
				...
				var$0 = var$1 || (aLocalName == nullptr && name->equals($(a->getNodeName())));
			}
			if (var$0) {
				return i;
			}
		} else {
			bool var$2 = namespaceURI->equals(aNamespaceURI);
			if (var$2 && $nc(name)->equals(aLocalName)) {
				return i;
			}
		}
	}
...
bool NamedNodeMapImpl::precedes($Node* a, $Node* b) {
	...
	if (this->nodes != nullptr) {
		int32_t size = this->nodes->size();
		for (int32_t i = 0; i < size; ++i) {
			$var($Node, n, $cast($Node, this->nodes->get(i)));
			if (n == a) {
				return true;
			}
			if (n == b) {
...
void NamedNodeMapImpl::removeItem(int32_t index) {
	if (this->nodes != nullptr && index < this->nodes->size()) {
...
$List* NamedNodeMapImpl::cloneMap($List* list$renamed) {
	...
	if (this->nodes != nullptr) {
		$assign(list, $new($ArrayList, this->nodes));
	}
	return list;
}
...
void NamedNodeMapImpl::readObject($ObjectInputStream* in) {
	...
	if (this->nodes != nullptr) {
		$set(this, nodes, $new($ArrayList, $cast($AbstractCollection, $cast($Vector, this->nodes))));
	}
}
...
void NamedNodeMapImpl::writeObject($ObjectOutputStream* out) {
	...
	try {
		if (oldNodes != nullptr) {
			$set(this, nodes, $new($Vector, oldNodes));
		}
