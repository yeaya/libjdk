...
$XSAnnotationImpl* XSDAbstractTraverser::traverseAnnotationDecl($Element* annotationDecl, $ObjectArray* parentAttrs, bool isGlobal, $XSDocumentInfo* schemaDoc) {
	...
	if (child != nullptr) {
		do {
			...
			if (!(var$0 || (name->equals($SchemaSymbols::ELT_DOCUMENTATION)))) {
				reportSchemaError("src-annotation"_s, $$new($ObjectArray, {name}), child);
			} else {
	...
	if (annotationLocalAttrs != nullptr && !annotationLocalAttrs->isEmpty()) {
		...
		while (i < annotationLocalAttrs->size()) {
			...
			$var($String, uri, $nc(schemaDoc->fNamespaceSupport)->getURI($($nc(this->fSymbolTable)->addSymbol(prefix))));
		...
		$init($SchemaSymbols);
		int32_t annotationTokenEnd = contents->indexOf($SchemaSymbols::ELT_ANNOTATION);
...
$XSDAbstractTraverser$FacetInfo* XSDAbstractTraverser::traverseFacets($Element* content$renamed, $XSTypeDefinition* typeDef, $XSSimpleType* baseValidator, $XSDocumentInfo* schemaDoc) {
	...
	int16_t facetsPresent = 0;
	int16_t facetsFixed = 0;
	...
	$var($List, enumNSDecls, hasQName ? $cast($List, $new($ArrayList)) : ($List*)nullptr);
	...
	while (content != nullptr) {
		...
		if ($nc(facet)->equals($SchemaSymbols::ELT_ENUMERATION)) {
			...
			if (var$0 && baseValidator->getPrimitiveKind() == $XSSimpleType::PRIMITIVE_NOTATION) {
				...
				try {
					$var($QName, temp, $cast($QName, $nc(XSDAbstractTraverser::fQNameDV)->validate(enumVal, schemaDoc->fValidationContext, nullptr)));
			...
			if ((facetsPresent & currentFacet) != 0) {
				reportSchemaError("src-single-facet-value"_s, $$new($ObjectArray, {facet}), content);
				$nc(this->fAttrChecker)->returnAttrArray(attrs, schemaDoc);
				$assign(content, $DOMUtil::getNextSiblingElement(content));
				continue;
			}
			if ($nc(attrs)->get($XSAttributeChecker::ATTIDX_VALUE) == nullptr) {
				if ($nc(content)->getAttributeNodeNS(nullptr, "value"_s) == nullptr) {
					reportSchemaError("s4s-att-must-appear"_s, $$new($ObjectArray, {
			...
			if ($nc($cast($Boolean, attrs->get($XSAttributeChecker::ATTIDX_FIXED)))->booleanValue()) {
				facetsFixed |= currentFacet;
			}
			switch (currentFacet) {
			case $XSSimpleType::FACET_MINLENGTH:
				this->xsFacets->minLength = $nc($cast($XInt, attrs->get($XSAttributeChecker::ATTIDX_VALUE)))->intValue();
				hasMinLengthFacet = true;
				break;
			case $XSSimpleType::FACET_MAXLENGTH:
				this->xsFacets->maxLength = $nc($cast($XInt, attrs->get($XSAttributeChecker::ATTIDX_VALUE)))->intValue();
				hasMaxLengthFacet = true;
				break;
			case $XSSimpleType::FACET_MAXEXCLUSIVE:
				$set(this->xsFacets, maxExclusive, $cast($String, attrs->get($XSAttributeChecker::ATTIDX_VALUE)));
				break;
			case $XSSimpleType::FACET_MAXINCLUSIVE:
				$set(this->xsFacets, maxInclusive, $cast($String, attrs->get($XSAttributeChecker::ATTIDX_VALUE)));
				break;
			case $XSSimpleType::FACET_MINEXCLUSIVE:
				$set(this->xsFacets, minExclusive, $cast($String, attrs->get($XSAttributeChecker::ATTIDX_VALUE)));
				break;
			case $XSSimpleType::FACET_MININCLUSIVE:
				$set(this->xsFacets, minInclusive, $cast($String, attrs->get($XSAttributeChecker::ATTIDX_VALUE)));
				break;
			case $XSSimpleType::FACET_TOTALDIGITS:
				this->xsFacets->totalDigits = $nc($cast($XInt, attrs->get($XSAttributeChecker::ATTIDX_VALUE)))->intValue();
				break;
			case $XSSimpleType::FACET_FRACTIONDIGITS:
				this->xsFacets->fractionDigits = $nc($cast($XInt, attrs->get($XSAttributeChecker::ATTIDX_VALUE)))->intValue();
				break;
			case $XSSimpleType::FACET_WHITESPACE:
				this->xsFacets->whiteSpace = $nc($cast($XInt, attrs->get($XSAttributeChecker::ATTIDX_VALUE)))->shortValue();
				break;
			case $XSSimpleType::FACET_LENGTH:
				this->xsFacets->length = $nc($cast($XInt, attrs->get($XSAttributeChecker::ATTIDX_VALUE)))->intValue();
				hasLengthFacet = true;
				break;
			}
			$var($Element, child, $DOMUtil::getFirstChildElement(content));
			...
			switch (currentFacet) {
			case $XSSimpleType::FACET_MINLENGTH:
				$set(this->xsFacets, minLengthAnnotation, annotation);
				break;
			case $XSSimpleType::FACET_MAXLENGTH:
				$set(this->xsFacets, maxLengthAnnotation, annotation);
				break;
			case $XSSimpleType::FACET_MAXEXCLUSIVE:
				$set(this->xsFacets, maxExclusiveAnnotation, annotation);
				break;
			case $XSSimpleType::FACET_MAXINCLUSIVE:
				$set(this->xsFacets, maxInclusiveAnnotation, annotation);
				break;
			case $XSSimpleType::FACET_MINEXCLUSIVE:
				$set(this->xsFacets, minExclusiveAnnotation, annotation);
				break;
			case $XSSimpleType::FACET_MININCLUSIVE:
				$set(this->xsFacets, minInclusiveAnnotation, annotation);
				break;
			case $XSSimpleType::FACET_TOTALDIGITS:
				$set(this->xsFacets, totalDigitsAnnotation, annotation);
				break;
			case $XSSimpleType::FACET_FRACTIONDIGITS:
				$set(this->xsFacets, fractionDigitsAnnotation, annotation);
				break;
			case $XSSimpleType::FACET_WHITESPACE:
				$set(this->xsFacets, whiteSpaceAnnotation, annotation);
				break;
			case $XSSimpleType::FACET_LENGTH:
				$set(this->xsFacets, lengthAnnotation, annotation);
				break;
			}
			if (child != nullptr) {
				reportSchemaError("s4s-elt-must-match.1"_s, $$new($ObjectArray, {
	...
	if (enumData != nullptr) {
		...
		$set(this->xsFacets, enumeration, enumData);
		$set(this->xsFacets, enumNSDecls, enumNSDecls);
		$set(this->xsFacets, enumAnnotations, enumAnnotations);
	}
	if (seenPattern) {
		facetsPresent |= $XSSimpleType::FACET_PATTERN;
		$set(this->xsFacets, pattern, $nc(this->fPattern)->toString());
		$set(this->xsFacets, patternAnnotations, patternAnnotations);
	}
...
void XSDAbstractTraverser::checkEnumerationAndMaxLengthInconsistency($XSSimpleType* baseValidator, $List* enumData, $Element* contextNode, $String* typeName) {
	...
	if (var$0 && $nc($SchemaSymbols::ATTVAL_HEXBINARY)->equals($(baseValidator->getName()))) {
		for (int32_t enumIdx = 0; enumIdx < $nc(enumData)->size(); ++enumIdx) {
			$var($String, enumVal, $cast($String, enumData->get(enumIdx)));
			if ($nc(enumVal)->length() / 2 > this->xsFacets->maxLength) {
		...
		bool var$1 = $SchemaSymbols::URI_SCHEMAFORSCHEMA->equals($(baseValidator->getNamespace()));
		if (var$1 && $nc($SchemaSymbols::ATTVAL_BASE64BINARY)->equals($(baseValidator->getName()))) {
			for (int32_t enumIdx = 0; enumIdx < $nc(enumData)->size(); ++enumIdx) {
				$var($String, enumVal, $cast($String, enumData->get(enumIdx)));
				$var($bytes, decodedVal, $Base64::decode(enumVal));
				if (decodedVal != nullptr && ($$new($String, decodedVal))->length() > this->xsFacets->maxLength) {
					reportSchemaWarning("FacetsContradict"_s, $$new($ObjectArray, {
						enumVal,
						$SchemaSymbols::ELT_MAXLENGTH,
						typeName
			...
			for (int32_t enumIdx = 0; enumIdx < $nc(enumData)->size(); ++enumIdx) {
				$var($String, enumVal, $cast($String, enumData->get(enumIdx)));
				if ($nc(enumVal)->length() > this->xsFacets->maxLength) {
					reportSchemaWarning("FacetsContradict"_s, $$new($ObjectArray, {
						enumVal,
						$SchemaSymbols::ELT_MAXLENGTH,
						typeName
...
void XSDAbstractTraverser::checkEnumerationAndMinLengthInconsistency($XSSimpleType* baseValidator, $List* enumData, $Element* contextNode, $String* typeName) {
	...
	if (var$0 && $nc($SchemaSymbols::ATTVAL_HEXBINARY)->equals($(baseValidator->getName()))) {
		for (int32_t enumIdx = 0; enumIdx < $nc(enumData)->size(); ++enumIdx) {
			$var($String, enumVal, $cast($String, enumData->get(enumIdx)));
			if ($nc(enumVal)->length() / 2 < this->xsFacets->minLength) {
				reportSchemaWarning("FacetsContradict"_s, $$new($ObjectArray, {
					enumVal,
		...
		if (var$1 && $nc($SchemaSymbols::ATTVAL_BASE64BINARY)->equals($(baseValidator->getName()))) {
			for (int32_t enumIdx = 0; enumIdx < $nc(enumData)->size(); ++enumIdx) {
				$var($String, enumVal, $cast($String, enumData->get(enumIdx)));
				$var($bytes, decodedVal, $Base64::decode(enumVal));
				if (decodedVal != nullptr && ($$new($String, decodedVal))->length() < this->xsFacets->minLength) {
					reportSchemaWarning("FacetsContradict"_s, $$new($ObjectArray, {
						enumVal,
						$SchemaSymbols::ELT_MINLENGTH,
						typeName
			...
			for (int32_t enumIdx = 0; enumIdx < $nc(enumData)->size(); ++enumIdx) {
				$var($String, enumVal, $cast($String, enumData->get(enumIdx)));
				if ($nc(enumVal)->length() < this->xsFacets->minLength) {
					reportSchemaWarning("FacetsContradict"_s, $$new($ObjectArray, {
						enumVal,
						$SchemaSymbols::ELT_MINLENGTH,
						typeName
...
void XSDAbstractTraverser::checkEnumerationAndLengthInconsistency($XSSimpleType* baseValidator, $List* enumData, $Element* contextNode, $String* typeName) {
	...
	if (var$0 && $nc($SchemaSymbols::ATTVAL_HEXBINARY)->equals($(baseValidator->getName()))) {
		for (int32_t enumIdx = 0; enumIdx < $nc(enumData)->size(); ++enumIdx) {
			$var($String, enumVal, $cast($String, enumData->get(enumIdx)));
			if ($nc(enumVal)->length() / 2 != this->xsFacets->length) {
				reportSchemaWarning("FacetsContradict"_s, $$new($ObjectArray, {
					enumVal,
		...
		if (var$1 && $nc($SchemaSymbols::ATTVAL_BASE64BINARY)->equals($(baseValidator->getName()))) {
			for (int32_t enumIdx = 0; enumIdx < $nc(enumData)->size(); ++enumIdx) {
				$var($String, enumVal, $cast($String, enumData->get(enumIdx)));
				$var($bytes, decodedVal, $Base64::decode(enumVal));
				if (decodedVal != nullptr && ($$new($String, decodedVal))->length() != this->xsFacets->length) {
					reportSchemaWarning("FacetsContradict"_s, $$new($ObjectArray, {
						enumVal,
						$SchemaSymbols::ELT_LENGTH,
						typeName
			...
			for (int32_t enumIdx = 0; enumIdx < $nc(enumData)->size(); ++enumIdx) {
				$var($String, enumVal, $cast($String, enumData->get(enumIdx)));
				if ($nc(enumVal)->length() != this->xsFacets->length) {
					reportSchemaWarning("FacetsContradict"_s, $$new($ObjectArray, {
						enumVal,
						$SchemaSymbols::ELT_LENGTH,
						typeName
...
$Element* XSDAbstractTraverser::traverseAttrsAndAttrGrps($Element* firstAttr, $XSAttributeGroupDecl* attrGrp, $XSDocumentInfo* schemaDoc, $SchemaGrammar* grammar, $XSComplexTypeDecl* enclosingCT) {
	...
	for ($assign(child, firstAttr); child != nullptr; $assign(child, $DOMUtil::getNextSiblingElement(child))) {
		...
		if ($nc(childName)->equals($SchemaSymbols::ELT_ATTRIBUTE)) {
			...
			$var($String, var$0, $nc(tempAttrUse->fAttrDecl)->getNamespace());
			$assign(otherUse, $nc(attrGrp)->getAttributeUseNoProhibited(var$0, $(tempAttrUse->fAttrDecl->getName())));
			if (otherUse == nullptr) {
				$var($String, idName, $nc(attrGrp)->addAttributeUse(tempAttrUse));
				if (idName != nullptr) {
					$var($String, code, (enclosingCT == nullptr) ? "ag-props-correct.3"_s : "ct-props-correct.5"_s);
					$var($String, name, (enclosingCT == nullptr) ? attrGrp->fName : enclosingCT->getName());
					reportSchemaError(code, $$new($ObjectArray, {
						name,
						$($nc(tempAttrUse->fAttrDecl)->getName()),
						idName
					}), child);
				}
			} else if (!$equals(otherUse, tempAttrUse)) {
				$var($String, code, (enclosingCT == nullptr) ? "ag-props-correct.2"_s : "ct-props-correct.4"_s);
				$var($String, name, (enclosingCT == nullptr) ? $nc(attrGrp)->fName : enclosingCT->getName());
				reportSchemaError(code, $$new($ObjectArray, {
					name,
					$($nc(tempAttrUse->fAttrDecl)->getName())
				}), child);
			...
			for (int32_t i = 0; i < attrCount; ++i) {
				...
				$var($String, var$1, $nc(oneAttrUse->fAttrDecl)->getNamespace());
				$assign(otherUse, $nc(attrGrp)->getAttributeUseNoProhibited(var$1, $(oneAttrUse->fAttrDecl->getName())));
				if (otherUse == nullptr) {
					$var($String, idName, $nc(attrGrp)->addAttributeUse(oneAttrUse));
					if (idName != nullptr) {
						$var($String, code, (enclosingCT == nullptr) ? "ag-props-correct.3"_s : "ct-props-correct.5"_s);
						$var($String, name, (enclosingCT == nullptr) ? attrGrp->fName : enclosingCT->getName());
						reportSchemaError(code, $$new($ObjectArray, {
							name,
							$($nc(oneAttrUse->fAttrDecl)->getName()),
							idName
						}), child);
					}
			...
			if (tempAttrGrp->fAttributeWC != nullptr) {
				if ($nc(attrGrp)->fAttributeWC == nullptr) {
					...
					$set(attrGrp, fAttributeWC, attrGrp->fAttributeWC->performIntersectionWith(tempAttrGrp->fAttributeWC, attrGrp->fAttributeWC->fProcessContents));
					if (attrGrp->fAttributeWC == nullptr) {
						$var($String, code, (enclosingCT == nullptr) ? "src-attribute_group.2"_s : "src-ct.4"_s);
						$var($String, name, (enclosingCT == nullptr) ? attrGrp->fName : enclosingCT->getName());
						reportSchemaError(code, $$new($ObjectArray, {name}), child);
					}
				}
			}
		} else {
	...
	if (child != nullptr) {
		...
		if ($nc(childName)->equals($SchemaSymbols::ELT_ANYATTRIBUTE)) {
			...
			if ($nc(attrGrp)->fAttributeWC == nullptr) {
				...
				$set(attrGrp, fAttributeWC, $nc(tempAttrWC)->performIntersectionWith(attrGrp->fAttributeWC, $nc(tempAttrWC)->fProcessContents));
				if (attrGrp->fAttributeWC == nullptr) {
					$var($String, code, (enclosingCT == nullptr) ? "src-attribute_group.2"_s : "src-ct.4"_s);
					$var($String, name, (enclosingCT == nullptr) ? attrGrp->fName : enclosingCT->getName());
					reportSchemaError(code, $$new($ObjectArray, {name}), child);
...
void XSDAbstractTraverser::checkNotationType($String* refName, $XSTypeDefinition* typeDecl, $Element* elem) {
	...
	bool var$0 = var$1 && $cast($XSSimpleType, typeDecl)->getVariety() == $XSSimpleType::VARIETY_ATOMIC;
	if (var$0 && $cast($XSSimpleType, typeDecl)->getPrimitiveKind() == $XSSimpleType::PRIMITIVE_NOTATION) {
		if (($cast($XSSimpleType, typeDecl)->getDefinedFacets() & $XSSimpleType::FACET_ENUMERATION) == 0) {
			reportSchemaError("enumeration-required-notation"_s, $$new($ObjectArray, {
				$(typeDecl->getName()),
				refName,
				$($DOMUtil::getLocalName(elem))
			}), elem);
		}
	}
}
...
$XSParticleDecl* XSDAbstractTraverser::checkOccurrences($XSParticleDecl* particle, $String* particleName, $Element* parent, int32_t allContextFlags, int64_t defaultVals) {
	...
	$init($XSAttributeChecker);
	bool defaultMin = (defaultVals & ($sl(1, $XSAttributeChecker::ATTIDX_MINOCCURS))) != 0;
	bool defaultMax = (defaultVals & ($sl(1, $XSAttributeChecker::ATTIDX_MAXOCCURS))) != 0;
	bool processingAllEl = ((allContextFlags & XSDAbstractTraverser::PROCESSING_ALL_EL) != 0);
	bool processingAllGP = ((allContextFlags & XSDAbstractTraverser::PROCESSING_ALL_GP) != 0);
	bool groupRefWithAll = ((allContextFlags & XSDAbstractTraverser::GROUP_REF_WITH_ALL) != 0);
	bool isGroupChild = ((allContextFlags & XSDAbstractTraverser::CHILD_OF_GROUP) != 0);
	if (isGroupChild) {
		if (!defaultMin) {
	...
	if (processingAllEl) {
		if (max != 1) {
			...
			reportSchemaError("cos-all-limited.2"_s, $$new($ObjectArray, {
				...
				$($nc($cast($XSElementDecl, particle->fValue))->getName())
			}), parent);
			max = 1;
			if (min > 1) {
				min = 1;
...
void XSDAbstractTraverser::clinit$($Class* clazz) {
...
$Class* XSDAbstractTraverser::load$($String* name, bool initialize) {
	...
	$loadClass(XSDAbstractTraverser, name, initialize, &classInfo$$, XSDAbstractTraverser::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XSDAbstractTraverser);
	});
	return class$;
}
