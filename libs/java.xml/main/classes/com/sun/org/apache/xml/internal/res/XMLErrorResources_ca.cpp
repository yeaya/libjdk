...
void XMLErrorResources_ca::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(XMLErrorResources_ca::_contents, $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_FUNCTION_NOT_SUPPORTED,
			u"Aquesta funció no té suport. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_CANNOT_OVERWRITE_CAUSE,
			"No es pot sobreescriure una causa "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_NO_DEFAULT_IMPL,
			u"No s\'ha trobat cap implementació per defecte "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_CHUNKEDINTARRAY_NOT_SUPPORTED,
			u"En l\'\'actualitat ChunkedIntArray({0}) no té suport "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_OFFSET_BIGGER_THAN_SLOT,
			u"El desplaçament és més gran que la ranura "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_COROUTINE_NOT_AVAIL,
			u"Coroutine no està disponible, id={0} "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_COROUTINE_CO_EXIT,
			u"CoroutineManager ha rebut una petició co_exit() "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_COJOINROUTINESET_FAILED,
			u"S\'ha produït un error a co_joinCoroutineSet() "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_COROUTINE_PARAM,
			u"Error de paràmetre coroutine ({0}) "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_PARSER_DOTERMINATE_ANSWERS,
			"\nUNEXPECTED: doTerminate de l\'\'analitzador respon {0}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_NO_PARSE_CALL_WHILE_PARSING,
			u"L\'anàlisi no es pot cridar mentre s\'està duent a terme "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_TYPED_ITERATOR_AXIS_NOT_IMPLEMENTED,
			"Error: l\'\'iterador de tipus de l\'\'eix {0} no s\'\'ha implementat "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_ITERATOR_AXIS_NOT_IMPLEMENTED,
			"Error: l\'\'iterador de l\'\'eix {0} no s\'\'ha implementat "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_ITERATOR_CLONE_NOT_SUPPORTED,
			u"El clonatge de l\'iterador no té suport "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_UNKNOWN_AXIS_TYPE,
			u"Tipus de commutació de l\'\'eix desconeguda: {0} "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_AXIS_NOT_SUPPORTED,
			u"La commutació de l\'\'eix no té suport: {0} "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_NO_DTMIDS_AVAIL,
			u"No hi ha més ID de DTM disponibles "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_NOT_SUPPORTED,
			u"No té suport: {0} "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_NODE_NON_NULL,
			"El node no ha de ser nul per a getDTMHandleFromNode "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_COULD_NOT_RESOLVE_NODE,
			"No s\'ha pogut resoldre el node en un manejador "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_STARTPARSE_WHILE_PARSING,
			u"startParse no es pot cridar mentre s\'està duent a terme l\'anàlisi "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_STARTPARSE_NEEDS_SAXPARSER,
			"startParse necessita un SAXParser que no sigui nul "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_COULD_NOT_INIT_PARSER,
			"No s\'ha pogut inicialitzar l\'analitzador amb "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_EXCEPTION_CREATING_POOL,
			u"S\'ha produït una excepció en crear una nova instància de l\'agrupació "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_PATH_CONTAINS_INVALID_ESCAPE_SEQUENCE,
			u"La via d\'accés conté una seqüència d\'escapament no vàlida "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_SCHEME_REQUIRED,
			"Es necessita l\'esquema "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_NO_SCHEME_IN_URI,
			"No s\'\'ha trobat cap esquema a l\'\'URI: {0} "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_NO_SCHEME_INURI,
			"No s\'ha trobat cap esquema a l\'URI "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_PATH_INVALID_CHAR,
			u"La via d\'accés conté un caràcter no vàlid {0} "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_SCHEME_FROM_NULL_STRING,
			u"No es pot establir un esquema des d\'una cadena nul·la "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_SCHEME_NOT_CONFORMANT,
			u"L\'esquema no té conformitat. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_HOST_ADDRESS_NOT_WELLFORMED,
			u"El sistema principal no té una adreça ben formada "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_PORT_WHEN_HOST_NULL,
			u"El port no es pot establir quan el sistema principal és nul "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_INVALID_PORT,
			u"Número de port no vàlid "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_FRAG_FOR_GENERIC_URI,
			u"El fragment només es pot establir per a un URI genèric "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_FRAG_WHEN_PATH_NULL,
			u"El fragment no es pot establir si la via d\'accés és nul·la "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_FRAG_INVALID_CHAR,
			u"El fragment conté un caràcter no vàlid "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_PARSER_IN_USE,
			u"L\'analitzador ja s\'està utilitzant "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_CANNOT_CHANGE_WHILE_PARSING,
			u"No es pot modificar {0} {1} mentre es du a terme l\'\'anàlisi "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_SELF_CAUSATION_NOT_PERMITTED,
			u"La causalitat pròpia no està permesa. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_NO_USERINFO_IF_NO_HOST,
			u"No es pot especificar informació de l\'usuari si no s\'especifica el sistema principal "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_NO_PORT_IF_NO_HOST,
			"No es pot especificar el port si no s\'especifica el sistema principal "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_NO_QUERY_STRING_IN_PATH,
			u"No es pot especificar una cadena de consulta en la via d\'accés i la cadena de consulta "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_NO_FRAGMENT_STRING_IN_PATH,
			u"No es pot especificar un fragment tant en la via d\'accés com en el fragment "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_CANNOT_INIT_URI_EMPTY_PARMS,
			u"No es pot inicialitzar l\'URI amb paràmetres buits "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_METHOD_NOT_SUPPORTED,
			u"Aquest mètode encara no té suport "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_INCRSAXSRCFILTER_NOT_RESTARTABLE,
			"Ara mateix no es pot reiniciar IncrementalSAXSource_Filter "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_XMLRDR_NOT_BEFORE_STARTPARSE,
			u"XMLReader no es pot produir abans de la petició d\'startParse "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_AXIS_TRAVERSER_NOT_SUPPORTED,
			u"La commutació de l\'\'eix no té suport: {0} "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_ERRORHANDLER_CREATED_WITH_NULL_PRINTWRITER,
			"S\'\'ha creat ListingErrorHandler amb PrintWriter nul "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_SYSTEMID_UNKNOWN,
			"ID del sistema (SystemId) desconegut "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_LOCATION_UNKNOWN,
			u"Ubicació de l\'error desconeguda"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_PREFIX_MUST_RESOLVE,
			"El prefix s\'\'ha de resoldre en un espai de noms: {0} "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_CREATEDOCUMENT_NOT_SUPPORTED,
			u"createDocument() no té suport a XPathContext "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_CHILD_HAS_NO_OWNER_DOCUMENT,
			u"El subordinat de l\'atribut no té un document de propietari. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_CHILD_HAS_NO_OWNER_DOCUMENT_ELEMENT,
			u"El subordinat de l\'atribut no té un element de document de propietari. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_CANT_OUTPUT_TEXT_BEFORE_DOC,
			u"Avís: no es pot produir text abans de l\'element de document. Es passa per alt. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_CANT_HAVE_MORE_THAN_ONE_ROOT,
			u"No hi pot haver més d\'una arrel en un DOM. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_ARG_LOCALNAME_NULL,
			u"L\'argument \'localName\' és nul. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_ARG_LOCALNAME_INVALID,
			u"El nom local de QNAME ha de ser un NCName vàlid. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_ARG_PREFIX_INVALID,
			u"El prefix de QNAME ha de ser un NCName vàlid. "_s
		}),
		$$new($ObjectArray, {
			"BAD_CODE"_s,
			u"El paràmetre de createMessage estava fora dels límits. "_s
		}),
		$$new($ObjectArray, {
			"FORMAT_FAILED"_s,
			u"S\'ha generat una excepció durant la crida messageFormat. "_s
		}),
		$$new($ObjectArray, {
			"line"_s,
			u"Línia núm. "_s
		...
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_SERIALIZER_NOT_CONTENTHANDLER,
			"La classe de serialitzador \'\'{0}\'\' no implementa org.xml.sax.ContentHandler."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_RESOURCE_COULD_NOT_FIND,
			"No s\'\'ha trobat el recurs [ {0} ].\n {1}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_RESOURCE_COULD_NOT_LOAD,
			"El recurs [ {0} ] no s\'\'ha pogut carregar: {1} \n {2} \t {3}"_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_BUFFER_SIZE_LESSTHAN_ZERO,
			u"Grandària del buffer <=0 "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_INVALID_UTF16_SURROGATE,
			u"S\'\'ha detectat un suplent UTF-16 no vàlid: {0} ? "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_OIERROR,
			"Error d\'E/S "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_ILLEGAL_ATTRIBUTE_POSITION,
			u"No es pot afegir l\'\'atribut {0} després dels nodes subordinats o abans que es produeixi un element. Es passarà per alt l\'\'atribut. "_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_NAMESPACE_PREFIX,
			"L\'\'espai de noms del prefix \'\'{0}\'\' no s\'\'ha declarat."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_STRAY_ATTRIBUTE,
			"L\'\'atribut \'\'{0}\'\' es troba fora de l\'\'element."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_STRAY_NAMESPACE,
			u"La declaració d\'\'espai de noms \'\'{0}\'\'=\'\'{1}\'\' es troba fora de l\'\'element."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_COULD_NOT_LOAD_RESOURCE,
			"No s\'\'ha pogut carregar \'\'{0}\'\' (comproveu la CLASSPATH); ara s\'\'estan fent servir els valors per defecte."_s
		}),
		$$new($ObjectArray, {
			XMLErrorResources_ca::ER_COULD_NOT_LOAD_METHOD_PROPERTY,
			u"No s\'\'ha pogut carregar el fitxer de propietats \'\'{0}\'\' del mètode de sortida \'\'{1}\'\' (comproveu la CLASSPATH)"_s
		})
	}));
}
...
$Class* XMLErrorResources_ca::load$($String* name, bool initialize) {
	...
	$loadClass(XMLErrorResources_ca, name, initialize, &classInfo$$, XMLErrorResources_ca::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMLErrorResources_ca);
	});
	return class$;
}
