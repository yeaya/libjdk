...
void UTF16Reader::init$($InputStream* inputStream, bool isBigEndian) {
	...
	UTF16Reader::init$(inputStream, UTF16Reader::DEFAULT_BUFFER_SIZE, isBigEndian, var$0, $($Locale::getDefault()));
}
...
int32_t UTF16Reader::read() {
	...
	int32_t b1 = this->fInputStream->read();
	if (b1 == -1) {
		expectedTwoBytes();
	}
...
int32_t UTF16Reader::read($chars* ch, int32_t offset, int32_t length) {
	...
	if ((byteCount & 1) != 0) {
		int32_t b = this->fInputStream->read();
		if (b == -1) {
			expectedTwoBytes();
		}
		this->fBuffer->set(byteCount++, (int8_t)b);
	}
...
void UTF16Reader::processBE($chars* ch, int32_t offset, int32_t count) {
	...
	for (int32_t i = 0; i < count; ++i) {
		int32_t b0 = $nc(this->fBuffer)->get(curPos++) & 0xff;
...
$Class* UTF16Reader::load$($String* name, bool initialize) {
	...
	$loadClass(UTF16Reader, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
