...
void XSSimpleTypeDecl$XSMVFacetImpl::init$(int16_t kind, $StringList* svalues, $ObjectList* avalues, $XSObjectList* annotations) {
	...
	$set(this, annotations, (annotations != nullptr) ? annotations : $cast($XSObjectList, $XSObjectListImpl::EMPTY_LIST));
}
...
$Class* XSSimpleTypeDecl$XSMVFacetImpl::load$($String* name, bool initialize) {
	...
	$loadClass(XSSimpleTypeDecl$XSMVFacetImpl, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XSSimpleTypeDecl$XSMVFacetImpl);
	});
	return class$;
}
