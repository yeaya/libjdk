...
$String* SAXImpl::getDocumentURI() {
	...
	if (baseURI != nullptr) {
		...
		var$1->append("rtf"_s);
		var$1->append(SAXImpl::_documentURIIndex++);
...
$Node* SAXImpl::makeNode(int32_t index) {
	...
	if (nodeID < 0) {
		...
		return (this->_nodes->get(nodeID) != nullptr) ? this->_nodes->get(nodeID) : (this->_nodes->set(nodeID, $$cast($Document, $new($DTMNodeProxy, $cast($DTM, this), index))));
	} else {
		return $cast($Document, $new($DTMNodeProxy, $cast($DTM, this), index));
	}
}
...
$NodeList* SAXImpl::makeNodeList(int32_t index) {
	...
	if (nodeID < 0) {
		...
		return (this->_nodeLists->get(nodeID) != nullptr) ? this->_nodeLists->get(nodeID) : (this->_nodeLists->set(nodeID, $$new($DTMAxisIterNodeList, this, $$new($DTMDefaultBaseIterators$SingletonIterator, this, index))));
	} else {
...
$DTMAxisIterator* SAXImpl::getNodeValueIterator($DTMAxisIterator* iterator, int32_t type, $String* value, bool op) {
	return $cast($DTMAxisIterator, ($new($SAXImpl$NodeValueIterator, this, iterator, type, value, op)));
}
...
int32_t SAXImpl::getNSType(int32_t node) {
	...
	return $$sure($Integer, $nc(this->_nsIndex)->get($($Integer::valueOf(eType))))->intValue();
}
...
int32_t SAXImpl::getGeneralizedType($String* name, bool searchOnly) {
	...
	$assign(lName, (lNameStartIdx == 0) ? name : name->substring(lNameStartIdx));
...
$shorts* SAXImpl::getReverseNamespaceMapping($StringArray* namespaces) {
	...
	for (i = 0; i < length; ++i) {
		int32_t eType = getIdForNamespace(namespaces->get(i));
		$var($Integer, type, $cast($Integer, $nc(this->_nsIndex)->get($($Integer::valueOf(eType)))));
		result->set(i, (type == nullptr) ? -1 : type->shortValue());
	}
	return result;
}
...
void SAXImpl::init$($XSLTCDTMManager* mgr, $Source* source, int32_t dtmIdentity, $DTMWSFilter* whiteSpaceFilter, $XMLStringFactory* xstringfactory, bool doIndexing, int32_t blocksize, bool buildIdIndex, bool newNameTable) {
	...
	this->_xmlSpaceStack->set(0, $DTMDefaultBase::ROOTNODE);
...
int32_t SAXImpl::getElementById($String* idString) {
	...
	if (node != nullptr) {
		...
		return (id != nullptr) ? id->intValue() : $DTM::NULL;
	} else {
		return $DTM::NULL;
	}
}
...
void SAXImpl::definePrefixAndUri($String* prefix, $String* uri) {
	...
	if ($nc(this->_nsIndex)->get(eType) == nullptr) {
		this->_nsIndex->put(eType, $($Integer::valueOf(this->_uriCount++)));
	}
}
...
void SAXImpl::print(int32_t node, int32_t level) {
	...
	default:
		...
		for (int32_t a = getFirstAttribute(node); a != $DTM::NULL; a = getNextAttribute(a)) {
			...
			var$0->append("\n"_s);
			var$0->append($(getNodeName(a)));
			var$0->append("=\""_s);
			var$0->append($(getStringValueX(a)));
			var$0->append("\""_s);
		...
		$System::out->println($$str({"</"_s, name, $$str(u'>')}));
		break;
	}
}
...
$SerializationHandler* SAXImpl::getOutputDomBuilder() {
	return $new($ToXMLSAXHandler, $cast($SAX2DTM2, this), "UTF-8"_s);
}
...
$String* SAXImpl::getUnparsedEntityURI($String* name) {
	...
	if (this->_document != nullptr) {
		$var($String, uri, ""_s);
		$var($DocumentType, doctype, this->_document->getDoctype());
		if (doctype != nullptr) {
...
void SAXImpl::clinit$($Class* clazz) {
...
$Class* SAXImpl::load$($String* name, bool initialize) {
	...
	$loadClass(SAXImpl, name, initialize, &classInfo$$, SAXImpl::clinit$, []($Class* clazz) -> $Object* {
