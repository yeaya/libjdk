...
void PropertyManager::initConfigurableReaderProperties() {
	...
	this->supportedProps->put($XMLInputFactory::REPORTER, nullptr);
	this->supportedProps->put($XMLInputFactory::RESOLVER, nullptr);
	this->supportedProps->put($XMLInputFactory::ALLOCATOR, nullptr);
	this->supportedProps->put(PropertyManager::STAX_NOTATIONS, nullptr);
	$init($Constants);
	this->supportedProps->put($$str({$Constants::SAX_FEATURE_PREFIX, $Constants::STRING_INTERNING_FEATURE}), $($Boolean::valueOf(true)));
	this->supportedProps->put($$str({$Constants::XERCES_FEATURE_PREFIX, $Constants::ALLOW_JAVA_ENCODINGS_FEATURE}), $($Boolean::valueOf(true)));
	this->supportedProps->put($Constants::ADD_NAMESPACE_DECL_AS_ATTRIBUTE, $Boolean::FALSE);
	this->supportedProps->put($Constants::READER_IN_DEFINED_STATE, $($Boolean::valueOf(true)));
	this->supportedProps->put($Constants::REUSE_INSTANCE, $($Boolean::valueOf(true)));
	this->supportedProps->put($$str({$Constants::ZEPHYR_PROPERTY_PREFIX, $Constants::STAX_REPORT_CDATA_EVENT}), $($Boolean::valueOf(false)));
	this->supportedProps->put($$str({$Constants::ZEPHYR_PROPERTY_PREFIX, $Constants::IGNORE_EXTERNAL_DTD}), $Boolean::FALSE);
	this->supportedProps->put($$str({$Constants::XERCES_FEATURE_PREFIX, $Constants::WARN_ON_DUPLICATE_ATTDEF_FEATURE}), $($Boolean::valueOf(false)));
	this->supportedProps->put($$str({$Constants::XERCES_FEATURE_PREFIX, $Constants::WARN_ON_DUPLICATE_ENTITYDEF_FEATURE}), $($Boolean::valueOf(false)));
	this->supportedProps->put($$str({$Constants::XERCES_FEATURE_PREFIX, $Constants::WARN_ON_UNDECLARED_ELEMDEF_FEATURE}), $($Boolean::valueOf(false)));
	$set(this, fSecurityManager, $new($XMLSecurityManager, true));
	this->supportedProps->put(PropertyManager::SECURITY_MANAGER, this->fSecurityManager);
	$set(this, fSecurityPropertyMgr, $new($XMLSecurityPropertyManager));
	this->supportedProps->put(PropertyManager::XML_SECURITY_PROPERTY_MANAGER, this->fSecurityPropertyMgr);
	$init($XMLConstants);
	$init($JdkXmlUtils);
	this->supportedProps->put($XMLConstants::USE_CATALOG, $($Boolean::valueOf($JdkXmlUtils::USE_CATALOG_DEFAULT)));
	{
...
bool PropertyManager::containsProperty($String* property) {
	...
	bool var$0 = var$1 || (this->fSecurityManager != nullptr && this->fSecurityManager->getIndex(property) > -1);
	return var$0 || (this->fSecurityPropertyMgr != nullptr && this->fSecurityPropertyMgr->getIndex(property) > -1);
}

$Object* PropertyManager::getProperty($String* property) {
	$var($String, propertyValue, (this->fSecurityManager != nullptr) ? this->fSecurityManager->getLimitAsString(property) : ($String*)nullptr);
	return propertyValue != nullptr ? $of(propertyValue) : $nc(this->supportedProps)->get(property);
}
...
void PropertyManager::setProperty($String* property, Object$* value) {
	...
	if ($nc(property)->equals($XMLInputFactory::IS_NAMESPACE_AWARE)) {
		...
		if (($instanceOf($Boolean, value)) && $cast($Boolean, value)->booleanValue()) {
	...
	if (this->fSecurityManager == nullptr || !this->fSecurityManager->setLimit(property, $JdkProperty$State::APIPROPERTY, value)) {
		$init($XMLSecurityPropertyManager$State);
		if (this->fSecurityPropertyMgr == nullptr || !this->fSecurityPropertyMgr->setValue(property, $XMLSecurityPropertyManager$State::APIPROPERTY, value)) {
...
void PropertyManager::clinit$($Class* clazz) {
...
$Class* PropertyManager::load$($String* name, bool initialize) {
	...
	$loadClass(PropertyManager, name, initialize, &classInfo$$, PropertyManager::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(PropertyManager);
	});
	return class$;
}
