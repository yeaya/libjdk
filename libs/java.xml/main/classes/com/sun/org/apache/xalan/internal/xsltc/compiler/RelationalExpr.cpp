...
$Type* RelationalExpr::typeCheck($SymbolTable* stable) {
	...
	$var($String, var$0, $Operators::getOpNames(this->_op));
	$init($Type);
	$var($MethodType, ptype, lookupPrimop(stable, var$0, $$new($MethodType, $Type::Void, tleft, tright)));
	if (ptype != nullptr) {
...
void RelationalExpr::translate($ClassGenerator* classGen, $MethodGenerator* methodGen) {
	...
	if (var$0 || hasReferenceArgs()) {
		...
		var$1->append("("_s);
		var$1->append($($$nc($nc(this->_left)->getType())->toSignature()));
		var$1->append($($$nc($nc(this->_right)->getType())->toSignature()));
		var$1->append("I"_s);
		var$1->append($Constants::DOM_INTF_SIG);
		var$1->append(")Z"_s);
		int32_t index = $nc(cpg)->addMethodref($Constants::BASIS_LIBRARY_CLASS, "compare"_s, $$str(var$1));
...
void RelationalExpr::translateDesynthesized($ClassGenerator* classGen, $MethodGenerator* methodGen) {
	...
	if (var$0 || hasReferenceArgs()) {
		...
		if ($instanceOf($RealType, tleft)) {
			$nc(il)->append($(tleft->CMP(this->_op == $Operators::LT || this->_op == $Operators::LE)));
			$init($Type);
		...
		}
		{
			...
			default:
				$init($ErrorMsg);
				$assign(msg, $new($ErrorMsg, $ErrorMsg::ILLEGAL_RELAT_OP_ERR, this));
...
$Class* RelationalExpr::load$($String* name, bool initialize) {
	...
	$loadClass(RelationalExpr, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(RelationalExpr);
	});
	return class$;
}
