...
$ObjectArray2* BreakIteratorInfo_th::getContents() {
	...
	return $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			"WordData"_s,
			"WordBreakIteratorData_th"_s
		}),
		$$new($ObjectArray, {
			"LineData"_s,
			"LineBreakIteratorData_th"_s
		}),
		$$new($ObjectArray, {
			"WordDictionary"_s,
			"thai_dict"_s
...
$Class* BreakIteratorInfo_th::load$($String* name, bool initialize) {
	...
	$loadClass(BreakIteratorInfo_th, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(BreakIteratorInfo_th);
	});
	return class$;
}
