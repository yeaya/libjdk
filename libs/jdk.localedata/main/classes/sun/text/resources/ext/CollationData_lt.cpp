...
$ObjectArray2* CollationData_lt::getContents() {
	return $new($ObjectArray2, {$$new($ObjectArray, {
		"Rule"_s,
		u"@& C < č , Č & I ; y = ı , Y = İ & S < š , Š & X < ý, Ý < ŷ , Ŷ < ÿ, Ÿ & Z < ž , Ž "_s
	})});
}
...
$Class* CollationData_lt::load$($String* name, bool initialize) {
	...
	$loadClass(CollationData_lt, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CollationData_lt);
	});
	return class$;
}
