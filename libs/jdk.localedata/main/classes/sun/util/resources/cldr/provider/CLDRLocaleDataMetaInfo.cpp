...
$Class* CLDRLocaleDataMetaInfo::load$($String* name, bool initialize) {
	...
	$loadClass(CLDRLocaleDataMetaInfo, name, initialize, &classInfo$$, CLDRLocaleDataMetaInfo::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(CLDRLocaleDataMetaInfo);
	});
	return class$;
}
