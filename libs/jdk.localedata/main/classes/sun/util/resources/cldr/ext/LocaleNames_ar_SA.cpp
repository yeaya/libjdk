...
$ObjectArray2* LocaleNames_ar_SA::getContents() {
	...
	$var($ObjectArray2, data, $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			"MO"_s,
			u"ماكاو الصينية (منطقة إدارية خاصة)"_s
		}),
		$$new($ObjectArray, {
			"MS"_s,
			u"مونتيسيرات"_s
...
$Class* LocaleNames_ar_SA::load$($String* name, bool initialize) {
	...
	$loadClass(LocaleNames_ar_SA, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LocaleNames_ar_SA);
	});
	return class$;
}
