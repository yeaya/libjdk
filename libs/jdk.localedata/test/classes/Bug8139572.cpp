...
void Bug8139572::main($StringArray* args) {
	$init(Bug8139572);
	$useLocalObjectStack();
	for (int32_t i = 0; i < Bug8139572::PATTERNS->length; ++i) {
		$var($SimpleDateFormat, fmt, $new($SimpleDateFormat, Bug8139572::PATTERNS->get(i), Bug8139572::RUSSIAN));
		$var($Date, standAloneDate, fmt->parse(Bug8139572::APPLIED->get(i)));
		$var($String, str, fmt->format(standAloneDate));
		if (!$nc(Bug8139572::EXPECTED->get(i))->equals(str)) {
			$throwNew($RuntimeException, $$str({"bad result: got \'"_s, str, "\', expected \'"_s, Bug8139572::EXPECTED->get(i), "\'"_s}));
		}
	}
	$var($SimpleDateFormat, fmt, $new($SimpleDateFormat, ""_s, Bug8139572::RUSSIAN));
	for (int32_t j = 0; j < Bug8139572::PATTERNS->length; ++j) {
		fmt->applyPattern(Bug8139572::PATTERNS->get(j));
		$var($String, str, fmt->format(Bug8139572::SEPT12));
		if (!$nc(Bug8139572::EXPECTED->get(j))->equals(str)) {
			$throwNew($RuntimeException, $$str({"bad result: got \'"_s, str, "\', expected \'"_s, Bug8139572::EXPECTED->get(j), "\'"_s}));
		}
	}
}

void Bug8139572::clinit$($Class* clazz) {
...
$Class* Bug8139572::load$($String* name, bool initialize) {
	...
	$loadClass(Bug8139572, name, initialize, &classInfo$$, Bug8139572::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Bug8139572);
	});
	return class$;
}
