...
$Class* Clipboard$$Lambda$lambda$setContents$0::load$($String* name, bool initialize) {
	...
	$loadClass(Clipboard$$Lambda$lambda$setContents$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Clipboard$$Lambda$lambda$setContents$0);
	});
	return class$;
}
...
$Class* Clipboard$$Lambda$lambda$fireFlavorsChanged$2$1::load$($String* name, bool initialize) {
	...
	$loadClass(Clipboard$$Lambda$lambda$fireFlavorsChanged$2$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Clipboard$$Lambda$lambda$fireFlavorsChanged$2$1);
	});
	return class$;
}
...
$Class* Clipboard$$Lambda$lambda$fireFlavorsChanged$1$2::load$($String* name, bool initialize) {
	...
	$loadClass(Clipboard$$Lambda$lambda$fireFlavorsChanged$1$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Clipboard$$Lambda$lambda$fireFlavorsChanged$1$2);
	});
	return class$;
}
...
void Clipboard::setContents($Transferable* contents, $ClipboardOwner* owner) {
	$synchronized(this) {
		...
		if (oldOwner != nullptr && oldOwner != owner) {
			$$nc($DataFlavorUtil::getDesktopService())->invokeOnEventThread($$new(Clipboard$$Lambda$lambda$setContents$0, this, oldOwner, oldContents));
		}
		fireFlavorsChanged();
	}
}
...
$Object* Clipboard::getData($DataFlavor* flavor) {
	...
	return $nc(cntnts)->getTransferData(flavor);
}
...
$FlavorListenerArray* Clipboard::getFlavorListeners() {
	$synchronized(this) {
		return this->flavorListeners == nullptr ? $new($FlavorListenerArray, 0) : $cast($FlavorListenerArray, this->flavorListeners->toArray($$new($FlavorListenerArray, this->flavorListeners->size())));
	}
}
...
void Clipboard::fireFlavorsChanged() {
	...
	$nc(this->flavorListeners)->forEach($$new(Clipboard$$Lambda$lambda$fireFlavorsChanged$2$1, this));
}
...
void Clipboard::lambda$fireFlavorsChanged$2($FlavorListener* listener) {
	$useLocalObjectStack();
	$$nc($DataFlavorUtil::getDesktopService())->invokeOnEventThread($$new(Clipboard$$Lambda$lambda$fireFlavorsChanged$1$2, this, listener));
}
...
$Class* Clipboard::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.awt.datatransfer.Clipboard$$Lambda$lambda$setContents$0")) {
			return Clipboard$$Lambda$lambda$setContents$0::load$(name, initialize);
		}
		if (name->equals("java.awt.datatransfer.Clipboard$$Lambda$lambda$fireFlavorsChanged$2$1")) {
			return Clipboard$$Lambda$lambda$fireFlavorsChanged$2$1::load$(name, initialize);
		}
		if (name->equals("java.awt.datatransfer.Clipboard$$Lambda$lambda$fireFlavorsChanged$1$2")) {
	...
	$loadClass(Clipboard, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Clipboard);
	});
	return class$;
}
