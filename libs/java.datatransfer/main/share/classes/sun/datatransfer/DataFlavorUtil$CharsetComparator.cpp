...
void DataFlavorUtil$CharsetComparator::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(DataFlavorUtil$CharsetComparator::UNSUPPORTED_CHARSET_INDEX, $Integer::valueOf($Integer::MIN_VALUE));
	{
		...
		$var($Object, var$0, $DataFlavorUtil::canonicalName("UTF-16LE"_s));
		charsetsMap->put(var$0, $($Integer::valueOf(4)));
		$var($Object, var$1, $DataFlavorUtil::canonicalName("UTF-16BE"_s));
		charsetsMap->put(var$1, $($Integer::valueOf(5)));
		$var($Object, var$2, $DataFlavorUtil::canonicalName("UTF-8"_s));
		charsetsMap->put(var$2, $($Integer::valueOf(6)));
		$var($Object, var$3, $DataFlavorUtil::canonicalName("UTF-16"_s));
...
$Class* DataFlavorUtil$CharsetComparator::load$($String* name, bool initialize) {
	...
	$loadClass(DataFlavorUtil$CharsetComparator, name, initialize, &classInfo$$, DataFlavorUtil$CharsetComparator::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(DataFlavorUtil$CharsetComparator);
	});
	return class$;
}
