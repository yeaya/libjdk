...
$Class* TimeoutOrdering$$Lambda$lambda$main$0::load$($String* name, bool initialize) {
	...
	$loadClass(TimeoutOrdering$$Lambda$lambda$main$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TimeoutOrdering$$Lambda$lambda$main$0);
	});
	return class$;
}
...
$Class* TimeoutOrdering$$Lambda$lambda$main$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(TimeoutOrdering$$Lambda$lambda$main$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TimeoutOrdering$$Lambda$lambda$main$1$1);
	});
	return class$;
}
		...
		try {
			try {
				...
				for (int32_t i = 0; i < $nc(TimeoutOrdering::TIMEOUTS)->length; ++i) {
					requests->set(i, $($$nc($$nc($$nc($HttpRequest::newBuilder(uri))->timeout($($Duration::ofSeconds(TimeoutOrdering::TIMEOUTS->get(i)))))->GET())->build()));
					$var($HttpRequest, req, requests->get(i));
					int32_t j = i;
					$var($CompletableFuture, response, $cast($CompletableFuture, $$nc($nc(client)->sendAsync(req, $($HttpResponse$BodyHandlers::replacing(nullptr))))->whenComplete($$new(TimeoutOrdering$$Lambda$lambda$main$0, j, req))));
				}
				$System::out->println("All requests submitted. Waiting ..."_s);
				checkReturnOrder(requests);
				if (TimeoutOrdering::error) {
					$throwNew($RuntimeException, "Failed. Check output"_s);
				}
				$System::out->println("--- TESTING Sync"_s);
				$var($ExecutorService, executor, $Executors::newCachedThreadPool());
				for (int32_t i = 0; i < $nc(TimeoutOrdering::TIMEOUTS)->length; ++i) {
					requests->set(i, $($$nc($$nc($$nc($HttpRequest::newBuilder(uri))->timeout($($Duration::ofSeconds(TimeoutOrdering::TIMEOUTS->get(i)))))->GET())->build()));
					$var($HttpRequest, req, requests->get(i));
					int32_t j = i;
					$nc(executor)->execute($$new(TimeoutOrdering$$Lambda$lambda$main$1$1, client, req, j));
				}
				$System::out->println("All requests submitted. Waiting ..."_s);
				checkReturnOrder(requests);
...
void TimeoutOrdering::checkReturnOrder($HttpRequestArray* requests) {
	...
	for (int32_t j = 0; j < $nc(TimeoutOrdering::TIMEOUTS)->length; ++j) {
		...
		var$0->append("Got request from queue "_s);
		var$0->append(req);
		var$0->append(", order: "_s);
		var$0->append($(getRequest(req, requests)));
		$nc($System::out)->println($$str(var$0));
		switch (j) {
		case 0:
		case 1:
			if (!(req == $nc(requests)->get(1) || req == requests->get(5))) {
		...
		case 3:
		case 4:
			if (!(req == $nc(requests)->get(0) || req == requests->get(3) || req == requests->get(4))) {
...
void TimeoutOrdering::lambda$main$1($HttpClient* client, $HttpRequest* req, int32_t j) {
	...
	try {
		try {
			...
			$var($Throwable, c, ee->getCause() == nullptr ? $cast($Throwable, ee) : ee->getCause());
...
void TimeoutOrdering::clinit$($Class* clazz) {
...
$Class* TimeoutOrdering::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("TimeoutOrdering$$Lambda$lambda$main$0")) {
			return TimeoutOrdering$$Lambda$lambda$main$0::load$(name, initialize);
		}
		if (name->equals("TimeoutOrdering$$Lambda$lambda$main$1$1")) {
	...
	$loadClass(TimeoutOrdering, name, initialize, &classInfo$$, TimeoutOrdering::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(TimeoutOrdering);
	});
	return class$;
}
