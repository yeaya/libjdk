...
void B5087907::main($StringArray* args) {
	...
	bool preferIPv6Addresses = (prop == nullptr) ? false : prop->equals("true"_s);
	$nc($System::out)->println($$str({"java.net.preferIPv6Addresses: "_s, $$str(preferIPv6Addresses)}));
	$System::out->println("localhost resolves to:"_s);
	{
		$var($InetAddressArray, arr$, addrs);
		for (int32_t len$ = $nc(arr$)->length, i$ = 0; i$ < len$; ++i$) {
			$var($InetAddress, addr, arr$->get(i$));
	...
	$System::out->println($$str({"InetAddres.getByName returned: "_s, lh}));
	bool failed = false;
...
$Class* B5087907::load$($String* name, bool initialize) {
	...
	$loadClass(B5087907, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(B5087907);
	});
	return class$;
}
