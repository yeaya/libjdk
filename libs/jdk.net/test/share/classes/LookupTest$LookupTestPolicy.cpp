...
void LookupTest$LookupTestPolicy::init$() {
	...
	this->perms->add($$new($NetPermission, "setProxySelector"_s));
	this->perms->add($$new($SocketPermission, "localhost:1024-"_s, "resolve,accept"_s));
	$init($LookupTest);
	this->perms->add($$new($URLPermission, $$str({"http://allowedAndFound.com:"_s, $$str($LookupTest::port), "/-"_s}), "*:*"_s));
	this->perms->add($$new($URLPermission, $$str({"http://allowedButNotfound.com:"_s, $$str($LookupTest::port), "/-"_s}), "*:*"_s));
	this->perms->add($$new($FilePermission, "<<ALL FILES>>"_s, "read,write,delete"_s));
}
...
bool LookupTest$LookupTestPolicy::implies($ProtectionDomain* domain, $Permission* perm) {
	bool var$0 = this->perms->implies(perm);
	$init($LookupTest);
...
$Class* LookupTest$LookupTestPolicy::load$($String* name, bool initialize) {
	...
	$loadClass(LookupTest$LookupTestPolicy, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LookupTest$LookupTestPolicy);
	});
	return class$;
}
