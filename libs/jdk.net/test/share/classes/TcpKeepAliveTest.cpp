							...
							try {
								...
								try {
									try {
										...
										if ($$nc(ss->supportedOptions())->contains($ExtendedSocketOptions::TCP_KEEPIDLE)) {
											ss->setOption($ExtendedSocketOptions::TCP_KEEPIDLE, $($Integer::valueOf(TcpKeepAliveTest::DEFAULT_KEEP_ALIVE_TIME)));
											if ($$sure($Integer, ss->getOption($ExtendedSocketOptions::TCP_KEEPIDLE))->intValue() != TcpKeepAliveTest::DEFAULT_KEEP_ALIVE_TIME) {
												$throwNew($RuntimeException, $$str({"Test failed, TCP_KEEPIDLE should have been "_s, $$str(TcpKeepAliveTest::DEFAULT_KEEP_ALIVE_TIME)}));
											}
										}
										if ($$nc(ss->supportedOptions())->contains($ExtendedSocketOptions::TCP_KEEPCOUNT)) {
											ss->setOption($ExtendedSocketOptions::TCP_KEEPCOUNT, $($Integer::valueOf(TcpKeepAliveTest::DEFAULT_KEEP_ALIVE_PROBES)));
											if ($$sure($Integer, ss->getOption($ExtendedSocketOptions::TCP_KEEPCOUNT))->intValue() != TcpKeepAliveTest::DEFAULT_KEEP_ALIVE_PROBES) {
										...
										if ($$nc(ss->supportedOptions())->contains($ExtendedSocketOptions::TCP_KEEPINTERVAL)) {
											ss->setOption($ExtendedSocketOptions::TCP_KEEPINTERVAL, $($Integer::valueOf(TcpKeepAliveTest::DEFAULT_KEEP_ALIVE_INTVL)));
											if ($$sure($Integer, ss->getOption($ExtendedSocketOptions::TCP_KEEPINTERVAL))->intValue() != TcpKeepAliveTest::DEFAULT_KEEP_ALIVE_INTVL) {
										...
										if ($$nc(s->supportedOptions())->contains($ExtendedSocketOptions::TCP_KEEPIDLE)) {
											...
											if ($$sure($Integer, s->getOption($ExtendedSocketOptions::TCP_KEEPIDLE))->intValue() != TcpKeepAliveTest::DEFAULT_KEEP_ALIVE_TIME) {
												$throwNew($RuntimeException, $$str({"Test failed, TCP_KEEPIDLE should have been "_s, $$str(TcpKeepAliveTest::DEFAULT_KEEP_ALIVE_TIME)}));
											}
										}
										if ($$nc(s->supportedOptions())->contains($ExtendedSocketOptions::TCP_KEEPCOUNT)) {
											...
											if ($$sure($Integer, s->getOption($ExtendedSocketOptions::TCP_KEEPCOUNT))->intValue() != TcpKeepAliveTest::DEFAULT_KEEP_ALIVE_PROBES) {
												$throwNew($RuntimeException, $$str({"Test failed, TCP_KEEPCOUNT should have been "_s, $$str(TcpKeepAliveTest::DEFAULT_KEEP_ALIVE_PROBES)}));
											}
										}
										if ($$nc(s->supportedOptions())->contains($ExtendedSocketOptions::TCP_KEEPINTERVAL)) {
											...
											if ($$sure($Integer, s->getOption($ExtendedSocketOptions::TCP_KEEPINTERVAL))->intValue() != TcpKeepAliveTest::DEFAULT_KEEP_ALIVE_INTVL) {
												$throwNew($RuntimeException, $$str({"Test failed, TCP_KEEPINTERVAL should have been "_s, $$str(TcpKeepAliveTest::DEFAULT_KEEP_ALIVE_INTVL)}));
											}
										}
...
$Class* TcpKeepAliveTest::load$($String* name, bool initialize) {
	...
	$loadClass(TcpKeepAliveTest, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TcpKeepAliveTest);
	});
	return class$;
}
