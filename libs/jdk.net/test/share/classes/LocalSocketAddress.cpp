		...
		$var($List, nics, $Collections::list($($NetworkInterface::getNetworkInterfaces())));
		{
			...
			for (; $nc(i$)->hasNext();) {
				$var($NetworkInterface, nic, $cast($NetworkInterface, i$->next()));
				{
					...
					$var($List, addrs, $Collections::list($(nic->getInetAddresses())));
					{
...
$Class* LocalSocketAddress::load$($String* name, bool initialize) {
	...
	$loadClass(LocalSocketAddress, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LocalSocketAddress);
	});
	return class$;
}
