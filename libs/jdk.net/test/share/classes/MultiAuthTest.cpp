...
void MultiAuthTest::main($StringArray* args) {
	...
	try {
		...
		$System::out->println($$str({"URI: "_s, uri}));
		$System::out->println("\nTesting with client #1, Authenticator #1"_s);
		test(client1, ca, uri, 1, nullptr);
		$System::out->println("Testing again with client #1, Authenticator #1"_s);
		test(client1, ca, uri, 1, nullptr);
		$System::out->println("Testing with client #2, Authenticator #1"_s);
		test(client2, ca, uri, 2, nullptr);
		$System::out->println("Testing with default client (HttpClient.newHttpClient()), no authenticator"_s);
		$load($IOException);
		test($($HttpClient::newHttpClient()), ca, uri, 2, $IOException::class$);
		$System::out->println("\nSetting default authenticator\n"_s);
		$Authenticator::setDefault(ca);
		$System::out->println("Testing default client (HttpClient.newHttpClient()), no authenticator"_s);
		test($($HttpClient::newHttpClient()), ca, uri, 3, $IOException::class$);
		$System::out->println("Testing with client #4, no authenticator"_s);
		...
		$set(sa, passwd, "changed"_s);
		$System::out->println("\nChanged server password\n"_s);
		$System::out->println($$str({"Testing with client #1, Authenticator #1 (count="_s, $$str($nc(ca->count)->get()), ")"_s}));
		test(client1, ca, uri, 7, $IOException::class$);
		$System::out->println($$str({"Testing again with client #1, Authenticator #1 (count="_s, $$str(ca->count->get()), ")"_s}));
		test(client1, ca, uri, 10, $IOException::class$);
		$System::out->println($$str({"Testing with client #2, Authenticator #1 (count="_s, $$str(ca->count->get()), ")"_s}));
		test(client2, ca, uri, 14, $IOException::class$);
		$System::out->println($$str({"\nRestored server password (count="_s, $$str(ca->count->get()), ")\n"_s}));
		$set(sa, passwd, oldpwd);
		int32_t count = ca->count->get();
		$System::out->println("Testing with client #1, Authenticator #1"_s);
...
void MultiAuthTest::test($HttpClient* client, $MultiAuthTest$ClientAuth* ca, $URI* uri, int32_t expectCount, $Class* expectFailure) {
	...
	try {
		...
		MultiAuthTest::ok = var$0 && $$nc(resp->body())->equals(MultiAuthTest::RESPONSE);
		bool var$1 = resp->statusCode() == 401;
		if (var$1 || resp->statusCode() == 407) {
			$throwNew($IOException, $($String::valueOf(resp)));
		}
	...
	if (!MultiAuthTest::ok || $nc($nc(ca)->count)->get() != expectCount) {
		$throwNew($RuntimeException, $$str({"Test failed: ok="_s, $$str(MultiAuthTest::ok), " count="_s, $$str($nc($nc(ca)->count)->get()), " (expected="_s, $$str(expectCount), ")"_s}));
	}
	$assign(resp, $nc(client)->send(req, $($HttpResponse$BodyHandlers::ofString())));
	bool var$2 = $nc(resp)->statusCode() == 200;
	MultiAuthTest::ok = var$2 && $$nc(resp->body())->equals(MultiAuthTest::RESPONSE);
	if (!MultiAuthTest::ok || $nc($nc(ca)->count)->get() != expectCount) {
		$throwNew($RuntimeException, $$str({"Test failed: ok="_s, $$str(MultiAuthTest::ok), " count="_s, $$str($nc($nc(ca)->count)->get()), " (expected="_s, $$str(expectCount), ")"_s}));
	}
	...
	MultiAuthTest::ok = $nc(resp)->statusCode() == 200;
	if (!MultiAuthTest::ok || $nc($nc(ca)->count)->get() != expectCount) {
		$throwNew($RuntimeException, "Test failed"_s);
	}
}

void MultiAuthTest::clinit$($Class* clazz) {
...
$Class* MultiAuthTest::load$($String* name, bool initialize) {
	...
	$loadClass(MultiAuthTest, name, initialize, &classInfo$$, MultiAuthTest::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(MultiAuthTest);
	});
	return class$;
}
