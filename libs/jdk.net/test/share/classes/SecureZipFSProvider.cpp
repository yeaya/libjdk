...
$Class* SecureZipFSProvider$$Lambda$lambda$newFileSystem$0::load$($String* name, bool initialize) {
	...
	$loadClass(SecureZipFSProvider$$Lambda$lambda$newFileSystem$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SecureZipFSProvider$$Lambda$lambda$newFileSystem$0);
	});
	return class$;
}
...
$FileSystem* SecureZipFSProvider::newFileSystem($FileSystem* fs) {
	return $cast($FileSystem, this->map->computeIfAbsent(fs, $$new(SecureZipFSProvider$$Lambda$lambda$newFileSystem$0, this, fs)));
}
...
$FileSystem* SecureZipFSProvider::getFileSystem($URI* uri) {
	return $cast($FileSystem, this->map->get($($nc(this->defaultProvider)->getFileSystem(uri))));
}

$Path* SecureZipFSProvider::getPath($URI* uri) {
	$useLocalObjectStack();
	$var($Path, p, $nc(this->defaultProvider)->getPath(uri));
	return $$sure($SecureZipFSProvider$SecureZipFS, this->map->get($(this->defaultProvider->getFileSystem(uri))))->wrap(p);
}
...
$Class* SecureZipFSProvider::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("SecureZipFSProvider$$Lambda$lambda$newFileSystem$0")) {
	...
	$loadClass(SecureZipFSProvider, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SecureZipFSProvider);
	});
	return class$;
}
