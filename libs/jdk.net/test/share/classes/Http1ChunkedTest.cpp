...
void Http1ChunkedTest::main($StringArray* args) {
	...
	for (; $nc(i$)->hasNext();) {
		...
		{
			test($($nc(resp)->name()), $nc(resp)->headers, $nc(resp)->body, $nc(resp)->delay);
		}
	}
}
	...
	$useLocalObjectStack();
	{
		...
		try {
			try {
				...
				bool received = false;
				while (!received) {
					$System::out->printf("%s: Recieving request...%n"_s, $$new($ObjectArray, {name}));
					{
							...
							try {
								...
								while (!$nc(($assign(line, reader->readLine())))->isEmpty()) {
									if ($nc(line)->contains(path)) {
										received = true;
										$System::out->printf("%s: Recieved request: %s%n"_s, $$new($ObjectArray, {
											name,
											line
										}));
									}
								...
								if (!received) {
									$System::out->printf("%s: Unexpected headers received: dropping request.%n"_s, $$new($ObjectArray, 0));
									// continue;
									continue$4 = true;
									goto $finally1;
								}
								...
								os->flush();
								$System::out->printf("%s: Response headers sent.%n"_s, $$new($ObjectArray, {name}));
								{
								...
								$System::out->printf("%s: Response body sent.%n"_s, $$new($ObjectArray, {name}));
								$var($HttpResponse, resp, $cast($HttpResponse, $nc(respCF)->join()));
								$System::out->printf("%s: Response received:%n"_s, $$new($ObjectArray, {name}));
								{
									$System::out->println(resp);
								}
								validate($$cast($String, $nc(resp)->body()));
								$System::out->printf("%n%s: body validated%n%n"_s, $$new($ObjectArray, {name}));
							} catch ($Throwable& t$) {
...
void Http1ChunkedTest::validate($String* response) {
	...
	for (int32_t i = 0; i < $nc(response)->length(); ++i) {
		...
		if (var$0 != Http1ChunkedTest::LOREM->charAt($mod(i, Http1ChunkedTest::LOREM->length()))) {
...
void Http1ChunkedTest::clinit$($Class* clazz) {
...
$Class* Http1ChunkedTest::load$($String* name, bool initialize) {
	...
	$loadClass(Http1ChunkedTest, name, initialize, &classInfo$$, Http1ChunkedTest::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Http1ChunkedTest);
	});
	return class$;
}
