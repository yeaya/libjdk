...
void SocketPermissionCollection::main($StringArray* args) {
	...
	perms->add($$new($SocketPermission, "www.example.com"_s, "connect"_s));
	...
	$System::out->println("test 9: implies returns true for matching port range and action"_s);
	...
	$System::out->println("test 10: elements returns correct number of perms"_s);
...
$Class* SocketPermissionCollection::load$($String* name, bool initialize) {
	...
	$loadClass(SocketPermissionCollection, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
