...
bool URLPermissionTest$URLImpliesTest::execute() {
	...
	if (result != this->expected) {
		...
		$System::out->println($$str({"p2 = "_s, p2}));
	}
...
$Class* URLPermissionTest$URLImpliesTest::load$($String* name, bool initialize) {
	...
	$loadClass(URLPermissionTest$URLImpliesTest, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(URLPermissionTest$URLImpliesTest);
	});
	return class$;
}
