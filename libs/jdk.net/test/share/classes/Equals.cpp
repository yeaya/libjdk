...
int32_t Equals::compareJarURLS($String* urlStr1, $String* urlStr2, $String* entry1, $String* entry2, bool expectEqual) {
	...
	if (!url1->equals(url2)) {
		...
		$System::out->println($$str({"url1: "_s, url1, ", url2:"_s, url2}));
		return 1;
	}
	...
	if (expectEqual) {
		...
		if (hash1 != hash2) {
			...
			$System::out->println($$str({"jarUrl2.hashCode = "_s, $$str(hash2)}));
			$System::out->println($$str({"Equal urls should have same hashCode. "_s, jarUrl1, ", "_s, jarUrl2}));
			++failed;
		}
	}
	return failed;
...
void Equals::clinit$($Class* clazz) {
...
$Class* Equals::load$($String* name, bool initialize) {
	...
	$loadClass(Equals, name, initialize, &classInfo$$, Equals::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Equals);
	});
	return class$;
}
