...
$ObjectArray2* jar_zh_CN::getContents() {
	...
	return $new($ObjectArray2, {
		$$new($ObjectArray, {
			"error.bad.cflag"_s,
			u"\'c\' 标记要求指定清单或输入文件!"_s
		}),
		$$new($ObjectArray, {
			"error.bad.dflag"_s,
			u"\'-d, --describe-module\' 选项不需要指定任何输入文件"_s
		}),
		$$new($ObjectArray, {
			"error.bad.eflag"_s,
			u"不能同时指定 \'e\' 标记和具有 \'Main-Class\' 属性的\n清单!"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			"error.bad.option"_s,
			u"必须指定 {ctxuid} 中的任一选项。"_s
		}),
		$$new($ObjectArray, {
			"error.bad.reason"_s,
			u"错误的原因: {0}, 必须为 deprecated, deprecated-for-removal 或 incubating 之一"_s
		}),
		$$new($ObjectArray, {
			"error.bad.uflag"_s,
			u"\'u\' 标记要求指定清单, \'e\' 标记或输入文件!"_s
		}),
		$$new($ObjectArray, {
			"error.cant.open"_s,
			u"无法打开: {0}"_s
		...
		$$new($ObjectArray, {
			"error.hash.dep"_s,
			u"正在对模块 {0} 的被依赖对象执行散列处理, 在模块路径中找不到模块 {1}"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"--module-version 或 --hash-modules 之一没有 module-info.class"_s
		}),
		$$new($ObjectArray, {
			"error.multiple.main.operations"_s,
			u"不能指定多个 \'-cuxtid\' 选项"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"条目 {0} 包含与较早版本不兼容的类版本"_s
		}),
		$$new($ObjectArray, {
			"error.validator.info.exports.notequal"_s,
			u"版本化目录中的 module-info.class 包含不同的 \"exports\""_s
		}),
		$$new($ObjectArray, {
			"error.validator.info.manclass.notequal"_s,
			u"{0}: 版本化目录中的 module-info.class 包含不同的 \"main-class\""_s
		}),
		$$new($ObjectArray, {
			"error.validator.info.name.notequal"_s,
			u"版本化目录中的 module-info.class 包含不正确的名称"_s
		}),
		$$new($ObjectArray, {
			"error.validator.info.opens.notequal"_s,
			u"版本化目录中的 module-info.class 包含不同的 \"opens\""_s
		}),
		$$new($ObjectArray, {
			"error.validator.info.provides.notequal"_s,
			u"版本化目录中的 module-info.class 包含不同的 \"provides\""_s
		}),
		$$new($ObjectArray, {
			"error.validator.info.requires.added"_s,
			u"版本化目录中的 module-info.class 包含额外的 \"requires\""_s
		}),
		$$new($ObjectArray, {
			"error.validator.info.requires.dropped"_s,
			u"版本化目录中的 module-info.class 包含缺少的 \"requires\""_s
		}),
		$$new($ObjectArray, {
			"error.validator.info.requires.transitive"_s,
			u"版本化目录中的 module-info.class 包含额外的 \"requires transitive\""_s
		}),
		$$new($ObjectArray, {
			"error.validator.info.version.notequal"_s,
			u"{0}: 版本化目录中的 module-info.class 包含不同的 \"version\""_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"删除的多发行版 jar 文件 {0} 无效"_s
		}),
		$$new($ObjectArray, {
			"error.validator.names.mismatch"_s,
			u"条目 {0} 包含内部名称为 {1} 的类, 名称不匹配"_s
		}),
		$$new($ObjectArray, {
			"error.validator.new.public.class"_s,
			u"条目 {0} 包含未在基础条目中找到的新公共类"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"  -d, --describe-module      输出模块描述符或自动模块名称"_s
		}),
		$$new($ObjectArray, {
			"main.help.opt.main.extract"_s,
			u"  -x, --extract              从档案中提取指定的 (或全部) 文件"_s
		}),
		$$new($ObjectArray, {
			"main.help.opt.main.generate-index"_s,
			u"  -i, --generate-index=FILE  为指定的 jar 档案生成\n                             索引信息"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"  -?, -h, --help[:compat]    提供此帮助，也可以选择性地提供兼容性帮助"_s
		}),
		$$new($ObjectArray, {
			"main.help.opt.other.help-extra"_s,
			u"      --help-extra           提供额外选项的帮助"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"警告：{0} 选项已过时，计划在未来的 JDK 发行版中删除。\n"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"警告: 条目 {0} 包含与 jar 中的\n现有条目相同的类"_s
		}),
		$$new($ObjectArray, {
			"warn.validator.resources.with.same.name"_s,
			u"警告: 条目 {0}, 多个资源具有相同名称"_s
		})
	});
}
...
$Class* jar_zh_CN::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PUBLIC, $method(jar_zh_CN, init$, void)},
		{"getContents", "()[[Ljava/lang/Object;", nullptr, $PROTECTED | $FINAL, $virtualMethod(jar_zh_CN, getContents, $ObjectArray2*)},
		{}
	};
	$ClassInfo classInfo$$ = {
	...
	$loadClass(jar_zh_CN, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(jar_zh_CN);
	});
	return class$;
}
