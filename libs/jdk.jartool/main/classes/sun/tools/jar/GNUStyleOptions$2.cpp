...
void GNUStyleOptions$2::process($Main* tool, $String* opt, $String* arg) {
	$useLocalObjectStack();
	if ($nc(tool)->cflag || tool->tflag || tool->uflag || tool->xflag || tool->dflag || tool->validate) {
...
$Class* GNUStyleOptions$2::load$($String* name, bool initialize) {
	...
	$loadClass(GNUStyleOptions$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(GNUStyleOptions$2);
	});
	return class$;
}
