...
void GNUStyleOptions$1::process($Main* tool, $String* opt, $String* arg) {
	$useLocalObjectStack();
	if ($nc(tool)->iflag || tool->tflag || tool->uflag || tool->xflag || tool->dflag || tool->validate) {
...
$Class* GNUStyleOptions$1::load$($String* name, bool initialize) {
	...
	$loadClass(GNUStyleOptions$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(GNUStyleOptions$1);
	});
	return class$;
}
