	...
	if (props != nullptr) {
		...
		if (realms != nullptr) {
			...
			for (int32_t i = 0; i < tokenCount; ++i) {
				...
				$init($AbstractSaslImpl);
				$init($Level);
				$nc($AbstractSaslImpl::logger)->log($Level::FINE, "DIGEST81:Server supports realm {0}"_s, token);
...
$bytes* DigestMD5Server::evaluateResponse($bytes* response) {
	...
	$var($bytes, challenge, nullptr);
	{
		...
		case 1:
			...
			if ((this->allQop & $AbstractSaslImpl::PRIVACY_PROTECTION) != 0) {
				...
				for (int32_t i = 0; i < $nc($DigestMD5Base::CIPHER_TOKENS)->length; ++i) {
					if ($nc(this->myCiphers)->get(i) != 0) {
						...
						sb->append($DigestMD5Base::CIPHER_TOKENS->get(i));
					}
				}
...
$bytes* DigestMD5Server::generateChallenge($List* realms, $String* qopStr, $String* cipherStr) {
	...
	for (int32_t i = 0; realms != nullptr && i < realms->size(); ++i) {
		...
		writeQuotedStringValue(out, $($$sure($String, realms->get(i))->getBytes(this->encoding)));
		out->write(u'\"');
		out->write(u',');
	}
	...
	if (this->useUTF8) {
		out->write($(DigestMD5Server::UTF8_DIRECTIVE->getBytes(this->encoding)));
	}
	...
	out->write($(DigestMD5Server::ALGORITHM_DIRECTIVE->getBytes(this->encoding)));
...
$bytes* DigestMD5Server::validateClientResponse($byteArray2* responseVal) {
	...
	int32_t clntMaxBufSize = (responseVal->get(DigestMD5Server::MAXBUF) == nullptr) ? $DigestMD5Base::DEFAULT_MAXBUF : $Integer::parseInt($$new($String, responseVal->get(DigestMD5Server::MAXBUF), this->encoding));
	...
	if (responseVal->get(DigestMD5Server::USERNAME) != nullptr) {
		$assign(username, $new($String, responseVal->get(DigestMD5Server::USERNAME), this->encoding));
		$init($AbstractSaslImpl);
		$init($Level);
		$nc($AbstractSaslImpl::logger)->log($Level::FINE, "DIGEST82:Username: {0}"_s, username);
	} else {
		$throwNew($SaslException, "DIGEST-MD5: digest response format violation. Missing username."_s);
	}
	$set(this, negotiatedRealm, (responseVal->get(DigestMD5Server::REALM) != nullptr) ? $new($String, responseVal->get(DigestMD5Server::REALM), this->encoding) : ""_s);
	$init($AbstractSaslImpl);
	$init($Level);
	$nc($AbstractSaslImpl::logger)->log($Level::FINE, "DIGEST83:Client negotiated realm: {0}"_s, this->negotiatedRealm);
	...
	if (responseVal->get(DigestMD5Server::NONCE) == nullptr) {
		$throwNew($SaslException, "DIGEST-MD5: digest response format violation. Missing nonce."_s);
	}
	$var($bytes, nonceFromClient, responseVal->get(DigestMD5Server::NONCE));
	...
	$AbstractSaslImpl::logger->log($Level::FINE, "DIGEST84:Client negotiated qop: {0}"_s, this->negotiatedQop);
	int8_t cQop = 0;
	{
	...
	if (this->privacy) {
		...
		$AbstractSaslImpl::logger->log($Level::FINE, "DIGEST85:Client negotiated cipher: {0}"_s, this->negotiatedCipher);
		$init($DigestMD5Base);
		for (int32_t j = 0; j < $nc($DigestMD5Base::CIPHER_TOKENS)->length; ++j) {
			if ($nc(this->negotiatedCipher)->equals($DigestMD5Base::CIPHER_TOKENS->get(j)) && $nc(this->myCiphers)->get(j) != 0) {
				foundCipher = j;
				break;
			}
		}
		...
		if (($nc($DigestMD5Base::CIPHER_MASKS)->get(foundCipher) & $AbstractSaslImpl::HIGH_STRENGTH) != 0) {
			$set(this, negotiatedStrength, "high"_s);
		} else if (($DigestMD5Base::CIPHER_MASKS->get(foundCipher) & $AbstractSaslImpl::MEDIUM_STRENGTH) != 0) {
			$set(this, negotiatedStrength, "medium"_s);
		} else {
			$set(this, negotiatedStrength, "low"_s);
		}
		$AbstractSaslImpl::logger->log($Level::FINE, "DIGEST86:Negotiated strength: {0}"_s, this->negotiatedStrength);
	}
	...
	if (digestUriFromResponse != nullptr) {
		$AbstractSaslImpl::logger->log($Level::FINE, "DIGEST87:digest URI: {0}"_s, digestUriFromResponse);
	}
	...
	if (authzidBytes != nullptr) {
		$AbstractSaslImpl::logger->log($Level::FINE, "DIGEST88:Authzid: {0}"_s, $$new($String, authzidBytes));
	}
...
void DigestMD5Server::clinit$($Class* clazz) {
...
$Class* DigestMD5Server::load$($String* name, bool initialize) {
	...
	$loadClass(DigestMD5Server, name, initialize, &classInfo$$, DigestMD5Server::clinit$, []($Class* clazz) -> $Object* {
