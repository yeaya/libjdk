...
$PreferencesFactory* Preferences::factory() {
	...
	$var($String, factoryName, $cast($String, $AccessController::doPrivileged($$new($Preferences$1))));
	...
	return $cast($PreferencesFactory, $AccessController::doPrivileged($$new($Preferences$2)));
}
...
$String* Preferences::nodeName($Class* c) {
	...
	$var($String, className, c->getName());
	int32_t pkgEndIndex = $nc(className)->lastIndexOf(u'.');
	if (pkgEndIndex < 0) {
		return "/<unnamed>"_s;
	}
...
void Preferences::clinit$($Class* clazz) {
...
$Class* Preferences::load$($String* name, bool initialize) {
	...
	$loadClass(Preferences, name, initialize, &classInfo$$, Preferences::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Preferences);
	});
	return class$;
}
