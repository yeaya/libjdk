...
void EUC_JP$Encoder::clinit$($Class* clazz) {
	$useLocalObjectStack();
...
$Class* EUC_JP$Encoder::load$($String* name, bool initialize) {
	...
	$loadClass(EUC_JP$Encoder, name, initialize, &classInfo$$, EUC_JP$Encoder::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(EUC_JP$Encoder);
	});
	return class$;
}
