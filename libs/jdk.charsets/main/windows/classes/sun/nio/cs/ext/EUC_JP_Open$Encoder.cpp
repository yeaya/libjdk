...
void EUC_JP_Open$Encoder::clinit$($Class* clazz) {
	$useLocalObjectStack();
...
$Class* EUC_JP_Open$Encoder::load$($String* name, bool initialize) {
	...
	$loadClass(EUC_JP_Open$Encoder, name, initialize, &classInfo$$, EUC_JP_Open$Encoder::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(EUC_JP_Open$Encoder);
	});
	return class$;
}
