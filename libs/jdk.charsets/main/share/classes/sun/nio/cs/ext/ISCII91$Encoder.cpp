...
bool ISCII91$Encoder::canEncode(char16_t ch) {
	$init($ISCII91);
	return ((ch >= (char16_t)0x0900 && ch <= (char16_t)0x097f && $nc($ISCII91::encoderMappingTable)->get(2 * (ch - (char16_t)0x0900)) != ISCII91$Encoder::NO_CHAR) || (ch == (char16_t)0x200d) || (ch == (char16_t)0x200c) || (ch <= (char16_t)0x7f));
}
...
$CoderResult* ISCII91$Encoder::encodeArrayLoop($CharBuffer* src, $ByteBuffer* dst) {
	...
	try {
		...
		while (sp < sl) {
			...
			if (inputChar == 8204) {
				inputChar = (char16_t)0x094d;
			} else if (inputChar == 8205) {
				inputChar = (char16_t)0x093c;
			}
			...
			if (index == $Integer::MIN_VALUE || $nc($ISCII91::encoderMappingTable)->get(index) == ISCII91$Encoder::NO_CHAR) {
				...
				if ($nc($ISCII91::encoderMappingTable)->get(index + 1) == ISCII91$Encoder::NO_CHAR) {
					...
					$nc(da)->set(dp++, $ISCII91::encoderMappingTable->get(index));
				} else {
					if (dl - dp < 2) {
						$init($CoderResult);
						$assign(var$6, $CoderResult::OVERFLOW);
					...
					$nc(da)->set(dp++, $ISCII91::encoderMappingTable->get(index));
					da->set(dp++, $ISCII91::encoderMappingTable->get(index + 1));
				}
				++sp;
			}
		}
	...
	try {
		...
		while (src->hasRemaining()) {
			...
			if (index == $Integer::MIN_VALUE || $nc($ISCII91::encoderMappingTable)->get(index) == ISCII91$Encoder::NO_CHAR) {
				...
				return$1 = true;
				goto $finally;
			} else if ($nc($ISCII91::encoderMappingTable)->get(index + 1) == ISCII91$Encoder::NO_CHAR) {
				if ($nc(dst)->remaining() < 1) {
					$init($CoderResult);
					$assign(var$2, $CoderResult::OVERFLOW);
					return$1 = true;
				...
				dst->put($ISCII91::encoderMappingTable->get(index));
			} else {
				...
				dst->put($ISCII91::encoderMappingTable->get(index));
				dst->put($ISCII91::encoderMappingTable->get(index + 1));
			}
			++mark;
		}
		$init($CoderResult);
...
$Class* ISCII91$Encoder::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(Ljava/nio/charset/Charset;)V", nullptr, $PRIVATE, $method(ISCII91$Encoder, init$, void, $Charset*)},
		{"canEncode", "(C)Z", nullptr, $PUBLIC, $virtualMethod(ISCII91$Encoder, canEncode, bool, char16_t)},
		{"encodeArrayLoop", "(Ljava/nio/CharBuffer;Ljava/nio/ByteBuffer;)Ljava/nio/charset/CoderResult;", nullptr, $PRIVATE, $method(ISCII91$Encoder, encodeArrayLoop, $CoderResult*, $CharBuffer*, $ByteBuffer*)},
		{"encodeBufferLoop", "(Ljava/nio/CharBuffer;Ljava/nio/ByteBuffer;)Ljava/nio/charset/CoderResult;", nullptr, $PRIVATE, $method(ISCII91$Encoder, encodeBufferLoop, $CoderResult*, $CharBuffer*, $ByteBuffer*)},
		{"encodeLoop", "(Ljava/nio/CharBuffer;Ljava/nio/ByteBuffer;)Ljava/nio/charset/CoderResult;", nullptr, $PROTECTED, $virtualMethod(ISCII91$Encoder, encodeLoop, $CoderResult*, $CharBuffer*, $ByteBuffer*)},
		{}
	};
	...
	$loadClass(ISCII91$Encoder, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ISCII91$Encoder);
	});
	return class$;
}
