...
ResponseBodyHandlers$PathBodyHandler* ResponseBodyHandlers$PathBodyHandler::create($Path* file, $List* openOptions) {
	...
	if (!ResponseBodyHandlers$PathBodyHandler::$assertionsDisabled && !(filePermission == nullptr || $$nc(filePermission->getActions())->equals("write"_s))) {
...
void ResponseBodyHandlers$PathBodyHandler::clinit$($Class* clazz) {
...
$Class* ResponseBodyHandlers$PathBodyHandler::load$($String* name, bool initialize) {
	...
	$loadClass(ResponseBodyHandlers$PathBodyHandler, name, initialize, &classInfo$$, ResponseBodyHandlers$PathBodyHandler::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ResponseBodyHandlers$PathBodyHandler);
	});
	return class$;
}
