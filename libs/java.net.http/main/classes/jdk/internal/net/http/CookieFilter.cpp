...
void CookieFilter::request($HttpRequestImpl* r, $MultiExchange* e) {
	...
	if ($nc(cookieHandlerOpt)->isPresent()) {
		...
		if ($nc(cookies)->isEmpty()) {
			$Log::logTrace("Request: no cookie to add for {0}"_s, $$new($ObjectArray, {$(r->uri())}));
		} else {
			$Log::logTrace("Request: adding cookies for {0}"_s, $$new($ObjectArray, {$(r->uri())}));
		}
		{
		...
		$Log::logTrace("Request: No cookie manager found for {0}"_s, $$new($ObjectArray, {$($nc(r)->uri())}));
	}
}
...
$HttpRequestImpl* CookieFilter::response($Response* r) {
	...
	$Log::logTrace("Response: processing cookies for {0}"_s, $$new($ObjectArray, {$($nc(request)->uri())}));
	$var($Optional, cookieHandlerOpt, $$nc($nc(e)->client())->cookieHandler());
	if ($nc(cookieHandlerOpt)->isPresent()) {
		$var($CookieHandler, cookieHandler, $cast($CookieHandler, cookieHandlerOpt->get()));
		$Log::logTrace("Response: parsing cookies from {0}"_s, $$new($ObjectArray, {$($nc(hdrs)->map())}));
		$var($URI, var$0, request->uri());
		$nc(cookieHandler)->put(var$0, $(hdrs->map()));
	} else {
		$Log::logTrace("Response: No cookie manager found for {0}"_s, $$new($ObjectArray, {$(request->uri())}));
	}
	return nullptr;
}
...
$Class* CookieFilter::load$($String* name, bool initialize) {
	...
	$loadClass(CookieFilter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CookieFilter);
	});
	return class$;
}
