...
bool Http1Response$Receiver::accept($ByteBuffer* buf, Object$* parser, $CompletableFuture* cf) {
	if (cf == nullptr || parser == nullptr || cf->isDone()) {
		return false;
	}
...
$Class* Http1Response$Receiver::load$($String* name, bool initialize) {
	...
	$loadClass(Http1Response$Receiver, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Http1Response$Receiver);
	});
	return class$;
}
