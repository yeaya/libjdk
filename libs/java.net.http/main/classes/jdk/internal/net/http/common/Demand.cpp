...
$Class* Demand$$Lambda$lambda$increase$0::load$($String* name, bool initialize) {
	...
	$loadClass(Demand$$Lambda$lambda$increase$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Demand$$Lambda$lambda$increase$0);
	});
	return class$;
}
...
bool Demand::increase(int64_t n) {
	...
	int64_t prev = this->val->getAndAccumulate(n, $$new(Demand$$Lambda$lambda$increase$0));
	return prev == 0;
}
...
$Class* Demand::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("jdk.internal.net.http.common.Demand$$Lambda$lambda$increase$0")) {
	...
	$loadClass(Demand, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Demand);
	});
	return class$;
}
