...
$Class* HeaderTable$$Lambda$lambda$static$0::load$($String* name, bool initialize) {
	...
	$loadClass(HeaderTable$$Lambda$lambda$static$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(HeaderTable$$Lambda$lambda$static$0);
	});
	return class$;
}
...
$Class* HeaderTable$$Lambda$lambda$add$2$1::load$($String* name, bool initialize) {
	...
	$loadClass(HeaderTable$$Lambda$lambda$add$2$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(HeaderTable$$Lambda$lambda$add$2$1);
	});
	return class$;
}
...
int32_t HeaderTable::indexOf($CharSequence* name, $CharSequence* value) {
	...
	if (didx > 0) {
		...
		if (values != nullptr) {
			return -$$sure($Integer, $$nc($$nc(values->values())->iterator())->next())->intValue();
		} else {
			$init($SimpleHeaderTable);
			return -$SimpleHeaderTable::STATIC_TABLE_LENGTH + didx;
		}
...
void HeaderTable::add($SimpleHeaderTable$HeaderField* f) {
	...
	$var($Map, values, $cast($Map, this->map->computeIfAbsent($nc(f)->name, $$new(HeaderTable$$Lambda$lambda$static$0))));
	$var($Deque, indexes, $cast($Deque, $nc(values)->computeIfAbsent(f->value, $$new(HeaderTable$$Lambda$lambda$add$2$1))));
...
bool HeaderTable::indexesUniqueAndOrdered($Deque* indexes) {
	...
	int64_t maxIndexSoFar = -1;
	{
		...
		for (; $nc(i$)->hasNext();) {
			int64_t l = $$sure($Long, i$->next())->longValue();
			if (l <= maxIndexSoFar) {
				return false;
			} else {
				maxIndexSoFar = l;
...
int32_t HeaderTable::search($String* name, $String* value) {
	$useLocalObjectStack();
	$var($Map, values, $cast($Map, this->map->get(name)));
	if (values == nullptr) {
		return 0;
	}
	$var($Deque, indexes, $cast($Deque, $nc(values)->get(value)));
	if (indexes != nullptr) {
		return (int32_t)(this->counter - $$sure($Long, indexes->peekLast())->longValue());
	} else {
		if (!HeaderTable::$assertionsDisabled && ! !values->isEmpty()) {
			$throwNew($AssertionError);
		}
		$var($Long, any, $cast($Long, $$sure($Deque, $$nc($$nc(values->values())->iterator())->next())->peekLast()));
...
$SimpleHeaderTable$HeaderField* HeaderTable::remove() {
	...
	$var($Map, values, $cast($Map, this->map->get($nc(f)->name)));
	$var($Deque, indexes, $cast($Deque, $nc(values)->get(f->value)));
...
void HeaderTable::clinit$($Class* clazz) {
	$useLocalObjectStack();
	HeaderTable::$assertionsDisabled = !HeaderTable::class$->desiredAssertionStatus();
	{
		...
		for (int32_t i = 1; i <= $SimpleHeaderTable::STATIC_TABLE_LENGTH; ++i) {
			...
			$var($Map, values, $cast($Map, map->computeIfAbsent($nc(f)->name, $$new(HeaderTable$$Lambda$lambda$static$0))));
			$nc(values)->put(f->value, $($Integer::valueOf(i)));
		}
...
$Class* HeaderTable::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("jdk.internal.net.http.hpack.HeaderTable$$Lambda$lambda$static$0")) {
			return HeaderTable$$Lambda$lambda$static$0::load$(name, initialize);
		}
		if (name->equals("jdk.internal.net.http.hpack.HeaderTable$$Lambda$lambda$add$2$1")) {
	...
	$loadClass(HeaderTable, name, initialize, &classInfo$$, HeaderTable::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(HeaderTable);
	});
	return class$;
}
