...
int32_t PushPromiseFrame::length() {
	return this->headerLength + ((this->flags & PushPromiseFrame::PADDED) != 0 ? 5 : 4);
}
...
$Class* PushPromiseFrame::load$($String* name, bool initialize) {
	...
	$loadClass(PushPromiseFrame, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PushPromiseFrame);
	});
	return class$;
}
