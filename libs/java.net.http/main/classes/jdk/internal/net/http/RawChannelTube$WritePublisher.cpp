...
void RawChannelTube$WritePublisher::subscribe($Flow$Subscriber* subscriber) {
	if ($nc(this->this$0->debug)->on()) {
		this->this$0->debug->log("WritePublisher::subscribe"_s);
	}
...
$Class* RawChannelTube$WritePublisher::load$($String* name, bool initialize) {
	...
	$loadClass(RawChannelTube$WritePublisher, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(RawChannelTube$WritePublisher);
	});
	return class$;
}
