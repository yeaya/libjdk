...
$Class* MessageDecoder$$Lambda$toString::load$($String* name, bool initialize) {
	...
	$loadClass(MessageDecoder$$Lambda$toString, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MessageDecoder$$Lambda$toString);
	});
	return class$;
}
...
void MessageDecoder::fin(bool value) {
	...
	if ($nc(MessageDecoder::debug)->on()) {
		MessageDecoder::debug->log("fin %s"_s, $$new($ObjectArray, {$($Boolean::valueOf(value))}));
	}
	this->fin$ = value;
}
...
void MessageDecoder::rsv1(bool value) {
	$useLocalObjectStack();
	if ($nc(MessageDecoder::debug)->on()) {
		MessageDecoder::debug->log("rsv1 %s"_s, $$new($ObjectArray, {$($Boolean::valueOf(value))}));
	}
	if (value) {
...
void MessageDecoder::rsv2(bool value) {
	$useLocalObjectStack();
	if ($nc(MessageDecoder::debug)->on()) {
		MessageDecoder::debug->log("rsv2 %s"_s, $$new($ObjectArray, {$($Boolean::valueOf(value))}));
	}
	if (value) {
...
void MessageDecoder::rsv3(bool value) {
	$useLocalObjectStack();
	if ($nc(MessageDecoder::debug)->on()) {
		MessageDecoder::debug->log("rsv3 %s"_s, $$new($ObjectArray, {$($Boolean::valueOf(value))}));
	}
	if (value) {
...
void MessageDecoder::opcode($Frame$Opcode* v) {
	$useLocalObjectStack();
	if ($nc(MessageDecoder::debug)->on()) {
		MessageDecoder::debug->log("opcode %s"_s, $$new($ObjectArray, {v}));
	}
	$init($Frame$Opcode);
	if (v == $Frame$Opcode::PING || v == $Frame$Opcode::PONG || v == $Frame$Opcode::CLOSE) {
		...
		$set(this, opcode$, v);
	} else if (v == $Frame$Opcode::TEXT || v == $Frame$Opcode::BINARY) {
	...
	} else if (v == $Frame$Opcode::CONTINUATION) {
		if (this->originatingOpcode == nullptr) {
			$throwNew($FailWebSocketException, $($String::format("Unexpected frame %s (fin=%s)"_s, $$new($ObjectArray, {
				v,
				$($Boolean::valueOf(this->fin$))
...
void MessageDecoder::mask(bool value) {
	$useLocalObjectStack();
	if ($nc(MessageDecoder::debug)->on()) {
		MessageDecoder::debug->log("mask %s"_s, $$new($ObjectArray, {$($Boolean::valueOf(value))}));
	}
	if (value) {
...
void MessageDecoder::payloadLen(int64_t value) {
	$useLocalObjectStack();
	if ($nc(MessageDecoder::debug)->on()) {
		MessageDecoder::debug->log("payloadLen %s"_s, $$new($ObjectArray, {$($Long::valueOf(value))}));
	}
	...
	if ($nc(this->opcode$)->isControl()) {
		if (this->binaryData != nullptr) {
			...
			if (!MessageDecoder::$assertionsDisabled && !(remaining < $Frame::MAX_CONTROL_FRAME_PAYLOAD_LENGTH)) {
				$throwNew($AssertionError, $$of($Utils::dump($$new($ObjectArray, {$($Integer::valueOf(remaining))}))));
			}
		...
		if (!text) {
			...
			try {
				$assign(textData, this->decoder->decode(data, last));
...
void MessageDecoder::endFrame() {
	$useLocalObjectStack();
	if ($nc(MessageDecoder::debug)->on()) {
		MessageDecoder::debug->log("end frame"_s);
	}
	if ($nc(this->opcode$)->isControl()) {
	...
	}
	{
		...
		default:
			if (!MessageDecoder::$assertionsDisabled && !(this->opcode$ == $Frame$Opcode::TEXT || this->opcode$ == $Frame$Opcode::BINARY || this->opcode$ == $Frame$Opcode::CONTINUATION)) {
				$throwNew($AssertionError, $$of($Utils::dump($$new($ObjectArray, {this->opcode$}))));
			}
			if (this->fin$) {
...
void MessageDecoder::clinit$($Class* clazz) {
	MessageDecoder::$assertionsDisabled = !MessageDecoder::class$->desiredAssertionStatus();
	$init($Utils);
	$assignStatic(MessageDecoder::debug, $Utils::getWebSocketLogger($$new(MessageDecoder$$Lambda$toString, "[Input]"_s), $Utils::DEBUG_WS));
}
...
$Class* MessageDecoder::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("jdk.internal.net.http.websocket.MessageDecoder$$Lambda$toString")) {
	...
	$loadClass(MessageDecoder, name, initialize, &classInfo$$, MessageDecoder::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(MessageDecoder);
	});
	return class$;
}
