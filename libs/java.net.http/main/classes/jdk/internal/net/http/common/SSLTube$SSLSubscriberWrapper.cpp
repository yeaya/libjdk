...
$String* SSLTube$SSLSubscriberWrapper::toString() {
	...
	var$0->append("SSLSubscriberWrapper["_s);
	var$0->append(this->this$0);
	var$0->append(", delegate: "_s);
	var$0->append(sub == nullptr ? pend : sub);
	var$0->append(", getALPN: "_s);
	var$0->append(sslFD == nullptr ? ($CompletableFuture*)nullptr : $(sslFD->alpn()));
	var$0->append(", onCompleteReceived: "_s);
	var$0->append(this->onCompleteReceived);
	var$0->append(", onError: "_s);
	var$0->append($(this->errorRef->get()));
	var$0->append("]"_s);
	return $str(var$0);
}
...
void SSLTube$SSLSubscriberWrapper::setDelegate($Flow$Subscriber* delegate) {
	...
	if ($nc(this->this$0->debug)->on()) {
		this->this$0->debug->log("SSLSubscriberWrapper (reader) got delegate: %s"_s, $$new($ObjectArray, {delegate}));
	}
	...
	if (subscription == nullptr) {
		if (this->this$0->debug->on()) {
			this->this$0->debug->log("SSLSubscriberWrapper (reader) no subscription yet"_s);
		}
		return;
	}
...
void SSLTube$SSLSubscriberWrapper::processPendingSubscriber() {
	...
	if (subscription == nullptr) {
		if ($nc(this->this$0->debug)->on()) {
			this->this$0->debug->log("SSLSubscriberWrapper (reader) processPendingSubscriber: no subscription yet"_s);
		}
		return;
	}
...
void SSLTube$SSLSubscriberWrapper::onSubscribe($Flow$Subscription* subscription) {
	if ($nc(this->this$0->debug)->on()) {
		this->this$0->debug->log("SSLSubscriberWrapper (reader) onSubscribe(%s)"_s, $$new($ObjectArray, {subscription}));
	}
...
void SSLTube$SSLSubscriberWrapper::onSubscribeImpl($Flow$Subscription* subscription) {
	...
	if (subscriberImpl == nullptr && pending == nullptr) {
		if ($nc(this->this$0->debug)->on()) {
			this->this$0->debug->log("SSLSubscriberWrapper (reader) onSubscribeImpl: no delegate yet"_s);
		}
		return;
	}
	if (pending == nullptr) {
		if ($nc(this->this$0->debug)->on()) {
			this->this$0->debug->log("SSLSubscriberWrapper (reader) onSubscribeImpl: resubscribing"_s);
		}
		onNewSubscription(subscriberImpl, subscription);
	} else {
		if ($nc(this->this$0->debug)->on()) {
			this->this$0->debug->log("SSLSubscriberWrapper (reader) onSubscribeImpl: subscribing pending"_s);
		}
		processPendingSubscriber();
	}
}
...
void SSLTube$SSLSubscriberWrapper::complete($SSLTube$DelegateWrapper* subscriberImpl, $Throwable* t$renamed) {
	...
	try {
		...
		if ($nc(this->this$0->debug)->on()) {
			this->this$0->debug->log("subscriber completed %s"_s, $$new($ObjectArray, {((t == nullptr) ? "normally"_s : ($$str({"with error: "_s, t})))}));
		}
...
void SSLTube$SSLSubscriberWrapper::onNewSubscription($SSLTube$DelegateWrapper* subscriberImpl, $Flow$Subscription* subscription) {
	...
	if (failed != nullptr) {
		if ($nc(this->this$0->debug)->on()) {
			this->this$0->debug->log("onNewSubscription: subscriberImpl:%s, invoking onError:%s"_s, $$new($ObjectArray, {
				subscriberImpl,
				failed
			}));
		}
		...
		if ($nc(this->this$0->debug)->on()) {
			this->this$0->debug->log("onNewSubscription: subscriberImpl:%s, invoking onCompleted"_s, $$new($ObjectArray, {subscriberImpl}));
		}
...
void SSLTube$SSLSubscriberWrapper::onErrorImpl($Throwable* throwable$renamed) {
	...
	this->errorRef->compareAndSet(nullptr, throwable);
	$var($Throwable, failed, $cast($Throwable, this->errorRef->get()));
	this->this$0->finished = true;
	if ($nc(this->this$0->debug)->on()) {
		this->this$0->debug->log("%s: onErrorImpl: %s"_s, $$new($ObjectArray, {
			this,
			throwable
		}));
	}
	...
	if (subscriberImpl != nullptr) {
		...
		this->this$0->debug->log("%s: delegate null, stored %s"_s, $$new($ObjectArray, {
			this,
			failed
		}));
	}
...
void SSLTube$SSLSubscriberWrapper::clinit$($Class* clazz) {
	$load($SSLTube);
...
$Class* SSLTube$SSLSubscriberWrapper::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(Ljdk/internal/net/http/common/SSLTube;)V", nullptr, 0, $method(SSLTube$SSLSubscriberWrapper, init$, void, $SSLTube*)},
		{"complete", "(Ljdk/internal/net/http/common/SSLTube$DelegateWrapper;Ljava/lang/Throwable;)V", nullptr, $PRIVATE, $method(SSLTube$SSLSubscriberWrapper, complete, void, $SSLTube$DelegateWrapper*, $Throwable*)},
		{"dropSubscription", "()V", nullptr, $PUBLIC, $virtualMethod(SSLTube$SSLSubscriberWrapper, dropSubscription, void)},
		{"onComplete", "()V", nullptr, $PUBLIC, $virtualMethod(SSLTube$SSLSubscriberWrapper, onComplete, void)},
		{"onError", "(Ljava/lang/Throwable;)V", nullptr, $PUBLIC, $virtualMethod(SSLTube$SSLSubscriberWrapper, onError, void, $Throwable*)},
		{"onErrorImpl", "(Ljava/lang/Throwable;)V", nullptr, $PUBLIC, $method(SSLTube$SSLSubscriberWrapper, onErrorImpl, void, $Throwable*)},
		{"onNewSubscription", "(Ljdk/internal/net/http/common/SSLTube$DelegateWrapper;Ljava/util/concurrent/Flow$Subscription;)V", nullptr, $PRIVATE, $method(SSLTube$SSLSubscriberWrapper, onNewSubscription, void, $SSLTube$DelegateWrapper*, $Flow$Subscription*)},
		{"onNext", "(Ljava/util/List;)V", "(Ljava/util/List<Ljava/nio/ByteBuffer;>;)V", $PUBLIC, $method(SSLTube$SSLSubscriberWrapper, onNext, void, $List*)},
		{"onNext", "(Ljava/lang/Object;)V", nullptr, $PUBLIC | $VOLATILE | $SYNTHETIC, $virtualMethod(SSLTube$SSLSubscriberWrapper, onNext, void, Object$*)},
		{"onSubscribe", "(Ljava/util/concurrent/Flow$Subscription;)V", nullptr, $PUBLIC, $virtualMethod(SSLTube$SSLSubscriberWrapper, onSubscribe, void, $Flow$Subscription*)},
		{"onSubscribeImpl", "(Ljava/util/concurrent/Flow$Subscription;)V", nullptr, $PRIVATE, $method(SSLTube$SSLSubscriberWrapper, onSubscribeImpl, void, $Flow$Subscription*)},
		{"processPendingSubscriber", "()V", nullptr, 0, $method(SSLTube$SSLSubscriberWrapper, processPendingSubscriber, void)},
		{"setDelegate", "(Ljava/util/concurrent/Flow$Subscriber;)V", "(Ljava/util/concurrent/Flow$Subscriber<-Ljava/util/List<Ljava/nio/ByteBuffer;>;>;)V", 0, $method(SSLTube$SSLSubscriberWrapper, setDelegate, void, $Flow$Subscriber*)},
		{"toString", "()Ljava/lang/String;", nullptr, $PUBLIC, $virtualMethod(SSLTube$SSLSubscriberWrapper, toString, $String*)},
		{}
	};
	...
	$loadClass(SSLTube$SSLSubscriberWrapper, name, initialize, &classInfo$$, SSLTube$SSLSubscriberWrapper::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(SSLTube$SSLSubscriberWrapper);
	});
	return class$;
}
