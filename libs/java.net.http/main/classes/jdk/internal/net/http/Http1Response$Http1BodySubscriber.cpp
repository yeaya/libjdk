...
void Http1Response$Http1BodySubscriber::onSubscribe($Flow$Subscription* subscription) {
	...
	} else if (!Http1Response$Http1BodySubscriber::$assertionsDisabled && !this->completed->get()) {
		$throwNew($AssertionError);
	}
}

void Http1Response$Http1BodySubscriber::onNext($List* item) {
	if (!Http1Response$Http1BodySubscriber::$assertionsDisabled && ! !this->completed->get()) {
		$throwNew($AssertionError);
	}
...
void Http1Response$Http1BodySubscriber::clinit$($Class* clazz) {
	$load($Http1Response);
...
$Class* Http1Response$Http1BodySubscriber::load$($String* name, bool initialize) {
	...
	$loadClass(Http1Response$Http1BodySubscriber, name, initialize, &classInfo$$, Http1Response$Http1BodySubscriber::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Http1Response$Http1BodySubscriber);
	});
	return class$;
}
