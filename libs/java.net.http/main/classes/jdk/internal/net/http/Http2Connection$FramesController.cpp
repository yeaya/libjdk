...
$Class* Http2Connection$FramesController$$Lambda$lambda$processReceivedData$0::load$($String* name, bool initialize) {
	...
	$loadClass(Http2Connection$FramesController$$Lambda$lambda$processReceivedData$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Http2Connection$FramesController$$Lambda$lambda$processReceivedData$0);
	});
	return class$;
}
...
bool Http2Connection$FramesController::processReceivedData($FramesDecoder* decoder, $ByteBuffer* buf) {
	...
	if (!this->prefaceSent) {
		if ($nc(this->this$0->debug)->on()) {
			this->this$0->debug->log("Preface not sent: buffering %d"_s, $$new($ObjectArray, {$($Integer::valueOf($nc(buf)->remaining()))}));
		}
		$synchronized(this) {
			if (!this->prefaceSent) {
				...
				if (this->this$0->debug->on()) {
					this->this$0->debug->log($$str({"there are now %d bytes buffered waiting for preface to be sent"_s, $$str($Utils::remaining(this->pending))}));
				}
				return false;
			}
		}
	...
	if (pending != nullptr) {
		if ($nc(this->this$0->debug)->on()) {
			this->this$0->debug->log($$new(Http2Connection$FramesController$$Lambda$lambda$processReceivedData$0, pending));
		}
		{
	...
	if (buf != $Http2Connection::EMPTY_TRIGGER) {
		if ($nc(this->this$0->debug)->on()) {
			this->this$0->debug->log("Processing %d"_s, $$new($ObjectArray, {$($Integer::valueOf($nc(buf)->remaining()))}));
		}
...
void Http2Connection$FramesController::clinit$($Class* clazz) {
...
$Class* Http2Connection$FramesController::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("jdk.internal.net.http.Http2Connection$FramesController$$Lambda$lambda$processReceivedData$0")) {
	...
	$loadClass(Http2Connection$FramesController, name, initialize, &classInfo$$, Http2Connection$FramesController::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Http2Connection$FramesController);
	});
	return class$;
}
