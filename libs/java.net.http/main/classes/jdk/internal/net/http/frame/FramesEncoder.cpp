...
$List* FramesEncoder::encodeFrames($List* frames) {
	...
	{
		$var($Iterator, i$, frames->iterator());
		for (; $nc(i$)->hasNext();) {
	...
	default:
		{
			...
			var$1->append("Not supported frame "_s);
			var$1->append(frame->type());
			var$1->append(" ("_s);
			var$1->append($($of(frame)->getClass()->getName()));
			var$1->append(")"_s);
			$throwNew($UnsupportedOperationException, $$str(var$1));
		}
	}
	return var$0;
}
...
$List* FramesEncoder::encodeDataFrame($DataFrame* frame) {
	...
	if ($nc(frame)->getFlag($DataFrame::PADDED)) {
		$var($List, var$0, frame->getData());
		return joinWithPadding(buf, var$0, frame->getPadLength());
	} else {
...
$ByteBuffer* FramesEncoder::encodeDataFrameStart($DataFrame* frame) {
	...
	putHeader(buf, length, $DataFrame::TYPE, var$1, frame->streamid());
	if (isPadded) {
...
$List* FramesEncoder::encodeHeadersFrame($HeadersFrame* frame) {
	...
	if ($nc(frame)->getFlag($HeadersFrame::PADDED)) {
		$var($List, var$0, frame->getHeaderBlock());
		return joinWithPadding(buf, var$0, frame->getPadLength());
	} else {
...
$ByteBuffer* FramesEncoder::encodeHeadersFrameStart($HeadersFrame* frame) {
	...
	int32_t var$1 = frame->getFlags();
	putHeader(buf, length, $HeadersFrame::TYPE, var$1, frame->streamid());
	if (isPadded) {
		$nc(buf)->put((int8_t)frame->getPadLength());
	}
	if (hasPriority) {
		bool var$2 = frame->getExclusive();
		int32_t var$3 = frame->getStreamDependency();
		putPriority(buf, var$2, var$3, frame->getWeight());
	}
	$nc(buf)->flip();
	return buf;
}
...
$List* FramesEncoder::encodePriorityFrame($PriorityFrame* frame) {
	...
	bool var$0 = frame->exclusive();
	int32_t var$1 = frame->streamDependency();
	putPriority(buf, var$0, var$1, frame->weight());
	$nc(buf)->flip();
	return $List::of(buf);
}
...
$List* FramesEncoder::encodeResetFrame($ResetFrame* frame) {
	...
	$nc(buf)->putInt(frame->getErrorCode());
	buf->flip();
	return $List::of(buf);
}
...
$List* FramesEncoder::encodePushPromiseFrame($PushPromiseFrame* frame) {
	...
	int32_t var$0 = frame->getFlags();
	putHeader(buf, length, $PushPromiseFrame::TYPE, var$0, frame->streamid());
	if (isPadded) {
		$nc(buf)->put((int8_t)frame->getPadLength());
	}
	...
	if (frame->getFlag($PushPromiseFrame::PADDED)) {
		...
		return joinWithPadding(buf, var$1, frame->getPadLength());
	} else {
		return join(buf, $(frame->getHeaderBlock()));
	}
}
...
$List* FramesEncoder::encodeContinuationFrame($ContinuationFrame* frame) {
	...
	int32_t var$0 = frame->getFlags();
	putHeader(buf, length, $ContinuationFrame::TYPE, var$0, frame->streamid());
	$nc(buf)->flip();
	return join(buf, $(frame->getHeaderBlock()));
}
...
$List* FramesEncoder::joinWithPadding($ByteBuffer* buf, $List* data, int32_t padLength) {
	...
	if (len == 0) {
		...
		$var($Object, var$0, $cast($ByteBuffer, data->get(0)));
		return $List::of(buf, var$0, $(getPadding(padLength)));
	} else if (len == 2) {
		$var($Object, var$1, $cast($ByteBuffer, data->get(0)));
		$var($Object, var$2, $cast($ByteBuffer, data->get(1)));
		return $List::of(buf, var$1, var$2, $(getPadding(padLength)));
	}
...
$List* FramesEncoder::join($ByteBuffer* buf, $List* data) {
	...
	if (len == 0) {
		...
		$var($Object, var$0, $cast($ByteBuffer, data->get(0)));
		return $List::of(buf, var$0, $$cast($ByteBuffer, data->get(1)));
	}
...
void FramesEncoder::clinit$($Class* clazz) {
...
$Class* FramesEncoder::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PUBLIC, $method(FramesEncoder, init$, void)},
		{"encodeConnectionPreface", "([BLjdk/internal/net/http/frame/SettingsFrame;)Ljava/nio/ByteBuffer;", nullptr, $PUBLIC, $virtualMethod(FramesEncoder, encodeConnectionPreface, $ByteBuffer*, $bytes*, $SettingsFrame*)},
		{"encodeContinuationFrame", "(Ljdk/internal/net/http/frame/ContinuationFrame;)Ljava/util/List;", "(Ljdk/internal/net/http/frame/ContinuationFrame;)Ljava/util/List<Ljava/nio/ByteBuffer;>;", $PRIVATE, $method(FramesEncoder, encodeContinuationFrame, $List*, $ContinuationFrame*)},
		{"encodeDataFrame", "(Ljdk/internal/net/http/frame/DataFrame;)Ljava/util/List;", "(Ljdk/internal/net/http/frame/DataFrame;)Ljava/util/List<Ljava/nio/ByteBuffer;>;", $PRIVATE, $method(FramesEncoder, encodeDataFrame, $List*, $DataFrame*)},
		{"encodeDataFrameStart", "(Ljdk/internal/net/http/frame/DataFrame;)Ljava/nio/ByteBuffer;", nullptr, $PRIVATE, $method(FramesEncoder, encodeDataFrameStart, $ByteBuffer*, $DataFrame*)},
		{"encodeFrame", "(Ljdk/internal/net/http/frame/Http2Frame;)Ljava/util/List;", "(Ljdk/internal/net/http/frame/Http2Frame;)Ljava/util/List<Ljava/nio/ByteBuffer;>;", $PUBLIC, $virtualMethod(FramesEncoder, encodeFrame, $List*, $Http2Frame*)},
		{"encodeFrames", "(Ljava/util/List;)Ljava/util/List;", "(Ljava/util/List<Ljdk/internal/net/http/frame/HeaderFrame;>;)Ljava/util/List<Ljava/nio/ByteBuffer;>;", $PUBLIC, $virtualMethod(FramesEncoder, encodeFrames, $List*, $List*)},
		{"encodeGoAwayFrame", "(Ljdk/internal/net/http/frame/GoAwayFrame;)Ljava/util/List;", "(Ljdk/internal/net/http/frame/GoAwayFrame;)Ljava/util/List<Ljava/nio/ByteBuffer;>;", $PRIVATE, $method(FramesEncoder, encodeGoAwayFrame, $List*, $GoAwayFrame*)},
		{"encodeHeadersFrame", "(Ljdk/internal/net/http/frame/HeadersFrame;)Ljava/util/List;", "(Ljdk/internal/net/http/frame/HeadersFrame;)Ljava/util/List<Ljava/nio/ByteBuffer;>;", $PRIVATE, $method(FramesEncoder, encodeHeadersFrame, $List*, $HeadersFrame*)},
		{"encodeHeadersFrameStart", "(Ljdk/internal/net/http/frame/HeadersFrame;)Ljava/nio/ByteBuffer;", nullptr, $PRIVATE, $method(FramesEncoder, encodeHeadersFrameStart, $ByteBuffer*, $HeadersFrame*)},
		{"encodePingFrame", "(Ljdk/internal/net/http/frame/PingFrame;)Ljava/util/List;", "(Ljdk/internal/net/http/frame/PingFrame;)Ljava/util/List<Ljava/nio/ByteBuffer;>;", $PRIVATE, $method(FramesEncoder, encodePingFrame, $List*, $PingFrame*)},
		{"encodePriorityFrame", "(Ljdk/internal/net/http/frame/PriorityFrame;)Ljava/util/List;", "(Ljdk/internal/net/http/frame/PriorityFrame;)Ljava/util/List<Ljava/nio/ByteBuffer;>;", $PRIVATE, $method(FramesEncoder, encodePriorityFrame, $List*, $PriorityFrame*)},
		{"encodePushPromiseFrame", "(Ljdk/internal/net/http/frame/PushPromiseFrame;)Ljava/util/List;", "(Ljdk/internal/net/http/frame/PushPromiseFrame;)Ljava/util/List<Ljava/nio/ByteBuffer;>;", $PRIVATE, $method(FramesEncoder, encodePushPromiseFrame, $List*, $PushPromiseFrame*)},
		{"encodeResetFrame", "(Ljdk/internal/net/http/frame/ResetFrame;)Ljava/util/List;", "(Ljdk/internal/net/http/frame/ResetFrame;)Ljava/util/List<Ljava/nio/ByteBuffer;>;", $PRIVATE, $method(FramesEncoder, encodeResetFrame, $List*, $ResetFrame*)},
		{"encodeSettingsFrame", "(Ljdk/internal/net/http/frame/SettingsFrame;)Ljava/util/List;", "(Ljdk/internal/net/http/frame/SettingsFrame;)Ljava/util/List<Ljava/nio/ByteBuffer;>;", $PRIVATE, $method(FramesEncoder, encodeSettingsFrame, $List*, $SettingsFrame*)},
		{"encodeWindowUpdateFrame", "(Ljdk/internal/net/http/frame/WindowUpdateFrame;)Ljava/util/List;", "(Ljdk/internal/net/http/frame/WindowUpdateFrame;)Ljava/util/List<Ljava/nio/ByteBuffer;>;", $PRIVATE, $method(FramesEncoder, encodeWindowUpdateFrame, $List*, $WindowUpdateFrame*)},
		{"getBuffer", "(I)Ljava/nio/ByteBuffer;", nullptr, $PRIVATE, $method(FramesEncoder, getBuffer, $ByteBuffer*, int32_t)},
		{"getPadding", "(I)Ljava/nio/ByteBuffer;", nullptr, $PUBLIC, $virtualMethod(FramesEncoder, getPadding, $ByteBuffer*, int32_t)},
		{"join", "(Ljava/nio/ByteBuffer;Ljava/util/List;)Ljava/util/List;", "(Ljava/nio/ByteBuffer;Ljava/util/List<Ljava/nio/ByteBuffer;>;)Ljava/util/List<Ljava/nio/ByteBuffer;>;", $PRIVATE, $method(FramesEncoder, join, $List*, $ByteBuffer*, $List*)},
		{"joinWithPadding", "(Ljava/nio/ByteBuffer;Ljava/util/List;I)Ljava/util/List;", "(Ljava/nio/ByteBuffer;Ljava/util/List<Ljava/nio/ByteBuffer;>;I)Ljava/util/List<Ljava/nio/ByteBuffer;>;", $PRIVATE, $method(FramesEncoder, joinWithPadding, $List*, $ByteBuffer*, $List*, int32_t)},
		{"putHeader", "(Ljava/nio/ByteBuffer;IIII)V", nullptr, $PRIVATE, $method(FramesEncoder, putHeader, void, $ByteBuffer*, int32_t, int32_t, int32_t, int32_t)},
		{"putPriority", "(Ljava/nio/ByteBuffer;ZII)V", nullptr, $PRIVATE, $method(FramesEncoder, putPriority, void, $ByteBuffer*, bool, int32_t, int32_t)},
		{"putSettingsFrame", "(Ljava/nio/ByteBuffer;Ljdk/internal/net/http/frame/SettingsFrame;I)V", nullptr, $PRIVATE, $method(FramesEncoder, putSettingsFrame, void, $ByteBuffer*, $SettingsFrame*, int32_t)},
		{}
	};
	$ClassInfo classInfo$$ = {
		$PUBLIC | $ACC_SUPER,
	...
	$loadClass(FramesEncoder, name, initialize, &classInfo$$, FramesEncoder::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(FramesEncoder);
	});
	return class$;
}
