...
$Class* Http1Response$$Lambda$dbgString::load$($String* name, bool initialize) {
	...
	$loadClass(Http1Response$$Lambda$dbgString, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Http1Response$$Lambda$dbgString);
	});
	return class$;
}
...
$Class* Http1Response$$Lambda$advance$1::load$($String* name, bool initialize) {
	...
	$loadClass(Http1Response$$Lambda$advance$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Http1Response$$Lambda$advance$1);
	});
	return class$;
}
...
$Class* Http1Response$$Lambda$lambda$readHeadersAsync$0$2::load$($String* name, bool initialize) {
	...
	$loadClass(Http1Response$$Lambda$lambda$readHeadersAsync$0$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Http1Response$$Lambda$lambda$readHeadersAsync$0$2);
	});
	return class$;
}
...
$Class* Http1Response$$Lambda$lambda$readBody$3$3::load$($String* name, bool initialize) {
	...
	$loadClass(Http1Response$$Lambda$lambda$readBody$3$3, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Http1Response$$Lambda$lambda$readBody$3$3);
	});
	return class$;
}
...
$Class* Http1Response$$Lambda$lambda$readBody$4$4::load$($String* name, bool initialize) {
	...
	$loadClass(Http1Response$$Lambda$lambda$readBody$4$4, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Http1Response$$Lambda$lambda$readBody$4$4);
	});
	return class$;
}
...
$Class* Http1Response$$Lambda$lambda$readBody$5$5::load$($String* name, bool initialize) {
	...
	$loadClass(Http1Response$$Lambda$lambda$readBody$5$5, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Http1Response$$Lambda$lambda$readBody$5$5);
	});
	return class$;
}
...
$Class* Http1Response$$Lambda$lambda$onReadError$6$6::load$($String* name, bool initialize) {
	...
	$loadClass(Http1Response$$Lambda$lambda$onReadError$6$6, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Http1Response$$Lambda$lambda$onReadError$6$6);
	});
	return class$;
}
...
$Class* Http1Response$$Lambda$lambda$onReadError$7$7::load$($String* name, bool initialize) {
	...
	$loadClass(Http1Response$$Lambda$lambda$onReadError$7$7, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Http1Response$$Lambda$lambda$onReadError$7$7);
	});
	return class$;
}
...
$Class* Http1Response$$Lambda$onFinished$8::load$($String* name, bool initialize) {
	...
	$loadClass(Http1Response$$Lambda$onFinished$8, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Http1Response$$Lambda$onFinished$8);
	});
	return class$;
}
...
$Class* Http1Response$$Lambda$lambda$readBody$1$9::load$($String* name, bool initialize) {
	...
	$loadClass(Http1Response$$Lambda$lambda$readBody$1$9, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Http1Response$$Lambda$lambda$readBody$1$9);
	});
	return class$;
}
...
$Class* Http1Response$$Lambda$lambda$readBody$2$10::load$($String* name, bool initialize) {
	...
	$loadClass(Http1Response$$Lambda$lambda$readBody$2$10, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Http1Response$$Lambda$lambda$readBody$2$10);
	});
	return class$;
}
...
void Http1Response::init$($HttpConnection* conn, $Http1Exchange* exchange, $Http1AsyncReceiver* asyncReceiver) {
	...
	$init($Utils);
	$set(this, debug, $Utils::getDebugLogger($$new(Http1Response$$Lambda$dbgString, this), $Utils::DEBUG));
	this->id = Http1Response::responseCount->incrementAndGet();
	...
	$set(this, headersReader, $new($Http1Response$HeadersReader, this, $$new(Http1Response$$Lambda$advance$1, this)));
	$set(this, bodyReader, $new($Http1Response$BodyReader, this, $$new(Http1Response$$Lambda$advance$1, this)));
...
$CompletableFuture* Http1Response::readHeadersAsync($Executor* executor) {
	$useLocalObjectStack();
	if ($nc(this->debug)->on()) {
		this->debug->log($$str({"Reading Headers: (remaining: "_s, $$str($nc(this->asyncReceiver)->remaining()), ") "_s, this->readProgress}));
	}
	if (this->firstTimeAround) {
		if (this->debug->on()) {
			this->debug->log("First time around"_s);
		}
	...
	$var($CompletableFuture, cf, this->headersReader->completion());
	if (!Http1Response::$assertionsDisabled && !(cf != nullptr)) {
		$throwNew($AssertionError, $of("parsing not started"_s));
	}
	if (this->debug->on()) {
		this->debug->log("headersReader is %s"_s, $$new($ObjectArray, {cf == nullptr ? $of("not yet started"_s) : $$of(cf->isDone() ? "already completed"_s : "not yet completed"_s)}));
	}
	$var($Function, lambda, $new(Http1Response$$Lambda$lambda$readHeadersAsync$0$2, this));
	if (executor != nullptr) {
...
int64_t Http1Response::fixupContentLen(int64_t clen) {
	...
	if (clen == -1) {
		if ($$sure($String, $$nc($nc(this->headers)->firstValue("Transfer-encoding"_s))->orElse(""_s))->equalsIgnoreCase("chunked"_s)) {
			return -1;
		}
...
$CompletableFuture* Http1Response::readBody($HttpResponse$BodySubscriber* p, bool return2Cache, $Executor* executor) {
	$useLocalObjectStack();
	if ($nc(this->debug)->on()) {
		this->debug->log($$str({"readBody: return2Cache: "_s, $$str(return2Cache)}));
		if ($nc(this->request)->isWebSocket() && return2Cache && this->connection != nullptr) {
			this->debug->log($$str({"websocket connection will be returned to cache: "_s, this->connection->getClass(), "/"_s, this->connection}));
		}
	}
	...
	$nc(executor)->execute($$new(Http1Response$$Lambda$lambda$readBody$3$3, this, clen, subscriber, cf, refCountTracker));
	$ResponseSubscribers::getBodyAsync(executor, p, cf, $$new(Http1Response$$Lambda$lambda$readBody$4$4, this, cf));
	return $cast($CompletableFuture, cf->whenComplete($$new(Http1Response$$Lambda$lambda$readBody$5$5, refCountTracker)));
}
...
void Http1Response::onFinished() {
	...
	if (this->return2Cache) {
		$Log::logTrace("Attempting to return connection to the pool: {0}"_s, $$new($ObjectArray, {this->connection}));
		if ($nc(this->debug)->on()) {
			this->debug->log($$str({$($nc(this->connection)->getConnectionFlow()), ": return to HTTP/1.1 pool"_s}));
		}
...
void Http1Response::onReadError($Throwable* t) {
	...
	$var($CompletableFuture, cf, receiver == nullptr ? ($CompletableFuture*)nullptr : receiver->completion());
	$init($System$Logger$Level);
	$nc(this->debug)->log($System$Logger$Level::DEBUG, $$new(Http1Response$$Lambda$lambda$onReadError$6$6, cf));
	if (cf != nullptr) {
		cf->completeExceptionally(t);
	} else {
		this->debug->log($System$Logger$Level::DEBUG, "onReadError"_s, t);
	}
	this->debug->log($System$Logger$Level::DEBUG, $$new(Http1Response$$Lambda$lambda$onReadError$7$7, t));
	$nc(this->connection)->close();
}
...
$String* Http1Response::lambda$onReadError$6($CompletableFuture* cf) {
	$init(Http1Response);
	return $str({"onReadError: cf is "_s, (cf == nullptr ? "null"_s : (cf->isDone() ? "already completed"_s : "not yet completed"_s))});
}
...
void Http1Response::lambda$readBody$4($CompletableFuture* cf, $Throwable* t) {
	...
	this->asyncReceiver->onReadError(t);
}
...
void Http1Response::lambda$readBody$3(int64_t clen, $Http1Response$Http1BodySubscriber* subscriber, $CompletableFuture* cf, $Http1Response$ClientRefCountTracker* refCountTracker) {
	...
	try {
		try {
			$set(this, content, $new($ResponseContent, this->connection, clen, this->headers, subscriber, $$new(Http1Response$$Lambda$onFinished$8, this)));
			...
			$set(this, bodyParser, $nc(this->content)->getBodyParser($$new(Http1Response$$Lambda$lambda$readBody$1$9, this, subscriber, cf)));
			this->bodyReader->start(this->bodyParser);
			$var($CompletableFuture, bodyReaderCF, this->bodyReader->completion());
			$nc(this->asyncReceiver)->subscribe(this->bodyReader);
			if (!Http1Response::$assertionsDisabled && !(bodyReaderCF != nullptr)) {
				$throwNew($AssertionError, $of("parsing not started"_s));
			}
			$var($CompletableFuture, trailingOp, $cast($CompletableFuture, $nc(bodyReaderCF)->whenComplete($$new(Http1Response$$Lambda$lambda$readBody$2$10, this, subscriber, cf, refCountTracker))));
			$nc(this->connection)->addTrailingOperation(trailingOp);
		} catch ($Throwable& t) {
			if ($nc(this->debug)->on()) {
				this->debug->log($$str({"Failed reading body: "_s, t}));
			}
...
void Http1Response::lambda$readBody$2($Http1Response$Http1BodySubscriber* subscriber, $CompletableFuture* cf, $Http1Response$ClientRefCountTracker* refCountTracker, $Http1Response$State* s, $Throwable* t$renamed) {
	...
	try {
		try {
			if (t == nullptr) {
				if ($nc(this->debug)->on()) {
					this->debug->log($$str({"Finished reading body: "_s, s}));
				}
				$init($Http1Response$State);
...
$Response* Http1Response::lambda$readHeadersAsync$0($Http1Response$State* completed) {
	...
	if ($nc(this->debug)->on()) {
		this->debug->log($$str({"Reading Headers: creating Response object; state is now "_s, this->readProgress}));
	}
	$nc(this->asyncReceiver)->unsubscribe(this->headersReader);
...
void Http1Response::clinit$($Class* clazz) {
...
$Class* Http1Response::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("jdk.internal.net.http.Http1Response$$Lambda$dbgString")) {
			return Http1Response$$Lambda$dbgString::load$(name, initialize);
		}
		if (name->equals("jdk.internal.net.http.Http1Response$$Lambda$advance$1")) {
			return Http1Response$$Lambda$advance$1::load$(name, initialize);
		}
		if (name->equals("jdk.internal.net.http.Http1Response$$Lambda$lambda$readHeadersAsync$0$2")) {
			return Http1Response$$Lambda$lambda$readHeadersAsync$0$2::load$(name, initialize);
		}
		if (name->equals("jdk.internal.net.http.Http1Response$$Lambda$lambda$readBody$3$3")) {
			return Http1Response$$Lambda$lambda$readBody$3$3::load$(name, initialize);
		}
		if (name->equals("jdk.internal.net.http.Http1Response$$Lambda$lambda$readBody$4$4")) {
			return Http1Response$$Lambda$lambda$readBody$4$4::load$(name, initialize);
		}
		if (name->equals("jdk.internal.net.http.Http1Response$$Lambda$lambda$readBody$5$5")) {
			return Http1Response$$Lambda$lambda$readBody$5$5::load$(name, initialize);
		}
		if (name->equals("jdk.internal.net.http.Http1Response$$Lambda$lambda$onReadError$6$6")) {
			return Http1Response$$Lambda$lambda$onReadError$6$6::load$(name, initialize);
		}
		if (name->equals("jdk.internal.net.http.Http1Response$$Lambda$lambda$onReadError$7$7")) {
			return Http1Response$$Lambda$lambda$onReadError$7$7::load$(name, initialize);
		}
		if (name->equals("jdk.internal.net.http.Http1Response$$Lambda$onFinished$8")) {
			return Http1Response$$Lambda$onFinished$8::load$(name, initialize);
		}
		if (name->equals("jdk.internal.net.http.Http1Response$$Lambda$lambda$readBody$1$9")) {
			return Http1Response$$Lambda$lambda$readBody$1$9::load$(name, initialize);
		}
		if (name->equals("jdk.internal.net.http.Http1Response$$Lambda$lambda$readBody$2$10")) {
	...
	$loadClass(Http1Response, name, initialize, &classInfo$$, Http1Response::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Http1Response);
	});
	return class$;
}
