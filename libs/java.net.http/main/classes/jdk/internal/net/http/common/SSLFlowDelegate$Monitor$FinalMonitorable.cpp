...
$String* SSLFlowDelegate$Monitor$FinalMonitorable::getInfo() {
	$nc(this->this$0->finalList)->remove(this);
...
$Class* SSLFlowDelegate$Monitor$FinalMonitorable::load$($String* name, bool initialize) {
	...
	$loadClass(SSLFlowDelegate$Monitor$FinalMonitorable, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SSLFlowDelegate$Monitor$FinalMonitorable);
	});
	return class$;
}
