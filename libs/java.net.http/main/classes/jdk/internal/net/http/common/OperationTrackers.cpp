...
$OperationTrackers$Tracker* OperationTrackers::getTracker($HttpClient* client) {
	if ($instanceOf($OperationTrackers$Trackable, client)) {
		return $cast($OperationTrackers$Trackable, client)->getOperationsTracker();
	} else {
		return nullptr;
	}
}
...
$Class* OperationTrackers::load$($String* name, bool initialize) {
	...
	$loadClass(OperationTrackers, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(OperationTrackers);
	});
	return class$;
}
