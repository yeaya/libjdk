...
$JCTree$JCClassDecl* LambdaToMethod$LambdaAnalyzerPreprocessor::analyzeAndPreprocessClass($JCTree$JCClassDecl* tree) {
	...
	return $cast($JCTree$JCClassDecl, translate(tree));
}
...
void LambdaToMethod$LambdaAnalyzerPreprocessor::visitApply($JCTree$JCMethodInvocation* tree) {
	...
	try {
		...
		if (methName == $nc(this->this$0->names)->_this || methName == this->this$0->names->_super) {
...
void LambdaToMethod$LambdaAnalyzerPreprocessor::visitBlock($JCTree$JCBlock* tree) {
	...
	try {
		...
		$init($JCTree$Tag);
		if (var$1 && $nc($nc(($cast($LambdaToMethod$LambdaAnalyzerPreprocessor$Frame, this->frameStack->head)))->tree)->hasTag($JCTree$Tag::CLASSDEF)) {
			...
			if ($nc(tree->sym)->hasOuterInstance()) {
				...
				while (localContext != nullptr && !$nc(localContext->owner)->isStatic()) {
					...
					if ($nc(localContext->tree)->hasTag($JCTree$Tag::LAMBDA)) {
						...
						$cast($LambdaToMethod$LambdaAnalyzerPreprocessor$LambdaTranslationContext, localContext)->addSymbol(outerInstanceSymbol, $LambdaToMethod$LambdaSymbolKind::CAPTURED_THIS);
					}
...
void LambdaToMethod$LambdaAnalyzerPreprocessor::visitIdent($JCTree$JCIdent* tree) {
	...
	if (var$0 && lambdaIdentSymbolFilter($nc(tree)->sym)) {
		$init($Kinds$Kind);
		if ($nc(tree->sym)->kind == $Kinds$Kind::VAR && $nc(tree->sym->owner)->kind == $Kinds$Kind::MTH && $nc(tree->type)->constValue() == nullptr) {
			$var($LambdaToMethod$LambdaAnalyzerPreprocessor$TranslationContext, localContext, context());
			while (localContext != nullptr) {
				...
				if ($nc(localContext->tree)->getTag() == $JCTree$Tag::LAMBDA) {
					...
					$init($LambdaToMethod$LambdaSymbolKind);
					$cast($LambdaToMethod$LambdaAnalyzerPreprocessor$LambdaTranslationContext, localContext)->addSymbol(tree->sym, $LambdaToMethod$LambdaSymbolKind::CAPTURED_VAR);
				}
				$assign(localContext, localContext->prev);
			}
		} else if ($nc($nc(tree->sym)->owner)->kind == $Kinds$Kind::TYP) {
			$var($LambdaToMethod$LambdaAnalyzerPreprocessor$TranslationContext, localContext, context());
			while (localContext != nullptr && !$nc(localContext->owner)->isStatic()) {
				$init($JCTree$Tag);
				if ($nc(localContext->tree)->hasTag($JCTree$Tag::LAMBDA)) {
...
void LambdaToMethod$LambdaAnalyzerPreprocessor::analyzeLambda($JCTree$JCLambda* tree, $JCTree$JCExpression* methodReferenceReceiver) {
	$useLocalObjectStack();
	$var($JCTree$JCExpression, rcvr, $cast($JCTree$JCExpression, translate(methodReferenceReceiver)));
...
$LambdaToMethod$LambdaAnalyzerPreprocessor$LambdaTranslationContext* LambdaToMethod$LambdaAnalyzerPreprocessor::analyzeLambda($JCTree$JCLambda* tree, $String* statKey) {
	...
	try {
		...
		$set(this, frameStack, $nc(this->frameStack)->prepend($$new($LambdaToMethod$LambdaAnalyzerPreprocessor$Frame, this, tree)));
		{
			...
			for (; $nc(i$)->hasNext();) {
				$var($JCTree$JCVariableDecl, param, $cast($JCTree$JCVariableDecl, i$->next()));
				{
					...
					$nc($cast($LambdaToMethod$LambdaAnalyzerPreprocessor$Frame, $nc(this->frameStack)->head))->addLocal(param->sym);
				}
			}
		}
		...
		if (this->this$0->dumpLambdaToMethodStats) {
			$nc(this->this$0->log)->note(tree, $($nc(this->this$0->diags)->noteKey(statKey, $$new($ObjectArray, {
				$($Boolean::valueOf(context->needsAltMetafactory())),
				context->translatedSym
			}))));
		}
...
void LambdaToMethod$LambdaAnalyzerPreprocessor::captureLocalClassDefs($Symbol* csym, $LambdaToMethod$LambdaAnalyzerPreprocessor$LambdaTranslationContext* lambdaContext) {
	...
	if (localCDef != nullptr && $nc($nc(lambdaContext)->freeVarProcessedLocalClasses)->add(csym)) {
		$var($Lower$BasicFreeVarCollector, fvc, $new($LambdaToMethod$LambdaAnalyzerPreprocessor$1, this, $nc(this->this$0->lower), lambdaContext));
		fvc->scan(localCDef);
	}
}
...
void LambdaToMethod$LambdaAnalyzerPreprocessor::visitReference($JCTree$JCMemberReference* tree) {
	...
	if (rcontext->needsConversionToLambda()) {
		...
		if (this->this$0->dumpLambdaToMethodStats) {
			$nc(this->this$0->log)->note(tree, $($CompilerProperties$Notes::MrefStat(rcontext->needsAltMetafactory(), nullptr)));
		}
	}
}
...
void LambdaToMethod$LambdaAnalyzerPreprocessor::visitSelect($JCTree$JCFieldAccess* tree) {
	$useLocalObjectStack();
	$init($Kinds$Kind);
	if (context() != nullptr && $nc($nc(tree)->sym)->kind == $Kinds$Kind::VAR && (tree->sym->name == $nc(this->this$0->names)->_this || tree->sym->name == this->this$0->names->_super)) {
		$var($LambdaToMethod$LambdaAnalyzerPreprocessor$TranslationContext, localContext, context());
		while (localContext != nullptr && !$nc(localContext->owner)->isStatic()) {
			$init($JCTree$Tag);
			if ($nc(localContext->tree)->hasTag($JCTree$Tag::LAMBDA)) {
				...
				$init($LambdaToMethod$LambdaSymbolKind);
				$cast($LambdaToMethod$LambdaAnalyzerPreprocessor$LambdaTranslationContext, localContext)->addSymbol($nc(clazz)->sym, $LambdaToMethod$LambdaSymbolKind::CAPTURED_THIS);
			}
			$assign(localContext, localContext->prev);
		}
	}
...
$Symbol* LambdaToMethod$LambdaAnalyzerPreprocessor::owner(bool skipLambda) {
	...
	while ($nc(frameStack2)->nonEmpty()) {
		...
		case 5:
			if ($nc($cast($JCTree$JCVariableDecl, $nc(($cast($LambdaToMethod$LambdaAnalyzerPreprocessor$Frame, $nc(frameStack2)->head)))->tree)->sym)->isDirectlyOrIndirectlyLocal()) {
				$assign(frameStack2, frameStack2->tail);
				break;
			}
			$assign(cdecl$, $cast($JCTree$JCClassDecl, $nc(($cast($LambdaToMethod$LambdaAnalyzerPreprocessor$Frame, $nc($nc(frameStack2)->tail)->head)))->tree));
			return initSym($nc(cdecl$)->sym, $nc($cast($JCTree$JCVariableDecl, $nc(($cast($LambdaToMethod$LambdaAnalyzerPreprocessor$Frame, frameStack2->head)))->tree)->sym)->flags() & 8);
		case 6:
			$assign(cdecl2, $cast($JCTree$JCClassDecl, $nc(($cast($LambdaToMethod$LambdaAnalyzerPreprocessor$Frame, $nc($nc(frameStack2)->tail)->head)))->tree));
			return initSym($nc(cdecl2)->sym, $cast($JCTree$JCBlock, $nc(($cast($LambdaToMethod$LambdaAnalyzerPreprocessor$Frame, frameStack2->head)))->tree)->flags & 8);
		case 4:
			return $cast($JCTree$JCClassDecl, $nc(($cast($LambdaToMethod$LambdaAnalyzerPreprocessor$Frame, $nc(frameStack2)->head)))->tree)->sym;
		case 7:
			return $cast($JCTree$JCMethodDecl, $nc(($cast($LambdaToMethod$LambdaAnalyzerPreprocessor$Frame, $nc(frameStack2)->head)))->tree)->sym;
		case 8:
			if (!skipLambda) {
				return $nc($$cast($LambdaToMethod$LambdaAnalyzerPreprocessor$LambdaTranslationContext, $nc(this->this$0->contextMap)->get($nc(($cast($LambdaToMethod$LambdaAnalyzerPreprocessor$Frame, $nc(frameStack2)->head)))->tree)))->translatedSym;
			}
		default:
			$assign(frameStack2, $nc(frameStack2)->tail);
		}
	}
	$Assert::error();
	return nullptr;
...
$Symbol* LambdaToMethod$LambdaAnalyzerPreprocessor::initSym($Symbol$ClassSymbol* csym, int64_t flags) {
	...
	if (isStatic) {
		...
		if (clinit == nullptr) {
			...
			$var($Type, var$1, $nc(this->this$0->syms)->voidType);
			$assign(clinit, this->this$0->makePrivateSyntheticMethod(8, $nc(this->this$0->names)->clinit, $$new($Type$MethodType, var$0, var$1, $($List::nil()), this->this$0->syms->methodClass), csym));
			$nc(this->clinits)->put(csym, clinit);
		}
		return clinit;
	} else {
...
$JCTree* LambdaToMethod$LambdaAnalyzerPreprocessor::directlyEnclosingLambda() {
	...
	while ($nc(frameStack2)->nonEmpty()) {
		...
		case 7:
			return nullptr;
		case 8:
			return ($cast($LambdaToMethod$LambdaAnalyzerPreprocessor$Frame, frameStack2->head))->tree;
		default:
			$assign(frameStack2, frameStack2->tail);
		}
	}
			...
			{
				{
					...
					case 4:
						{
							$assign(clazz, $cast($JCTree$JCClassDecl, block->tree)->sym);
					...
					case 5:
						$init($Kinds$Kind);
						if (($equals($cast($JCTree$JCVariableDecl, block->tree)->sym, sym) && $nc($nc(sym)->owner)->kind == $Kinds$Kind::MTH) || (block->locals != nullptr && block->locals->contains(sym))) {
							return currentDepth > depth ? ($JCTree*)nullptr : block->tree;
						}
						break;
					case 6:
					case 7:
					case 8:
						if (block->locals != nullptr && block->locals->contains(sym)) {
							return currentDepth > depth ? ($JCTree*)nullptr : block->tree;
						}
						break;
					default:
						$Assert::error($$str({"bad decl kind "_s, $(block->tree->getTag())}));
					}
				}
				--currentDepth;
			}
...
bool LambdaToMethod$LambdaAnalyzerPreprocessor::lambdaIdentSymbolFilter($Symbol* sym) {
	$init($Kinds$Kind);
	return ($nc(sym)->kind == $Kinds$Kind::VAR || sym->kind == $Kinds$Kind::MTH) && !sym->isStatic() && sym->name != $nc(this->this$0->names)->init;
}
...
bool LambdaToMethod$LambdaAnalyzerPreprocessor::lambdaFieldAccessFilter($JCTree$JCFieldAccess* fAccess) {
	...
	$init($Kinds$Kind);
	bool var$0 = (var$1) && !$nc($nc(fAccess)->sym)->isStatic() && fAccess->name == $nc(this->this$0->names)->_this && ($nc(fAccess->sym->owner)->kind == $Kinds$Kind::TYP);
	$init($LambdaToMethod$LambdaSymbolKind);
	return var$0 && !$$sure($Map, $nc($nc(lambdaContext)->translatedSymbols)->get($LambdaToMethod$LambdaSymbolKind::CAPTURED_OUTER_THIS))->isEmpty();
}
...
$Class* LambdaToMethod$LambdaAnalyzerPreprocessor::load$($String* name, bool initialize) {
	...
	$loadClass(LambdaToMethod$LambdaAnalyzerPreprocessor, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LambdaToMethod$LambdaAnalyzerPreprocessor);
	});
	return class$;
}
