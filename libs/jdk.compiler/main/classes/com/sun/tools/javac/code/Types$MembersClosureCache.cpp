...
$Scope$CompoundScope* Types$MembersClosureCache::visitClassType($Type$ClassType* t, $Void* _unused) {
	...
	try {
		...
		$var($Symbol$ClassSymbol, csym, $cast($Symbol$ClassSymbol, t->tsym));
		...
		$nc(this->seenTypes)->remove(t->tsym);
	}
	if (var$0 != nullptr) {
		$throw(var$0);
	}
...
$Class* Types$MembersClosureCache::load$($String* name, bool initialize) {
	...
	$loadClass(Types$MembersClosureCache, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Types$MembersClosureCache);
	});
	return class$;
}
