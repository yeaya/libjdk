...
bool Resolve$MostSpecificCheck$MostSpecificCheckContext::compatible($Type* found, $Type* req, $Warner* warn) {
	...
	$init($TypeTag);
	if (var$0 && (this->actual != nullptr && this->actual->getTag() == $TypeTag::DEFERRED)) {
		...
		if (speculativeTree != $nc(this->this$1->this$0->deferredAttr)->stuckTree) {
...
bool Resolve$MostSpecificCheck$MostSpecificCheckContext::compatibleBySubtyping($Type* found$renamed, $Type* req) {
	...
	if (var$0) {
		$assign(found, $nc(found)->isPrimitive() ? $nc($($nc($nc(this->this$1->this$0)->types)->boxedClass(found)))->type : $nc($nc(this->this$1->this$0)->types)->unboxedType(found));
	}
...
bool Resolve$MostSpecificCheck$MostSpecificCheckContext::unrelatedFunctionalInterfaces($Type* t, $Type* s) {
	...
	bool var$0 = var$1 && this->this$1->this$0->types->isFunctionalInterface($nc(s)->tsym);
...
bool Resolve$MostSpecificCheck$MostSpecificCheckContext::unrelatedInterfaces($Type* t, $Type* s) {
	...
	if ($nc(t)->isCompound()) {
		...
		return var$0 && this->this$1->this$0->types->asSuper(s, t->tsym) == nullptr;
	}
}
...
bool Resolve$MostSpecificCheck$MostSpecificCheckContext::functionalInterfaceMostSpecific($Type* t, $Type* s, $JCTree* tree) {
	...
	$var($Type, tDescNoCapture, $nc(this->this$1->this$0->types)->findDescriptorType(t));
	$var($Type, sDesc, $nc(this->this$1->this$0->types)->findDescriptorType(s));
	...
	if (var$0 && !$nc(this->this$1->this$0->types)->hasSameBounds($cast($Type$ForAll, tDesc), $cast($Type$ForAll, tDescNoCapture))) {
		return false;
	}
	...
	while (true) {
		...
		}
		{
			...
			$var($Type, sBound, $nc(this->this$1->this$0->types)->subst($($nc($cast($Type, $nc(sIter)->head))->getUpperBound()), sTypeParams, tTypeParams));
			bool var$2 = $nc(tBound)->containsAny(tTypeParams);
			if (var$2 && $$nc(inferenceContext())->free(sBound)) {
				return false;
			}
			if (!$nc(this->this$1->this$0->types)->isSameType(tBound, $($$nc(inferenceContext())->asUndetVar(sBound)))) {
				return false;
			}
	...
	while (true) {
		...
		}
		{
			...
			$var($Type, tParamNoCapture, $nc(this->this$1->this$0->types)->subst($cast($Type, $nc(tParamsNoCapture)->head), tTypeParamsNoCapture, tTypeParams));
			$var($Type, sParam, $nc(this->this$1->this$0->types)->subst($cast($Type, $nc(sParams)->head), sTypeParams, tTypeParams));
			bool var$6 = $nc(tParam)->containsAny(tTypeParams);
			if (var$6 && $$nc(inferenceContext())->free(sParam)) {
				return false;
			}
			if (!$nc(this->this$1->this$0->types)->isSubtype($($$nc(inferenceContext())->asUndetVar(sParam)), tParam)) {
				return false;
			}
			if (!$nc(this->this$1->this$0->types)->isSameType(tParamNoCapture, $($$nc(inferenceContext())->asUndetVar(sParam)))) {
				return false;
			}
	...
	$var($Type, sRet, $nc(this->this$1->this$0->types)->subst($(sDesc->getReturnType()), sTypeParams, tTypeParams));
...
$Class* Resolve$MostSpecificCheck$MostSpecificCheckContext::load$($String* name, bool initialize) {
	...
	$loadClass(Resolve$MostSpecificCheck$MostSpecificCheckContext, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Resolve$MostSpecificCheck$MostSpecificCheckContext);
	});
	return class$;
}
