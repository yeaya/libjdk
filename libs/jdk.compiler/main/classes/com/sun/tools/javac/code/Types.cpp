...
$Class* Types$$Lambda$lambda$new$4::load$($String* name, bool initialize) {
	...
	$loadClass(Types$$Lambda$lambda$new$4, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Types$$Lambda$lambda$new$4);
	});
	return class$;
}
...
$Class* Types$$Lambda$lambda$removeWildcards$0$1::load$($String* name, bool initialize) {
	...
	$loadClass(Types$$Lambda$lambda$removeWildcards$0$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Types$$Lambda$lambda$removeWildcards$0$1);
	});
	return class$;
}
...
$Class* Types$$Lambda$lambda$areDisjoint$1$2::load$($String* name, bool initialize) {
	...
	$loadClass(Types$$Lambda$lambda$areDisjoint$1$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Types$$Lambda$lambda$areDisjoint$1$2);
	});
	return class$;
}
...
$Class* Types$$Lambda$lambda$membersClosure$2$3::load$($String* name, bool initialize) {
	...
	$loadClass(Types$$Lambda$lambda$membersClosure$2$3, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Types$$Lambda$lambda$membersClosure$2$3);
	});
	return class$;
}
...
$Class* Types$$Lambda$lambda$closureCollector$3$4::load$($String* name, bool initialize) {
	...
	$loadClass(Types$$Lambda$lambda$closureCollector$3$4, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Types$$Lambda$lambda$closureCollector$3$4);
	});
	return class$;
}
...
$Class* Types$$Lambda$add$5::load$($String* name, bool initialize) {
	...
	$loadClass(Types$$Lambda$add$5, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Types$$Lambda$add$5);
	});
	return class$;
}
...
$Class* Types$$Lambda$merge$6::load$($String* name, bool initialize) {
	...
	$loadClass(Types$$Lambda$merge$6, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Types$$Lambda$merge$6);
	});
	return class$;
}
...
$Class* Types$$Lambda$closure$7::load$($String* name, bool initialize) {
	...
	$loadClass(Types$$Lambda$closure$7, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Types$$Lambda$closure$7);
	});
	return class$;
}
...
void Types::init$($Context* context) {
	...
	$set(this, basicClosureSkip, $new(Types$$Lambda$lambda$new$4));
	$set(this, mergeCache, $new($HashSet));
	$nc(context)->put(Types::typesKey, this);
...
$Type* Types::wildUpperBound($Type* t) {
	...
	if ($nc(t)->hasTag($TypeTag::WILDCARD)) {
		...
		if (w->isSuperBound()) {
			return w->bound == nullptr ? $nc(this->syms)->objectType : w->bound->getUpperBound();
		} else {
...
$Type* Types::cvarUpperBound($Type* t) {
	...
	if ($nc(t)->hasTag($TypeTag::TYPEVAR)) {
		...
		return v->isCaptured() ? cvarUpperBound($(v->getUpperBound())) : $cast($Type, v);
	} else {
		return t;
	}
}
...
bool Types::isUnbounded($Type* t) {
	return $$sure($Boolean, this->isUnbounded$->visit(t))->booleanValue();
}

$Type* Types::asSub($Type* t, $Symbol* sym) {
	return $cast($Type, this->asSub$->visit(t, sym));
}
...
$Type* Types::removeWildcards($Type* site) {
	$useLocalObjectStack();
	if ($$nc($$nc($nc(site)->getTypeArguments())->stream())->anyMatch($$new(Types$$Lambda$lambda$removeWildcards$0$1))) {
...
$Symbol$ClassSymbol* Types::makeFunctionalInterfaceClass($Env* env, $Name* name, $Type* target, int64_t cflags) {
	...
	$var($Symbol$MethodSymbol, instDescSym, $new($Symbol$MethodSymbol, $nc(descSym)->flags(), $nc(descSym)->name, descType, csym));
	...
	$set(ctype, interfaces_field, target->isIntersection() ? directSupertypes(target) : $List::of(target));
	$set(csym, type, ctype);
		...
		} else if (descSym->overrides(m2, origin, this, false)) {
			{
				...
				for (; $nc(i$)->hasNext();) {
					$var($Symbol, m3, $cast($Symbol, i$->next()));
					{
						...
						if (!var$0) {
							bool var$2 = m3->overrides(m2, origin, this, false);
							if (var$2) {
								bool var$3 = pendingBridges($cast($Symbol$ClassSymbol, origin), $(m3->enclClass()));
								var$2 = var$3 || ($cast($Symbol$MethodSymbol, m2)->binaryImplementation($cast($Symbol$ClassSymbol, m3->owner), this) != nullptr);
							}
							var$0 = var$2;
						}
						if (var$0) {
...
bool Types::pendingBridges($Symbol$ClassSymbol* origin, $Symbol$TypeSymbol* s) {
	...
	bool var$0 = $nc(origin)->classfile != nullptr && origin->classfile->getKind() == $JavaFileObject$Kind::CLASS;
	...
	{
		$var($Iterator, i$, $$nc(interfaces(origin->type))->iterator());
		for (; $nc(i$)->hasNext();) {
...
bool Types::isSubtypeUncheckedInternal($Type* t, $Type* s, bool capture, $Warner* warn) {
	...
	if (var$0 && $nc(s)->hasTag($TypeTag::ARRAY)) {
		if ($nc($cast($Type$ArrayType, t)->elemtype)->isPrimitive()) {
...
bool Types::isSubtype($Type* t, $Type* s, bool capture) {
	...
	return $$sure($Boolean, $nc(this->isSubtype$)->visit(capture ? $(this->capture(t)) : t, s))->booleanValue();
}
...
bool Types::isSignaturePolymorphic($Symbol$MethodSymbol* msym) {
	...
	bool var$0 = (msym->flags_field & 0x0100) != 0 && ($equals(msym->owner, $nc($nc(this->syms)->methodHandleType)->tsym) || $equals(msym->owner, $nc(this->syms->varHandleType)->tsym)) && $nc(argtypes)->length() == 1;
	$init($TypeTag);
	return var$0 && $nc($cast($Type, argtypes->head))->hasTag($TypeTag::ARRAY) && $nc($cast($Type$ArrayType, argtypes->head)->elemtype)->tsym == $nc(this->syms->objectType)->tsym;
}

bool Types::isSameType($Type* t, $Type* s) {
	return $$sure($Boolean, $nc(this->isSameTypeVisitor)->visit(t, s))->booleanValue();
}
...
bool Types::containsType($Type* t, $Type* s) {
	return $$sure($Boolean, $nc(this->containsType$)->visit(t, s))->booleanValue();
}

bool Types::isCaptureOf($Type* s, $Type$WildcardType* t) {
	$init($TypeTag);
	bool var$0 = !$nc(s)->hasTag($TypeTag::TYPEVAR);
	if (var$0 || !$cast($Type$TypeVar, s)->isCaptured()) {
		return false;
	}
	return isSameWildcard(t, $cast($Type$CapturedType, s)->wildcard);
}
...
bool Types::isSameWildcard($Type$WildcardType* t, $Type* s) {
	...
	return w->kind == $nc(t)->kind && w->type == t->type;
}
...
bool Types::isCastable($Type* t$renamed, $Type* s, $Warner* warn) {
	...
	if (!$equals(warn, $nc(this->warnStack)->head)) {
		...
		try {
			$set(this, warnStack, this->warnStack->prepend(warn));
			checkUnsafeVarargsConversion(t, s, warn);
			result = $$sure($Boolean, $nc(this->isCastable$)->visit(t, s))->booleanValue();
	...
	bool var$6 = var$7 && s->hasTag($TypeTag::CLASS);
	bool var$5 = var$6 && $nc($nc(s->tsym)->kind)->matches($Kinds$KindSelector::TYP);
	if (var$5) {
		bool var$9 = t->tsym->isSealed();
		var$5 = var$9 || s->tsym->isSealed();
	}
	if (var$5) {
		bool var$10 = $nc(t)->isCompound();
		return (var$10 || s->isCompound()) ? false : !areDisjoint($cast($Symbol$ClassSymbol, t->tsym), $cast($Symbol$ClassSymbol, s->tsym));
	}
	return result;
}
...
bool Types::areDisjoint($Symbol$ClassSymbol* ts, $Symbol$ClassSymbol* ss) {
	...
	bool var$2 = ts->isInterface();
	bool var$1 = var$2 == ss->isInterface();
	if (var$1) {
	...
	bool var$4 = ts->isInterface();
	if (var$4 && !ss->isInterface()) {
		return areDisjoint(ss, ts);
	}
	bool var$5 = !ts->isInterface();
	if (var$5 && ts->isFinal()) {
		return true;
	}
	bool var$6 = ts->isSealed();
	if (var$6 || ss->isSealed()) {
		$var($Symbol$ClassSymbol, sealedOne, ts->isSealed() ? ts : ss);
		$var($Symbol$ClassSymbol, other, sealedOne == ts ? ss : ts);
		return $$nc($nc($nc(sealedOne)->permitted)->stream())->allMatch($$new(Types$$Lambda$lambda$areDisjoint$1$2, this, other));
	}
	return false;
}
...
bool Types::disjointType($Type* t, $Type* s) {
	return $$sure($Boolean, $nc(this->disjointType$)->visit(t, s))->booleanValue();
}
...
bool Types::isReifiable($Type* t) {
	return $$sure($Boolean, $nc(this->isReifiable$)->visit(t))->booleanValue();
}
...
$Type* Types::elemtype($Type* t) {
	...
	case 1:
		return $cast($Type$ArrayType, t)->elemtype;
	case 18:
		return elemtype($cast($Type$ForAll, t)->qtype);
	case 16:
		return t;
	default:
		return nullptr;
...
$Type* Types::asOuterSuper($Type* t$renamed, $Symbol* sym) {
	...
	case 2:
		do {
			...
			$assign(t, $nc(t)->getEnclosingType());
		} while ($nc(t)->hasTag($TypeTag::CLASS));
		return nullptr;
	case 1:
		return isSubtype(t, $nc(sym)->type) ? sym->type : ($Type*)nullptr;
	case 12:
...
$Type* Types::asEnclosingSuper($Type* t$renamed, $Symbol* sym) {
	...
	case 2:
		do {
			...
			$assign(t, ($nc(outer)->hasTag($TypeTag::CLASS)) ? outer : ($nc($nc(t->tsym)->owner)->enclClass() != nullptr) ? $nc($(t->tsym->owner->enclClass()))->type : $cast($Type, $Type::noType));
		} while ($nc(t)->hasTag($TypeTag::CLASS));
		return nullptr;
	case 1:
		return isSubtype(t, $nc(sym)->type) ? sym->type : ($Type*)nullptr;
...
$Type* Types::memberType($Type* t, $Symbol* sym) {
	return ($nc(sym)->flags() & 8) != 0 ? sym->type : $cast($Type, $nc(this->memberType$)->visit(t, sym));
}
...
bool Types::isAssignable($Type* t, $Type* s, $Warner* warn) {
	...
	if (var$0 && t->constValue() != nullptr) {
		int32_t value = $$sure($Number, t->constValue())->intValue();
		$init($Types$25);
		switch ($nc($Types$25::$SwitchMap$com$sun$tools$javac$code$TypeTag)->get(($$nc($nc(s)->getTag()))->ordinal())) {
		case 3:
		case 4:
		...
		case 6:
			...
			break;
		case 2:
			switch ($Types$25::$SwitchMap$com$sun$tools$javac$code$TypeTag->get(($$nc($$nc(unboxedType(s))->getTag()))->ordinal())) {
			case 3:
			case 4:
			case 5:
...
bool Types::eraseNotNeeded($Type* t) {
	return ($nc(t)->isPrimitive()) || ($nc($nc(this->syms)->stringType)->tsym == t->tsym);
}
...
$List* Types::erasure($List* ts) {
	return $nc(this->erasure$)->visit(ts, $($Boolean::valueOf(false)));
}
...
$List* Types::erasureRecursive($List* ts) {
	return $nc(this->erasure$)->visit(ts, $($Boolean::valueOf(true)));
}
...
$Type$IntersectionClassType* Types::makeIntersectionType($List* bounds$renamed, bool allInterfaces) {
	...
	$var($Type, firstExplicitBound, $cast($Type, bounds->head));
	if (allInterfaces) {
		$assign(bounds, bounds->prepend($nc(this->syms)->objectType));
	}
	$init($Type);
	$var($Symbol$ClassSymbol, bc, $new($Symbol$ClassSymbol, (((0x0400 | 1) | 0x1000) | 0x01000000) | 0x40000000, $Type::moreInfo ? $($nc(this->names)->fromString($($nc(bounds)->toString()))) : $nc(this->names)->empty, nullptr, $nc(this->syms)->noSymbol));
	...
	$init($TypeTag);
	$set(bc, erasure_field, ($nc($cast($Type, $nc(bounds)->head))->hasTag($TypeTag::TYPEVAR)) ? this->syms->objectType : erasure(firstExplicitBound));
...
$List* Types::directSupertypes($Type* t) {
	return $cast($List, this->directSupertypes$->visit(t));
}
...
void Types::setBounds($Type$TypeVar* t, $List* bounds, bool allInterfaces) {
	$nc(t)->setUpperBound($nc($nc(bounds)->tail)->isEmpty() ? $cast($Type, bounds->head) : $$cast($Type, makeIntersectionType(bounds, allInterfaces)));
	t->rank_field = -1;
}
...
$List* Types::getBounds($Type$TypeVar* t) {
	...
	if ($$nc($nc(t)->getUpperBound())->hasTag($TypeTag::NONE)) {
		...
		bool var$0 = $$nc(t->getUpperBound())->isErroneous();
		if (var$0 || !$$nc(t->getUpperBound())->isCompound()) {
			return $List::of($(t->getUpperBound()));
		} else if (($nc($nc($(erasure(t)))->tsym)->flags() & 0x0200) == 0) {
...
bool Types::overridesObjectMethod($Symbol$TypeSymbol* origin, $Symbol* msym) {
	...
	for (; $nc(i$)->hasNext();) {
		...
		if (msym->overrides(sym, origin, this, true)) {
			return true;
		}
	}
	return false;
...
$Optional* Types::mergeAbstracts($List* ambiguousInOrder, $Type* site, bool sigCheck) {
	...
	$var($List, erasedParams, $$nc($nc($cast($Symbol, $nc(ambiguousInOrder)->head))->erasure(this))->getParameterTypes());
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			{
				bool var$0 = ($nc(s)->flags() & 0x0400) == 0;
				if (var$0 || (sigCheck && !isSameTypes(erasedParams, $($$nc(s->erasure(this))->getParameterTypes())))) {
			...
			$Types$MostSpecificReturnCheck* mostSpecificReturnCheck = arr$->get(i$);
			{
				...
				for (; $nc(i$)->hasNext();) {
					$var($Symbol, s, $cast($Symbol, i$->next()));
					{
								...
								if (s != s2) {
									...
									if (var$1 || !$nc(mostSpecificReturnCheck)->test(mt, mt2, this)) {
										...
										if (!mt->hasTag($TypeTag::FORALL) && shouldErase) {
											...
											$var($List, var$2, mt2->getTypeArguments());
											$assign(thrownTypes2, subst(thrownTypes2, var$2, $(mt->getTypeArguments())));
										}
						...
						$var($Optional, var$3, nullptr);
						if (allThrown == mt->getThrownTypes()) {
							$assign(var$3, $Optional::of(s));
						} else {
							int64_t var$4 = $nc(s)->flags();
							$var($Name, var$5, s->name);
							$assign(var$3, $Optional::of($$new($Types$19, this, var$4, var$5, $(createMethodTypeWithThrown(s->type, allThrown)), s->owner, s)));
						}
						return var$3;
					}
				}
...
$Scope$CompoundScope* Types::membersClosure($Type* site, bool skipInterface) {
	...
	$Assert::checkNonNull(cs, $$new(Types$$Lambda$lambda$membersClosure$2$3, site));
	return skipInterface ? $cast($Scope$CompoundScope, $new($Types$MembersClosureCache$MembersScope, $nc(this->membersCache), cs)) : cs;
}
...
$Symbol$MethodSymbol* Types::firstUnimplementedAbstractImpl($Symbol$ClassSymbol* impl, $Symbol$ClassSymbol* c) {
	...
	if (c == impl || ($nc(c)->flags() & (0x0400 | 0x0200)) != 0) {
		$var($Scope, s, $nc(c)->members());
		{
			...
			for (; $nc(i$)->hasNext();) {
				$var($Symbol, sym, $cast($Symbol, i$->next()));
				$init($Kinds$Kind);
				if ($nc(sym)->kind == $Kinds$Kind::MTH && (sym->flags() & ((0x0400 | (int64_t)0x0000080000000000) | 2)) == 0x0400) {
		...
		}
		{
			...
			for (; undef == nullptr && $nc(l)->nonEmpty(); $assign(l, l->tail)) {
...
$List* Types::interfaceCandidates($Type* site, $Symbol$MethodSymbol* ms) {
	$useLocalObjectStack();
	$var($Types$CandidatesCache$Entry, e, $new($Types$CandidatesCache$Entry, $nc(this->candidatesCache), site, ms));
	$var($List, candidates, this->candidatesCache->get(e));
...
bool Types::hasSameArgs($Type* t, $Type* s, $Types$TypeRelation* hasSameArgs) {
	return $$sure($Boolean, $nc(hasSameArgs)->visit(t, s))->booleanValue();
}
...
$List* Types::substBounds($List* tvars, $List* from$renamed, $List* to$renamed) {
	...
	$var($ListBuffer, newTvars, $new($ListBuffer));
	{
		...
		for (; $nc(i$)->hasNext();) {
			$var($Type, t, $cast($Type, i$->next()));
			{
				newTvars->append($$new($Type$TypeVar, $nc(t)->tsym, nullptr, $nc(this->syms)->botType, $($nc(t)->getMetadata())));
			}
		}
	}
	...
	for (; !$nc(newBounds)->isEmpty(); $assign(newBounds, newBounds->tail)) {
	...
	$assign(newBounds, newBoundsBuf->toList());
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			{
				$var($Type$TypeVar, tv, $cast($Type$TypeVar, t));
				$nc(tv)->setUpperBound($cast($Type, $nc(newBounds)->head));
...
$Type$TypeVar* Types::substBound($Type$TypeVar* t, $List* from, $List* to) {
	...
	if (bound1 == t->getUpperBound()) {
		...
		tv->setUpperBound($(subst(bound1, var$0, $($List::of(tv)))));
		return tv;
	}
}
...
$List* Types::newInstances($List* tvars) {
	...
	$var($List, tvars1, $nc(tvars)->map(Types::newInstanceFun));
	{
		...
		for (; $nc(l)->nonEmpty(); $assign(l, l->tail)) {
			...
			$nc(tv)->setUpperBound($(subst($($nc(tv)->getUpperBound()), tvars, tvars1)));
		}
	}
	return tvars1;
}
...
$Type* Types::createMethodTypeWithParameters($Type* original, $List* newParams) {
	return $cast($Type, $nc(original)->accept(this->methodWithParameters, newParams));
}

$Type* Types::createMethodTypeWithThrown($Type* original, $List* newThrown) {
	return $cast($Type, $nc(original)->accept(this->methodWithThrown, newThrown));
}

$Type* Types::createMethodTypeWithReturn($Type* original, $Type* newReturn) {
	return $cast($Type, $nc(original)->accept(this->methodWithReturn, newReturn));
}

$Type* Types::createErrorType($Type* originalType) {
	return $new($Type$ErrorType, originalType, $nc(this->syms)->errSymbol);
}
...
$Collector* Types::closureCollector(bool minClosure, $BiPredicate* shouldSkip) {
	$useLocalObjectStack();
	$var($Supplier, var$0, $new(Types$$Lambda$lambda$closureCollector$3$4, this, minClosure, shouldSkip));
	$var($BiConsumer, var$1, $new(Types$$Lambda$add$5));
	$var($BinaryOperator, var$2, $new(Types$$Lambda$merge$6));
	return $Collector::of(var$0, var$1, var$2, $$new(Types$$Lambda$closure$7), $$new($Collector$CharacteristicsArray, 0));
}
...
$List* Types::intersect($List* cl1, $List* cl2) {
	...
	if ($nc($nc(($cast($Type, cl1->head)))->tsym)->precedes($nc(($cast($Type, $nc(cl2)->head)))->tsym, this)) {
		return intersect(cl1->tail, cl2);
	}
	if ($nc($nc(($cast($Type, cl2->head)))->tsym)->precedes($nc(($cast($Type, cl1->head)))->tsym, this)) {
		return intersect(cl1, cl2->tail);
	}
	if (isSameType($cast($Type, cl1->head), $cast($Type, cl2->head))) {
		return $$nc(intersect(cl1->tail, cl2->tail))->prepend($cast($Type, cl1->head));
	}
	$init($TypeTag);
	bool var$1 = $nc(($cast($Type, cl1->head)))->tsym == $nc(($cast($Type, cl2->head)))->tsym && $cast($Type, cl1->head)->hasTag($TypeTag::CLASS);
	if (var$1 && $cast($Type, cl2->head)->hasTag($TypeTag::CLASS)) {
...
$Type* Types::merge($Type* c1, $Type* c2) {
	...
	while (true) {
		...
		{
			if (containsType($cast($Type, act1->head), $cast($Type, $nc(act2)->head))) {
				...
				merged->append($($nc(m)->withTypeVar($cast($Type, $nc(typarams)->head))));
			}
			$assign(act1, act1->tail);
			$assign(act2, act2->tail);
			$assign(typarams, $nc(typarams)->tail);
		}
	}
...
$Type* Types::lub($List* ts) {
	$useLocalObjectStack();
	return lub($$cast($TypeArray, $nc(ts)->toArray($$new($TypeArray, $nc(ts)->length()))));
}
...
$Type* Types::lub($TypeArray* ts) {
	...
	for (int32_t i = 0; i < ts->length; ++i) {
		...
		case 12:
			do {
				$assign(t, $nc(t)->getUpperBound());
			} while ($nc(t)->hasTag($TypeTag::TYPEVAR));
...
case$2:
			// CLASS_BOUND
			{
				...
				for (int32_t i = startIdx + 1; i < ts->length; ++i) {
					...
					if (var$1 || t->hasTag($TypeTag::TYPEVAR)) {
				...
				$assign(candidates, $List::nil());
				{
					...
					for (; $nc(i$)->hasNext();) {
						$var($Type, erasedSupertype, $cast($Type, i$->next()));
						{
							...
							for (int32_t i = startIdx + 1; i < ts->length; ++i) {
								$var($Type, superType, asSuper(ts->get(i), erasedSupertype->tsym));
...
$Type* Types::glb($Type* t, $Type* s) {
	...
	$var($List, var$1, this->closure(t));
	$var($List, closure, union$(var$1, $(this->closure(s))));
...
int32_t Types::hashCode($Type* t, bool strict) {
	$useLocalObjectStack();
	return $nc((strict ? $$cast($Integer, Types::hashCodeStrictVisitor->visit(t)) : $$cast($Integer, Types::hashCodeVisitor->visit(t))))->intValue();
}
...
bool Types::returnTypeSubstitutable($Type* r1, $Type* r2, $Type* r2res, $Warner* warner) {
	...
	bool var$0 = $$nc(r1->getReturnType())->isPrimitive();
	...
	$var($Type, var$1, r1->getReturnType());
...
$Type* Types::unboxedType($Type* t) {
	...
	for (int32_t i = 0; i < $nc($nc(this->syms)->boxedName)->length; ++i) {
		$var($Name, box, this->syms->boxedName->get(i));
		if (box != nullptr && asSuper(t, $(this->syms->enterClass(this->syms->java_base, box))) != nullptr) {
			return $nc(this->syms->typeOfTag)->get(i);
		}
	}
	return $Type::noType;
}
...
$Type* Types::capture($Type* t$renamed) {
	...
	if (!$equals(t->getEnclosingType(), $Type::noType)) {
		...
		if (capturedEncl != t->getEnclosingType()) {
			...
			$var($List, var$0, $nc($nc(t->tsym)->type)->getTypeArguments());
			$assign(t, subst(type1, var$0, $(t->getTypeArguments())));
		}
	}
	$var($Type$ClassType, cls, $cast($Type$ClassType, t));
	bool var$1 = $nc(cls)->isRaw();
	if (var$1 || !cls->isParameterized()) {
		return cls;
	}
	...
	bool captured = false;
	while (true) {
		bool var$3 = !$nc(currentA)->isEmpty();
		bool var$2 = var$3 && !$nc(currentT)->isEmpty();
		if (!(var$2 && !$nc(currentS)->isEmpty())) {
			break;
		}
		{
			if (!$equals($nc(currentS)->head, $nc(currentT)->head)) {
				captured = true;
				...
				case 3:
					...
					$set(Si, lower, $nc(this->syms)->botType);
					break;
				case 1:
					{
						$var($Type, var$4, $cast($Type, Ti->getExtendsBound()));
						$nc(Si)->setUpperBound($(glb(var$4, $(subst(Ui, A, S)))));
						$set($nc(Si), lower, $nc(this->syms)->botType);
						break;
					}
				case 2:
					$nc(Si)->setUpperBound($(subst(Ui, A, S)));
					$set(Si, lower, $cast($Type, Ti->getSuperBound()));
					break;
				}
				$var($Type, tmpBound, $$nc($nc(Si)->getUpperBound())->hasTag($TypeTag::UNDETVAR) ? $nc($$cast($Type$UndetVar, Si->getUpperBound()))->qtype : Si->getUpperBound());
				$var($Type, tmpLower, $nc(Si->lower)->hasTag($TypeTag::UNDETVAR) ? $cast($Type$UndetVar, Si->lower)->qtype : Si->lower);
				bool var$6 = !$$nc(Si->getUpperBound())->hasTag($TypeTag::ERROR);
				bool var$5 = var$6 && !$nc(Si->lower)->hasTag($TypeTag::ERROR);
				if (var$5 && isSameType(tmpBound, tmpLower)) {
	...
	bool var$8 = !$nc(currentA)->isEmpty();
	bool var$7 = var$8 || !$nc(currentT)->isEmpty();
	if (var$7 || !$nc(currentS)->isEmpty()) {
		return erasure(t);
	}
	if (captured) {
		$var($Type, var$9, cls->getEnclosingType());
		$var($Symbol$TypeSymbol, var$10, cls->tsym);
		return $new($Type$ClassType, var$9, S, var$10, $(cls->getMetadata()));
	} else {
		return t;
	}
}
...
$List* Types::freshTypeVariables($List* types) {
	...
	$var($ListBuffer, result, $new($ListBuffer));
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			$init($TypeTag);
			if ($nc(t)->hasTag($TypeTag::WILDCARD)) {
				$var($Type, bound, $cast($Type, $cast($Type$WildcardType, t)->getExtendsBound()));
				if (bound == nullptr) {
...
bool Types::sideCast($Type* from$renamed, $Type* to$renamed, $Warner* warn) {
	...
	bool reverse = false;
	$var($Type, target, to);
	if (($nc($nc(to)->tsym)->flags() & 0x0200) == 0) {
		$Assert::check(($nc($nc(from)->tsym)->flags() & 0x0200) != 0);
		reverse = true;
		$assign(to, from);
		$assign(from, target);
	}
	...
	while ($nc(commonSupers)->nonEmpty()) {
...
bool Types::sideCastFinal($Type* from$renamed, $Type* to$renamed, $Warner* warn) {
	...
	$Assert::check(($nc($nc(from)->tsym)->flags() & 0x10) != 0);
	$var($Type, t1, asSuper(from, to->tsym));
	if (t1 == nullptr) {
		return false;
	}
	$var($Type, t2, to);
	$var($List, var$0, $nc(t1)->getTypeArguments());
	if (disjointTypes(var$0, $(t2->getTypeArguments()))) {
		return false;
	}
...
$Type$WildcardType* Types::makeSuperWildcard($Type* bound, $Type$TypeVar* formal) {
	...
	if ($nc(bound)->hasTag($TypeTag::BOT)) {
		$init($BoundKind);
		return $new($Type$WildcardType, $nc(this->syms)->objectType, $BoundKind::UNBOUND, $nc(this->syms)->boundClass, formal);
	} else {
		$init($BoundKind);
		return $new($Type$WildcardType, bound, $BoundKind::SUPER, $nc(this->syms)->boundClass, formal);
	}
}
...
$Type* Types::constantType($PoolConstant$LoadableConstant* c) {
	...
	case $ClassFile::CONSTANT_Dynamic:
		return $cast($Symbol$DynamicVarSymbol, c)->type;
	default:
...
$Class* Types::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.code.Types$$Lambda$lambda$new$4")) {
			return Types$$Lambda$lambda$new$4::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.code.Types$$Lambda$lambda$removeWildcards$0$1")) {
			return Types$$Lambda$lambda$removeWildcards$0$1::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.code.Types$$Lambda$lambda$areDisjoint$1$2")) {
			return Types$$Lambda$lambda$areDisjoint$1$2::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.code.Types$$Lambda$lambda$membersClosure$2$3")) {
			return Types$$Lambda$lambda$membersClosure$2$3::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.code.Types$$Lambda$lambda$closureCollector$3$4")) {
			return Types$$Lambda$lambda$closureCollector$3$4::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.code.Types$$Lambda$add$5")) {
			return Types$$Lambda$add$5::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.code.Types$$Lambda$merge$6")) {
			return Types$$Lambda$merge$6::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.code.Types$$Lambda$closure$7")) {
	...
	$CompoundAttribute toStringmethodAnnotations$$$2[] = {
	...
	$MethodInfo methodInfos$$[] = {
		...
		{"toString", "(Lcom/sun/tools/javac/code/Type;)Ljava/lang/String;", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Types, toString, $String*, $Type*), nullptr, nullptr, toStringmethodAnnotations$$$2},
	...
	$loadClass(Types, name, initialize, &classInfo$$, Types::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Types);
	});
	return class$;
}
