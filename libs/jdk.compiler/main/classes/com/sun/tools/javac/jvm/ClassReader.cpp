...
$Class* ClassReader$$Lambda$getNameAndType::load$($String* name, bool initialize) {
	...
	$loadClass(ClassReader$$Lambda$getNameAndType, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ClassReader$$Lambda$getNameAndType);
	});
	return class$;
}
...
$Class* ClassReader$$Lambda$getName$1::load$($String* name, bool initialize) {
	...
	$loadClass(ClassReader$$Lambda$getName$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ClassReader$$Lambda$getName$1);
	});
	return class$;
}
...
$Class* ClassReader$$Lambda$lambda$readClass$0$2::load$($String* name, bool initialize) {
	...
	$loadClass(ClassReader$$Lambda$lambda$readClass$0$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ClassReader$$Lambda$lambda$readClass$0$2);
	});
	return class$;
}
...
$Class* ClassReader$$Lambda$lambda$lookupMethod$1$3::load$($String* name, bool initialize) {
	...
	$loadClass(ClassReader$$Lambda$lambda$lookupMethod$1$3, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ClassReader$$Lambda$lambda$lookupMethod$1$3);
	});
	return class$;
}
...
$Class* ClassReader$$Lambda$getClass$4::load$($String* name, bool initialize) {
	...
	$loadClass(ClassReader$$Lambda$getClass$4, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ClassReader$$Lambda$getClass$4);
	});
	return class$;
}
...
void ClassReader::init$($Context* context) {
	...
	$set(this, CLASS_OR_MEMBER_ATTRIBUTE, $EnumSet::of($ClassReader$AttributeKind::CLASS, $ClassReader$AttributeKind::MEMBER));
	...
	this->filling = false;
	$nc(context)->put(ClassReader::classReaderKey, this);
...
void ClassReader::enterMember($Symbol$ClassSymbol* c, $Symbol* sym) {
	if (($nc(sym)->flags_field & (0x1000 | (int64_t)0x80000000)) != 0x1000 || $nc(sym->name)->startsWith($nc(this->names)->lambda)) {
...
$ClassFinder$BadClassFile* ClassReader::badClassFile($String* key, $ObjectArray* args) {
	$useLocalObjectStack();
	$var($Symbol$TypeSymbol, var$0, $nc(this->currentOwner)->enclClass());
...
int32_t ClassReader::nextByte() {
	return $nc(this->buf)->getByte(this->bp++) & 0xff;
}
...
$Type* ClassReader::sigToType() {
	...
	case u'T':
		++this->sigp;
		start = this->sigp;
		while (this->signature->get(this->sigp) != u';') {
			++this->sigp;
		}
		++this->sigp;
		$init($Type);
		return this->sigEnterPhase ? $cast($Type, $Type::noType) : findTypeVar($($nc(this->names)->fromUtf(this->signature, start, this->sigp - 1 - start)));
	case u'+':
		{
			++this->sigp;
			...
			}
			{
				...
				for (; $nc(l)->nonEmpty(); $assign(l, l->tail)) {
					...
					if ($nc($cast($Type, l->head))->hasTag($TypeTag::TYPEVAR)) {
						$nc($nc(($cast($Type, l->head)))->tsym)->flags_field |= (int64_t)0x0000800000000000;
					}
				}
			}
	...
	default:
		$throw($(badClassFile("bad.signature"_s, $$new($ObjectArray, {$($Convert::utf2string(this->signature, this->sigp, 10))}))));
	}
}
...
$Type* ClassReader::classSigToType() {
	...
	if ($nc(this->signature)->get(this->sigp) != u'L') {
		$throw($(badClassFile("bad.class.signature"_s, $$new($ObjectArray, {$($Convert::utf2string(this->signature, this->sigp, 10))}))));
	}
	++this->sigp;
	$init($Type);
	...
	while (true) {
		int8_t c = $nc(this->signature)->get(this->sigp++);
		{
			...
			case u';':
				{
					...
					bool return$1 = false;
					try {
						$assign(var$2, ($equals(outer, $Type::noType)) ? $nc(t)->erasure(this->types) : $cast($Type, $new($Type$ClassType, outer, $($List::nil()), t)));
						return$1 = true;
						goto $finally1;
			...
			case u'<':
				...
				case u';':
					if (this->sigp < this->siglimit && this->signature->get(this->sigp) == u'.') {
				...
				case u'.':
					...
					break;
				default:
					$throwNew($AssertionError, (int32_t)this->signature->get(this->sigp - 1));
				}
				continue;
			case u'.':
...
$Type* ClassReader::sigToTypeParam() {
	...
	if ($nc(this->signature)->get(this->sigp) == u':' && this->signature->get(this->sigp + 1) == u':') {
		++this->sigp;
		allInterfaces = true;
	}
	while (this->signature->get(this->sigp) == u':') {
...
$Type* ClassReader::findTypeVar($Name* name) {
	...
	if (s != nullptr) {
		...
		$throw($(badClassFile("undecl.type.var"_s, $$new($ObjectArray, {name}))));
	}
}
...
void ClassReader::initAttributeReaders() {
	...
	$var($ClassReader$AttributeReaderArray, readers, $new($ClassReader$AttributeReaderArray, {
		$$new($ClassReader$2, this, $nc(this->names)->Code, $ClassFile$Version::V45_3, this->MEMBER_ATTRIBUTE),
		$$new($ClassReader$3, this, $nc(this->names)->ConstantValue, $ClassFile$Version::V45_3, this->MEMBER_ATTRIBUTE),
		$$new($ClassReader$4, this, $nc(this->names)->Deprecated, $ClassFile$Version::V45_3, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$5, this, $nc(this->names)->Exceptions, $ClassFile$Version::V45_3, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$6, this, $nc(this->names)->InnerClasses, $ClassFile$Version::V45_3, this->CLASS_ATTRIBUTE),
		$$new($ClassReader$7, this, $nc(this->names)->LocalVariableTable, $ClassFile$Version::V45_3, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$8, this, $nc(this->names)->SourceFile, $ClassFile$Version::V45_3, this->CLASS_ATTRIBUTE),
		$$new($ClassReader$9, this, $nc(this->names)->Synthetic, $ClassFile$Version::V45_3, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$10, this, $nc(this->names)->EnclosingMethod, $ClassFile$Version::V49, this->CLASS_ATTRIBUTE),
		$$new($ClassReader$11, this, $nc(this->names)->Signature, $ClassFile$Version::V49, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$12, this, $nc(this->names)->AnnotationDefault, $ClassFile$Version::V49, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$13, this, $nc(this->names)->RuntimeInvisibleAnnotations, $ClassFile$Version::V49, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$14, this, $nc(this->names)->RuntimeInvisibleParameterAnnotations, $ClassFile$Version::V49, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$15, this, $nc(this->names)->RuntimeVisibleAnnotations, $ClassFile$Version::V49, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$16, this, $nc(this->names)->RuntimeVisibleParameterAnnotations, $ClassFile$Version::V49, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$17, this, $nc(this->names)->Annotation, $ClassFile$Version::V49, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$18, this, $nc(this->names)->Bridge, $ClassFile$Version::V49, this->MEMBER_ATTRIBUTE),
		$$new($ClassReader$19, this, $nc(this->names)->Enum, $ClassFile$Version::V49, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$20, this, $nc(this->names)->Varargs, $ClassFile$Version::V49, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$21, this, $nc(this->names)->RuntimeVisibleTypeAnnotations, $ClassFile$Version::V52, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$22, this, $nc(this->names)->RuntimeInvisibleTypeAnnotations, $ClassFile$Version::V52, this->CLASS_OR_MEMBER_ATTRIBUTE),
		$$new($ClassReader$23, this, $nc(this->names)->MethodParameters, $ClassFile$Version::V52, this->MEMBER_ATTRIBUTE),
		$$new($ClassReader$24, this, $nc(this->names)->Module, $ClassFile$Version::V53, this->CLASS_ATTRIBUTE),
		$$new($ClassReader$25, this, $nc(this->names)->ModuleResolution, $ClassFile$Version::V53, this->CLASS_ATTRIBUTE),
		$$new($ClassReader$26, this, $nc(this->names)->Record, $ClassFile$Version::V58, this->CLASS_ATTRIBUTE),
		$$new($ClassReader$27, this, $nc(this->names)->PermittedSubclasses, $ClassFile$Version::V59, this->CLASS_ATTRIBUTE)
	}));
	{
...
void ClassReader::readEnclosingMethodAttr($Symbol* sym) {
	...
	$var($PoolConstant$NameAndType, nt, $cast($PoolConstant$NameAndType, optPoolEntry(var$0, $$new(ClassReader$$Lambda$getNameAndType, $nc(this->poolReader)), nullptr)));
	$init($Kinds$Kind);
	...
	$set(self, owner, m != nullptr ? $cast($Symbol, m) : $cast($Symbol, c));
...
$Name* ClassReader::simpleBinaryName($Name* self, $Name* enclosing) {
	...
	if (!$nc(self)->startsWith(enclosing)) {
		$throw($(badClassFile("bad.enclosing.method"_s, $$new($ObjectArray, {self}))));
	}
	$var($String, simpleBinaryName, $$nc(self->toString())->substring($$nc($nc(enclosing)->toString())->length()));
...
$Symbol$MethodSymbol* ClassReader::findMethod($PoolConstant$NameAndType* nt$renamed, $Scope* scope, int64_t flags) {
	...
	if ($$nc(nt->type->getParameterTypes())->isEmpty()) {
		return nullptr;
	}
	$var($List, var$0, $nc($(nt->type->getParameterTypes()))->tail);
	$var($Type, var$1, nt->type->getReturnType());
	$assign(nt, $new($PoolConstant$NameAndType, nt->name, $$new($Type$MethodType, var$0, var$1, $(nt->type->getThrownTypes()), $nc(this->syms)->methodClass)));
...
bool ClassReader::isSameBinaryType($Type$MethodType* mt1, $Type$MethodType* mt2) {
	...
	$var($List, types2, $$nc($nc(mt2)->getParameterTypes())->prepend($($nc(mt2)->getReturnType())));
	while (true) {
		...
		{
			if ($nc(($cast($Type, types1->head)))->tsym != $nc(($cast($Type, $nc(types2)->head)))->tsym) {
				return false;
			}
	...
	$var($ListBuffer, proxies, $new($ListBuffer));
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			if ($nc($nc($nc(proxy)->type)->tsym)->flatName() == $nc($nc($nc(this->syms)->proprietaryType)->tsym)->flatName()) {
				$nc(sym)->flags_field |= (int64_t)0x0000004000000000;
						...
						$var($Pair, v, $cast($Pair, i$->next()));
						{
							...
							if (var$0) {
								if ($equals($nc(constant)->type, $nc(this->syms)->intType) && $nc($cast($Integer, constant->value))->intValue() > this->profile->value) {
									$nc(sym)->flags_field |= (int64_t)0x0000200000000000;
								}
							}
						}
					}
				...
				$nc(sym)->flags_field |= (int64_t)0x0100000000000000;
				setFlagIfAttributeTrue(proxy, sym, $nc(this->names)->reflective, (int64_t)0x0400000000000000);
				...
				sym->flags_field |= (int64_t)0x0020000000000000;
			} else {
				if ($nc(proxy->type)->tsym == $nc($nc(this->syms)->annotationTargetType)->tsym) {
					$set(this, target, proxy);
				} else if (proxy->type->tsym == $nc(this->syms->repeatableType)->tsym) {
					$set(this, repeatable, proxy);
				} else if (proxy->type->tsym == $nc(this->syms->deprecatedType)->tsym) {
					$nc(sym)->flags_field |= (0x00020000 | (int64_t)0x0040000000000000);
					setFlagIfAttributeTrue(proxy, sym, $nc(this->names)->forRemoval, (int64_t)0x0080000000000000);
				} else if ($nc(proxy->type)->tsym == $nc($nc(this->syms)->previewFeatureType)->tsym) {
					$nc(sym)->flags_field |= (int64_t)0x0100000000000000;
					setFlagIfAttributeTrue(proxy, sym, $nc(this->names)->reflective, (int64_t)0x0400000000000000);
				} else {
					$init($Kinds$Kind);
					if (proxy->type->tsym == $nc(this->syms->valueBasedType)->tsym && $nc(sym)->kind == $Kinds$Kind::TYP) {
						sym->flags_field |= (int64_t)0x0020000000000000;
					}
				}
...
void ClassReader::readParameterAnnotations($Symbol* meth) {
	$useLocalObjectStack();
	int32_t numParameters = $nc(this->buf)->getByte(this->bp++) & 0xff;
	if (this->parameterAnnotations == nullptr) {
		...
		$throw($(badClassFile("bad.runtime.invisible.param.annotations"_s, $$new($ObjectArray, {meth}))));
	}
	for (int32_t pnum = 0; pnum < numParameters; ++pnum) {
		if ($nc(this->parameterAnnotations)->get(pnum) == nullptr) {
			this->parameterAnnotations->set(pnum, $$new($ClassReader$ParameterAnnotations));
		}
		$nc(this->parameterAnnotations->get(pnum))->add($(readAnnotations()));
	}
}
...
$TypeAnnotationPosition* ClassReader::readPosition() {
	...
	if (!$TargetType::isValidTargetTypeValue(tag)) {
		$throw($(badClassFile("bad.type.annotation.value"_s, $$new($ObjectArray, {$($String::format("0x%02X"_s, $$new($ObjectArray, {$($Integer::valueOf(tag))})))}))));
	}
...
$Object* ClassReader::optPoolEntry(int32_t index, $IntFunction* poolFunc, Object$* defaultValue) {
	return (index == 0) ? $of(defaultValue) : $nc(poolFunc)->apply(index);
}
...
$Symbol$MethodSymbol* ClassReader::readMethod() {
	...
	$var($Name, name, $nc(this->poolReader)->getName(nextChar()));
	$var($Type, type, $nc(this->poolReader)->getType(nextChar()));
	bool var$0 = $nc(this->currentOwner)->isInterface() && (flags & 0x0400) == 0;
	if (var$0 && !$nc(name)->equals($nc(this->names)->clinit)) {
		$init($ClassFile$Version);
		if (this->majorVersion > $ClassFile$Version::V52->major || (this->majorVersion == $ClassFile$Version::V52->major && this->minorVersion >= $ClassFile$Version::V52->minor)) {
			if ((flags & (8 | 2)) == 0) {
				$nc(this->currentOwner)->flags_field |= (int64_t)0x0000080000000000;
				flags |= (int64_t)0x0000080000000000 | 0x0400;
			}
		} else {
			$throw($(badClassFile((flags & 8) == 0 ? "invalid.default.interface"_s : "invalid.static.interface"_s, $$new($ObjectArray, {
				$($Integer::toString(this->majorVersion)),
	...
	if (name == $nc(this->names)->init && $nc(this->currentOwner)->hasOuterInstance()) {
		...
		if (!$nc($nc(this->currentOwner)->name)->isEmpty() && !local) {
			...
			$var($Type, var$2, type->getReturnType());
	...
	if ((flags & (int64_t)0x0000000400000000) != 0) {
		...
		$init($TypeTag);
		if (last == nullptr || !last->hasTag($TypeTag::ARRAY)) {
			m->flags_field &= (uint64_t)~(int64_t)0x0000000400000000;
			$throw($(badClassFile("malformed.vararg.method"_s, $$new($ObjectArray, {m}))));
		}
	}
	return m;
}
...
void ClassReader::validateMethodType($Name* name, $Type* t) {
	...
	if (var$0 || (name == $nc(this->names)->init && !$$nc(t->getReturnType())->hasTag($TypeTag::VOID))) {
		$throw($(badClassFile("method.descriptor.invalid"_s, $$new($ObjectArray, {name}))));
	}
}
...
$List* ClassReader::adjustMethodParams(int64_t flags, $List* args$renamed) {
	...
	bool isVarargs = (flags & (int64_t)0x0000000400000000) != 0;
	if (isVarargs) {
		$var($Type, varargsElem, $cast($Type, args->last()));
		$var($ListBuffer, adjustedArgs, $new($ListBuffer));
		{
			...
			for (; $nc(i$)->hasNext();) {
				...
				{
					adjustedArgs->append(t != varargsElem ? t : $$cast($Type, $nc($cast($Type$ArrayType, t))->makeVarargs()));
				}
			}
		}
...
void ClassReader::initParameterNames($Symbol$MethodSymbol* sym) {
	...
	if (this->parameterNameIndices == nullptr || this->parameterNameIndices->length < expectedParameterSlots) {
...
void ClassReader::setParameters($Symbol$MethodSymbol* sym, $Type* jvmType) {
	...
	if (!this->sawMethodParameters) {
		firstParam = (($nc(sym)->flags() & 8) == 0) ? 1 : 0;
		if (sym->name == $nc(this->names)->init && $nc(this->currentOwner)->hasOuterInstance()) {
	...
	int32_t annotationIndex = 0;
	{
		...
		for (; $nc(i$)->hasNext();) {
			$var($Type, t, $cast($Type, i$->next()));
			{
				...
				params->append(param);
				if (this->parameterAnnotations != nullptr) {
					$var($ClassReader$ParameterAnnotations, annotations, this->parameterAnnotations->get(annotationIndex));
					if (annotations != nullptr && annotations->proxies != nullptr && !annotations->proxies->isEmpty()) {
	...
	if (this->parameterAnnotations != nullptr && this->parameterAnnotations->length != annotationIndex) {
		$throw($(badClassFile("bad.runtime.invisible.param.annotations"_s, $$new($ObjectArray, {sym}))));
	}
...
$Symbol$VarSymbol* ClassReader::parameter(int32_t index, $Type* t, $Symbol$MethodSymbol* owner, $Set* exclude) {
	$useLocalObjectStack();
	int64_t flags = (int64_t)0x0000000200000000;
	$var($Name, argName, nullptr);
	if (this->parameterAccessFlags != nullptr && index < this->parameterAccessFlags->length && this->parameterAccessFlags->get(index) != 0) {
		flags |= this->parameterAccessFlags->get(index);
	}
	if (this->parameterNameIndices != nullptr && index < this->parameterNameIndices->length && this->parameterNameIndices->get(index) != 0) {
		$assign(argName, $cast($Name, optPoolEntry(this->parameterNameIndices->get(index), $$new(ClassReader$$Lambda$getName$1, $nc(this->poolReader)), $nc(this->names)->empty)));
		flags |= (int64_t)0x0010000000000000;
	} else {
...
void ClassReader::enterTypevars($Symbol* sym, $Type* t) {
	...
	{
		$var($List, xs, t->getTypeArguments());
		for (; $nc(xs)->nonEmpty(); $assign(xs, xs->tail)) {
...
void ClassReader::readClass($Symbol$ClassSymbol* c) {
	...
	if ((flags & (int64_t)0x0008000000000000) == 0) {
		$init($Kinds$Kind);
		if ($nc(c->owner)->kind == $Kinds$Kind::PCK || c->owner->kind == $Kinds$Kind::ERR) {
			c->flags_field = flags;
		}
		$set(this, currentModule, $nc($(c->packge()))->modle);
		$var($Symbol$ClassSymbol, self, $nc(this->poolReader)->getClass(nextChar()));
		if (c != self) {
			$throw($(badClassFile("class.file.wrong.class"_s, $$new($ObjectArray, {self->flatname}))));
		}
	} else {
		$init($ClassFile$Version);
	...
	readClassAttrs(c);
	if (c->permitted != nullptr && !c->permitted->isEmpty()) {
		c->flags_field |= (int64_t)0x4000000000000000;
	}
	this->bp = startbp;
	int32_t n = nextChar();
	if ((flags & (int64_t)0x0008000000000000) != 0 && n > 0) {
		$throw($(badClassFile("module.info.invalid.super.class"_s, $$new($ObjectArray, 0))));
	}
	if (ct->supertype_field == nullptr) {
		$set(ct, supertype_field, $cast($Type, optPoolEntry(n, $$new(ClassReader$$Lambda$lambda$readClass$0$2, this), $Type::noType)));
	}
	n = nextChar();
	...
	if (ct->interfaces_field == nullptr) {
	...
	if (c->isRecord()) {
		...
		for (; $nc(i$)->hasNext();) {
			...
			{
				$set($nc(rc), accessor, lookupMethod(c, $nc(rc)->name, $($List::nil())));
			}
		}
	}
	$set(this, typevars, $nc(this->typevars)->leave());
...
$Symbol$MethodSymbol* ClassReader::lookupMethod($Symbol$TypeSymbol* tsym, $Name* name, $List* argtypes) {
	$useLocalObjectStack();
	$var($Iterator, i$, $$nc($$nc($nc(tsym)->members())->getSymbolsByName(name, $$new(ClassReader$$Lambda$lambda$lookupMethod$1$3)))->iterator());
	for (; $nc(i$)->hasNext();) {
...
void ClassReader::readInnerClasses($Symbol$ClassSymbol* c) {
	...
	for (int32_t i = 0; i < n; ++i) {
		...
		$var($Symbol$ClassSymbol, outer, $cast($Symbol$ClassSymbol, optPoolEntry(outerIdx, $$new(ClassReader$$Lambda$getClass$4, $nc(this->poolReader)), nullptr)));
		$var($Name, name, $cast($Name, optPoolEntry(nameIdx, $$new(ClassReader$$Lambda$getName$1, $nc(this->poolReader)), $nc(this->names)->empty)));
		if (name == nullptr) {
			$assign(name, this->names->empty);
		}
		int64_t flags = adjustClassFlags(nextChar());
		if (outer != nullptr) {
			if (name == this->names->empty) {
				$assign(name, this->names->one);
			}
			...
			if ((flags & 8) == 0) {
				...
				if (member->erasure_field != nullptr) {
					$cast($Type$ClassType, member->erasure_field)->setEnclosingType($($nc(this->types)->erasure(outer->type)));
				}
			}
			if (c == outer) {
	...
	try {
		try {
			...
			if (var$1 && !$nc(this->foundTypeVariables)->isEmpty()) {
				...
				$set($nc(ct), supertype_field, $nc(this->types)->subst($nc(ct)->supertype_field, missing, found));
				...
				$set(ct, typarams_field, $nc(this->types)->substBounds(ct->typarams_field, missing, found));
				{
					...
					for (; $nc(types)->nonEmpty(); $assign(types, types->tail)) {
						$set($nc($nc(($cast($Type, types->head)))->tsym), type, $cast($Type, types->head));
					}
				}
			} else {
				bool var$2 = $nc(this->missingTypeVariables)->isEmpty();
				if (var$2 != $nc(this->foundTypeVariables)->isEmpty()) {
			...
			if (c == $nc(this->currentModule)->module_info) {
				bool var$3 = $nc(this->interimUses)->nonEmpty();
				if (var$3 || $nc(this->interimProvides)->nonEmpty()) {
			...
			$throw($(badClassFile("unable.to.access.file"_s, $$new($ObjectArray, {$(ex->toString())}))));
		} catch ($ClosedFileSystemException& ex) {
			$throw($(badClassFile("unable.to.access.file"_s, $$new($ObjectArray, {$(ex->toString())}))));
		} catch ($ArrayIndexOutOfBoundsException& ex) {
			$throw($(badClassFile("bad.class.file"_s, $$new($ObjectArray, {c->flatname}))));
		}
	} catch ($Throwable& var$4) {
		$assign(var$0, var$4);
	} /*finally*/ {
...
int64_t ClassReader::adjustMethodFlags(int64_t flags) {
	if ((flags & 0x40) != 0) {
		flags &= (uint64_t)~64;
		flags |= (int64_t)0x0000000080000000;
	}
	if ((flags & 0x80) != 0) {
		flags &= (uint64_t)~128;
		flags |= (int64_t)0x0000000400000000;
	}
	return flags;
}
...
int64_t ClassReader::adjustClassFlags(int64_t flags) {
	if ((flags & 0x8000) != 0) {
		flags &= (uint64_t)~0x00008000;
		flags |= (int64_t)0x0008000000000000;
	}
	return flags & ~0x20;
}
...
void ClassReader::clinit$($Class* clazz) {
...
$Class* ClassReader::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.jvm.ClassReader$$Lambda$getNameAndType")) {
			return ClassReader$$Lambda$getNameAndType::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.jvm.ClassReader$$Lambda$getName$1")) {
			return ClassReader$$Lambda$getName$1::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.jvm.ClassReader$$Lambda$lambda$readClass$0$2")) {
			return ClassReader$$Lambda$lambda$readClass$0$2::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.jvm.ClassReader$$Lambda$lambda$lookupMethod$1$3")) {
			return ClassReader$$Lambda$lambda$lookupMethod$1$3::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.jvm.ClassReader$$Lambda$getClass$4")) {
	...
	$loadClass(ClassReader, name, initialize, &classInfo$$, ClassReader::clinit$, []($Class* clazz) -> $Object* {
