...
void ListBuffer::copy() {
	if ($nc(this->elems)->nonEmpty()) {
		...
		while ($nc(($assign(orig, $nc(orig)->tail)))->nonEmpty()) {
			$set($nc(this->last$), tail, $List::of($nc(orig)->head));
...
$Object* ListBuffer::last() {
	return this->last$ != nullptr ? this->last$->head : ($Object*)nullptr;
}
...
$Class* ListBuffer::load$($String* name, bool initialize) {
	...
	$loadClass(ListBuffer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
