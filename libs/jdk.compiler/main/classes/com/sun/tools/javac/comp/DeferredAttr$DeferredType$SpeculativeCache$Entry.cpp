...
$Class* DeferredAttr$DeferredType$SpeculativeCache$Entry::load$($String* name, bool initialize) {
	...
	$loadClass(DeferredAttr$DeferredType$SpeculativeCache$Entry, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DeferredAttr$DeferredType$SpeculativeCache$Entry);
	});
	return class$;
}
