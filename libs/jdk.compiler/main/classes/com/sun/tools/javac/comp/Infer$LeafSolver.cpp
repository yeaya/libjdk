...
$Infer$GraphSolver$InferenceGraph$Node* Infer$LeafSolver::pickNode($Infer$GraphSolver$InferenceGraph* g) {
	...
	return $cast($Infer$GraphSolver$InferenceGraph$Node, g->nodes->get(0));
}
...
$Class* Infer$LeafSolver::load$($String* name, bool initialize) {
	...
	$loadClass(Infer$LeafSolver, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Infer$LeafSolver);
	});
	return class$;
}
