...
$EnumSet* Analyzer$AnalyzerMode::getAnalyzerModes($String* opt, $Source* source) {
	...
	}
	{
		...
		for (int32_t len$ = arr$->length, i$ = 0; i$ < len$; ++i$) {
			...
			{
				bool var$0 = modes->contains($$str({"-"_s, $nc(mode)->opt}));
				if (var$0 || !mode->feature->allowedInSource(source)) {
					$nc(res)->remove(mode);
...
void Analyzer$AnalyzerMode::clinit$($Class* clazz) {
	$init($Source$Feature);
...
$Class* Analyzer$AnalyzerMode::load$($String* name, bool initialize) {
	...
	$loadClass(Analyzer$AnalyzerMode, name, initialize, &classInfo$$, Analyzer$AnalyzerMode::clinit$, []($Class* clazz) -> $Object* {
