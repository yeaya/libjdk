...
$Class* JavacProcessingEnvironment$Round$$Lambda$lambda$new$1::load$($String* name, bool initialize) {
	...
	$loadClass(JavacProcessingEnvironment$Round$$Lambda$lambda$new$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JavacProcessingEnvironment$Round$$Lambda$lambda$new$1);
	});
	return class$;
}
...
$Class* JavacProcessingEnvironment$Round$$Lambda$lambda$new$2$1::load$($String* name, bool initialize) {
	...
	$loadClass(JavacProcessingEnvironment$Round$$Lambda$lambda$new$2$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JavacProcessingEnvironment$Round$$Lambda$lambda$new$2$1);
	});
	return class$;
}
...
$Class* JavacProcessingEnvironment$Round$$Lambda$lambda$new$0$2::load$($String* name, bool initialize) {
	...
	$loadClass(JavacProcessingEnvironment$Round$$Lambda$lambda$new$0$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JavacProcessingEnvironment$Round$$Lambda$lambda$new$0$2);
	});
	return class$;
}
...
void JavacProcessingEnvironment$Round::init$($JavacProcessingEnvironment* this$0, int32_t number, $Set* treesToClean, $Log$DeferredDiagnosticHandler* deferredDiagnosticHandler) {
	...
	$set(this, ACCEPT_NON_RECOVERABLE, $new(JavacProcessingEnvironment$Round$$Lambda$lambda$new$1));
	$set(this, ACCEPT_ALL, $new(JavacProcessingEnvironment$Round$$Lambda$lambda$new$2$1));
	this->number = number;
	if (number == 1) {
...
void JavacProcessingEnvironment$Round::init$($JavacProcessingEnvironment* this$0, JavacProcessingEnvironment$Round* prev, $Set* newSourceFiles, $Map* newClassFiles) {
	$useLocalObjectStack();
	JavacProcessingEnvironment$Round::init$(this$0, $nc(prev)->number + 1, $nc(prev)->treesToClean, nullptr);
	prev->newRound();
	...
	$set(this, roots, this$0->compiler->initModules(this->roots));
	...
	$var($List, newClasses, enterClassFiles(newClassFiles));
	{
		...
		for (; $nc(i$)->hasNext();) {
			$var($Map$Entry, moduleAndClassFiles, $cast($Map$Entry, i$->next()));
			{
				...
				$$sure($Map, $nc(this->genClassFiles)->computeIfAbsent(var$0, $$new(JavacProcessingEnvironment$Round$$Lambda$lambda$new$0$2)))->putAll($$cast($Map, moduleAndClassFiles->getValue()));
			}
		}
	}
...
void JavacProcessingEnvironment$Round::findAnnotationsPresent() {
	...
	$set(this, annotationsPresent, $new($LinkedHashSet));
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			annotationComputer->scan($cast($Symbol$TypeSymbol, classSym), this->annotationsPresent);
		}
	}
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			annotationComputer->scan($cast($Symbol$TypeSymbol, pkgSym), this->annotationsPresent);
		}
	}
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			annotationComputer->scan($cast($Symbol$TypeSymbol, mdlSym), this->annotationsPresent);
		}
	}
}
...
$List* JavacProcessingEnvironment$Round::enterClassFiles($Map* modulesAndClassFiles) {
	...
	$var($List, list, $List::nil());
	{
		...
		for (; $nc(i$)->hasNext();) {
			$var($Map$Entry, moduleAndClassFiles, $cast($Map$Entry, i$->next()));
			{
				$var($Iterator, i$, $$nc($$sure($Map, $nc(moduleAndClassFiles)->getValue())->entrySet())->iterator());
				for (; $nc(i$)->hasNext();) {
					$var($Map$Entry, entry, $cast($Map$Entry, i$->next()));
					{
						$var($Name, name, $nc(this->this$0->names)->fromString($$cast($String, $nc(entry)->getKey())));
						$var($JavaFileObject, file, $cast($JavaFileObject, entry->getValue()));
						$init($JavaFileObject$Kind);
						if ($nc(file)->getKind() != $JavaFileObject$Kind::CLASS) {
							$throwNew($AssertionError, file);
						}
						...
						if (this->this$0->isPkgInfo(file, $JavaFileObject$Kind::CLASS)) {
							...
							if ($nc(p)->package_info == nullptr) {
								...
								$set(p, package_info, this->this$0->symtab->enterClass(var$0, $($Convert::shortName(name)), p));
							}
							$assign(cs, p->package_info);
							$nc(cs)->reset();
	...
	try {
		try {
			if (lastRound) {
				...
				$$sure($JavacProcessingEnvironment$DiscoveredProcessors$ProcessorStateIterator, $nc(this->this$0->discoveredProcs)->iterator())->runContributingProcs(renv);
			} else {
...
void JavacProcessingEnvironment$Round::printRoundInfo(bool lastRound) {
	...
	if (this->this$0->printRounds || this->this$0->verbose) {
		...
		$nc(this->this$0->log)->printLines("x.print.rounds"_s, $$new($ObjectArray, {
			...
			$$str({"{"_s, $($nc(tlc)->toString(", "_s)), "}"_s}),
			ap,
			$($Boolean::valueOf(lastRound))
		}));
	}
}
...
void JavacProcessingEnvironment$Round::newRound() {
	...
	}
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			{
				$nc(this->this$0->treeCleaner)->scan(node);
			}
		}
	}
	$nc(this->this$0->chk)->newRound();
	this->this$0->enter->newRound();
	...
	if (foundError) {
		$var($Iterator, i$, $$nc(this->this$0->symtab->getAllClasses())->iterator());
		for (; $nc(i$)->hasNext();) {
			$var($Symbol$ClassSymbol, cs, $cast($Symbol$ClassSymbol, i$->next()));
			$init($Kinds$Kind);
			if ($nc(cs)->classfile != nullptr || cs->kind == $Kinds$Kind::ERR) {
...
bool JavacProcessingEnvironment$Round::lambda$new$1($JCDiagnostic* d) {
	...
	$init($JCDiagnostic$DiagnosticFlag);
	bool var$0 = var$1 || !d->isFlagSet($JCDiagnostic$DiagnosticFlag::RECOVERABLE);
	return var$0 || d->isFlagSet($JCDiagnostic$DiagnosticFlag::API);
}
...
$Class* JavacProcessingEnvironment$Round::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.processing.JavacProcessingEnvironment$Round$$Lambda$lambda$new$1")) {
			return JavacProcessingEnvironment$Round$$Lambda$lambda$new$1::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.processing.JavacProcessingEnvironment$Round$$Lambda$lambda$new$2$1")) {
			return JavacProcessingEnvironment$Round$$Lambda$lambda$new$2$1::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.processing.JavacProcessingEnvironment$Round$$Lambda$lambda$new$0$2")) {
	...
	$loadClass(JavacProcessingEnvironment$Round, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JavacProcessingEnvironment$Round);
	});
	return class$;
}
