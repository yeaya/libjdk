...
$Class* JavaCompiler$$Lambda$lambda$new$0::load$($String* name, bool initialize) {
	...
	$loadClass(JavaCompiler$$Lambda$lambda$new$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JavaCompiler$$Lambda$lambda$new$0);
	});
	return class$;
}
...
$Class* JavaCompiler$$Lambda$findPackageInFile$1::load$($String* name, bool initialize) {
	...
	$loadClass(JavaCompiler$$Lambda$findPackageInFile$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JavaCompiler$$Lambda$findPackageInFile$1);
	});
	return class$;
}
...
$Class* JavaCompiler$$Lambda$readModuleName$2::load$($String* name, bool initialize) {
	...
	$loadClass(JavaCompiler$$Lambda$readModuleName$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JavaCompiler$$Lambda$readModuleName$2);
	});
	return class$;
}
...
$Class* JavaCompiler$$Lambda$lambda$readSourceFile$1$3::load$($String* name, bool initialize) {
	...
	$loadClass(JavaCompiler$$Lambda$lambda$readSourceFile$1$3, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JavaCompiler$$Lambda$lambda$readSourceFile$1$3);
	});
	return class$;
}
...
$Class* JavaCompiler$$Lambda$lambda$readSourceFile$2$4::load$($String* name, bool initialize) {
	...
	$loadClass(JavaCompiler$$Lambda$lambda$readSourceFile$2$4, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JavaCompiler$$Lambda$lambda$readSourceFile$2$4);
	});
	return class$;
}
...
$Class* JavaCompiler$$Lambda$lambda$readModuleName$3$5::load$($String* name, bool initialize) {
	...
	$loadClass(JavaCompiler$$Lambda$lambda$readModuleName$3$5, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JavaCompiler$$Lambda$lambda$readModuleName$3$5);
	});
	return class$;
}
...
$Class* JavaCompiler$$Lambda$lambda$findPackageInFile$4$6::load$($String* name, bool initialize) {
	...
	$loadClass(JavaCompiler$$Lambda$lambda$findPackageInFile$4$6, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JavaCompiler$$Lambda$lambda$findPackageInFile$4$6);
	});
	return class$;
}
...
void JavaCompiler::init$($Context* context) {
	$useLocalObjectStack();
	$set(this, sourceCompleter, $new(JavaCompiler$$Lambda$lambda$new$0, this));
	...
	$nc(context)->put(JavaCompiler::compilerKey, this);
	...
	$set($nc(this->modules), findPackageInFile, $new(JavaCompiler$$Lambda$findPackageInFile$1, this));
	$set($nc(this->moduleFinder), moduleNameFromSourceReader, $new(JavaCompiler$$Lambda$readModuleName$2, this));
	...
	this->lineDebugInfo = var$0 || this->options->isSet($Option::G_CUSTOM, "lines"_s);
	...
	$set(this, completionFailureName, $nc(this->options)->isSet("failcomplete"_s) ? $nc(this->names)->fromString($(this->options->get("failcomplete"_s))) : ($Name*)nullptr);
	$init($CompileStates$CompileState);
	$set(this, shouldStopPolicyIfError, $nc(this->options)->isSet("should-stop.at"_s) ? $CompileStates$CompileState::valueOf($(this->options->get("should-stop.at"_s))) : this->options->isSet("should-stop.ifError"_s) ? $CompileStates$CompileState::valueOf($(this->options->get("should-stop.ifError"_s))) : $CompileStates$CompileState::INIT);
	$set(this, shouldStopPolicyIfNoError, $nc(this->options)->isSet("should-stop.ifNoError"_s) ? $CompileStates$CompileState::valueOf($(this->options->get("should-stop.ifNoError"_s))) : $CompileStates$CompileState::GENERATE);
...
int32_t JavaCompiler::errorCount() {
	if (this->werror && $nc(this->log)->nerrors == 0 && this->log->nwarnings > 0) {
...
$Queue* JavaCompiler::stopIfError($CompileStates$CompileState* cs, $Queue* queue) {
	return shouldStop(cs) ? $cast($Queue, $new($ListBuffer)) : queue;
}
...
$JCTree$JCCompilationUnit* JavaCompiler::parse($JavaFileObject* filename, $CharSequence* content) {
	...
	if (content != nullptr) {
		if (this->verbose) {
			$nc(this->log)->printVerbose("parsing.started"_s, $$new($ObjectArray, {filename}));
		}
		...
		bool var$0 = keepComments();
		bool var$1 = this->genEndPos;
		bool var$2 = this->lineDebugInfo;
		$init($JavaFileObject$Kind);
		$var($Parser, parser, $nc(this->parserFactory)->newParser(content, var$0, var$1, var$2, $nc(filename)->isNameCompatible("module-info"_s, $JavaFileObject$Kind::SOURCE)));
		$assign(tree, $nc(parser)->parseCompilationUnit());
		if (this->verbose) {
			$nc(this->log)->printVerbose("parsing.done"_s, $$new($ObjectArray, {$($Long::toString(elapsed(msec)))}));
		}
	}
	$set($nc(tree), sourcefile, filename);
	if (content != nullptr && !$nc(this->taskListener)->isEmpty()) {
		$init($TaskEvent$Kind);
		$var($TaskEvent, e, $new($TaskEvent, $TaskEvent$Kind::PARSE, tree));
...
$JCTree$JCCompilationUnit* JavaCompiler::parse($String* filename) {
	...
	return parse($$cast($JavaFileObject, $$nc($$nc($nc(fm)->getJavaFileObjectsFromStrings($$cast($AbstractCollection, $List::of(filename))))->iterator())->next()));
}
...
$Symbol* JavaCompiler::resolveBinaryNameOrIdent($Symbol$ModuleSymbol* msym, $String* name) {
	$useLocalObjectStack();
	try {
		$var($Name, flatname, $nc(this->names)->fromString($($nc(name)->replace("/"_s, "."_s))));
...
$Symbol* JavaCompiler::resolveIdent($Symbol$ModuleSymbol* msym, $String* name) {
	...
	try {
		...
		{
			$var($StringArray, arr$, name->split("\\."_s, -1));
			for (int32_t len$ = arr$->length, i$ = 0; i$ < len$; ++i$) {
				$var($String, s, arr$->get(i$));
				{
					...
					$assign(tree, (tree == nullptr) ? $cast($JCTree$JCExpression, $nc(this->make)->Ident($($nc(this->names)->fromString(s)))) : $cast($JCTree$JCExpression, $nc(this->make)->Select(tree, $($nc(this->names)->fromString(s)))));
				}
			}
		}
		...
		$assign(var$2, $nc(this->attr)->attribIdent(tree, toplevel));
		return$1 = true;
		goto $finally;
	} catch ($Throwable& var$3) {
		$assign(var$0, var$3);
...
$JavaFileObject* JavaCompiler::printSource($Env* env, $JCTree$JCClassDecl* cdef) {
	...
	if ($nc(this->inputFiles)->contains(outFile)) {
		$var($JCDiagnostic$DiagnosticPosition, var$0, cdef->pos());
...
void JavaCompiler::readSourceFile($JCTree$JCCompilationUnit* tree$renamed, $Symbol$ClassSymbol* c) {
	...
	if (this->completionFailureName == $nc(c)->fullname) {
		$throwNew($Symbol$CompletionFailure, c, $$new(JavaCompiler$$Lambda$lambda$readSourceFile$1$3, this), this->dcfh);
	}
	$var($JavaFileObject, filename, c->classfile);
	$var($JavaFileObject, prev, $nc(this->log)->useSource(filename));
	if (tree == nullptr) {
		$var($Throwable, var$0, nullptr);
		try {
	...
	if (!$nc(this->taskListener)->isEmpty()) {
		$init($TaskEvent$Kind);
		$var($TaskEvent, e, $new($TaskEvent, $TaskEvent$Kind::ENTER, tree));
		$nc(this->taskListener)->started(e);
	}
	if (!$nc(this->modules)->enter($($List::of(tree)), c)) {
		$throwNew($Symbol$CompletionFailure, c, $$new(JavaCompiler$$Lambda$lambda$readSourceFile$2$4, this), this->dcfh);
	}
...
void JavaCompiler::compile($List* sourceFileObject) {
	$useLocalObjectStack();
	$var($Collection, var$0, $cast($AbstractCollection, $List::nil()));
	compile($cast($AbstractCollection, sourceFileObject), var$0, nullptr, $$cast($AbstractCollection, $List::nil()));
}
...
void JavaCompiler::compile($Collection* sourceFileObjects, $Collection* classnames, $Iterable* processors, $Collection* addModules) {
	...
	try {
		try {
			initProcessAnnotations(processors, sourceFileObjects, classnames);
			{
				...
				for (; $nc(i$)->hasNext();) {
					...
					{
						int32_t sep = $nc(className)->indexOf(u'/');
						if (sep != -1) {
			...
			if (!$CompileStates$CompileState::ATTR->isAfter(this->shouldStopPolicyIfNoError)) {
				...
				case 3:
					while (!$nc(this->todo)->isEmpty()) {
						generate($(desugar($(flow($(attribute($$cast($Env, this->todo->remove()))))))));
					}
					break;
				default:
		...
		if (this->verbose) {
			...
			$nc(this->log)->printVerbose("total"_s, $$new($ObjectArray, {$($Long::toString(this->elapsed_msec))}));
		}
...
$List* JavaCompiler::enterTrees($List* roots) {
	...
	if (!$nc(this->taskListener)->isEmpty()) {
		...
		for (; $nc(i$)->hasNext();) {
			...
			{
				$init($TaskEvent$Kind);
				$var($TaskEvent, e, $new($TaskEvent, $TaskEvent$Kind::ENTER, unit));
				$nc(this->taskListener)->started(e);
			}
		}
	}
...
void JavaCompiler::initProcessAnnotations($Iterable* processors, $Collection* initialFiles, $Collection* initialClassNames) {
	...
	if ($nc(this->options)->isSet($Option::PROC, "none"_s)) {
		...
		if (this->processAnnotations$) {
			...
			$$sure($JavacFiler, $nc(this->procEnvImpl)->getFiler())->setInitialState(initialFiles, initialClassNames);
		} else {
...
void JavaCompiler::processAnnotations($List* roots) {
	processAnnotations(roots, $$cast($AbstractCollection, $List::nil()));
}
...
void JavaCompiler::processAnnotations($List* roots, $Collection* classnames) {
	...
	try {
				...
				bool errors = false;
				{
					...
					for (; $nc(i$)->hasNext();) {
						$var($String, nameStr, $cast($String, i$->next()));
						{
							...
							$init($Kinds$Kind);
							if (sym == nullptr || (sym->kind == $Kinds$Kind::PCK && !this->processPcks) || sym->kind == $Kinds$Kind::ABSENT_TYP) {
		...
		$var($Throwable, var$0, nullptr);
		try {
			this->annotationProcessingOccurred = $nc(this->procEnvImpl)->doProcessing(roots, classSymbols, $cast($AbstractCollection, pckSymbols), this->deferredDiagnosticHandler);
		} catch ($Throwable& var$1) {
			$assign(var$0, var$1);
		} /*finally*/ {
			$nc(this->procEnvImpl)->close();
...
bool JavaCompiler::unrecoverableError() {
	$useLocalObjectStack();
	if (this->deferredDiagnosticHandler != nullptr) {
		$var($Iterator, i$, $$nc(this->deferredDiagnosticHandler->getDiagnostics())->iterator());
		for (; $nc(i$)->hasNext();) {
...
bool JavaCompiler::explicitAnnotationProcessingRequested($Options* options) {
	...
	bool var$2 = var$3 || options->isSet($Option::PROCESSOR_PATH);
	bool var$1 = var$2 || options->isSet($Option::PROCESSOR_MODULE_PATH);
	bool var$0 = var$1 || options->isSet($Option::PROC, "only"_s);
	return var$0 || options->isSet($Option::XPRINT);
}
...
$Env* JavaCompiler::attribute($Env* env) {
	...
	if (this->verbose) {
		$nc(this->log)->printVerbose("checking.attribution"_s, $$new($ObjectArray, {$nc($nc(env)->enclClass)->sym}));
	}
	if (!$nc(this->taskListener)->isEmpty()) {
		$init($TaskEvent$Kind);
		$var($TaskEvent, e, $new($TaskEvent, $TaskEvent$Kind::ANALYZE, $nc(env)->toplevel, $as($TypeElement, $nc($nc(env)->enclClass)->sym)));
		$nc(this->taskListener)->started(e);
	}
	$var($JavaFileObject, prev, $nc(this->log)->useSource($nc($nc($nc(env)->enclClass)->sym)->sourcefile != nullptr ? env->enclClass->sym->sourcefile : $nc(env->toplevel)->sourcefile));
	$var($Throwable, var$0, nullptr);
	try {
		$nc(this->attr)->attrib(env);
		bool var$1 = errorCount() > 0;
		if (var$1 && !shouldStop($CompileStates$CompileState::ATTR)) {
			$nc(this->attr)->postAttr(env->tree);
		}
...
void JavaCompiler::desugar($Env* env, $Queue* results) {
	...
	if (!$nc(this->modules)->multiModuleMode && $nc($nc(env)->toplevel)->modle != this->modules->getDefaultModule()) {
		return;
	}
	...
	if (this->verboseCompilePolicy) {
		printNote($$str({"[desugar "_s, $nc(env->enclClass)->sym, "]"_s}));
	}
	$var($JavaFileObject, prev, $nc(this->log)->useSource($nc($nc(env->enclClass)->sym)->sourcefile != nullptr ? env->enclClass->sym->sourcefile : $nc(env->toplevel)->sourcefile));
	$var($Throwable, var$0, nullptr);
	bool return$1 = false;
	try {
		$var($JCTree, untranslated, env->tree);
		$nc(this->make)->at($Position::FIRSTPOS);
		$var($TreeMaker, localMake, $nc(this->make)->forToplevel(env->toplevel));
		$init($JCTree$Tag);
		bool var$2 = $nc(env->tree)->hasTag($JCTree$Tag::PACKAGEDEF);
		if (var$2 || env->tree->hasTag($JCTree$Tag::MODULEDEF)) {
			if (!(this->sourceOutput)) {
	...
	}
	{
		...
		for (; $nc(i$)->hasNext();) {
			$var($Pair, x, $cast($Pair, i$->next()));
			{
					...
					try {
						...
						if (this->sourceOutput) {
							...
							if (var$2 && $nc(this->jniWriter)->needsHeader($nc(cdef)->sym)) {
								$nc(this->jniWriter)->write(cdef->sym);
							}
				...
				if (!$nc(this->taskListener)->isEmpty()) {
					$init($TaskEvent$Kind);
					$var($TaskEvent, e, $new($TaskEvent, $TaskEvent$Kind::GENERATE, env->toplevel, $as($TypeElement, $nc(cdef)->sym)));
					$nc(this->taskListener)->finished(e);
				}
			}
		}
...
$JCTree$JCClassDecl* JavaCompiler::removeMethodBodies($JCTree$JCClassDecl* cdef) {
	bool isInterface = ($nc($nc(cdef)->mods)->flags & $Flags::INTERFACE) != 0;
	{
	}
	$var($JavaCompiler$1MethodBodyRemover, r, $new($JavaCompiler$1MethodBodyRemover, this, isInterface));
	return $cast($JCTree$JCClassDecl, r->translate(cdef));
}
...
void JavaCompiler::reportDeferredDiagnostics() {
	...
	if ($nc(this->log)->compressedOutput) {
		...
		this->log->mandatoryNote(nullptr, $CompilerProperties$Notes::CompressedDiags);
	}
}
...
$Name* JavaCompiler::readModuleName($JavaFileObject* fo) {
	return parseAndGetName(fo, $$new(JavaCompiler$$Lambda$lambda$readModuleName$3$5));
}

$Name* JavaCompiler::findPackageInFile($JavaFileObject* fo) {
	return parseAndGetName(fo, $$new(JavaCompiler$$Lambda$lambda$findPackageInFile$4$6));
}
...
void JavaCompiler::printCount($String* kind, int32_t count) {
	...
	if (count != 0) {
		...
		$init($Log$WriterKind);
		$nc(this->log)->printLines($Log$WriterKind::ERROR, key, $$new($ObjectArray, {$($String::valueOf(count))}));
...
$Name* JavaCompiler::lambda$findPackageInFile$4($JCTree$JCCompilationUnit* t) {
	$init(JavaCompiler);
	$useLocalObjectStack();
	return $cast($JCTree$JCPackageDecl, $nc(t)->getPackage()) != nullptr ? $TreeInfo::fullName($$cast($JCTree, $$sure($JCTree$JCPackageDecl, t->getPackage())->getPackageName())) : ($Name*)nullptr;
}
...
$Name* JavaCompiler::lambda$readModuleName$3($JCTree$JCCompilationUnit* t) {
	...
	return md != nullptr ? $TreeInfo::fullName($$cast($JCTree, md->getName())) : ($Name*)nullptr;
}
...
void JavaCompiler::clinit$($Class* clazz) {
...
$Class* JavaCompiler::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.main.JavaCompiler$$Lambda$lambda$new$0")) {
			return JavaCompiler$$Lambda$lambda$new$0::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.main.JavaCompiler$$Lambda$findPackageInFile$1")) {
			return JavaCompiler$$Lambda$findPackageInFile$1::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.main.JavaCompiler$$Lambda$readModuleName$2")) {
			return JavaCompiler$$Lambda$readModuleName$2::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.main.JavaCompiler$$Lambda$lambda$readSourceFile$1$3")) {
			return JavaCompiler$$Lambda$lambda$readSourceFile$1$3::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.main.JavaCompiler$$Lambda$lambda$readSourceFile$2$4")) {
			return JavaCompiler$$Lambda$lambda$readSourceFile$2$4::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.main.JavaCompiler$$Lambda$lambda$readModuleName$3$5")) {
			return JavaCompiler$$Lambda$lambda$readModuleName$3$5::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.main.JavaCompiler$$Lambda$lambda$findPackageInFile$4$6")) {
	...
	$CompoundAttribute parsemethodAnnotations$$$1[] = {
	...
	$MethodInfo methodInfos$$[] = {
		...
		{"parse", "(Ljava/lang/String;)Lcom/sun/tools/javac/tree/JCTree$JCCompilationUnit;", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(JavaCompiler, parse, $JCTree$JCCompilationUnit*, $String*), nullptr, nullptr, parsemethodAnnotations$$$1},
	...
	$loadClass(JavaCompiler, name, initialize, &classInfo$$, JavaCompiler::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(JavaCompiler);
	});
	return class$;
}
