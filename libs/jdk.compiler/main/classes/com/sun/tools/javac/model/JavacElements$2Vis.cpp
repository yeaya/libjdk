...
void JavacElements$2Vis::visitCompound($Attribute$Compound* anno) {
	...
	for (; $nc(i$)->hasNext();) {
		$var($Pair, pair, $cast($Pair, i$->next()));
		{
			...
			if (expr != nullptr) {
				$var($JCTree, match, this->this$0->matchAttributeToTree(this->val$findme, $cast($Attribute, pair->snd), expr));
				if (match != nullptr) {
...
void JavacElements$2Vis::visitArray($Attribute$Array* array) {
	...
	if ($nc(this->val$tree)->hasTag($JCTree$Tag::NEWARRAY)) {
		$var($List, elems, $cast($JCTree$JCNewArray, this->val$tree)->elems);
		{
			...
			for (int32_t len$ = $nc(arr$)->length, i$ = 0; i$ < len$; ++i$) {
				$var($Attribute, value, arr$->get(i$));
				{
					...
					$assign(elems, elems->tail);
				}
			}
		}
	} else if ($nc($nc(array)->values)->length == 1) {
		$set(this, result, this->this$0->matchAttributeToTree(this->val$findme, array->values->get(0), this->val$tree));
	}
}
...
$Class* JavacElements$2Vis::load$($String* name, bool initialize) {
	...
	$loadClass(JavacElements$2Vis, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JavacElements$2Vis);
	});
	return class$;
}
