...
$ObjectArray2* javac_zh_CN::getContents() {
	...
	return $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			"javac.msg.bug"_s,
			u"编译器 ({0}) 中出现异常错误。如果在 Bug Database (http://bugs.java.com) 中没有找到该错误，请通过 Java Bug 报告页 (http://bugreport.java.com) 建立该 Java 编译器 Bug。请在报告中附上您的程序、以下诊断信息以及传递到 Java 编译器的参数。谢谢。"_s
		}),
		$$new($ObjectArray, {
			"javac.msg.io"_s,
			u"\n\n发生输入/输出错误。\n有关详细信息, 请参阅以下堆栈跟踪。\n"_s
		...
		$$new($ObjectArray, {
			...
			u"正在将 javac 参数打印到：{0}"_s
		}),
		$$new($ObjectArray, {
			"javac.msg.parameters.output.error"_s,
			u"尝试将 javac 参数打印到 {0} 时出错，参数将采用："_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"用法: {0} <options> <source files>\n其中, 可能的选项包括:"_s
		}),
		$$new($ObjectArray, {
			"javac.msg.usage.nonstandard.footer"_s,
			u"这些额外选项如有更改, 恕不另行通知。"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.A"_s,
			u"传递给注释处理程序的选项"_s
		...
		$$new($ObjectArray, {
			...
			u"有关辅助类在源文件中隐藏, 但在其他文件中使用的警告。"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"有关与类文件内容相关的问题的警告。"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.Xlint.desc.dep-ann"_s,
			u"有关项在 JavaDoc 中标记为已过时但未使用 @Deprecated 注释的警告。"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"有关 if 之后没有语句的警告。"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.Xlint.desc.exports"_s,
			u"有关与模块导出相关的问题的警告。"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.Xlint.desc.fallthrough"_s,
			u"有关从 switch 语句的一个 case 向下顺序执行到下一个 case 的警告。"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.Xlint.desc.finally"_s,
			u"有关 finally 子句未正常终止的警告。"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.Xlint.desc.missing-explicit-ctor"_s,
			u"有关导出的程序包中的公共和受保护类中缺少显式构造器的警告。"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"由注释处理程序创建的文件的备用目标模块 (如果未指定或推断任何模块)。"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.deprecation"_s,
			u"输出使用已过时的 API 的源位置"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.diags"_s,
			u"选择诊断模式"_s
		...
		$$new($ObjectArray, {
			...
			u"指定是否为隐式引用文件生成类文件"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.inherit_runtime_environment"_s,
			u"从运行时环境继承模块系统配置选项。"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"生成元数据以用于方法参数的反射"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.patch"_s,
			u"使用 JAR 文件或目录中的类和资源覆盖\n        或增强模块"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.pkginfo"_s,
			u"指定 package-info 文件的处理"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.plugin"_s,
			u"要运行的插件的名称和可选参数"_s
		...
		$$new($ObjectArray, {
			"javac.opt.prefer"_s,
			u"指定读取文件, 当同时找到隐式编译类的源文件和类文件时"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.preview"_s,
			u"启用预览语言功能。要与 -source 或 --release 一起使用。"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.print"_s,
			u"输出指定类型的文本表示"_s
		...
		$$new($ObjectArray, {
			...
			u"输出有关搜索类文件的位置的信息"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.proc.none.only"_s,
			u"控制是否执行注释处理和/或编译。"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.processor"_s,
			u"要运行的注释处理程序的名称; 绕过默认的搜索进程"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.processormodulepath"_s,
			u"指定查找注释处理程序的模块路径"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"为指定的 Java SE 发行版编译。支持的发行版：{0}"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.s"_s,
			u"发出 java 源而不是类文件"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.source"_s,
			u"提供与指定的 Java SE 发行版的源兼容性。支持的发行版：{0}"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			"javac.opt.target"_s,
			u"生成适合指定的 Java SE 发行版的类文件。支持的发行版：{0}"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"在引导类路径之前而不是之后搜索类的类路径和源路径"_s
		}),
		$$new($ObjectArray, {
			"javac.opt.verbose"_s,
			u"输出有关编译器正在执行的操作的消息"_s
		}),
		$$new($ObjectArray, {
...
$Class* javac_zh_CN::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PUBLIC, $method(javac_zh_CN, init$, void)},
		{"getContents", "()[[Ljava/lang/Object;", nullptr, $PROTECTED | $FINAL, $virtualMethod(javac_zh_CN, getContents, $ObjectArray2*)},
		{}
	};
	$ClassInfo classInfo$$ = {
	...
	$loadClass(javac_zh_CN, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(javac_zh_CN);
	});
	return class$;
}
