...
$JCTree$JCLambda* LambdaToMethod$MemberReferenceToLambda::lambda() {
	...
	try {
		...
		$var($JCTree$JCLambda, slam, $nc(this->this$0->make)->Lambda($(this->params->toList()), expr));
		$set($nc(slam), target, this->tree->target);
		$set(slam, type, this->tree->type);
...
$Symbol$VarSymbol* LambdaToMethod$MemberReferenceToLambda::addParametersReturnReceiver() {
	...
	switch ($nc($LambdaToMethod$1::$SwitchMap$com$sun$tools$javac$tree$JCTree$JCMemberReference$ReferenceKind)->get($nc((this->tree->kind))->ordinal())) {
	case 3:
		$assign(rcvr, addParameter("rec$"_s, $nc($$cast($JCTree$JCExpression, this->tree->getQualifierExpression()))->type, false));
		$set(this, receiverExpression, $nc(this->this$0->attr)->makeNullCheck($$cast($JCTree$JCExpression, this->tree->getQualifierExpression())));
		break;
	case 4:
	...
	$var($List, implPTypes, $nc($nc(this->tree->sym)->type)->getParameterTypes());
	int32_t implSize = $nc(implPTypes)->size();
	int32_t samSize = $nc(samPTypes)->size();
	int32_t last = this->localContext->needsVarArgsConversion() ? implSize - 1 : implSize;
	bool checkForIntersection = this->tree->varargsElement != nullptr || implSize == $nc(descPTypes)->size();
	for (int32_t i = 0; $nc(implPTypes)->nonEmpty() && i < last; ++i) {
		$var($Type, parmType, $cast($Type, implPTypes->head));
		if (checkForIntersection) {
			$init($TypeKind);
			if ($nc($cast($Type, $nc(descPTypes)->head))->getKind() == $TypeKind::INTERSECTION) {
				$assign(parmType, $cast($Type, $nc(samPTypes)->head));
			}
	...
	for (int32_t i = last; i < samSize; ++i) {
		addParameter($$str({"xva$"_s, $$str(i)}), this->tree->varargsElement, true);
	}
	return rcvr;
}
...
$JCTree$JCExpression* LambdaToMethod$MemberReferenceToLambda::makeReceiver($Symbol$VarSymbol* rcvr) {
	...
	$var($JCTree$JCExpression, rcvrExpr, $nc(this->this$0->make)->Ident(rcvr));
	bool protAccess = this->this$0->isProtectedInSuperClassOfEnclosingClassInOtherPackage($nc(this->tree)->sym, this->owner);
	$var($Type, rcvrType, this->tree->ownerAccessible && !protAccess ? $nc($($nc(this->tree->sym)->enclClass()))->type : $nc(this->tree->expr)->type);
	if (rcvrType == $nc($nc(this->this$0->syms)->arrayClass)->type) {
		$assign(rcvrType, $nc($$cast($JCTree$JCExpression, this->tree->getQualifierExpression()))->type);
	}
	if (!$nc($nc($nc(rcvr)->type)->tsym)->isSubClass($nc(rcvrType)->tsym, this->this$0->types)) {
		$assign(rcvrExpr, $$nc($nc(this->this$0->make)->TypeCast($($nc(this->this$0->make)->Type(rcvrType)), rcvrExpr))->setType(rcvrType));
	}
	return rcvrExpr;
}
...
$JCTree$JCExpression* LambdaToMethod$MemberReferenceToLambda::expressionInvoke($Symbol$VarSymbol* rcvr) {
	...
	$set($nc(select), sym, this->tree->sym);
	$set(select, type, $nc(this->tree->sym)->erasure(this->this$0->types));
	$var($List, var$0, $List::nil());
	$var($JCTree$JCExpression, apply, $$nc($nc(this->this$0->make)->Apply(var$0, select, $(this->this$0->convertArgs(this->tree->sym, $(this->args->toList()), this->tree->varargsElement))))->setType($($$nc($nc(this->tree->sym)->erasure(this->this$0->types))->getReturnType())));
	...
	this->this$0->setVarargsIfNeeded(apply, this->tree->varargsElement);
	return apply;
}
...
$JCTree$JCExpression* LambdaToMethod$MemberReferenceToLambda::expressionNew() {
	...
	if ($nc(this->tree)->kind == $JCTree$JCMemberReference$ReferenceKind::ARRAY_CTOR) {
		$var($JCTree$JCExpression, var$0, $nc(this->this$0->make)->Type($($nc(this->this$0->types)->elemtype($nc($$cast($JCTree$JCExpression, this->tree->getQualifierExpression()))->type))));
		$var($JCTree$JCNewArray, newArr, $nc(this->this$0->make)->NewArray(var$0, $($List::of($(this->this$0->make->Ident($$cast($JCTree$JCVariableDecl, this->params->first()))))), nullptr));
		$set($nc(newArr), type, $nc($$cast($JCTree$JCExpression, this->tree->getQualifierExpression()))->type);
		return newArr;
	} else {
		$var($List, var$1, $List::nil());
		$var($JCTree$JCExpression, var$2, $nc(this->this$0->make)->Type($nc($$cast($JCTree$JCExpression, this->tree->getQualifierExpression()))->type));
		$var($JCTree$JCNewClass, newClass, $nc(this->this$0->make)->NewClass(nullptr, var$1, var$2, $(this->this$0->convertArgs(this->tree->sym, $(this->args->toList()), this->tree->varargsElement)), nullptr));
		$set($nc(newClass), constructor, this->tree->sym);
		$set(newClass, constructorType, $nc(this->tree->sym)->erasure(this->this$0->types));
		$set(newClass, type, $nc($$cast($JCTree$JCExpression, this->tree->getQualifierExpression()))->type);
		this->this$0->setVarargsIfNeeded(newClass, this->tree->varargsElement);
		return newClass;
	}
}
...
$Symbol$VarSymbol* LambdaToMethod$MemberReferenceToLambda::addParameter($String* name, $Type* p, bool genArg) {
	$useLocalObjectStack();
	$var($Symbol$VarSymbol, vsym, $new($Symbol$VarSymbol, (int64_t)0x0000000200000000 | 0x1000, $($nc(this->this$0->names)->fromString(name)), p, this->owner));
	vsym->pos = $nc(this->tree)->pos$;
	this->params->append($($nc(this->this$0->make)->VarDef(vsym, nullptr)));
	if (genArg) {
		this->args->append($($nc(this->this$0->make)->Ident(vsym)));
	}
	return vsym;
}
...
$Class* LambdaToMethod$MemberReferenceToLambda::load$($String* name, bool initialize) {
	...
	$loadClass(LambdaToMethod$MemberReferenceToLambda, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LambdaToMethod$MemberReferenceToLambda);
	});
	return class$;
}
