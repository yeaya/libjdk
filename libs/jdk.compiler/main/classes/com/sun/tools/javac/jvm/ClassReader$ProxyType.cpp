...
$Class* ClassReader$ProxyType$$Lambda$sigToType::load$($String* name, bool initialize) {
	...
	$loadClass(ClassReader$ProxyType$$Lambda$sigToType, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ClassReader$ProxyType$$Lambda$sigToType);
	});
	return class$;
}
...
$Type* ClassReader$ProxyType::resolve() {
	return $cast($Type, $nc(this->name)->map($$new(ClassReader$ProxyType$$Lambda$sigToType, this->this$0)));
}
...
$Class* ClassReader$ProxyType::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.jvm.ClassReader$ProxyType$$Lambda$sigToType")) {
	...
	$loadClass(ClassReader$ProxyType, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
