...
$Env* TypeEnter$AbstractHeaderPhase::baseEnv($JCTree$JCClassDecl* tree, $Env* env) {
	...
	if (tree->typarams != nullptr) {
		$var($List, typarams, tree->typarams);
		for (; $nc(typarams)->nonEmpty(); $assign(typarams, typarams->tail)) {
			$nc(baseScope)->enter($nc($nc(($cast($JCTree$JCTypeParameter, typarams->head)))->type)->tsym);
		}
	}
	$var($Env, outer, env->outer);
	$var($Env, localEnv, $nc(outer)->dup(tree, $($nc($cast($AttrContext, $nc(outer)->info))->dup(baseScope))));
...
$JCTree$JCExpression* TypeEnter$AbstractHeaderPhase::enumBase(int32_t pos, $Symbol$ClassSymbol* c) {
	...
	$var($JCTree$JCExpression, result, $$nc($nc(this->this$0->make)->at(pos))->TypeApply(var$0, $($List::of($(this->this$0->make->Type($nc(c)->type))))));
	return result;
}
...
$Type* TypeEnter$AbstractHeaderPhase::modelMissingTypes($Env* env, $Type* t, $JCTree$JCExpression* tree, bool interfaceExpected) {
	...
	return $new($TypeEnter$AbstractHeaderPhase$1, this, $(t->getOriginalType()), t->tsym, env, interfaceExpected, tree);
}
...
void TypeEnter$AbstractHeaderPhase::attribSuperTypes($Env* env, $Env* baseEnv) {
	...
	if (tree->extending != nullptr) {
		...
		if (supertype == $nc(this->this$0->syms)->recordType) {
			$nc(this->this$0->log)->error(tree, $($CompilerProperties$Errors::InvalidSupertypeRecord($nc(supertype)->tsym)));
		}
	} else {
		$assign(extending, nullptr);
		$init($Type);
		$assign(supertype, (($nc(tree->mods)->flags & $Flags::ENUM) != 0) ? $nc(this->this$0->attr)->attribBase($(enumBase(tree->pos$, sym)), baseEnv, true, false, false) : (sym->fullname == $nc(this->this$0->names)->java_lang_Object) ? $cast($Type, $Type::noType) : sym->isRecord() ? $nc(this->this$0->syms)->recordType : $nc(this->this$0->syms)->objectType);
	}
	...
	if ((sym->flags_field & 0x2000) != 0) {
		...
		$set(ct, all_interfaces_field, (all_interfaces == nullptr) ? ct->interfaces_field : all_interfaces->toList());
	}
...
$Class* TypeEnter$AbstractHeaderPhase::load$($String* name, bool initialize) {
	...
	$loadClass(TypeEnter$AbstractHeaderPhase, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TypeEnter$AbstractHeaderPhase);
	});
	return class$;
}
