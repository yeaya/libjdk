...
$Kinds$KindName* Kinds::kindName($Symbol* sym) {
	...
	case 14:
		$init($Kinds$KindName);
		return ((sym->flags_field & (int64_t)0x2000000000000000) != 0) ? $Kinds$KindName::RECORD_COMPONENT : $Kinds$KindName::VAR;
	case 15:
		$init($Kinds$KindName);
		return $Kinds$KindName::CONSTRUCTOR;
	case 16:
...
$Kinds$KindName* Kinds::typeKindName($Type* t) {
	...
	if (!var$0) {
		bool var$1 = t->hasTag($TypeTag::CLASS);
		var$0 = var$1 && ($nc(t->tsym)->flags() & 0x01000000) != 0;
	}
	if (var$0) {
		$init($Kinds$KindName);
		return $Kinds$KindName::BOUND;
	} else if (t->hasTag($TypeTag::PACKAGE)) {
		$init($Kinds$KindName);
		return $Kinds$KindName::PACKAGE;
	} else if (($nc(t->tsym)->flags_field & 0x2000) != 0) {
		$init($Kinds$KindName);
		return $Kinds$KindName::ANNOTATION;
	} else if ((t->tsym->flags_field & 0x0200) != 0) {
		$init($Kinds$KindName);
		return $Kinds$KindName::INTERFACE;
	} else {
		$init($Kinds$KindName);
...
$Class* Kinds::load$($String* name, bool initialize) {
	...
	$loadClass(Kinds, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Kinds);
	});
	return class$;
}
