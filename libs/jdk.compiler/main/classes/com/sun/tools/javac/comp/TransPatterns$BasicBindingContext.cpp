...
$Class* TransPatterns$BasicBindingContext$$Lambda$lambda$getBindingFor$0::load$($String* name, bool initialize) {
	...
	$loadClass(TransPatterns$BasicBindingContext$$Lambda$lambda$getBindingFor$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TransPatterns$BasicBindingContext$$Lambda$lambda$getBindingFor$0);
	});
	return class$;
}
...
$Class* TransPatterns$BasicBindingContext$$Lambda$lambda$getBindingFor$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(TransPatterns$BasicBindingContext$$Lambda$lambda$getBindingFor$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TransPatterns$BasicBindingContext$$Lambda$lambda$getBindingFor$1$1);
	});
	return class$;
}
...
$Symbol$VarSymbol* TransPatterns$BasicBindingContext::bindingDeclared($Symbol$BindingSymbol* varSymbol) {
	...
	if (res == nullptr) {
		$assign(res, $new($Symbol$VarSymbol, $nc(varSymbol)->flags(), $nc(varSymbol)->name, $nc(varSymbol)->type, this->this$0->currentMethodSym));
...
$Symbol$VarSymbol* TransPatterns$BasicBindingContext::getBindingFor($Symbol$BindingSymbol* varSymbol) {
	...
	return $cast($Symbol$VarSymbol, $$nc($$nc($$nc($$nc($$nc($nc(this->hoistedVarMap)->entrySet())->stream())->filter($$new(TransPatterns$BasicBindingContext$$Lambda$lambda$getBindingFor$0, varSymbol)))->findFirst())->map($$new(TransPatterns$BasicBindingContext$$Lambda$lambda$getBindingFor$1$1)))->orElse(nullptr));
}
...
$List* TransPatterns$BasicBindingContext::bindingVars(int32_t diagPos) {
	...
	{
		$var($Iterator, i$, $$nc(this->hoistedVarMap->entrySet())->iterator());
		for (; $nc(i$)->hasNext();) {
			$var($Map$Entry, e, $cast($Map$Entry, i$->next()));
			{
				$var($JCTree$JCVariableDecl, decl, makeHoistedVarDecl(diagPos, $$cast($Symbol$VarSymbol, $nc(e)->getValue())));
				bool var$0 = !$$sure($Symbol$BindingSymbol, e->getKey())->isPreserved();
				if (var$0 || !$nc(this->parent)->tryPrepend($$cast($Symbol$BindingSymbol, e->getKey()), decl)) {
					stats->add(decl);
				}
			}
		}
...
$JCTree$JCStatement* TransPatterns$BasicBindingContext::decorateStatement($JCTree$JCStatement* stat$renamed) {
	...
	if ($nc(stats)->nonEmpty()) {
		$assign(stat, $$nc($nc(this->this$0->make)->at(stat->pos$))->Block(0, $(stats->append(stat))));
	}
	return stat;
}
...
bool TransPatterns$BasicBindingContext::lambda$getBindingFor$0($Symbol$BindingSymbol* varSymbol, $Map$Entry* e) {
	$init(TransPatterns$BasicBindingContext);
	return $$sure($Symbol$BindingSymbol, $nc(e)->getKey())->isAliasFor(varSymbol);
}
...
$Class* TransPatterns$BasicBindingContext::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.comp.TransPatterns$BasicBindingContext$$Lambda$lambda$getBindingFor$0")) {
			return TransPatterns$BasicBindingContext$$Lambda$lambda$getBindingFor$0::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.comp.TransPatterns$BasicBindingContext$$Lambda$lambda$getBindingFor$1$1")) {
	...
	$loadClass(TransPatterns$BasicBindingContext, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TransPatterns$BasicBindingContext);
	});
	return class$;
}
