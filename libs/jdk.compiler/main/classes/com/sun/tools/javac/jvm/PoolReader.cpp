...
$Class* PoolReader$$Lambda$sigToType::load$($String* name, bool initialize) {
	...
	$loadClass(PoolReader$$Lambda$sigToType, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PoolReader$$Lambda$sigToType);
	});
	return class$;
}
...
$Class* PoolReader$$Lambda$internalize$1::load$($String* name, bool initialize) {
	...
	$loadClass(PoolReader$$Lambda$internalize$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PoolReader$$Lambda$internalize$1);
	});
	return class$;
}
...
$Object* PoolReader::peekClassName(int32_t index, $Name$NameMapper* mapper) {
	return peekName($nc(this->buf)->getChar($nc(this->pool)->offset(index)), mapper);
}
...
$Type* PoolReader::getType(int32_t index) {
	$useLocalObjectStack();
	return $cast($Type, $$nc(getName(index))->map($$new(PoolReader$$Lambda$sigToType, $nc(this->reader))));
}
...
$Object* PoolReader::getConstant(int32_t index) {
	return $nc(this->pool)->readIfNeeded(index, PoolReader::constantCP);
}
...
$Object* PoolReader::getUtf8(int32_t index, $Name$NameMapper* mapper) {
	...
	if (tag == 1) {
		...
		return $nc(mapper)->map($nc(this->pool)->poolbuf->elems, offset + 2, len);
	} else {
	...
	case 7:
		{
			...
			$var($Name, name, $nc(this->names)->fromUtf($$cast($bytes, $$nc(getName(index))->map($$new(PoolReader$$Lambda$internalize$1)))));
			return $of($nc(this->syms)->enterClass($nc(this->reader)->currentModule, name));
		}
	case 12:
		{
			$var($Name, name, getName($nc(poolbuf)->getChar(offset)));
			$var($Type, type, getType(poolbuf->getChar(offset + 2)));
...
void PoolReader::clinit$($Class* clazz) {
	...
	{
		PoolReader::classCP->set(7);
		PoolReader::constantCP->set(3, 8 + 1);
		PoolReader::moduleCP->set(19);
		PoolReader::packageCP->set(20);
		PoolReader::utf8CP->set(1);
		PoolReader::nameAndTypeCP->set(12);
	}
}
...
$Class* PoolReader::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.jvm.PoolReader$$Lambda$sigToType")) {
			return PoolReader$$Lambda$sigToType::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.jvm.PoolReader$$Lambda$internalize$1")) {
	...
	$loadClass(PoolReader, name, initialize, &classInfo$$, PoolReader::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(PoolReader);
	});
	return class$;
}
