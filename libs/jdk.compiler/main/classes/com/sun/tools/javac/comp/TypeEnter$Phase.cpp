...
$List* TypeEnter$Phase::completeEnvs($List* envs) {
	$useLocalObjectStack();
	bool firstToComplete = this->queue->isEmpty();
	...
	if (firstToComplete) {
		$var($List, out, this->queue->toList());
		this->queue->clear();
		return this->next != nullptr ? this->next->completeEnvs(out) : out;
	} else {
		return $List::nil();
	}
}
		...
		$var($Env, env, $cast($Env, i$->next()));
		{
			...
			try {
				try {
					...
					$nc(this->this$0->chk)->completionError($(tree->pos()), ex);
				}
			} catch ($Throwable& var$1) {
				$assign(var$0, var$1);
			} /*finally*/ {
...
$Class* TypeEnter$Phase::load$($String* name, bool initialize) {
	...
	$loadClass(TypeEnter$Phase, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TypeEnter$Phase);
	});
	return class$;
}
