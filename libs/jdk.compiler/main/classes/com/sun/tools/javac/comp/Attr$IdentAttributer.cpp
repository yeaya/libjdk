...
$Symbol* Attr$IdentAttributer::visitMemberSelect($MemberSelectTree* node, $Env* env) {
	$useLocalObjectStack();
	$var($Symbol, site, $cast($Symbol, visit($($nc(node)->getExpression()), env)));
	$init($Kinds$Kind);
	if ($nc(site)->kind == $Kinds$Kind::ERR || site->kind == $Kinds$Kind::ABSENT_TYP || site->kind == $Kinds$Kind::HIDDEN) {
		return site;
	}
	$var($Name, name, $cast($Name, node->getIdentifier()));
	if (site->kind == $Kinds$Kind::PCK) {
...
$Class* Attr$IdentAttributer::load$($String* name, bool initialize) {
	...
	$loadClass(Attr$IdentAttributer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Attr$IdentAttributer);
	});
	return class$;
}
