	...
	try {
		try {
			...
			while (($assign(ln, r->readLine())) != nullptr) {
				...
				int32_t n = ln->length();
				if (n != 0) {
					bool var$3 = ln->indexOf(u' ') >= 0;
					if (var$3 || (ln->indexOf(u'\t') >= 0)) {
...
void ServiceProxy::clinit$($Class* clazz) {
...
$Class* ServiceProxy::load$($String* name, bool initialize) {
	...
	$loadClass(ServiceProxy, name, initialize, &classInfo$$, ServiceProxy::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ServiceProxy);
	});
	return class$;
}
