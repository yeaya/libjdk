...
$Class* ArgumentAttr$$Lambda$lambda$processArg$0::load$($String* name, bool initialize) {
	...
	$loadClass(ArgumentAttr$$Lambda$lambda$processArg$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ArgumentAttr$$Lambda$lambda$processArg$0);
	});
	return class$;
}
...
$Class* ArgumentAttr$$Lambda$lambda$visitParens$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(ArgumentAttr$$Lambda$lambda$visitParens$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ArgumentAttr$$Lambda$lambda$visitParens$1$1);
	});
	return class$;
}
...
$Class* ArgumentAttr$$Lambda$lambda$visitConditional$2$2::load$($String* name, bool initialize) {
	...
	$loadClass(ArgumentAttr$$Lambda$lambda$visitConditional$2$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ArgumentAttr$$Lambda$lambda$visitConditional$2$2);
	});
	return class$;
}
...
$Class* ArgumentAttr$$Lambda$lambda$visitSwitchExpression$3$3::load$($String* name, bool initialize) {
	...
	$loadClass(ArgumentAttr$$Lambda$lambda$visitSwitchExpression$3$3, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ArgumentAttr$$Lambda$lambda$visitSwitchExpression$3$3);
	});
	return class$;
}
...
$Class* ArgumentAttr$$Lambda$lambda$visitLambda$4$4::load$($String* name, bool initialize) {
	...
	$loadClass(ArgumentAttr$$Lambda$lambda$visitLambda$4$4, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ArgumentAttr$$Lambda$lambda$visitLambda$4$4);
	});
	return class$;
}
...
$Class* ArgumentAttr$$Lambda$lambda$visitApply$5$5::load$($String* name, bool initialize) {
	...
	$loadClass(ArgumentAttr$$Lambda$lambda$visitApply$5$5, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ArgumentAttr$$Lambda$lambda$visitApply$5$5);
	});
	return class$;
}
...
$Class* ArgumentAttr$$Lambda$lambda$visitNewClass$6$6::load$($String* name, bool initialize) {
	...
	$loadClass(ArgumentAttr$$Lambda$lambda$visitNewClass$6$6, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ArgumentAttr$$Lambda$lambda$visitNewClass$6$6);
	});
	return class$;
}
...
void ArgumentAttr::init$($Context* context) {
	...
	$nc(context)->put(ArgumentAttr::methodAttrKey, this);
...
$Type* ArgumentAttr::checkSpeculative($JCDiagnostic$DiagnosticPosition* pos, $Type* t, $Attr$ResultInfo* resultInfo) {
	...
	if ($nc(t)->hasTag($TypeTag::DEFERRED)) {
		return $cast($DeferredAttr$DeferredType, t)->check(resultInfo);
	} else {
...
void ArgumentAttr::processArg($JCTree$JCExpression* that, $Function* argumentTypeFactory) {
	...
	processArg(that, $$new(ArgumentAttr$$Lambda$lambda$processArg$0, this, that, pos, argumentTypeFactory));
}
...
void ArgumentAttr::visitParens($JCTree$JCParens* that) {
	processArg(that, $$new(ArgumentAttr$$Lambda$lambda$visitParens$1$1, this, that));
}

void ArgumentAttr::visitConditional($JCTree$JCConditional* that) {
	processArg(that, $$new(ArgumentAttr$$Lambda$lambda$visitConditional$2$2, this, that));
}

void ArgumentAttr::visitSwitchExpression($JCTree$JCSwitchExpression* that) {
	processArg(that, $$new(ArgumentAttr$$Lambda$lambda$visitSwitchExpression$3$3, this, that));
}
...
void ArgumentAttr::visitReference($JCTree$JCMemberReference* tree) {
	...
	$var($Attr$ResultInfo, var$1, $nc(this->attr)->memberReferenceQualifierResult(tree));
	$assign(exprTree, $cast($JCTree$JCExpression, $nc(this->deferredAttr)->attribSpeculative(var$0, localEnv, var$1, $(withLocalCacheContext()))));
	$var($JCTree$JCMemberReference, mref2, $cast($JCTree$JCMemberReference, $$new($TreeCopier, this->attr->make)->copy(tree)));
	...
	$var($Symbol, res, $nc(this->attr->rs)->getMemberReference(tree, localEnv, mref2, $nc(exprTree)->type, tree->name));
	if (!$nc($nc(res)->kind)->isResolutionError()) {
		$set(tree, sym, res);
	}
	if ($nc(res->kind)->isResolutionTargetError()) {
		$init($JCTree$JCMemberReference$OverloadKind);
		tree->setOverloadKind($JCTree$JCMemberReference$OverloadKind::ERROR);
	} else {
		$init($TypeTag);
		bool var$3 = res->type != nullptr && res->type->hasTag($TypeTag::FORALL);
		bool var$2 = var$3 || (res->flags() & $Flags::VARARGS) != 0;
		if (!var$2) {
			bool var$5 = $TreeInfo::isStaticSelector(exprTree, $nc($nc(tree->name)->table)->names);
			bool var$4 = var$5 && $nc(exprTree->type)->isRaw();
			var$2 = var$4 && !exprTree->type->hasTag($TypeTag::ARRAY);
		}
		if (var$2) {
			$init($JCTree$JCMemberReference$OverloadKind);
	...
	setResult(tree, $$new($DeferredAttr$DeferredType, $nc(this->deferredAttr), tree, this->env));
}
...
void ArgumentAttr::visitLambda($JCTree$JCLambda* that) {
	...
	if ($nc(that)->paramKind == $JCTree$JCLambda$ParameterKind::EXPLICIT) {
		processArg(that, $$new(ArgumentAttr$$Lambda$lambda$visitLambda$4$4, this, that));
	} else {
		setResult(that, $$new($DeferredAttr$DeferredType, $nc(this->deferredAttr), that, this->env));
	}
}

void ArgumentAttr::visitApply($JCTree$JCMethodInvocation* that) {
	$useLocalObjectStack();
	if ($$sure($List, $nc(that)->getTypeArguments())->isEmpty()) {
		processArg(that, $$new(ArgumentAttr$$Lambda$lambda$visitApply$5$5, this, that));
	} else {
...
void ArgumentAttr::visitNewClass($JCTree$JCNewClass* that) {
	...
	if ($TreeInfo::isDiamond(that)) {
		processArg(that, $$new(ArgumentAttr$$Lambda$lambda$visitNewClass$6$6, this, that));
	} else {
		setResult(that, $($nc(this->attr)->attribTree(that, this->env, $nc(this->attr)->unknownExprInfo)));
	}
}
...
$ArgumentAttr$ArgumentType* ArgumentAttr::lambda$processArg$0($JCTree$JCExpression* that, $ArgumentAttr$UniquePos* pos, $Function* argumentTypeFactory) {
	$useLocalObjectStack();
	$var($JCTree$JCExpression, speculativeTree, $cast($JCTree$JCExpression, $nc(this->deferredAttr)->attribSpeculative(that, this->env, $$new($ArgumentAttr$1, this, $nc(this->attr), pos))));
	return $cast($ArgumentAttr$ArgumentType, $nc(argumentTypeFactory)->apply(speculativeTree));
}

void ArgumentAttr::clinit$($Class* clazz) {
...
$Class* ArgumentAttr::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.comp.ArgumentAttr$$Lambda$lambda$processArg$0")) {
			return ArgumentAttr$$Lambda$lambda$processArg$0::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.comp.ArgumentAttr$$Lambda$lambda$visitParens$1$1")) {
			return ArgumentAttr$$Lambda$lambda$visitParens$1$1::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.comp.ArgumentAttr$$Lambda$lambda$visitConditional$2$2")) {
			return ArgumentAttr$$Lambda$lambda$visitConditional$2$2::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.comp.ArgumentAttr$$Lambda$lambda$visitSwitchExpression$3$3")) {
			return ArgumentAttr$$Lambda$lambda$visitSwitchExpression$3$3::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.comp.ArgumentAttr$$Lambda$lambda$visitLambda$4$4")) {
			return ArgumentAttr$$Lambda$lambda$visitLambda$4$4::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.comp.ArgumentAttr$$Lambda$lambda$visitApply$5$5")) {
			return ArgumentAttr$$Lambda$lambda$visitApply$5$5::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.comp.ArgumentAttr$$Lambda$lambda$visitNewClass$6$6")) {
	...
	$loadClass(ArgumentAttr, name, initialize, &classInfo$$, ArgumentAttr::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ArgumentAttr);
	});
	return class$;
}
