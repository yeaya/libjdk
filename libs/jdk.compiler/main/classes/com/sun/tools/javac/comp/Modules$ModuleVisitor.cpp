...
$Class* Modules$ModuleVisitor$$Lambda$lambda$visitModuleDef$0::load$($String* name, bool initialize) {
	...
	$loadClass(Modules$ModuleVisitor$$Lambda$lambda$visitModuleDef$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Modules$ModuleVisitor$$Lambda$lambda$visitModuleDef$0);
	});
	return class$;
}
...
$Class* Modules$ModuleVisitor$$Lambda$lambda$visitExports$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(Modules$ModuleVisitor$$Lambda$lambda$visitExports$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Modules$ModuleVisitor$$Lambda$lambda$visitExports$1$1);
	});
	return class$;
}
...
void Modules$ModuleVisitor::visitModuleDef($JCTree$JCModuleDecl* tree) {
	...
	$nc(this->sym)->flags_field |= ($nc(tree->mods)->flags & $Flags::DEPRECATED);
	$set(this->sym, requires, $List::nil());
	$set($nc(this->sym), exports, $List::nil());
	$set($nc(this->sym), opens, $List::nil());
	$nc(tree->directives)->forEach($$new(Modules$ModuleVisitor$$Lambda$lambda$visitModuleDef$0, this));
...
void Modules$ModuleVisitor::visitRequires($JCTree$JCRequires* tree) {
	...
	if ($nc(msym)->kind != $Kinds$Kind::MDL) {
		$var($JCDiagnostic$DiagnosticPosition, var$0, $nc(tree->moduleName)->pos());
		...
		$var($JCDiagnostic$DiagnosticPosition, var$1, $nc(tree->moduleName)->pos());
		...
		if (tree->isTransitive$) {
			$init($Source);
			if (msym == $nc(this->this$0->syms)->java_base && this->this$0->source->compareTo($Source::JDK10) >= 0) {
...
void Modules$ModuleVisitor::visitExports($JCTree$JCExports* tree) {
	...
	$nc(this->this$0->attr)->setPackageSymbols(tree->qualid, packge);
	$var($List, exportsForPackage, $cast($List, this->allExports->computeIfAbsent(packge, $$new(Modules$ModuleVisitor$$Lambda$lambda$visitExports$1$1))));
	{
	...
	if (tree->moduleNames != nullptr) {
		...
		{
			$var($Iterator, i$, tree->moduleNames->iterator());
			for (; $nc(i$)->hasNext();) {
		...
		$assign(toModules, $List::from(to));
	}
	if (toModules == nullptr || !toModules->isEmpty()) {
...
void Modules$ModuleVisitor::checkDuplicateExportsToModule($JCTree$JCExpression* name, $Symbol$ModuleSymbol* msym, $Directive$ExportsDirective* d) {
	...
	if ($nc(d)->modules != nullptr) {
		$var($Iterator, i$, d->modules->iterator());
		for (; $nc(i$)->hasNext();) {
...
void Modules$ModuleVisitor::visitOpens($JCTree$JCOpens* tree) {
	...
	if ($nc($nc(this->sym)->flags$)->contains($Symbol$ModuleFlags::OPEN)) {
		...
		$nc(this->this$0->log)->error($(tree->pos()), $CompilerProperties$Errors::NoOpensUnlessStrong);
	}
	$var($List, opensForPackage, $cast($List, this->allOpens->computeIfAbsent(packge, $$new(Modules$ModuleVisitor$$Lambda$lambda$visitExports$1$1))));
	{
...
void Modules$ModuleVisitor::ensureJavaBase() {
	...
	{
		$var($Iterator, i$, $nc(this->sym->requires)->iterator());
		for (; $nc(i$)->hasNext();) {
			$var($Directive$RequiresDirective, d, $cast($Directive$RequiresDirective, i$->next()));
			if ($nc($nc(d)->module)->name == this->this$0->names->java_base) {
				return;
			}
		}
	}
	$var($Symbol$ModuleSymbol, java_base, $nc(this->this$0->syms)->enterModule(this->this$0->names->java_base));
	$init($Directive$RequiresFlag);
...
$Class* Modules$ModuleVisitor::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.comp.Modules$ModuleVisitor$$Lambda$lambda$visitModuleDef$0")) {
			return Modules$ModuleVisitor$$Lambda$lambda$visitModuleDef$0::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.comp.Modules$ModuleVisitor$$Lambda$lambda$visitExports$1$1")) {
	...
	$loadClass(Modules$ModuleVisitor, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Modules$ModuleVisitor);
	});
	return class$;
}
