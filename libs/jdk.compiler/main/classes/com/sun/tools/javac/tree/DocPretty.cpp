...
void DocPretty::printTagName($DocTree* node) {
	...
	this->out->write($nc($($nc(node)->getKind()))->tagName);
}
...
$Void* DocPretty::visitAttribute($AttributeTree* node, $Void* p) {
	...
	try {
		...
		$init($DocPretty$1);
		switch ($nc($DocPretty$1::$SwitchMap$com$sun$source$doctree$AttributeTree$ValueKind)->get(($$nc(node->getValueKind()))->ordinal())) {
		case 1:
...
$Void* DocPretty::visitIndex($IndexTree* node, $Void* p) {
	...
	try {
		...
		if (!$$nc(node->getDescription())->isEmpty()) {
			print(" "_s);
...
$Void* DocPretty::visitLink($LinkTree* node, $Void* p) {
	...
	try {
		...
		printTagName(node);
		print(" "_s);
		print($($nc(node)->getReference()));
		if (!$$nc(node->getLabel())->isEmpty()) {
			print(" "_s);
			print($(node->getLabel()));
		}
		print("}"_s);
...
$Void* DocPretty::visitReturn($ReturnTree* node, $Void* p) {
	try {
		...
		printTagName($cast($BlockTagTree, node));
		print(" "_s);
		print($(node->getDescription()));
		if (node->isInline()) {
			print("}"_s);
...
$Void* DocPretty::visitStartElement($StartElementTree* node, $Void* p) {
	...
	try {
		print("<"_s);
		print($($nc(node)->getName()));
		$var($List, attrs, node->getAttributes());
		if (!$nc(attrs)->isEmpty()) {
			print(" "_s);
			print(attrs, " "_s);
...
$Void* DocPretty::visitThrows($ThrowsTree* node, $Void* p) {
	...
	try {
		printTagName(node);
		print(" "_s);
		print($($nc(node)->getExceptionName()));
		if (!$$nc(node->getDescription())->isEmpty()) {
			print(" "_s);
			print($(node->getDescription()));
		}
...
$Object* DocPretty::visitReference($ReferenceTree* node, Object$* p) {
	return this->visitReference(node, $cast($Void, p));
}

$Object* DocPretty::visitProvides($ProvidesTree* node, Object$* p) {
	return this->visitProvides(node, $cast($Void, p));
}
...
$Object* DocPretty::visitLiteral($LiteralTree* node, Object$* p) {
	return this->visitLiteral(node, $cast($Void, p));
}

$Object* DocPretty::visitLink($LinkTree* node, Object$* p) {
	return this->visitLink(node, $cast($Void, p));
}
...
$Object* DocPretty::visitDeprecated($DeprecatedTree* node, Object$* p) {
	return this->visitDeprecated(node, $cast($Void, p));
}

$Object* DocPretty::visitComment($CommentTree* node, Object$* p) {
	return this->visitComment(node, $cast($Void, p));
}

$Object* DocPretty::visitAuthor($AuthorTree* node, Object$* p) {
	return this->visitAuthor(node, $cast($Void, p));
}

$Object* DocPretty::visitAttribute($AttributeTree* node, Object$* p) {
	return this->visitAttribute(node, $cast($Void, p));
}
...
$Class* DocPretty::load$($String* name, bool initialize) {
	...
	$loadClass(DocPretty, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DocPretty);
	});
	return class$;
}
