	...
	case 12:
		{
			if (s->hasTag($TypeTag::TYPEVAR)) {
				...
				return $Boolean::valueOf(var$0 && $$sure($Boolean, visit(t, $(this->this$0->wildUpperBound(s))))->booleanValue());
			}
		}
	default:
...
$Boolean* Types$5::visitClassType($Type$ClassType* t, $Type* s) {
	...
	bool var$0 = s->isSuperBound();
	if (var$0 && !s->isExtendsBound()) {
		bool var$1 = $$sure($Boolean, visit(t, $(this->this$0->wildUpperBound(s))))->booleanValue();
		return $Boolean::valueOf(var$1 && $$sure($Boolean, visit(t, $(this->this$0->wildLowerBound(s))))->booleanValue());
	}
	bool var$2 = $nc(t)->isCompound();
	if (var$2 && s->isCompound()) {
		$var($Type, var$3, this->this$0->supertype(t));
		if (!$$sure($Boolean, visit(var$3, $(this->this$0->supertype(s))))->booleanValue()) {
			return $Boolean::valueOf(false);
		}
		...
		}
		{
			...
			for (; $nc(i$)->hasNext();) {
				$var($Type, si, $cast($Type, i$->next()));
				{
					...
					$var($Type, ti, $cast($Type, tMap->remove(si->tsym)));
					if (!$$sure($Boolean, visit(ti, si))->booleanValue()) {
						return $Boolean::valueOf(false);
					}
				}
			}
	...
	bool var$5 = t->tsym == s->tsym;
	if (var$5) {
		$var($Type, var$6, t->getEnclosingType());
		var$5 = $$sure($Boolean, visit(var$6, $(s->getEnclosingType())))->booleanValue();
	}
	bool var$4 = var$5;
	if (var$4) {
...
$Boolean* Types$5::visitArrayType($Type$ArrayType* t, $Type* s) {
	...
	$init($TypeTag);
	bool var$0 = s->hasTag($TypeTag::ARRAY);
...
$Boolean* Types$5::visitMethodType($Type$MethodType* t, $Type* s) {
	...
	if (var$0) {
		...
		var$0 = $$sure($Boolean, visit(var$1, $($nc(s)->getReturnType())))->booleanValue();
	}
...
$Boolean* Types$5::visitUndetVar($Type$UndetVar* t, $Type* s) {
	...
	bool var$0 = $equals(t, s) || $nc(t)->qtype == s || s->hasTag($TypeTag::ERROR);
	if (var$0 || s->hasTag($TypeTag::UNKNOWN)) {
		return $Boolean::valueOf(true);
	}
...
$Class* Types$5::load$($String* name, bool initialize) {
	...
	$loadClass(Types$5, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Types$5);
	});
	return class$;
}
