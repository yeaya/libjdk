...
void TypeEnter$RecordPhase::runPhase($Env* env) {
	...
	if (($nc(sym)->flags_field & (int64_t)0x2000000000000000) != 0) {
		...
		$nc(this->this$0->memberEnter)->memberEnter(fields, env);
		{
			...
			for (; $nc(i$)->hasNext();) {
				...
				{
					sym->getRecordComponent(field, true, $nc($nc($nc(field)->mods)->annotations)->isEmpty() ? $($List::nil()) : $($$new($TreeCopier, $($nc(this->this$0->make)->at(field->pos$)))->copy(field->mods->annotations)));
				}
			}
		}
		enterThisAndSuper(sym, env);
		{
			...
			for (; $nc(i$)->hasNext();) {
				...
				if ($TreeInfo::isConstructor(def)) {
					this->this$0->memberEnter->memberEnter(def, env);
				}
			}
		}
	}
...
$Class* TypeEnter$RecordPhase::load$($String* name, bool initialize) {
	...
	$loadClass(TypeEnter$RecordPhase, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TypeEnter$RecordPhase);
	});
	return class$;
}
