...
$Class* Types$CaptureScanner$$Lambda$lambda$visitClassType$0::load$($String* name, bool initialize) {
	...
	$loadClass(Types$CaptureScanner$$Lambda$lambda$visitClassType$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Types$CaptureScanner$$Lambda$lambda$visitClassType$0);
	});
	return class$;
}
...
$Void* Types$CaptureScanner::visitClassType($Type$ClassType* t, $Set* seen) {
	$useLocalObjectStack();
	if ($nc(t)->isCompound()) {
		$$nc(this->this$0->directSupertypes(t))->forEach($$new(Types$CaptureScanner$$Lambda$lambda$visitClassType$0, this, seen));
	} else {
		$$nc(t->allparams())->forEach($$new(Types$CaptureScanner$$Lambda$lambda$visitClassType$0, this, seen));
	}
	return nullptr;
}
...
$Void* Types$CaptureScanner::visitTypeVar($Type$TypeVar* t, $Set* seen) {
	bool var$0 = ($nc($nc(t)->tsym)->flags() & $Flags::SYNTHETIC) != 0;
...
$Class* Types$CaptureScanner::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.code.Types$CaptureScanner$$Lambda$lambda$visitClassType$0")) {
	...
	$loadClass(Types$CaptureScanner, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Types$CaptureScanner);
	});
	return class$;
}
