...
$Class* Infer$$Lambda$lambda$roots$0::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PUBLIC, $method(Infer$$Lambda$lambda$roots$0, init$, void)},
		{"test", "(Ljava/lang/Object;)Z", nullptr, $PUBLIC, $virtualMethod(Infer$$Lambda$lambda$roots$0, test, bool, Object$*)},
		{}
	};
	$ClassInfo classInfo$$ = {
		$PUBLIC | $FINAL,
	...
	$loadClass(Infer$$Lambda$lambda$roots$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Infer$$Lambda$lambda$roots$0);
	});
	return class$;
}
...
$Class* Infer$$Lambda$lambda$instantiateFunctionalInterface$1$1::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(Lcom/sun/tools/javac/code/Type;)V", nullptr, $PUBLIC, $method(Infer$$Lambda$lambda$instantiateFunctionalInterface$1$1, init$, void, $Type*)},
		{"test", "(Ljava/lang/Object;)Z", nullptr, $PUBLIC, $virtualMethod(Infer$$Lambda$lambda$instantiateFunctionalInterface$1$1, test, bool, Object$*)},
		{}
	};
	$ClassInfo classInfo$$ = {
		$PUBLIC | $FINAL,
	...
	$loadClass(Infer$$Lambda$lambda$instantiateFunctionalInterface$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Infer$$Lambda$lambda$instantiateFunctionalInterface$1$1);
	});
	return class$;
}
...
void Infer::init$($Context* context) {
	...
	$nc(context)->put(Infer::inferKey, this);
	...
	$set(this, dependenciesFolder, $nc(options)->get("debug.dumpInferenceGraphsTo"_s));
	...
	try {
		...
		if (this->allowGraphInference && resultInfo != nullptr && resultInfo->pt == Infer::anyPoly) {
			...
			if (!$nc(warn)->hasNonSilentLint($Lint$LintCategory::UNCHECKED)) {
				bool shouldPropagate = this->shouldPropagate($(mt->getReturnType()), resultInfo, inferenceContext);
		...
		roots->add($($nc(mt)->getReturnType()));
		{
			...
			for (; $nc(i$)->hasNext();) {
				$var($DeferredAttr$DeferredAttrNode, n, $cast($DeferredAttr$DeferredAttrNode, i$->next()));
				{
					...
					roots->addAll($($nc(n->deferredStuckPolicy)->depVars()));
				}
			}
		}
		$var($List, thrownVars, $cast($List, $$nc($$nc($nc($nc(deferredAttrContext->inferenceContext)->inferencevars)->stream())->filter($$new(Infer$$Lambda$lambda$roots$0)))->collect($($List::collector()))));
		...
		try {
			...
			for (; $nc(i$)->hasNext();) {
				$var($String, graph, $cast($String, i$->next()));
				{
					...
					$var($Name, name, $nc(msym)->name == $nc($nc($nc($nc(msym)->name)->table)->names)->init ? $nc(msym->owner)->name : msym->name);
					$var($String, filename, $String::format("%s@%s[mode=%s,step=%s]_%d.dot"_s, $$new($ObjectArray, {
						name,
						$($Integer::valueOf($nc(pos)->getStartPosition())),
						$($nc(rsContext)->attrMode()),
						$nc(rsContext)->step,
...
$Type* Infer::generateReturnConstraints($JCTree* tree, $Attr$ResultInfo* resultInfo, $Type$MethodType* mt, $InferenceContext* inferenceContext) {
		...
		$assign(from, $nc(this->types)->capture(from));
		{
			...
			for (; $nc(i$)->hasNext();) {
				$var($Type, t, $cast($Type, i$->next()));
				{
					...
					if (var$0 && $cast($Type$TypeVar, t)->isCaptured()) {
						inferenceContext->addVar($cast($Type$TypeVar, t));
					}
				}
			}
		}
	}
	$var($Type, qtype, inferenceContext->asUndetVar(from));
	$var($Type, to, resultInfo->pt);
	$init($TypeTag);
	if ($nc(qtype)->hasTag($TypeTag::VOID)) {
		$assign(to, $nc(this->syms)->voidType);
	} else if ($nc(to)->hasTag($TypeTag::NONE)) {
	...
	bool var$2 = !resultInfo->checkContext->compatible(qtype, $($nc(rsInfoInfContext)->asUndetVar(to)), retWarn);
	$init($Lint$LintCategory);
	if (var$2 || (!this->allowGraphInference && retWarn->hasLint($Lint$LintCategory::UNCHECKED))) {
		$var($1List, var$3, inferenceContext->restvars());
	...
	if ($nc(to)->isPrimitive()) {
		...
		for (; $nc(i$)->hasNext();) {
			$var($Type, t, $cast($Type, i$->next()));
			{
				...
				$init($TypeTag);
				if (boundAsPrimitive == nullptr || boundAsPrimitive->hasTag($TypeTag::NONE)) {
					continue;
				}
				return true;
			}
...
bool Infer::commonSuperWithDiffParameterization($Type* t, $Type* s) {
	...
	for (; $nc(i$)->hasNext();) {
		...
		if (!$nc(this->types)->isSameType($cast($Type, $nc(supers)->fst), $cast($Type, $nc(supers)->snd))) {
			return true;
		}
	}
	return false;
...
$Type* Infer::generateReferenceToTargetConstraint($JCTree* tree, $Type$UndetVar* from, $Type* to, $Attr$ResultInfo* resultInfo, $InferenceContext* inferenceContext) {
	...
	$nc(inferenceContext)->notifyChange();
	$var($Type, var$1, from->getInst());
	$var($Type, capturedType, $$nc($nc($nc(resultInfo)->checkContext)->inferenceContext())->cachedCapture(tree, var$1, !$nc(resultInfo)->checkMode->updateTreeType()));
		...
		for (; $nc(i$)->hasNext();) {
			$var($Type, t, $cast($Type, i$->next()));
			{
				...
				if ($Type::containsAny(upperBounds, vars)) {
					...
					$set(fresh_tvar, type, $new($Type$TypeVar, fresh_tvar, $($nc(this->types)->makeIntersectionType($(uv->getBounds($$new($Type$UndetVar$InferenceBoundArray, {$Type$UndetVar$InferenceBound::UPPER}))))), $nc(this->syms)->botType));
					todo->append(uv);
...
$Type* Infer::instantiatePolymorphicSignatureInstance($Env* env, $Symbol$MethodSymbol* spMethod, $Resolve$MethodResolutionContext* resolveContext, $List* argtypes) {
	...
	$var($Type, spType, spMethod == nullptr ? $nc(this->syms)->objectType : spMethod->getReturnType());
	{
		$init($Infer$4);
		...
		case 1:
			$assign(castTree, $cast($JCTree$JCTypeCast, env->next->tree));
			$assign(restype, ($equals($TreeInfo::skipParens($nc(castTree)->expr), env->tree)) ? $nc(castTree->clazz)->type : spType);
			break;
		case 2:
			$assign(execTree, $cast($JCTree$JCExpressionStatement, $nc(env->next)->tree));
			$assign(restype, ($equals($TreeInfo::skipParens($nc(execTree)->expr), env->tree)) ? $cast($Type, $nc(this->syms)->voidType) : spType);
			break;
		default:
...
$Type* Infer::instantiateFunctionalInterface($JCDiagnostic$DiagnosticPosition* pos, $Type* funcInterface, $List* paramTypes$renamed, $Check$CheckContext* checkContext) {
	...
	if ($nc(this->types)->capture(funcInterface) == funcInterface) {
		...
		$var($InferenceContext, funcInterfaceContext, $new($InferenceContext, this, $($nc(funcInterface->tsym->type)->getTypeArguments())));
		...
		{
			$var($Iterator, i$, descParameterTypes->iterator());
			for (; $nc(i$)->hasNext();) {
				$var($Type, p, $cast($Type, i$->next()));
				{
					...
					$assign(paramTypes, paramTypes->tail);
				}
			}
		}
		$var($List, actualTypeargs, funcInterface->getTypeArguments());
		{
			...
			for (; $nc(i$)->hasNext();) {
				$var($Type, t, $cast($Type, i$->next()));
				{
					$var($Type$UndetVar, uv, $cast($Type$UndetVar, t));
					$init($Type$UndetVar$InferenceBound);
					$var($Optional, inst, $$nc($$nc($$nc($nc(uv)->getBounds($$new($Type$UndetVar$InferenceBoundArray, {$Type$UndetVar$InferenceBound::EQ})))->stream())->filter($$new(Infer$$Lambda$lambda$instantiateFunctionalInterface$1$1, formalInterface)))->findFirst());
					uv->setInst($$cast($Type, $nc(inst)->orElse($cast($Type, $nc(actualTypeargs)->head))));
					$assign(actualTypeargs, actualTypeargs->tail);
				}
			}
		}
				...
				for (; $nc(i$)->hasNext();) {
					$var($Type, t, $cast($Type, i$->next()));
					{
						...
						if (!$nc($nc(uv)->incorporationActions)->isEmpty()) {
							progress = true;
							$$sure($Infer$IncorporationAction, uv->incorporationActions->removeFirst())->apply(inferenceContext, warn);
						}
					}
				}
			}
...
$List* Infer::getParameterizedSupers($Type* t, $Type* s) {
	...
	if (lubResult == $nc(this->syms)->errType || lubResult == this->syms->botType) {
		return $List::nil();
	}
	$var($List, supertypesToCheck, $nc(lubResult)->isIntersection() ? $cast($Type$IntersectionClassType, lubResult)->getComponents() : $List::of(lubResult));
...
$Type* Infer::asSuper($Type* t, $Type* sup) {
	...
	if ($nc(sup)->hasTag($TypeTag::ARRAY)) {
		...
		$assign(var$0, $new($Type$ArrayType, $(asSuper(var$1, $(this->types->elemtype(sup)))), $nc(this->syms)->arrayClass));
	} else {
		$assign(var$0, this->types->asSuper(t, $nc(sup)->tsym));
	}
	return var$0;
}
...
bool Infer::doIncorporationOp($Infer$IncorporationBinaryOpKind* opKind, $Type* op1, $Type* op2, $Warner* warn) {
	...
	if (res == nullptr) {
		this->incorporationCache->put(newOp, $assign(res, $Boolean::valueOf(newOp->apply(warn))));
	}
...
void Infer::reportBoundError($Type$UndetVar* uv, $Type$UndetVar$InferenceBound* ib) {
	...
	switch ($nc($Infer$4::$SwitchMap$com$sun$tools$javac$code$Type$UndetVar$InferenceBound)->get($nc((ib))->ordinal())) {
	case 1:
		$throw($(error($($nc(this->diags)->fragment($($CompilerProperties$Fragments::IncompatibleEqBounds($nc(uv)->qtype, $($nc(uv)->getBounds($$new($Type$UndetVar$InferenceBoundArray, {ib}))))))))));
	case 3:
		$throw($(error($($nc(this->diags)->fragment($($CompilerProperties$Fragments::IncompatibleUpperBounds($nc(uv)->qtype, $($nc(uv)->getBounds($$new($Type$UndetVar$InferenceBoundArray, {ib}))))))))));
	case 2:
...
bool Infer::lambda$roots$0($Type* tv) {
	$init(Infer);
	return ($nc($nc(tv)->tsym)->flags() & $Flags::THROWS) != 0;
}

void Infer::clinit$($Class* clazz) {
...
$Class* Infer::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.comp.Infer$$Lambda$lambda$roots$0")) {
			return Infer$$Lambda$lambda$roots$0::load$(name, initialize);
		}
		if (name->equals("com.sun.tools.javac.comp.Infer$$Lambda$lambda$instantiateFunctionalInterface$1$1")) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(Lcom/sun/tools/javac/util/Context;)V", nullptr, $PROTECTED, $method(Infer, init$, void, $Context*)},
		{"asSuper", "(Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/code/Type;)Lcom/sun/tools/javac/code/Type;", nullptr, $PRIVATE, $method(Infer, asSuper, $Type*, $Type*, $Type*)},
		{"commonSuperWithDiffParameterization", "(Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/code/Type;)Z", nullptr, $PRIVATE, $method(Infer, commonSuperWithDiffParameterization, bool, $Type*, $Type*)},
		{"doIncorporation", "(Lcom/sun/tools/javac/comp/InferenceContext;Lcom/sun/tools/javac/util/Warner;)V", nullptr, 0, $virtualMethod(Infer, doIncorporation, void, $InferenceContext*, $Warner*), "com.sun.tools.javac.comp.Infer$InferenceException"},
		{"doIncorporationOp", "(Lcom/sun/tools/javac/comp/Infer$IncorporationBinaryOpKind;Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/util/Warner;)Z", nullptr, 0, $virtualMethod(Infer, doIncorporationOp, bool, $Infer$IncorporationBinaryOpKind*, $Type*, $Type*, $Warner*)},
		{"dumpGraphsIfNeeded", "(Lcom/sun/tools/javac/util/JCDiagnostic$DiagnosticPosition;Lcom/sun/tools/javac/code/Symbol;Lcom/sun/tools/javac/comp/Resolve$MethodResolutionContext;)V", nullptr, $PRIVATE, $method(Infer, dumpGraphsIfNeeded, void, $JCDiagnostic$DiagnosticPosition*, $Symbol*, $Resolve$MethodResolutionContext*)},
		{"error", "(Lcom/sun/tools/javac/util/JCDiagnostic;)Lcom/sun/tools/javac/comp/Infer$InferenceException;", nullptr, 0, $virtualMethod(Infer, error, $Infer$InferenceException*, $JCDiagnostic*)},
		{"generateReferenceToTargetConstraint", "(Lcom/sun/tools/javac/tree/JCTree;Lcom/sun/tools/javac/code/Type$UndetVar;Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/comp/Attr$ResultInfo;Lcom/sun/tools/javac/comp/InferenceContext;)Lcom/sun/tools/javac/code/Type;", nullptr, $PRIVATE, $method(Infer, generateReferenceToTargetConstraint, $Type*, $JCTree*, $Type$UndetVar*, $Type*, $Attr$ResultInfo*, $InferenceContext*)},
		{"generateReturnConstraints", "(Lcom/sun/tools/javac/tree/JCTree;Lcom/sun/tools/javac/comp/Attr$ResultInfo;Lcom/sun/tools/javac/code/Type$MethodType;Lcom/sun/tools/javac/comp/InferenceContext;)Lcom/sun/tools/javac/code/Type;", nullptr, 0, $virtualMethod(Infer, generateReturnConstraints, $Type*, $JCTree*, $Attr$ResultInfo*, $Type$MethodType*, $InferenceContext*)},
		{"getBoundFragment", "(Lcom/sun/tools/javac/code/Type$UndetVar$InferenceBound;Lcom/sun/tools/javac/util/List;)Lcom/sun/tools/javac/util/JCDiagnostic$Fragment;", "(Lcom/sun/tools/javac/code/Type$UndetVar$InferenceBound;Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/code/Type;>;)Lcom/sun/tools/javac/util/JCDiagnostic$Fragment;", 0, $virtualMethod(Infer, getBoundFragment, $JCDiagnostic$Fragment*, $Type$UndetVar$InferenceBound*, $List*)},
		{"getParameterizedSupers", "(Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/code/Type;)Lcom/sun/tools/javac/util/List;", "(Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/code/Type;)Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/util/Pair<Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/code/Type;>;>;", $PRIVATE, $method(Infer, getParameterizedSupers, $List*, $Type*, $Type*)},
		{"incorporationEngine", "()Lcom/sun/tools/javac/comp/Infer$AbstractIncorporationEngine;", nullptr, 0, $virtualMethod(Infer, incorporationEngine, $Infer$AbstractIncorporationEngine*)},
		{"instance", "(Lcom/sun/tools/javac/util/Context;)Lcom/sun/tools/javac/comp/Infer;", nullptr, $PUBLIC | $STATIC, $staticMethod(Infer, instance, Infer*, $Context*)},
		{"instantiateAsUninferredVars", "(Lcom/sun/tools/javac/util/List;Lcom/sun/tools/javac/comp/InferenceContext;)V", "(Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/code/Type;>;Lcom/sun/tools/javac/comp/InferenceContext;)V", 0, $virtualMethod(Infer, instantiateAsUninferredVars, void, $List*, $InferenceContext*)},
		{"instantiateFunctionalInterface", "(Lcom/sun/tools/javac/util/JCDiagnostic$DiagnosticPosition;Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/util/List;Lcom/sun/tools/javac/comp/Check$CheckContext;)Lcom/sun/tools/javac/code/Type;", "(Lcom/sun/tools/javac/util/JCDiagnostic$DiagnosticPosition;Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/code/Type;>;Lcom/sun/tools/javac/comp/Check$CheckContext;)Lcom/sun/tools/javac/code/Type;", $PUBLIC, $virtualMethod(Infer, instantiateFunctionalInterface, $Type*, $JCDiagnostic$DiagnosticPosition*, $Type*, $List*, $Check$CheckContext*)},
		{"instantiateMethod", "(Lcom/sun/tools/javac/comp/Env;Lcom/sun/tools/javac/util/List;Lcom/sun/tools/javac/code/Type$MethodType;Lcom/sun/tools/javac/comp/Attr$ResultInfo;Lcom/sun/tools/javac/code/Symbol$MethodSymbol;Lcom/sun/tools/javac/util/List;ZZLcom/sun/tools/javac/comp/Resolve$MethodResolutionContext;Lcom/sun/tools/javac/util/Warner;)Lcom/sun/tools/javac/code/Type;", "(Lcom/sun/tools/javac/comp/Env<Lcom/sun/tools/javac/comp/AttrContext;>;Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/code/Type;>;Lcom/sun/tools/javac/code/Type$MethodType;Lcom/sun/tools/javac/comp/Attr$ResultInfo;Lcom/sun/tools/javac/code/Symbol$MethodSymbol;Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/code/Type;>;ZZLcom/sun/tools/javac/comp/Resolve$MethodResolutionContext;Lcom/sun/tools/javac/util/Warner;)Lcom/sun/tools/javac/code/Type;", 0, $virtualMethod(Infer, instantiateMethod, $Type*, $Env*, $List*, $Type$MethodType*, $Attr$ResultInfo*, $Symbol$MethodSymbol*, $List*, bool, bool, $Resolve$MethodResolutionContext*, $Warner*), "com.sun.tools.javac.comp.Infer$InferenceException"},
		{"instantiatePolymorphicSignatureInstance", "(Lcom/sun/tools/javac/comp/Env;Lcom/sun/tools/javac/code/Symbol$MethodSymbol;Lcom/sun/tools/javac/comp/Resolve$MethodResolutionContext;Lcom/sun/tools/javac/util/List;)Lcom/sun/tools/javac/code/Type;", "(Lcom/sun/tools/javac/comp/Env<Lcom/sun/tools/javac/comp/AttrContext;>;Lcom/sun/tools/javac/code/Symbol$MethodSymbol;Lcom/sun/tools/javac/comp/Resolve$MethodResolutionContext;Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/code/Type;>;)Lcom/sun/tools/javac/code/Type;", 0, $virtualMethod(Infer, instantiatePolymorphicSignatureInstance, $Type*, $Env*, $Symbol$MethodSymbol*, $Resolve$MethodResolutionContext*, $List*)},
		{"lambda$instantiateFunctionalInterface$1", "(Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/code/Type;)Z", nullptr, $PRIVATE | $STATIC | $SYNTHETIC, $staticMethod(Infer, lambda$instantiateFunctionalInterface$1, bool, $Type*, $Type*)},
		{"lambda$roots$0", "(Lcom/sun/tools/javac/code/Type;)Z", nullptr, $PRIVATE | $STATIC | $SYNTHETIC, $staticMethod(Infer, lambda$roots$0, bool, $Type*)},
		{"needsEagerInstantiation", "(Lcom/sun/tools/javac/code/Type$UndetVar;Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/comp/InferenceContext;)Z", nullptr, $PRIVATE, $method(Infer, needsEagerInstantiation, bool, $Type$UndetVar*, $Type*, $InferenceContext*)},
		{"reportBoundError", "(Lcom/sun/tools/javac/code/Type$UndetVar;Lcom/sun/tools/javac/code/Type$UndetVar$InferenceBound;)V", nullptr, 0, $virtualMethod(Infer, reportBoundError, void, $Type$UndetVar*, $Type$UndetVar$InferenceBound*)},
		{"reportBoundError", "(Lcom/sun/tools/javac/code/Type$UndetVar;Lcom/sun/tools/javac/code/Type$UndetVar$InferenceBound;Lcom/sun/tools/javac/code/Type$UndetVar$InferenceBound;)V", nullptr, 0, $virtualMethod(Infer, reportBoundError, void, $Type$UndetVar*, $Type$UndetVar$InferenceBound*, $Type$UndetVar$InferenceBound*)},
		{"reportInstError", "(Lcom/sun/tools/javac/code/Type$UndetVar;Lcom/sun/tools/javac/code/Type$UndetVar$InferenceBound;)V", nullptr, 0, $virtualMethod(Infer, reportInstError, void, $Type$UndetVar*, $Type$UndetVar$InferenceBound*)},
		{"roots", "(Lcom/sun/tools/javac/code/Type$MethodType;Lcom/sun/tools/javac/comp/DeferredAttr$DeferredAttrContext;)Lcom/sun/tools/javac/util/List;", "(Lcom/sun/tools/javac/code/Type$MethodType;Lcom/sun/tools/javac/comp/DeferredAttr$DeferredAttrContext;)Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/code/Type;>;", $PRIVATE, $method(Infer, roots, $List*, $Type$MethodType*, $DeferredAttr$DeferredAttrContext*)},
		{"shouldPropagate", "(Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/comp/Attr$ResultInfo;Lcom/sun/tools/javac/comp/InferenceContext;)Z", nullptr, $PRIVATE, $method(Infer, shouldPropagate, bool, $Type*, $Attr$ResultInfo*, $InferenceContext*)},
		{}
	};
	...
	$loadClass(Infer, name, initialize, &classInfo$$, Infer::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Infer);
	});
	return class$;
}
