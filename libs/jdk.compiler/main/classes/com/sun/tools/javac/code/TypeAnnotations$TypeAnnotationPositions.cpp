...
$Class* TypeAnnotations$TypeAnnotationPositions$$Lambda$lambda$visitClassDef$0::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(Lcom/sun/tools/javac/code/TypeAnnotations$TypeAnnotationPositions;)V", nullptr, $PUBLIC, $method(TypeAnnotations$TypeAnnotationPositions$$Lambda$lambda$visitClassDef$0, init$, void, TypeAnnotations$TypeAnnotationPositions*)},
		{"accept", "(Ljava/lang/Object;)V", nullptr, $PUBLIC, $virtualMethod(TypeAnnotations$TypeAnnotationPositions$$Lambda$lambda$visitClassDef$0, accept, void, Object$*)},
		{}
	};
	$ClassInfo classInfo$$ = {
		$PUBLIC | $FINAL,
	...
	$loadClass(TypeAnnotations$TypeAnnotationPositions$$Lambda$lambda$visitClassDef$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TypeAnnotations$TypeAnnotationPositions$$Lambda$lambda$visitClassDef$0);
	});
	return class$;
}
...
$JCTree* TypeAnnotations$TypeAnnotationPositions::pop() {
	...
	$set(this, frames, this->frames->tail);
	return t;
}
...
void TypeAnnotations$TypeAnnotationPositions::separateAnnotationsKinds($JCTree* typetree, $Type* type$renamed, $Symbol* sym, $TypeAnnotationPosition* pos) {
	...
	if (type == nullptr) {
		$assign(type, $cast($Type, $$sure($Symbol, sym->getEnclosingElement())->asType()));
	...
	} else if (sym->getKind() == $ElementKind::PARAMETER && this->currentLambda == nullptr) {
		$set(sym, type, type);
		if ($$nc(sym->getQualifiedName())->equals($nc(this->this$0->names)->_this)) {
		...
		if (type->getKind() == $TypeKind::UNION) {
			...
			while (true) {
				...
				if (var$0) {
					...
					bool var$3 = var$4 || enclTr->getKind() == $Tree$Kind::PARAMETERIZED_TYPE;
					var$0 = var$3 || enclTr->getKind() == $Tree$Kind::ANNOTATED_TYPE;
				}
				if (!(var$0)) {
					break;
				}
				{
					$init($Tree$Kind);
					if ($nc(enclTr)->getKind() == $Tree$Kind::MEMBER_SELECT) {
						...
						$assign(enclTr, $cast($JCTree$JCTypeApply, enclTr)->getType());
					} else {
						$assign(enclTr, $cast($JCTree, $cast($JCTree$JCAnnotatedType, enclTr)->getUnderlyingType()));
					}
				}
			}
			...
			while (true) {
				...
				{
					$assign(topTy, $nc(topTy)->getEnclosingType());
					$assign(enclEl, $nc(enclEl)->getEnclosingElement());
			...
			if ($nc(depth)->nonEmpty()) {
				$set($nc(pos), location, $nc($nc(pos)->location)->appendList($(depth->toList())));
			}
...
$Type* TypeAnnotations$TypeAnnotationPositions::rewriteArrayType($Type$ArrayType* type, $List* annotations, $TypeAnnotationPosition* pos) {
	...
	if ($nc(tmpType)->getMetadata() != nullptr) {
		...
		if ($$sure($List, tmpType->getAnnotationMirrors())->isEmpty()) {
		...
		$assign(elemType, tmpType->cloneWithMetadata($$new($TypeMetadata, $$new($TypeMetadata$Annotations, annotations))));
	}
...
$Type* TypeAnnotations$TypeAnnotationPositions::typeWithAnnotations($Type* type, $Type* stopAt, $List* annotations) {
	...
	return $cast($Type, $nc(type)->accept(visitor, annotations));
}
...
$TypeAnnotationPosition* TypeAnnotations$TypeAnnotationPositions::resolveFrame($JCTree* tree, $JCTree* frame, $List* path, $JCTree$JCLambda* currentLambda, int32_t outer_type_index, $ListBuffer* location$renamed) {
	...
	case 8:
	case 9:
		if ($equals($cast($JCTree$JCClassDecl, frame)->extending, tree)) {
			return $TypeAnnotationPosition::classExtends($($nc(location)->toList()), currentLambda, frame->pos$);
	...
	case 11:
		{
			...
			if ($equals($cast($JCTree$JCTypeApply, frame)->clazz, tree)) {
				...
				$init($JCTree$Tag);
				if ($nc(newPath)->tail != nullptr && $nc($cast($JCTree, newPath->tail->head))->hasTag($JCTree$Tag::NEWCLASS)) {
					$assign(typeToUse, $nc(($cast($JCTree, newPath->tail->head)))->type);
				} else {
			...
			return resolveFrame($cast($JCTree, $nc(newPath)->head), $cast($JCTree, $nc($nc(newPath)->tail)->head), newPath, currentLambda, outer_type_index, location);
		}
	case 12:
		{
			...
			} else if (mrframe->typeargs != nullptr && mrframe->typeargs->contains(tree)) {
				int32_t type_index = mrframe->typeargs->indexOf(tree);
				switch ($nc($TypeAnnotations$1::$SwitchMap$com$sun$source$tree$MemberReferenceTree$ReferenceMode)->get($nc((mrframe->mode))->ordinal())) {
				case 1:
	...
	case 14:
		...
		if ($nc($cast($JCTree, $nc($nc($nc(path)->tail)->tail)->head))->hasTag($JCTree$Tag::CLASSDEF)) {
			...
			int32_t parameter_index = $nc($nc(clazz)->typarams)->indexOf(path->tail->head);
			int32_t bound_index = $nc($nc(($$cast($JCTree$JCExpression, $nc($cast($JCTree$JCTypeParameter, frame)->bounds)->get(0))))->type)->isInterface() ? $cast($JCTree$JCTypeParameter, frame)->bounds->indexOf(tree) + 1 : $cast($JCTree$JCTypeParameter, frame)->bounds->indexOf(tree);
			return $TypeAnnotationPosition::typeParameterBound($($nc(location)->toList()), currentLambda, parameter_index, bound_index, frame->pos$);
		} else if ($nc($cast($JCTree, $nc($nc(path->tail)->tail)->head))->hasTag($JCTree$Tag::METHODDEF)) {
			$var($JCTree$JCMethodDecl, method, $cast($JCTree$JCMethodDecl, $nc($nc(path->tail)->tail)->head));
			int32_t parameter_index = $nc($nc(method)->typarams)->indexOf(path->tail->head);
			int32_t bound_index = $nc($nc(($$cast($JCTree$JCExpression, $nc($cast($JCTree$JCTypeParameter, frame)->bounds)->get(0))))->type)->isInterface() ? $cast($JCTree$JCTypeParameter, frame)->bounds->indexOf(tree) + 1 : $cast($JCTree$JCTypeParameter, frame)->bounds->indexOf(tree);
	...
	case 15:
		$assign(v, $cast($JCTree$JCVariableDecl, frame)->sym);
		$init($ElementKind);
		if ($nc(v)->getKind() != $ElementKind::FIELD) {
			appendTypeAnnotationsToOwner(v, $(v->getRawTypeAttributes()));
		}
		switch ($nc($TypeAnnotations$1::$SwitchMap$javax$lang$model$element$ElementKind)->get(($$nc(v->getKind()))->ordinal())) {
		case 1:
		case 2:
		...
		case 4:
			if ($$nc(v->getQualifiedName())->equals($nc(this->this$0->names)->_this)) {
		...
		default:
			{
				...
				var$0->append("Found unexpected type annotation for variable: "_s);
				var$0->append(v);
				var$0->append(" with kind: "_s);
				var$0->append($(v->getKind()));
	...
	case 18:
		{
			...
			$var($List, newPath, $nc(path)->tail);
			return resolveFrame($cast($JCTree, $nc(newPath)->head), $cast($JCTree, $nc($nc(newPath)->tail)->head), newPath, currentLambda, $nc(isect->bounds)->indexOf(tree), location);
		}
	case 19:
		{
	...
	case 21:
		{
			$var($List, newPath, $nc(path)->tail);
			$init($TypeAnnotationPosition$TypePathEntry);
			return resolveFrame($cast($JCTree, $nc(newPath)->head), $cast($JCTree, $nc($nc(newPath)->tail)->head), newPath, currentLambda, outer_type_index, $($nc(location)->prepend($TypeAnnotationPosition$TypePathEntry::WILDCARD)));
		}
	case 22:
		{
			$var($List, newPath, $nc(path)->tail);
	...
	default:
		{
			...
			var$3->append("Unresolved frame: "_s);
			var$3->append(frame);
			var$3->append(" of kind: "_s);
			var$3->append($(frame->getKind()));
			var$3->append("\n    Looking for tree: "_s);
			var$3->append(tree);
			$throwNew($AssertionError, $$of($str(var$3)));
		}
	}
}
...
$ListBuffer* TypeAnnotations$TypeAnnotationPositions::locateNestedTypes($Type* type, $ListBuffer* depth$renamed) {
	...
	while (true) {
		...
		}
		{
			...
			$assign(encl, $nc(encl)->getEnclosingType());
		}
	}
	return depth;
}
...
int32_t TypeAnnotations$TypeAnnotationPositions::methodParamIndex($List* path, $JCTree* param) {
	...
	while (true) {
		...
		if (!(var$0 && $cast($JCTree, curr->head)->getTag() != $JCTree$Tag::LAMBDA)) {
			break;
		}
		{
	...
	} else if ($nc($cast($JCTree, curr->head))->getTag() == $JCTree$Tag::LAMBDA) {
...
void TypeAnnotations$TypeAnnotationPositions::visitClassDef($JCTree$JCClassDecl* tree) {
	...
	scan($nc(tree)->defs);
	if ($nc(tree->sym)->isRecord()) {
		$$nc($$nc($nc(tree->sym)->getRecordComponents())->stream())->forEach($$new(TypeAnnotations$TypeAnnotationPositions$$Lambda$lambda$visitClassDef$0, this));
	}
}
...
void TypeAnnotations$TypeAnnotationPositions::visitMethodDef($JCTree$JCMethodDecl* tree) {
	...
	if (this->sigOnly) {
		if (!$nc($nc(tree->mods)->annotations)->isEmpty()) {
		...
		if (tree->recvparam != nullptr && tree->recvparam->sym != nullptr && !$nc($nc(tree->recvparam->mods)->annotations)->isEmpty()) {
			$var($TypeAnnotationPosition, pos, $TypeAnnotationPosition::methodReceiver($nc(tree->recvparam->vartype)->pos$));
			push(tree->recvparam);
			$var($Throwable, var$0, nullptr);
			try {
...
void TypeAnnotations$TypeAnnotationPositions::visitVarDef($JCTree$JCVariableDecl* tree) {
	...
	if ($nc($nc($nc(tree)->mods)->annotations)->isEmpty()) {
		...
		$init($ElementKind);
		if (tree->sym->getKind() == $ElementKind::PARAMETER) {
		} else if ($nc(tree->sym)->getKind() == $ElementKind::FIELD) {
			if (this->sigOnly) {
...
void TypeAnnotations$TypeAnnotationPositions::propagateNewClassAnnotationsToOwner($JCTree$JCNewClass* tree) {
	...
	{
		$var($Iterator, i$, $$nc(sym->getRawTypeAttributes())->iterator());
		for (; $nc(i$)->hasNext();) {
...
void TypeAnnotations$TypeAnnotationPositions::visitNewClass($JCTree$JCNewClass* tree) {
	if ($nc(tree)->def != nullptr && tree->def->sym != nullptr) {
...
void TypeAnnotations$TypeAnnotationPositions::visitNewArray($JCTree$JCNewArray* tree) {
	...
	for (int32_t i = 0; i < dimAnnosCount; ++i) {
		...
		if (i != 0) {
			...
			$assign(location, location->appendList($($nc(depth)->toList())));
		}
		$var($TypeAnnotationPosition, p, $TypeAnnotationPosition::newObj($($nc(location)->toList()), this->currentLambda, tree->pos$));
	...
	while (elemType != nullptr) {
		...
		if (elemType->hasTag($JCTree$Tag::ANNOTATED_TYPE)) {
			...
			$var($List, location, $$nc($nc(locationbuf)->toList())->prependList($($nc(depth)->toList())));
			...
			$assign(depth, $nc(depth)->append($TypeAnnotationPosition$TypePathEntry::ARRAY));
			$assign(elemType, $cast($JCTree$JCArrayTypeTree, elemType)->elemtype);
		} else if (elemType->hasTag($JCTree$Tag::SELECT)) {
			$assign(elemType, $cast($JCTree$JCFieldAccess, elemType)->selected);
		} else {
			break;
		}
	}
...
void TypeAnnotations$TypeAnnotationPositions::setTypeAnnotationPos($List* annotations, $TypeAnnotationPosition* position) {
	...
	for (; $nc(i$)->hasNext();) {
		...
		if ($nc(anno)->attribute != nullptr) {
			$set($cast($Attribute$TypeCompound, anno->attribute), position, position);
		}
	}
}
...
$Class* TypeAnnotations$TypeAnnotationPositions::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.code.TypeAnnotations$TypeAnnotationPositions$$Lambda$lambda$visitClassDef$0")) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(Lcom/sun/tools/javac/code/TypeAnnotations;Z)V", nullptr, 0, $method(TypeAnnotations$TypeAnnotationPositions, init$, void, $TypeAnnotations*, bool)},
		{"appendTypeAnnotationsToOwner", "(Lcom/sun/tools/javac/code/Symbol;Lcom/sun/tools/javac/util/List;)V", "(Lcom/sun/tools/javac/code/Symbol;Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/code/Attribute$TypeCompound;>;)V", $PRIVATE, $method(TypeAnnotations$TypeAnnotationPositions, appendTypeAnnotationsToOwner, void, $Symbol*, $List*)},
		{"findPosition", "(Lcom/sun/tools/javac/tree/JCTree;Lcom/sun/tools/javac/tree/JCTree;Lcom/sun/tools/javac/util/List;)V", "(Lcom/sun/tools/javac/tree/JCTree;Lcom/sun/tools/javac/tree/JCTree;Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/tree/JCTree$JCAnnotation;>;)V", $PRIVATE, $method(TypeAnnotations$TypeAnnotationPositions, findPosition, void, $JCTree*, $JCTree*, $List*)},
		{"lambda$visitClassDef$0", "(Lcom/sun/tools/javac/code/Symbol$RecordComponent;)V", nullptr, $PRIVATE | $SYNTHETIC, $method(TypeAnnotations$TypeAnnotationPositions, lambda$visitClassDef$0, void, $Symbol$RecordComponent*)},
		{"locateNestedTypes", "(Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/util/ListBuffer;)Lcom/sun/tools/javac/util/ListBuffer;", "(Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/util/ListBuffer<Lcom/sun/tools/javac/code/TypeAnnotationPosition$TypePathEntry;>;)Lcom/sun/tools/javac/util/ListBuffer<Lcom/sun/tools/javac/code/TypeAnnotationPosition$TypePathEntry;>;", $PRIVATE, $method(TypeAnnotations$TypeAnnotationPositions, locateNestedTypes, $ListBuffer*, $Type*, $ListBuffer*)},
		{"methodParamIndex", "(Lcom/sun/tools/javac/util/List;Lcom/sun/tools/javac/tree/JCTree;)I", "(Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/tree/JCTree;>;Lcom/sun/tools/javac/tree/JCTree;)I", $PRIVATE, $method(TypeAnnotations$TypeAnnotationPositions, methodParamIndex, int32_t, $List*, $JCTree*)},
		{"peek2", "()Lcom/sun/tools/javac/tree/JCTree;", nullptr, $PRIVATE, $method(TypeAnnotations$TypeAnnotationPositions, peek2, $JCTree*)},
		{"pop", "()Lcom/sun/tools/javac/tree/JCTree;", nullptr, $PROTECTED, $virtualMethod(TypeAnnotations$TypeAnnotationPositions, pop, $JCTree*)},
		{"propagateNewClassAnnotationsToOwner", "(Lcom/sun/tools/javac/tree/JCTree$JCNewClass;)V", nullptr, $PRIVATE, $method(TypeAnnotations$TypeAnnotationPositions, propagateNewClassAnnotationsToOwner, void, $JCTree$JCNewClass*)},
		{"push", "(Lcom/sun/tools/javac/tree/JCTree;)V", nullptr, $PROTECTED, $virtualMethod(TypeAnnotations$TypeAnnotationPositions, push, void, $JCTree*)},
		{"resolveFrame", "(Lcom/sun/tools/javac/tree/JCTree;Lcom/sun/tools/javac/tree/JCTree;Lcom/sun/tools/javac/util/List;Lcom/sun/tools/javac/tree/JCTree$JCLambda;ILcom/sun/tools/javac/util/ListBuffer;)Lcom/sun/tools/javac/code/TypeAnnotationPosition;", "(Lcom/sun/tools/javac/tree/JCTree;Lcom/sun/tools/javac/tree/JCTree;Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/tree/JCTree;>;Lcom/sun/tools/javac/tree/JCTree$JCLambda;ILcom/sun/tools/javac/util/ListBuffer<Lcom/sun/tools/javac/code/TypeAnnotationPosition$TypePathEntry;>;)Lcom/sun/tools/javac/code/TypeAnnotationPosition;", $PRIVATE, $method(TypeAnnotations$TypeAnnotationPositions, resolveFrame, $TypeAnnotationPosition*, $JCTree*, $JCTree*, $List*, $JCTree$JCLambda*, int32_t, $ListBuffer*)},
		{"rewriteArrayType", "(Lcom/sun/tools/javac/code/Type$ArrayType;Lcom/sun/tools/javac/util/List;Lcom/sun/tools/javac/code/TypeAnnotationPosition;)Lcom/sun/tools/javac/code/Type;", "(Lcom/sun/tools/javac/code/Type$ArrayType;Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/code/Attribute$TypeCompound;>;Lcom/sun/tools/javac/code/TypeAnnotationPosition;)Lcom/sun/tools/javac/code/Type;", $PRIVATE, $method(TypeAnnotations$TypeAnnotationPositions, rewriteArrayType, $Type*, $Type$ArrayType*, $List*, $TypeAnnotationPosition*)},
		{"scan", "(Lcom/sun/tools/javac/tree/JCTree;)V", nullptr, $PUBLIC, $virtualMethod(TypeAnnotations$TypeAnnotationPositions, scan, void, $JCTree*)},
		{"separateAnnotationsKinds", "(Lcom/sun/tools/javac/tree/JCTree;Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/code/Symbol;Lcom/sun/tools/javac/code/TypeAnnotationPosition;)V", nullptr, $PRIVATE, $method(TypeAnnotations$TypeAnnotationPositions, separateAnnotationsKinds, void, $JCTree*, $Type*, $Symbol*, $TypeAnnotationPosition*)},
		{"setTypeAnnotationPos", "(Lcom/sun/tools/javac/util/List;Lcom/sun/tools/javac/code/TypeAnnotationPosition;)V", "(Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/tree/JCTree$JCAnnotation;>;Lcom/sun/tools/javac/code/TypeAnnotationPosition;)V", $PRIVATE, $method(TypeAnnotations$TypeAnnotationPositions, setTypeAnnotationPos, void, $List*, $TypeAnnotationPosition*)},
		{"toString", "()Ljava/lang/String;", nullptr, $PUBLIC, $virtualMethod(TypeAnnotations$TypeAnnotationPositions, toString, $String*)},
		{"toTypeCompound", "(Lcom/sun/tools/javac/code/Attribute$Compound;Lcom/sun/tools/javac/code/TypeAnnotationPosition;)Lcom/sun/tools/javac/code/Attribute$TypeCompound;", nullptr, $PRIVATE, $method(TypeAnnotations$TypeAnnotationPositions, toTypeCompound, $Attribute$TypeCompound*, $Attribute$Compound*, $TypeAnnotationPosition*)},
		{"typeWithAnnotations", "(Lcom/sun/tools/javac/tree/JCTree;Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/util/List;Lcom/sun/tools/javac/util/List;Lcom/sun/tools/javac/code/TypeAnnotationPosition;)Lcom/sun/tools/javac/code/Type;", "(Lcom/sun/tools/javac/tree/JCTree;Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/code/Attribute$TypeCompound;>;Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/code/Attribute$TypeCompound;>;Lcom/sun/tools/javac/code/TypeAnnotationPosition;)Lcom/sun/tools/javac/code/Type;", $PRIVATE, $method(TypeAnnotations$TypeAnnotationPositions, typeWithAnnotations, $Type*, $JCTree*, $Type*, $List*, $List*, $TypeAnnotationPosition*)},
		{"typeWithAnnotations", "(Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/util/List;)Lcom/sun/tools/javac/code/Type;", "(Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/code/Type;Lcom/sun/tools/javac/util/List<Lcom/sun/tools/javac/code/Attribute$TypeCompound;>;)Lcom/sun/tools/javac/code/Type;", $PRIVATE, $method(TypeAnnotations$TypeAnnotationPositions, typeWithAnnotations, $Type*, $Type*, $Type*, $List*)},
		{"visitAnnotatedType", "(Lcom/sun/tools/javac/tree/JCTree$JCAnnotatedType;)V", nullptr, $PUBLIC, $virtualMethod(TypeAnnotations$TypeAnnotationPositions, visitAnnotatedType, void, $JCTree$JCAnnotatedType*)},
		{"visitBlock", "(Lcom/sun/tools/javac/tree/JCTree$JCBlock;)V", nullptr, $PUBLIC, $virtualMethod(TypeAnnotations$TypeAnnotationPositions, visitBlock, void, $JCTree$JCBlock*)},
		{"visitClassDef", "(Lcom/sun/tools/javac/tree/JCTree$JCClassDecl;)V", nullptr, $PUBLIC, $virtualMethod(TypeAnnotations$TypeAnnotationPositions, visitClassDef, void, $JCTree$JCClassDecl*)},
		{"visitLambda", "(Lcom/sun/tools/javac/tree/JCTree$JCLambda;)V", nullptr, $PUBLIC, $virtualMethod(TypeAnnotations$TypeAnnotationPositions, visitLambda, void, $JCTree$JCLambda*)},
		{"visitMethodDef", "(Lcom/sun/tools/javac/tree/JCTree$JCMethodDecl;)V", nullptr, $PUBLIC, $virtualMethod(TypeAnnotations$TypeAnnotationPositions, visitMethodDef, void, $JCTree$JCMethodDecl*)},
		{"visitNewArray", "(Lcom/sun/tools/javac/tree/JCTree$JCNewArray;)V", nullptr, $PUBLIC, $virtualMethod(TypeAnnotations$TypeAnnotationPositions, visitNewArray, void, $JCTree$JCNewArray*)},
		{"visitNewClass", "(Lcom/sun/tools/javac/tree/JCTree$JCNewClass;)V", nullptr, $PUBLIC, $virtualMethod(TypeAnnotations$TypeAnnotationPositions, visitNewClass, void, $JCTree$JCNewClass*)},
		{"visitTypeParameter", "(Lcom/sun/tools/javac/tree/JCTree$JCTypeParameter;)V", nullptr, $PUBLIC, $virtualMethod(TypeAnnotations$TypeAnnotationPositions, visitTypeParameter, void, $JCTree$JCTypeParameter*)},
		{"visitVarDef", "(Lcom/sun/tools/javac/tree/JCTree$JCVariableDecl;)V", nullptr, $PUBLIC, $virtualMethod(TypeAnnotations$TypeAnnotationPositions, visitVarDef, void, $JCTree$JCVariableDecl*)},
		{}
	};
	...
	$loadClass(TypeAnnotations$TypeAnnotationPositions, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TypeAnnotations$TypeAnnotationPositions);
	});
	return class$;
}
