...
$Type* Infer$InferenceStep$2::solve($Type$UndetVar* uv, $InferenceContext* inferenceContext) {
	...
	$var($Type, owntype, $nc($nc(lobounds)->tail)->tail == nullptr ? $cast($Type, lobounds->head) : $nc($nc(infer)->types)->lub(lobounds));
	bool var$0 = $nc(owntype)->isPrimitive();
	$init($TypeTag);
	if (var$0 || owntype->hasTag($TypeTag::ERROR)) {
		$throw($($nc(infer)->error($($nc($nc(infer)->diags)->fragment($($CompilerProperties$Fragments::NoUniqueMinimalInstanceExists($nc(uv)->qtype, lobounds)))))));
	} else {
		return owntype;
	}
}
...
$Class* Infer$InferenceStep$2::load$($String* name, bool initialize) {
	...
	$loadClass(Infer$InferenceStep$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
