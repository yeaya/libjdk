...
void LambdaToMethod$KlassInfo::init$($LambdaToMethod* this$0, $JCTree$JCClassDecl* clazz) {
	...
	$var($Type, var$1, this$0->syms->objectType);
	$var($Type$MethodType, type, $new($Type$MethodType, var$0, var$1, $($List::nil()), this$0->syms->methodClass));
...
$Class* LambdaToMethod$KlassInfo::load$($String* name, bool initialize) {
	...
	$loadClass(LambdaToMethod$KlassInfo, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LambdaToMethod$KlassInfo);
	});
	return class$;
}
