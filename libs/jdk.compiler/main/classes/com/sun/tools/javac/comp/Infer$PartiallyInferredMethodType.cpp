	...
	try {
		try {
			...
			if (!unchecked) {
				...
				$var($InferenceContext, minContext, shouldPropagate ? this->inferenceContext->min($(this->this$0->roots($(asMethodType()), nullptr)), false, this->warn) : this->inferenceContext);
				...
				if (shouldPropagate) {
					...
					minContext->dupTo(var$3, $$nc(resultInfo->checkContext->deferredAttrContext())->insideOverloadPhase());
			...
			$var($Type, ret, $$nc(this->inferenceContext->asInstType(this))->getReturnType());
			if (unchecked) {
...
$Class* Infer$PartiallyInferredMethodType::load$($String* name, bool initialize) {
	...
	$loadClass(Infer$PartiallyInferredMethodType, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
