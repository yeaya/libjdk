...
void JNIWriter::init$($Context* context) {
	$nc(context)->put(JNIWriter::jniWriterKey, this);
...
bool JNIWriter::needsHeader($Symbol$ClassSymbol* c) {
	...
	return (this->checkAll) ? needsHeader($(c->outermostClass()), true) : needsHeader(c, false);
}
	...
	}
	{
		...
		for (; $nc(i$)->hasNext();) {
			$var($Symbol, sym, $cast($Symbol, i$->next()));
			{
				...
				{
					$var($Iterator, i$, $$nc(sym->getDeclarationAttributes())->iterator());
					for (; $nc(i$)->hasNext();) {
...
$FileObject* JNIWriter::write($Symbol$ClassSymbol* c) {
	...
	if (this->multiModuleMode) {
		...
		$assign(outLocn, $nc(this->fileManager)->getLocationForModule($StandardLocation::NATIVE_HEADER_OUTPUT, $($nc($nc(msym)->name)->toString())));
	} else {
		$init($StandardLocation);
...
void JNIWriter::writeStatics($PrintWriter* out, $Symbol$ClassSymbol* sym) {
	$useLocalObjectStack();
	$var($List, clist, $new($ArrayList));
	{
		$var($Symbol$ClassSymbol, cd, sym);
		for (; cd != nullptr; $assign(cd, $cast($Symbol$ClassSymbol, $nc($(cd->getSuperclass()))->tsym))) {
			clist->add(cd);
		}
	}
	$Collections::reverse(clist);
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			{
				$var($Iterator, i$, $$sure($1List, $nc(cd)->getEnclosedElements())->iterator());
				for (; $nc(i$)->hasNext();) {
...
void JNIWriter::printStaticDefines($PrintWriter* out, $Pair* p) {
	...
	$var($String, valueStr, nullptr);
	{
		...
		float fv = 0;
		double d = 0;
		switch ($nc($JNIWriter$1::$SwitchMap$javax$lang$model$type$TypeKind)->get(($$nc($$sure($Type, f->asType())->getKind()))->ordinal())) {
		case 1:
		...
		case 3:
		case 4:
			$assign(valueStr, $str({$($nc(value)->toString()), "L"_s}));
			break;
		case 5:
			$assign(valueStr, $str({$($nc(value)->toString()), ((JNIWriter::isWindows) ? "i64"_s : "LL"_s)}));
			break;
		case 6:
			$assign(ch, $cast($Character, value));
			$assign(valueStr, $str({$($String::valueOf(((int32_t)$nc(ch)->charValue()) & 0xffff)), "L"_s}));
			break;
		case 7:
			fv = $nc($cast($Float, value))->floatValue();
			$assign(valueStr, ($Float::isInfinite(fv)) ? $str({((fv < 0) ? "-"_s : ""_s), "Inff"_s}) : $str({$(value->toString()), "f"_s}));
			break;
		case 8:
			d = $nc($cast($Double, value))->doubleValue();
			$assign(valueStr, ($Double::isInfinite(d)) ? $str({((d < 0) ? "-"_s : ""_s), "InfD"_s}) : value->toString());
			break;
		default:
...
void JNIWriter::writeMethods($PrintWriter* out, $Symbol$ClassSymbol* sym, $String* cname) {
	$useLocalObjectStack();
	$var($List, classmethods, $nc(sym)->getEnclosedElements());
	{
			...
			if (isNative(md)) {
				...
				bool isOverloaded = false;
				{
					...
					for (; $nc(i$)->hasNext();) {
						...
						{
							bool var$0 = (md2 != md) && ($nc(methodName)->equals($($nc(md2)->getSimpleName())));
				...
				var$1->append("JNIEXPORT "_s);
				var$1->append($(jniType($($nc(this->types)->erasure($($nc(md->type)->getReturnType()))))));
				var$1->append(" JNICALL "_s);
				var$1->append($(encodeMethod(md, sym, isOverloaded)));
				out->println($$str(var$1));
				out->print("  (JNIEnv *, "_s);
...
$String* JNIWriter::jniType($Type* t) {
	...
	case 9:
		{
			$var($Type, ct, $cast($Type, $cast($Type$ArrayType, t)->getComponentType()));
			switch ($JNIWriter$1::$SwitchMap$javax$lang$model$type$TypeKind->get(($$nc($nc(ct)->getKind()))->ordinal())) {
			case 1:
	...
	case 10:
		{
			if ($nc(t->tsym)->type == $nc(this->syms)->stringType) {
				return "jstring"_s;
			} else if ($nc(this->types)->isAssignable(t, this->syms->throwableType)) {
...
$String* JNIWriter::encodeMethod($Symbol* msym, $Symbol$ClassSymbol* clazz, bool isOverloaded) {
	...
	if (isOverloaded) {
		...
		$var($StringBuilder, sig, typeSig->getParameterSignature(msym->type, true));
...
$String* JNIWriter::encodeChar(char16_t ch) {
	...
	for (int32_t i = nzeros + 1, j = 0; i < 6; ++i, ++j) {
...
void JNIWriter::clinit$($Class* clazz) {
...
$Class* JNIWriter::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(Lcom/sun/tools/javac/util/Context;)V", nullptr, $PRIVATE, $method(JNIWriter, init$, void, $Context*)},
		{"cppGuardBegin", "(Ljava/io/PrintWriter;)V", nullptr, $PROTECTED, $virtualMethod(JNIWriter, cppGuardBegin, void, $PrintWriter*)},
		{"cppGuardEnd", "(Ljava/io/PrintWriter;)V", nullptr, $PROTECTED, $virtualMethod(JNIWriter, cppGuardEnd, void, $PrintWriter*)},
		{"encode", "(Ljava/lang/CharSequence;Lcom/sun/tools/javac/jvm/JNIWriter$EncoderType;)Ljava/lang/String;", nullptr, $STATIC, $staticMethod(JNIWriter, encode, $String*, $CharSequence*, $JNIWriter$EncoderType*)},
		{"encodeChar", "(C)Ljava/lang/String;", nullptr, $STATIC, $staticMethod(JNIWriter, encodeChar, $String*, char16_t)},
		{"encodeMethod", "(Lcom/sun/tools/javac/code/Symbol;Lcom/sun/tools/javac/code/Symbol$ClassSymbol;Z)Ljava/lang/String;", nullptr, 0, $virtualMethod(JNIWriter, encodeMethod, $String*, $Symbol*, $Symbol$ClassSymbol*, bool), "com.sun.tools.javac.jvm.JNIWriter$TypeSignature$SignatureException"},
		{"fileTop", "(Ljava/io/PrintWriter;)V", nullptr, $PROTECTED, $virtualMethod(JNIWriter, fileTop, void, $PrintWriter*)},
		{"guardBegin", "(Ljava/io/PrintWriter;Ljava/lang/String;)V", nullptr, $PROTECTED, $virtualMethod(JNIWriter, guardBegin, void, $PrintWriter*, $String*)},
		{"guardEnd", "(Ljava/io/PrintWriter;)V", nullptr, $PROTECTED, $virtualMethod(JNIWriter, guardEnd, void, $PrintWriter*)},
		{"hasFlag", "(Lcom/sun/tools/javac/code/Symbol;I)Z", nullptr, $PRIVATE | $STATIC, $staticMethod(JNIWriter, hasFlag, bool, $Symbol*, int32_t)},
		{"includes", "(Ljava/io/PrintWriter;)V", nullptr, $PROTECTED, $virtualMethod(JNIWriter, includes, void, $PrintWriter*)},
		{"instance", "(Lcom/sun/tools/javac/util/Context;)Lcom/sun/tools/javac/jvm/JNIWriter;", nullptr, $PUBLIC | $STATIC, $staticMethod(JNIWriter, instance, JNIWriter*, $Context*)},
		{"isFinal", "(Lcom/sun/tools/javac/code/Symbol;)Z", nullptr, $STATIC, $staticMethod(JNIWriter, isFinal, bool, $Symbol*)},
		{"isNative", "(Lcom/sun/tools/javac/code/Symbol;)Z", nullptr, $STATIC, $staticMethod(JNIWriter, isNative, bool, $Symbol*)},
		{"isStatic", "(Lcom/sun/tools/javac/code/Symbol;)Z", nullptr, $STATIC, $staticMethod(JNIWriter, isStatic, bool, $Symbol*)},
		{"isSynthetic", "(Lcom/sun/tools/javac/code/Symbol;)Z", nullptr, $STATIC, $staticMethod(JNIWriter, isSynthetic, bool, $Symbol*)},
		{"isalnum", "(C)Z", nullptr, $PRIVATE | $STATIC, $staticMethod(JNIWriter, isalnum, bool, char16_t)},
		{"isprint", "(C)Z", nullptr, $PRIVATE | $STATIC, $staticMethod(JNIWriter, isprint, bool, char16_t)},
		{"jniType", "(Lcom/sun/tools/javac/code/Type;)Ljava/lang/String;", nullptr, $PROTECTED | $FINAL, $method(JNIWriter, jniType, $String*, $Type*)},
		{"lazyInit", "()V", nullptr, $PRIVATE, $method(JNIWriter, lazyInit, void)},
		{"needsHeader", "(Lcom/sun/tools/javac/code/Symbol$ClassSymbol;)Z", nullptr, $PUBLIC, $virtualMethod(JNIWriter, needsHeader, bool, $Symbol$ClassSymbol*)},
		{"needsHeader", "(Lcom/sun/tools/javac/code/Symbol$ClassSymbol;Z)Z", nullptr, $PRIVATE, $method(JNIWriter, needsHeader, bool, $Symbol$ClassSymbol*, bool)},
		{"printStaticDefines", "(Ljava/io/PrintWriter;Lcom/sun/tools/javac/util/Pair;)V", "(Ljava/io/PrintWriter;Lcom/sun/tools/javac/util/Pair<Lcom/sun/tools/javac/code/Symbol$ClassSymbol;Lcom/sun/tools/javac/code/Symbol$VarSymbol;>;)V", $STATIC, $staticMethod(JNIWriter, printStaticDefines, void, $PrintWriter*, $Pair*)},
		{"write", "(Lcom/sun/tools/javac/code/Symbol$ClassSymbol;)Ljavax/tools/FileObject;", nullptr, $PUBLIC, $virtualMethod(JNIWriter, write, $FileObject*, $Symbol$ClassSymbol*), "java.io.IOException"},
		{"write", "(Ljava/io/PrintWriter;Lcom/sun/tools/javac/code/Symbol$ClassSymbol;)V", nullptr, $PUBLIC, $virtualMethod(JNIWriter, write, void, $PrintWriter*, $Symbol$ClassSymbol*), "java.io.IOException"},
		{"writeMethods", "(Ljava/io/PrintWriter;Lcom/sun/tools/javac/code/Symbol$ClassSymbol;Ljava/lang/String;)V", nullptr, $PROTECTED, $virtualMethod(JNIWriter, writeMethods, void, $PrintWriter*, $Symbol$ClassSymbol*, $String*), "java.io.IOException,com.sun.tools.javac.jvm.JNIWriter$TypeSignature$SignatureException"},
		{"writeStatics", "(Ljava/io/PrintWriter;Lcom/sun/tools/javac/code/Symbol$ClassSymbol;)V", nullptr, $PROTECTED, $virtualMethod(JNIWriter, writeStatics, void, $PrintWriter*, $Symbol$ClassSymbol*), "java.io.IOException"},
		{}
	};
	...
	$loadClass(JNIWriter, name, initialize, &classInfo$$, JNIWriter::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(JNIWriter);
	});
	return class$;
}
