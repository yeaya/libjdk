...
void PoolWriter$SharedSignatureGenerator::assembleSig($Type* type) {
	...
	case 1:
	case 2:
		assembleSig($($nc(this->this$0->types)->erasure($cast($UninitializedType, type)->qtype)));
		break;
	default:
...
$Class* PoolWriter$SharedSignatureGenerator::load$($String* name, bool initialize) {
	...
	$loadClass(PoolWriter$SharedSignatureGenerator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PoolWriter$SharedSignatureGenerator);
	});
	return class$;
}
