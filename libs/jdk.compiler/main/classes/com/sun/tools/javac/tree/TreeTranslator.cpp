...
void TreeTranslator::visitPackageDef($JCTree$JCPackageDecl* tree) {
	$set($nc(tree), annotations, translate($nc(tree)->annotations));
	$set(tree, pid, $cast($JCTree$JCExpression, translate(tree->pid)));
	$set(this, result, tree);
}

void TreeTranslator::visitImport($JCTree$JCImport* tree) {
	$set($nc(tree), qualid, translate($nc(tree)->qualid));
	$set(this, result, tree);
}
...
void TreeTranslator::visitMethodDef($JCTree$JCMethodDecl* tree) {
	...
	$set(tree, typarams, translateTypeParams(tree->typarams));
	$set(tree, recvparam, $cast($JCTree$JCVariableDecl, translate(tree->recvparam)));
...
void TreeTranslator::visitVarDef($JCTree$JCVariableDecl* tree) {
	$set($nc(tree), mods, $cast($JCTree$JCModifiers, translate($nc(tree)->mods)));
	$set(tree, nameexpr, $cast($JCTree$JCExpression, translate(tree->nameexpr)));
...
void TreeTranslator::visitForeachLoop($JCTree$JCEnhancedForLoop* tree) {
	$set($nc(tree), var, $cast($JCTree$JCVariableDecl, translate($nc(tree)->var)));
...
void TreeTranslator::visitSwitch($JCTree$JCSwitch* tree) {
	$set($nc(tree), selector, $cast($JCTree$JCExpression, translate($nc(tree)->selector)));
	$set(tree, cases, translateCases(tree->cases));
	$set(this, result, tree);
}

void TreeTranslator::visitCase($JCTree$JCCase* tree) {
	$set($nc(tree), labels, translate($nc(tree)->labels));
...
void TreeTranslator::visitSynchronized($JCTree$JCSynchronized* tree) {
	$set($nc(tree), lock, $cast($JCTree$JCExpression, translate($nc(tree)->lock)));
	$set(tree, body, $cast($JCTree$JCBlock, translate(tree->body)));
	$set(this, result, tree);
}

void TreeTranslator::visitTry($JCTree$JCTry* tree) {
	$set($nc(tree), resources, translate($nc(tree)->resources));
	$set(tree, body, $cast($JCTree$JCBlock, translate(tree->body)));
...
void TreeTranslator::visitCatch($JCTree$JCCatch* tree) {
	$set($nc(tree), param, $cast($JCTree$JCVariableDecl, translate($nc(tree)->param)));
	$set(tree, body, $cast($JCTree$JCBlock, translate(tree->body)));
	$set(this, result, tree);
}
...
void TreeTranslator::visitConditional($JCTree$JCConditional* tree) {
	$set($nc(tree), cond, $cast($JCTree$JCExpression, translate($nc(tree)->cond)));
	$set(tree, truepart, $cast($JCTree$JCExpression, translate(tree->truepart)));
	$set(tree, falsepart, $cast($JCTree$JCExpression, translate(tree->falsepart)));
	$set(this, result, tree);
}
...
void TreeTranslator::visitExec($JCTree$JCExpressionStatement* tree) {
	$set($nc(tree), expr, $cast($JCTree$JCExpression, translate($nc(tree)->expr)));
	$set(this, result, tree);
}
...
void TreeTranslator::visitYield($JCTree$JCYield* tree) {
	$set($nc(tree), value, $cast($JCTree$JCExpression, translate($nc(tree)->value)));
	$set(this, result, tree);
}
...
void TreeTranslator::visitAssert($JCTree$JCAssert* tree) {
	$set($nc(tree), cond, $cast($JCTree$JCExpression, translate($nc(tree)->cond)));
	$set(tree, detail, $cast($JCTree$JCExpression, translate(tree->detail)));
	$set(this, result, tree);
}

void TreeTranslator::visitApply($JCTree$JCMethodInvocation* tree) {
	$set($nc(tree), meth, $cast($JCTree$JCExpression, translate($nc(tree)->meth)));
	$set(tree, args, translate(tree->args));
	$set(this, result, tree);
}

void TreeTranslator::visitNewClass($JCTree$JCNewClass* tree) {
	$set($nc(tree), encl, $cast($JCTree$JCExpression, translate($nc(tree)->encl)));
	$set(tree, clazz, $cast($JCTree$JCExpression, translate(tree->clazz)));
	$set(tree, args, translate(tree->args));
	$set(tree, def, $cast($JCTree$JCClassDecl, translate(tree->def)));
	$set(this, result, tree);
}

void TreeTranslator::visitLambda($JCTree$JCLambda* tree) {
	$set($nc(tree), params, translate($nc(tree)->params));
...
void TreeTranslator::visitNewArray($JCTree$JCNewArray* tree) {
	...
	$set(tree, elemtype, $cast($JCTree$JCExpression, translate(tree->elemtype)));
...
void TreeTranslator::visitAssign($JCTree$JCAssign* tree) {
	$set($nc(tree), lhs, $cast($JCTree$JCExpression, translate($nc(tree)->lhs)));
	$set(tree, rhs, $cast($JCTree$JCExpression, translate(tree->rhs)));
	$set(this, result, tree);
}
...
void TreeTranslator::visitUnary($JCTree$JCUnary* tree) {
	$set($nc(tree), arg, $cast($JCTree$JCExpression, translate($nc(tree)->arg)));
	$set(this, result, tree);
}
...
void TreeTranslator::visitParenthesizedPattern($JCTree$JCParenthesizedPattern* tree) {
	$set($nc(tree), pattern, $cast($JCTree$JCPattern, translate($nc(tree)->pattern)));
	$set(this, result, tree);
}

void TreeTranslator::visitGuardPattern($JCTree$JCGuardPattern* tree) {
	$set($nc(tree), patt, $cast($JCTree$JCPattern, translate($nc(tree)->patt)));
	$set(tree, expr, $cast($JCTree$JCExpression, translate(tree->expr)));
	$set(this, result, tree);
}
...
void TreeTranslator::visitIndexed($JCTree$JCArrayAccess* tree) {
	...
	$set(tree, index, $cast($JCTree$JCExpression, translate(tree->index)));
	$set(this, result, tree);
}

void TreeTranslator::visitSelect($JCTree$JCFieldAccess* tree) {
	$set($nc(tree), selected, $cast($JCTree$JCExpression, translate($nc(tree)->selected)));
	$set(this, result, tree);
}
...
void TreeTranslator::visitWildcard($JCTree$JCWildcard* tree) {
	$set($nc(tree), kind, $cast($JCTree$TypeBoundKind, translate($nc(tree)->kind)));
...
void TreeTranslator::visitModifiers($JCTree$JCModifiers* tree) {
	$set($nc(tree), annotations, translateAnnotations($nc(tree)->annotations));
	$set(this, result, tree);
}

void TreeTranslator::visitAnnotation($JCTree$JCAnnotation* tree) {
	$set($nc(tree), annotationType, translate($nc(tree)->annotationType));
	$set(tree, args, translate(tree->args));
	$set(this, result, tree);
}
...
$Class* TreeTranslator::load$($String* name, bool initialize) {
	...
	$loadClass(TreeTranslator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TreeTranslator);
	});
	return class$;
}
