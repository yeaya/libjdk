...
void Flow$LambdaFlowAnalyzer::visitLambda($JCTree$JCLambda* tree) {
	$useLocalObjectStack();
	if (($nc(tree)->type != nullptr && tree->type->isErroneous()) || this->inLambda) {
		return;
	}
...
$Class* Flow$LambdaFlowAnalyzer::load$($String* name, bool initialize) {
	...
	$loadClass(Flow$LambdaFlowAnalyzer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Flow$LambdaFlowAnalyzer);
	});
	return class$;
}
