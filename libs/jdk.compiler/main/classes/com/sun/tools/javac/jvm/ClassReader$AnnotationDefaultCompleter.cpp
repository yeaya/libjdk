...
void ClassReader$AnnotationDefaultCompleter::init$($ClassReader* this$0, $Symbol$MethodSymbol* sym, $Attribute* value) {
	...
	$init($Kinds$Kind);
	$ClassReader$AnnotationDeproxy::init$(this$0, $nc(this$0->currentOwner)->kind == $Kinds$Kind::MTH ? $(this$0->currentOwner->enclClass()) : $cast($Symbol$ClassSymbol, this$0->currentOwner));
...
void ClassReader$AnnotationDefaultCompleter::run() {
	...
	try {
		...
		$set(this->sym, defaultValue, deproxy($($nc(this->sym->type)->getReturnType()), this->value));
...
$Class* ClassReader$AnnotationDefaultCompleter::load$($String* name, bool initialize) {
	...
	$loadClass(ClassReader$AnnotationDefaultCompleter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
