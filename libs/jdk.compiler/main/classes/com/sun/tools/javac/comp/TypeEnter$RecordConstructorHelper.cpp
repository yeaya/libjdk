...
$Class* TypeEnter$RecordConstructorHelper$$Lambda$lambda$new$0::load$($String* name, bool initialize) {
	...
	$loadClass(TypeEnter$RecordConstructorHelper$$Lambda$lambda$new$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TypeEnter$RecordConstructorHelper$$Lambda$lambda$new$0);
	});
	return class$;
}
...
void TypeEnter$RecordConstructorHelper::init$($TypeEnter* this$0, $Symbol$ClassSymbol* owner, $List* recordFieldDecls) {
	...
	this->lastIsVarargs = $$nc($$nc($nc(owner)->getRecordComponents())->stream())->anyMatch($$new(TypeEnter$RecordConstructorHelper$$Lambda$lambda$new$0));
}
...
$Type* TypeEnter$RecordConstructorHelper::constructorType() {
	...
	if (this->$TypeEnter$BasicConstructorHelper::constructorType$ == nullptr) {
		...
		$set(this, constructorType$, $new($Type$MethodType, var$0, var$1, $($List::nil()), this->this$0->syms->methodClass));
	}
...
$Symbol$MethodSymbol* TypeEnter$RecordConstructorHelper::constructorSymbol() {
	...
	$var($JCTree$JCVariableDecl, lastField, $cast($JCTree$JCVariableDecl, $nc(this->recordFieldDecls)->last()));
	{
		...
		for (; $nc(i$)->hasNext();) {
			$var($JCTree$JCVariableDecl, field, $cast($JCTree$JCVariableDecl, i$->next()));
			{
				params->add($$new($Symbol$VarSymbol, ((0x01000000 | (int64_t)0x0000000200000000) | (int64_t)0x2000000000000000) | (field == lastField && this->lastIsVarargs ? $Flags::VARARGS : 0), $nc(field)->name, $nc($nc(field)->sym)->type, csym));
			}
		}
	}
	$set(csym, params$, params->toList());
	csym->flags_field |= (int64_t)0x2000000000000000;
	return csym;
}
	...
	$var($List, tmpRecordFieldDecls, this->recordFieldDecls);
	{
		...
		for (; $nc(i$)->hasNext();) {
			$var($JCTree$JCVariableDecl, arg, $cast($JCTree$JCVariableDecl, i$->next()));
			{
				...
				$var($TreeCopier, tc, $new($TreeCopier, $($nc(this->this$0->make)->at(arg->pos$))));
				$set($nc(arg->mods), annotations, $$nc($nc(rc)->getOriginalAnnos())->isEmpty() ? $List::nil() : tc->copy($(rc->getOriginalAnnos())));
				$set(arg, vartype, $cast($JCTree$JCExpression, tc->copy($nc(($cast($JCTree$JCVariableDecl, $nc(tmpRecordFieldDecls)->head)))->vartype)));
				$assign(tmpRecordFieldDecls, tmpRecordFieldDecls->tail);
			}
		}
	}
	return md;
...
$Class* TypeEnter$RecordConstructorHelper::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.comp.TypeEnter$RecordConstructorHelper$$Lambda$lambda$new$0")) {
	...
	$loadClass(TypeEnter$RecordConstructorHelper, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TypeEnter$RecordConstructorHelper);
	});
	return class$;
}
