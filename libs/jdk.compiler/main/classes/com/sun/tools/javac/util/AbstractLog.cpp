...
$JavaFileObject* AbstractLog::useSource($JavaFileObject* file) {
	$var($JavaFileObject, prev, this->source == nullptr ? ($JavaFileObject*)nullptr : this->source->getFile());
...
$JCDiagnostic$DiagnosticPosition* AbstractLog::wrap(int32_t pos) {
	return (pos == $Position::NOPOS ? ($JCDiagnostic$DiagnosticPosition*)nullptr : $cast($JCDiagnostic$DiagnosticPosition, $new($JCDiagnostic$SimpleDiagnosticPosition, pos)));
}
...
$Class* AbstractLog::load$($String* name, bool initialize) {
	...
	$loadClass(AbstractLog, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AbstractLog);
	});
	return class$;
}
