...
void Infer$GraphSolver$InferenceGraph::initNodes() {
	...
	{
		$var($Iterator, i$, $$nc($GraphUtils::tarjan($cast($AbstractCollection, this->nodes)))->iterator());
		for (; $nc(i$)->hasNext();) {
			$var($List, conSubGraph, $cast($List, i$->next()));
			{
				...
				acyclicNodes->add($cast($Infer$GraphSolver$InferenceGraph$Node, conSubGraph->head));
			}
		}
	}
		...
		for (; $nc(i$)->hasNext();) {
			$var($Type, t, $cast($Type, i$->next()));
			{
				...
				buf->append($($String::format("var %s - upper bounds = %s, lower bounds = %s, eq bounds = %s\\n"_s, $$new($ObjectArray, {
					$nc(uv)->qtype,
					$($nc(uv)->getBounds($$new($Type$UndetVar$InferenceBoundArray, {$Type$UndetVar$InferenceBound::UPPER}))),
					$($nc(uv)->getBounds($$new($Type$UndetVar$InferenceBoundArray, {$Type$UndetVar$InferenceBound::LOWER}))),
					$($nc(uv)->getBounds($$new($Type$UndetVar$InferenceBoundArray, {$Type$UndetVar$InferenceBound::EQ})))
				}))));
			}
		}
	}
	$var($Collection, var$0, $cast($AbstractCollection, this->nodes));
...
$Class* Infer$GraphSolver$InferenceGraph::load$($String* name, bool initialize) {
	...
	$loadClass(Infer$GraphSolver$InferenceGraph, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Infer$GraphSolver$InferenceGraph);
	});
	return class$;
}
