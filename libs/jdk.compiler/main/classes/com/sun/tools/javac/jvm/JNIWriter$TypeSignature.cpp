...
$StringBuilder* JNIWriter$TypeSignature::getParameterSignature($Type* mType, bool useFlatname) {
	...
	$var($StringBuilder, result, $new($StringBuilder));
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			{
				result->append($$cast($CharSequence, getJvmSignature(pType, useFlatname)));
			}
		}
	}
	return result;
...
$StringBuilder* JNIWriter$TypeSignature::getSignature($Type* mType) {
	...
	sb->append("("_s)->append($$cast($CharSequence, getParameterSignature(mType, false)))->append(")"_s);
	sb->append($$cast($CharSequence, getReturnSignature(mType)));
	return sb;
}
...
void JNIWriter$TypeSignature::clinit$($Class* clazz) {
...
$Class* JNIWriter$TypeSignature::load$($String* name, bool initialize) {
	...
	$loadClass(JNIWriter$TypeSignature, name, initialize, &classInfo$$, JNIWriter$TypeSignature::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(JNIWriter$TypeSignature);
	});
	return class$;
}
