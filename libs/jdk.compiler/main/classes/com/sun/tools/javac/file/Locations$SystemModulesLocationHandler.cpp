...
$Class* Locations$SystemModulesLocationHandler$$Lambda$lambda$initSystemModules$0::load$($String* name, bool initialize) {
	...
	$loadClass(Locations$SystemModulesLocationHandler$$Lambda$lambda$initSystemModules$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Locations$SystemModulesLocationHandler$$Lambda$lambda$initSystemModules$0);
	});
	return class$;
}
...
$Collection* Locations$SystemModulesLocationHandler::getPaths() {
	return (this->systemJavaHome == nullptr) ? ($Collection*)nullptr : $cast($Collection, $Collections::singleton(this->systemJavaHome));
}
...
void Locations$SystemModulesLocationHandler::initSystemModules() {
		...
		try {
			...
			if (isCurrentPlatform(this->systemJavaHome)) {
				...
				try {
					...
					$assign(jrtfs, $FileSystems::newFileSystem(jrtURI, $($Collections::emptyMap()), fsLoader));
	...
	$set(this, moduleTable, $new($Locations$ModuleTable, this->this$0));
	{
		$var($DirectoryStream, stream, $Files::newDirectoryStream(this->modules, $$new(Locations$SystemModulesLocationHandler$$Lambda$lambda$initSystemModules$0)));
...
$Class* Locations$SystemModulesLocationHandler::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.file.Locations$SystemModulesLocationHandler$$Lambda$lambda$initSystemModules$0")) {
	...
	$loadClass(Locations$SystemModulesLocationHandler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Locations$SystemModulesLocationHandler);
	});
	return class$;
}
