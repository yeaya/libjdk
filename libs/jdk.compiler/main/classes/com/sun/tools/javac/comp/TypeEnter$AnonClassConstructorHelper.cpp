...
$Class* TypeEnter$AnonClassConstructorHelper$$Lambda$lambda$superArgs$0::load$($String* name, bool initialize) {
	...
	$loadClass(TypeEnter$AnonClassConstructorHelper$$Lambda$lambda$superArgs$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TypeEnter$AnonClassConstructorHelper$$Lambda$lambda$superArgs$0);
	});
	return class$;
}
...
void TypeEnter$AnonClassConstructorHelper::init$($TypeEnter* this$0, $Symbol$TypeSymbol* owner, $Symbol$MethodSymbol* constr, $JCTree$JCExpression* encl) {
	...
	$init($Type);
	$set(this, encl, encl != nullptr ? encl->type : $cast($Type, $Type::noType));
}
...
$Type* TypeEnter$AnonClassConstructorHelper::constructorType() {
	...
	if (this->$TypeEnter$BasicConstructorHelper::constructorType$ == nullptr) {
		...
		if (!$$nc(enclosingType())->hasTag($TypeTag::NONE)) {
			$assign(ctype, this->this$0->types->createMethodTypeWithParameters(ctype, $($$nc($nc(ctype)->getParameterTypes())->prepend($(enclosingType())))));
			this->based = true;
		}
...
$Symbol$MethodSymbol* TypeEnter$AnonClassConstructorHelper::constructorSymbol() {
	...
	$nc(csym)->flags_field |= 0x20000000 | ($nc(this->constr)->flags() & (int64_t)0x0000000400000000);
	csym->flags_field |= this->based ? (int64_t)0x0200000000000000 : 0;
	...
	if (!$$nc(enclosingType())->hasTag($TypeTag::NONE)) {
		...
		$assign(params, params->prepend($$new($Symbol$VarSymbol, (int64_t)0x0000000200000000, var$0, $(enclosingType()), csym)));
	}
	if ($nc(this->constr)->params$ != nullptr) {
		$var($Iterator, i$, this->constr->params$->iterator());
		for (; $nc(i$)->hasNext();) {
			$var($Symbol$VarSymbol, p, $cast($Symbol$VarSymbol, i$->next()));
			{
				$nc(params)->add($$new($Symbol$VarSymbol, (int64_t)0x0000000200000000 | $nc(p)->flags(), $nc(p)->name, $cast($Type, $nc(argtypes)->head), csym));
				$assign(argtypes, argtypes->tail);
			}
		}
	}
	$set(csym, params$, $nc(params)->toList());
	return csym;
}
...
$List* TypeEnter$AnonClassConstructorHelper::superArgs() {
	...
	return $nc(params)->map($$new(TypeEnter$AnonClassConstructorHelper$$Lambda$lambda$superArgs$0));
}
...
$Class* TypeEnter$AnonClassConstructorHelper::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.javac.comp.TypeEnter$AnonClassConstructorHelper$$Lambda$lambda$superArgs$0")) {
	...
	$loadClass(TypeEnter$AnonClassConstructorHelper, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TypeEnter$AnonClassConstructorHelper);
	});
	return class$;
}
