...
void Option$11::processMatching($ArgumentIterator* iter, $OptionHelper* helper) {
	...
	$var($String, trArg, $cast($String, iter->next()));
	...
	$var($String, suffix, m->group("suffix"_s));
	...
	try {
		...
		$var($Transformer, transformer, $cast($Transformer, $$nc(trCls->getConstructor($$new($ClassArray, 0)))->newInstance($$new($ObjectArray, 0))));
...
$Class* Option$11::load$($String* name, bool initialize) {
	...
	$loadClass(Option$11, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
