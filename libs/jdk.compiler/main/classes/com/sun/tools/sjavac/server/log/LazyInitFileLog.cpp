...
void LazyInitFileLog::printLogMsg($Log$Level* msgLevel, $String* msg) {
	...
	try {
		if (this->out == nullptr && isLevelLogged(msgLevel)) {
			...
			$set(this, out, $set(this, err, $new($PrintWriter, $$new($FileWriter, $($nc(this->destination)->toFile())), true)));
		}
		$Log::printLogMsg(msgLevel, msg);
	} catch ($IOException& e) {
		$nc($System::out)->println($$str({"IO error occurred: "_s, $(e->getMessage())}));
		$System::out->println($$str({"Original message: ["_s, msgLevel, "] "_s, msg}));
	}
}
...
$Class* LazyInitFileLog::load$($String* name, bool initialize) {
	...
	$loadClass(LazyInitFileLog, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LazyInitFileLog);
	});
	return class$;
}
