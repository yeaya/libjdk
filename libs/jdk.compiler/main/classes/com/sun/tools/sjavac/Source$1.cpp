...
$Class* Source$1$$Lambda$lambda$visitFile$0::load$($String* name, bool initialize) {
	...
	$loadClass(Source$1$$Lambda$lambda$visitFile$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Source$1$$Lambda$lambda$visitFile$0);
	});
	return class$;
}
...
$FileVisitResult* Source$1::visitFile($Path* file, $BasicFileAttributes* attrs) {
	...
	bool var$1 = $$nc($nc(this->val$includeMatchers)->stream())->anyMatch($$new(Source$1$$Lambda$lambda$visitFile$0, relToRoot));
	bool var$0 = var$1 && $$nc($nc(this->val$excludeMatchers)->stream())->noneMatch($$new(Source$1$$Lambda$lambda$visitFile$0, relToRoot));
	if (var$0 && $nc(this->val$suffixes)->contains($($Util::fileSuffix(file)))) {
		...
		if (existing != nullptr) {
			...
			var$2->append("You have already added the file "_s);
			var$2->append(file);
			var$2->append(" from "_s);
			var$2->append($($$nc(existing->file())->getPath()));
		...
		if (existing != nullptr) {
			if (this->val$inLinksrc) {
				if (existing->isLinkedOnly()) {
					...
					var$3->append("You have already added the link only file "_s);
					var$3->append(file);
					var$3->append(" from "_s);
					var$3->append($($$nc(existing->file())->getPath()));
					$throwNew($IOException, $$str(var$3));
				}
				this->val$foundFiles->put($(file->toString()), existing);
			} else {
				$var($StringBuilder, var$4, $new($StringBuilder));
				var$4->append("Internal error: Double add of file "_s);
				var$4->append(file);
				var$4->append(" from "_s);
				var$4->append($($$nc(existing->file())->getPath()));
				$throwNew($IOException, $$str(var$4));
			}
		} else {
			$var($Module, var$5, this->val$currentModule);
			$var($String, var$6, file->toString());
			...
			$var($String, pkg, $Source::packageOfJavaFile($(this->val$root->toPath()), file));
			$assign(pkg, $str({$(this->val$currentModule->name()), ":"_s, pkg}));
			this->val$foundFiles->put($(file->toString()), s);
			this->val$currentModule->addSource(pkg, s);
		}
	}
...
$Class* Source$1::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.tools.sjavac.Source$1$$Lambda$lambda$visitFile$0")) {
	...
	$loadClass(Source$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
