...
void RequestHandler::run() {
	...
	try {
		try {
			...
			try {
				try {
					$var($PrintWriter, out, $new($PrintWriter, $(this->socket->getOutputStream()), true));
...
$Class* RequestHandler::load$($String* name, bool initialize) {
	...
	$loadClass(RequestHandler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(RequestHandler);
	});
	return class$;
}
