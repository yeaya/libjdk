...
$Object* SimpleDocTreeVisitor::visitReturn($ReturnTree* node, Object$* p) {
	return defaultAction($cast($BlockTagTree, node), p);
}
...
$Class* SimpleDocTreeVisitor::load$($String* name, bool initialize) {
	...
	$loadClass(SimpleDocTreeVisitor, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SimpleDocTreeVisitor);
	});
	return class$;
}
