...
$Object* DocTreeScanner::visitDocComment($DocCommentTree* node, Object$* p) {
	$useLocalObjectStack();
	$var($Object, r, scan($($nc(node)->getFirstSentence()), p));
	$assign(r, scanAndReduce($(node->getBody()), p, r));
	$assign(r, scanAndReduce($(node->getBlockTags()), p, r));
	return r;
}
...
$Object* DocTreeScanner::visitIndex($IndexTree* node, Object$* p) {
	...
	$assign(r, scanAndReduce($(node->getDescription()), p, r));
	return r;
}
...
$Object* DocTreeScanner::visitLink($LinkTree* node, Object$* p) {
	$useLocalObjectStack();
	$var($Object, r, scan($($nc(node)->getReference()), p));
	$assign(r, scanAndReduce($(node->getLabel()), p, r));
	return r;
}
...
$Object* DocTreeScanner::visitParam($ParamTree* node, Object$* p) {
	$useLocalObjectStack();
	$var($Object, r, scan($($nc(node)->getName()), p));
	$assign(r, scanAndReduce($(node->getDescription()), p, r));
	return r;
}
...
$Object* DocTreeScanner::visitProvides($ProvidesTree* node, Object$* p) {
	$useLocalObjectStack();
	$var($Object, r, scan($($nc(node)->getServiceType()), p));
	$assign(r, scanAndReduce($(node->getDescription()), p, r));
	return r;
}
...
$Object* DocTreeScanner::visitReturn($ReturnTree* node, Object$* p) {
	return scan($($nc(node)->getDescription()), p);
}

$Object* DocTreeScanner::visitSee($SeeTree* node, Object$* p) {
	return scan($($nc(node)->getReference()), p);
}
...
$Object* DocTreeScanner::visitStartElement($StartElementTree* node, Object$* p) {
	return scan($($nc(node)->getAttributes()), p);
}

$Object* DocTreeScanner::visitSummary($SummaryTree* node, Object$* p) {
	return scan($($nc(node)->getSummary()), p);
}
...
$Object* DocTreeScanner::visitThrows($ThrowsTree* node, Object$* p) {
	$useLocalObjectStack();
	$var($Object, r, scan($($nc(node)->getExceptionName()), p));
	$assign(r, scanAndReduce($(node->getDescription()), p, r));
	return r;
}

$Object* DocTreeScanner::visitUnknownBlockTag($UnknownBlockTagTree* node, Object$* p) {
	return scan($($nc(node)->getContent()), p);
}
...
$Class* DocTreeScanner::load$($String* name, bool initialize) {
	...
	$loadClass(DocTreeScanner, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DocTreeScanner);
	});
	return class$;
}
