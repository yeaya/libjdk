...
void JavadocFormatter$FormatJavadocScanner::init$($JavadocFormatter* this$0, $StringBuilder* result, $JavacTask* task) {
	...
	$set(this, task, task);
	$set(this, trees, $DocTrees::instance(task));
}
...
$Object* JavadocFormatter$FormatJavadocScanner::visitDocComment($DocCommentTree* node, Object$* p) {
	...
	scan($($nc(node)->getFirstSentence()), p);
	scan($(node->getBody()), p);
	$JavadocFormatter::reflow(this->result, this->reflownTo, this->indent, this->limit);
	{
		...
		for (; $nc(i$)->hasNext();) {
			$JavadocFormatter$Sections* current = $cast($JavadocFormatter$Sections, i$->next());
			{
				bool seenAny = false;
				{
					$var($Iterator, i$, $$nc(node->getBlockTags())->iterator());
					for (; $nc(i$)->hasNext();) {
				...
				if (current == $JavadocFormatter$Sections::RETURNS && !seenAny) {
					...
					$init($DocTree$Kind);
					if (var$0 && $$sure($DocTree, firstSentence->get(0))->getKind() == $DocTree$Kind::RETURN) {
						startSection(current);
						$var($DocTree, var$1, $cast($DocTree, firstSentence->get(0)));
						scan(var$1, $($Boolean::valueOf(true)));
					}
				}
			}
		}
...
void JavadocFormatter$FormatJavadocScanner::startSection($JavadocFormatter$Sections* current) {
	...
	this->result->append($(this->this$0->escape("\u001b[4m"_s)))->append($$cast($String, $nc($JavadocFormatter::docSections)->get(current)))->append($(this->this$0->escape("\u001b[0m"_s)))->append("\n"_s);
}
...
$Object* JavadocFormatter$FormatJavadocScanner::visitParam($ParamTree* node, Object$* p) {
	$useLocalObjectStack();
	$var($CharSequence, var$0, $$nc($nc(node)->getName())->getName());
	return formatDef(var$0, $(node->getDescription()));
}

$Object* JavadocFormatter$FormatJavadocScanner::visitThrows($ThrowsTree* node, Object$* p) {
	$useLocalObjectStack();
	$var($CharSequence, var$0, $$nc($nc(node)->getExceptionName())->getSignature());
	return formatDef(var$0, $(node->getDescription()));
}
...
$Object* JavadocFormatter$FormatJavadocScanner::formatDef($CharSequence* name, $List* description) {
	...
	bool return$1 = false;
	try {
		$assign(var$2, scan(description, nullptr));
		return$1 = true;
		goto $finally;
...
$Object* JavadocFormatter$FormatJavadocScanner::visitReturn($ReturnTree* node, Object$* p) {
	...
	if ($nc(node)->isInline() && p == nullptr) {
		...
		$nc(this->result)->append($JavadocFormatter::inlineReturns, 0, p0);
		$var($Throwable, var$0, nullptr);
		$var($Object, var$2, nullptr);
		bool return$1 = false;
		try {
			...
			this->result->append($($JavadocFormatter::inlineReturns->substring(p0 + MARKER->length())));
		}
		if (var$0 != nullptr) {
			$throw(var$0);
		}
...
$Object* JavadocFormatter$FormatJavadocScanner::visitStartElement($StartElementTree* node, Object$* p) {
	...
	case 1:
		{
			$init($DocTree$Kind);
			bool var$0 = this->lastNode != nullptr && this->lastNode->getKind() == $DocTree$Kind::START_ELEMENT;
			if (var$0 && $JavadocFormatter$HtmlTag::get($($cast($StartElementTree, this->lastNode)->getName())) == $JavadocFormatter$HtmlTag::LI) {
				break;
			}
			reflowTillNow();
	...
	case 7:
		...
		if (!$nc(this->listStack)->empty()) {
			$JavadocFormatter::addNewLineIfNeeded(this->result);
			int32_t top = $$sure($Integer, $nc(this->listStack)->pop())->intValue();
			if (top == (-1)) {
				...
				this->result->append($$str({""_s, $$str(top++), ". "_s}));
			}
	...
	case 8:
		...
		if (!$nc(this->defStack)->isEmpty()) {
			$JavadocFormatter::addNewLineIfNeeded(this->result);
			this->indent = $$sure($Integer, $nc(this->defStack)->peek())->intValue();
	...
	case 9:
		reflowTillNow();
		if (!$nc(this->defStack)->isEmpty()) {
			if (this->indent == $$sure($Integer, this->defStack->peek())->intValue()) {
				$nc(this->result)->append($(this->this$0->escape("\u001b[0m"_s)));
			}
			$JavadocFormatter::addNewLineIfNeeded(this->result);
			this->indent = $$sure($Integer, $nc(this->defStack)->peek())->intValue() + 4;
			$nc(this->result)->append($($JavadocFormatter::indentString(this->indent)));
		}
		break;
	case 10:
	...
	case 15:
		...
		break;
	case 16:
		columns = $$sure($Integer, $nc(this->tableColumns)->get(node))->intValue();
		if (columns == 0) {
			break;
		}
		reflowTillNow();
	...
	case 19:
		...
		$nc(this->result)->append("\n"_s);
		this->reflownTo = this->result->length();
		$$sure($List, $nc(this->cellsStack)->peek())->add($($Integer::valueOf(this->result->length())));
		$$sure($List, $nc(this->headerStack)->peek())->add($($Boolean::valueOf($JavadocFormatter$HtmlTag::get($(node->getName())) == $JavadocFormatter$HtmlTag::TH)));
		break;
	case 20:
		{
			$var($Iterator, i$, $$nc(node->getAttributes())->iterator());
			for (; $nc(i$)->hasNext();) {
				$var($DocTree, attr, $cast($DocTree, i$->next()));
				{
					...
					if ("alt"_s->equals($($StringUtils::toLowerCase($($$nc(at->getName())->toString()))))) {
						$JavadocFormatter::addSpaceIfNeeded(this->result);
						scan($(at->getValue()), nullptr);
						$JavadocFormatter::addSpaceIfNeeded(this->result);
						break;
					}
				}
...
$Object* JavadocFormatter$FormatJavadocScanner::visitEndElement($EndElementTree* node, Object$* p) {
	...
	return $DocTreeScanner::visitEndElement(node, p);
}
...
void JavadocFormatter$FormatJavadocScanner::handleEndElement($Name* name) {
	...
	case 6:
		...
		reflowTillNow();
		if (this->indent == $$sure($Integer, $nc(this->defStack)->peek())->intValue()) {
			$nc(this->result)->append($(this->this$0->escape("\u001b[0m"_s)));
		}
		this->indent = $$sure($Integer, $nc(this->defStack)->pop())->intValue();
		$JavadocFormatter::addNewLineIfNeeded(this->result);
		break;
	case 10:
	case 11:
	...
	case 16:
		{
			...
			this->limit = $$sure($Integer, this->tableStack->pop())->intValue();
			break;
		}
	case 17:
		if ($nc(this->cellsStack)->isEmpty()) {
		...
		for (int32_t line = 0; line < maxLines; ++line) {
			for (int32_t column = 0; column < $nc(content)->size(); ++column) {
				...
				this->result->append("| "_s);
				bool header = $$sure($Boolean, $nc(headerFlags)->get(column))->booleanValue();
				if (header) {
					this->result->append($(this->this$0->escape("\u001b[1m"_s)));
				}
				this->result->append(currentLine);
				if (header) {
					this->result->append($(this->this$0->escape("\u001b[0m"_s)));
				}
...
$Object* JavadocFormatter$FormatJavadocScanner::visitEntity($EntityTree* node, Object$* p) {
	...
	return $DocTreeScanner::visitEntity(node, p);
}
...
void JavadocFormatter$FormatJavadocScanner::reflowTillNow() {
	while (true) {
		...
		if (!(var$0 && this->result->charAt(this->result->length() - 1) == u' ')) {
			break;
		}
		{
			int32_t var$1 = this->result->length() - 1;
			this->result->delete$(var$1, this->result->length());
		}
	}
...
$Class* JavadocFormatter$FormatJavadocScanner::load$($String* name, bool initialize) {
	...
	$loadClass(JavadocFormatter$FormatJavadocScanner, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JavadocFormatter$FormatJavadocScanner);
	});
	return class$;
}
