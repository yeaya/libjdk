...
$Class* RuntimeMXBean$$Lambda$lambda$getPid$0::load$($String* name, bool initialize) {
	...
	$loadClass(RuntimeMXBean$$Lambda$lambda$getPid$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(RuntimeMXBean$$Lambda$lambda$getPid$0);
	});
	return class$;
}
...
int64_t RuntimeMXBean::getPid() {
	...
	return $$sure($Long, $AccessController::doPrivileged($cast($PrivilegedAction, $$new(RuntimeMXBean$$Lambda$lambda$getPid$0))))->longValue();
}
...
$Class* RuntimeMXBean::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.lang.management.RuntimeMXBean$$Lambda$lambda$getPid$0")) {
	...
	$loadClass(RuntimeMXBean, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(RuntimeMXBean);
	});
	return class$;
}
