...
void MemoryPoolImpl::setUsageThreshold(int64_t newThreshold) {
	...
	if (var$0 && newThreshold > usage->getMax()) {
		...
		var$1->append("Invalid threshold: "_s);
		var$1->append(newThreshold);
		var$1->append(" must be <= maxSize. Committed = "_s);
		var$1->append(usage->getCommitted());
		var$1->append(" Max = "_s);
		var$1->append(usage->getMax());
...
$MemoryUsage* MemoryPoolImpl::getUsage0() {
	$prepareNative(getUsage0, $MemoryUsage*);
	$var($MemoryUsage, $ret, $invokeNativeObject());
	$finishNative();
	return $ret;
}

$MemoryUsage* MemoryPoolImpl::getPeakUsage0() {
	$prepareNative(getPeakUsage0, $MemoryUsage*);
	$var($MemoryUsage, $ret, $invokeNativeObject());
	$finishNative();
	return $ret;
}
...
$MemoryUsage* MemoryPoolImpl::getCollectionUsage0() {
	$prepareNative(getCollectionUsage0, $MemoryUsage*);
	$var($MemoryUsage, $ret, $invokeNativeObject());
	$finishNative();
	return $ret;
}
...
void MemoryPoolImpl::setUsageThreshold0(int64_t current, int64_t newThreshold) {
	$prepareNative(setUsageThreshold0, void, int64_t current, int64_t newThreshold);
	$invokeNative(current, newThreshold);
	$finishNative();
}

void MemoryPoolImpl::setCollectionThreshold0(int64_t current, int64_t newThreshold) {
	$prepareNative(setCollectionThreshold0, void, int64_t current, int64_t newThreshold);
	$invokeNative(current, newThreshold);
	$finishNative();
}

void MemoryPoolImpl::resetPeakUsage0() {
	$prepareNative(resetPeakUsage0, void);
	$invokeNative();
	$finishNative();
}

$MemoryManagerMXBeanArray* MemoryPoolImpl::getMemoryManagers0() {
	$prepareNative(getMemoryManagers0, $MemoryManagerMXBeanArray*);
	$var($MemoryManagerMXBeanArray, $ret, $invokeNativeObject());
	$finishNative();
	return $ret;
}

void MemoryPoolImpl::setPoolUsageSensor($Sensor* s) {
	$prepareNative(setPoolUsageSensor, void, $Sensor* s);
	$invokeNative(s);
	$finishNative();
}

void MemoryPoolImpl::setPoolCollectionSensor($Sensor* s) {
	$prepareNative(setPoolCollectionSensor, void, $Sensor* s);
	$invokeNative(s);
	$finishNative();
}
...
$Class* MemoryPoolImpl::load$($String* name, bool initialize) {
	...
	$loadClass(MemoryPoolImpl, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MemoryPoolImpl);
	});
	return class$;
}
