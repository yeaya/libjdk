...
void FileLoginModule::initialize($Subject* subject, $CallbackHandler* callbackHandler, $Map* sharedState, $Map* options) {
	...
	this->useFirstPass = "true"_s->equalsIgnoreCase($$cast($String, options->get("useFirstPass"_s)));
	this->storePass = "true"_s->equalsIgnoreCase($$cast($String, options->get("storePass"_s)));
	this->clearPass = "true"_s->equalsIgnoreCase($$cast($String, options->get("clearPass"_s)));
	this->hashPasswords = "true"_s->equalsIgnoreCase($$cast($String, options->get("hashPasswords"_s)));
	$set(this, passwordFile, $cast($String, options->get("passwordFile"_s)));
...
bool FileLoginModule::login() {
	...
	if (FileLoginModule::logger->debugOn()) {
		FileLoginModule::logger->debug("login"_s, $$str({"Using password file: "_s, this->passwordFileDisplayName}));
	}
	if (this->tryFirstPass) {
		try {
			...
			this->succeeded = true;
			if (FileLoginModule::logger->debugOn()) {
				FileLoginModule::logger->debug("login"_s, "Authentication using cached password has succeeded"_s);
			}
			return true;
		} catch ($LoginException& le) {
			cleanState();
			FileLoginModule::logger->debug("login"_s, "Authentication using cached password has failed"_s);
		}
	...
	if (FileLoginModule::logger->debugOn()) {
		FileLoginModule::logger->debug("login"_s, "Acquiring password"_s);
	}
	try {
		attemptAuthentication(false);
		this->succeeded = true;
		if (FileLoginModule::logger->debugOn()) {
			FileLoginModule::logger->debug("login"_s, "Authentication has succeeded"_s);
		}
		return true;
	} catch ($LoginException& le) {
		cleanState();
		FileLoginModule::logger->debug("login"_s, "Authentication has failed"_s);
		$throw(le);
	}
	$shouldNotReachHere();
}
...
bool FileLoginModule::commit() {
	...
	if (this->succeeded == false) {
		...
		if (!$$nc($nc(this->subject)->getPrincipals())->contains(this->user)) {
			$$nc(this->subject->getPrincipals())->add(this->user);
		}
		if (FileLoginModule::logger->debugOn()) {
			FileLoginModule::logger->debug("commit"_s, "Authentication has completed successfully"_s);
		}
	}
	cleanState();
...
bool FileLoginModule::abort() {
	if (FileLoginModule::logger->debugOn()) {
		FileLoginModule::logger->debug("abort"_s, "Authentication has not completed successfully"_s);
	}
	if (this->succeeded == false) {
		return false;
...
bool FileLoginModule::logout() {
	...
	$set(this, user, nullptr);
	if (FileLoginModule::logger->debugOn()) {
		FileLoginModule::logger->debug("logout"_s, "Subject is being logged out"_s);
	}
	return true;
}
...
void FileLoginModule::attemptAuthentication(bool usePasswdFromSharedState) {
	...
	if (!$nc(this->hashPwdMgr)->authenticate(this->username, this->password)) {
		if (FileLoginModule::logger->debugOn()) {
			FileLoginModule::logger->debug("login"_s, "Invalid username or password"_s);
		}
	...
	if (var$0 && !this->sharedState->containsKey(FileLoginModule::PASSWORD_KEY)) {
		this->sharedState->put(FileLoginModule::USERNAME_KEY, this->username);
		this->sharedState->put(FileLoginModule::PASSWORD_KEY, this->password);
	}
	$set(this, user, $new($JMXPrincipal, this->username));
	if (FileLoginModule::logger->debugOn()) {
		FileLoginModule::logger->debug("login"_s, $$str({"User \'"_s, this->username, "\' successfully validated"_s}));
	}
}
...
void FileLoginModule::getUsernamePassword(bool usePasswdFromSharedState) {
	...
	if (usePasswdFromSharedState) {
		...
		$set(this, password, $cast($chars, this->sharedState->get(FileLoginModule::PASSWORD_KEY)));
		return;
	}
	...
	try {
		...
		$var($LoginException, le, $new($LoginException, $$str({"Error: "_s, $($$nc(uce->getCallback())->toString()), " not available to garner authentication information from the user"_s})));
...
void FileLoginModule::cleanState() {
	...
	if (this->clearPass) {
		...
		this->sharedState->remove(FileLoginModule::PASSWORD_KEY);
	}
}

void FileLoginModule::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$init($File);
	$assignStatic(FileLoginModule::DEFAULT_PASSWORD_FILE_NAME, $str({$$cast($String, $AccessController::doPrivileged($$new($GetPropertyAction, "java.home"_s))), $$str($File::separatorChar), "conf"_s, $$str($File::separatorChar), "management"_s, $$str($File::separatorChar), FileLoginModule::PASSWORD_FILE_NAME}));
...
$Class* FileLoginModule::load$($String* name, bool initialize) {
	...
	$loadClass(FileLoginModule, name, initialize, &classInfo$$, FileLoginModule::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(FileLoginModule);
	});
	return class$;
}
