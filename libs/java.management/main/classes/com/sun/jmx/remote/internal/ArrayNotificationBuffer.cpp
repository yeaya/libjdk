...
$NotificationBuffer* ArrayNotificationBuffer::getNotificationBuffer($MBeanServer* mbs, $Map* env$renamed) {
	...
	$synchronized(ArrayNotificationBuffer::globalLock) {
		$assign(buf, $cast(ArrayNotificationBuffer, ArrayNotificationBuffer::mbsToBuffer->get(mbs)));
		create = (buf == nullptr);
		if (create) {
			$assign(buf, $new(ArrayNotificationBuffer, mbs, queueSize));
			ArrayNotificationBuffer::mbsToBuffer->put(mbs, buf);
		}
		$assign(sharer, $new($ArrayNotificationBuffer$ShareBuffer, $nc(buf), queueSize));
	}
	if (create) {
		buf->createListeners();
	}
...
void ArrayNotificationBuffer::removeNotificationBuffer($MBeanServer* mbs) {
	$init(ArrayNotificationBuffer);
	$synchronized(ArrayNotificationBuffer::globalLock) {
		ArrayNotificationBuffer::mbsToBuffer->remove(mbs);
	}
}
...
void ArrayNotificationBuffer::removeSharer($ArrayNotificationBuffer$ShareBuffer* sharer) {
	...
	$synchronized(ArrayNotificationBuffer::globalLock) {
		...
		if (empty) {
			...
			int32_t max = 0;
			{
				$var($Iterator, i$, this->sharers->iterator());
				for (; $nc(i$)->hasNext();) {
...
void ArrayNotificationBuffer::init$($MBeanServer* mbs, int32_t queueSize) {
	$useLocalObjectStack();
	this->disposed = false;
	$set(this, sharers, $cast($AbstractCollection, $new($HashSet, 1)));
	...
	if (ArrayNotificationBuffer::logger->traceOn()) {
		ArrayNotificationBuffer::logger->trace("Constructor"_s, $$str({"queueSize="_s, $$str(queueSize)}));
	}
	...
	ArrayNotificationBuffer::logger->trace("Constructor"_s, "ends"_s);
}
...
$NotificationResult* ArrayNotificationBuffer::fetchNotifications($NotificationBufferFilter* filter, int64_t startSequenceNumber, int64_t timeout, int32_t maxNotifications) {
	$useLocalObjectStack();
	ArrayNotificationBuffer::logger->trace("fetchNotifications"_s, "starts"_s);
	...
	if (filter == nullptr || startSequenceNumber < 0 || timeout < 0 || maxNotifications < 0) {
		ArrayNotificationBuffer::logger->trace("fetchNotifications"_s, "Bad args"_s);
		$throwNew($IllegalArgumentException, "Bad args to fetch"_s);
	}
	if (ArrayNotificationBuffer::logger->debugOn()) {
		ArrayNotificationBuffer::logger->trace("fetchNotifications"_s, $$str({"filter="_s, filter, "; startSeq="_s, $$str(startSequenceNumber), "; timeout="_s, $$str(timeout), "; max="_s, $$str(maxNotifications)}));
	}
	if (startSequenceNumber > nextSequenceNumber()) {
		...
		ArrayNotificationBuffer::logger->trace("fetchNotifications"_s, msg);
	...
	if (ArrayNotificationBuffer::logger->debugOn()) {
		ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, $$str({"endTime="_s, $$str(endTime)}));
	}
	int64_t earliestSeq = -1;
	int64_t nextSeq = startSequenceNumber;
	$var($List, notifs, $new($ArrayList));
	while (true) {
		ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, "main loop starts"_s);
		$var($ArrayNotificationBuffer$NamedNotification, candidate, nullptr);
		$synchronized(this) {
			if (earliestSeq < 0) {
				earliestSeq = earliestSequenceNumber();
				if (ArrayNotificationBuffer::logger->debugOn()) {
					ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, $$str({"earliestSeq="_s, $$str(earliestSeq)}));
				}
				if (nextSeq < earliestSeq) {
					nextSeq = earliestSeq;
					ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, "nextSeq=earliestSeq"_s);
				}
			} else {
				earliestSeq = earliestSequenceNumber();
			}
			if (nextSeq < earliestSeq) {
				ArrayNotificationBuffer::logger->trace("fetchNotifications"_s, $$str({"nextSeq="_s, $$str(nextSeq), " < earliestSeq="_s, $$str(earliestSeq), " so may have lost notifs"_s}));
				break;
			}
			if (nextSeq < nextSequenceNumber()) {
				...
				if (!($instanceOf($ServerNotifForwarder$NotifForwarderBufferFilter, filter))) {
					try {
						...
						if (ArrayNotificationBuffer::logger->debugOn()) {
							ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, $$str({"candidate: "_s, candidate, " skipped. exception "_s, e}));
						}
						++nextSeq;
						continue;
				...
				if (ArrayNotificationBuffer::logger->debugOn()) {
					ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, $$str({"candidate: "_s, candidate}));
					ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, $$str({"nextSeq now "_s, $$str(nextSeq)}));
				}
			} else {
				if (notifs->size() > 0) {
					ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, "no more notifs but have some so don\'t wait"_s);
					break;
				}
				int64_t toWait = endTime - $System::currentTimeMillis();
				if (toWait <= 0) {
					ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, "timeout"_s);
					break;
				}
				if (isDisposed()) {
					if (ArrayNotificationBuffer::logger->debugOn()) {
						ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, "dispose callled, no wait"_s);
					}
				...
				if (ArrayNotificationBuffer::logger->debugOn()) {
					ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, $$str({"wait("_s, $$str(toWait), ")"_s}));
				}
				$of(this)->wait(toWait);
				continue;
			}
		...
		ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, "applying filter to candidate"_s);
		$nc(filter)->apply(matchedNotifs, name, notif);
		if (matchedNotifs->size() > 0) {
			if (maxNotifications <= 0) {
				ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, "reached maxNotifications"_s);
				break;
			}
			--maxNotifications;
			if (ArrayNotificationBuffer::logger->debugOn()) {
				ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, $$str({"add: "_s, matchedNotifs}));
			}
	...
	if (ArrayNotificationBuffer::logger->debugOn()) {
		ArrayNotificationBuffer::logger->debug("fetchNotifications"_s, $(nr->toString()));
	}
	ArrayNotificationBuffer::logger->trace("fetchNotifications"_s, "ends"_s);
	return nr;
}
...
void ArrayNotificationBuffer::addNotification($ArrayNotificationBuffer$NamedNotification* notif) {
	$synchronized(this) {
		$useLocalObjectStack();
		if (ArrayNotificationBuffer::logger->traceOn()) {
			ArrayNotificationBuffer::logger->trace("addNotification"_s, $($nc(notif)->toString()));
		}
		while ($nc(this->queue)->size() >= this->queueSize) {
			dropNotification();
			if (ArrayNotificationBuffer::logger->debugOn()) {
				ArrayNotificationBuffer::logger->debug("addNotification"_s, $$str({"dropped oldest notif, earliestSeq="_s, $$str(this->earliestSequenceNumber$)}));
			}
		}
		this->queue->add(notif);
		++this->nextSequenceNumber$;
		if (ArrayNotificationBuffer::logger->debugOn()) {
			ArrayNotificationBuffer::logger->debug("addNotification"_s, $$str({"nextSeq="_s, $$str(this->nextSequenceNumber$)}));
		}
		$of(this)->notifyAll();
	}
}
...
$ArrayNotificationBuffer$NamedNotification* ArrayNotificationBuffer::notificationAt(int64_t seqNo) {
	$synchronized(this) {
		...
		if (index < 0 || index > $Integer::MAX_VALUE) {
			...
			ArrayNotificationBuffer::logger->trace("notificationAt"_s, msg);
			$throwNew($IllegalArgumentException, msg);
		}
...
void ArrayNotificationBuffer::createListeners() {
	$useLocalObjectStack();
	ArrayNotificationBuffer::logger->debug("createListeners"_s, "starts"_s);
	$synchronized(this) {
	...
	try {
		...
		ArrayNotificationBuffer::logger->debug("createListeners"_s, "added creationListener"_s);
	} catch ($Exception& e) {
		...
		ArrayNotificationBuffer::logger->fine("createListeners"_s, $$str({msg, e}));
		ArrayNotificationBuffer::logger->debug("createListeners"_s, e);
		$throw(re);
	}
	$var($Set, names, queryNames(nullptr, ArrayNotificationBuffer::broadcasterQuery));
	$assign(names, $new($HashSet, names));
	$synchronized(this) {
	...
	ArrayNotificationBuffer::logger->debug("createListeners"_s, "ends"_s);
}

void ArrayNotificationBuffer::addBufferListener($ObjectName* name) {
	checkNoLocks();
	if (ArrayNotificationBuffer::logger->debugOn()) {
		ArrayNotificationBuffer::logger->debug("addBufferListener"_s, $($nc(name)->toString()));
	}
	try {
		addNotificationListener(name, this->bufferListener, nullptr, name);
	} catch ($Exception& e) {
		ArrayNotificationBuffer::logger->trace("addBufferListener"_s, e);
	}
}
...
void ArrayNotificationBuffer::removeBufferListener($ObjectName* name) {
	checkNoLocks();
	if (ArrayNotificationBuffer::logger->debugOn()) {
		ArrayNotificationBuffer::logger->debug("removeBufferListener"_s, $($nc(name)->toString()));
	}
	try {
		removeNotificationListener(name, this->bufferListener);
	} catch ($Exception& e) {
		ArrayNotificationBuffer::logger->trace("removeBufferListener"_s, e);
	}
}
...
void ArrayNotificationBuffer::addNotificationListener($ObjectName* name, $NotificationListener* listener, $NotificationFilter* filter, Object$* handback) {
	...
	try {
		$AccessController::doPrivileged($$new($ArrayNotificationBuffer$1, this, name, listener, filter, handback));
	} catch ($Exception& e) {
...
void ArrayNotificationBuffer::removeNotificationListener($ObjectName* name, $NotificationListener* listener) {
	...
	$beforeCallerSensitive();
	try {
		$AccessController::doPrivileged($$new($ArrayNotificationBuffer$2, this, name, listener));
	} catch ($Exception& e) {
		$throw($(extractException(e)));
	}
}
...
$Set* ArrayNotificationBuffer::queryNames($ObjectName* name, $QueryExp* query) {
	...
	try {
		...
		ArrayNotificationBuffer::logger->fine("queryNames"_s, $$str({"Failed to query names: "_s, e}));
		ArrayNotificationBuffer::logger->debug("queryNames"_s, e);
		$throw(e);
	}
	$shouldNotReachHere();
}
...
bool ArrayNotificationBuffer::isInstanceOf($MBeanServer* mbs, $ObjectName* name, $String* className) {
	...
	try {
		return $$sure($Boolean, $AccessController::doPrivileged(act))->booleanValue();
	} catch ($Exception& e) {
		ArrayNotificationBuffer::logger->fine("isInstanceOf"_s, $$str({"failed: "_s, e}));
		ArrayNotificationBuffer::logger->debug("isInstanceOf"_s, e);
		return false;
	}
	$shouldNotReachHere();
}
...
void ArrayNotificationBuffer::createdNotification($MBeanServerNotification* n) {
	...
	if (!$$nc($nc(n)->getType())->equals(shouldEqual)) {
		ArrayNotificationBuffer::logger->warning("createNotification"_s, $$str({"bad type: "_s, $(n->getType())}));
		return;
	}
	$var($ObjectName, name, n->getMBeanName());
	if (ArrayNotificationBuffer::logger->debugOn()) {
		ArrayNotificationBuffer::logger->debug("createdNotification"_s, $$str({"for: "_s, name}));
	}
	$synchronized(this) {
...
void ArrayNotificationBuffer::destroyListeners() {
	$useLocalObjectStack();
	checkNoLocks();
	ArrayNotificationBuffer::logger->debug("destroyListeners"_s, "starts"_s);
	try {
		$init($MBeanServerDelegate);
		removeNotificationListener($MBeanServerDelegate::DELEGATE_NAME, this->creationListener);
	} catch ($Exception& e) {
		ArrayNotificationBuffer::logger->warning("remove listener from MBeanServer delegate"_s, e);
	}
	$var($Set, names, queryNames(nullptr, ArrayNotificationBuffer::broadcasterQuery));
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			{
				if (ArrayNotificationBuffer::logger->debugOn()) {
					ArrayNotificationBuffer::logger->debug("destroyListeners"_s, $$str({"remove listener from "_s, name}));
				}
				removeBufferListener(name);
			}
		}
	}
	ArrayNotificationBuffer::logger->debug("destroyListeners"_s, "ends"_s);
}
...
void ArrayNotificationBuffer::checkNoLocks() {
	...
	if (var$0 || $Thread::holdsLock(ArrayNotificationBuffer::globalLock)) {
		ArrayNotificationBuffer::logger->warning("checkNoLocks"_s, "lock protocol violation"_s);
	}
}
...
void ArrayNotificationBuffer::clinit$($Class* clazz) {
...
$Class* ArrayNotificationBuffer::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(Ljavax/management/MBeanServer;I)V", nullptr, $PRIVATE, $method(ArrayNotificationBuffer, init$, void, $MBeanServer*, int32_t)},
		{"addBufferListener", "(Ljavax/management/ObjectName;)V", nullptr, $PRIVATE, $method(ArrayNotificationBuffer, addBufferListener, void, $ObjectName*)},
		{"addNotification", "(Lcom/sun/jmx/remote/internal/ArrayNotificationBuffer$NamedNotification;)V", nullptr, $SYNCHRONIZED, $virtualMethod(ArrayNotificationBuffer, addNotification, void, $ArrayNotificationBuffer$NamedNotification*)},
		{"addNotificationListener", "(Ljavax/management/ObjectName;Ljavax/management/NotificationListener;Ljavax/management/NotificationFilter;Ljava/lang/Object;)V", nullptr, $PRIVATE, $method(ArrayNotificationBuffer, addNotificationListener, void, $ObjectName*, $NotificationListener*, $NotificationFilter*, Object$*), "java.lang.Exception"},
		{"addSharer", "(Lcom/sun/jmx/remote/internal/ArrayNotificationBuffer$ShareBuffer;)V", nullptr, 0, $virtualMethod(ArrayNotificationBuffer, addSharer, void, $ArrayNotificationBuffer$ShareBuffer*)},
		{"checkNoLocks", "()V", nullptr, $PRIVATE, $method(ArrayNotificationBuffer, checkNoLocks, void)},
		{"createListeners", "()V", nullptr, $PRIVATE, $method(ArrayNotificationBuffer, createListeners, void)},
		{"createdNotification", "(Ljavax/management/MBeanServerNotification;)V", nullptr, $PRIVATE, $method(ArrayNotificationBuffer, createdNotification, void, $MBeanServerNotification*)},
		{"destroyListeners", "()V", nullptr, $PRIVATE, $method(ArrayNotificationBuffer, destroyListeners, void)},
		{"dispose", "()V", nullptr, $PUBLIC, $virtualMethod(ArrayNotificationBuffer, dispose, void)},
		{"dropNotification", "()V", nullptr, $PRIVATE, $method(ArrayNotificationBuffer, dropNotification, void)},
		{"earliestSequenceNumber", "()J", nullptr, $SYNCHRONIZED, $virtualMethod(ArrayNotificationBuffer, earliestSequenceNumber, int64_t)},
		{"extractException", "(Ljava/lang/Exception;)Ljava/lang/Exception;", nullptr, $PRIVATE | $STATIC, $staticMethod(ArrayNotificationBuffer, extractException, $Exception*, $Exception*)},
		{"fetchNotifications", "(Lcom/sun/jmx/remote/internal/NotificationBufferFilter;JJI)Ljavax/management/remote/NotificationResult;", nullptr, $PUBLIC, $virtualMethod(ArrayNotificationBuffer, fetchNotifications, $NotificationResult*, $NotificationBufferFilter*, int64_t, int64_t, int32_t), "java.lang.InterruptedException"},
		{"getNotificationBuffer", "(Ljavax/management/MBeanServer;Ljava/util/Map;)Lcom/sun/jmx/remote/internal/NotificationBuffer;", "(Ljavax/management/MBeanServer;Ljava/util/Map<Ljava/lang/String;*>;)Lcom/sun/jmx/remote/internal/NotificationBuffer;", $PUBLIC | $STATIC, $staticMethod(ArrayNotificationBuffer, getNotificationBuffer, $NotificationBuffer*, $MBeanServer*, $Map*)},
		{"isDisposed", "()Z", nullptr, $PRIVATE | $SYNCHRONIZED, $method(ArrayNotificationBuffer, isDisposed, bool)},
		{"isInstanceOf", "(Ljavax/management/MBeanServer;Ljavax/management/ObjectName;Ljava/lang/String;)Z", nullptr, $PRIVATE | $STATIC, $staticMethod(ArrayNotificationBuffer, isInstanceOf, bool, $MBeanServer*, $ObjectName*, $String*)},
		{"nextSequenceNumber", "()J", nullptr, $SYNCHRONIZED, $virtualMethod(ArrayNotificationBuffer, nextSequenceNumber, int64_t)},
		{"notificationAt", "(J)Lcom/sun/jmx/remote/internal/ArrayNotificationBuffer$NamedNotification;", nullptr, $SYNCHRONIZED, $virtualMethod(ArrayNotificationBuffer, notificationAt, $ArrayNotificationBuffer$NamedNotification*, int64_t)},
		{"queryNames", "(Ljavax/management/ObjectName;Ljavax/management/QueryExp;)Ljava/util/Set;", "(Ljavax/management/ObjectName;Ljavax/management/QueryExp;)Ljava/util/Set<Ljavax/management/ObjectName;>;", $PRIVATE, $method(ArrayNotificationBuffer, queryNames, $Set*, $ObjectName*, $QueryExp*)},
		{"removeBufferListener", "(Ljavax/management/ObjectName;)V", nullptr, $PRIVATE, $method(ArrayNotificationBuffer, removeBufferListener, void, $ObjectName*)},
		{"removeNotificationBuffer", "(Ljavax/management/MBeanServer;)V", nullptr, $STATIC, $staticMethod(ArrayNotificationBuffer, removeNotificationBuffer, void, $MBeanServer*)},
		{"removeNotificationListener", "(Ljavax/management/ObjectName;Ljavax/management/NotificationListener;)V", nullptr, $PRIVATE, $method(ArrayNotificationBuffer, removeNotificationListener, void, $ObjectName*, $NotificationListener*), "java.lang.Exception"},
		{"removeSharer", "(Lcom/sun/jmx/remote/internal/ArrayNotificationBuffer$ShareBuffer;)V", nullptr, $PRIVATE, $method(ArrayNotificationBuffer, removeSharer, void, $ArrayNotificationBuffer$ShareBuffer*)},
		{"resize", "(I)V", nullptr, $PRIVATE | $SYNCHRONIZED, $method(ArrayNotificationBuffer, resize, void, int32_t)},
		{}
	};
	...
	$loadClass(ArrayNotificationBuffer, name, initialize, &classInfo$$, ArrayNotificationBuffer::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ArrayNotificationBuffer);
	});
	return class$;
}
