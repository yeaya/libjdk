...
$Class* HashedPasswordManager$$Lambda$lambda$loadPasswords$0::load$($String* name, bool initialize) {
	...
	$loadClass(HashedPasswordManager$$Lambda$lambda$loadPasswords$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(HashedPasswordManager$$Lambda$lambda$loadPasswords$0);
	});
	return class$;
}
...
$StringArray* HashedPasswordManager::getHash($String* algorithm, $String* password) {
	...
	try {
		...
		if (HashedPasswordManager::logger->debugOn()) {
			HashedPasswordManager::logger->debug("getHash"_s, $$str({"Invalid algorithm : "_s, algorithm}));
		}
		return $new($StringArray, {
			""_s,
			""_s
	...
	$synchronized(HashedPasswordManager::class$) {
		{
			...
			try {
				try {
					$init($StandardCharsets);
					$var($OutputStreamWriter, out, $new($OutputStreamWriter, fout, $StandardCharsets::UTF_8));
					$var($Throwable, var$1, nullptr);
					try {
						try {
...
bool HashedPasswordManager::authenticate($String* userName, $chars* inputPassword) {
	$synchronized(this) {
		$useLocalObjectStack();
		if (this->userCredentialsMap->containsKey(userName)) {
			try {
				$var($HashedPasswordManager$UserCredentials, us, $cast($HashedPasswordManager$UserCredentials, this->userCredentialsMap->get(userName)));
				$var($bytes, salt, $$nc($Base64::getDecoder())->decode($nc(us)->b64Salt));
				$var($bytes, targetHash, $$nc($Base64::getDecoder())->decode(us->b64Hash));
				$var($MessageDigest, digest, $MessageDigest::getInstance(us->hashAlgorithm));
				$nc(digest)->reset();
				digest->update(salt);
				...
				if (HashedPasswordManager::logger->debugOn()) {
					HashedPasswordManager::logger->debug("authenticate"_s, $$str({"Unrecognized hash algorithm : "_s, $nc(($$cast($HashedPasswordManager$UserCredentials, this->userCredentialsMap->get(userName))))->hashAlgorithm, " - for user : "_s, userName}));
				}
				return false;
			}
		} else {
			if (HashedPasswordManager::logger->debugOn()) {
				HashedPasswordManager::logger->debug("authenticate"_s, $$str({"Unknown user : "_s, userName}));
			}
			return false;
		}
	}
...
void HashedPasswordManager::loadPasswords() {
	$synchronized(this) {
		...
		$$nc($Arrays::stream($(readPasswordFile())))->forEach($$new(HashedPasswordManager$$Lambda$lambda$loadPasswords$0, this, sbuf, hasClearPasswords));
		if (!this->shouldHashPasswords && hasClearPasswords->get()) {
			if (HashedPasswordManager::logger->debugOn()) {
				HashedPasswordManager::logger->debug("loadPasswords"_s, $$str({"Passwords in "_s, this->passwordFile, " are in clear but are requested not to be hashed !!!"_s}));
			}
		}
		...
		if (this->shouldHashPasswords && hasClearPasswords->get()) {
			if ($$new($File, this->passwordFile)->canWrite()) {
				...
				if (HashedPasswordManager::logger->debugOn()) {
					HashedPasswordManager::logger->debug("loadPasswords"_s, $$str({"Wrote hashed passwords to file : "_s, this->passwordFile}));
				}
			} else if (HashedPasswordManager::logger->debugOn() && !this->isLogged) {
				this->isLogged = true;
				HashedPasswordManager::logger->debug("loadPasswords"_s, $$str({"Passwords in "_s, this->passwordFile, " are in clear and password file is read-only. Passwords cannot be hashed !!!!"_s}));
			}
		}
	}
}
...
void HashedPasswordManager::lambda$loadPasswords$0($StringBuilder* sbuf, $AtomicBoolean* hasClearPasswords, $String* line) {
	...
	$var($StringArray, tokens, line->split("\\s+"_s));
	switch (tokens->length) {
	case 2:
		{
			$var($StringArray, b64str, getHash(HashedPasswordManager::DefaultHashAlgorithm, tokens->get(1)));
			$var($HashedPasswordManager$UserCredentials, us, $new($HashedPasswordManager$UserCredentials, tokens->get(0), HashedPasswordManager::DefaultHashAlgorithm, $nc(b64str)->get(0), $nc(b64str)->get(1)));
			...
			if (this->userCredentialsMap->get(tokens->get(0)) != nullptr) {
				if (HashedPasswordManager::logger->debugOn()) {
					HashedPasswordManager::logger->debug("loadPasswords"_s, $$str({"Ignoring entry for role : "_s, tokens->get(0)}));
				}
			}
			this->userCredentialsMap->put(tokens->get(0), us);
			$nc(hasClearPasswords)->set(true);
			if (HashedPasswordManager::logger->debugOn()) {
				HashedPasswordManager::logger->debug("loadPasswords"_s, "Found atleast one clear password"_s);
			}
			break;
		}
	case 3:
...
void HashedPasswordManager::clinit$($Class* clazz) {
...
$Class* HashedPasswordManager::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("com.sun.jmx.remote.security.HashedPasswordManager$$Lambda$lambda$loadPasswords$0")) {
	...
	$loadClass(HashedPasswordManager, name, initialize, &classInfo$$, HashedPasswordManager::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(HashedPasswordManager);
	});
	return class$;
}
