...
$Object* MXBeanProxy$InvokeHandler::invoke($MBeanServerConnection* mbsc, $ObjectName* name, $ObjectArray* args) {
	return $nc(mbsc)->invoke(name, $(getName()), args, this->signature);
}
...
$Class* MXBeanProxy$InvokeHandler::load$($String* name, bool initialize) {
	...
	$loadClass(MXBeanProxy$InvokeHandler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MXBeanProxy$InvokeHandler);
	});
	return class$;
}
