...
bool Repository$ObjectNamePattern::matchKeys($ObjectName* name) {
	...
	if (this->isPropertyValuePattern || this->isPropertyListPattern) {
		for (int32_t i = $nc(this->keys)->length - 1; i >= 0; --i) {
			$var($String, v, $nc(name)->getKeyProperty(this->keys->get(i)));
			if (v == nullptr) {
				return false;
			}
			if (this->isPropertyValuePattern && $nc(this->pattern)->isPropertyValuePattern(this->keys->get(i))) {
...
$Class* Repository$ObjectNamePattern::load$($String* name, bool initialize) {
	...
	$loadClass(Repository$ObjectNamePattern, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Repository$ObjectNamePattern);
	});
	return class$;
}
