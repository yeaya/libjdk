...
bool MBeanServerPermissionCollection::implies($Permission* permission) {
	$synchronized(this) {
		return (this->collectionPermission != nullptr && this->collectionPermission->implies(permission));
	}
}
...
$Enumeration* MBeanServerPermissionCollection::elements() {
	$synchronized(this) {
		...
		if (this->collectionPermission == nullptr) {
			...
			$assign(set, $Collections::singleton($cast($Permission, this->collectionPermission)));
		}
...
$Class* MBeanServerPermissionCollection::load$($String* name, bool initialize) {
	...
	$loadClass(MBeanServerPermissionCollection, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MBeanServerPermissionCollection);
	});
	return class$;
}
