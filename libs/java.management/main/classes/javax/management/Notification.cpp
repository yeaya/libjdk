...
void Notification::writeObject($ObjectOutputStream* out) {
	if (Notification::compat) {
		...
		$nc(fields)->put("type"_s, this->type);
		...
		fields->put("message"_s, this->message);
...
void Notification::clinit$($Class* clazz) {
	$useLocalObjectStack();
...
$Class* Notification::load$($String* name, bool initialize) {
	...
	$loadClass(Notification, name, initialize, &classInfo$$, Notification::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Notification);
	});
	return class$;
}
