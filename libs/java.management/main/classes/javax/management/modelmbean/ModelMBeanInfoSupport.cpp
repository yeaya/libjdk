...
$Class* ModelMBeanInfoSupport$$Lambda$toString::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(Ljava/lang/StringBuilder;)V", nullptr, $PUBLIC, $method(ModelMBeanInfoSupport$$Lambda$toString, init$, void, $StringBuilder*)},
		{"get", "()Ljava/lang/Object;", nullptr, $PUBLIC, $virtualMethod(ModelMBeanInfoSupport$$Lambda$toString, get, $Object*)},
		{}
	};
	$ClassInfo classInfo$$ = {
		$PUBLIC | $FINAL,
	...
	$loadClass(ModelMBeanInfoSupport$$Lambda$toString, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ModelMBeanInfoSupport$$Lambda$toString);
	});
	return class$;
}
...
void ModelMBeanInfoSupport::init$($ModelMBeanInfo* mbi) {
	...
	$set(this, modelMBeanAttributes, mbi->getAttributes());
	...
	try {
		...
		if ($nc($JmxProperties::MODELMBEAN_LOGGER)->isLoggable($System$Logger$Level::TRACE)) {
			$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "ModelMBeanInfo(ModelMBeanInfo) Could not get a valid modelMBeanDescriptor, setting a default Descriptor"_s);
		}
	}
	$init($JmxProperties);
	$init($System$Logger$Level);
...
void ModelMBeanInfoSupport::init$($String* className, $String* description, $ModelMBeanAttributeInfoArray* attributes, $ModelMBeanConstructorInfoArray* constructors, $ModelMBeanOperationInfoArray* operations, $ModelMBeanNotificationInfoArray* notifications, $Descriptor* mbeandescriptor) {
	$useLocalObjectStack();
	$MBeanInfo::init$(className, description, $$cast($MBeanAttributeInfoArray, (attributes != nullptr) ? attributes : ModelMBeanInfoSupport::NO_ATTRIBUTES), $$cast($MBeanConstructorInfoArray, (constructors != nullptr) ? constructors : ModelMBeanInfoSupport::NO_CONSTRUCTORS), $$cast($MBeanOperationInfoArray, (operations != nullptr) ? operations : ModelMBeanInfoSupport::NO_OPERATIONS), $$cast($MBeanNotificationInfoArray, (notifications != nullptr) ? notifications : ModelMBeanInfoSupport::NO_NOTIFICATIONS));
	$set(this, modelMBeanDescriptor, nullptr);
	$set(this, modelMBeanAttributes, $cast($MBeanAttributeInfoArray, attributes));
	$set(this, modelMBeanConstructors, $cast($MBeanConstructorInfoArray, constructors));
	$set(this, modelMBeanOperations, $cast($MBeanOperationInfoArray, operations));
	$set(this, modelMBeanNotifications, $cast($MBeanNotificationInfoArray, notifications));
	$set(this, modelMBeanDescriptor, validDescriptor(mbeandescriptor));
	$init($JmxProperties);
	$init($System$Logger$Level);
	if ($nc($JmxProperties::MODELMBEAN_LOGGER)->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "ModelMBeanInfoSupport(String,String,ModelMBeanAttributeInfo[],ModelMBeanConstructorInfo[],ModelMBeanOperationInfo[],ModelMBeanNotificationInfo[],Descriptor) Exit"_s);
	}
}
...
void ModelMBeanInfoSupport::setDescriptors($DescriptorArray* inDescriptors) {
	...
	for (int32_t j = 0; j < inDescriptors->length; ++j) {
...
$Descriptor* ModelMBeanInfoSupport::getDescriptor($String* inDescriptorName, $String* inDescriptorType) {
	...
	if (ModelMBeanInfoSupport::MMB->equalsIgnoreCase(inDescriptorType)) {
		return $cast($Descriptor, $nc(this->modelMBeanDescriptor)->clone());
	}
	if (ModelMBeanInfoSupport::ATTR->equalsIgnoreCase(inDescriptorType) || inDescriptorType == nullptr) {
	...
	if (ModelMBeanInfoSupport::OPER->equalsIgnoreCase(inDescriptorType) || inDescriptorType == nullptr) {
	...
	if (ModelMBeanInfoSupport::CONS->equalsIgnoreCase(inDescriptorType) || inDescriptorType == nullptr) {
	...
	if (ModelMBeanInfoSupport::NOTF->equalsIgnoreCase(inDescriptorType) || inDescriptorType == nullptr) {
...
void ModelMBeanInfoSupport::setDescriptor($Descriptor* inDescriptor$renamed, $String* inDescriptorType$renamed) {
	...
	if ((inDescriptorType == nullptr) || (inDescriptorType->isEmpty())) {
		...
		if (inDescriptorType == nullptr) {
			$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, $$str({"descriptorType null in both String parameter and Descriptor, defaulting to "_s, ModelMBeanInfoSupport::MMB}));
	...
	if (inDescriptorName == nullptr) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, $$str({"descriptor name null, defaulting to "_s, $(this->getClassName())}));
	...
	if ($nc(inDescriptorType)->equalsIgnoreCase(ModelMBeanInfoSupport::MMB)) {
		...
		for (int32_t i = 0; i < numAttrs; ++i) {
			if ($nc(inDescriptorName)->equals($($nc($nc(attrList)->get(i))->getName()))) {
				found = true;
				$var($ModelMBeanAttributeInfo, mmbai, $cast($ModelMBeanAttributeInfo, attrList->get(i)));
				$nc(mmbai)->setDescriptor(inDescriptor);
				if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
					$var($StringBuilder, strb, $$new($StringBuilder)->append("Setting descriptor to "_s)->append(inDescriptor)->append("\t\n local: AttributeInfo descriptor is "_s)->append($(mmbai->getDescriptor()))->append("\t\n modelMBeanInfo: AttributeInfo descriptor is "_s)->append($(this->getDescriptor(inDescriptorName, "attribute"_s))));
					$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, $$new(ModelMBeanInfoSupport$$Lambda$toString, strb));
				}
			}
		}
	} else if (inDescriptorType->equalsIgnoreCase(ModelMBeanInfoSupport::OPER)) {
		...
		for (int32_t i = 0; i < numOpers; ++i) {
			if ($nc(inDescriptorName)->equals($($nc($nc(operList)->get(i))->getName()))) {
				found = true;
				$var($ModelMBeanOperationInfo, mmboi, $cast($ModelMBeanOperationInfo, operList->get(i)));
		...
		for (int32_t i = 0; i < numCons; ++i) {
			if ($nc(inDescriptorName)->equals($($nc($nc(consList)->get(i))->getName()))) {
				found = true;
				$var($ModelMBeanConstructorInfo, mmbci, $cast($ModelMBeanConstructorInfo, consList->get(i)));
		...
		for (int32_t i = 0; i < numNotifs; ++i) {
			if ($nc(inDescriptorName)->equals($($nc($nc(notifList)->get(i))->getName()))) {
				found = true;
				$var($ModelMBeanNotificationInfo, mmbni, $cast($ModelMBeanNotificationInfo, notifList->get(i)));
...
$ModelMBeanAttributeInfo* ModelMBeanInfoSupport::getAttribute($String* inName) {
	...
	for (int32_t i = 0; (i < numAttrs) && (retInfo == nullptr); ++i) {
		if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
			$var($StringBuilder, strb, $$new($StringBuilder)->append("\t\n this.getAttributes() MBeanAttributeInfo Array "_s)->append(i)->append(":"_s)->append($($nc($cast($ModelMBeanAttributeInfo, $nc(attrList)->get(i)))->getDescriptor()))->append("\t\n this.modelMBeanAttributes MBeanAttributeInfo Array "_s)->append(i)->append(":"_s)->append($($nc($cast($ModelMBeanAttributeInfo, $nc(this->modelMBeanAttributes)->get(i)))->getDescriptor())));
			$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, $$new(ModelMBeanInfoSupport$$Lambda$toString, strb));
		}
		if ($nc(inName)->equals($($nc($nc(attrList)->get(i))->getName()))) {
			$assign(retInfo, $cast($ModelMBeanAttributeInfo, $nc(attrList->get(i))->clone()));
		}
	}
	if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Exit"_s);
...
$ModelMBeanOperationInfo* ModelMBeanInfoSupport::getOperation($String* inName) {
	...
	for (int32_t i = 0; (i < numOpers) && (retInfo == nullptr); ++i) {
		if ($nc(inName)->equals($($nc($nc(operList)->get(i))->getName()))) {
			$assign(retInfo, $cast($ModelMBeanOperationInfo, $nc(operList->get(i))->clone()));
		}
	}
	if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Exit"_s);
...
$ModelMBeanConstructorInfo* ModelMBeanInfoSupport::getConstructor($String* inName) {
	...
	for (int32_t i = 0; (i < numCons) && (retInfo == nullptr); ++i) {
		if ($nc(inName)->equals($($nc($nc(consList)->get(i))->getName()))) {
			$assign(retInfo, $cast($ModelMBeanConstructorInfo, $nc(consList->get(i))->clone()));
		}
	}
	if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Exit"_s);
...
$ModelMBeanNotificationInfo* ModelMBeanInfoSupport::getNotification($String* inName) {
	...
	for (int32_t i = 0; (i < numNotifs) && (retInfo == nullptr); ++i) {
		if ($nc(inName)->equals($($nc($nc(notifList)->get(i))->getName()))) {
			$assign(retInfo, $cast($ModelMBeanNotificationInfo, $nc(notifList->get(i))->clone()));
		}
	}
	if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Exit"_s);
...
$Descriptor* ModelMBeanInfoSupport::getMBeanDescriptorNoException() {
	...
	if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, $$str({"Exit, returning: "_s, this->modelMBeanDescriptor}));
	}
	return $cast($Descriptor, $nc(this->modelMBeanDescriptor)->clone());
}
...
$Descriptor* ModelMBeanInfoSupport::validDescriptor($Descriptor* in) {
	...
	if (clone->getFieldValue("persistPolicy"_s) == nullptr) {
	...
	if (clone->getFieldValue("log"_s) == nullptr) {
	...
	if (clone->getFieldValue("visibility"_s) == nullptr) {
	...
	if (!$$sure($String, clone->getFieldValue("descriptorType"_s))->equalsIgnoreCase(ModelMBeanInfoSupport::MMB)) {
...
void ModelMBeanInfoSupport::readObject($ObjectInputStream* in) {
	if (ModelMBeanInfoSupport::compat) {
		...
		$set(this, modelMBeanDescriptor, $cast($Descriptor, $nc(fields)->get("modelMBeanDescriptor"_s, nullptr)));
		if (fields->defaulted("modelMBeanDescriptor"_s)) {
			$throwNew($NullPointerException, "modelMBeanDescriptor"_s);
		}
		$set(this, modelMBeanAttributes, $cast($MBeanAttributeInfoArray, fields->get("mmbAttributes"_s, nullptr)));
		if (fields->defaulted("mmbAttributes"_s)) {
			$throwNew($NullPointerException, "mmbAttributes"_s);
		}
		$set(this, modelMBeanConstructors, $cast($MBeanConstructorInfoArray, fields->get("mmbConstructors"_s, nullptr)));
		if (fields->defaulted("mmbConstructors"_s)) {
			$throwNew($NullPointerException, "mmbConstructors"_s);
		}
		$set(this, modelMBeanNotifications, $cast($MBeanNotificationInfoArray, fields->get("mmbNotifications"_s, nullptr)));
		if (fields->defaulted("mmbNotifications"_s)) {
			$throwNew($NullPointerException, "mmbNotifications"_s);
		}
		$set(this, modelMBeanOperations, $cast($MBeanOperationInfoArray, fields->get("mmbOperations"_s, nullptr)));
...
void ModelMBeanInfoSupport::writeObject($ObjectOutputStream* out) {
	if (ModelMBeanInfoSupport::compat) {
		...
		$nc(fields)->put("modelMBeanDescriptor"_s, this->modelMBeanDescriptor);
		fields->put("mmbAttributes"_s, this->modelMBeanAttributes);
		fields->put("mmbConstructors"_s, this->modelMBeanConstructors);
		fields->put("mmbNotifications"_s, this->modelMBeanNotifications);
		fields->put("mmbOperations"_s, this->modelMBeanOperations);
		fields->put("currClass"_s, ModelMBeanInfoSupport::currClass);
		out->writeFields();
	} else {
...
void ModelMBeanInfoSupport::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$load($Descriptor);
	$load($MBeanAttributeInfo);
	$load($MBeanConstructorInfo);
	$load($MBeanNotificationInfo);
	$load($MBeanOperationInfo);
...
$Class* ModelMBeanInfoSupport::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("javax.management.modelmbean.ModelMBeanInfoSupport$$Lambda$toString")) {
	...
	$MethodInfo methodInfos$$[] = {
		...
		{"<init>", "(Ljavax/management/modelmbean/ModelMBeanInfo;)V", nullptr, $PUBLIC, $method(ModelMBeanInfoSupport, init$, void, $ModelMBeanInfo*)},
		{"<init>", "(Ljava/lang/String;Ljava/lang/String;[Ljavax/management/modelmbean/ModelMBeanAttributeInfo;[Ljavax/management/modelmbean/ModelMBeanConstructorInfo;[Ljavax/management/modelmbean/ModelMBeanOperationInfo;[Ljavax/management/modelmbean/ModelMBeanNotificationInfo;)V", nullptr, $PUBLIC, $method(ModelMBeanInfoSupport, init$, void, $String*, $String*, $ModelMBeanAttributeInfoArray*, $ModelMBeanConstructorInfoArray*, $ModelMBeanOperationInfoArray*, $ModelMBeanNotificationInfoArray*)},
		{"<init>", "(Ljava/lang/String;Ljava/lang/String;[Ljavax/management/modelmbean/ModelMBeanAttributeInfo;[Ljavax/management/modelmbean/ModelMBeanConstructorInfo;[Ljavax/management/modelmbean/ModelMBeanOperationInfo;[Ljavax/management/modelmbean/ModelMBeanNotificationInfo;Ljavax/management/Descriptor;)V", nullptr, $PUBLIC, $method(ModelMBeanInfoSupport, init$, void, $String*, $String*, $ModelMBeanAttributeInfoArray*, $ModelMBeanConstructorInfoArray*, $ModelMBeanOperationInfoArray*, $ModelMBeanNotificationInfoArray*, $Descriptor*)},
		{"clone", "()Ljava/lang/Object;", nullptr, $PUBLIC, $virtualMethod(ModelMBeanInfoSupport, clone, $Object*)},
		{"getAttribute", "(Ljava/lang/String;)Ljavax/management/modelmbean/ModelMBeanAttributeInfo;", nullptr, $PUBLIC, $virtualMethod(ModelMBeanInfoSupport, getAttribute, $ModelMBeanAttributeInfo*, $String*), "javax.management.MBeanException,javax.management.RuntimeOperationsException"},
		{"getConstructor", "(Ljava/lang/String;)Ljavax/management/modelmbean/ModelMBeanConstructorInfo;", nullptr, $PUBLIC, $virtualMethod(ModelMBeanInfoSupport, getConstructor, $ModelMBeanConstructorInfo*, $String*), "javax.management.MBeanException,javax.management.RuntimeOperationsException"},
		{"getDescriptor", "(Ljava/lang/String;)Ljavax/management/Descriptor;", nullptr, $PUBLIC, $virtualMethod(ModelMBeanInfoSupport, getDescriptor, $Descriptor*, $String*), "javax.management.MBeanException,javax.management.RuntimeOperationsException"},
		{"getDescriptor", "(Ljava/lang/String;Ljava/lang/String;)Ljavax/management/Descriptor;", nullptr, $PUBLIC, $virtualMethod(ModelMBeanInfoSupport, getDescriptor, $Descriptor*, $String*, $String*), "javax.management.MBeanException,javax.management.RuntimeOperationsException"},
		{"getDescriptor", "()Ljavax/management/Descriptor;", nullptr, $PUBLIC, $virtualMethod(ModelMBeanInfoSupport, getDescriptor, $Descriptor*)},
		{"getDescriptors", "(Ljava/lang/String;)[Ljavax/management/Descriptor;", nullptr, $PUBLIC, $virtualMethod(ModelMBeanInfoSupport, getDescriptors, $DescriptorArray*, $String*), "javax.management.MBeanException,javax.management.RuntimeOperationsException"},
		{"getMBeanDescriptor", "()Ljavax/management/Descriptor;", nullptr, $PUBLIC, $virtualMethod(ModelMBeanInfoSupport, getMBeanDescriptor, $Descriptor*), "javax.management.MBeanException"},
		{"getMBeanDescriptorNoException", "()Ljavax/management/Descriptor;", nullptr, $PRIVATE, $method(ModelMBeanInfoSupport, getMBeanDescriptorNoException, $Descriptor*)},
		{"getNotification", "(Ljava/lang/String;)Ljavax/management/modelmbean/ModelMBeanNotificationInfo;", nullptr, $PUBLIC, $virtualMethod(ModelMBeanInfoSupport, getNotification, $ModelMBeanNotificationInfo*, $String*), "javax.management.MBeanException,javax.management.RuntimeOperationsException"},
		{"getOperation", "(Ljava/lang/String;)Ljavax/management/modelmbean/ModelMBeanOperationInfo;", nullptr, $PUBLIC, $virtualMethod(ModelMBeanInfoSupport, getOperation, $ModelMBeanOperationInfo*, $String*), "javax.management.MBeanException,javax.management.RuntimeOperationsException"},
		{"readObject", "(Ljava/io/ObjectInputStream;)V", nullptr, $PRIVATE, $method(ModelMBeanInfoSupport, readObject, void, $ObjectInputStream*), "java.io.IOException,java.lang.ClassNotFoundException"},
		{"setDescriptor", "(Ljavax/management/Descriptor;Ljava/lang/String;)V", nullptr, $PUBLIC, $virtualMethod(ModelMBeanInfoSupport, setDescriptor, void, $Descriptor*, $String*), "javax.management.MBeanException,javax.management.RuntimeOperationsException"},
		{"setDescriptors", "([Ljavax/management/Descriptor;)V", nullptr, $PUBLIC, $virtualMethod(ModelMBeanInfoSupport, setDescriptors, void, $DescriptorArray*), "javax.management.MBeanException,javax.management.RuntimeOperationsException"},
		{"setMBeanDescriptor", "(Ljavax/management/Descriptor;)V", nullptr, $PUBLIC, $virtualMethod(ModelMBeanInfoSupport, setMBeanDescriptor, void, $Descriptor*), "javax.management.MBeanException,javax.management.RuntimeOperationsException"},
		{"*toString", "()Ljava/lang/String;", nullptr, $PUBLIC},
		{"validDescriptor", "(Ljavax/management/Descriptor;)Ljavax/management/Descriptor;", nullptr, $PRIVATE, $method(ModelMBeanInfoSupport, validDescriptor, $Descriptor*, $Descriptor*), "javax.management.RuntimeOperationsException"},
		{"writeObject", "(Ljava/io/ObjectOutputStream;)V", nullptr, $PRIVATE, $method(ModelMBeanInfoSupport, writeObject, void, $ObjectOutputStream*), "java.io.IOException"},
		{}
	};
	$ClassInfo classInfo$$ = {
		$PUBLIC | $ACC_SUPER,
	...
	$loadClass(ModelMBeanInfoSupport, name, initialize, &classInfo$$, ModelMBeanInfoSupport::clinit$, []($Class* clazz) -> $Object* {
