...
void DescriptorSupport::init$() {
	...
	if ($nc($JmxProperties::MODELMBEAN_LOGGER)->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Constructor"_s);
	}
	init(nullptr);
}
...
void DescriptorSupport::init$(int32_t initNumFields) {
	...
	$init($System$Logger$Level);
	if ($nc($JmxProperties::MODELMBEAN_LOGGER)->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, $$str({"Descriptor(initNumFields = "_s, $$str(initNumFields), ") Constructor"_s}));
	}
	if (initNumFields <= 0) {
		if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
			$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Illegal arguments: initNumFields <= 0"_s);
		}
...
void DescriptorSupport::init$(DescriptorSupport* inDescr) {
	...
	$init($System$Logger$Level);
	if ($nc($JmxProperties::MODELMBEAN_LOGGER)->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Descriptor(Descriptor) Constructor"_s);
	}
	if (inDescr == nullptr) {
		init(nullptr);
	} else {
...
void DescriptorSupport::init$($String* inStr) {
	...
	$init($System$Logger$Level);
	if ($nc($JmxProperties::MODELMBEAN_LOGGER)->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, $$str({"Descriptor(String = \'"_s, inStr, "\') Constructor"_s}));
	}
	if (inStr == nullptr) {
		if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
			$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Descriptor(String = null) Illegal arguments"_s);
		}
	...
	while (st->hasMoreTokens()) {
		...
		if ($nc(tok)->equalsIgnoreCase("FIELD"_s)) {
			...
			if ((fieldName != nullptr) && (fieldValue != nullptr)) {
				int32_t var$1 = fieldName->indexOf(u'\"') + 1;
				$assign(fieldName, fieldName->substring(var$1, fieldName->lastIndexOf(u'\"')));
			...
			int32_t eq_separator = tok->indexOf(u'=');
			if (eq_separator > 0) {
	...
	if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Descriptor(XMLString) Exit"_s);
	}
}
...
void DescriptorSupport::init$($StringArray* fieldNames, $ObjectArray* fieldValues) {
	...
	$init($System$Logger$Level);
	if ($nc($JmxProperties::MODELMBEAN_LOGGER)->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Descriptor(fieldNames,fieldObjects) Constructor"_s);
	}
	if ((fieldNames == nullptr) || (fieldValues == nullptr) || (fieldNames->length != fieldValues->length)) {
		if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
			$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Descriptor(fieldNames,fieldObjects) Illegal arguments"_s);
		}
	...
	if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Descriptor(fieldNames,fieldObjects) Exit"_s);
	}
}
...
void DescriptorSupport::init$($StringArray* fields) {
	...
	$init($System$Logger$Level);
	if ($nc($JmxProperties::MODELMBEAN_LOGGER)->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Descriptor(String... fields) Constructor"_s);
	}
	init(nullptr);
	...
	for (int32_t i = 0; i < $nc(fields)->length; ++i) {
		...
		int32_t eq_separator = $nc(fields->get(i))->indexOf(u'=');
		if (eq_separator < 0) {
			if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
				$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Descriptor(String... fields) Illegal arguments: field does not have \'=\' as a name and value separator"_s);
			}
		...
		if (fieldName->isEmpty()) {
			if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
				$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Descriptor(String... fields) Illegal arguments: fieldName is empty"_s);
			}
	...
	if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
		$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Descriptor(String... fields) Exit"_s);
	}
}
...
$Object* DescriptorSupport::getFieldValue($String* fieldName) {
	$synchronized(this) {
		...
		if ((fieldName == nullptr) || (fieldName->isEmpty())) {
			...
			$init($System$Logger$Level);
			if ($nc($JmxProperties::MODELMBEAN_LOGGER)->isLoggable($System$Logger$Level::TRACE)) {
				$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Illegal arguments: null field name"_s);
			}
		...
		$init($System$Logger$Level);
		if ($nc($JmxProperties::MODELMBEAN_LOGGER)->isLoggable($System$Logger$Level::TRACE)) {
			$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, $$str({"getFieldValue(String fieldName = "_s, fieldName, ") Returns \'"_s, retValue, "\'"_s}));
		}
		return (retValue);
	}
}
...
void DescriptorSupport::setField($String* fieldName, Object$* fieldValue) {
	$synchronized(this) {
		...
		if ((fieldName == nullptr) || (fieldName->isEmpty())) {
			...
			$init($System$Logger$Level);
			if ($nc($JmxProperties::MODELMBEAN_LOGGER)->isLoggable($System$Logger$Level::TRACE)) {
				$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Illegal arguments: null or empty field name"_s);
			}
		...
		if (!validateField(fieldName, fieldValue)) {
			...
			$init($System$Logger$Level);
			if ($nc($JmxProperties::MODELMBEAN_LOGGER)->isLoggable($System$Logger$Level::TRACE)) {
				$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Illegal arguments"_s);
			}
		...
		$init($System$Logger$Level);
		if ($nc($JmxProperties::MODELMBEAN_LOGGER)->isLoggable($System$Logger$Level::TRACE)) {
			$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, $$str({"Entry: setting \'"_s, fieldName, "\' to \'"_s, fieldValue, "\'"_s}));
		}
...
$StringArray* DescriptorSupport::getFields() {
	$synchronized(this) {
		...
		$var($Set, returnedSet, this->descriptorMap->entrySet());
		int32_t i = 0;
		if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
			$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, $$str({"Returning "_s, $$str(numberOfEntries), " fields"_s}));
		}
		{
			...
			for (; $nc(iter)->hasNext(); ++i) {
				...
				if (currElement == nullptr) {
					if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
						$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Element is null"_s);
					}
				} else {
					$var($Object, currValue, currElement->getValue());
					if (currValue == nullptr) {
						...
						var$0->append($$cast($String, currElement->getKey()));
						var$0->append("="_s);
						var$0->append($(currValue->toString()));
						responseFields->set(i, $$str(var$0));
					} else {
						$var($StringBuilder, var$1, $new($StringBuilder));
						var$1->append($$cast($String, currElement->getKey()));
						var$1->append("=("_s);
						var$1->append($(currValue->toString()));
						var$1->append(")"_s);
						responseFields->set(i, $$str(var$1));
					}
				}
			}
		}
...
$StringArray* DescriptorSupport::getFieldNames() {
	$synchronized(this) {
		...
		}
		{
			...
			for (; $nc(iter)->hasNext(); ++i) {
				$var($Map$Entry, currElement, $cast($Map$Entry, iter->next()));
				if ((currElement == nullptr) || (currElement->getKey() == nullptr)) {
					if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
						$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Field is null"_s);
					}
				} else {
					responseFields->set(i, $($$sure($String, currElement->getKey())->toString()));
				}
			}
		}
		if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
...
$ObjectArray* DescriptorSupport::getFieldValues($StringArray* fieldNames) {
	$synchronized(this) {
		...
		int32_t numberOfEntries = (fieldNames == nullptr) ? $nc(this->descriptorMap)->size() : fieldNames->length;
...
void DescriptorSupport::setFields($StringArray* fieldNames, $ObjectArray* fieldValues) {
	$synchronized(this) {
		...
		for (int32_t i = 0; i < $nc(fieldNames)->length; ++i) {
			if ((fieldNames->get(i) == nullptr) || ($nc(fieldNames->get(i))->isEmpty())) {
				if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
					$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, $$str({"Null field name encountered at element "_s, $$str(i)}));
				}
...
int32_t DescriptorSupport::hashCode() {
	$synchronized(this) {
		...
		$var($StringArray, var$0, $cast($StringArray, $$nc(this->descriptorMap->keySet())->toArray($$new($StringArray, size))));
		return $Util::hashCode(var$0, $($$nc(this->descriptorMap->values())->toArray($$new($ObjectArray, size))));
	}
}
...
bool DescriptorSupport::isValid() {
	$synchronized(this) {
		...
		if (returnedSet == nullptr) {
			if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
				$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "isValid() Returns false (null set)"_s);
			}
			return false;
		}
		...
		bool var$0 = (thisName == nullptr) || (thisDescType == nullptr) || (thisName->isEmpty());
		if (var$0 || (thisDescType->isEmpty())) {
			return false;
		}
		{
			...
			for (; $nc(i$)->hasNext();) {
				...
				if (currElement != nullptr) {
					if (currElement->getValue() != nullptr) {
						$var($String, var$1, $$sure($String, currElement->getKey())->toString());
						if (validateField(var$1, $(($$nc(currElement->getValue()))->toString()))) {
							continue;
						} else {
							if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
								$var($StringBuilder, var$2, $new($StringBuilder));
								var$2->append("Field "_s);
								var$2->append($$cast($String, currElement->getKey()));
								var$2->append("="_s);
								var$2->append($(currElement->getValue()));
								var$2->append(" is not valid"_s);
								$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, $$str(var$2));
							}
							return false;
						}
					}
		...
		if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
			$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "isValid() Returns true"_s);
		}
		return true;
	}
}
...
bool DescriptorSupport::validateField($String* fldName, Object$* fldValue) {
	if ((fldName == nullptr) || (fldName->isEmpty())) {
		return false;
	}
	...
	bool nameOrDescriptorType = (var$0 || fldName->equalsIgnoreCase("DescriptorType"_s));
	...
	if (var$1 || fldName->equalsIgnoreCase("Class"_s)) {
		...
		if ((fldValue != nullptr) && (isAString)) {
			...
			v = $cast($Integer, fldValue)->intValue();
		} else {
			return false;
		}
		if (v >= 1 && v <= 4) {
		...
		bool var$12 = fldName->equalsIgnoreCase("PersistPeriod"_s);
		bool var$11 = var$12 || fldName->equalsIgnoreCase("CurrencyTimeLimit"_s);
		bool var$10 = var$11 || fldName->equalsIgnoreCase("LastUpdatedTimeStamp"_s);
		if (var$10 || fldName->equalsIgnoreCase("LastReturnedTimeStamp"_s)) {
			int64_t v = 0;
			if ((fldValue != nullptr) && (isAString)) {
				v = toNumeric(SfldValue);
			} else if ($instanceOf($Number, fldValue)) {
				v = $cast($Number, fldValue)->longValue();
			} else {
				return false;
			}
			return (v >= -1);
		} else if (fldName->equalsIgnoreCase("log"_s)) {
			bool var$13 = $instanceOf($Boolean, fldValue);
			if (!var$13) {
				bool var$14 = isAString;
				if (var$14) {
					bool var$17 = SfldValue->equalsIgnoreCase("T"_s);
					bool var$16 = var$17 || SfldValue->equalsIgnoreCase("true"_s);
					bool var$15 = var$16 || SfldValue->equalsIgnoreCase("F"_s);
					var$14 = var$15 || SfldValue->equalsIgnoreCase("false"_s);
				}
				var$13 = var$14;
			}
			return (var$13);
			...
			for (; $nc(i$)->hasNext();) {
				$var($Map$Entry, currElement, $cast($Map$Entry, i$->next()));
				{
					...
					if ($instanceOf($String, value)) {
						...
						bool var$0 = !svalue->startsWith("("_s);
						if (var$0 || !svalue->endsWith(")"_s)) {
...
bool DescriptorSupport::isMagic(char16_t c) {
	...
	return (c < $nc(DescriptorSupport::charToEntityMap)->length && DescriptorSupport::charToEntityMap->get(c) != nullptr);
}
...
$String* DescriptorSupport::unquote($String* s) {
	...
	if (var$0 || !s->endsWith("\""_s)) {
		$throwNew($XMLParseException, $$str({"Value must be quoted: <"_s, s, ">"_s}));
	}
	$var($StringBuilder, buf, $new($StringBuilder));
	int32_t len = s->length() - 1;
	for (int32_t i = 1; i < len; ++i) {
		char16_t c = s->charAt(i);
		int32_t semi = 0;
		$var($Character, quoted, nullptr);
		bool var$1 = c == u'&' && (semi = s->indexOf(u';', i + 1)) >= 0;
		if (var$1 && (($assign(quoted, $cast($Character, DescriptorSupport::entityToCharMap->get($(s->substring(i, semi + 1)))))) != nullptr)) {
			buf->append(quoted);
			i = semi;
		} else {
			buf->append(c);
...
$String* DescriptorSupport::makeFieldValue(Object$* value) {
	...
	try {
		valueClass->getConstructor($$new($ClassArray, {$String::class$}));
	...
	return $str({"("_s, $(valueClass->getName()), "/"_s, quotedValueString, ")"_s});
}
...
$Object* DescriptorSupport::parseQuotedFieldValue($String* s$renamed) {
	...
	bool var$0 = !s->startsWith("("_s);
	if (var$0 || !s->endsWith(")"_s)) {
		return $of(s);
	}
	int32_t slash = s->indexOf(u'/');
	if (slash < 0) {
	...
	try {
		...
		$assign(constr, c->getConstructor($$new($ClassArray, {$String::class$})));
	} catch ($Exception& e) {
		$throwNew($XMLParseException, e, $$str({"Cannot parse value: <"_s, s, ">"_s}));
	}
	$var($String, arg, s->substring(slash + 1, s->length() - 1));
	try {
		return $nc(constr)->newInstance($$new($ObjectArray, {arg}));
	} catch ($Exception& e) {
...
$String* DescriptorSupport::toString() {
	$synchronized(this) {
		...
		if ((fields == nullptr) || (fields->length == 0)) {
			if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
				$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, "Empty Descriptor"_s);
			}
			return respStr;
		}
		if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
			$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, $$str({"Printing "_s, $$str($nc(fields)->length), " fields"_s}));
		}
		for (int32_t i = 0; i < $nc(fields)->length; ++i) {
		...
		if ($JmxProperties::MODELMBEAN_LOGGER->isLoggable($System$Logger$Level::TRACE)) {
			$JmxProperties::MODELMBEAN_LOGGER->log($System$Logger$Level::TRACE, $$str({"Exit returning "_s, respStr}));
		}
		return respStr;
	}
}
...
void DescriptorSupport::readObject($ObjectInputStream* in) {
	...
	$var($Map, descriptor, $cast($Map, $Util::cast($($nc(fields)->get("descriptor"_s, nullptr)))));
	init(nullptr);
...
void DescriptorSupport::writeObject($ObjectOutputStream* out) {
	...
	if (compat) {
		$nc(fields)->put("currClass"_s, DescriptorSupport::currClass);
	}
	...
	if (var$0 || "1.2.1"_s->equals(DescriptorSupport::serialForm)) {
		...
		{
			$var($Iterator, i$, $$nc(startMap->entrySet())->iterator());
			for (; $nc(i$)->hasNext();) {
				$var($Map$Entry, entry, $cast($Map$Entry, i$->next()));
				$var($Object, var$1, $$sure($String, $nc(entry)->getKey())->toLowerCase());
				descriptor->put(var$1, $(entry->getValue()));
			}
		}
	} else {
		$assign(descriptor, $new($HashMap, startMap));
	}
	$nc(fields)->put("descriptor"_s, descriptor);
	out->writeFields();
}

void DescriptorSupport::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(DescriptorSupport::newSerialPersistentFields, $new($ObjectStreamFieldArray, {$$new($ObjectStreamField, "descriptor"_s, $HashMap::class$)}));
	{
		...
		try {
			...
			$assign(form, $cast($String, $AccessController::doPrivileged(act)));
	...
	{
		char16_t maxChar = 0;
		for (int32_t i = 0; i < DescriptorSupport::entities->length; ++i) {
			char16_t c = $nc(DescriptorSupport::entities->get(i))->charAt(0);
			if (c > maxChar) {
				maxChar = c;
			}
		}
		...
		for (int32_t i = 0; i < DescriptorSupport::entities->length; ++i) {
			char16_t c = $nc(DescriptorSupport::entities->get(i))->charAt(0);
			$var($String, entity, $nc(DescriptorSupport::entities->get(i))->substring(1));
			DescriptorSupport::charToEntityMap->set(c, entity);
			DescriptorSupport::entityToCharMap->put(entity, $($Character::valueOf(c)));
		}
	}
}
...
$Class* DescriptorSupport::load$($String* name, bool initialize) {
	...
	$loadClass(DescriptorSupport, name, initialize, &classInfo$$, DescriptorSupport::clinit$, []($Class* clazz) -> $Object* {
