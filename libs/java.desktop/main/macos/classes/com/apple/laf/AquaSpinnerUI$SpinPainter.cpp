...
void AquaSpinnerUI$SpinPainter::paint($Graphics* g) {
	...
	if (isEnabled()) {
		if (this->fTopModel != nullptr && this->fTopModel->isPressed()) {
			$init($JRSUIConstants$State);
			$nc($nc(this->painter)->state)->set($JRSUIConstants$State::PRESSED);
			$init($JRSUIConstants$BooleanValue);
			$nc(this->painter->state)->set($JRSUIConstants$BooleanValue::NO);
		} else if (this->fBottomModel != nullptr && this->fBottomModel->isPressed()) {
			$init($JRSUIConstants$State);
			$nc($nc(this->painter)->state)->set($JRSUIConstants$State::PRESSED);
			$init($JRSUIConstants$BooleanValue);
			$nc(this->painter->state)->set($JRSUIConstants$BooleanValue::YES);
		} else {
			$init($JRSUIConstants$State);
	...
	$nc(this->painter)->paint(g, this->this$0->spinner, 0, 0, $nc(bounds)->width, $nc(bounds)->height);
}
...
$Class* AquaSpinnerUI$SpinPainter::load$($String* name, bool initialize) {
	...
	$loadClass(AquaSpinnerUI$SpinPainter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
