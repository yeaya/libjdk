...
int64_t ScreenMenu::addMenuListeners(ScreenMenu* listener, int64_t nativeMenu) {
	$init(ScreenMenu);
	$prepareNativeStatic(addMenuListeners, int64_t, ScreenMenu* listener, int64_t nativeMenu);
	int64_t $ret = $invokeNativeStatic(listener, nativeMenu);
	$finishNativeStatic();
	return $ret;
}
...
void ScreenMenu::removeMenuListeners(int64_t modelPtr) {
	$init(ScreenMenu);
	$prepareNativeStatic(removeMenuListeners, void, int64_t modelPtr);
...
void ScreenMenu::init$($JMenu* invoker) {
	...
	int32_t count = this->fInvoker->getMenuComponentCount();
	if (count < 5) {
		count = 5;
	}
...
bool ScreenMenu::needsUpdate($ComponentArray* items, $ints* childHashArray) {
	...
	for (int32_t i = 0; i < items->length; ++i) {
		...
		if (hashCode != childHashArray->get(i)) {
			return true;
		}
	}
	return false;
...
void ScreenMenu::updateItems() {
	int32_t count = this->fInvoker->getMenuComponentCount();
	$var($ComponentArray, items, this->fInvoker->getMenuComponents());
	if (needsUpdate(items, this->childHashArray)) {
		...
		for (int32_t i = 0; i < count; ++i) {
			...
			$nc(this->childHashArray)->set(i, getHashCode(items->get(i)));
		}
	}
}
...
void ScreenMenu::invokeOpenLater() {
	...
	try {
		$LWCToolkit::invokeAndWait($$new($ScreenMenu$2, this, invoker), invoker);
	} catch ($Exception& e) {
...
void ScreenMenu::invokeMenuClosing() {
	...
	try {
		$LWCToolkit::invokeAndWait($$new($ScreenMenu$3, this, invoker), invoker);
	} catch ($Exception& e) {
		e->printStackTrace();
	}
}
...
void ScreenMenu::addNotify() {
	...
	$synchronized(getTreeLock()) {
		$Menu::addNotify();
		if (this->fModelPtr == 0) {
			$$nc(this->fInvoker->getPopupMenu())->addContainerListener(this);
			...
			$var($Icon, icon, this->fInvoker->getIcon());
			if (icon != nullptr) {
				setIcon(icon);
			}
			$var($String, tooltipText, this->fInvoker->getToolTipText());
			...
			if ($instanceOf($CMenu, peer)) {
				...
				int64_t nativeMenu = menu->getNativeMenu();
...
void ScreenMenu::removeNotify() {
	$synchronized(getTreeLock()) {
		...
		if (this->fModelPtr != 0) {
			...
			this->fModelPtr = 0;
			$$nc(this->fInvoker->getPopupMenu())->removeContainerListener(this);
...
void ScreenMenu::componentRemoved($ContainerEvent* e) {
	...
	$var($MenuItem, sm, $cast($MenuItem, this->fItems->remove(child)));
	if (sm == nullptr) {
		return;
	}
	remove(sm);
...
void ScreenMenu::addItem($Component* m) {
	...
	$var($MenuItem, sm, $cast($MenuItem, this->fItems->get(m)));
	if (sm == nullptr) {
...
void ScreenMenu::clinit$($Class* clazz) {
	$beforeCallerSensitive();
	{
		$AccessController::doPrivileged($$new($ScreenMenu$1));
	}
}
...
$Class* ScreenMenu::load$($String* name, bool initialize) {
	...
	$loadClass(ScreenMenu, name, initialize, &classInfo$$, ScreenMenu::clinit$, []($Class* clazz) -> $Object* {
