...
void AquaComboBoxUI$AquaComboBoxEditor::editorTextChanged() {
	...
	for (int32_t i = 0; i < items; ++i) {
		...
		$var($String, asString, $nc(element)->toString());
		if (asString == nullptr || !asString->equals(text)) {
			continue;
		}
...
$Class* AquaComboBoxUI$AquaComboBoxEditor::load$($String* name, bool initialize) {
	...
	$loadClass(AquaComboBoxUI$AquaComboBoxEditor, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
