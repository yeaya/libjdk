...
void AquaScrollBarUI::installListeners() {
	...
	this->fScrollTimer->setInitialDelay(AquaScrollBarUI::kInitialDelay);
}
...
void AquaScrollBarUI::syncState($JComponent* c) {
	...
	$nc(scrollBarState)->set(isHorizontal() ? $JRSUIConstants$Orientation::HORIZONTAL : $JRSUIConstants$Orientation::VERTICAL);
	int32_t var$1 = $nc(this->fScrollBar)->getMaximum();
	int32_t var$0 = var$1 - this->fScrollBar->getMinimum();
	float trackExtent = (float)(var$0 - $$nc(this->fScrollBar->getModel())->getExtent());
	if (trackExtent <= 0.0f) {
	...
	scrollBarState->setValue((var$2 - this->fScrollBar->getMinimum()) / trackExtent);
...
void AquaScrollBarUI::paint($Graphics* g, $JComponent* c) {
	...
	$nc(this->painter)->paint(g, c, 0, 0, var$0, this->fScrollBar->getHeight());
}
...
$JRSUIConstants$ScrollBarPart* AquaScrollBarUI::getPressedPart() {
	$useLocalObjectStack();
	if (!$nc(this->fTrackListener)->fInArrows || !this->fTrackListener->fStillInArrow) {
		$init($JRSUIConstants$ScrollBarPart);
		return $JRSUIConstants$ScrollBarPart::NONE;
	}
	$var($JRSUIConstants$ScrollBarPart, pressedPart, $cast($JRSUIConstants$ScrollBarPart, $$sure($Map, AquaScrollBarUI::hitToPressedPartMap->get())->get(this->fMousePart)));
...
$Rectangle* AquaScrollBarUI::getTrackBounds() {
	int32_t var$0 = $nc(this->fScrollBar)->getWidth();
	return $new($Rectangle, 0, 0, var$0, this->fScrollBar->getHeight());
}
...
void AquaScrollBarUI::scrollByBlock(int32_t direction) {
	$synchronized(this->fScrollBar) {
		int32_t oldValue = this->fScrollBar->getValue();
		...
		$set(this, fTrackHighlight, direction > 0 ? $JRSUIConstants$ScrollBarHit::TRACK_MAX : $JRSUIConstants$ScrollBarHit::TRACK_MIN);
		$nc(this->fScrollBar)->repaint();
...
void AquaScrollBarUI::scrollByUnit(int32_t direction) {
	$synchronized(this->fScrollBar) {
		int32_t delta = this->fScrollBar->getUnitIncrement(direction);
		if (direction <= 0) {
			delta = -delta;
		}
...
$JRSUIConstants$Hit* AquaScrollBarUI::getPartHit(int32_t x, int32_t y) {
	...
	return $JRSUIUtils$HitDetection::getHitForPoint(var$0, 0, 0, var$1, this->fScrollBar->getHeight(), x, y);
}
...
float AquaScrollBarUI::getThumbStart() {
	...
	return (float)(var$0 - this->fScrollBar->getMinimum()) / (float)extent;
}
...
$Point* AquaScrollBarUI::getScrollToHereStartPoint(int32_t clickPosX, int32_t clickPosY) {
	...
	int32_t var$1 = $nc(this->fScrollBar)->getWidth();
	$init($JRSUIConstants$ScrollBarPart);
	$JRSUIUtils$ScrollBar::getPartBounds(rect, var$0, 0, 0, var$1, this->fScrollBar->getHeight(), $JRSUIConstants$ScrollBarPart::THUMB);
	...
	if (isHorizontal()) {
		...
		int32_t limitRectRight = $nc(limitRect)->x + $nc(limitRect)->width;
	...
	int32_t limitRectBottom = $nc(limitRect)->y + $nc(limitRect)->height;
...
void AquaScrollBarUI::clinit$($Class* clazz) {
...
$Class* AquaScrollBarUI::load$($String* name, bool initialize) {
	...
	$loadClass(AquaScrollBarUI, name, initialize, &classInfo$$, AquaScrollBarUI::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(AquaScrollBarUI);
	});
	return class$;
}
