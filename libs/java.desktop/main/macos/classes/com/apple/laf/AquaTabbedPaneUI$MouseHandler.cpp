...
void AquaTabbedPaneUI$MouseHandler::dispose() {
	this->popupTimer->removeActionListener(this);
...
void AquaTabbedPaneUI$MouseHandler::mousePressed($MouseEvent* e) {
	...
	if (!var$0) {
		...
		var$0 = var$1 && this->trackingTab == pane->getSelectedIndex();
	}
	if (var$0) {
		this->trackingTab = -3;
		return;
...
void AquaTabbedPaneUI$MouseHandler::mouseReleased($MouseEvent* e) {
	...
	if (this->trackingTab == -1 && currentTab == -1) {
		$nc(pane)->setSelectedIndex($nc(pane)->getSelectedIndex() + 1);
	}
...
int32_t AquaTabbedPaneUI$MouseHandler::getCurrentTab($JTabbedPane* pane, $Point* p) {
	$useLocalObjectStack();
	int32_t tabIndex = this->this$0->tabForCoordinate(pane, $nc(p)->x, $nc(p)->y);
	if (tabIndex >= 0 && $nc(pane)->isEnabledAt(tabIndex)) {
		return tabIndex;
	}
	bool var$0 = $nc(this->this$0->visibleTabState)->needsLeftScrollTab();
	if (var$0 && $$nc(this->this$0->visibleTabState->getLeftScrollTabRect())->contains(p)) {
		return -2;
	}
	bool var$1 = this->this$0->visibleTabState->needsRightScrollTab();
	if (var$1 && $$nc(this->this$0->visibleTabState->getRightScrollTabRect())->contains(p)) {
		return -1;
	}
	return -3;
}
...
void AquaTabbedPaneUI$MouseHandler::showFullPopup(bool firstTab) {
	...
	if (firstTab) {
		...
		popup->show(this->this$0->tabPane, $nc(leftScrollTabRect)->x - $nc(popupRect)->width, $nc(leftScrollTabRect)->y + 7);
	} else {
		$var($Rectangle, rightScrollTabRect, $nc(this->this$0->visibleTabState)->getRightScrollTabRect());
		popup->show(this->this$0->tabPane, $nc(rightScrollTabRect)->x + $nc(rightScrollTabRect)->width, $nc(rightScrollTabRect)->y + 7);
	}
...
$JMenuItem* AquaTabbedPaneUI$MouseHandler::createMenuItem(int32_t i) {
	...
	if (component == nullptr) {
		...
		$assign(menuItem, $new($JMenuItem, var$0, $(this->this$0->tabPane->getIconAt(i))));
	} else {
...
$Class* AquaTabbedPaneUI$MouseHandler::load$($String* name, bool initialize) {
	...
	$loadClass(AquaTabbedPaneUI$MouseHandler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
