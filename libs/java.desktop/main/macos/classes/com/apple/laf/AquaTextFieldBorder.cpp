...
AquaTextFieldBorder* AquaTextFieldBorder::getTextFieldBorder() {
	...
	return $cast(AquaTextFieldBorder, AquaTextFieldBorder::instance->get());
}
...
void AquaTextFieldBorder::init$() {
	...
	$nc(this->painter->state)->set($JRSUIConstants$FrameOnly::YES);
	$init($JRSUIConstants$Size);
	$nc(this->painter->state)->set($JRSUIConstants$Size::LARGE);
}
...
void AquaTextFieldBorder::paintBorder($Component* c, $Graphics* g, int32_t x, int32_t y, int32_t width, int32_t height) {
	...
	if (!($instanceOf($JTextComponent, c))) {
		...
		$nc(this->painter->state)->set($JRSUIConstants$Focused::NO);
	...
	$init($JRSUIConstants$State);
	$init($JRSUIConstants$Focused);
	$nc(this->painter->state)->set($JRSUIConstants$State::ACTIVE == state && $nc(jc)->hasFocus() ? $JRSUIConstants$Focused::YES : $JRSUIConstants$Focused::NO);
	if ($nc(jc)->isOpaque()) {
		this->painter->paint(g, c, x, y, width, height);
		return;
	}
...
$Insets* AquaTextFieldBorder::getSubInsets(int32_t shrinkage) {
	...
	if (shrinkage > 0) {
		return $new($InsetsUIResource, $nc(insets)->top - shrinkage, $nc(insets)->left, $nc(insets)->bottom - shrinkage, $nc(insets)->right);
	}
	return insets;
}

$Insets* AquaTextFieldBorder::getBorderInsets($Component* c) {
	if (!($instanceOf($JTextComponent, c)) || c->isOpaque()) {
...
void AquaTextFieldBorder::clinit$($Class* clazz) {
...
$Class* AquaTextFieldBorder::load$($String* name, bool initialize) {
	...
	$loadClass(AquaTextFieldBorder, name, initialize, &classInfo$$, AquaTextFieldBorder::clinit$, []($Class* clazz) -> $Object* {
