...
void AquaInternalFrameUI$AquaBorderListener::updateRollover($MouseEvent* e) {
	...
	if (var$0) {
		...
		var$0 = $nc(this->this$0->fAquaBorder)->getWithinRolloverArea(i, var$1, e->getY());
	}
...
void AquaInternalFrameUI$AquaBorderListener::mouseReleased($MouseEvent* e) {
	...
	if (this->resizeDir == AquaInternalFrameUI$AquaBorderListener::RESIZE_NONE) {
		...
		if ($instanceOf($JFrame, c)) {
			$$nc($$sure($JFrame, $$nc($AquaInternalFrameUI::access$1400(this->this$0))->getTopLevelAncestor())->getGlassPane())->setCursor($($Cursor::getPredefinedCursor($Cursor::DEFAULT_CURSOR)));
			$$nc($$sure($JFrame, $$nc($AquaInternalFrameUI::access$1500(this->this$0))->getTopLevelAncestor())->getGlassPane())->setVisible(false);
		} else if ($instanceOf($JApplet, c)) {
			$$nc($cast($JApplet, c)->getGlassPane())->setCursor($($Cursor::getPredefinedCursor($Cursor::DEFAULT_CURSOR)));
			$$nc($cast($JApplet, c)->getGlassPane())->setVisible(false);
		} else if ($instanceOf($JWindow, c)) {
			$$nc($cast($JWindow, c)->getGlassPane())->setCursor($($Cursor::getPredefinedCursor($Cursor::DEFAULT_CURSOR)));
			$$nc($cast($JWindow, c)->getGlassPane())->setVisible(false);
		} else if ($instanceOf($JDialog, c)) {
			$$nc($cast($JDialog, c)->getGlassPane())->setCursor($($Cursor::getPredefinedCursor($Cursor::DEFAULT_CURSOR)));
			$$nc($cast($JDialog, c)->getGlassPane())->setVisible(false);
		}
...
bool AquaInternalFrameUI$AquaBorderListener::isTitleBarDraggableArea($MouseEvent* e) {
	...
	if (point->x > $$nc($AquaInternalFrameUI::access$3000(this->this$0))->getWidth() - insets->left - insets->right) {
		return false;
	}
	return true;
}
...
void AquaInternalFrameUI$AquaBorderListener::mouseDragged($MouseEvent* e) {
	...
	if (var$3 || ((e->getModifiers() & $InputEvent::BUTTON1_MASK) != $InputEvent::BUTTON1_MASK)) {
		return;
	}
...
bool AquaInternalFrameUI$AquaBorderListener::didForwardEventInternal($MouseEvent* e) {
	...
	int32_t var$0 = e->getID();
	int64_t var$1 = e->getWhen();
	int32_t var$2 = e->getModifiers();
	int32_t var$3 = $nc(hitComponentPoint)->x;
	int32_t var$4 = hitComponentPoint->y;
	int32_t var$5 = e->getClickCount();
	bool var$6 = e->isPopupTrigger();
	$nc(hitComponent)->dispatchEvent($$new($MouseEvent, hitComponent, var$0, var$1, var$2, var$3, var$4, var$5, var$6, e->getButton()));
	return true;
}
...
bool AquaInternalFrameUI$AquaBorderListener::isEventInWindowShadow($Point* point) {
	...
	if (point->x > $nc(bounds)->width - insets->right) {
		return true;
	}
	if (point->y < insets->top) {
		return true;
	}
	if (point->y > bounds->height - insets->bottom) {
		return true;
	}
	return false;
}
...
$Class* AquaInternalFrameUI$AquaBorderListener::load$($String* name, bool initialize) {
	...
	$loadClass(AquaInternalFrameUI$AquaBorderListener, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
