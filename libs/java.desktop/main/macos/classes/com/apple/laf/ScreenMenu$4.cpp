...
void ScreenMenu$4::run() {
	...
	if (target != this->this$0->fLastMouseEventTarget) {
		...
		if (target != nullptr) {
			$LWToolkit::postEvent($$new($MouseEvent, target, $MouseEvent::MOUSE_ENTERED, this->val$when, this->val$modifiers, this->val$x - $nc(targetRect)->x, this->val$y - $nc(targetRect)->y, 0, false));
		}
	...
	$LWToolkit::postEvent($$new($MouseEvent, target, this->val$kind, this->val$when, this->val$modifiers, this->val$x - $nc(targetRect)->x, this->val$y - $nc(targetRect)->y, 0, false));
}
...
$Class* ScreenMenu$4::load$($String* name, bool initialize) {
	...
	$loadClass(ScreenMenu$4, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ScreenMenu$4);
	});
	return class$;
}
