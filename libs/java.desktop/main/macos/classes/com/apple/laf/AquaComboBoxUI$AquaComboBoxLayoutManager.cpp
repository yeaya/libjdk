...
void AquaComboBoxUI$AquaComboBoxLayoutManager::layoutContainer($Container* parent) {
	...
	if (var$0 && !$$nc($AquaComboBoxUI::access$5400(this->this$0))->isEditable()) {
		...
		$$nc($AquaComboBoxUI::access$5800(this->this$0))->setBounds($nc(insets)->left, $nc(insets)->top, width - ($nc(insets)->left + $nc(insets)->right), height - ($nc(insets)->top + $nc(insets)->bottom));
		return;
	}
...
$Class* AquaComboBoxUI$AquaComboBoxLayoutManager::load$($String* name, bool initialize) {
	...
	$loadClass(AquaComboBoxUI$AquaComboBoxLayoutManager, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AquaComboBoxUI$AquaComboBoxLayoutManager);
	});
	return class$;
}
