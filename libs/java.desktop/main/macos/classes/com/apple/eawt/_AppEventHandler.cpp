...
void _AppEventHandler::nativeOpenCocoaAboutWindow() {
	...
	$prepareNativeStatic(nativeOpenCocoaAboutWindow, void);
	$invokeNativeStatic();
	$finishNativeStatic();
}
...
void _AppEventHandler::nativeReplyToAppShouldTerminate(bool shouldTerminate) {
	$init(_AppEventHandler);
	$prepareNativeStatic(nativeReplyToAppShouldTerminate, void, bool shouldTerminate);
...
void _AppEventHandler::nativeRegisterForNotification(int32_t notification) {
	$init(_AppEventHandler);
	$prepareNativeStatic(nativeRegisterForNotification, void, int32_t notification);
...
void _AppEventHandler::init$() {
	...
	if ("CLOSE_ALL_WINDOWS"_s->equals(strategyProp)) {
		...
		bool var$0 = "SYSTEM_EXIT_O"_s->equals(strategyProp);
		if (var$0 || "NORMAL_EXIT"_s->equals(strategyProp)) {
...
void _AppEventHandler::addListener($SystemEventListener* listener) {
	if ($instanceOf($AppReopenedListener, listener)) {
		this->reOpenAppDispatcher->addListener($cast($AppReopenedListener, listener));
	}
	if ($instanceOf($AppForegroundListener, listener)) {
		this->foregroundAppDispatcher->addListener($cast($AppForegroundListener, listener));
	}
	if ($instanceOf($AppHiddenListener, listener)) {
		this->hiddenAppDispatcher->addListener($cast($AppHiddenListener, listener));
	}
	if ($instanceOf($UserSessionListener, listener)) {
		this->userSessionDispatcher->addListener($cast($UserSessionListener, listener));
	}
	if ($instanceOf($ScreenSleepListener, listener)) {
		this->screenSleepDispatcher->addListener($cast($ScreenSleepListener, listener));
	}
	if ($instanceOf($SystemSleepListener, listener)) {
		this->systemSleepDispatcher->addListener($cast($SystemSleepListener, listener));
	}
}

void _AppEventHandler::removeListener($SystemEventListener* listener) {
	if ($instanceOf($AppReopenedListener, listener)) {
		this->reOpenAppDispatcher->removeListener($cast($AppReopenedListener, listener));
	}
	if ($instanceOf($AppForegroundListener, listener)) {
		this->foregroundAppDispatcher->removeListener($cast($AppForegroundListener, listener));
	}
	if ($instanceOf($AppHiddenListener, listener)) {
		this->hiddenAppDispatcher->removeListener($cast($AppHiddenListener, listener));
	}
	if ($instanceOf($UserSessionListener, listener)) {
		this->userSessionDispatcher->removeListener($cast($UserSessionListener, listener));
	}
	if ($instanceOf($ScreenSleepListener, listener)) {
		this->screenSleepDispatcher->removeListener($cast($ScreenSleepListener, listener));
	}
	if ($instanceOf($SystemSleepListener, listener)) {
		this->systemSleepDispatcher->removeListener($cast($SystemSleepListener, listener));
	}
}
...
void _AppEventHandler::handlePrintFiles($List* filenames) {
	$init(_AppEventHandler);
	$useLocalObjectStack();
	$nc(_AppEventHandler::instance->printFilesDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, {filenames})));
}

void _AppEventHandler::handleOpenFiles($List* filenames, $String* searchTerm) {
	$init(_AppEventHandler);
	$useLocalObjectStack();
	$nc(_AppEventHandler::instance->openFilesDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, {
		filenames,
		searchTerm
	})));
}
...
void _AppEventHandler::handleOpenURI($String* uri) {
	$init(_AppEventHandler);
	$useLocalObjectStack();
	$nc(_AppEventHandler::instance->openURIDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, {uri})));
}
...
void _AppEventHandler::handleNativeNotification(int32_t code) {
	...
	case _AppEventHandler::NOTIFY_ABOUT:
		$nc(_AppEventHandler::instance->aboutDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, 0)));
		break;
	case _AppEventHandler::NOTIFY_PREFS:
		$nc(_AppEventHandler::instance->preferencesDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, 0)));
		break;
	case _AppEventHandler::NOTIFY_OPEN_APP:
		$nc(_AppEventHandler::instance->openAppDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, 0)));
		break;
	case _AppEventHandler::NOTIFY_REOPEN_APP:
		$nc(_AppEventHandler::instance->reOpenAppDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, 0)), $$new($ObjectArray, 0));
		break;
	case _AppEventHandler::NOTIFY_QUIT:
		$nc(_AppEventHandler::instance->quitDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, 0)));
		break;
	case _AppEventHandler::NOTIFY_SHUTDOWN:
		break;
	case _AppEventHandler::NOTIFY_ACTIVE_APP_GAINED:
		$nc(_AppEventHandler::instance->foregroundAppDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, {$Boolean::TRUE})), $$new($ObjectArray, 0));
		break;
	case _AppEventHandler::NOTIFY_ACTIVE_APP_LOST:
		$nc(_AppEventHandler::instance->foregroundAppDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, {$Boolean::FALSE})), $$new($ObjectArray, 0));
		break;
	case _AppEventHandler::NOTIFY_APP_HIDDEN:
		$nc(_AppEventHandler::instance->hiddenAppDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, {$Boolean::TRUE})), $$new($ObjectArray, 0));
		break;
	case _AppEventHandler::NOTIFY_APP_SHOWN:
		$nc(_AppEventHandler::instance->hiddenAppDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, {$Boolean::FALSE})), $$new($ObjectArray, 0));
		break;
	case _AppEventHandler::NOTIFY_USER_SESSION_ACTIVE:
		$nc(_AppEventHandler::instance->userSessionDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, {$Boolean::TRUE})), $$new($ObjectArray, 0));
		break;
	case _AppEventHandler::NOTIFY_USER_SESSION_INACTIVE:
		$nc(_AppEventHandler::instance->userSessionDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, {$Boolean::FALSE})), $$new($ObjectArray, 0));
		break;
	case _AppEventHandler::NOTIFY_SCREEN_SLEEP:
		$nc(_AppEventHandler::instance->screenSleepDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, {$Boolean::TRUE})), $$new($ObjectArray, 0));
		break;
	case _AppEventHandler::NOTIFY_SCREEN_WAKE:
		$nc(_AppEventHandler::instance->screenSleepDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, {$Boolean::FALSE})), $$new($ObjectArray, 0));
		break;
	case _AppEventHandler::NOTIFY_SYSTEM_SLEEP:
		$nc(_AppEventHandler::instance->systemSleepDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, {$Boolean::TRUE})), $$new($ObjectArray, 0));
		break;
	case _AppEventHandler::NOTIFY_SYSTEM_WAKE:
		$nc(_AppEventHandler::instance->systemSleepDispatcher)->dispatch($$new($_AppEventHandler$_NativeEvent, $$new($ObjectArray, {$Boolean::FALSE})), $$new($ObjectArray, 0));
		break;
	default:
...
void _AppEventHandler::clinit$($Class* clazz) {
...
$Class* _AppEventHandler::load$($String* name, bool initialize) {
	...
	$loadClass(_AppEventHandler, name, initialize, &classInfo$$, _AppEventHandler::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(_AppEventHandler);
	});
	return class$;
}
