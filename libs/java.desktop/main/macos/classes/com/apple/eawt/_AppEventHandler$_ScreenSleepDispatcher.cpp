...
$Class* _AppEventHandler$_ScreenSleepDispatcher::load$($String* name, bool initialize) {
	...
	$loadClass(_AppEventHandler$_ScreenSleepDispatcher, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(_AppEventHandler$_ScreenSleepDispatcher);
	});
	return class$;
}
