	...
	$synchronized(this->val$ret) {
		try {
			...
			if (pageResult != $Printable::NO_SUCH_PAGE) {
				this->val$ret->set(0, $(this->this$0->getPageFormatArea(this->val$pageFormat)));
			}
		} catch ($Exception& e) {
		}
	}
...
$Class* CPrinterJob$6::load$($String* name, bool initialize) {
	...
	$loadClass(CPrinterJob$6, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CPrinterJob$6);
	});
	return class$;
}
