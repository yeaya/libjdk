...
$Class* CViewEmbeddedFrame$$Lambda$lambda$validateWithBounds$0::load$($String* name, bool initialize) {
	...
	$loadClass(CViewEmbeddedFrame$$Lambda$lambda$validateWithBounds$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CViewEmbeddedFrame$$Lambda$lambda$validateWithBounds$0);
	});
	return class$;
}
...
void CViewEmbeddedFrame::addNotify() {
	...
	if (!isDisplayable()) {
		...
		setPeer($$cast($LWComponentPeer, $nc(toolkit)->createEmbeddedFrame(this)));
	}
...
void CViewEmbeddedFrame::validateWithBounds(int32_t x, int32_t y, int32_t width, int32_t height) {
	try {
		$LWCToolkit::invokeAndWait($$new(CViewEmbeddedFrame$$Lambda$lambda$validateWithBounds$0, this, width, height), this);
...
$Class* CViewEmbeddedFrame::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("sun.lwawt.macosx.CViewEmbeddedFrame$$Lambda$lambda$validateWithBounds$0")) {
	...
	$loadClass(CViewEmbeddedFrame, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
