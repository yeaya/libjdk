...
int32_t CDesktopPeer::_lsOpenURI($String* uri) {
	$init(CDesktopPeer);
	$prepareNativeStatic(_lsOpenURI, int32_t, $String* uri);
	int32_t $ret = $invokeNativeStatic(uri);
	$finishNativeStatic();
	return $ret;
}
...
int32_t CDesktopPeer::_lsOpenFile($String* path, bool print) {
	$init(CDesktopPeer);
	$prepareNativeStatic(_lsOpenFile, int32_t, $String* path, bool print);
	int32_t $ret = $invokeNativeStatic(path, print);
	$finishNativeStatic();
	return $ret;
}
...
$Class* CDesktopPeer::load$($String* name, bool initialize) {
	...
	$loadClass(CDesktopPeer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CDesktopPeer);
	});
	return class$;
}
