...
$String* NSEvent::toString() {
	...
	var$0->append("NSEvent["_s);
	var$0->append(getType());
	var$0->append(" ,"_s);
	var$0->append(getModifierFlags());
	var$0->append(" ,"_s);
	var$0->append(getClickCount());
	var$0->append(" ,"_s);
	var$0->append(getButtonNumber());
	var$0->append(" ,"_s);
	var$0->append(getX());
	var$0->append(" ,"_s);
	var$0->append(getY());
	var$0->append(" ,"_s);
	var$0->append(getAbsX());
	var$0->append(" ,"_s);
	var$0->append(getAbsY());
	var$0->append(" ,"_s);
	var$0->append(getKeyCode());
	var$0->append(" ,"_s);
	var$0->append($(getCharacters()));
	var$0->append(" ,"_s);
	var$0->append($(getCharactersIgnoringModifiers()));
	var$0->append("]"_s);
	return $str(var$0);
}
...
int32_t NSEvent::nsToJavaModifiers(int32_t modifierFlags) {
	$init(NSEvent);
	$prepareNativeStatic(nsToJavaModifiers, int32_t, int32_t modifierFlags);
	int32_t $ret = $invokeNativeStatic(modifierFlags);
	$finishNativeStatic();
	return $ret;
}
...
bool NSEvent::nsToJavaKeyInfo($ints* in, $ints* out) {
	$init(NSEvent);
	$prepareNativeStatic(nsToJavaKeyInfo, bool, $ints* in, $ints* out);
	bool $ret = $invokeNativeStatic(in, out);
	$finishNativeStatic();
	return $ret;
}
...
void NSEvent::nsKeyModifiersToJavaKeyInfo($ints* in, $ints* out) {
	$init(NSEvent);
	$prepareNativeStatic(nsKeyModifiersToJavaKeyInfo, void, $ints* in, $ints* out);
...
char16_t NSEvent::nsToJavaChar(char16_t nsChar, int32_t modifierFlags, bool spaceKeyTyped) {
	$init(NSEvent);
	$prepareNativeStatic(nsToJavaChar, char16_t, char16_t nsChar, int32_t modifierFlags, bool spaceKeyTyped);
	char16_t $ret = $invokeNativeStatic(nsChar, modifierFlags, spaceKeyTyped);
	$finishNativeStatic();
	return $ret;
}

bool NSEvent::isPopupTrigger(int32_t jmodifiers) {
	bool isRightButtonDown = ((jmodifiers & $InputEvent::BUTTON3_DOWN_MASK) != 0);
	bool isLeftButtonDown = ((jmodifiers & $InputEvent::BUTTON1_DOWN_MASK) != 0);
	bool isControlDown = ((jmodifiers & $InputEvent::CTRL_DOWN_MASK) != 0);
...
$Class* NSEvent::load$($String* name, bool initialize) {
	...
	$loadClass(NSEvent, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(NSEvent);
	});
	return class$;
}
