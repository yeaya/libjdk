...
void CAccessibleText$4::run() {
	...
	$nc(aet)->selectText(this->val$startIndex, this->val$endIndex);
}
...
$Class* CAccessibleText$4::load$($String* name, bool initialize) {
	...
	$loadClass(CAccessibleText$4, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CAccessibleText$4);
	});
	return class$;
}
