...
$Class* CPopupMenu$$Lambda$lambda$show$0::load$($String* name, bool initialize) {
	...
	$loadClass(CPopupMenu$$Lambda$lambda$show$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CPopupMenu$$Lambda$lambda$show$0);
	});
	return class$;
}
...
int64_t CPopupMenu::nativeCreatePopupMenu() {
	$prepareNative(nativeCreatePopupMenu, int64_t);
...
int64_t CPopupMenu::nativeShowPopupMenu(int64_t modelPtr, int32_t x, int32_t y) {
	$prepareNative(nativeShowPopupMenu, int64_t, int64_t modelPtr, int32_t x, int32_t y);
	int64_t $ret = $invokeNative(modelPtr, x, y);
	$finishNative();
	return $ret;
}
...
void CPopupMenu::show($Event* e) {
	...
	if (origin != nullptr) {
		...
		e->x += $nc(loc)->x;
		e->y += loc->y;
		execute($$new(CPopupMenu$$Lambda$lambda$show$0, this, e));
	}
}

void CPopupMenu::lambda$show$0($Event* e, int64_t ptr) {
	nativeShowPopupMenu(ptr, $nc(e)->x, $nc(e)->y);
}
...
$Class* CPopupMenu::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("sun.lwawt.macosx.CPopupMenu$$Lambda$lambda$show$0")) {
	...
	$loadClass(CPopupMenu, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
