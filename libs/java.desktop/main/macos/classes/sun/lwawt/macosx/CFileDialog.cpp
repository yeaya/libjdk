...
$StringArray* CFileDialog::nativeRunFileDialog($String* title, int32_t mode, bool multipleMode, bool shouldNavigateApps, bool canChooseDirectories, bool hasFilenameFilter, $String* directory, $String* file) {
	$prepareNative(nativeRunFileDialog, $StringArray*, $String* title, int32_t mode, bool multipleMode, bool shouldNavigateApps, bool canChooseDirectories, bool hasFilenameFilter, $String* directory, $String* file);
	$var($StringArray, $ret, $invokeNativeObject(title, mode, multipleMode, shouldNavigateApps, canChooseDirectories, hasFilenameFilter, directory, file));
	$finishNative();
	return $ret;
}
...
$Class* CFileDialog::load$($String* name, bool initialize) {
	...
	$loadClass(CFileDialog, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CFileDialog);
	});
	return class$;
}
