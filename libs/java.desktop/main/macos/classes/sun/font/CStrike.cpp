...
int64_t CStrike::createNativeStrikePtr(int64_t nativeFontPtr, $doubles* glyphTx, $doubles* invDevTxMatrix, int32_t aaHint, int32_t fmHint) {
	$init(CStrike);
	$prepareNativeStatic(createNativeStrikePtr, int64_t, int64_t nativeFontPtr, $doubles* glyphTx, $doubles* invDevTxMatrix, int32_t aaHint, int32_t fmHint);
	int64_t $ret = $invokeNativeStatic(nativeFontPtr, glyphTx, invDevTxMatrix, aaHint, fmHint);
	$finishNativeStatic();
	return $ret;
}
...
void CStrike::disposeNativeStrikePtr(int64_t nativeStrikePtr) {
	$init(CStrike);
	$prepareNativeStatic(disposeNativeStrikePtr, void, int64_t nativeStrikePtr);
...
$StrikeMetrics* CStrike::getFontMetrics(int64_t nativeStrikePtr) {
	$init(CStrike);
	$prepareNativeStatic(getFontMetrics, $StrikeMetrics*, int64_t nativeStrikePtr);
	$var($StrikeMetrics, $ret, $invokeNativeStaticObject(nativeStrikePtr));
	$finishNativeStatic();
	return $ret;
}
...
void CStrike::getGlyphImagePtrsNative(int64_t nativeStrikePtr, $longs* glyphInfos, $ints* uniCodes, int32_t len) {
	$init(CStrike);
	$prepareNativeStatic(getGlyphImagePtrsNative, void, int64_t nativeStrikePtr, $longs* glyphInfos, $ints* uniCodes, int32_t len);
...
float CStrike::getNativeGlyphAdvance(int64_t nativeStrikePtr, int32_t glyphCode) {
	$init(CStrike);
	$prepareNativeStatic(getNativeGlyphAdvance, float, int64_t nativeStrikePtr, int32_t glyphCode);
	float $ret = $invokeNativeStatic(nativeStrikePtr, glyphCode);
	$finishNativeStatic();
	return $ret;
}
...
$GeneralPath* CStrike::getNativeGlyphOutline(int64_t nativeStrikePtr, int32_t glyphCode, double x, double y) {
	$init(CStrike);
	$prepareNativeStatic(getNativeGlyphOutline, $GeneralPath*, int64_t nativeStrikePtr, int32_t glyphCode, double x, double y);
	$var($GeneralPath, $ret, $invokeNativeStaticObject(nativeStrikePtr, glyphCode, x, y));
	$finishNativeStatic();
	return $ret;
}
...
void CStrike::getNativeGlyphImageBounds(int64_t nativeStrikePtr, int32_t glyphCode, $Rectangle2D$Float* result, double x, double y) {
	$init(CStrike);
	$prepareNativeStatic(getNativeGlyphImageBounds, void, int64_t nativeStrikePtr, int32_t glyphCode, $Rectangle2D$Float* result, double x, double y);
...
void CStrike::init$($CFont* font, $FontStrikeDesc* inDesc) {
	...
	if ($nc(inDesc)->devTx != nullptr && !inDesc->devTx->isIdentity()) {
		try {
			$set(this, invDevTx, inDesc->devTx->createInverse());
...
int64_t CStrike::getNativeStrikePtr() {
	...
	if (this->invDevTx == nullptr) {
		invDevTxMatrix->set(0, 1);
		invDevTxMatrix->set(3, 1);
	} else {
		this->invDevTx->getMatrix(invDevTxMatrix);
	}
	int32_t aaHint = $nc(this->desc)->aaHint;
	int32_t fmHint = this->desc->fmHint;
	$synchronized(this) {
		if (this->nativeStrikePtr != 0) {
			return this->nativeStrikePtr;
		}
...
void CStrike::getGlyphImageBounds(int32_t glyphCode, $Point2D$Float* pt, $Rectangle* result) {
	...
	getGlyphImageBounds(glyphCode, $nc(pt)->x, $nc(pt)->y, floatRect);
	if (floatRect->width == 0 && floatRect->height == 0) {
		$nc(result)->setRect(0, 0, -1, -1);
		return;
	}
	$nc(result)->setRect(floatRect->x + pt->x, floatRect->y + pt->y, floatRect->width, floatRect->height);
}
...
int64_t CStrike::getGlyphImagePtr(int32_t glyphCode) {
	...
	$synchronized(this->glyphInfoCache) {
		int64_t ptr = this->glyphInfoCache->get(glyphCode);
		if (ptr != 0) {
			return ptr;
		}
...
void CStrike::getGlyphImagePtrs($ints* glyphCodes, $longs* images, int32_t len) {
	...
	$synchronized(this->glyphInfoCache) {
		...
		for (int32_t i = 0; i < len; ++i) {
			...
			int64_t ptr = this->glyphInfoCache->get(code);
			if (ptr != 0) {
				$nc(images)->set(i, ptr);
			} else {
				$nc(images)->set(i, 0);
		...
		int32_t dupes = 0;
		for (int32_t i = 0; i < len; ++i) {
			if ($nc(images)->get(i) != 0) {
				continue;
			}
			int32_t code = $nc(glyphCodes)->get(i);
			if (this->glyphInfoCache->get(code) == -1) {
			...
			this->glyphInfoCache->put(code, -1);
		...
		j = 0;
		for (int32_t i = 0; i < len; ++i) {
			if ($nc(images)->get(i) != 0 && images->get(i) != -1) {
				continue;
			}
			int32_t k = filteredIndicies->get(j);
			int32_t code = $nc(glyphCodes)->get(i);
			if (k == -1) {
				images->set(i, this->glyphInfoCache->get(code));
			} else {
...
$Class* CStrike::load$($String* name, bool initialize) {
	...
	$loadClass(CStrike, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CStrike);
	});
	return class$;
}
