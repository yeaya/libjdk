...
$SurfaceData* OSXOffScreenSurfaceData::getSurfaceData($BufferedImage* bufImg) {
	$init(OSXOffScreenSurfaceData);
	$prepareNativeStatic(getSurfaceData, $SurfaceData*, $BufferedImage* bufImg);
	$var($SurfaceData, $ret, $invokeNativeStaticObject(bufImg));
	$finishNativeStatic();
	return $ret;
}
...
void OSXOffScreenSurfaceData::setSurfaceData($BufferedImage* bufImg, $SurfaceData* sData) {
	$init(OSXOffScreenSurfaceData);
	$prepareNativeStatic(setSurfaceData, void, $BufferedImage* bufImg, $SurfaceData* sData);
...
OSXOffScreenSurfaceData* OSXOffScreenSurfaceData::createNewSurface($BufferedImage* bufImg) {
	...
	default:
		{
			...
			if ((numOfComponents == 3) && ($instanceOf($ComponentColorModel, cm)) && ($instanceOf($PixelInterleavedSampleModel, sm))) {
				...
				$var($ints, offs, $cast($ComponentSampleModel, sm)->getBandOffsets());
				...
				if ((pixelSize == 32) && (transferType == $DataBuffer::TYPE_INT)) {
					...
					if (var$0 && cm->isAlphaPremultiplied() && sizes->get(3) == 8) {
						...
						bool var$1 = validsizes && littleendian && cm->hasAlpha();
						if (var$1 && cm->isAlphaPremultiplied() && sizes->get(3) == 8) {
							try {
					...
					bool var$2 = validsizes && bigendian && cm->hasAlpha();
					...
					if (validsizes && bigendian && cm->hasAlpha() && sizes->get(3) == 8) {
						...
						bool var$3 = validsizes && littleendian && cm->hasAlpha();
	...
	if (sData == nullptr) {
		...
		int32_t var$4 = bufImg->getWidth();
		int32_t var$5 = bufImg->getHeight();
		$var($IntegerNIORaster, backupRaster, $cast($IntegerNIORaster, $IntegerNIORaster::createNIORaster(var$4, var$5, $($nc(OSXOffScreenSurfaceData::dcmBackup)->getMasks()), nullptr)));
		$set(offsd, bimBackup, $new($BufferedImage, OSXOffScreenSurfaceData::dcmBackup, backupRaster, $nc(OSXOffScreenSurfaceData::dcmBackup)->isAlphaPremultiplied(), nullptr));
		$var($IntBuffer, var$6, $nc(backupRaster)->getBuffer());
		int32_t var$7 = backupRaster->getWidth();
		offsd->initCustomRaster(var$6, var$7, backupRaster->getHeight(), offsd->fGraphicsStates, offsd->fGraphicsStatesObject, offsd->fImageInfo);
...
$SurfaceData* OSXOffScreenSurfaceData::createDataIC($BufferedImage* bImg, $SurfaceType* sType, int32_t iType) {
	...
	$var($Object, var$0, $nc(icRaster)->getDataStorage());
...
$SurfaceData* OSXOffScreenSurfaceData::createDataSC($BufferedImage* bImg, $SurfaceType* sType, $IndexColorModel* icm, int32_t iType) {
	...
	$var($Object, var$0, $nc(scRaster)->getDataStorage());
...
$SurfaceData* OSXOffScreenSurfaceData::createDataBC($BufferedImage* bImg, $SurfaceType* sType, int32_t primaryBank, int32_t iType) {
	...
	$var($Object, var$0, $nc(bcRaster)->getDataStorage());
...
$SurfaceData* OSXOffScreenSurfaceData::createDataBP($BufferedImage* bImg, $SurfaceType* sType, int32_t iType) {
	...
	$var($ColorModel, cm, bImg->getColorModel());
	$var($IndexColorModel, icm, ($instanceOf($IndexColorModel, cm)) ? $cast($IndexColorModel, cm) : ($IndexColorModel*)nullptr);
	$var($Object, var$0, $nc(bpRaster)->getDataStorage());
...
void OSXOffScreenSurfaceData::initRaster(Object$* theArray, int32_t offset, int32_t width, int32_t height, int32_t pixStr, int32_t scanStr, $IndexColorModel* icm, int32_t type, $ByteBuffer* graphicsStates, Object$* graphicsStatesObjects, $ByteBuffer* imageInfo) {
	$prepareNative(initRaster, void, Object$* theArray, int32_t offset, int32_t width, int32_t height, int32_t pixStr, int32_t scanStr, $IndexColorModel* icm, int32_t type, $ByteBuffer* graphicsStates, Object$* graphicsStatesObjects, $ByteBuffer* imageInfo);
...
void OSXOffScreenSurfaceData::initCustomRaster($IntBuffer* buffer, int32_t width, int32_t height, $ByteBuffer* graphicsStates, Object$* graphicsStatesObjects, $ByteBuffer* imageInfo) {
	$prepareNative(initCustomRaster, void, $IntBuffer* buffer, int32_t width, int32_t height, $ByteBuffer* graphicsStates, Object$* graphicsStatesObjects, $ByteBuffer* imageInfo);
...
bool OSXOffScreenSurfaceData::xorSurfacePixels($SurfaceData* src, int32_t colorXOR, int32_t x, int32_t y, int32_t w, int32_t h) {
	$prepareNative(xorSurfacePixels, bool, $SurfaceData* src, int32_t colorXOR, int32_t x, int32_t y, int32_t w, int32_t h);
	bool $ret = $invokeNative(src, colorXOR, x, y, w, h);
	$finishNative();
	return $ret;
}
...
bool OSXOffScreenSurfaceData::clearSurfacePixels(int32_t w, int32_t h) {
	$prepareNative(clearSurfacePixels, bool, int32_t w, int32_t h);
	bool $ret = $invokeNative(w, h);
	$finishNative();
	return $ret;
}
...
$SurfaceData* OSXOffScreenSurfaceData::getCopyWithBgColor($Color* bgColor) {
	...
	bool var$0 = (this->copyWithBgColor_cache == nullptr) || (this->copyWithBgColor_cache->getWidth() < bimW);
	if (var$0 || (this->copyWithBgColor_cache->getHeight() < bimH)) {
...
void OSXOffScreenSurfaceData::syncToJavaPixels() {
	$prepareNative(syncToJavaPixels, void);
	$invokeNative();
	$finishNative();
}
...
void OSXOffScreenSurfaceData::clinit$($Class* clazz) {
	{
...
$Class* OSXOffScreenSurfaceData::load$($String* name, bool initialize) {
	...
	$loadClass(OSXOffScreenSurfaceData, name, initialize, &classInfo$$, OSXOffScreenSurfaceData::clinit$, []($Class* clazz) -> $Object* {
