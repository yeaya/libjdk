...
void CGLSurfaceData::initOps($OGLGraphicsConfig* gc, int64_t pConfigInfo, int64_t pPeerData, int64_t layerPtr, int32_t xoff, int32_t yoff, bool isOpaque) {
	$prepareNative(initOps, void, $OGLGraphicsConfig* gc, int64_t pConfigInfo, int64_t pPeerData, int64_t layerPtr, int32_t xoff, int32_t yoff, bool isOpaque);
...
void CGLSurfaceData::init$($CGLLayer* layer, $CGLGraphicsConfig* gc, $ColorModel* cm, int32_t type, int32_t width, int32_t height) {
	...
	this->scale = type == $AccelSurface::TEXTURE ? 1 : $$sure($CGraphicsDevice, $nc(gc)->getDevice())->getScaleFactor();
	...
	initOps(gc, $nc(gc)->getNativeConfigInfo(), 0, layerPtr, 0, 0, isOpaque);
}
...
$CGLSurfaceData$CGLLayerSurfaceData* CGLSurfaceData::createData($CGLLayer* layer) {
	...
	return $new($CGLSurfaceData$CGLLayerSurfaceData, layer, gc, $nc(r)->width, $nc(r)->height);
}
...
$Class* CGLSurfaceData::load$($String* name, bool initialize) {
	...
	$loadClass(CGLSurfaceData, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
