...
void MTLRenderer::validateContext($SunGraphics2D* sg2d) {
	...
	try {
		$assign(dstData, $cast($MTLSurfaceData, sg2d->surfaceData));
	} catch ($ClassCastException& e) {
		$throwNew($InvalidPipeException, $$str({"wrong surface data type: "_s, sg2d->surfaceData}));
	}
	$MTLContext::validateContext(dstData, dstData, $(sg2d->getCompClip()), sg2d->composite, nullptr, sg2d->paint, sg2d, ctxflags);
}
...
void MTLRenderer::validateContextAA($SunGraphics2D* sg2d) {
	...
	$MTLContext::validateContext(dstData, dstData, $($nc(sg2d)->getCompClip()), $nc(sg2d)->composite, nullptr, $nc(sg2d)->paint, sg2d, ctxflags);
}
...
void MTLRenderer::copyArea($SunGraphics2D* sg2d, int32_t x, int32_t y, int32_t w, int32_t h, int32_t dx, int32_t dy) {
	...
	try {
		...
		$MTLContext::validateContext(dstData, dstData, $(sg2d->getCompClip()), sg2d->composite, nullptr, nullptr, nullptr, ctxflags);
...
$Class* MTLRenderer::load$($String* name, bool initialize) {
	...
	$loadClass(MTLRenderer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
