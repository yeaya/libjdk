...
void JRSUIConstants$Key::clinit$($Class* clazz) {
...
$Class* JRSUIConstants$Key::load$($String* name, bool initialize) {
	...
	$loadClass(JRSUIConstants$Key, name, initialize, &classInfo$$, JRSUIConstants$Key::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(JRSUIConstants$Key);
	});
	return class$;
}
