...
int64_t JRSUIConstants::getPtrForConstant(int32_t constant) {
	$init(JRSUIConstants);
	$prepareNativeStatic(getPtrForConstant, int64_t, int32_t constant);
	int64_t $ret = $invokeNativeStatic(constant);
	$finishNativeStatic();
	return $ret;
}
...
$Class* JRSUIConstants::load$($String* name, bool initialize) {
	...
	$loadClass(JRSUIConstants, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JRSUIConstants);
	});
	return class$;
}
