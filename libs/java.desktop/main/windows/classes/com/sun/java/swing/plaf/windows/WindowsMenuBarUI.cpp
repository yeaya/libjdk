...
void WindowsMenuBarUI::installWindowListener() {
	if (this->windowListener == nullptr) {
		...
		if ($instanceOf($Window, component)) {
			...
			$cast($Window, component)->addWindowListener(this->windowListener);
		}
	}
}
...
bool WindowsMenuBarUI::isActive($JComponent* c) {
	...
	if (rootPane != nullptr) {
		...
		if ($instanceOf($Window, component)) {
			return $cast($Window, component)->isActive();
		}
	}
	return true;
}
...
$Class* WindowsMenuBarUI::load$($String* name, bool initialize) {
	...
	$loadClass(WindowsMenuBarUI, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(WindowsMenuBarUI);
	});
	return class$;
}
