...
AnimationController$PartUIClientPropertyKey* AnimationController$PartUIClientPropertyKey::getKey($TMSchema$Part* part) {
	...
	$synchronized(class$) {
		$var(AnimationController$PartUIClientPropertyKey, rv, $cast(AnimationController$PartUIClientPropertyKey, AnimationController$PartUIClientPropertyKey::map->get(part)));
		if (rv == nullptr) {
			$assign(rv, $new(AnimationController$PartUIClientPropertyKey, part));
			AnimationController$PartUIClientPropertyKey::map->put(part, rv);
		}
		return rv;
	}
}
...
void AnimationController$PartUIClientPropertyKey::clinit$($Class* clazz) {
	$load($TMSchema$Part);
...
$Class* AnimationController$PartUIClientPropertyKey::load$($String* name, bool initialize) {
	...
	$loadClass(AnimationController$PartUIClientPropertyKey, name, initialize, &classInfo$$, AnimationController$PartUIClientPropertyKey::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(AnimationController$PartUIClientPropertyKey);
	});
	return class$;
}
