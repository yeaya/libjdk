		...
		for (; $nc(i$)->hasNext();) {
			$var($Integer, i, $cast($Integer, i$->next()));
			{
				...
				if (!var$0) {
					...
					var$0 = var$1 && i->intValue() > retindex;
				}
				if (var$0) {
					retindex = $nc(i)->intValue();
					dist = $Math::abs(i->intValue() - w);
					$assign(retVal, $cast($Image, this->resolutionVariants->get(i)));
					if (i->intValue() == w) {
						break;
					}
				}
...
$Class* Win32ShellFolder2$MultiResolutionIconImage::load$($String* name, bool initialize) {
	...
	$loadClass(Win32ShellFolder2$MultiResolutionIconImage, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
