...
void Win32GraphicsDevice::initDevice(int32_t screen) {
	$prepareNative(initDevice, void, int32_t screen);
	$invokeNative(screen);
	$finishNative();
}

void Win32GraphicsDevice::initNativeScale(int32_t screen) {
	$prepareNative(initNativeScale, void, int32_t screen);
	$invokeNative(screen);
	$finishNative();
}

void Win32GraphicsDevice::setNativeScale(int32_t screen, float scaleX, float scaleY) {
	$prepareNative(setNativeScale, void, int32_t screen, float scaleX, float scaleY);
	$invokeNative(screen, scaleX, scaleY);
	$finishNative();
}

float Win32GraphicsDevice::getNativeScaleX(int32_t screen) {
	$prepareNative(getNativeScaleX, float, int32_t screen);
	float $ret = $invokeNative(screen);
	$finishNative();
	return $ret;
}

float Win32GraphicsDevice::getNativeScaleY(int32_t screen) {
	$prepareNative(getNativeScaleY, float, int32_t screen);
	float $ret = $invokeNative(screen);
	$finishNative();
	return $ret;
}
	...
	if (this->configs == nullptr) {
		...
		if (var$0 && isDefaultDevice()) {
			...
			if (this->defaultConfig != nullptr) {
				...
				return $cast($GraphicsConfigurationArray, this->configs->clone());
			}
		}
...
int32_t Win32GraphicsDevice::getMaxConfigsImpl(int32_t screen) {
	$prepareNative(getMaxConfigsImpl, int32_t, int32_t screen);
...
bool Win32GraphicsDevice::isPixFmtSupported(int32_t index, int32_t screen) {
	$prepareNative(isPixFmtSupported, bool, int32_t index, int32_t screen);
	bool $ret = $invokeNative(index, screen);
	$finishNative();
	return $ret;
}
...
int32_t Win32GraphicsDevice::getDefaultPixIDImpl(int32_t screen) {
	$prepareNative(getDefaultPixIDImpl, int32_t, int32_t screen);
	int32_t $ret = $invokeNative(screen);
	$finishNative();
	return $ret;
}
...
void Win32GraphicsDevice::enterFullScreenExclusive(int32_t screen, $WindowPeer* w) {
	$prepareNative(enterFullScreenExclusive, void, int32_t screen, $WindowPeer* w);
	$invokeNative(screen, w);
	$finishNative();
}

void Win32GraphicsDevice::exitFullScreenExclusive(int32_t screen, $WindowPeer* w) {
	$prepareNative(exitFullScreenExclusive, void, int32_t screen, $WindowPeer* w);
	$invokeNative(screen, w);
	$finishNative();
}
...
void Win32GraphicsDevice::setDisplayMode($DisplayMode* dm$renamed) {
	$synchronized(this) {
		...
		if (w != nullptr) {
			...
			configDisplayMode(var$0, peer, var$1, var$2, var$3, dm->getRefreshRate());
			$var($Rectangle, screenBounds, $$nc(getDefaultConfiguration())->getBounds());
			w->setBounds($nc(screenBounds)->x, $nc(screenBounds)->y, $nc(screenBounds)->width, $nc(screenBounds)->height);
		} else {
...
$DisplayMode* Win32GraphicsDevice::getCurrentDisplayMode(int32_t screen) {
	$prepareNative(getCurrentDisplayMode, $DisplayMode*, int32_t screen);
	$var($DisplayMode, $ret, $invokeNativeObject(screen));
	$finishNative();
	return $ret;
}
...
void Win32GraphicsDevice::configDisplayMode(int32_t screen, $WindowPeer* w, int32_t width, int32_t height, int32_t bitDepth, int32_t refreshRate) {
	$prepareNative(configDisplayMode, void, int32_t screen, $WindowPeer* w, int32_t width, int32_t height, int32_t bitDepth, int32_t refreshRate);
...
void Win32GraphicsDevice::enumDisplayModes(int32_t screen, $ArrayList* modes) {
	$prepareNative(enumDisplayModes, void, int32_t screen, $ArrayList* modes);
...
$ColorModel* Win32GraphicsDevice::makeColorModel(int32_t screen, bool dynamic) {
	$prepareNative(makeColorModel, $ColorModel*, int32_t screen, bool dynamic);
	$var($ColorModel, $ret, $invokeNativeObject(screen, dynamic));
	$finishNative();
	return $ret;
}
...
void Win32GraphicsDevice::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$beforeCallerSensitive();
	{
		$var($String, nopixfmt, $cast($String, $AccessController::doPrivileged($$new($GetPropertyAction, "sun.awt.nopixfmt"_s))));
...
$Class* Win32GraphicsDevice::load$($String* name, bool initialize) {
	...
	$loadClass(Win32GraphicsDevice, name, initialize, &classInfo$$, Win32GraphicsDevice::clinit$, []($Class* clazz) -> $Object* {
