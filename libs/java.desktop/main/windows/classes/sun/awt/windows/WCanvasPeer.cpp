...
void WCanvasPeer::paint($Graphics* g) {
	...
	if ($instanceOf($Graphics2D, g)) {
		g->clearRect(0, 0, $nc(d)->width, $nc(d)->height);
	} else {
		$nc(g)->setColor($($nc($cast($Component, this->target))->getBackground()));
		g->fillRect(0, 0, $nc(d)->width, $nc(d)->height);
...
void WCanvasPeer::setNativeBackgroundErase(bool doErase, bool doEraseOnResize) {
	$prepareNative(setNativeBackgroundErase, void, bool doErase, bool doEraseOnResize);
...
$Class* WCanvasPeer::load$($String* name, bool initialize) {
	...
	$loadClass(WCanvasPeer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
