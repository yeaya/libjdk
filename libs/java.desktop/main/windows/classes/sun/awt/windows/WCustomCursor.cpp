...
void WCustomCursor::createNativeCursor($Image* im, $ints* pixels, int32_t w, int32_t h, int32_t xHotSpot, int32_t yHotSpot) {
	...
	}
	{
		...
		$var($ints, var$2, $$sure($DataBufferInt, $$nc(bimage->getRaster())->getDataBuffer())->getData());
		int32_t var$3 = raster->getWidth();
		createCursorIndirect(var$2, andMask, ficW, var$3, raster->getHeight(), xHotSpot, yHotSpot);
	}
}
...
void WCustomCursor::createCursorIndirect($ints* rData, $bytes* andMask, int32_t nScanStride, int32_t width, int32_t height, int32_t xHotSpot, int32_t yHotSpot) {
	$prepareNative(createCursorIndirect, void, $ints* rData, $bytes* andMask, int32_t nScanStride, int32_t width, int32_t height, int32_t xHotSpot, int32_t yHotSpot);
...
int32_t WCustomCursor::getCursorWidth() {
	$init(WCustomCursor);
	$prepareNativeStatic(getCursorWidth, int32_t);
...
int32_t WCustomCursor::getCursorHeight() {
	$init(WCustomCursor);
	$prepareNativeStatic(getCursorHeight, int32_t);
	int32_t $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}
...
$Class* WCustomCursor::load$($String* name, bool initialize) {
	...
	$loadClass(WCustomCursor, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(WCustomCursor);
	});
	return class$;
}
