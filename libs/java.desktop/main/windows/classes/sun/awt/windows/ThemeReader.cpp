...
bool ThemeReader::initThemes() {
	$init(ThemeReader);
	$prepareNativeStatic(initThemes, bool);
...
$Long* ThemeReader::getThemeImpl($String* widget) {
	$init(ThemeReader);
	$useLocalObjectStack();
	$var($Long, theme, $cast($Long, ThemeReader::widgetToTheme->get(widget)));
	if (theme == nullptr) {
		...
		ThemeReader::widgetToTheme->put(widget, theme);
	}
	return theme;
}
...
$Long* ThemeReader::getTheme($String* widget) {
	...
	if (!ThemeReader::valid) {
		...
		try {
			if (!ThemeReader::valid) {
				{
					$var($Iterator, i$, $$nc(ThemeReader::widgetToTheme->values())->iterator());
					for (; $nc(i$)->hasNext();) {
...
void ThemeReader::paintBackground($ints* buffer, int64_t theme, int32_t part, int32_t state, int32_t x, int32_t y, int32_t w, int32_t h, int32_t stride) {
	$init(ThemeReader);
	$prepareNativeStatic(paintBackground, void, $ints* buffer, int64_t theme, int32_t part, int32_t state, int32_t x, int32_t y, int32_t w, int32_t h, int32_t stride);
...
$Insets* ThemeReader::getThemeMargins(int64_t theme, int32_t part, int32_t state, int32_t marginType) {
	$init(ThemeReader);
	$prepareNativeStatic(getThemeMargins, $Insets*, int64_t theme, int32_t part, int32_t state, int32_t marginType);
	$var($Insets, $ret, $invokeNativeStaticObject(theme, part, state, marginType));
	$finishNativeStatic();
	return $ret;
}
...
bool ThemeReader::isThemePartDefined(int64_t theme, int32_t part, int32_t state) {
	$init(ThemeReader);
	$prepareNativeStatic(isThemePartDefined, bool, int64_t theme, int32_t part, int32_t state);
	bool $ret = $invokeNativeStatic(theme, part, state);
	$finishNativeStatic();
	return $ret;
}
...
$Color* ThemeReader::getColor(int64_t theme, int32_t part, int32_t state, int32_t property) {
	$init(ThemeReader);
	$prepareNativeStatic(getColor, $Color*, int64_t theme, int32_t part, int32_t state, int32_t property);
	$var($Color, $ret, $invokeNativeStaticObject(theme, part, state, property));
	$finishNativeStatic();
	return $ret;
}
...
int32_t ThemeReader::getInt(int64_t theme, int32_t part, int32_t state, int32_t property) {
	$init(ThemeReader);
	$prepareNativeStatic(getInt, int32_t, int64_t theme, int32_t part, int32_t state, int32_t property);
	int32_t $ret = $invokeNativeStatic(theme, part, state, property);
	$finishNativeStatic();
	return $ret;
}
...
int32_t ThemeReader::getEnum(int64_t theme, int32_t part, int32_t state, int32_t property) {
	$init(ThemeReader);
	$prepareNativeStatic(getEnum, int32_t, int64_t theme, int32_t part, int32_t state, int32_t property);
	int32_t $ret = $invokeNativeStatic(theme, part, state, property);
	$finishNativeStatic();
	return $ret;
}
...
bool ThemeReader::getBoolean(int64_t theme, int32_t part, int32_t state, int32_t property) {
	$init(ThemeReader);
	$prepareNativeStatic(getBoolean, bool, int64_t theme, int32_t part, int32_t state, int32_t property);
	bool $ret = $invokeNativeStatic(theme, part, state, property);
	$finishNativeStatic();
	return $ret;
}
...
bool ThemeReader::getSysBoolean(int64_t theme, int32_t property) {
	$init(ThemeReader);
	$prepareNativeStatic(getSysBoolean, bool, int64_t theme, int32_t property);
	bool $ret = $invokeNativeStatic(theme, property);
	$finishNativeStatic();
	return $ret;
}
...
$Point* ThemeReader::getPoint(int64_t theme, int32_t part, int32_t state, int32_t property) {
	$init(ThemeReader);
	$prepareNativeStatic(getPoint, $Point*, int64_t theme, int32_t part, int32_t state, int32_t property);
	$var($Point, $ret, $invokeNativeStaticObject(theme, part, state, property));
	$finishNativeStatic();
	return $ret;
}
...
$Dimension* ThemeReader::getPosition(int64_t theme, int32_t part, int32_t state, int32_t property) {
	$init(ThemeReader);
	$prepareNativeStatic(getPosition, $Dimension*, int64_t theme, int32_t part, int32_t state, int32_t property);
	$var($Dimension, $ret, $invokeNativeStaticObject(theme, part, state, property));
	$finishNativeStatic();
	return $ret;
}
...
$Dimension* ThemeReader::getPartSize(int64_t theme, int32_t part, int32_t state) {
	$init(ThemeReader);
	$prepareNativeStatic(getPartSize, $Dimension*, int64_t theme, int32_t part, int32_t state);
	$var($Dimension, $ret, $invokeNativeStaticObject(theme, part, state));
	$finishNativeStatic();
	return $ret;
}
...
int64_t ThemeReader::openTheme($String* widget) {
	$init(ThemeReader);
	$prepareNativeStatic(openTheme, int64_t, $String* widget);
	int64_t $ret = $invokeNativeStatic(widget);
	$finishNativeStatic();
	return $ret;
}
...
void ThemeReader::closeTheme(int64_t theme) {
	$init(ThemeReader);
	$prepareNativeStatic(closeTheme, void, int64_t theme);
...
void ThemeReader::setWindowTheme($String* subAppName) {
	$init(ThemeReader);
	$prepareNativeStatic(setWindowTheme, void, $String* subAppName);
...
int64_t ThemeReader::getThemeTransitionDuration(int64_t theme, int32_t part, int32_t stateFrom, int32_t stateTo, int32_t propId) {
	$init(ThemeReader);
	$prepareNativeStatic(getThemeTransitionDuration, int64_t, int64_t theme, int32_t part, int32_t stateFrom, int32_t stateTo, int32_t propId);
	int64_t $ret = $invokeNativeStatic(theme, part, stateFrom, stateTo, propId);
	$finishNativeStatic();
	return $ret;
}
...
$Insets* ThemeReader::getThemeBackgroundContentMargins(int64_t theme, int32_t part, int32_t state, int32_t boundingWidth, int32_t boundingHeight) {
	$init(ThemeReader);
	$prepareNativeStatic(getThemeBackgroundContentMargins, $Insets*, int64_t theme, int32_t part, int32_t state, int32_t boundingWidth, int32_t boundingHeight);
	$var($Insets, $ret, $invokeNativeStaticObject(theme, part, state, boundingWidth, boundingHeight));
	$finishNativeStatic();
	return $ret;
}
...
void ThemeReader::clinit$($Class* clazz) {
	$assignStatic(ThemeReader::widgetToTheme, $new($HashMap));
	$assignStatic(ThemeReader::readWriteLock, $new($ReentrantReadWriteLock));
	$assignStatic(ThemeReader::readLock, ThemeReader::readWriteLock->readLock());
	$assignStatic(ThemeReader::writeLock, ThemeReader::readWriteLock->writeLock());
	ThemeReader::valid = false;
}
...
$Class* ThemeReader::load$($String* name, bool initialize) {
	...
	$loadClass(ThemeReader, name, initialize, &classInfo$$, ThemeReader::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ThemeReader);
	});
	return class$;
}
