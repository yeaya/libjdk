...
int32_t WDropTargetContextPeerIStream::Available(int64_t istream) {
	$prepareNative(Available, int32_t, int64_t istream);
	int32_t $ret = $invokeNative(istream);
	$finishNative();
	return $ret;
}
...
int32_t WDropTargetContextPeerIStream::Read(int64_t istream) {
	$prepareNative(Read, int32_t, int64_t istream);
	int32_t $ret = $invokeNative(istream);
	$finishNative();
	return $ret;
}
...
int32_t WDropTargetContextPeerIStream::ReadBytes(int64_t istream, $bytes* b, int32_t off, int32_t len) {
	$prepareNative(ReadBytes, int32_t, int64_t istream, $bytes* b, int32_t off, int32_t len);
	int32_t $ret = $invokeNative(istream, b, off, len);
	$finishNative();
	return $ret;
}
...
void WDropTargetContextPeerIStream::Close(int64_t istream) {
	$prepareNative(Close, void, int64_t istream);
...
$Class* WDropTargetContextPeerIStream::load$($String* name, bool initialize) {
	...
	$loadClass(WDropTargetContextPeerIStream, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(WDropTargetContextPeerIStream);
	});
	return class$;
}
