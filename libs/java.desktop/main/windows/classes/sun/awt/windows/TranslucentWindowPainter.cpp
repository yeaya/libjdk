...
TranslucentWindowPainter* TranslucentWindowPainter::createInstance($WWindowPeer* peer) {
	...
	if (!TranslucentWindowPainter::forceSW && $instanceOf($AccelGraphicsConfig, gc)) {
		$var($String, gcName, gc->getClass()->getSimpleName());
		$var($AccelGraphicsConfig, agc, $cast($AccelGraphicsConfig, gc));
		if (($$nc(agc->getContextCapabilities())->getCaps() & 0x0100) != 0 || TranslucentWindowPainter::forceOpt) {
			if ($nc(gcName)->startsWith("D3D"_s)) {
				return $new($TranslucentWindowPainter$VIOptD3DWindowPainter, peer);
			} else if (TranslucentWindowPainter::forceOpt && gcName->startsWith("WGL"_s)) {
...
void TranslucentWindowPainter::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$beforeCallerSensitive();
	TranslucentWindowPainter::forceOpt = $($Boolean::valueOf($$cast($String, $AccessController::doPrivileged($$new($GetPropertyAction, "sun.java2d.twp.forceopt"_s, "false"_s)))))->booleanValue();
	TranslucentWindowPainter::forceSW = $($Boolean::valueOf($$cast($String, $AccessController::doPrivileged($$new($GetPropertyAction, "sun.java2d.twp.forcesw"_s, "false"_s)))))->booleanValue();
}
...
$Class* TranslucentWindowPainter::load$($String* name, bool initialize) {
	...
	$loadClass(TranslucentWindowPainter, name, initialize, &classInfo$$, TranslucentWindowPainter::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(TranslucentWindowPainter);
	});
	return class$;
}
