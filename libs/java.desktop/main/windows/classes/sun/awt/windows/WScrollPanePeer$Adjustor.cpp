...
void WScrollPanePeer$Adjustor::run() {
	...
	if (this->orient == $Adjustable::VERTICAL) {
		...
		if ($nc($WScrollPanePeer::log)->isLoggable($PlatformLogger$Level::FINE)) {
			$WScrollPanePeer::log->fine("Assertion failed: unknown orient"_s);
		}
	}
	if (adj == nullptr) {
		return;
	...
	default:
		$init($PlatformLogger$Level);
		if ($nc($WScrollPanePeer::log)->isLoggable($PlatformLogger$Level::FINE)) {
			$WScrollPanePeer::log->fine("Assertion failed: unknown type"_s);
		}
		return;
	}
	...
	if ($nc($WScrollPanePeer::log)->isLoggable($PlatformLogger$Level::FINE)) {
		if (hwAncestor == nullptr) {
			$WScrollPanePeer::log->fine("Assertion (hwAncestor != null) failed, couldn\'t find heavyweight ancestor of scroll pane child"_s);
		}
	}
...
$Class* WScrollPanePeer$Adjustor::load$($String* name, bool initialize) {
	...
	$loadClass(WScrollPanePeer$Adjustor, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(WScrollPanePeer$Adjustor);
	});
	return class$;
}
