...
D3DRenderQueue* D3DRenderQueue::getInstance() {
	...
	$synchronized(class$) {
		if (D3DRenderQueue::theInstance == nullptr) {
			...
			D3DRenderQueue::theInstance->flushAndInvokeNow($$new($D3DRenderQueue$1));
		}
...
void D3DRenderQueue::flushBuffer(int64_t buf, int32_t limit, $Runnable* task) {
	$prepareNative(flushBuffer, void, int64_t buf, int32_t limit, $Runnable* task);
...
$Class* D3DRenderQueue::load$($String* name, bool initialize) {
	...
	$loadClass(D3DRenderQueue, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(D3DRenderQueue);
	});
	return class$;
}
