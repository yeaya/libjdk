...
void D3DRenderer::validateContext($SunGraphics2D* sg2d) {
	...
	try {
		$assign(dstData, $cast($D3DSurfaceData, sg2d->surfaceData));
	} catch ($ClassCastException& e) {
		$throwNew($InvalidPipeException, $$str({"wrong surface data type: "_s, sg2d->surfaceData}));
	}
	$D3DContext::validateContext(dstData, dstData, $(sg2d->getCompClip()), sg2d->composite, nullptr, sg2d->paint, sg2d, ctxflags);
}
...
void D3DRenderer::validateContextAA($SunGraphics2D* sg2d) {
	...
	$D3DContext::validateContext(dstData, dstData, $($nc(sg2d)->getCompClip()), $nc(sg2d)->composite, nullptr, $nc(sg2d)->paint, sg2d, ctxflags);
}
...
void D3DRenderer::copyArea($SunGraphics2D* sg2d, int32_t x, int32_t y, int32_t w, int32_t h, int32_t dx, int32_t dy) {
	...
	try {
		...
		$D3DContext::validateContext(dstData, dstData, $(sg2d->getCompClip()), sg2d->composite, nullptr, nullptr, nullptr, ctxflags);
...
$Class* D3DRenderer::load$($String* name, bool initialize) {
	...
	$loadClass(D3DRenderer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
