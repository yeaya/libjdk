...
int32_t WGLGraphicsConfig::getDefaultPixFmt(int32_t screennum) {
	...
	$prepareNativeStatic(getDefaultPixFmt, int32_t, int32_t screennum);
	int32_t $ret = $invokeNativeStatic(screennum);
	$finishNativeStatic();
	return $ret;
}
...
bool WGLGraphicsConfig::initWGL() {
	$init(WGLGraphicsConfig);
	$prepareNativeStatic(initWGL, bool);
...
int64_t WGLGraphicsConfig::getWGLConfigInfo(int32_t screennum, int32_t visualnum) {
	$init(WGLGraphicsConfig);
	$prepareNativeStatic(getWGLConfigInfo, int64_t, int32_t screennum, int32_t visualnum);
...
void WGLGraphicsConfig::clinit$($Class* clazz) {
...
$Class* WGLGraphicsConfig::load$($String* name, bool initialize) {
	...
	$loadClass(WGLGraphicsConfig, name, initialize, &classInfo$$, WGLGraphicsConfig::clinit$, []($Class* clazz) -> $Object* {
