	...
	if (isRootVisible() != rootVisible && this->root != nullptr) {
		if (rootVisible) {
			...
			if (this->treeSelectionModel != nullptr) {
				this->treeSelectionModel->removeSelectionPath($($nc(this->root)->getTreePath()));
			}
		}
		if (this->treeSelectionModel != nullptr) {
...
int32_t VariableHeightLayoutCache::getVisibleChildCount($TreePath* path) {
	$var($VariableHeightLayoutCache$TreeStateNode, node, getNodeForPath(path, true, false));
	return (node != nullptr) ? node->getVisibleChildCount() : 0;
}
...
void VariableHeightLayoutCache::rebuild(bool clearSelection) {
	...
	if (this->treeModel != nullptr && ($assign(rootObject, this->treeModel->getRoot())) != nullptr) {
...
$Class* VariableHeightLayoutCache::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		...
		{"setModel", "(Ljavax/swing/tree/TreeModel;)V", nullptr, $PUBLIC, $virtualMethod(VariableHeightLayoutCache, setModel, void, $TreeModel*), nullptr, nullptr, setModelmethodAnnotations$$},
		...
		{"setRootVisible", "(Z)V", nullptr, $PUBLIC, $virtualMethod(VariableHeightLayoutCache, setRootVisible, void, bool), nullptr, nullptr, setRootVisiblemethodAnnotations$$},
		{"setRowHeight", "(I)V", nullptr, $PUBLIC, $virtualMethod(VariableHeightLayoutCache, setRowHeight, void, int32_t), nullptr, nullptr, setRowHeightmethodAnnotations$$},
	...
	$loadClass(VariableHeightLayoutCache, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(VariableHeightLayoutCache);
	});
	return class$;
}
