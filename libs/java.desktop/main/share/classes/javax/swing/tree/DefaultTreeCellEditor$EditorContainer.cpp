...
$Dimension* DefaultTreeCellEditor$EditorContainer::getPreferredSize() {
	$useLocalObjectStack();
	if (this->this$0->editingComponent != nullptr) {
		$var($Dimension, pSize, this->this$0->editingComponent->getPreferredSize());
		$nc(pSize)->width += this->this$0->offset + 5;
		$var($Dimension, rSize, (this->this$0->renderer != nullptr) ? this->this$0->renderer->getPreferredSize() : ($Dimension*)nullptr);
		if (rSize != nullptr) {
			pSize->height = $Math::max(pSize->height, rSize->height);
		}
		if (this->this$0->editingIcon != nullptr) {
			pSize->height = $Math::max(pSize->height, this->this$0->editingIcon->getIconHeight());
		}
...
$Class* DefaultTreeCellEditor$EditorContainer::load$($String* name, bool initialize) {
	...
	$loadClass(DefaultTreeCellEditor$EditorContainer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
