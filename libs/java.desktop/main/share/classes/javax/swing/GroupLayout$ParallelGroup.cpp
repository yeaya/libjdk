...
void GroupLayout$ParallelGroup::insertAutopadding(int32_t axis, $List* leadingPadding, $List* trailingPadding, $List* leading, $List* trailing, bool insert) {
	...
	for (; $nc(i$)->hasNext();) {
		...
		if ($instanceOf($GroupLayout$ComponentSpring, spring)) {
			if ($cast($GroupLayout$ComponentSpring, spring)->isVisible()) {
				{
			...
			$cast($GroupLayout$Group, spring)->insertAutopadding(axis, leadingPadding, trailingPadding, leading, trailing, insert);
		} else if ($instanceOf($GroupLayout$AutoPreferredGapSpring, spring)) {
			$cast($GroupLayout$AutoPreferredGapSpring, spring)->setSources(leading);
...
$Class* GroupLayout$ParallelGroup::load$($String* name, bool initialize) {
	...
	$loadClass(GroupLayout$ParallelGroup, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(GroupLayout$ParallelGroup);
	});
	return class$;
}
