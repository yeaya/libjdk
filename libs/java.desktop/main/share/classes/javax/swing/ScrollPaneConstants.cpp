...
void ScrollPaneConstants::clinit$($Class* clazz) {
...
$Class* ScrollPaneConstants::load$($String* name, bool initialize) {
	...
	$loadClass(ScrollPaneConstants, name, initialize, &classInfo$$, ScrollPaneConstants::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ScrollPaneConstants);
	});
	return class$;
}
