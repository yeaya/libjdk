...
$AccessibleIconArray* AbstractButton$AccessibleAbstractButton::getAccessibleIcon() {
	...
	if ($instanceOf($Accessible, defaultIcon)) {
		$var($AccessibleContext, ac, $cast($Accessible, defaultIcon)->getAccessibleContext());
...
$AccessibleRelationSet* AbstractButton$AccessibleAbstractButton::getAccessibleRelationSet() {
	...
	if (!$nc(relationSet)->contains($AccessibleRelation::MEMBER_OF)) {
		...
		if (model != nullptr && $instanceOf($DefaultButtonModel, model)) {
			$var($ButtonGroup, group, $cast($DefaultButtonModel, model)->getGroup());
			if (group != nullptr) {
...
$Rectangle* AbstractButton$AccessibleAbstractButton::getTextRectangle() {
	...
	$var($JComponent, var$0, this->this$0);
	...
	$var($String, clippedText, $SwingUtilities::layoutCompoundLabel(var$0, var$1, text, icon, var$2, var$3, var$4, this->this$0->getHorizontalTextPosition(), paintViewR, paintIconR, paintTextR, 0));
	return paintTextR;
}
...
$Class* AbstractButton$AccessibleAbstractButton::load$($String* name, bool initialize) {
	...
	$loadClass(AbstractButton$AccessibleAbstractButton, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
