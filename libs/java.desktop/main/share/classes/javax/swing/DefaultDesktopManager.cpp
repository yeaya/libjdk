...
void DefaultDesktopManager::openFrame($JInternalFrame* f) {
	...
	if ($$nc($nc(f)->getDesktopIcon())->getParent() != nullptr) {
		$$nc($$nc(f->getDesktopIcon())->getParent())->add(f);
		removeIconFor(f);
	}
}
...
void DefaultDesktopManager::maximizeFrame($JInternalFrame* f) {
	...
	if ($nc(f)->isIcon()) {
		...
		setBoundsForFrame(f, 0, 0, $nc(desktopBounds)->width, $nc(desktopBounds)->height);
	}
	try {
		f->setSelected(true);
	} catch ($PropertyVetoException& e2) {
...
void DefaultDesktopManager::minimizeFrame($JInternalFrame* f) {
	...
	if ((f->getNormalBounds()) != nullptr) {
		...
		setBoundsForFrame(f, $nc(r)->x, $nc(r)->y, $nc(r)->width, $nc(r)->height);
	}
}
...
void DefaultDesktopManager::iconifyFrame($JInternalFrame* f) {
	...
	if ($instanceOf($JLayeredPane, c)) {
		...
		int32_t layer = $JLayeredPane::getLayer(f);
...
void DefaultDesktopManager::deiconifyFrame($JInternalFrame* f) {
	...
	if (c != nullptr && d != nullptr) {
		...
		if (f->isMaximum()) {
			...
			if (var$0 || f->getHeight() != desktopBounds->height) {
...
void DefaultDesktopManager::activateFrame($JInternalFrame* f) {
	...
	$var($JDesktopPane, d, f->getDesktopPane());
	$var($JInternalFrame, currentlyActiveFrame, (d == nullptr) ? ($JInternalFrame*)nullptr : d->getSelectedFrame());
	if (p == nullptr) {
...
void DefaultDesktopManager::beginDraggingFrame($JComponent* f) {
	...
	if (this->dragMode == DefaultDesktopManager::FASTER_DRAG_MODE) {
		...
		if ($instanceOf($JComponent, desktop)) {
			$set(this, desktopBounds, $cast($JComponent, desktop)->getVisibleRect());
		} else {
...
void DefaultDesktopManager::setupDragMode($JComponent* f) {
	...
	if (p != nullptr) {
		...
		if (window != nullptr && !window->isOpaque()) {
			...
			bool var$1 = mode != nullptr && mode->equals("faster"_s) && $instanceOf($JInternalFrame, f);
			bool var$0 = var$1 && $cast($JInternalFrame, f)->isOpaque();
			if (var$0 && (parent == nullptr || parent->isOpaque())) {
				this->dragMode = DefaultDesktopManager::FASTER_DRAG_MODE;
			} else if (p->getDragMode() == $JDesktopPane::OUTLINE_DRAG_MODE) {
				this->dragMode = DefaultDesktopManager::OUTLINE_DRAG_MODE;
			} else {
				bool var$2 = p->getDragMode() == $JDesktopPane::LIVE_DRAG_MODE && $instanceOf($JInternalFrame, f);
				if (var$2 && $cast($JInternalFrame, f)->isOpaque()) {
					this->dragMode = DefaultDesktopManager::FASTER_DRAG_MODE;
				} else {
					this->dragMode = DefaultDesktopManager::DEFAULT_DRAG_MODE;
				}
	...
	if (this->dragMode == DefaultDesktopManager::OUTLINE_DRAG_MODE) {
		...
		if (desktopPane != nullptr) {
			...
			if (this->currentLoc != nullptr) {
				int32_t var$0 = this->currentLoc->x;
				int32_t var$1 = this->currentLoc->y;
				int32_t var$2 = $nc(f)->getWidth() - 1;
				g->drawRect(var$0, var$1, var$2, f->getHeight() - 1);
			}
			int32_t var$3 = $nc(f)->getWidth() - 1;
			g->drawRect(newX, newY, var$3, f->getHeight() - 1);
			$var($SurfaceData, sData, $cast($SunGraphics2D, g)->getSurfaceData());
		...
		int32_t var$4 = $nc(f)->getWidth();
		setBoundsForFrame(f, newX, newY, var$4, f->getHeight());
	}
}
...
void DefaultDesktopManager::endDraggingFrame($JComponent* f) {
	if (this->dragMode == DefaultDesktopManager::OUTLINE_DRAG_MODE && this->currentLoc != nullptr) {
		int32_t var$0 = this->currentLoc->x;
		int32_t var$1 = this->currentLoc->y;
		int32_t var$2 = $nc(f)->getWidth();
		setBoundsForFrame(f, var$0, var$1, var$2, f->getHeight());
		$set(this, currentLoc, nullptr);
	} else if (this->dragMode == DefaultDesktopManager::FASTER_DRAG_MODE) {
		$set(this, currentBounds, nullptr);
...
void DefaultDesktopManager::endResizingFrame($JComponent* f) {
	if (this->dragMode == DefaultDesktopManager::OUTLINE_DRAG_MODE && this->currentBounds != nullptr) {
		setBoundsForFrame(f, this->currentBounds->x, this->currentBounds->y, this->currentBounds->width, this->currentBounds->height);
		$set(this, currentBounds, nullptr);
	}
}
...
$Rectangle* DefaultDesktopManager::getBoundsForIconOf($JInternalFrame* f) {
	...
	if (c == nullptr) {
		return $new($Rectangle, 0, 0, $nc(prefSize)->width, $nc(prefSize)->height);
	}
...
void DefaultDesktopManager::dragFrameFaster($JComponent* f, int32_t newX, int32_t newY) {
	...
	try {
		if (!floaterCollision) {
			currentManager->copyArea(parent, this->desktopGraphics, $nc(visBounds)->x, $nc(visBounds)->y, $nc(visBounds)->width, $nc(visBounds)->height, newX - previousBounds->x, newY - previousBounds->y, true);
		}
		f->setBounds(this->currentBounds);
		if (!floaterCollision) {
			$var($Rectangle, r, this->currentBounds);
			currentManager->notifyRepaintPerformed(parent, $nc(r)->x, $nc(r)->y, $nc(r)->width, $nc(r)->height);
		}
		if (floaterCollision) {
			$cast($JInternalFrame, f)->isDragging = false;
	...
	if (var$2 && $cast($SunToolkit, tk)->needUpdateWindow()) {
...
bool DefaultDesktopManager::isFloaterCollision($Rectangle* moveFrom, $Rectangle* moveTo) {
	...
	for (int32_t i = 0; i < $nc(this->floatingItems)->length; ++i) {
		bool intersectsFrom = $nc(moveFrom)->intersects(this->floatingItems->get(i));
		if (intersectsFrom) {
			return true;
		}
...
$RectangleArray* DefaultDesktopManager::findFloatingItems($JComponent* f) {
	...
	for (i = 0; i < floaters->length; ++i) {
		floaters->set(i, $($nc(children->get(i))->getBounds()));
	}
	return floaters;
}
...
void DefaultDesktopManager::clinit$($Class* clazz) {
...
$Class* DefaultDesktopManager::load$($String* name, bool initialize) {
	...
	$loadClass(DefaultDesktopManager, name, initialize, &classInfo$$, DefaultDesktopManager::clinit$, []($Class* clazz) -> $Object* {
