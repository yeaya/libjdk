...
int32_t TablePrintable::print($Graphics* graphics, $PageFormat* pageFormat, int32_t pageIndex) {
	...
	if (this->headerFormat != nullptr) {
		$assign(headerText, this->headerFormat->format(pageNumber));
	}
	$var($String, footerText, nullptr);
	if (this->footerFormat != nullptr) {
		$assign(footerText, this->footerFormat->format(pageNumber));
	}
	...
	while (this->last < pageIndex) {
		...
		int32_t scaledHeight = $cast(int32_t, ((availableSpace - this->hclip->height) / sf));
	...
	if (footerText != nullptr) {
		$assign(oldTrans, $nc(g2d)->getTransform());
	...
	if (headerText != nullptr) {
		...
		$nc(g2d)->translate(0, headerTextSpace + TablePrintable::H_F_SPACE);
	}
	this->tempRect->x = 0;
	this->tempRect->y = 0;
	this->tempRect->width = imgWidth;
	this->tempRect->height = availableSpace;
	$nc(g2d)->clip(this->tempRect);
	if (sf != 1.0) {
		g2d->scale(sf, sf);
	} else {
		int32_t diff = (imgWidth - this->clip->width) / 2;
	...
	$var($Rectangle, visibleBounds, this->clip->intersection($($nc(this->table)->getBounds())));
	...
	if (this->printMode == $JTable$PrintMode::FIT_WIDTH) {
		g2d->drawRect(0, 0, this->clip->width, this->hclip->height + rowHeight);
	} else {
		g2d->drawRect(0, 0, visibleBounds->width, this->hclip->height + rowHeight);
	}
	if (this->printMode == $JTable$PrintMode::FIT_WIDTH) {
...
void TablePrintable::printText($Graphics2D* g2d, $String* text, $Rectangle2D* rect, $Font* font, int32_t imgWidth) {
	...
	int32_t ty = $cast(int32_t, $Math::ceil($Math::abs(rect->getY())));
	$init($Color);
...
void TablePrintable::clinit$($Class* clazz) {
...
$Class* TablePrintable::load$($String* name, bool initialize) {
	...
	$loadClass(TablePrintable, name, initialize, &classInfo$$, TablePrintable::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(TablePrintable);
	});
	return class$;
}
