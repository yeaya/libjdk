...
void HTMLWriter::init$($Writer* w, $HTMLDocument* doc, int32_t pos, int32_t len) {
	$AbstractWriter::init$(w, $cast($AbstractDocument, doc), pos, len);
...
void HTMLWriter::write() {
	...
	while (!$nc(this->blockElementStack)->empty()) {
		$assign(current, $cast($Element, this->blockElementStack->pop()));
...
void HTMLWriter::writeAttributes($AttributeSet* attr) {
	$useLocalObjectStack();
	$nc(this->convAttr)->removeAttributes(this->convAttr);
	...
	while ($nc(names)->hasMoreElements()) {
		...
		var$0->append(name);
		var$0->append("=\""_s);
		var$0->append($($nc(this->convAttr)->getAttribute(name)));
		var$0->append("\""_s);
		write($$str(var$0));
	}
}
...
void HTMLWriter::emptyTag($Element* elem) {
	...
	if (matchNameAttribute(attr, $HTML$Tag::CONTENT)) {
		...
		$init($StyleConstants);
		$var($Object, nameTag, (attr != nullptr) ? attr->getAttribute($StyleConstants::NameAttribute) : ($Object*)nullptr);
		$init($HTML$Attribute);
		$var($Object, endTag, (attr != nullptr) ? attr->getAttribute($HTML$Attribute::ENDTAG) : ($Object*)nullptr);
		bool outputEndTag = false;
		if (nameTag != nullptr && endTag != nullptr && ($instanceOf($String, endTag)) && endTag->equals("true"_s)) {
			outputEndTag = true;
		}
		if (this->completeDoc && matchNameAttribute(attr, $HTML$Tag::HEAD)) {
			if (outputEndTag) {
				writeStyles($($$sure($HTMLDocument, getDocument())->getStyleSheet()));
			}
			this->wroteHead = true;
		}
		write(u'<');
...
void HTMLWriter::startTag($Element* elem) {
	...
	} else if (this->completeDoc && name == $HTML$Tag::BODY) {
		writeMaps($($$sure($HTMLDocument, getDocument())->getMaps()));
...
void HTMLWriter::text($Element* elem) {
	...
	if (start < end) {
		...
		this->newlineOutputed = false;
		if ($nc(this->segment)->count > 0) {
			if ($nc(this->segment->array)->get(this->segment->offset + this->segment->count - 1) == u'\n') {
...
void HTMLWriter::selectContent($AttributeSet* attr) {
	...
	if ($instanceOf($OptionListModel, model)) {
		...
		int32_t size = listModel->getSize();
		...
		int32_t size = comboBoxModel->getSize();
		for (int32_t i = 0; i < size; ++i) {
...
void HTMLWriter::writeAdditionalComments() {
	...
	if ($instanceOf($Vector, comments)) {
		...
		for (int32_t counter = 0, maxCounter = v->size(); counter < maxCounter; ++counter) {
			writeComment($($$nc(v->elementAt(counter))->toString()));
		}
	}
}
...
bool HTMLWriter::noMatchForTagInAttributes($AttributeSet* attr, $HTML$Tag* t, Object$* tagValue) {
	if (attr != nullptr && attr->isDefined(t)) {
		...
		if ((tagValue == nullptr) ? (newValue == nullptr) : (newValue != nullptr && $of(tagValue)->equals(newValue))) {
			return false;
		}
	}
	return true;
...
void HTMLWriter::writeMaps($Enumeration* maps) {
	...
	if (maps != nullptr) {
		while (maps->hasMoreElements()) {
			...
			if (areas != nullptr) {
				for (int32_t counter = 0, maxCounter = areas->length; counter < maxCounter; ++counter) {
					indentSmart();
					write("<area"_s);
	...
	if (attributes != nullptr) {
		while (attributes->hasMoreElements()) {
			...
			if ($instanceOf($CSS$Attribute, attribute)) {
				$var($String, value, $$nc(style->getAttribute(attribute))->toString());
				if (value != nullptr) {
					if (!outputStyle) {
						writeStyleStartTag();
						outputStyle = true;
...
void HTMLWriter::convertToHTML32($AttributeSet* from, $MutableAttributeSet* to) {
	...
	while ($nc(keys)->hasMoreElements()) {
		...
		if ($instanceOf($CSS$Attribute, key)) {
			...
			if (($equals(key, $CSS$Attribute::FONT_FAMILY)) || ($equals(key, $CSS$Attribute::FONT_SIZE)) || ($equals(key, $CSS$Attribute::COLOR))) {
			...
			} else if ($equals(key, $CSS$Attribute::FONT_WEIGHT)) {
			...
			} else if ($equals(key, $CSS$Attribute::FONT_STYLE)) {
				$var($String, s, $$nc(from->getAttribute(key))->toString());
				if ($nc(s)->indexOf("italic"_s) >= 0) {
					$init($HTML$Tag);
					$init($SimpleAttributeSet);
					addAttribute(to, $HTML$Tag::I, $SimpleAttributeSet::EMPTY);
				}
			} else if ($equals(key, $CSS$Attribute::TEXT_DECORATION)) {
				$var($String, decor, $$nc(from->getAttribute(key))->toString());
				if ($nc(decor)->indexOf("underline"_s) >= 0) {
					$init($HTML$Tag);
					$init($SimpleAttributeSet);
					addAttribute(to, $HTML$Tag::U, $SimpleAttributeSet::EMPTY);
				}
				if (decor->indexOf("line-through"_s) >= 0) {
					$init($HTML$Tag);
					$init($SimpleAttributeSet);
					addAttribute(to, $HTML$Tag::STRIKE, $SimpleAttributeSet::EMPTY);
				}
			} else if ($equals(key, $CSS$Attribute::VERTICAL_ALIGN)) {
				$var($String, vAlign, $$nc(from->getAttribute(key))->toString());
				if ($nc(vAlign)->indexOf("sup"_s) >= 0) {
					$init($HTML$Tag);
					$init($SimpleAttributeSet);
					addAttribute(to, $HTML$Tag::SUP, $SimpleAttributeSet::EMPTY);
				}
				if (vAlign->indexOf("sub"_s) >= 0) {
					$init($HTML$Tag);
					$init($SimpleAttributeSet);
					addAttribute(to, $HTML$Tag::SUB, $SimpleAttributeSet::EMPTY);
				}
			} else if ($equals(key, $CSS$Attribute::TEXT_ALIGN)) {
				$init($HTML$Attribute);
				addAttribute(to, $HTML$Attribute::ALIGN, $($$nc(from->getAttribute(key))->toString()));
			} else {
				if (value->length() > 0) {
				...
				var$0->append(key);
				var$0->append(": "_s);
				var$0->append($(from->getAttribute(key)));
				$assign(value, $str(var$0));
			}
		} else {
			$var($Object, attr, from->getAttribute(key));
			if ($instanceOf($AttributeSet, attr)) {
				$assign(attr, $cast($AttributeSet, attr)->copyAttributes());
			}
...
void HTMLWriter::addAttribute($MutableAttributeSet* to, Object$* key, Object$* value) {
	...
	if (attr == nullptr || $equals(attr, $SimpleAttributeSet::EMPTY)) {
		...
		$cast($MutableAttributeSet, attr)->addAttributes($cast($AttributeSet, value));
	}
}
...
void HTMLWriter::createFontAttribute($CSS$Attribute* a, $AttributeSet* from, $MutableAttributeSet* to) {
	...
	$var($String, htmlValue, $$nc($nc(from)->getAttribute(a))->toString());
	$init($CSS$Attribute);
...
void HTMLWriter::output($String* string) {
	...
	if (this->tempChars == nullptr || this->tempChars->length < length) {
...
$Class* HTMLWriter::load$($String* name, bool initialize) {
	...
	$loadClass(HTMLWriter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(HTMLWriter);
	});
	return class$;
}
