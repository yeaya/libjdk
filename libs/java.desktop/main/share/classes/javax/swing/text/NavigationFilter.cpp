...
int32_t NavigationFilter::getNextVisualPositionFrom($JTextComponent* text, int32_t pos, $Position$Bias* bias, int32_t direction, $Position$BiasArray* biasRet) {
	return $$sure($TextUI, $nc(text)->getUI())->getNextVisualPositionFrom(text, pos, bias, direction, biasRet);
}
...
$Class* NavigationFilter::load$($String* name, bool initialize) {
	...
	$loadClass(NavigationFilter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(NavigationFilter);
	});
	return class$;
}
