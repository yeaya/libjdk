...
void HTMLDocument$HTMLReader$LinkAction::start($HTML$Tag* t, $MutableAttributeSet* a) {
	...
	if (rel != nullptr) {
		...
		if (var$0 || rel->equals("alternate stylesheet"_s)) {
			...
			this->this$1->styles->addElement($(a->copyAttributes()));
		}
	}
...
$Class* HTMLDocument$HTMLReader$LinkAction::load$($String* name, bool initialize) {
	...
	$loadClass(HTMLDocument$HTMLReader$LinkAction, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(HTMLDocument$HTMLReader$LinkAction);
	});
	return class$;
}
