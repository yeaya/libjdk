...
$Class* AccessibleHTML$TableElementInfo$TableAccessibleContext$AccessibleHeadersTable::load$($String* name, bool initialize) {
	...
	$loadClass(AccessibleHTML$TableElementInfo$TableAccessibleContext$AccessibleHeadersTable, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AccessibleHTML$TableElementInfo$TableAccessibleContext$AccessibleHeadersTable);
	});
	return class$;
}
