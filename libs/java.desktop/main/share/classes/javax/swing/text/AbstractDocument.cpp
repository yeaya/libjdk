...
void AbstractDocument::init$($AbstractDocument$Content* data, $AbstractDocument$AttributeContext* context) {
	...
	if (AbstractDocument::defaultI18NProperty == nullptr) {
		$var($String, o, $cast($String, $AccessController::doPrivileged($$new($AbstractDocument$1, this))));
		if (o != nullptr) {
...
void AbstractDocument::fireUndoableEditUpdate($UndoableEditEvent* e$renamed) {
	...
	if ($instanceOf($AbstractDocument$DefaultDocumentEvent, $($nc(e)->getEdit()))) {
		...
		$assign(e, $new($UndoableEditEvent, var$0, $$cast($AbstractDocument$DefaultDocumentEvent, $new($AbstractDocument$DefaultDocumentEventUndoableWrapper, this, $$cast($AbstractDocument$DefaultDocumentEvent, e->getEdit())))));
	}
	$var($ObjectArray, listeners, $nc(this->listenerList)->getListenerList());
	for (int32_t i = $nc(listeners)->length - 2; i >= 0; i -= 2) {
...
$DocumentListenerArray* AbstractDocument::getDocumentListeners() {
	$load($DocumentListener);
	return $cast($DocumentListenerArray, $nc(this->listenerList)->getListeners($DocumentListener::class$));
}
...
$UndoableEditListenerArray* AbstractDocument::getUndoableEditListeners() {
	$load($UndoableEditListener);
	return $cast($UndoableEditListenerArray, $nc(this->listenerList)->getListeners($UndoableEditListener::class$));
}

$Object* AbstractDocument::getProperty(Object$* key) {
	return $$nc(getDocumentProperties())->get(key);
}
...
void AbstractDocument::replace(int32_t offset, int32_t length, $String* text, $AttributeSet* attrs) {
	$useLocalObjectStack();
	if (length == 0 && (text == nullptr || text->length() == 0)) {
		return;
	}
	$var($DocumentFilter, filter, getDocumentFilter());
	writeLock();
...
void AbstractDocument::handleInsertString(int32_t offs, $String* str, $AttributeSet* a) {
	...
	if ($$nc(getProperty(AbstractDocument::I18NProperty))->equals($Boolean::FALSE)) {
		$init($TextAttribute);
	...
	fireInsertUpdate(e);
	$init($StyleConstants);
	if (u != nullptr && (a == nullptr || !a->isDefined($StyleConstants::ComposedTextAttribute))) {
...
bool AbstractDocument::isLeftToRight($Document* doc, int32_t p0, int32_t p1) {
	...
	if ($nc($Boolean::TRUE)->equals($($nc(doc)->getProperty(AbstractDocument::I18NProperty)))) {
		if ($instanceOf(AbstractDocument, doc)) {
			...
			$var($Element, bidiRoot, adoc->getBidiRootElement());
...
void AbstractDocument::insertUpdate($AbstractDocument$DefaultDocumentEvent* chng, $AttributeSet* attr) {
	$useLocalObjectStack();
	if ($$nc(getProperty(AbstractDocument::I18NProperty))->equals($Boolean::TRUE)) {
		updateBidi(chng);
	}
	$init($DocumentEvent$EventType);
...
void AbstractDocument::updateBidi($AbstractDocument$DefaultDocumentEvent* chng) {
	...
	int32_t lastPEnd = 0;
	$init($DocumentEvent$EventType);
	if ($nc(chng)->type == $DocumentEvent$EventType::INSERT || chng->type == $DocumentEvent$EventType::CHANGE) {
	...
	if (lastSpanEnd <= getLength()) {
		...
		if (nextLevel == levels->get(levels->length - 1)) {
	...
	int32_t lastSpanStart = levels->length;
...
$bytes* AbstractDocument::calculateBidiLevels(int32_t firstPStart, int32_t lastPEnd) {
	...
	for (int32_t o = firstPStart; o < lastPEnd;) {
		...
		$assign(bidiAnalyzer, $new($Bidi, $nc(seg)->array, $nc(seg)->offset, nullptr, 0, $nc(seg)->count, bidiflag));
...
void AbstractDocument::dump($PrintStream* out) {
	...
	if ($instanceOf($AbstractDocument$AbstractElement, root)) {
		$cast($AbstractDocument$AbstractElement, root)->dump(out, 0);
	}
...
void AbstractDocument::readObject($ObjectInputStream* s) {
	...
	$set(this, documentProperties, $cast($Dictionary, $nc(f)->get("documentProperties"_s, nullptr)));
	$set(this, listenerList, $new($EventListenerList));
	$set(this, data, $cast($AbstractDocument$Content, f->get("data"_s, nullptr)));
	$set(this, context, $cast($AbstractDocument$AttributeContext, f->get("context"_s, nullptr)));
	$set(this, documentFilter, $cast($DocumentFilter, f->get("documentFilter"_s, nullptr)));
	$set(this, bidiRoot, $new($AbstractDocument$BidiRootElement, this));
	$var($Throwable, var$0, nullptr);
	try {
		writeLock();
...
void AbstractDocument::clinit$($Class* clazz) {
...
$Class* AbstractDocument::load$($String* name, bool initialize) {
	...
	$loadClass(AbstractDocument, name, initialize, &classInfo$$, AbstractDocument::clinit$, []($Class* clazz) -> $Object* {
