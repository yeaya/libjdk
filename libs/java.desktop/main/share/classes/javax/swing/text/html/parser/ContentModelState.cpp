...
bool ContentModelState::terminate() {
	...
	case u'*':
	case u'?':
		return (this->next == nullptr) || this->next->terminate();
	case u'|':
		{
			$var($ContentModel, m, $cast($ContentModel, $nc(this->model)->content));
			for (; m != nullptr; $assign(m, m->next)) {
				if (m->empty()) {
					return (this->next == nullptr) || this->next->terminate();
				}
			}
	...
	case u'&':
		{
			$var($ContentModel, m, $cast($ContentModel, $nc(this->model)->content));
			for (int32_t i = 0; m != nullptr; ++i, $assign(m, m->next)) {
				if ((this->value & ($sl((int64_t)1, i))) == 0) {
					if (!m->empty()) {
						return false;
					}
				}
...
$Element* ContentModelState::first() {
	...
	case u',':
		{
			$var($ContentModel, m, $cast($ContentModel, this->model->content));
			for (int32_t i = 0; i < this->value; ++i, $assign(m, $nc(m)->next)) {
				;
			}
			return $nc(m)->first();
...
ContentModelState* ContentModelState::advance(Object$* token) {
	...
	default:
		if ($equals($nc(this->model)->content, token)) {
			if (this->next == nullptr && ($instanceOf($Element, token)) && $cast($Element, token)->content != nullptr) {
...
$Class* ContentModelState::load$($String* name, bool initialize) {
	...
	$loadClass(ContentModelState, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ContentModelState);
	});
	return class$;
}
