...
void JTextComponent$ComposedTextCaret::install($JTextComponent* c) {
	...
	if ($instanceOf($StyledDocument, doc)) {
		...
		$var($Element, elem, sDoc->getCharacterElement($nc(c->composedTextStart)->getOffset()));
...
void JTextComponent$ComposedTextCaret::paint($Graphics* g) {
	if (isVisible()) {
		try {
			...
			g->drawLine($nc(r)->x, $nc(r)->y, $nc(r)->x, $nc(r)->y + $nc(r)->height - 1);
			g->setPaintMode();
...
$Class* JTextComponent$ComposedTextCaret::load$($String* name, bool initialize) {
	...
	$loadClass(JTextComponent$ComposedTextCaret, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
