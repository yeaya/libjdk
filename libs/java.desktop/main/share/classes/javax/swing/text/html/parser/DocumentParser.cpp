...
void DocumentParser::handleStartTag($TagElement* tag) {
	...
	if (tag->fictional()) {
		...
		$nc(this->callback)->handleStartTag(var$0, attrs, getBlockStartPosition());
	} else {
		$var($HTML$Tag, var$1, tag->getHTMLTag());
		$var($MutableAttributeSet, var$2, getAttributes());
		$nc(this->callback)->handleStartTag(var$1, var$2, getBlockStartPosition());
		flushAttributes();
	}
}
...
void DocumentParser::handleEmptyTag($TagElement* tag) {
	...
	if (elem == $nc(this->dtd)->meta && !this->ignoreCharSet) {
		...
		if (atts != nullptr) {
			...
			if (content != nullptr) {
				...
				} else if ("charset"_s->equalsIgnoreCase($$cast($String, atts->getAttribute($HTML$Attribute::HTTPEQUIV)))) {
	...
	if (this->inbody != 0 || elem == $nc(this->dtd)->meta || elem == $nc(this->dtd)->base || elem == $nc(this->dtd)->isindex || elem == $nc(this->dtd)->style || elem == $nc(this->dtd)->link) {
		...
		if (tag->fictional()) {
			...
			attrs->addAttribute($HTMLEditorKit$ParserCallback::IMPLIED, $Boolean::TRUE);
			$var($HTML$Tag, var$1, tag->getHTMLTag());
			$nc(this->callback)->handleSimpleTag(var$1, attrs, getBlockStartPosition());
		} else {
			$var($HTML$Tag, var$2, tag->getHTMLTag());
			$var($MutableAttributeSet, var$3, getAttributes());
			$nc(this->callback)->handleSimpleTag(var$2, var$3, getBlockStartPosition());
			flushAttributes();
		}
	}
}
...
$Class* DocumentParser::load$($String* name, bool initialize) {
	...
	$loadClass(DocumentParser, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DocumentParser);
	});
	return class$;
}
