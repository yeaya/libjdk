...
void Parser::flushAttributes() {
	$nc(this->attributes)->removeAttributes(this->attributes);
}
...
void Parser::handleText($TagElement* tag) {
	...
	if (this->textpos == 0) {
		...
		if (var$0 || !this->stack->advance($nc(this->dtd)->pcdata)) {
			$set(this, last, tag);
			this->space = false;
	...
	if (this->space) {
		if (!this->ignoreSpace) {
			if (this->textpos + 1 > $nc(this->text)->length) {
				$var($chars, newtext, $new($chars, this->text->length + 200));
				$System::arraycopy(this->text, 0, newtext, 0, this->text->length);
				$set(this, text, newtext);
			}
			this->text->set(this->textpos++, u' ');
			if (!this->strict && !$$nc(tag->getElement())->isEmpty()) {
				this->ignoreSpace = true;
			}
		}
		this->space = false;
	...
	if ($$nc($$nc(tag->getElement())->getName())->equals("title"_s)) {
		handleTitle(newtext);
	} else {
		handleText(newtext);
	}
...
void Parser::startTag($TagElement* tag) {
	...
	if (var$0 || ((this->last != nullptr) && !this->last->breaksFlow()) || (this->textpos != 0)) {
		handleText(tag);
	} else {
		$set(this, last, tag);
		this->space = false;
	...
	this->lastBlockStartPos = this->currentBlockStartPos;
	{
		$var($AttributeList, a, elem->atts);
		for (; a != nullptr; $assign(a, a->next)) {
			bool var$1 = a->modifier == $DTDConstants::REQUIRED;
			if (var$1) {
				bool var$2 = $nc(this->attributes)->isEmpty();
				if (!var$2) {
					bool var$3 = !this->attributes->isDefined(a->name);
					var$2 = var$3 && (!this->attributes->isDefined($($HTML::getAttributeKey(a->name))));
				}
				var$1 = var$2;
			}
			if (var$1) {
...
void Parser::endTag(bool omitted) {
	...
	$set(this, recent, (this->stack != nullptr) ? this->stack->elem : ($Element*)nullptr);
}
...
bool Parser::ignoreElement($Element* elem) {
	...
	bool var$0 = var$1 || (elemName->equals("head"_s) && this->seenHead);
	if (var$0 || (elemName->equals("body"_s) && this->seenBody)) {
		return true;
	}
	bool var$2 = elemName->equals("dt"_s);
	if (var$2 || elemName->equals("dd"_s)) {
	...
	bool var$7 = var$8 && (!elemName->equals("#pcdata"_s));
	bool var$6 = var$7 && (!elemName->equals("input"_s));
	if (!var$6) {
		bool var$9 = elemName->equals("font"_s);
		if (var$9) {
			bool var$10 = stackElement->equals("ul"_s);
			var$9 = var$10 || stackElement->equals("ol"_s);
		}
		var$6 = var$9;
	}
	bool var$5 = var$6;
	bool var$4 = var$5 || (elemName->equals("meta"_s) && this->stack != nullptr);
	bool var$3 = var$4 || (elemName->equals("style"_s) && this->seenBody);
	if (!var$3) {
		bool var$11 = stackElement->equals("table"_s);
		var$3 = var$11 && elemName->equals("a"_s);
	}
	if (var$3) {
		return true;
	}
...
bool Parser::legalElementContext($Element* elem) {
	...
	if (var$0) {
		...
		if (!var$2) {
			bool var$4 = stackElemName->equals("table"_s);
			var$2 = var$4 && $nc(elemName)->equals("th"_s);
		}
		bool var$1 = var$2;
		if (!var$1) {
			bool var$5 = stackElemName->equals("tr"_s);
	...
	if (var$12 && (!this->strict || $nc(this->stack->elem)->omitEnd())) {
	...
	if (next != nullptr && (!this->strict || next->omitStart()) && !(next == $nc(this->dtd)->head && elem == this->dtd->pcdata)) {
			...
			content->getElements(elemVec);
			{
				...
				for (; $nc(i$)->hasNext();) {
					$var($Element, e, $cast($Element, i$->next()));
					{
						...
						bool reqAtts = false;
						{
							$var($AttributeList, a, e->getAttributes());
							for (; a != nullptr; $assign(a, a->next)) {
	...
	bool var$14 = $nc(this->stack)->terminate() && (this->stack->elem != $nc(this->dtd)->body);
	if (var$14 && (!this->strict || $nc(this->stack->elem)->omitEnd())) {
...
void Parser::legalTagContext($TagElement* tag) {
	...
	bool var$0 = tag->breaksFlow() && (this->stack != nullptr);
	if (var$0 && !$nc(this->stack->tag)->breaksFlow()) {
		endTag(true);
		legalTagContext(tag);
		return;
	}
	...
	error("tag.unexpected"_s, $($$nc(tag->getElement())->getName()));
}

void Parser::errorContext() {
	for (; (this->stack != nullptr) && ($nc(this->stack->tag)->getElement() != $nc(this->dtd)->body); $set(this, stack, $nc(this->stack)->next)) {
...
void Parser::addString(int32_t c) {
	if (this->strpos == $nc(this->str)->length) {
		$var($chars, newstr, $new($chars, this->str->length + 128));
		$System::arraycopy(this->str, 0, newstr, 0, this->str->length);
		$set(this, str, newstr);
	}
	this->str->set(this->strpos++, (char16_t)c);
}
...
$chars* Parser::mapNumericReference(int32_t c) {
	...
	if (c >= 0x0000ffff) {
		...
		data->set(0, (c < 130 || c > 159) ? (char16_t)c : Parser::cp1252Map->get(c - 130));
	}
	return data;
}
...
void Parser::parseLiteral(bool replace) {
	...
	while (true) {
		int32_t c = this->ch;
		{
			...
			case u'>':
				{
					...
					if (var$0 && (this->text->get(i) == u'/')) {
						while (true) {
							bool var$2 = ++i < this->textpos;
							if (var$2) {
								char16_t var$3 = $Character::toLowerCase(this->text->get(i));
								var$2 = var$3 == this->stack->elem->name->charAt(j++);
							}
							if (!(var$2)) {
								break;
							}
						...
						if (i == this->textpos) {
							...
							if ((this->textpos > 0) && (this->text->get(this->textpos - 1) == u'\n')) {
								--this->textpos;
							}
							endTag(false);
							return;
			...
			case u'&':
				...
				if (this->textpos + $nc(data)->length > $nc(this->text)->length) {
					$var($chars, newtext, $new($chars, $Math::max(this->textpos + data->length + 128, this->text->length * 2)));
					$System::arraycopy(this->text, 0, newtext, 0, this->text->length);
					$set(this, text, newtext);
				}
		...
		if (this->textpos == $nc(this->text)->length) {
			$var($chars, newtext, $new($chars, this->text->length + 128));
			$System::arraycopy(this->text, 0, newtext, 0, this->text->length);
			$set(this, text, newtext);
		}
		this->text->set(this->textpos++, (char16_t)c);
	}
}
...
void Parser::parseAttributeSpecificationList($Element* elem) {
	...
	while (true) {
		...
		if (attvalue == nullptr) {
			$init($HTML);
			$assign(attvalue, ((att != nullptr) && (att->value != nullptr)) ? att->value : $HTML::NULL_ATTRIBUTE_VALUE);
		} else if ((att != nullptr) && (att->values != nullptr) && !att->values->contains(attvalue)) {
...
$String* Parser::parseDTDMarkup() {
	...
	while (true) {
		...
		case u'\r':
			...
			break;
		default:
			strBuff->append((char16_t)(this->ch & 0xff));
			this->ch = readCh();
			break;
		}
	}
...
bool Parser::parseMarkupDeclarations($StringBuffer* strBuff) {
	...
	if (var$0 && ($($(strBuff->toString())->toUpperCase())->equals("DOCTYPE"_s))) {
		parseDTDMarkup();
		return true;
	}
	return false;
		...
		case u'/':
			{
				...
				if ((this->textpos > 0) && ($nc(this->text)->get(this->textpos - 1) == u'\n')) {
					if ($nc(this->stack)->pre) {
						if ((this->textpos > 1) && (this->text->get(this->textpos - 2) != u'\n')) {
							--this->textpos;
						}
					} else {
						--this->textpos;
				...
				if (!this->strict) {
					...
					bool var$0 = stackElem->equals("tr"_s);
					if (var$0 || stackElem->equals("td"_s)) {
				...
				if (var$2) {
					if ($nc(elemName)->equals("center"_s)) {
...
void Parser::parseScript() {
	$var($chars, charsToAdd, $new($chars, Parser::SCRIPT_END_TAG->length));
	bool insideComment = false;
	while (true) {
		int32_t i = 0;
		while (!insideComment && i < Parser::SCRIPT_END_TAG->length && (Parser::SCRIPT_END_TAG->get(i) == this->ch || Parser::SCRIPT_END_TAG_UPPER_CASE->get(i) == this->ch)) {
		...
		if (!insideComment && i == 1 && charsToAdd->get(0) == Parser::START_COMMENT->charAt(0)) {
			while (true) {
				bool var$0 = i < Parser::START_COMMENT->length();
				if (!(var$0 && Parser::START_COMMENT->charAt(i) == this->ch)) {
					break;
				}
				{
					charsToAdd->set(i, (char16_t)this->ch);
			...
			if (i == Parser::START_COMMENT->length()) {
				insideComment = true;
			}
		}
		if (insideComment) {
			while (true) {
				bool var$1 = i < Parser::END_COMMENT->length();
				if (!(var$1 && Parser::END_COMMENT->charAt(i) == this->ch)) {
					break;
				}
				{
					charsToAdd->set(i, (char16_t)this->ch);
			...
			if (i == Parser::END_COMMENT->length()) {
				insideComment = false;
			}
		}
		if (i > 0) {
...
void Parser::parseContent() {
	...
	for (;;) {
		...
		if (this->recent == $nc(this->dtd)->script) {
			...
			int32_t var$0 = Parser::START_COMMENT->length();
			int32_t minLength = var$0 + Parser::END_COMMENT->length();
			...
			if (var$1 && str->length() >= (minLength)) {
				int32_t var$3 = Parser::START_COMMENT->length();
				int32_t var$4 = str->length();
				$assign(str, str->substring(var$3, var$4 - Parser::END_COMMENT->length()));
			}
			...
			case u'/':
				this->ch = readCh();
				if ((this->stack != nullptr) && this->stack->net) {
					endTag(false);
					continue;
			...
			case u'\n':
				...
				++this->lfCount;
				this->ch = readCh();
				if ((this->stack != nullptr) && this->stack->pre) {
					break;
				}
				if (this->textpos == 0) {
					this->lastBlockStartPos = this->currentPosition;
	...
	$synchronized(this) {
		...
		try {
			try {
				...
				$var($String, var$2, e->getClass()->getName());
