...
void HTMLDocument$HTMLReader$ConvertAction::start($HTML$Tag* t, $MutableAttributeSet* attr) {
	...
	if (t == $HTML$Tag::B) {
		...
		$assign(value, (v != nullptr) ? $str({value, ","_s, $(v->toString())}) : value);
		...
		$init($HTML$Attribute);
		$var($String, color, $cast($String, attr->getAttribute($HTML$Attribute::COLOR)));
		if (color != nullptr) {
			$init($CSS$Attribute);
...
$Class* HTMLDocument$HTMLReader$ConvertAction::load$($String* name, bool initialize) {
	...
	$loadClass(HTMLDocument$HTMLReader$ConvertAction, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(HTMLDocument$HTMLReader$ConvertAction);
	});
	return class$;
}
