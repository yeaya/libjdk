...
bool MaskFormatter$MaskCharacter::isValidCharacter(char16_t aChar) {
	...
	if (filter != nullptr && filter->indexOf(aChar) == -1) {
		return false;
	}
...
void MaskFormatter$MaskCharacter::append($StringBuilder* buff, $String* formatting, $ints* index, $String* placeholder) {
	...
	char16_t aChar = inString ? formatting->charAt(index->get(0)) : 0;
	if (isLiteral()) {
...
$Class* MaskFormatter$MaskCharacter::load$($String* name, bool initialize) {
	...
	$loadClass(MaskFormatter$MaskCharacter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MaskFormatter$MaskCharacter);
	});
	return class$;
}
