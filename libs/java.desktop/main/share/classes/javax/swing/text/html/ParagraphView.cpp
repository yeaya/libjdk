...
bool ParagraphView::isVisible() {
	...
	if (var$1 == $$nc(getDocument())->getLength()) {
		...
		if ($instanceOf($JTextComponent, c)) {
			editable = $cast($JTextComponent, c)->isEditable();
		}
		if (!editable) {
			return false;
		}
...
void ParagraphView::paint($Graphics* g, $Shape* a) {
	...
	$nc(this->painter)->paint(g, (float)$nc(r)->x, (float)$nc(r)->y, (float)$nc(r)->width, (float)$nc(r)->height, this);
...
$Class* ParagraphView::load$($String* name, bool initialize) {
	...
	$loadClass(ParagraphView, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
