...
bool StyleContext$SmallAttributeSet::equals(Object$* obj) {
	if ($instanceOf($AttributeSet, obj)) {
		...
		bool var$0 = var$1 == attrs->getAttributeCount();
...
$Class* StyleContext$SmallAttributeSet::load$($String* name, bool initialize) {
	...
	$loadClass(StyleContext$SmallAttributeSet, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(StyleContext$SmallAttributeSet);
	});
	return class$;
}
