...
$Object* InternationalFormatter::stringToValue($String* text) {
	...
	if (var$0 && !$nc(getValueClass())->isInstance(value)) {
		$assign(value, $DefaultFormatter::stringToValue($(value->toString())));
	}
	try {
...
$Format$FieldArray* InternationalFormatter::getFields(int32_t offset) {
	...
	if (attrs != nullptr && attrs->size() > 0) {
		...
		return $cast($Format$FieldArray, al->toArray(InternationalFormatter::EMPTY_FIELD_ARRAY));
	}
...
$ActionArray* InternationalFormatter::getActions() {
	...
	if (getSupportsIncrement()) {
		return $new($ActionArray, {
			$$new($InternationalFormatter$IncrementAction, this, "increment"_s, 1),
			$$new($InternationalFormatter$IncrementAction, this, "decrement"_s, -1)
		});
	}
	return nullptr;
}
...
char16_t InternationalFormatter::getLiteral(int32_t index) {
	...
	if (var$0 && index < this->string->length()) {
...
bool InternationalFormatter::canReplace($DefaultFormatter$ReplaceHolder* rh) {
	...
	if (!getAllowsInvalid()) {
		...
		if (tl == 0 && rh->length == 1 && $nc(c)->getSelectionStart() != rh->offset) {
			...
			if (overflown || $nc(c)->getSelectedText() == nullptr) {
		...
		$cast($InternationalFormatter$ExtendedReplaceHolder, rh)->endOffset = rh->offset;
		$cast($InternationalFormatter$ExtendedReplaceHolder, rh)->endTextLength = (rh->text != nullptr) ? rh->text->length() : 0;
	} else {
...
bool InternationalFormatter::replace($DefaultFormatter$ReplaceHolder* rh) {
	...
	bool var$0 = $nc(rh)->length > 0 && (rh->text == nullptr || rh->text->length() == 0);
	if (var$0 && ($$nc(getFormattedTextField())->getSelectionStart() != rh->offset || rh->length > 1)) {
		direction = -1;
	}
	if (!getAllowsInvalid()) {
		if ((rh->text == nullptr || rh->text->length() == 0) && rh->length > 0) {
	...
	if ($DefaultFormatter::replace(rh)) {
		if (start != -1) {
			int32_t end = $cast($InternationalFormatter$ExtendedReplaceHolder, rh)->endOffset;
			end += $cast($InternationalFormatter$ExtendedReplaceHolder, rh)->endTextLength;
			repositionCursor(literalCount, end, direction);
		} else {
			start = $cast($InternationalFormatter$ExtendedReplaceHolder, rh)->endOffset;
			if (direction == 1) {
...
char16_t InternationalFormatter::getBufferedChar(int32_t index) {
	if (isValidMask()) {
		if (this->string != nullptr && index < this->string->length()) {
			return this->string->charAt(index);
		}
	}
	return (char16_t)0;
...
void InternationalFormatter::updateMask($AttributedCharacterIterator* iterator) {
	...
	if (iterator != nullptr) {
		...
		if (this->literalMask == nullptr) {
			...
			for (int32_t counter = this->literalMask->length() - 1; counter >= 0; --counter) {
...
void InternationalFormatter::clinit$($Class* clazz) {
...
$Class* InternationalFormatter::load$($String* name, bool initialize) {
	...
	$loadClass(InternationalFormatter, name, initialize, &classInfo$$, InternationalFormatter::clinit$, []($Class* clazz) -> $Object* {
