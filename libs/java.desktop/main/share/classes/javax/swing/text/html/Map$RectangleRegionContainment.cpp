...
void Map$RectangleRegionContainment::init$($AttributeSet* as) {
	...
	if (coords == nullptr || coords->length != 4) {
		...
		if (this->x0 < 0 || this->y0 < 0 || this->x1 < 0 || this->y1 < 0) {
			...
			for (int32_t counter = 0; counter < 4; ++counter) {
				if (coords->get(counter) < 0) {
					this->percents->set(counter, $Math::abs(coords->get(counter)) / 100.0f);
				} else {
					this->percents->set(counter, -1.0f);
				}
			}
		}
	}
...
bool Map$RectangleRegionContainment::contains(int32_t x, int32_t y, int32_t width, int32_t height) {
	...
	if (this->lastWidth != width || this->lastHeight != height) {
		...
		if ($nc(this->percents)->get(0) != -1.0f) {
			this->x0 = $cast(int32_t, (this->percents->get(0) * width));
		}
		if (this->percents->get(1) != -1.0f) {
			this->y0 = $cast(int32_t, (this->percents->get(1) * height));
		}
		if (this->percents->get(2) != -1.0f) {
			this->x1 = $cast(int32_t, (this->percents->get(2) * width));
		}
		if (this->percents->get(3) != -1.0f) {
			this->y1 = $cast(int32_t, (this->percents->get(3) * height));
		}
	}
	return contains(x, y);
}
...
$Class* Map$RectangleRegionContainment::load$($String* name, bool initialize) {
	...
	$loadClass(Map$RectangleRegionContainment, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Map$RectangleRegionContainment);
	});
	return class$;
}
