...
void CSS$ShorthandFontParser::parseShorthandFont($CSS* css, $String* value, $MutableAttributeSet* attr) {
	...
	if (index < count) {
		...
		int32_t slashIndex = $nc(fontSize)->indexOf(u'/');
		if (slashIndex != -1) {
...
bool CSS$ShorthandFontParser::isFontStyle($String* string) {
	...
	return (var$0 || string->equals("oblique"_s));
}
...
bool CSS$ShorthandFontParser::isFontWeight($String* string) {
	...
	bool var$1 = var$2 || string->equals("bolder"_s);
	bool var$0 = var$1 || string->equals("italic"_s);
	if (var$0 || string->equals("lighter"_s)) {
		return true;
	}
	bool var$6 = string->length() == 3;
...
$Class* CSS$ShorthandFontParser::load$($String* name, bool initialize) {
	...
	$loadClass(CSS$ShorthandFontParser, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CSS$ShorthandFontParser);
	});
	return class$;
}
