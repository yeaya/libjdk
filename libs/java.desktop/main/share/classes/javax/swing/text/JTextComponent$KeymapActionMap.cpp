...
int32_t JTextComponent$KeymapActionMap::size() {
	...
	int32_t keymapCount = (actions == nullptr) ? 0 : actions->length;
...
$Class* JTextComponent$KeymapActionMap::load$($String* name, bool initialize) {
	...
	$loadClass(JTextComponent$KeymapActionMap, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JTextComponent$KeymapActionMap);
	});
	return class$;
}
