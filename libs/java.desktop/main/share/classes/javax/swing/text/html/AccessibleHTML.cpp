...
$View* AccessibleHTML::getRootView() {
	$useLocalObjectStack();
	return $$sure($TextUI, $$nc(getTextComponent())->getUI())->getRootView($(getTextComponent()));
}
...
$AccessibleHTML$ElementInfo* AccessibleHTML::createElementInfo($Element* e, $AccessibleHTML$ElementInfo* parent) {
	...
	if (attrs != nullptr) {
		...
		} else if ($equals(name, $HTML$Tag::CONTENT) || $equals(name, $HTML$Tag::CAPTION)) {
			return $new($AccessibleHTML$TextElementInfo, this, e, parent);
		} else if ($equals(name, $HTML$Tag::TABLE)) {
...
$Class* AccessibleHTML::load$($String* name, bool initialize) {
	...
	$loadClass(AccessibleHTML, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AccessibleHTML);
	});
	return class$;
}
