...
void LookAndFeel::installProperty($JComponent* c, $String* propertyName, Object$* propertyValue) {
	if ($SunToolkit::isInstanceOf(c, "javax.swing.JPasswordField"_s)) {
...
void LookAndFeel::loadKeyBindings($InputMap* retMap, $ObjectArray* keys) {
	$useLocalObjectStack();
	if (keys != nullptr) {
		for (int32_t counter = 0, maxCounter = keys->length; counter < maxCounter; ++counter) {
...
$Icon* LookAndFeel::getDisabledIcon($JComponent* component, $Icon* icon) {
	...
	if ($instanceOf($ImageIcon, icon)) {
		return $new($ImageIconUIResource, $($GrayFilter::createDisabledImage($($cast($ImageIcon, icon)->getImage()))));
	}
	return nullptr;
}
...
$Class* LookAndFeel::load$($String* name, bool initialize) {
	...
	$loadClass(LookAndFeel, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LookAndFeel);
	});
	return class$;
}
