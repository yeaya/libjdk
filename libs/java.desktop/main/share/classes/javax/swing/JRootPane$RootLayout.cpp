...
$Dimension* JRootPane$RootLayout::preferredLayoutSize($Container* parent) {
	...
	if (this->this$0->contentPane != nullptr) {
		$assign(rd, this->this$0->contentPane->getPreferredSize());
	} else {
		$assign(rd, $nc(parent)->getSize());
	}
	if (this->this$0->menuBar != nullptr && this->this$0->menuBar->isVisible()) {
		$assign(mbd, this->this$0->menuBar->getPreferredSize());
	} else {
		$assign(mbd, $new($Dimension, 0, 0));
	}
	return $new($Dimension, $Math::max($nc(rd)->width, $nc(mbd)->width) + $nc(i)->left + $nc(i)->right, $nc(rd)->height + $nc(mbd)->height + $nc(i)->top + $nc(i)->bottom);
}
...
$Dimension* JRootPane$RootLayout::minimumLayoutSize($Container* parent) {
	...
	$var($Insets, i, this->this$0->getInsets());
	if (this->this$0->contentPane != nullptr) {
		$assign(rd, this->this$0->contentPane->getMinimumSize());
	} else {
		$assign(rd, $nc(parent)->getSize());
	}
	if (this->this$0->menuBar != nullptr && this->this$0->menuBar->isVisible()) {
		$assign(mbd, this->this$0->menuBar->getMinimumSize());
	} else {
		$assign(mbd, $new($Dimension, 0, 0));
	}
	return $new($Dimension, $Math::max($nc(rd)->width, $nc(mbd)->width) + $nc(i)->left + $nc(i)->right, $nc(rd)->height + $nc(mbd)->height + $nc(i)->top + $nc(i)->bottom);
...
$Dimension* JRootPane$RootLayout::maximumLayoutSize($Container* target) {
	...
	$var($Insets, i, this->this$0->getInsets());
	if (this->this$0->menuBar != nullptr && this->this$0->menuBar->isVisible()) {
		$assign(mbd, this->this$0->menuBar->getMaximumSize());
	} else {
		$assign(mbd, $new($Dimension, 0, 0));
	}
	if (this->this$0->contentPane != nullptr) {
		$assign(rd, this->this$0->contentPane->getMaximumSize());
	} else {
		$assign(rd, $new($Dimension, $Integer::MAX_VALUE, $Integer::MAX_VALUE - $nc(i)->top - $nc(i)->bottom - $nc(mbd)->height - 1));
	}
	return $new($Dimension, $Math::max($nc(rd)->width, $nc(mbd)->width) + $nc(i)->left + $nc(i)->right, $nc(rd)->height + $nc(mbd)->height + $nc(i)->top + $nc(i)->bottom);
}
...
void JRootPane$RootLayout::layoutContainer($Container* parent) {
	...
	if (this->this$0->layeredPane != nullptr) {
		this->this$0->layeredPane->setBounds(i->left, i->top, w, h);
	}
	if (this->this$0->glassPane != nullptr) {
		this->this$0->glassPane->setBounds(i->left, i->top, w, h);
	}
	if (this->this$0->menuBar != nullptr && this->this$0->menuBar->isVisible()) {
		$var($Dimension, mbd, this->this$0->menuBar->getPreferredSize());
		$nc(this->this$0->menuBar)->setBounds(0, 0, w, $nc(mbd)->height);
		contentY += mbd->height;
	}
	if (this->this$0->contentPane != nullptr) {
		this->this$0->contentPane->setBounds(0, contentY, w, h - contentY);
	}
}
...
$Class* JRootPane$RootLayout::load$($String* name, bool initialize) {
	...
	$loadClass(JRootPane$RootLayout, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
