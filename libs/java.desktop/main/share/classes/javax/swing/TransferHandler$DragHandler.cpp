...
void TransferHandler$DragHandler::dragDropEnd($DragSourceDropEvent* dsde) {
	...
	if (dsde->getDropSuccess()) {
		$var($Transferable, var$0, dsc->getTransferable());
		$$nc($nc(c)->getTransferHandler())->exportDone(c, var$0, dsde->getDropAction());
	} else {
...
$Class* TransferHandler$DragHandler::load$($String* name, bool initialize) {
	...
	$loadClass(TransferHandler$DragHandler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
