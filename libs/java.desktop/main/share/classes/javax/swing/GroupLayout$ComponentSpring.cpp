...
int32_t GroupLayout$ComponentSpring::getSizeAlongAxis(int32_t axis, $Dimension* size) {
	return (axis == 0) ? $nc(size)->width : $nc(size)->height;
}
...
void GroupLayout$ComponentSpring::clinit$($Class* clazz) {
	$load($GroupLayout);
...
$Class* GroupLayout$ComponentSpring::load$($String* name, bool initialize) {
	...
	$loadClass(GroupLayout$ComponentSpring, name, initialize, &classInfo$$, GroupLayout$ComponentSpring::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(GroupLayout$ComponentSpring);
	});
	return class$;
}
