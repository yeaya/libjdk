...
void JFormattedTextField$AbstractFormatter::installDocumentFilter($DocumentFilter* filter) {
	...
	if (ftf != nullptr) {
		...
		if ($instanceOf($AbstractDocument, doc)) {
			$cast($AbstractDocument, doc)->setDocumentFilter(filter);
		}
		$load($DocumentFilter);
...
$Class* JFormattedTextField$AbstractFormatter::load$($String* name, bool initialize) {
	...
	$loadClass(JFormattedTextField$AbstractFormatter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JFormattedTextField$AbstractFormatter);
	});
	return class$;
}
