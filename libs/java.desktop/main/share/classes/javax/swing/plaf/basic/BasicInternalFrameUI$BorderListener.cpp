...
void BasicInternalFrameUI$BorderListener::mouseClicked($MouseEvent* e) {
	...
	if (var$0) {
		...
		if (var$1 && this->this$0->frame->isIcon()) {
			try {
...
void BasicInternalFrameUI$BorderListener::finishMouseReleased() {
	...
	if (this->resizeDir == BasicInternalFrameUI$BorderListener::RESIZE_NONE) {
		...
		if ($instanceOf($RootPaneContainer, c)) {
			$var($Component, glassPane, $cast($RootPaneContainer, c)->getGlassPane());
...
void BasicInternalFrameUI$BorderListener::mouseDragged($MouseEvent* e) {
	...
	if (this->this$0->dragging) {
		...
		if (var$2 || ((e->getModifiers() & $InputEvent::BUTTON1_MASK) != $InputEvent::BUTTON1_MASK)) {
			return;
		}
		int32_t pWidth = 0;
		int32_t pHeight = 0;
	...
	case $SwingConstants::NORTH:
		...
		} else if (this->startingBounds->height + deltaY > $nc(max)->height) {
	...
	case $SwingConstants::NORTH_EAST:
		...
		} else if (this->startingBounds->width - deltaX > $nc(max)->width) {
			deltaX = -(max->width - this->startingBounds->width);
		}
		if (this->startingBounds->x + this->startingBounds->width - deltaX > $nc(this->this$0->parentBounds)->width) {
			deltaX = this->startingBounds->x + this->startingBounds->width - this->this$0->parentBounds->width;
		}
		newX = this->startingBounds->x;
		newY = this->startingBounds->y - deltaY;
	...
	case $SwingConstants::SOUTH_EAST:
		...
		if (this->startingBounds->y + this->startingBounds->height - deltaY > this->this$0->parentBounds->height) {
			deltaY = this->startingBounds->y + this->startingBounds->height - this->this$0->parentBounds->height;
		}
		newW = this->startingBounds->width - deltaX;
		newH = this->startingBounds->height - deltaY;
		break;
	case $SwingConstants::SOUTH:
		...
		if (this->startingBounds->y + this->startingBounds->height - deltaY > $nc(this->this$0->parentBounds)->height) {
			deltaY = this->startingBounds->y + this->startingBounds->height - this->this$0->parentBounds->height;
		}
		newW = this->startingBounds->width;
		newH = this->startingBounds->height - deltaY;
...
$Class* BasicInternalFrameUI$BorderListener::load$($String* name, bool initialize) {
	...
	$loadClass(BasicInternalFrameUI$BorderListener, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
