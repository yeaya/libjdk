...
$Class* MetalComboBoxUI$MetalPropertyChangeListener::load$($String* name, bool initialize) {
	...
	$loadClass(MetalComboBoxUI$MetalPropertyChangeListener, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MetalComboBoxUI$MetalPropertyChangeListener);
	});
	return class$;
}
