...
void BasicTabbedPaneUI::paintTab($Graphics* g, int32_t tabPlacement, $RectangleArray* rects, int32_t tabIndex, $Rectangle* iconRect, $Rectangle* textRect) {
	...
	if ($nc(this->tabPane)->getTabComponentAt(tabIndex) == nullptr) {
		...
		if (var$0 && isHorizontalTabPlacement()) {
			...
			bool var$4 = !scrollableTabLayoutEnabled();
			if (var$4 && isHorizontalTabPlacement()) {
...
$Polygon* BasicTabbedPaneUI::createCroppedTabShape(int32_t tabPlacement, $Rectangle* tabRect, int32_t cropline) {
	...
	for (int32_t i = 0; i < rcnt; ++i) {
		for (int32_t j = 0; j < $nc(BasicTabbedPaneUI::xCropLen)->length; ++j) {
			xp->set(pcnt, cropline - BasicTabbedPaneUI::xCropLen->get(j));
	...
	int32_t y = 0;
	{
		...
		case $SwingConstants::RIGHT:
			...
			while (xx <= x + $nc($nc(this->rects)->get(tabIndex))->width) {
				for (int32_t i = 0; i < $nc(BasicTabbedPaneUI::xCropLen)->length; i += 2) {
					g->drawLine(xx + $nc(BasicTabbedPaneUI::yCropLen)->get(i), y - BasicTabbedPaneUI::xCropLen->get(i), xx + $nc(BasicTabbedPaneUI::yCropLen)->get(i + 1) - 1, y - BasicTabbedPaneUI::xCropLen->get(i + 1));
				}
		...
		default:
			...
			while (yy <= y + $nc($nc(this->rects)->get(tabIndex))->height) {
				for (int32_t i = 0; i < $nc(BasicTabbedPaneUI::xCropLen)->length; i += 2) {
					g->drawLine(x - BasicTabbedPaneUI::xCropLen->get(i), yy + $nc(BasicTabbedPaneUI::yCropLen)->get(i), x - BasicTabbedPaneUI::xCropLen->get(i + 1), yy + $nc(BasicTabbedPaneUI::yCropLen)->get(i + 1) - 1);
				}
...
void BasicTabbedPaneUI::paintIcon($Graphics* g, int32_t tabPlacement, int32_t tabIndex, $Icon* icon, $Rectangle* iconRect, bool isSelected) {
	if (icon != nullptr) {
		...
		$cast($Graphics2D, g)->clip(iconRect);
...
void BasicTabbedPaneUI::clinit$($Class* clazz) {
...
$Class* BasicTabbedPaneUI::load$($String* name, bool initialize) {
	...
	$FieldInfo fieldInfos$$[] = {
		...
		{"upKey", "Ljavax/swing/KeyStroke;", nullptr, $PROTECTED | $DEPRECATED, $field(BasicTabbedPaneUI, upKey), upKeyfieldAnnotations$$},
		{"downKey", "Ljavax/swing/KeyStroke;", nullptr, $PROTECTED | $DEPRECATED, $field(BasicTabbedPaneUI, downKey), downKeyfieldAnnotations$$},
		{"leftKey", "Ljavax/swing/KeyStroke;", nullptr, $PROTECTED | $DEPRECATED, $field(BasicTabbedPaneUI, leftKey), leftKeyfieldAnnotations$$},
		{"rightKey", "Ljavax/swing/KeyStroke;", nullptr, $PROTECTED | $DEPRECATED, $field(BasicTabbedPaneUI, rightKey), rightKeyfieldAnnotations$$},
	...
	$loadClass(BasicTabbedPaneUI, name, initialize, &classInfo$$, BasicTabbedPaneUI::clinit$, []($Class* clazz) -> $Object* {
