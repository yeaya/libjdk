...
void BasicScrollBarUI$ScrollListener::actionPerformed($ActionEvent* e) {
	...
	if (this->useBlockIncrement) {
		...
		if ($nc(this->this$0->scrollbar)->getOrientation() == $JScrollBar::VERTICAL) {
			...
			bool var$1 = this->direction > 0 && !this->this$0->isMouseAfterThumb();
			if (var$1 || (this->direction < 0 && !this->this$0->isMouseBeforeThumb())) {
				$$sure($Timer, $nc(e)->getSource())->stop();
			}
		}
	} else {
	...
	bool var$2 = this->direction > 0;
	if (var$2) {
		int32_t var$4 = $nc(this->this$0->scrollbar)->getValue();
		int32_t var$3 = var$4 + this->this$0->scrollbar->getVisibleAmount();
		var$2 = var$3 >= this->this$0->scrollbar->getMaximum();
	}
	if (var$2) {
		$$sure($Timer, $nc(e)->getSource())->stop();
	} else {
		bool var$5 = this->direction < 0;
		if (var$5) {
			int32_t var$6 = $nc(this->this$0->scrollbar)->getValue();
			var$5 = var$6 <= this->this$0->scrollbar->getMinimum();
		}
		if (var$5) {
			$$sure($Timer, $nc(e)->getSource())->stop();
		}
...
$Class* BasicScrollBarUI$ScrollListener::load$($String* name, bool initialize) {
	...
	$loadClass(BasicScrollBarUI$ScrollListener, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(BasicScrollBarUI$ScrollListener);
	});
	return class$;
}
