...
void MetalScrollBarUI$ScrollBarListener::handlePropertyChange(Object$* newValue) {
	if (newValue != nullptr) {
		bool temp = $cast($Boolean, newValue)->booleanValue();
...
$Class* MetalScrollBarUI$ScrollBarListener::load$($String* name, bool initialize) {
	...
	$loadClass(MetalScrollBarUI$ScrollBarListener, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MetalScrollBarUI$ScrollBarListener);
	});
	return class$;
}
