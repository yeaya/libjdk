...
void BasicInternalFrameUI$Handler::componentResized($ComponentEvent* e) {
	$useLocalObjectStack();
	$var($Rectangle, parentNewBounds, $$sure($Component, $nc(e)->getSource())->getBounds());
	$var($JInternalFrame$JDesktopIcon, icon, nullptr);
	if (this->this$0->frame != nullptr) {
		$assign(icon, this->this$0->frame->getDesktopIcon());
		if ($nc(this->this$0->frame)->isMaximum()) {
			$nc(this->this$0->frame)->setBounds(0, 0, $nc(parentNewBounds)->width, $nc(parentNewBounds)->height);
		}
	}
	if (icon != nullptr) {
...
$Dimension* BasicInternalFrameUI$Handler::preferredLayoutSize($Container* c) {
	...
	result->width += $nc(i)->left + $nc(i)->right;
...
void BasicInternalFrameUI$Handler::layoutContainer($Container* c) {
	...
	if (this->this$0->getSouthPane() != nullptr) {
		...
		$$nc(this->this$0->getSouthPane())->setBounds(cx, $nc(this->this$0->frame)->getHeight() - i->bottom - $nc(size)->height, cw, $nc(size)->height);
		ch -= size->height;
	}
	if (this->this$0->getWestPane() != nullptr) {
	...
	if (this->this$0->getEastPane() != nullptr) {
		...
		$$nc(this->this$0->getEastPane())->setBounds(cw - $nc(size)->width, cy, $nc(size)->width, ch);
		cw -= size->width;
	}
...
void BasicInternalFrameUI$Handler::propertyChange($PropertyChangeEvent* evt) {
	...
	if ($JInternalFrame::IS_CLOSED_PROPERTY == prop) {
		...
		} else if ($equals(newValue, $Boolean::FALSE) && $equals(oldValue, $Boolean::TRUE)) {
	...
	} else if ($JInternalFrame::TITLE_PROPERTY == prop || prop == "closable"_s || prop == "iconable"_s || prop == "maximizable"_s) {
...
$Class* BasicInternalFrameUI$Handler::load$($String* name, bool initialize) {
	...
	$loadClass(BasicInternalFrameUI$Handler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
