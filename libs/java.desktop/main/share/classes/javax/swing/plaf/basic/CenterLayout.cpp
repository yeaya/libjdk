...
$Dimension* CenterLayout::preferredLayoutSize($Container* container) {
	...
	if (c != nullptr) {
		...
		return $new($Dimension, $nc(size)->width + $nc(insets)->left + $nc(insets)->right, $nc(size)->height + $nc(insets)->top + $nc(insets)->bottom);
	} else {
...
void CenterLayout::layoutContainer($Container* container) {
	...
	if ($nc(container)->getComponentCount() > 0) {
		...
		containerWidth -= $nc(containerInsets)->left + $nc(containerInsets)->right;
