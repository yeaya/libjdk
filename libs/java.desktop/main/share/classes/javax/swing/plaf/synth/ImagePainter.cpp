...
$Paint9Painter* ImagePainter::getPaint9Painter() {
	...
	$synchronized(ImagePainter::CACHE_KEY) {
		...
		if (cacheRef == nullptr || ($assign(painter, $cast($Paint9Painter, cacheRef->get()))) == nullptr) {
...
void ImagePainter::init$(bool tiles, bool paintCenter, $Insets* sourceInsets, $Insets* destinationInsets, $URL* path, bool center) {
	...
	if (destinationInsets == nullptr) {
		...
		$set(this, dInsets, $cast($Insets, destinationInsets->clone()));
	}
	this->tiles = tiles;
...
$Image* ImagePainter::getImage() {
	if (this->image == nullptr) {
		$set(this, image, $$new($ImageIcon, this->path, nullptr)->getImage());
	}
	return this->image;
}
...
void ImagePainter::clinit$($Class* clazz) {
...
$Class* ImagePainter::load$($String* name, bool initialize) {
	...
	$loadClass(ImagePainter, name, initialize, &classInfo$$, ImagePainter::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ImagePainter);
	});
	return class$;
}
