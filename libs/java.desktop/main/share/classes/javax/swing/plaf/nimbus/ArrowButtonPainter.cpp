...
$Class* ArrowButtonPainter::load$($String* name, bool initialize) {
	...
	$loadClass(ArrowButtonPainter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ArrowButtonPainter);
	});
	return class$;
}
