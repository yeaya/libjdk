...
void MetalUtils$GradientPainter::paintToImage($Component* c, $Image* image, $Graphics* g, int32_t w, int32_t h, $ObjectArray* args) {
	...
	if (isVertical) {
		float var$0 = $$sure($Number, $nc(gradient)->get(0))->floatValue();
		float var$1 = $$sure($Number, gradient->get(1))->floatValue();
		$var($Color, var$2, $cast($Color, gradient->get(2)));
		$var($Color, var$3, $cast($Color, gradient->get(3)));
		drawVerticalGradient(g2, var$0, var$1, var$2, var$3, $$cast($Color, gradient->get(4)), w, h);
	} else {
		float var$4 = $$sure($Number, $nc(gradient)->get(0))->floatValue();
		float var$5 = $$sure($Number, gradient->get(1))->floatValue();
		$var($Color, var$6, $cast($Color, gradient->get(2)));
		$var($Color, var$7, $cast($Color, gradient->get(3)));
		drawHorizontalGradient(g2, var$4, var$5, var$6, var$7, $$cast($Color, gradient->get(4)), w, h);
	}
}
...
void MetalUtils$GradientPainter::clinit$($Class* clazz) {
...
$Class* MetalUtils$GradientPainter::load$($String* name, bool initialize) {
	...
	$loadClass(MetalUtils$GradientPainter, name, initialize, &classInfo$$, MetalUtils$GradientPainter::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(MetalUtils$GradientPainter);
	});
	return class$;
}
