...
void BasicColorChooserUI$Handler::propertyChange($PropertyChangeEvent* evt) {
	...
	if (prop == $JColorChooser::CHOOSER_PANELS_PROPERTY) {
		...
		if (numNewPanels == 0) {
			$nc(this->this$0->chooser)->remove(this->this$0->tabbedPane);
			return;
		} else if (numNewPanels == 1) {
			$nc(this->this$0->chooser)->remove(this->this$0->tabbedPane);
			$var($JPanel, centerWrapper, $new($JPanel, $$new($CenterLayout)));
			centerWrapper->setInheritsPopupMenu(true);
			centerWrapper->add(newPanels->get(0));
			$init($BorderLayout);
			$nc(this->this$0->singlePanel)->add(centerWrapper, $BorderLayout::CENTER);
			$nc(this->this$0->chooser)->add(this->this$0->singlePanel);
		} else {
			if (oldPanels->length < 2) {
				$nc(this->this$0->chooser)->remove(this->this$0->singlePanel);
				$init($BorderLayout);
				$nc(this->this$0->chooser)->add(this->this$0->tabbedPane, $BorderLayout::CENTER);
			}
			for (int32_t i = 0; i < newPanels->length; ++i) {
				...
				centerWrapper->add(newPanels->get(i));
...
$Class* BasicColorChooserUI$Handler::load$($String* name, bool initialize) {
	...
	$loadClass(BasicColorChooserUI$Handler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
