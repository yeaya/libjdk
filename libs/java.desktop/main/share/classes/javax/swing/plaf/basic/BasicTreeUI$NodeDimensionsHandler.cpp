...
$Rectangle* BasicTreeUI$NodeDimensionsHandler::getNodeDimensions(Object$* value, int32_t row, int32_t depth, bool expanded, $Rectangle* size$renamed) {
	...
	if (this->this$0->editingComponent != nullptr && this->this$0->editingRow == row) {
		$var($Dimension, prefSize, this->this$0->editingComponent->getPreferredSize());
		...
		if (size != nullptr) {
			...
			$assign(size, $new($Rectangle, getRowX(row, depth), 0, $nc(prefSize)->width, $nc(prefSize)->height));
		}
		return size;
	}
	if (this->this$0->currentCellRenderer != nullptr) {
		...
		bool var$1 = $nc(this->this$0->tree)->isRowSelected(row);
		$assign(aComponent, this->this$0->currentCellRenderer->getTreeCellRendererComponent(var$0, value, var$1, expanded, $nc(this->this$0->treeModel)->isLeaf(value), row, false));
...
$Class* BasicTreeUI$NodeDimensionsHandler::load$($String* name, bool initialize) {
	...
	$loadClass(BasicTreeUI$NodeDimensionsHandler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(BasicTreeUI$NodeDimensionsHandler);
	});
	return class$;
}
