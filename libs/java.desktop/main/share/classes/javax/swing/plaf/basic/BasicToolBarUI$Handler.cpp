...
void BasicToolBarUI$Handler::propertyChange($PropertyChangeEvent* evt) {
	...
	if (propertyName == "lookAndFeel"_s) {
		...
		int32_t orientation = $$sure($Integer, evt->getNewValue())->intValue();
		...
		if (propertyName == $BasicToolBarUI::IS_ROLLOVER) {
			...
			this->this$0->setRolloverBorders($$sure($Boolean, evt->getNewValue())->booleanValue());
		}
	}
}
...
$Class* BasicToolBarUI$Handler::load$($String* name, bool initialize) {
	...
	$loadClass(BasicToolBarUI$Handler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
