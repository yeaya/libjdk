...
$Path2D* CheckBoxMenuItemPainter::decodePath1() {
	...
	double var$1 = decodeX(0.4292683f);
	$nc(this->path)->lineTo(var$1, decodeY(1.5f));
	double var$2 = decodeX(0.7121951f);
	$nc(this->path)->lineTo(var$2, decodeY(2.4780488f));
	double var$3 = decodeX(2.5926828f);
...
$Class* CheckBoxMenuItemPainter::load$($String* name, bool initialize) {
	...
	$loadClass(CheckBoxMenuItemPainter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CheckBoxMenuItemPainter);
	});
	return class$;
}
