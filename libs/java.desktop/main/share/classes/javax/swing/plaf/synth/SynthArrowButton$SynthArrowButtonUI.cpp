...
void SynthArrowButton$SynthArrowButtonUI::paint($SynthContext* context, $Graphics* g) {
	...
	int32_t var$0 = $nc(button)->getWidth();
	int32_t var$1 = button->getHeight();
	$$nc(context->getPainter())->paintArrowButtonForeground(context, g, 0, 0, var$0, var$1, button->getDirection());
}
...
void SynthArrowButton$SynthArrowButtonUI::paintBackground($SynthContext* context, $Graphics* g, $JComponent* c) {
	...
	$$nc($nc(context)->getPainter())->paintArrowButtonBackground(context, g, 0, 0, var$0, c->getHeight());
}
...
$Dimension* SynthArrowButton$SynthArrowButtonUI::getPreferredSize($JComponent* c) {
	...
	if (dim == nullptr) {
		int32_t size = $$nc(context->getStyle())->getInt(context, "ArrowButton.size"_s, 16);
	...
	if ($instanceOf($JComponent, parent) && !($instanceOf($JComboBox, parent))) {
		$var($Object, scaleKey, $cast($JComponent, parent)->getClientProperty("JComponent.sizeVariant"_s));
		if (scaleKey != nullptr) {
			if ("large"_s->equals(scaleKey)) {
				$assign(dim, $new($Dimension, $cast(int32_t, ($nc(dim)->width * 1.15)), $cast(int32_t, ($nc(dim)->height * 1.15))));
			} else if ("small"_s->equals(scaleKey)) {
				$assign(dim, $new($Dimension, $cast(int32_t, ($nc(dim)->width * 0.857)), $cast(int32_t, ($nc(dim)->height * 0.857))));
			} else if ("mini"_s->equals(scaleKey)) {
				$assign(dim, $new($Dimension, $cast(int32_t, ($nc(dim)->width * 0.714)), $cast(int32_t, ($nc(dim)->height * 0.714))));
			}
		}
	}
	return dim;
...
$Class* SynthArrowButton$SynthArrowButtonUI::load$($String* name, bool initialize) {
	...
	$loadClass(SynthArrowButton$SynthArrowButtonUI, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
