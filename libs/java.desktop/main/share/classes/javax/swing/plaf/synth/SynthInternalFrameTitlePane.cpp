...
void SynthInternalFrameTitlePane::showSystemMenu() {
	...
	if (!$nc(this->frame)->isIcon()) {
		$var($Component, var$0, this->frame);
		int32_t var$1 = $nc(this->menuButton)->getX();
		int32_t var$2 = getY();
		$nc(this->systemPopupMenu)->show(var$0, var$1, var$2 + getHeight());
	} else {
		$var($Component, var$3, this->menuButton);
		int32_t var$4 = getX() - $nc(insets)->left - $nc(insets)->right;
		int32_t var$5 = getY();
...
void SynthInternalFrameTitlePane::paintComponent($Graphics* g) {
	...
	$$nc($nc(context)->getPainter())->paintInternalFrameTitlePaneBackground(context, g, 0, 0, var$0, getHeight());
	paint(context, g);
}
...
void SynthInternalFrameTitlePane::paint($SynthContext* context, $Graphics* g) {
	...
	if (title != nullptr) {
		...
		$var($FontMetrics, fm, $SwingUtilities2::getFontMetrics(this->frame, g));
		...
		if (ltr) {
			if (lastButton != nullptr) {
				...
				maxX = var$3 - $nc($(this->frame->getInsets()))->right - this->titleSpacing;
			}
			int32_t var$4 = $nc(this->menuButton)->getX();
			minX = var$4 + this->menuButton->getWidth() + this->titleSpacing;
		} else {
			if (lastButton != nullptr) {
		...
		if (clippedTitle == title) {
			if (titleAlignment == $SwingConstants::TRAILING) {
				minX = maxX - $$nc(style->getGraphicsUtils(context))->computeStringWidth(context, $(g->getFont()), fm, title);
			} else if (titleAlignment == $SwingConstants::CENTER) {
				int32_t width = $$nc(style->getGraphicsUtils(context))->computeStringWidth(context, $(g->getFont()), fm, title);
				minX = $Math::max(minX, (getWidth() - width) / 2);
				minX = $Math::min(maxX - width, minX);
			}
		}
		$$nc(style->getGraphicsUtils(context))->paintText(context, g, clippedTitle, minX, baseline - fm->getAscent(), -1);
	}
}
...
$LayoutManager* SynthInternalFrameTitlePane::createLayout() {
	...
	return (lm != nullptr) ? lm : $cast($LayoutManager, $new($SynthInternalFrameTitlePane$SynthTitlePaneLayout, this));
}
...
void SynthInternalFrameTitlePane::updateMenuIcon() {
	...
	if (frameIcon != nullptr) {
		...
		if ((var$0 || frameIcon->getIconHeight() > maxHeight) && ($instanceOf($ImageIcon, frameIcon))) {
			$assign(frameIcon, $new($ImageIcon, $($$nc($cast($ImageIcon, frameIcon)->getImage())->getScaledInstance(maxWidth, maxHeight, $Image::SCALE_SMOOTH))));
		}
	}
...
$Class* SynthInternalFrameTitlePane::load$($String* name, bool initialize) {
	...
	$loadClass(SynthInternalFrameTitlePane, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
