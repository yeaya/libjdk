...
$Class* OptionPaneMessageAreaOptionPaneLabelPainter::load$($String* name, bool initialize) {
	...
	$loadClass(OptionPaneMessageAreaOptionPaneLabelPainter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(OptionPaneMessageAreaOptionPaneLabelPainter);
	});
	return class$;
}
