...
void MetalComboBoxButton::paintComponent($Graphics* g) {
	...
	int32_t width = getWidth() - ($nc(insets)->left + $nc(insets)->right);
	...
	if (this->comboIcon != nullptr) {
		...
		if (var$0) {
			...
			var$0 = var$1 || this->comboBox->isEditable();
		}
		if (var$0) {
	...
	if (!this->iconOnly && this->comboBox != nullptr) {
		$var($ListCellRenderer, renderer, this->comboBox->getRenderer());
...
$Dimension* MetalComboBoxButton::getMinimumSize() {
	...
	$var($Insets, insets, getInsets());
	ret->width = $nc(insets)->left + $$nc(getComboIcon())->getIconWidth() + $nc(insets)->right;
...
$Class* MetalComboBoxButton::load$($String* name, bool initialize) {
	...
	$loadClass(MetalComboBoxButton, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
