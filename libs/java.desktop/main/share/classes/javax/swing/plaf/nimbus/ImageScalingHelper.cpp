...
void ImageScalingHelper::paint($Graphics* g, int32_t x, int32_t y, int32_t w, int32_t h, $Image* image, $Insets* sInsets$renamed, $Insets* dInsets$renamed, $ImageScalingHelper$PaintType* paintType, int32_t mask) {
	...
	} else if (paintType == $ImageScalingHelper$PaintType::TILE) {
		int32_t lastIY = 0;
...
void ImageScalingHelper::clinit$($Class* clazz) {
...
$Class* ImageScalingHelper::load$($String* name, bool initialize) {
	...
	$loadClass(ImageScalingHelper, name, initialize, &classInfo$$, ImageScalingHelper::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ImageScalingHelper);
	});
	return class$;
}
