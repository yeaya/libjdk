...
$SynthPainter* ParsedSynthStyle$DelegatingPainter::getPainter($SynthContext* context, $String* method, int32_t direction) {
	...
	return $$sure($ParsedSynthStyle, $nc(context)->getStyle())->getBestPainter(context, method, direction);
}
...
$Class* ParsedSynthStyle$DelegatingPainter::load$($String* name, bool initialize) {
	...
	$loadClass(ParsedSynthStyle$DelegatingPainter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ParsedSynthStyle$DelegatingPainter);
	});
	return class$;
}
