...
int32_t MultiTabbedPaneUI::tabForCoordinate($JTabbedPane* a, int32_t b, int32_t c) {
	$useLocalObjectStack();
	int32_t returnValue = $$cast($TabbedPaneUI, $nc(this->uis)->elementAt(0))->tabForCoordinate(a, b, c);
	for (int32_t i = 1; i < $nc(this->uis)->size(); ++i) {
		$$cast($TabbedPaneUI, this->uis->elementAt(i))->tabForCoordinate(a, b, c);
	}
	return returnValue;
}
...
$Rectangle* MultiTabbedPaneUI::getTabBounds($JTabbedPane* a, int32_t b) {
	$useLocalObjectStack();
	$var($Rectangle, returnValue, $$cast($TabbedPaneUI, $nc(this->uis)->elementAt(0))->getTabBounds(a, b));
	for (int32_t i = 1; i < $nc(this->uis)->size(); ++i) {
		$$cast($TabbedPaneUI, this->uis->elementAt(i))->getTabBounds(a, b);
	}
	return returnValue;
}
...
int32_t MultiTabbedPaneUI::getTabRunCount($JTabbedPane* a) {
	$useLocalObjectStack();
	int32_t returnValue = $$cast($TabbedPaneUI, $nc(this->uis)->elementAt(0))->getTabRunCount(a);
	for (int32_t i = 1; i < $nc(this->uis)->size(); ++i) {
		$$cast($TabbedPaneUI, this->uis->elementAt(i))->getTabRunCount(a);
	}
	return returnValue;
}
...
$Class* MultiTabbedPaneUI::load$($String* name, bool initialize) {
	...
	$loadClass(MultiTabbedPaneUI, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MultiTabbedPaneUI);
	});
	return class$;
}
