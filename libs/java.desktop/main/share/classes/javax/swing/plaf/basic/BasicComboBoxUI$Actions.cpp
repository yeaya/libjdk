...
void BasicComboBoxUI$Actions::actionPerformed($ActionEvent* e) {
	...
	if (key == BasicComboBoxUI$Actions::HIDE) {
		...
		if (ui != nullptr && (key == BasicComboBoxUI$Actions::TOGGLE || !comboBox->isEditable())) {
		...
		if (comboBox->isShowing() && ui != nullptr) {
			...
			if (root != nullptr) {
				...
				if (im != nullptr && am != nullptr) {
					...
					if (obj != nullptr) {
						...
						if (action != nullptr) {
							int32_t var$4 = e->getID();
							$var($String, var$5, e->getActionCommand());
							int64_t var$6 = e->getWhen();
							action->actionPerformed($$new($ActionEvent, root, var$4, var$5, var$6, e->getModifiers()));
						}
					}
				}
			}
...
void BasicComboBoxUI$Actions::clinit$($Class* clazz) {
...
$Class* BasicComboBoxUI$Actions::load$($String* name, bool initialize) {
	...
	$loadClass(BasicComboBoxUI$Actions, name, initialize, &classInfo$$, BasicComboBoxUI$Actions::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(BasicComboBoxUI$Actions);
	});
	return class$;
}
