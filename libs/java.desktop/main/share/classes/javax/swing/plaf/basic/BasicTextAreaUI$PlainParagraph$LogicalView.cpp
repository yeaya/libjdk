...
$Class* BasicTextAreaUI$PlainParagraph$LogicalView::load$($String* name, bool initialize) {
	...
	$loadClass(BasicTextAreaUI$PlainParagraph$LogicalView, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(BasicTextAreaUI$PlainParagraph$LogicalView);
	});
	return class$;
}
