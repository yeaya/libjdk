...
void BasicSliderUI$Actions::clinit$($Class* clazz) {
...
$Class* BasicSliderUI$Actions::load$($String* name, bool initialize) {
	...
	$loadClass(BasicSliderUI$Actions, name, initialize, &classInfo$$, BasicSliderUI$Actions::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(BasicSliderUI$Actions);
	});
	return class$;
}
