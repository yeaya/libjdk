...
$Path2D* MenuPainter::decodePath1() {
	...
	double var$1 = decodeX(2.7512195f);
...
$Path2D* MenuPainter::decodePath2() {
	...
	double var$1 = decodeX(1.9529617f);
...
$Class* MenuPainter::load$($String* name, bool initialize) {
	...
	$loadClass(MenuPainter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MenuPainter);
	});
	return class$;
}
