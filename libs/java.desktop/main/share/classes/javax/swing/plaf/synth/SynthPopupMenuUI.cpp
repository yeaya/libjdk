...
void SynthPopupMenuUI::update($Graphics* g, $JComponent* c) {
	...
	$$nc($nc(context)->getPainter())->paintPopupMenuBackground(context, g, 0, 0, var$0, c->getHeight());
	paint(context, g);
}
...
$Class* SynthPopupMenuUI::load$($String* name, bool initialize) {
	...
	$loadClass(SynthPopupMenuUI, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
