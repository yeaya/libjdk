...
void MetalInternalFrameTitlePane::init$($JInternalFrame* f) {
	...
	this->buttonsWidth = 0;
	$var($Color, var$0, $MetalLookAndFeel::getPrimaryControlHighlight());
	$var($Color, var$1, $MetalLookAndFeel::getPrimaryControlDarkShadow());
	$set(this, activeBumps, $new($MetalBumps, 0, 0, var$0, var$1, ($UIManager::get("InternalFrame.activeTitleGradient"_s) != nullptr) ? ($Color*)nullptr : $$cast($Color, $MetalLookAndFeel::getPrimaryControl())));
	$var($Color, var$2, $MetalLookAndFeel::getControlHighlight());
	$var($Color, var$3, $MetalLookAndFeel::getControlDarkShadow());
	$set(this, inactiveBumps, $new($MetalBumps, 0, 0, var$2, var$3, ($UIManager::get("InternalFrame.inactiveTitleGradient"_s) != nullptr) ? ($Color*)nullptr : $$cast($Color, $MetalLookAndFeel::getControl())));
...
void MetalInternalFrameTitlePane::paintPalette($Graphics* g) {
	...
	if (this->paletteBumps == nullptr) {
		$var($Color, var$0, $MetalLookAndFeel::getPrimaryControlHighlight());
		$var($Color, var$1, $MetalLookAndFeel::getPrimaryControlInfo());
	...
	if (frameTitle != nullptr) {
		...
		if (leftToRight) {
			if ($nc(rect)->x == 0) {
				...
				rect->x = var$1 - $nc($(this->frame->getInsets()))->right - 2;
			}
...
void MetalInternalFrameTitlePane::updateOptionPaneState() {
	...
	if ($instanceOf($Integer, obj)) {
		type = $cast($Integer, obj)->intValue();
	}
	switch (type) {
...
void MetalInternalFrameTitlePane::clinit$($Class* clazz) {
...
$Class* MetalInternalFrameTitlePane::load$($String* name, bool initialize) {
	...
	$loadClass(MetalInternalFrameTitlePane, name, initialize, &classInfo$$, MetalInternalFrameTitlePane::clinit$, []($Class* clazz) -> $Object* {
