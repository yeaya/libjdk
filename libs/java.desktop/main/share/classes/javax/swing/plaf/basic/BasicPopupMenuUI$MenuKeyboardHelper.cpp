...
void BasicPopupMenuUI$MenuKeyboardHelper::stateChanged($ChangeEvent* ev) {
	...
	if ($nc(this->lastPathSelected)->length != 0 && $nc(p)->length != 0) {
		if (!$BasicPopupMenuUI::checkInvokerEqual(p->get(0), this->lastPathSelected->get(0))) {
			removeItems();
			$set(this, lastPathSelected, $new($MenuElementArray, 0));
		}
	}
	if ($nc(this->lastPathSelected)->length == 0 && $nc(p)->length > 0) {
		...
		if (popup == nullptr) {
			...
			$var($Component, c, popup->getInvoker());
			if ($instanceOf($JFrame, c)) {
				$assign(invoker, $cast($JFrame, c)->getRootPane());
			} else if ($instanceOf($JDialog, c)) {
				$assign(invoker, $cast($JDialog, c)->getRootPane());
			} else if ($instanceOf($JApplet, c)) {
				$assign(invoker, $cast($JApplet, c)->getRootPane());
			} else {
	...
	} else if (this->lastPathSelected->length != 0 && $nc(p)->length == 0) {
		removeItems();
...
$Class* BasicPopupMenuUI$MenuKeyboardHelper::load$($String* name, bool initialize) {
	...
	$loadClass(BasicPopupMenuUI$MenuKeyboardHelper, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
