...
int32_t Spring$SpringMap::getMaximumValue() {
	return $Math::min($Short::MAX_VALUE, map($nc(this->s)->getMaximumValue()));
}
...
$Class* Spring$SpringMap::load$($String* name, bool initialize) {
	...
	$loadClass(Spring$SpringMap, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Spring$SpringMap);
	});
	return class$;
}
