...
void JTable$SortManager::dispose() {
	if (this->sorter != nullptr) {
		this->sorter->removeRowSorterListener(this->this$0);
	}
}
	...
	if (change != nullptr) {
		if (change->allRowsChanged) {
			...
			if (change->type == $TableModelEvent::INSERT) {
				this->modelRowSizes->insertEntries(change->startModelIndex, change->endModelIndex - change->startModelIndex + 1, this->this$0->getRowHeight());
...
$Class* JTable$SortManager::load$($String* name, bool initialize) {
	...
	$loadClass(JTable$SortManager, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JTable$SortManager);
	});
	return class$;
}
