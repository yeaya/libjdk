...
$Object* DefaultListModel::getElementAt(int32_t index) {
	return $nc(this->delegate)->elementAt(index);
}
...
$Object* DefaultListModel::firstElement() {
	return $nc(this->delegate)->firstElement();
}

$Object* DefaultListModel::lastElement() {
	return $nc(this->delegate)->lastElement();
}
...
$Class* DefaultListModel::load$($String* name, bool initialize) {
	...
	$loadClass(DefaultListModel, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
