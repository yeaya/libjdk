...
$String* ValueFormatter::valueToString(Object$* object) {
	...
	if ($instanceOf($Integer, object)) {
		...
		int32_t value = $cast($Integer, object)->intValue();
		int32_t index = this->length;
		$var($chars, array, $new($chars, index));
		while (0 < index--) {
			array->set(index, $Character::forDigit(value & 0x0f, this->radix));
			value >>= 4;
		}
		$init($Locale);
...
$Class* ValueFormatter::load$($String* name, bool initialize) {
	...
	$loadClass(ValueFormatter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
