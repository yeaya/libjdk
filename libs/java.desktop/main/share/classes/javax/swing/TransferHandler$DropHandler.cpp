...
void TransferHandler$DropHandler::init$() {
	...
	this->hysteresis = 10;
	$set(this, support, $new($TransferHandler$TransferSupport, nullptr, ($DropTargetEvent*)nullptr));
}

void TransferHandler$DropHandler::updateAutoscrollRegion($JComponent* c) {
	$useLocalObjectStack();
	$var($Rectangle, visible, $nc(c)->getVisibleRect());
	$nc(this->outer)->setBounds($nc(visible)->x, $nc(visible)->y, $nc(visible)->width, $nc(visible)->height);
	...
	$nc(this->inner)->setBounds(visible->x + i->left, visible->y + i->top, visible->width - (i->left + i->right), visible->height - (i->top + i->bottom));
}
...
void TransferHandler$DropHandler::autoscroll($JComponent* c, $Point* pos) {
	...
	if ($instanceOf($Scrollable, c)) {
		...
		if ($nc(pos)->y < $nc(this->inner)->y) {
			int32_t dy = s->getScrollableUnitIncrement(this->outer, $SwingConstants::VERTICAL, -1);
		...
		if (pos->x < $nc(this->inner)->x) {
			int32_t dx = s->getScrollableUnitIncrement(this->outer, $SwingConstants::HORIZONTAL, -1);
...
void TransferHandler$DropHandler::initPropertiesIfNecessary() {
	...
	if (this->timer == nullptr) {
		...
		$set(this, timer, $new($Timer, prop == nullptr ? 100 : prop->intValue(), this));
		$assign(prop, $cast($Integer, t->getDesktopProperty("DnD.Autoscroll.initialDelay"_s)));
		$nc(this->timer)->setInitialDelay(prop == nullptr ? 100 : prop->intValue());
...
void TransferHandler$DropHandler::setComponentDropLocation($TransferHandler$TransferSupport* support, bool forDrop) {
	$useLocalObjectStack();
	$var($TransferHandler$DropLocation, dropLocation, (support == nullptr) ? ($TransferHandler$DropLocation*)nullptr : support->getDropLocation());
	if ($SunToolkit::isInstanceOf(this->component, "javax.swing.text.JTextComponent"_s)) {
		...
		$set(this, state, $cast($JComponent, this->component)->setDropLocation(dropLocation, this->state, forDrop));
	}
}
...
void TransferHandler$DropHandler::handleDrag($DropTargetDragEvent* e) {
	...
	bool showLocation = $nc(this->support)->showDropLocationIsSet ? this->support->showDropLocation : canImport;
...
void TransferHandler$DropHandler::dragOver($DropTargetDragEvent* e) {
	...
	if (var$0 || $Math::abs(p->y - this->lastPosition->y) > this->hysteresis) {
...
void TransferHandler$DropHandler::cleanup(bool forDrop) {
	...
	if ($instanceOf($JComponent, this->component)) {
		$cast($JComponent, this->component)->dndDone();
	}
	if (this->timer != nullptr) {
		this->timer->stop();
	}
...
$Class* TransferHandler$DropHandler::load$($String* name, bool initialize) {
	...
	$loadClass(TransferHandler$DropHandler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
