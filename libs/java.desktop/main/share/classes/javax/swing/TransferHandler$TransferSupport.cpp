...
void TransferHandler$TransferSupport::setDNDVariables($Component* component, $DropTargetEvent* event) {
	...
	$var($Point, p, $instanceOf($DropTargetDragEvent, this->source) ? $cast($DropTargetDragEvent, this->source)->getLocation() : $nc($cast($DropTargetDropEvent, this->source))->getLocation());
	if ($SunToolkit::isInstanceOf(component, "javax.swing.text.JTextComponent"_s)) {
		...
		$set(this, dropLocation, $cast($JComponent, component)->dropLocationForPoint(p));
	}
}
...
void TransferHandler$TransferSupport::setDropAction(int32_t dropAction) {
	$useLocalObjectStack();
	assureIsDrop();
	int32_t action = dropAction & getSourceDropActions();
...
int32_t TransferHandler$TransferSupport::getUserDropAction() {
	assureIsDrop();
	return ($instanceOf($DropTargetDragEvent, this->source)) ? $cast($DropTargetDragEvent, this->source)->getDropAction() : $nc($cast($DropTargetDropEvent, this->source))->getDropAction();
}

int32_t TransferHandler$TransferSupport::getSourceDropActions() {
	assureIsDrop();
	return ($instanceOf($DropTargetDragEvent, this->source)) ? $cast($DropTargetDragEvent, this->source)->getSourceActions() : $nc($cast($DropTargetDropEvent, this->source))->getSourceActions();
}

$DataFlavorArray* TransferHandler$TransferSupport::getDataFlavors() {
	if (this->isDrop$) {
		if ($instanceOf($DropTargetDragEvent, this->source)) {
			return $cast($DropTargetDragEvent, this->source)->getCurrentDataFlavors();
		} else {
...
bool TransferHandler$TransferSupport::isDataFlavorSupported($DataFlavor* df) {
	if (this->isDrop$) {
		if ($instanceOf($DropTargetDragEvent, this->source)) {
			return $cast($DropTargetDragEvent, this->source)->isDataFlavorSupported(df);
		} else {
...
$Transferable* TransferHandler$TransferSupport::getTransferable() {
	if (this->isDrop$) {
		if ($instanceOf($DropTargetDragEvent, this->source)) {
			return $cast($DropTargetDragEvent, this->source)->getTransferable();
		} else {
...
void TransferHandler$TransferSupport::clinit$($Class* clazz) {
...
$Class* TransferHandler$TransferSupport::load$($String* name, bool initialize) {
	...
	$loadClass(TransferHandler$TransferSupport, name, initialize, &classInfo$$, TransferHandler$TransferSupport::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(TransferHandler$TransferSupport);
	});
	return class$;
}
