...
void GroupLayout$ComponentInfo::removeSpring($GroupLayout$Spring* spring) {
	if (spring != nullptr) {
		$nc($nc($$cast($GroupLayout$Group, spring->getParent()))->springs)->remove(spring);
	}
}
...
void GroupLayout$ComponentInfo::setBounds($Insets* insets, int32_t parentWidth, bool ltr) {
	...
	$nc(this->component)->setBounds(x + $nc(insets)->left, y + $nc(insets)->top, w, h);
}
...
void GroupLayout$ComponentInfo::clinit$($Class* clazz) {
	$load($GroupLayout);
...
$Class* GroupLayout$ComponentInfo::load$($String* name, bool initialize) {
	...
	$loadClass(GroupLayout$ComponentInfo, name, initialize, &classInfo$$, GroupLayout$ComponentInfo::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(GroupLayout$ComponentInfo);
	});
	return class$;
}
