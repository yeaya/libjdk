...
bool AudioFormat::matches(AudioFormat* format) {
	...
	if (var$5) {
		...
		if (!var$6) {
			int32_t var$7 = format->getChannels();
	...
	if (var$4) {
		...
		if (!var$8) {
			float var$9 = format->getSampleRate();
	...
	if (var$3) {
		...
		if (!var$10) {
			int32_t var$11 = format->getSampleSizeInBits();
	...
	if (var$2) {
		...
		if (!var$12) {
			float var$13 = format->getFrameRate();
	...
	if (var$1) {
		...
		if (!var$14) {
			int32_t var$15 = format->getFrameSize();
	...
	if (var$0) {
		...
		if (!var$16) {
			bool var$17 = format->isBigEndian();
...
$Class* AudioFormat::load$($String* name, bool initialize) {
	...
	$loadClass(AudioFormat, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AudioFormat);
	});
	return class$;
}
