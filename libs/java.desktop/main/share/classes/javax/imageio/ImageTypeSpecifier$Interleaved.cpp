...
bool ImageTypeSpecifier$Interleaved::equals(Object$* o) {
	...
	if ((!($nc(this->colorSpace)->equals($nc(that)->colorSpace))) || (this->dataType != that->dataType) || (this->hasAlpha != that->hasAlpha) || (this->isAlphaPremultiplied != that->isAlphaPremultiplied) || ($nc(this->bandOffsets)->length != $nc(that->bandOffsets)->length)) {
		return false;
	}
...
$Class* ImageTypeSpecifier$Interleaved::load$($String* name, bool initialize) {
	...
	$loadClass(ImageTypeSpecifier$Interleaved, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ImageTypeSpecifier$Interleaved);
	});
	return class$;
}
