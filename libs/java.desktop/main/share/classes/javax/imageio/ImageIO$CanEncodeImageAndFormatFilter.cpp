...
bool ImageIO$CanEncodeImageAndFormatFilter::filter(Object$* elt) {
	...
	return var$0 && spi->canEncodeImage(this->type);
}
...
$Class* ImageIO$CanEncodeImageAndFormatFilter::load$($String* name, bool initialize) {
	...
	$loadClass(ImageIO$CanEncodeImageAndFormatFilter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ImageIO$CanEncodeImageAndFormatFilter);
	});
	return class$;
}
