...
ImageTypeSpecifier* ImageTypeSpecifier::createFromRenderedImage($RenderedImage* image) {
	...
	if ($instanceOf($BufferedImage, image)) {
		int32_t bufferedImageType = $cast($BufferedImage, image)->getType();
...
$BufferedImage* ImageTypeSpecifier::createBufferedImage(int32_t width, int32_t height) {
	...
	try {
		...
		bool var$1 = $nc(this->colorModel)->isAlphaPremultiplied();
		return $new($BufferedImage, var$0, raster, var$1, $$new($Hashtable));
...
void ImageTypeSpecifier::clinit$($Class* clazz) {
	{
...
$Class* ImageTypeSpecifier::load$($String* name, bool initialize) {
	...
	$loadClass(ImageTypeSpecifier, name, initialize, &classInfo$$, ImageTypeSpecifier::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ImageTypeSpecifier);
	});
	return class$;
}
