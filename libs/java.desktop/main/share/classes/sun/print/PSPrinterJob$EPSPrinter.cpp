...
void PSPrinterJob$EPSPrinter::print() {
	...
	this->stream->println($$str({"%%BoundingBox: "_s, $$str(this->llx), " "_s, $$str(this->lly), " "_s, $$str(this->urx), " "_s, $$str(this->ury)}));
	this->stream->println($$str({"%%Title: "_s, this->epsTitle}));
	this->stream->println("%%Creator: Java Printing"_s);
	this->stream->println($$str({"%%CreationDate: "_s, $$new($Date)}));
	this->stream->println("%%EndComments"_s);
	this->stream->println("/pluginSave save def"_s);
	this->stream->println("mark"_s);
	$set(this, job, $new($PSPrinterJob));
	$set(this->job, epsPrinter, this);
	$set(this->job, mPSStream, this->stream);
	this->job->mDestType = $RasterPrinterJob::STREAM;
	this->job->startDoc();
	...
	try {
		...
		this->stream->println("pluginSave restore"_s);
		$nc(this->job)->endDoc();
	}
	if (var$0 != nullptr) {
		$throw(var$0);
...
$Class* PSPrinterJob$EPSPrinter::load$($String* name, bool initialize) {
	...
	$loadClass(PSPrinterJob$EPSPrinter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PSPrinterJob$EPSPrinter);
	});
	return class$;
}
