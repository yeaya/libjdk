...
$String* PrinterJobWrapper::toString() {
	return $str({"printerjob-wrapper: "_s, $($nc(this->job)->toString())});
}

int32_t PrinterJobWrapper::hashCode() {
	return $nc(this->job)->hashCode();
}
...
$Class* PrinterJobWrapper::load$($String* name, bool initialize) {
	...
	$loadClass(PrinterJobWrapper, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PrinterJobWrapper);
	});
	return class$;
}
