...
void ServiceDialog$SidesPanel::actionPerformed($ActionEvent* e) {
	...
	if ($nc(this->rbOneSide)->isSameAs(source)) {
		$init($Sides);
		$nc(this->this$0->asCurrent)->add($cast($DocAttribute, $Sides::ONE_SIDED));
	} else if ($nc(this->rbTumble)->isSameAs(source)) {
		$init($Sides);
		$nc(this->this$0->asCurrent)->add($cast($DocAttribute, $Sides::TUMBLE));
	} else if ($nc(this->rbDuplex)->isSameAs(source)) {
		$init($Sides);
		$nc(this->this$0->asCurrent)->add($cast($DocAttribute, $Sides::DUPLEX));
	}
}
...
void ServiceDialog$SidesPanel::updateInfo() {
	...
	if ($nc(this->this$0->psCurrent)->isAttributeCategorySupported(sdCategory)) {
		$var($Object, values, $nc(this->this$0->psCurrent)->getSupportedAttributeValues(sdCategory, this->this$0->docFlavor, $cast($HashAttributeSet, this->this$0->asCurrent)));
		if ($instanceOf($SidesArray, values)) {
			$var($SidesArray, svalues, $cast($SidesArray, values));
			for (int32_t i = 0; i < svalues->length; ++i) {
...
$Class* ServiceDialog$SidesPanel::load$($String* name, bool initialize) {
	...
	$loadClass(ServiceDialog$SidesPanel, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
