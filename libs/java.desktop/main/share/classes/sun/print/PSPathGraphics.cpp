	...
	if (scaleX != 0 && scaleY != 0) {
		...
		if (boundsWidth > 0 && boundsHeight > 0) {
			...
			if (drawOpaque == false) {
				...
				$nc(imageGraphics)->translate(var$14, -rotBounds->getY());
...
void PSPathGraphics::redrawRegion($Rectangle2D* region, double scaleX, double scaleY, $Shape* savedClip, $AffineTransform* savedTransform) {
	...
	double var$6 = -psPrinterJob->getPhysicalPrintableY($(pageFormat->getPaper()));
...
void PSPathGraphics::deviceFill($PathIterator* pathIter, $Color* color) {
	$useLocalObjectStack();
	$var($PSPrinterJob, psPrinterJob, $cast($PSPrinterJob, getPrinterJob()));
	$var($AffineTransform, var$0, getTransform());
	$nc(psPrinterJob)->deviceFill(pathIter, color, var$0, $(getClip()));
}
...
$Class* PSPathGraphics::load$($String* name, bool initialize) {
	...
	$loadClass(PSPathGraphics, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
