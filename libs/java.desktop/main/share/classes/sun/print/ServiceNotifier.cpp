...
bool ServiceNotifier::isEmpty() {
	return (this->listeners == nullptr || this->listeners->isEmpty());
}
		...
		$synchronized(this) {
			...
			if (this->listeners != nullptr) {
				if ($instanceOf($AttributeUpdater, this->service)) {
					$assign(psa, $cast($AttributeUpdater, this->service)->getUpdatedAttributes());
				} else {
					$assign(psa, $nc(this->service)->getAttributes());
				}
				if (psa != nullptr && !psa->isEmpty()) {
					for (int32_t i = 0; i < $nc(this->listeners)->size(); ++i) {
						$assign(listener, $cast($PrintServiceAttributeListener, this->listeners->elementAt(i)));
...
$Class* ServiceNotifier::load$($String* name, bool initialize) {
	...
	$loadClass(ServiceNotifier, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ServiceNotifier);
	});
	return class$;
}
