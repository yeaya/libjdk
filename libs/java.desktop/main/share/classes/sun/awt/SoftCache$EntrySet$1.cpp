...
bool SoftCache$EntrySet$1::hasNext() {
	...
	while ($nc(this->hashIterator)->hasNext()) {
		$var($Map$Entry, ent, $cast($Map$Entry, this->hashIterator->next()));
...
$Class* SoftCache$EntrySet$1::load$($String* name, bool initialize) {
	...
	$loadClass(SoftCache$EntrySet$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SoftCache$EntrySet$1);
	});
	return class$;
}
