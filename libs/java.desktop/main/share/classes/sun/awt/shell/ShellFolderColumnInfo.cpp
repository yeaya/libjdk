...
void ShellFolderColumnInfo::init$($String* title, int32_t width, int32_t alignment, bool visible) {
	$useLocalObjectStack();
	$var($Integer, var$0, $Integer::valueOf(width));
	ShellFolderColumnInfo::init$(title, var$0, $($Integer::valueOf(alignment)), visible, nullptr, nullptr);
}
...
$Class* ShellFolderColumnInfo::load$($String* name, bool initialize) {
	...
	$loadClass(ShellFolderColumnInfo, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ShellFolderColumnInfo);
	});
	return class$;
}
