...
$Class* SunClipboard$$Lambda$lambda$setContents$0::load$($String* name, bool initialize) {
	...
	$loadClass(SunClipboard$$Lambda$lambda$setContents$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SunClipboard$$Lambda$lambda$setContents$0);
	});
	return class$;
}
...
$Class* SunClipboard$$Lambda$lambda$lostOwnershipLater$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(SunClipboard$$Lambda$lambda$lostOwnershipLater$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SunClipboard$$Lambda$lambda$lostOwnershipLater$1$1);
	});
	return class$;
}
...
$Class* SunClipboard$$Lambda$lambda$checkChange$2$2::load$($String* name, bool initialize) {
	...
	$loadClass(SunClipboard$$Lambda$lambda$checkChange$2$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SunClipboard$$Lambda$lambda$checkChange$2$2);
	});
	return class$;
}
	...
	$synchronized(this) {
		...
		try {
			...
			if (oldOwner != nullptr && oldOwner != owner) {
				$EventQueue::invokeLater($$new(SunClipboard$$Lambda$lambda$setContents$0, this, oldOwner, oldContents));
			}
		}
		if (var$0 != nullptr) {
			$throw(var$0);
...
$Object* SunClipboard::getData($DataFlavor* flavor) {
	...
	return $$nc($DataTransferer::getInstance())->translateBytes(data, flavor, format, localeTransferable);
}
...
void SunClipboard::propertyChange($PropertyChangeEvent* evt) {
	...
	if (var$0 && $nc($Boolean::TRUE)->equals($(evt->getNewValue()))) {
		$var($AppContext, disposedContext, $cast($AppContext, evt->getSource()));
...
void SunClipboard::lostOwnershipLater($AppContext* disposedContext) {
	...
	$SunToolkit::postEvent(context, $$new($PeerEvent, this, $$new(SunClipboard$$Lambda$lambda$lostOwnershipLater$1$1, this, disposedContext), $PeerEvent::PRIORITY_EVENT));
}
...
$FlavorListenerArray* SunClipboard::getFlavorListeners() {
	$synchronized(this) {
		$useLocalObjectStack();
		$var($Set, flavorListeners, getFlavorListeners($($AppContext::getAppContext())));
		return flavorListeners == nullptr ? $new($FlavorListenerArray, 0) : $cast($FlavorListenerArray, flavorListeners->toArray($$new($FlavorListenerArray, flavorListeners->size())));
	}
}
...
void SunClipboard::checkChange($longs* formats) {
	...
	$set(this, currentFormats, formats);
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			{
				if (appContext == nullptr || appContext->isDisposed()) {
					continue;
				}
				$var($Set, flavorListeners, getFlavorListeners(appContext));
				if (flavorListeners != nullptr) {
					...
					for (; $nc(i$)->hasNext();) {
						...
						if (listener != nullptr) {
							$var($PeerEvent, peerEvent, $new($PeerEvent, this, $$new(SunClipboard$$Lambda$lambda$checkChange$2$2, this, listener), $PeerEvent::PRIORITY_EVENT));
...
$Class* SunClipboard::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("sun.awt.datatransfer.SunClipboard$$Lambda$lambda$setContents$0")) {
			return SunClipboard$$Lambda$lambda$setContents$0::load$(name, initialize);
		}
		if (name->equals("sun.awt.datatransfer.SunClipboard$$Lambda$lambda$lostOwnershipLater$1$1")) {
			return SunClipboard$$Lambda$lambda$lostOwnershipLater$1$1::load$(name, initialize);
		}
		if (name->equals("sun.awt.datatransfer.SunClipboard$$Lambda$lambda$checkChange$2$2")) {
	...
	$loadClass(SunClipboard, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
