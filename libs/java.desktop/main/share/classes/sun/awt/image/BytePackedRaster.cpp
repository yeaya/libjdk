...
void BytePackedRaster::init$($SampleModel* sampleModel, $Point* origin) {
	...
	BytePackedRaster::init$(sampleModel, var$0, $$new($Rectangle, var$1, var$2, var$3, sampleModel->getHeight()), origin, nullptr);
}
...
$Object* BytePackedRaster::getDataElements(int32_t x, int32_t y, Object$* obj) {
	...
	int32_t element = $nc(this->data)->get((y - this->minY) * this->scanlineStride + (bitnum >> 3)) & 0xff;
	int32_t shift = this->shiftOffset - (bitnum & 7);
	$nc(outData)->set(0, (int8_t)(($sr(element, shift)) & this->bitMask));
	return outData;
}

$Object* BytePackedRaster::getDataElements(int32_t x, int32_t y, int32_t w, int32_t h, Object$* outData) {
	return getByteData(x, y, w, h, $cast($bytes, outData));
}
...
$Object* BytePackedRaster::getPixelData(int32_t x, int32_t y, int32_t w, int32_t h, Object$* obj) {
	...
	for (int32_t j = 0; j < h; ++j) {
		...
		for (int32_t i = 0; i < w; ++i) {
			int32_t shift = this->shiftOffset - (bitnum & 7);
			$nc(outData)->set(outindex++, (int8_t)(this->bitMask & ($sr($nc(data)->get(index + (bitnum >> 3)), shift))));
			bitnum += pixbits;
		}
	...
	for (int32_t j = 0; j < h; ++j) {
		...
		case 1:
			for (; i < w - 7; i += 8) {
				...
				$nc(outData)->set(outindex++, (int8_t)((element >> 7) & 1));
				outData->set(outindex++, (int8_t)((element >> 6) & 1));
				outData->set(outindex++, (int8_t)((element >> 5) & 1));
				outData->set(outindex++, (int8_t)((element >> 4) & 1));
				outData->set(outindex++, (int8_t)((element >> 3) & 1));
				outData->set(outindex++, (int8_t)((element >> 2) & 1));
				outData->set(outindex++, (int8_t)((element >> 1) & 1));
				outData->set(outindex++, (int8_t)(element & 1));
				bitnum += 8;
			}
			break;
		case 2:
			for (; i < w - 7; i += 8) {
				element = $nc(data)->get(inIndex++);
				$nc(outData)->set(outindex++, (int8_t)((element >> 6) & 3));
				outData->set(outindex++, (int8_t)((element >> 4) & 3));
				outData->set(outindex++, (int8_t)((element >> 2) & 3));
				outData->set(outindex++, (int8_t)(element & 3));
				element = data->get(inIndex++);
				outData->set(outindex++, (int8_t)((element >> 6) & 3));
				outData->set(outindex++, (int8_t)((element >> 4) & 3));
				outData->set(outindex++, (int8_t)((element >> 2) & 3));
				outData->set(outindex++, (int8_t)(element & 3));
				bitnum += 16;
		...
		case 4:
			for (; i < w - 7; i += 8) {
				element = $nc(data)->get(inIndex++);
				$nc(outData)->set(outindex++, (int8_t)((element >> 4) & 0x0f));
				outData->set(outindex++, (int8_t)(element & 0x0f));
				element = data->get(inIndex++);
				outData->set(outindex++, (int8_t)((element >> 4) & 0x0f));
				outData->set(outindex++, (int8_t)(element & 0x0f));
				element = data->get(inIndex++);
				outData->set(outindex++, (int8_t)((element >> 4) & 0x0f));
				outData->set(outindex++, (int8_t)(element & 0x0f));
...
void BytePackedRaster::setDataElements(int32_t x, int32_t y, Object$* obj) {
	...
	element |= $sl($nc(inData)->get(0) & this->bitMask, shift);
	this->data->set(index, element);
	markDirty();
}

void BytePackedRaster::setDataElements(int32_t x, int32_t y, $Raster* inRaster) {
	if (!($instanceOf(BytePackedRaster, inRaster)) || $cast(BytePackedRaster, inRaster)->pixelBitStride != this->pixelBitStride) {
	...
	if ((inbit & 7) == (outbit & 7)) {
		...
		if (bitpos != 0) {
			...
			for (int32_t j = 0; j < height; ++j) {
				...
				element |= ($nc(inData)->get(inbyte) & mask);
		...
		if (copybits > 0) {
			int32_t inbyte = inbit >> 3;
			int32_t outbyte = outbit >> 3;
			int32_t mask = ($sr(0x0000ff00, copybits)) & 0xff;
			for (int32_t j = 0; j < height; ++j) {
				int32_t element = $nc(outData)->get(outbyte);
				element &= (uint32_t)~mask;
				element |= ($nc(inData)->get(inbyte) & mask);
		...
		if (bitpos != 0 || copybits < 8) {
			...
			for (int32_t j = 0; j < height; ++j) {
				...
				int32_t element = $nc(outData)->get(outbyte);
				element &= (uint32_t)~mask;
				element |= ($sr(($sl(inData0, lshift)) | ($sr(inData1 & 0xff, rshift)), bitpos)) & mask;
				outData->set(outbyte, (int8_t)element);
				inbyte += inscan;
				outbyte += outscan;
			}
		...
		if (copybits >= 8) {
			...
			for (int32_t j = 0; j < height; ++j) {
				...
				for (int32_t i = 0; i < copybytes; ++i) {
					...
					int32_t val = ($sl(inData0, lshift)) | ($sr(inData1 & 0xff, rshift));
		...
		if (copybits > 0) {
			...
			for (int32_t j = 0; j < height; ++j) {
				...
				int32_t element = $nc(outData)->get(outbyte);
				element &= (uint32_t)~mask;
				element |= (($sl(inData0, lshift)) | ($sr(inData1 & 0xff, rshift))) & mask;
				outData->set(outbyte, (int8_t)element);
				inbyte += inscan;
				outbyte += outscan;
			}
...
void BytePackedRaster::setRect(int32_t dx, int32_t dy, $Raster* srcRaster) {
	if (!($instanceOf(BytePackedRaster, srcRaster)) || $cast(BytePackedRaster, srcRaster)->pixelBitStride != this->pixelBitStride) {
...
void BytePackedRaster::putByteData(int32_t x, int32_t y, int32_t w, int32_t h, $bytes* inData) {
	...
	for (int32_t j = 0; j < h; ++j) {
		...
		while ((i < w) && ((bitnum & 7) != 0)) {
			...
			element |= $sl($nc(inData)->get(outindex++) & this->bitMask, shift);
		...
		case 1:
			for (; i < w - 7; i += 8) {
				element = ($nc(inData)->get(outindex++) & 1) << 7;
				element |= (inData->get(outindex++) & 1) << 6;
				element |= (inData->get(outindex++) & 1) << 5;
				element |= (inData->get(outindex++) & 1) << 4;
				element |= (inData->get(outindex++) & 1) << 3;
				element |= (inData->get(outindex++) & 1) << 2;
				element |= (inData->get(outindex++) & 1) << 1;
				element |= (inData->get(outindex++) & 1);
		...
		case 2:
			for (; i < w - 7; i += 8) {
				element = ($nc(inData)->get(outindex++) & 3) << 6;
				element |= (inData->get(outindex++) & 3) << 4;
				element |= (inData->get(outindex++) & 3) << 2;
				element |= (inData->get(outindex++) & 3);
				$nc(data)->set(inIndex++, (int8_t)element);
				element = (inData->get(outindex++) & 3) << 6;
				element |= (inData->get(outindex++) & 3) << 4;
				element |= (inData->get(outindex++) & 3) << 2;
				element |= (inData->get(outindex++) & 3);
		...
		case 4:
			for (; i < w - 7; i += 8) {
				element = ($nc(inData)->get(outindex++) & 0x0f) << 4;
				element |= (inData->get(outindex++) & 0x0f);
				$nc(data)->set(inIndex++, (int8_t)element);
				element = (inData->get(outindex++) & 0x0f) << 4;
				element |= (inData->get(outindex++) & 0x0f);
				data->set(inIndex++, (int8_t)element);
				element = (inData->get(outindex++) & 0x0f) << 4;
				element |= (inData->get(outindex++) & 0x0f);
...
$ints* BytePackedRaster::getPixels(int32_t x, int32_t y, int32_t w, int32_t h, $ints* iArray$renamed) {
	...
	for (int32_t j = 0; j < h; ++j) {
		...
		while ((i < w) && ((bitnum & 7) != 0)) {
			int32_t shift = this->shiftOffset - (bitnum & 7);
			$nc(iArray)->set(outindex++, this->bitMask & ($sr($nc(data)->get(index + (bitnum >> 3)), shift)));
			bitnum += pixbits;
			++i;
		}
		int32_t inIndex = index + (bitnum >> 3);
		...
		case 1:
			for (; i < w - 7; i += 8) {
				element = $nc(data)->get(inIndex++);
				$nc(iArray)->set(outindex++, (element >> 7) & 1);
				iArray->set(outindex++, (element >> 6) & 1);
				iArray->set(outindex++, (element >> 5) & 1);
				iArray->set(outindex++, (element >> 4) & 1);
				iArray->set(outindex++, (element >> 3) & 1);
				iArray->set(outindex++, (element >> 2) & 1);
				iArray->set(outindex++, (element >> 1) & 1);
				iArray->set(outindex++, element & 1);
				bitnum += 8;
			}
			break;
		case 2:
			for (; i < w - 7; i += 8) {
				element = $nc(data)->get(inIndex++);
				$nc(iArray)->set(outindex++, (element >> 6) & 3);
				iArray->set(outindex++, (element >> 4) & 3);
				iArray->set(outindex++, (element >> 2) & 3);
				iArray->set(outindex++, element & 3);
				element = data->get(inIndex++);
				iArray->set(outindex++, (element >> 6) & 3);
				iArray->set(outindex++, (element >> 4) & 3);
				iArray->set(outindex++, (element >> 2) & 3);
				iArray->set(outindex++, element & 3);
				bitnum += 16;
		...
		case 4:
			for (; i < w - 7; i += 8) {
				element = $nc(data)->get(inIndex++);
				$nc(iArray)->set(outindex++, (element >> 4) & 0x0f);
				iArray->set(outindex++, element & 0x0f);
				element = data->get(inIndex++);
				iArray->set(outindex++, (element >> 4) & 0x0f);
				iArray->set(outindex++, element & 0x0f);
				element = data->get(inIndex++);
				iArray->set(outindex++, (element >> 4) & 0x0f);
				iArray->set(outindex++, element & 0x0f);
...
void BytePackedRaster::setPixels(int32_t x, int32_t y, int32_t w, int32_t h, $ints* iArray) {
	...
	for (int32_t j = 0; j < h; ++j) {
		...
		while ((i < w) && ((bitnum & 7) != 0)) {
			...
			element = $nc(data)->get(index + (bitnum >> 3));
			element &= (uint32_t)~($sl(this->bitMask, shift));
			element |= $sl($nc(iArray)->get(outindex++) & this->bitMask, shift);
			data->set(index + (bitnum >> 3), (int8_t)element);
			bitnum += pixbits;
			++i;
		}
		...
		case 1:
			for (; i < w - 7; i += 8) {
				element = ($nc(iArray)->get(outindex++) & 1) << 7;
				element |= (iArray->get(outindex++) & 1) << 6;
				element |= (iArray->get(outindex++) & 1) << 5;
				element |= (iArray->get(outindex++) & 1) << 4;
				element |= (iArray->get(outindex++) & 1) << 3;
				element |= (iArray->get(outindex++) & 1) << 2;
				element |= (iArray->get(outindex++) & 1) << 1;
				element |= (iArray->get(outindex++) & 1);
				$nc(data)->set(inIndex++, (int8_t)element);
				bitnum += 8;
			}
			break;
		case 2:
			for (; i < w - 7; i += 8) {
				element = ($nc(iArray)->get(outindex++) & 3) << 6;
				element |= (iArray->get(outindex++) & 3) << 4;
				element |= (iArray->get(outindex++) & 3) << 2;
				element |= (iArray->get(outindex++) & 3);
				$nc(data)->set(inIndex++, (int8_t)element);
				element = (iArray->get(outindex++) & 3) << 6;
				element |= (iArray->get(outindex++) & 3) << 4;
				element |= (iArray->get(outindex++) & 3) << 2;
				element |= (iArray->get(outindex++) & 3);
				data->set(inIndex++, (int8_t)element);
		...
		case 4:
			for (; i < w - 7; i += 8) {
				element = ($nc(iArray)->get(outindex++) & 0x0f) << 4;
				element |= (iArray->get(outindex++) & 0x0f);
				$nc(data)->set(inIndex++, (int8_t)element);
				element = (iArray->get(outindex++) & 0x0f) << 4;
				element |= (iArray->get(outindex++) & 0x0f);
				data->set(inIndex++, (int8_t)element);
				element = (iArray->get(outindex++) & 0x0f) << 4;
				element |= (iArray->get(outindex++) & 0x0f);
...
$WritableRaster* BytePackedRaster::createWritableChild(int32_t x, int32_t y, int32_t width, int32_t height, int32_t x0, int32_t y0, $ints* bandList) {
	...
	return $new(BytePackedRaster, sm, var$0, var$1, $$new($Point, this->sampleModelTranslateX + deltaX, this->sampleModelTranslateY + deltaY), this);
}
...
void BytePackedRaster::clinit$($Class* clazz) {
	{
...
$Class* BytePackedRaster::load$($String* name, bool initialize) {
	...
	$loadClass(BytePackedRaster, name, initialize, &classInfo$$, BytePackedRaster::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(BytePackedRaster);
	});
	return class$;
}
