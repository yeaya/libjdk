...
case$1:
					// COLOR | ALPHA
					pngassert(len == 6);
					$assign(c, $new($Color, $nc(buf)->get(st) & 0xff, $nc(buf)->get(st + 2) & 0xff, $nc(buf)->get(st + 4) & 0xff));
					break;
case$2:
					// COLOR | PALETTE
case$3:
					// COLOR | PALETTE | ALPHA
					pngassert(len == 1);
					ix = $nc(buf)->get(st) & 0xff;
					pngassert(this->red_map != nullptr && ix < this->red_map->length);
					$assign(c, $new($Color, $nc(this->red_map)->get(ix) & 0xff, $nc(this->green_map)->get(ix) & 0xff, $nc(this->blue_map)->get(ix) & 0xff));
					break;
case$4:
					// GRAY
case$5:
					// GRAY | ALPHA
					pngassert(len == 2);
					t = $nc(buf)->get(st) & 0xff;
					$assign(c, $new($Color, t, t, t));
					break;
				} while (false);
			}
			if (c != nullptr) {
				property("background"_s, c);
			}
			break;
		}
	case PNGImageDecoder::cHRMChunk:
		{
			...
			property("chromaticities"_s, $$new($PNGImageDecoder$Chromaticities, var$0, var$1, var$2, var$3, var$4, var$5, var$6, getInt(st + 28)));
			break;
		}
	...
	case PNGImageDecoder::PLTEChunk:
		{
			...
			for (int32_t i = 0, j = st; i < tsize; ++i, j += 3) {
				this->red_map->set(i, $nc(buf)->get(j));
				this->green_map->set(i, buf->get(j + 1));
				this->blue_map->set(i, buf->get(j + 2));
			}
		}
		break;
	...
	case PNGImageDecoder::tIMEChunk:
		{
			...
			property("modtime"_s, $($$new($GregorianCalendar, var$8, var$9, var$10, var$11, var$12, getByte(st + 6))->getTime()));
			break;
		}
...
case$7:
				...
				while (--alen >= len) {
					this->alpha_map->set(alen, (int8_t)255);
				}
				break;
case$8:
				// COLOR
case$9:
				...
				if (this->bitDepth == 16) {
					...
					int32_t var$14 = (getShort(st + 0) & 0xff) << 16;
					int32_t var$13 = var$14 | ((getShort(st + 2) & 0xff) << 8);
					this->transparentPixel = var$13 | (getShort(st + 4) & 0xff);
				}
				break;
case$10:
				// GRAY
...
void PNGImageDecoder::produceImage() {
	...
	try {
		try {
			for (int32_t i = 0; i < PNGImageDecoder::signature->length; ++i) {
				if ((PNGImageDecoder::signature->get(i) & 0xff) != $nc(this->underlyingInputStream)->read()) {
				...
				// GRAY
				{
					...
					if (($set(this, cm, $nc(PNGImageDecoder::greyModels)->get(llog))) == nullptr) {
						...
						if (this->transparentPixel == -1) {
							...
							$set(this, cm, $new($IndexColorModel, this->bitDepth, ramp->length, ramp, ramp, ramp, (this->transparentPixel & 0xff)));
						}
						PNGImageDecoder::greyModels->set(llog, this->cm);
					}
				}
				$assign(bPixels, $new($bytes, pixSize));
				break;
...
case$18:
			...
			setHints(flags);
			headerComplete();
			int32_t samplesPerPixel = ((this->colorType & PNGImageDecoder::PALETTE) != 0 ? 1 : ((this->colorType & PNGImageDecoder::COLOR) != 0 ? 3 : 1) + ((this->colorType & PNGImageDecoder::ALPHA) != 0 ? 1 : 0));
			...
			while (++pass <= passLimit) {
				int32_t row = PNGImageDecoder::startingRow->get(pass);
				int32_t rowInc = PNGImageDecoder::rowIncrement->get(pass);
				int32_t colInc = PNGImageDecoder::colIncrement->get(pass);
				int32_t bWidth = PNGImageDecoder::blockWidth->get(pass);
				int32_t bHeight = PNGImageDecoder::blockHeight->get(pass);
				int32_t sCol = PNGImageDecoder::startingCol->get(pass);
...
case$19:
								// COLOR | ALPHA | (8 << 3)
								wPixels->set(col + rowOffset, ((((rowByteBuffer->get(spos) & 0xff) << 16) | ((rowByteBuffer->get(spos + 1) & 0xff) << 8)) | (rowByteBuffer->get(spos + 2) & 0xff)) | ((rowByteBuffer->get(spos + 3) & 0xff) << 24));
								spos += 4;
								break;
case$20:
								// COLOR | ALPHA | (16 << 3)
								wPixels->set(col + rowOffset, ((((rowByteBuffer->get(spos) & 0xff) << 16) | ((rowByteBuffer->get(spos + 2) & 0xff) << 8)) | (rowByteBuffer->get(spos + 4) & 0xff)) | ((rowByteBuffer->get(spos + 6) & 0xff) << 24));
								spos += 8;
								break;
case$21:
								// COLOR | (8 << 3)
								pixel = (((rowByteBuffer->get(spos) & 0xff) << 16) | ((rowByteBuffer->get(spos + 1) & 0xff) << 8)) | (rowByteBuffer->get(spos + 2) & 0xff);
...
case$22:
								// COLOR | (16 << 3)
								pixel = (((rowByteBuffer->get(spos) & 0xff) << 16) | ((rowByteBuffer->get(spos + 2) & 0xff) << 8)) | (rowByteBuffer->get(spos + 4) & 0xff);
								isTransparent = (this->transparentPixel_16 != nullptr);
								for (int32_t i = 0; isTransparent && (i < 6); ++i) {
									isTransparent &= (rowByteBuffer->get(spos + i) & 0xff) == ($nc(this->transparentPixel_16)->get(i) & 0xff);
								}
								if (!isTransparent) {
									pixel |= (int32_t)0xff000000;
								}
...
case$23:
								// GRAY | ALPHA | (8 << 3)
								{
									int32_t tx = rowByteBuffer->get(spos) & 0xff;
									wPixels->set(col + rowOffset, (((tx << 16) | (tx << 8)) | tx) | ((rowByteBuffer->get(spos + 1) & 0xff) << 24));
								}
								spos += 2;
								break;
case$24:
								...
								{
									int32_t tx = rowByteBuffer->get(spos) & 0xff;
									wPixels->set(col + rowOffset, (((tx << 16) | (tx << 8)) | tx) | ((rowByteBuffer->get(spos + 2) & 0xff) << 24));
								}
								spos += 4;
								break;
case$25:
							...
							switch (this->bitDepth) {
							case 1:
								$nc(bPixels)->set(col + rowOffset, (int8_t)(($sr(rowByteBuffer->get(spos >> 3), 7 - (spos & 7))) & 1));
								++spos;
								break;
							case 2:
								$nc(bPixels)->set(col + rowOffset, (int8_t)(($sr(rowByteBuffer->get(spos >> 2), (3 - (spos & 3)) * 2)) & 3));
								++spos;
								break;
							case 4:
								$nc(bPixels)->set(col + rowOffset, (int8_t)(($sr(rowByteBuffer->get(spos >> 1), (1 - (spos & 1)) * 4)) & 0x0f));
								++spos;
								break;
							case 8:
...
void PNGImageDecoder::filterRow($bytes* rowByteBuffer, $bytes* prevRow, int32_t rowFilter, int32_t rowByteWidth, int32_t bytesPerSample) {
	...
	case 1:
		for (x = bytesPerSample; x < rowByteWidth; ++x) {
			(*$nc(rowByteBuffer))[x] += $nc(rowByteBuffer)->get(x - bytesPerSample);
		}
		break;
	case 2:
		if (prevRow != nullptr) {
	...
	case 3:
		if (prevRow != nullptr) {
			for (; x < bytesPerSample; ++x) {
				(*$nc(rowByteBuffer))[x] += (0xff & prevRow->get(x)) >> 1;
			}
			for (; x < rowByteWidth; ++x) {
				(*$nc(rowByteBuffer))[x] += ((prevRow->get(x) & 0xff) + ($nc(rowByteBuffer)->get(x - bytesPerSample) & 0xff)) >> 1;
			}
		} else {
			for (x = bytesPerSample; x < rowByteWidth; ++x) {
				(*$nc(rowByteBuffer))[x] += ($nc(rowByteBuffer)->get(x - bytesPerSample) & 0xff) >> 1;
			}
		}
		break;
	case 4:
		if (prevRow != nullptr) {
			...
			for (; x < rowByteWidth; ++x) {
				...
				int32_t pc = 0;
				int32_t rval = 0;
				a = $nc(rowByteBuffer)->get(x - bytesPerSample) & 0xff;
				b = prevRow->get(x) & 0xff;
				c = prevRow->get(x - bytesPerSample) & 0xff;
				p = a + b - c;
				pa = p > a ? p - a : a - p;
				pb = p > b ? p - b : b - p;
				pc = p > c ? p - c : c - p;
...
void PNGImageDecoder::init$($InputStreamImageSource* src, $InputStream* input) {
	...
	$set(this, underlyingInputStream, this->inputStream->underlyingInputStream);
}
...
int32_t PNGImageDecoder::getInt(int32_t pos) {
	return (((($nc(this->inbuf)->get(pos) & 0xff) << 24) | (($nc(this->inbuf)->get(pos + 1) & 0xff) << 16)) | (($nc(this->inbuf)->get(pos + 2) & 0xff) << 8)) | ($nc(this->inbuf)->get(pos + 3) & 0xff);
}

int32_t PNGImageDecoder::getShort(int32_t pos) {
	return (int16_t)((($nc(this->inbuf)->get(pos) & 0xff) << 8) | ($nc(this->inbuf)->get(pos + 1) & 0xff));
}

int32_t PNGImageDecoder::getByte(int32_t pos) {
	return $nc(this->inbuf)->get(pos) & 0xff;
}
...
int32_t PNGImageDecoder::update_crc(int32_t crc, $bytes* buf, int32_t offset, int32_t len) {
	...
	int32_t c = crc;
	while (--len >= 0) {
		c = PNGImageDecoder::crc_table->get((c ^ $nc(buf)->get(offset++)) & 0xff) ^ ((int32_t)((uint32_t)c >> 8));
	}
	return c;
}
...
void PNGImageDecoder::clinit$($Class* clazz) {
	...
	$assignStatic(PNGImageDecoder::crc_table, $new($ints, 256));
	{
		for (int32_t n = 0; n < 256; ++n) {
			...
			PNGImageDecoder::crc_table->set(n, c);
		}
	}
}
...
$Class* PNGImageDecoder::load$($String* name, bool initialize) {
	...
	$loadClass(PNGImageDecoder, name, initialize, &classInfo$$, PNGImageDecoder::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(PNGImageDecoder);
	});
	return class$;
}
