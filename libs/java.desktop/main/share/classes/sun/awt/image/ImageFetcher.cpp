...
$ImageFetchable* ImageFetcher::nextImage() {
	...
	$synchronized($nc(info)->waitList) {
		...
		while (src == nullptr) {
			...
			$assign(src, $cast($ImageFetchable, info->waitList->elementAt(0)));
			info->waitList->removeElement(src);
		}
		return src;
	}
...
void ImageFetcher::run() {
	...
	try {
		...
		$synchronized($nc(info)->waitList) {
			...
			for (int32_t i = 0; i < $nc(info->fetchers)->length; ++i) {
				if (info->fetchers->get(i) == me) {
					info->fetchers->set(i, nullptr);
					--info->numFetchers;
				}
			}
		}
...
void ImageFetcher::stoppingAnimation($Thread* me) {
	...
	$synchronized($nc(info)->waitList) {
		...
		for (int32_t i = 0; i < $nc(info->fetchers)->length; ++i) {
			...
			if (info->fetchers->get(i) == nullptr) {
				index = i;
			}
		}
		if (index >= 0) {
			info->fetchers->set(index, me);
			++info->numFetchers;
...
void ImageFetcher::createFetchers($FetcherInfo* info) {
	...
	$AccessController::doPrivileged($$new($ImageFetcher$1, info, fetcherGroup));
	return;
}
...
$Class* ImageFetcher::load$($String* name, bool initialize) {
	...
	$loadClass(ImageFetcher, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ImageFetcher);
	});
	return class$;
}
