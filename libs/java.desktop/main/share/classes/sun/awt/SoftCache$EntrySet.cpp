...
bool SoftCache$EntrySet::remove(Object$* o) {
	...
	if ($instanceOf($SoftCache$Entry, o)) {
		return $nc(this->hashEntries)->remove($cast($SoftCache$Entry, o)->ent);
	} else {
		return false;
	}
}
...
$Class* SoftCache$EntrySet::load$($String* name, bool initialize) {
	...
	$loadClass(SoftCache$EntrySet, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
