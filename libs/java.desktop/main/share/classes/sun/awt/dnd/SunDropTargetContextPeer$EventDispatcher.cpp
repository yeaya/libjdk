...
void SunDropTargetContextPeer$EventDispatcher::init$($SunDropTargetContextPeer* peer, int32_t dropAction, int32_t actions, $longs* formats, int64_t nativeCtxt, bool dispatchType) {
	...
	$set(this, formats, (nullptr == formats) ? ($longs*)nullptr : $Arrays::copyOf(formats, formats->length));
...
void SunDropTargetContextPeer$EventDispatcher::dispatchEnterEvent($SunDropTargetEvent* e) {
	$synchronized(this->peer) {
		...
		$set(this->peer, currentT, this->formats);
...
void SunDropTargetContextPeer$EventDispatcher::dispatchMotionEvent($SunDropTargetEvent* e) {
	$synchronized(this->peer) {
		bool operationChanged = this->peer->previousDA != this->dropAction;
		this->peer->previousDA = this->dropAction;
		this->peer->nativeDragContext = this->nativeCtxt;
		$set(this->peer, currentT, this->formats);
		this->peer->currentSA = this->actions;
...
void SunDropTargetContextPeer$EventDispatcher::registerEvent($SunDropTargetEvent* e) {
	...
	bool var$0 = !this->eventSet->add(e);
	$init($SunDropTargetContextPeer);
	$init($PlatformLogger$Level);
	if (var$0 && $nc($SunDropTargetContextPeer::dndLog)->isLoggable($PlatformLogger$Level::FINE)) {
		$SunDropTargetContextPeer::dndLog->fine($$str({"Event is already registered: "_s, e}));
	}
	SunDropTargetContextPeer$EventDispatcher::handler->unlock();
}
	...
	try {
		...
		if (this->eventSet->isEmpty()) {
			if (!this->dispatcherDone && this->dispatchType == true) {
				SunDropTargetContextPeer$EventDispatcher::handler->exit();
			}
...
void SunDropTargetContextPeer$EventDispatcher::unregisterAllEvents() {
	...
	$var($Throwable, var$0, nullptr);
	try {
		$assign(events, this->eventSet->toArray());
	} catch ($Throwable& var$1) {
		$assign(var$0, var$1);
	} /*finally*/ {
		SunDropTargetContextPeer$EventDispatcher::handler->unlock();
...
void SunDropTargetContextPeer$EventDispatcher::clinit$($Class* clazz) {
...
$Class* SunDropTargetContextPeer$EventDispatcher::load$($String* name, bool initialize) {
	...
	$loadClass(SunDropTargetContextPeer$EventDispatcher, name, initialize, &classInfo$$, SunDropTargetContextPeer$EventDispatcher::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(SunDropTargetContextPeer$EventDispatcher);
	});
	return class$;
}
