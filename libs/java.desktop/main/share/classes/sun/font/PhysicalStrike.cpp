...
$Point2D$Float* PhysicalStrike::getGlyphPoint(int32_t glyphCode, int32_t ptNumber) {
	...
	if (this->glyphPointMapCache == nullptr) {
		...
		$assign(gp, $cast($Point2D$Float, this->glyphPointMapCache->get(ptKey)));
	}
	if (gp == nullptr) {
...
void PhysicalStrike::clinit$($Class* clazz) {
	{
		$init($StrikeCache);
...
$Class* PhysicalStrike::load$($String* name, bool initialize) {
	...
	$loadClass(PhysicalStrike, name, initialize, &classInfo$$, PhysicalStrike::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(PhysicalStrike);
	});
	return class$;
}
