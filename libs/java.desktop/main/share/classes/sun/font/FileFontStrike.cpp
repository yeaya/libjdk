...
bool FileFontStrike::initNative() {
	$init(FileFontStrike);
	$prepareNativeStatic(initNative, bool);
...
void FileFontStrike::init$($FileFont* fileFont, $FontStrikeDesc* desc) {
	...
	$var($AffineTransform, at, desc->glyphTx);
	$nc(at)->getMatrix(matrix);
	bool var$0 = !$nc(desc->devTx)->isIdentity();
	if (var$0 && desc->devTx->getType() != $AffineTransform::TYPE_TRANSLATION) {
		try {
	...
	if (var$1 || fileFont->getScaler() == nullptr) {
	...
	$set(this, mapper, fileFont->getMapper());
	...
	bool isSimpleTx = (at->getType() & $PhysicalStrike::complexTX) == 0;
	...
	if (var$5 && !$cast($TrueTypeFont, fileFont)->useEmbeddedBitmapsForSize(this->intPtSize)) {
		this->useNatives = true;
	}
	if ($FontUtilities::isLogging() && $FontUtilities::isWindows) {
		$var($StringBuilder, var$6, $new($StringBuilder));
		var$6->append("Strike for "_s);
		var$6->append(fileFont);
		var$6->append(" at size = "_s);
		var$6->append(this->intPtSize);
		var$6->append(" use natives = "_s);
		var$6->append(this->useNatives);
		var$6->append(" useJavaRasteriser = "_s);
		var$6->append(fileFont->useJavaRasterizer);
		var$6->append(" AAHint = "_s);
		var$6->append(desc->aaHint);
		var$6->append(" Has Embedded bitmaps = "_s);
		var$6->append($cast($TrueTypeFont, fileFont)->useEmbeddedBitmapsForSize(this->intPtSize));
		$FontUtilities::logInfo($$str(var$6));
	}
	...
	if (!this->getImageWithAdvance) {
		if (!this->segmentedCache) {
			...
			for (int32_t i = 0; i < numGlyphs; ++i) {
				this->horizontalAdvances->set(i, $Float::MAX_VALUE);
			}
		} else {
...
int64_t FileFontStrike::_getGlyphImageFromWindows($String* family, int32_t style, int32_t size, int32_t glyphCode, bool fracMetrics, int32_t fontDataSize) {
	$prepareNative(_getGlyphImageFromWindows, int64_t, $String* family, int32_t style, int32_t size, int32_t glyphCode, bool fracMetrics, int32_t fontDataSize);
	int64_t $ret = $invokeNative(family, style, size, glyphCode, fracMetrics, fontDataSize);
	$finishNative();
	return $ret;
}
...
int64_t FileFontStrike::getGlyphImageFromWindows(int32_t glyphCode) {
	...
	int32_t style = (($nc(this->desc)->style & $Font::BOLD) | ($nc(this->desc)->style & $Font::ITALIC)) | $nc(this->fileFont)->getStyle();
...
int64_t FileFontStrike::getGlyphImageFromX11(int32_t glyphCode) {
	...
	for (int32_t i = 0; i < $nc(this->nativeStrikes)->length; ++i) {
		...
		int32_t gc = $nc(mapper)->charToGlyph(charCode) & 0xffff;
...
int64_t FileFontStrike::getGlyphImagePtr(int32_t glyphCode) {
	...
	int64_t glyphPtr = 0;
	if ((glyphPtr = getCachedGlyphPtr(glyphCode)) != 0) {
		return glyphPtr;
	} else {
		if (this->useNatives) {
			glyphPtr = getGlyphImageFromNative(glyphCode);
			if (glyphPtr == 0 && $FontUtilities::isLogging()) {
...
void FileFontStrike::getGlyphImagePtrs($ints* glyphCodes, $longs* images, int32_t len) {
	for (int32_t i = 0; i < len; ++i) {
		...
		if (glyphCode >= FileFontStrike::INVISIBLE_GLYPHS) {
			...
			continue;
		} else if (($nc(images)->set(i, getCachedGlyphPtr(glyphCode))) != 0) {
			continue;
		} else {
			int64_t glyphPtr = 0;
			if (this->useNatives) {
...
int64_t FileFontStrike::getCachedGlyphPtrInternal(int32_t glyphCode) {
	...
	case FileFontStrike::INTARRAY:
		return $nc(this->intGlyphImages)->get(glyphCode) & $PhysicalStrike::INTMASK;
	case FileFontStrike::SEGINTARRAY:
		...
		if ($nc(this->segIntGlyphImages)->get(segIndex) != nullptr) {
			...
			return $nc(this->segIntGlyphImages->get(segIndex))->get(subIndex) & $PhysicalStrike::INTMASK;
		} else {
			return 0;
		}
	...
	case FileFontStrike::SEGLONGARRAY:
		...
		if ($nc(this->segLongGlyphImages)->get(segIndex) != nullptr) {
			int32_t subIndex = $mod(glyphCode, FileFontStrike::SEGSIZE);
			return $nc(this->segLongGlyphImages->get(segIndex))->get(subIndex);
		} else {
			return 0;
		}
	}
	...
	$useLocalObjectStack();
	{
		...
		case FileFontStrike::INTARRAY:
			if ($nc(this->intGlyphImages)->get(glyphCode) == 0) {
				this->intGlyphImages->set(glyphCode, (int32_t)glyphPtr);
				return glyphPtr;
			} else {
				$StrikeCache::freeIntPointer((int32_t)glyphPtr);
				return $nc(this->intGlyphImages)->get(glyphCode) & $PhysicalStrike::INTMASK;
		...
		case FileFontStrike::SEGINTARRAY:
			...
			if ($nc(this->segIntGlyphImages)->get(segIndex) == nullptr) {
				this->segIntGlyphImages->set(segIndex, $$new($ints, FileFontStrike::SEGSIZE));
			}
			if ($nc(this->segIntGlyphImages->get(segIndex))->get(subIndex) == 0) {
				$nc(this->segIntGlyphImages->get(segIndex))->set(subIndex, (int32_t)glyphPtr);
				return glyphPtr;
			} else {
				$StrikeCache::freeIntPointer((int32_t)glyphPtr);
				return $nc($nc(this->segIntGlyphImages)->get(segIndex))->get(subIndex) & $PhysicalStrike::INTMASK;
			}
		case FileFontStrike::LONGARRAY:
			if ($nc(this->longGlyphImages)->get(glyphCode) == 0) {
				this->longGlyphImages->set(glyphCode, glyphPtr);
				return glyphPtr;
			} else {
				$StrikeCache::freeLongPointer(glyphPtr);
				return $nc(this->longGlyphImages)->get(glyphCode);
		...
		case FileFontStrike::SEGLONGARRAY:
			...
			subIndex = $mod(glyphCode, FileFontStrike::SEGSIZE);
			if ($nc(this->segLongGlyphImages)->get(segIndex) == nullptr) {
				this->segLongGlyphImages->set(segIndex, $$new($longs, FileFontStrike::SEGSIZE));
			}
			if ($nc(this->segLongGlyphImages->get(segIndex))->get(subIndex) == 0) {
				$nc(this->segLongGlyphImages->get(segIndex))->set(subIndex, glyphPtr);
				return glyphPtr;
			} else {
				$StrikeCache::freeLongPointer(glyphPtr);
...
float FileFontStrike::getGlyphAdvance(int32_t glyphCode, bool getUserAdv) {
	...
	if (this->horizontalAdvances != nullptr) {
		advance = this->horizontalAdvances->get(glyphCode);
		...
		$var($floats, subArray, this->segHorizontalAdvances->get(segIndex));
		if (subArray != nullptr) {
	...
	if (this->horizontalAdvances != nullptr) {
		this->horizontalAdvances->set(glyphCode, advance);
	} else if (this->segmentedCache && this->segHorizontalAdvances != nullptr) {
		int32_t segIndex = $sr(glyphCode, FileFontStrike::SEGSHIFT);
		int32_t subIndex = $mod(glyphCode, FileFontStrike::SEGSIZE);
		if (this->segHorizontalAdvances->get(segIndex) == nullptr) {
			this->segHorizontalAdvances->set(segIndex, $$new($floats, FileFontStrike::SEGSIZE));
			for (int32_t i = 0; i < FileFontStrike::SEGSIZE; ++i) {
				$nc(this->segHorizontalAdvances->get(segIndex))->set(i, $Float::MAX_VALUE);
			}
		}
		$nc(this->segHorizontalAdvances->get(segIndex))->set(subIndex, advance);
	}
	return advance;
}
...
void FileFontStrike::getGlyphImageBounds(int32_t glyphCode, $Point2D$Float* pt, $Rectangle* result) {
	...
	if (ptr == 0) {
		...
		result->y = $cast(int32_t, $Math::floor(pt->y + 0.5f));
	...
	result->y = $cast(int32_t, $Math::floor(pt->y + topLeftY + 0.5f));
	result->width = $StrikeCache::unsafe->getShort(ptr + $StrikeCache::widthOffset) & 0xffff;
	result->height = $StrikeCache::unsafe->getShort(ptr + $StrikeCache::heightOffset) & 0xffff;
	if (($nc(this->desc)->aaHint == 4 || this->desc->aaHint == 5) && topLeftX <= -2.0f) {
...
int32_t FileFontStrike::getGlyphImageMinX(int64_t ptr, int32_t origMinX) {
	...
	int32_t height = $StrikeCache::unsafe->getChar(ptr + $StrikeCache::heightOffset);
	int32_t rowBytes = $StrikeCache::unsafe->getChar(ptr + $StrikeCache::rowBytesOffset);
	if (rowBytes == width) {
		return origMinX;
	}
	int64_t pixelData = $StrikeCache::unsafe->getAddress(ptr + $StrikeCache::pixelDataOffset);
	if (pixelData == 0) {
		return origMinX;
	}
...
$Point2D$Float* FileFontStrike::getGlyphMetrics(int32_t glyphCode, bool getImage) {
	...
	if (glyphPtr != 0) {
		...
		if (this->glyphMetricsMapRef != nullptr) {
			$assign(glyphMetricsMap, $cast($ConcurrentHashMap, this->glyphMetricsMapRef->get()));
		}
...
$GeneralPath* FileFontStrike::getGlyphOutline(int32_t glyphCode, float x, float y) {
	...
	if (this->outlineMapRef != nullptr) {
		$assign(outlineMap, $cast($ConcurrentHashMap, this->outlineMapRef->get()));
		if (outlineMap != nullptr) {
			$assign(gp, $cast($GeneralPath, outlineMap->get($($Integer::valueOf(glyphCode)))));
		}
	}
	if (gp == nullptr) {
		$assign(gp, $nc(this->fileFont)->getGlyphOutline(this->pScalerContext, glyphCode, 0, 0));
		if (outlineMap == nullptr) {
	...
	if (x != 0.0f || y != 0.0f) {
		$nc(gp)->transform($($AffineTransform::getTranslateInstance(x, y)));
	}
	return gp;
}
...
void FileFontStrike::clinit$($Class* clazz) {
...
$Class* FileFontStrike::load$($String* name, bool initialize) {
	...
	$loadClass(FileFontStrike, name, initialize, &classInfo$$, FileFontStrike::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(FileFontStrike);
	});
	return class$;
}
