...
void GlyphLayout$SDCache::init$($Font* font, $FontRenderContext* frc) {
	...
	double var$1 = this->dtx->getShearY();
	double var$2 = this->dtx->getShearX();
	$nc(this->dtx)->setTransform(var$0, var$1, var$2, this->dtx->getScaleY(), 0, 0);
	...
	if (font->isTransformed()) {
		...
		$set(this, delta, $new($Point2D$Float, var$3, (float)this->gtx->getTranslateY()));
		double var$4 = $nc(this->gtx)->getScaleX();
		double var$5 = this->gtx->getShearY();
		double var$6 = this->gtx->getShearX();
		$nc(this->gtx)->setTransform(var$4, var$5, var$6, this->gtx->getScaleY(), 0, 0);
...
GlyphLayout$SDCache* GlyphLayout$SDCache::get($Font* font, $FontRenderContext* frc$renamed) {
	...
	if ($nc(frc)->isTransformed()) {
		...
		if (var$0 || transform->getTranslateY() != 0) {
			double var$1 = transform->getScaleX();
			double var$2 = transform->getShearY();
			double var$3 = transform->getShearX();
			$assign(transform, $new($AffineTransform, var$1, var$2, var$3, transform->getScaleY(), 0, 0));
			$var($Object, var$4, frc->getAntiAliasingHint());
			$assign(frc, $new($FontRenderContext, transform, var$4, $(frc->getFractionalMetricsHint())));
		}
	}
	...
	if (GlyphLayout$SDCache::cacheRef != nullptr) {
		$assign(cache, $cast($ConcurrentHashMap, GlyphLayout$SDCache::cacheRef->get()));
		if (cache != nullptr) {
...
void GlyphLayout$SDCache::clinit$($Class* clazz) {
...
$Class* GlyphLayout$SDCache::load$($String* name, bool initialize) {
	...
	$loadClass(GlyphLayout$SDCache, name, initialize, &classInfo$$, GlyphLayout$SDCache::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(GlyphLayout$SDCache);
	});
	return class$;
}
