...
$Class* FreetypeFontScaler$$Lambda$lambda$disposeScaler$0::load$($String* name, bool initialize) {
	...
	$loadClass(FreetypeFontScaler$$Lambda$lambda$disposeScaler$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FreetypeFontScaler$$Lambda$lambda$disposeScaler$0);
	});
	return class$;
}
...
void FreetypeFontScaler::initIDs($Class* FFS) {
	...
	$prepareNativeStatic(initIDs, void, $Class* FFS);
...
void FreetypeFontScaler::disposeScaler() {
	$synchronized(this) {
		$useLocalObjectStack();
		if (this->nativeScaler != 0) {
			$$new($Thread, nullptr, $$new(FreetypeFontScaler$$Lambda$lambda$disposeScaler$0, this), "free scaler"_s, 0, false)->start();
		}
	}
}
...
int64_t FreetypeFontScaler::initNativeScaler($Font2D* font, int32_t type, int32_t indexInCollection, bool supportsCJK, int32_t filesize) {
	$prepareNative(initNativeScaler, int64_t, $Font2D* font, int32_t type, int32_t indexInCollection, bool supportsCJK, int32_t filesize);
	int64_t $ret = $invokeNative(font, type, indexInCollection, supportsCJK, filesize);
	$finishNative();
	return $ret;
}

$StrikeMetrics* FreetypeFontScaler::getFontMetricsNative($Font2D* font, int64_t pScalerContext, int64_t pScaler) {
	$prepareNative(getFontMetricsNative, $StrikeMetrics*, $Font2D* font, int64_t pScalerContext, int64_t pScaler);
	$var($StrikeMetrics, $ret, $invokeNativeObject(font, pScalerContext, pScaler));
	$finishNative();
	return $ret;
}

float FreetypeFontScaler::getGlyphAdvanceNative($Font2D* font, int64_t pScalerContext, int64_t pScaler, int32_t glyphCode) {
	$prepareNative(getGlyphAdvanceNative, float, $Font2D* font, int64_t pScalerContext, int64_t pScaler, int32_t glyphCode);
	float $ret = $invokeNative(font, pScalerContext, pScaler, glyphCode);
	$finishNative();
	return $ret;
}
...
void FreetypeFontScaler::getGlyphMetricsNative($Font2D* font, int64_t pScalerContext, int64_t pScaler, int32_t glyphCode, $Point2D$Float* metrics) {
	$prepareNative(getGlyphMetricsNative, void, $Font2D* font, int64_t pScalerContext, int64_t pScaler, int32_t glyphCode, $Point2D$Float* metrics);
...
int64_t FreetypeFontScaler::getGlyphImageNative($Font2D* font, int64_t pScalerContext, int64_t pScaler, int32_t glyphCode) {
	$prepareNative(getGlyphImageNative, int64_t, $Font2D* font, int64_t pScalerContext, int64_t pScaler, int32_t glyphCode);
	int64_t $ret = $invokeNative(font, pScalerContext, pScaler, glyphCode);
	$finishNative();
	return $ret;
}
...
$Rectangle2D$Float* FreetypeFontScaler::getGlyphOutlineBoundsNative($Font2D* font, int64_t pScalerContext, int64_t pScaler, int32_t glyphCode) {
	$prepareNative(getGlyphOutlineBoundsNative, $Rectangle2D$Float*, $Font2D* font, int64_t pScalerContext, int64_t pScaler, int32_t glyphCode);
	$var($Rectangle2D$Float, $ret, $invokeNativeObject(font, pScalerContext, pScaler, glyphCode));
	$finishNative();
	return $ret;
}
...
$GeneralPath* FreetypeFontScaler::getGlyphOutlineNative($Font2D* font, int64_t pScalerContext, int64_t pScaler, int32_t glyphCode, float x, float y) {
	$prepareNative(getGlyphOutlineNative, $GeneralPath*, $Font2D* font, int64_t pScalerContext, int64_t pScaler, int32_t glyphCode, float x, float y);
	$var($GeneralPath, $ret, $invokeNativeObject(font, pScalerContext, pScaler, glyphCode, x, y));
	$finishNative();
	return $ret;
}
...
$GeneralPath* FreetypeFontScaler::getGlyphVectorOutlineNative($Font2D* font, int64_t pScalerContext, int64_t pScaler, $ints* glyphs, int32_t numGlyphs, float x, float y) {
	$prepareNative(getGlyphVectorOutlineNative, $GeneralPath*, $Font2D* font, int64_t pScalerContext, int64_t pScaler, $ints* glyphs, int32_t numGlyphs, float x, float y);
	$var($GeneralPath, $ret, $invokeNativeObject(font, pScalerContext, pScaler, glyphs, numGlyphs, x, y));
	$finishNative();
	return $ret;
}
...
$Point2D$Float* FreetypeFontScaler::getGlyphPointNative($Font2D* font, int64_t pScalerContext, int64_t pScaler, int32_t glyphCode, int32_t ptNumber) {
	$prepareNative(getGlyphPointNative, $Point2D$Float*, $Font2D* font, int64_t pScalerContext, int64_t pScaler, int32_t glyphCode, int32_t ptNumber);
	$var($Point2D$Float, $ret, $invokeNativeObject(font, pScalerContext, pScaler, glyphCode, ptNumber));
	$finishNative();
	return $ret;
}

void FreetypeFontScaler::disposeNativeScaler($Font2D* font2D, int64_t pScaler) {
	$prepareNative(disposeNativeScaler, void, $Font2D* font2D, int64_t pScaler);
	$invokeNative(font2D, pScaler);
	$finishNative();
}

int32_t FreetypeFontScaler::getGlyphCodeNative($Font2D* font, int64_t pScaler, char16_t charCode) {
	$prepareNative(getGlyphCodeNative, int32_t, $Font2D* font, int64_t pScaler, char16_t charCode);
	int32_t $ret = $invokeNative(font, pScaler, charCode);
	$finishNative();
	return $ret;
}

int32_t FreetypeFontScaler::getNumGlyphsNative(int64_t pScaler) {
	$prepareNative(getNumGlyphsNative, int32_t, int64_t pScaler);
	int32_t $ret = $invokeNative(pScaler);
	$finishNative();
	return $ret;
}

int32_t FreetypeFontScaler::getMissingGlyphCodeNative(int64_t pScaler) {
	$prepareNative(getMissingGlyphCodeNative, int32_t, int64_t pScaler);
	int32_t $ret = $invokeNative(pScaler);
	$finishNative();
	return $ret;
}
...
int64_t FreetypeFontScaler::getUnitsPerEMNative(int64_t pScaler) {
	$prepareNative(getUnitsPerEMNative, int64_t, int64_t pScaler);
	int64_t $ret = $invokeNative(pScaler);
	$finishNative();
	return $ret;
}
...
int64_t FreetypeFontScaler::createScalerContextNative(int64_t pScaler, $doubles* matrix, int32_t aa, int32_t fm, float boldness, float italic) {
	$prepareNative(createScalerContextNative, int64_t, int64_t pScaler, $doubles* matrix, int32_t aa, int32_t fm, float boldness, float italic);
	int64_t $ret = $invokeNative(pScaler, matrix, aa, fm, boldness, italic);
	$finishNative();
	return $ret;
}
...
void FreetypeFontScaler::clinit$($Class* clazz) {
	{
...
$Class* FreetypeFontScaler::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("sun.font.FreetypeFontScaler$$Lambda$lambda$disposeScaler$0")) {
	...
	$loadClass(FreetypeFontScaler, name, initialize, &classInfo$$, FreetypeFontScaler::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(FreetypeFontScaler);
	});
	return class$;
}
