...
void FilePane$1::keyTyped($KeyEvent* e) {
	...
	if (time - this->lastTime < this->timeFactor) {
		bool var$2 = this->typedString->length() == 1;
		if (var$2 && this->typedString->charAt(0) == c) {
			++startIndex;
		} else {
...
int32_t FilePane$1::getNextMatch(int32_t startIndex, int32_t finishIndex) {
	...
	$var($String, prefix, $(this->typedString->toString())->toLowerCase());
...
$Class* FilePane$1::load$($String* name, bool initialize) {
	...
	$loadClass(FilePane$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FilePane$1);
	});
	return class$;
}
