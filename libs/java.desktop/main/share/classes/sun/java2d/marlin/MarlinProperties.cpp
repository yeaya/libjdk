...
bool MarlinProperties::getBoolean($String* key, $String* def) {
	...
	return $($Boolean::valueOf($$cast($String, $AccessController::doPrivileged($$new($GetPropertyAction, key, def)))))->booleanValue();
}
...
int32_t MarlinProperties::getInteger($String* key, int32_t def, int32_t min, int32_t max) {
	...
	$load(MarlinProperties);
	$beforeCallerSensitive();
	$var($String, property, $cast($String, $AccessController::doPrivileged($$new($GetPropertyAction, key))));
	int32_t value = def;
	if (property != nullptr) {
		try {
			value = $($Integer::decode(property))->intValue();
...
$Class* MarlinProperties::load$($String* name, bool initialize) {
	...
	$loadClass(MarlinProperties, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MarlinProperties);
	});
	return class$;
}
