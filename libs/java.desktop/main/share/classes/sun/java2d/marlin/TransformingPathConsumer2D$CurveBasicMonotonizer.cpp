...
$Class* TransformingPathConsumer2D$CurveBasicMonotonizer::load$($String* name, bool initialize) {
	...
	$loadClass(TransformingPathConsumer2D$CurveBasicMonotonizer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TransformingPathConsumer2D$CurveBasicMonotonizer);
	});
	return class$;
}
