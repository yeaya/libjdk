...
$Class* DMarlinRenderingEngine$NormalizingPathIterator$NearestPixelQuarter::load$($String* name, bool initialize) {
	...
	$loadClass(DMarlinRenderingEngine$NormalizingPathIterator$NearestPixelQuarter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DMarlinRenderingEngine$NormalizingPathIterator$NearestPixelQuarter);
	});
	return class$;
}
