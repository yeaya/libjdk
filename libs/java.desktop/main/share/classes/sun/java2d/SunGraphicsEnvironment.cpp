...
$FontArray* SunGraphicsEnvironment::getAllFonts() {
	...
	if (created == nullptr || created->length == 0) {
		return installedFonts;
	} else {
		int32_t newlen = $nc(installedFonts)->length + created->length;
		$var($FontArray, fonts, $cast($FontArray, $Arrays::copyOf(installedFonts, newlen)));
	...
	$useLocalObjectStack();
	{
		...
		for (int32_t len$ = $nc(arr$)->length, i$ = 0; i$ < len$; ++i$) {
			...
			if ($instanceOf($DisplayChangedListener, gd)) {
				$cast($DisplayChangedListener, gd)->displayChanged();
			}
		}
	}
...
double SunGraphicsEnvironment::getScaleFactor($String* propertyName) {
	...
	$var($String, scaleFactor, $cast($String, $AccessController::doPrivileged($$new($GetPropertyAction, propertyName, "-1"_s))));
	if (scaleFactor == nullptr || scaleFactor->equals("-1"_s)) {
		return -1;
	}
	try {
		double units = 1.0;
...
$Dimension* SunGraphicsEnvironment::toUserSpace($GraphicsConfiguration* gc, int32_t w, int32_t h) {
	...
	return $new($Dimension, var$0, $Region::clipRound(h / tx->getScaleY()));
}
...
$Rectangle* SunGraphicsEnvironment::toDeviceSpaceAbs($Rectangle* rect) {
	...
	$useLocalObjectStack();
	$var($GraphicsConfiguration, gc, $$nc($$nc(getLocalGraphicsEnvironment())->getDefaultScreenDevice())->getDefaultConfiguration());
	$assign(gc, getGraphicsConfigurationAtPoint(gc, (double)$nc(rect)->x, (double)$nc(rect)->y));
...
$Rectangle* SunGraphicsEnvironment::toDeviceSpaceAbs($GraphicsConfiguration* gc, int32_t x, int32_t y, int32_t w, int32_t h) {
	...
	int32_t var$0 = $nc(screen)->x + $Region::clipRound((x - $nc(screen)->x) * $nc(tx)->getScaleX());
	int32_t var$1 = screen->y + $Region::clipRound((y - screen->y) * tx->getScaleY());
	int32_t var$2 = $Region::clipRound(w * tx->getScaleX());
	return $new($Rectangle, var$0, var$1, var$2, $Region::clipRound(h * tx->getScaleY()));
}
...
$Rectangle* SunGraphicsEnvironment::toDeviceSpace($GraphicsConfiguration* gc, int32_t x, int32_t y, int32_t w, int32_t h) {
	...
	int32_t var$1 = $Region::clipRound(y * tx->getScaleY());
	int32_t var$2 = $Region::clipRound(w * tx->getScaleX());
	return $new($Rectangle, var$0, var$1, var$2, $Region::clipRound(h * tx->getScaleY()));
}

void SunGraphicsEnvironment::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$beforeCallerSensitive();
	{
		SunGraphicsEnvironment::uiScaleEnabled = "true"_s->equals($($AccessController::doPrivileged($$new($GetPropertyAction, "sun.java2d.uiScale.enabled"_s, "true"_s))));
		SunGraphicsEnvironment::debugScale = SunGraphicsEnvironment::uiScaleEnabled ? SunGraphicsEnvironment::getScaleFactor("sun.java2d.uiScale"_s) : -1;
	}
}
...
$Class* SunGraphicsEnvironment::load$($String* name, bool initialize) {
	...
	$CompoundAttribute useAlternateFontforJALocalesmethodAnnotations$$[] = {
	...
	$MethodInfo methodInfos$$[] = {
		...
		{"useAlternateFontforJALocales", "()V", nullptr, $PUBLIC | $STATIC | $DEPRECATED, $staticMethod(SunGraphicsEnvironment, useAlternateFontforJALocales, void), nullptr, nullptr, useAlternateFontforJALocalesmethodAnnotations$$},
		{}
	};
	$ClassInfo classInfo$$ = {
	...
	$loadClass(SunGraphicsEnvironment, name, initialize, &classInfo$$, SunGraphicsEnvironment::clinit$, []($Class* clazz) -> $Object* {
