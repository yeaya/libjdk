...
$String* OGLContext$OGLContextCaps::toString() {
	...
	if ((this->caps & OGLContext$OGLContextCaps::CAPS_EXT_FBOBJECT) != 0) {
		sb->append("CAPS_EXT_FBOBJECT|"_s);
	}
	if ((this->caps & OGLContext$OGLContextCaps::CAPS_DOUBLEBUFFERED) != 0) {
		sb->append("CAPS_DOUBLEBUFFERED|"_s);
	}
	if ((this->caps & OGLContext$OGLContextCaps::CAPS_EXT_LCD_SHADER) != 0) {
		sb->append("CAPS_EXT_LCD_SHADER|"_s);
	}
	if ((this->caps & OGLContext$OGLContextCaps::CAPS_EXT_BIOP_SHADER) != 0) {
		sb->append("CAPS_BIOP_SHADER|"_s);
	}
	if ((this->caps & OGLContext$OGLContextCaps::CAPS_EXT_GRAD_SHADER) != 0) {
		sb->append("CAPS_EXT_GRAD_SHADER|"_s);
	}
	if ((this->caps & OGLContext$OGLContextCaps::CAPS_EXT_TEXRECT) != 0) {
		sb->append("CAPS_EXT_TEXRECT|"_s);
	}
	if ((this->caps & OGLContext$OGLContextCaps::CAPS_EXT_TEXBARRIER) != 0) {
...
$Class* OGLContext$OGLContextCaps::load$($String* name, bool initialize) {
	...
	$loadClass(OGLContext$OGLContextCaps, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(OGLContext$OGLContextCaps);
	});
	return class$;
}
