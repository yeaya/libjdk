...
void OGLRenderer::validateContext($SunGraphics2D* sg2d) {
	...
	try {
		$assign(dstData, $cast($OGLSurfaceData, sg2d->surfaceData));
	} catch ($ClassCastException& e) {
		$throwNew($InvalidPipeException, $$str({"wrong surface data type: "_s, sg2d->surfaceData}));
	}
	$OGLContext::validateContext(dstData, dstData, $(sg2d->getCompClip()), sg2d->composite, nullptr, sg2d->paint, sg2d, ctxflags);
}
...
void OGLRenderer::validateContextAA($SunGraphics2D* sg2d) {
	...
	$OGLContext::validateContext(dstData, dstData, $($nc(sg2d)->getCompClip()), $nc(sg2d)->composite, nullptr, $nc(sg2d)->paint, sg2d, ctxflags);
}
...
void OGLRenderer::copyArea($SunGraphics2D* sg2d, int32_t x, int32_t y, int32_t w, int32_t h, int32_t dx, int32_t dy) {
	...
	try {
		...
		$OGLContext::validateContext(dstData, dstData, $(sg2d->getCompClip()), sg2d->composite, nullptr, nullptr, nullptr, ctxflags);
...
void OGLRenderer::drawPoly($ints* xPoints, $ints* yPoints, int32_t nPoints, bool isClosed, int32_t transX, int32_t transY) {
	$prepareNative(drawPoly, void, $ints* xPoints, $ints* yPoints, int32_t nPoints, bool isClosed, int32_t transX, int32_t transY);
...
$Class* OGLRenderer::load$($String* name, bool initialize) {
	...
	$loadClass(OGLRenderer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
