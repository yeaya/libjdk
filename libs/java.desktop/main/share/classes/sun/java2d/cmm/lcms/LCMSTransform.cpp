...
void LCMSTransform::init$($ICC_Profile* profile, int32_t renderType, int32_t transformType) {
	...
	this->profiles->set(0, profile);
	$set(this, lcmsProfiles, $new($LCMSProfileArray, 1));
	this->lcmsProfiles->set(0, $($LCMS::getProfileID(profile)));
...
void LCMSTransform::init$($ColorTransformArray* transforms) {
	...
	int32_t j = 0;
	for (int32_t i = 0; i < transforms->length; ++i) {
		$var(LCMSTransform, curTrans, $cast(LCMSTransform, transforms->get(i)));
		$System::arraycopy($nc(curTrans)->profiles, 0, this->profiles, j, $nc($nc(curTrans)->profiles)->length);
		$System::arraycopy(curTrans->lcmsProfiles, 0, this->lcmsProfiles, j, $nc(curTrans->lcmsProfiles)->length);
		j += curTrans->profiles->length;
	}
	this->renderType = $nc($cast(LCMSTransform, transforms->get(0)))->renderType;
	this->numInComponents = $nc(this->profiles->get(0))->getNumComponents();
	this->numOutComponents = $nc($nc(this->profiles)->get($nc(this->profiles)->length - 1))->getNumComponents();
}
...
void LCMSTransform::doTransform($LCMSImageLayout* in, $LCMSImageLayout* out) {
	$synchronized(this) {
		if (this->ID == 0 || this->inFormatter != $nc(in)->pixelType || this->isInIntPacked != $nc(in)->isIntPacked || this->outFormatter != $nc(out)->pixelType || this->isOutIntPacked != $nc(out)->isIntPacked) {
			if (this->ID != 0) {
				$set(this, disposerReferent, $new($Object));
			}
			this->inFormatter = $nc(in)->pixelType;
			this->isInIntPacked = in->isIntPacked;
			this->outFormatter = $nc(out)->pixelType;
...
bool LCMSTransform::isLCMSSupport($BufferedImage* src, $BufferedImage* dst) {
	...
	if (var$0 || dst->isAlphaPremultiplied()) {
		return false;
	}
	bool var$1 = $$nc(dst->getColorModel())->hasAlpha();
	return var$1 == $$nc(src->getColorModel())->hasAlpha();
}
...
void LCMSTransform::colorConvert($BufferedImage* src, $BufferedImage* dst) {
	...
	if (precision == 8) {
		...
		int32_t idx = 0;
		try {
			int32_t var$0 = $div(srcLine->length, getNumInComponents());
			int32_t var$2 = $LCMSImageLayout::CHANNELS_SH(getNumInComponents());
			int32_t var$1 = var$2 | $LCMSImageLayout::BYTES_SH(1);
			$assign(srcIL, $new($LCMSImageLayout, srcLine, var$0, var$1, getNumInComponents()));
			int32_t var$3 = $div(dstLine->length, getNumOutComponents());
			int32_t var$5 = $LCMSImageLayout::CHANNELS_SH(getNumOutComponents());
			int32_t var$4 = var$5 | $LCMSImageLayout::BYTES_SH(1);
			$assign(dstIL, $new($LCMSImageLayout, dstLine, var$3, var$4, getNumOutComponents()));
		} catch ($LCMSImageLayout$ImageLayoutException& e) {
			$throwNew($CMMException, "Unable to convert images"_s);
		}
		for (int32_t y = 0; y < h; ++y) {
			...
			for (int32_t x = 0; x < w; ++x) {
				for (int32_t i = 0; i < dstNumComp; ++i) {
					$nc(dstColor)->set(i, ((float)(dstLine->get(idx++) & 0xff)) * dstDiffMinMax->get(i) + dstMinVal->get(i));
				}
				if (needSrcAlpha) {
		...
		int32_t idx = 0;
		try {
			int32_t var$6 = $div(srcLine->length, getNumInComponents());
			int32_t var$8 = $LCMSImageLayout::CHANNELS_SH(getNumInComponents());
			int32_t var$7 = var$8 | $LCMSImageLayout::BYTES_SH(2);
			$assign(srcIL, $new($LCMSImageLayout, srcLine, var$6, var$7, getNumInComponents() * 2));
			int32_t var$9 = $div(dstLine->length, getNumOutComponents());
			int32_t var$11 = $LCMSImageLayout::CHANNELS_SH(getNumOutComponents());
			int32_t var$10 = var$11 | $LCMSImageLayout::BYTES_SH(2);
			$assign(dstIL, $new($LCMSImageLayout, dstLine, var$9, var$10, getNumOutComponents() * 2));
		} catch ($LCMSImageLayout$ImageLayoutException& e) {
			$throwNew($CMMException, "Unable to convert images"_s);
		}
		for (int32_t y = 0; y < h; ++y) {
			...
			for (int32_t x = 0; x < w; ++x) {
				for (int32_t i = 0; i < dstNumComp; ++i) {
					$nc(dstColor)->set(i, ((float)(dstLine->get(idx++) & 0xffff)) * dstDiffMinMax->get(i) + dstMinVal->get(i));
				}
				if (needSrcAlpha) {
					$nc(dstColor)->set(dstNumComp, $nc(alpha)->get(x));
				} else if (dstHasAlpha) {
...
void LCMSTransform::colorConvert($Raster* src, $WritableRaster* dst, $floats* srcMinVal, $floats* srcMaxVal, $floats* dstMinVal, $floats* dstMaxVal) {
	...
	try {
		int32_t var$0 = $div(srcLine->length, getNumInComponents());
		int32_t var$2 = $LCMSImageLayout::CHANNELS_SH(getNumInComponents());
		int32_t var$1 = var$2 | $LCMSImageLayout::BYTES_SH(2);
		$assign(srcIL, $new($LCMSImageLayout, srcLine, var$0, var$1, getNumInComponents() * 2));
		int32_t var$3 = $div(dstLine->length, getNumOutComponents());
		int32_t var$5 = $LCMSImageLayout::CHANNELS_SH(getNumOutComponents());
		int32_t var$4 = var$5 | $LCMSImageLayout::BYTES_SH(2);
		$assign(dstIL, $new($LCMSImageLayout, dstLine, var$3, var$4, getNumOutComponents() * 2));
	} catch ($LCMSImageLayout$ImageLayoutException& e) {
	...
	for (int32_t y = 0; y < h; ++y, ++ys, ++yd) {
		...
		for (int32_t x = 0; x < w; ++x, ++xd) {
			for (int32_t i = 0; i < dstNumBands; ++i) {
				sample = ((dstLine->get(idx++) & 0xffff) * dstScaleFactor->get(i)) + dstUseMinVal->get(i);
...
void LCMSTransform::colorConvert($Raster* src, $WritableRaster* dst) {
	...
	if (precision == 8) {
		...
		for (int32_t y = 0; y < h; ++y, ++ys, ++yd) {
			...
			for (int32_t x = 0; x < w; ++x, ++xd) {
				for (int32_t i = 0; i < dstNumBands; ++i) {
					sample = $cast(int32_t, (((dstLine->get(idx++) & 0xff) * dstScaleFactor->get(i)) + 0.5f));
					dst->setSample(xd, yd, i, sample);
				}
			}
		}
		...
		for (int32_t y = 0; y < h; ++y, ++ys, ++yd) {
			...
			for (int32_t x = 0; x < w; ++x, ++xd) {
				for (int32_t i = 0; i < dstNumBands; ++i) {
					sample = $cast(int32_t, (((dstLine->get(idx++) & 0xffff) * dstScaleFactor->get(i)) + 0.5f));
					dst->setSample(xd, yd, i, sample);
				}
			}
		}
...
$shorts* LCMSTransform::colorConvert($shorts* src, $shorts* dst$renamed) {
	...
	try {
		int32_t var$1 = $div($nc(src)->length, getNumInComponents());
		int32_t var$3 = $LCMSImageLayout::CHANNELS_SH(getNumInComponents());
		int32_t var$2 = var$3 | $LCMSImageLayout::BYTES_SH(2);
		$var($LCMSImageLayout, srcIL, $new($LCMSImageLayout, src, var$1, var$2, getNumInComponents() * 2));
		int32_t var$4 = $div($nc(dst)->length, getNumOutComponents());
		int32_t var$6 = $LCMSImageLayout::CHANNELS_SH(getNumOutComponents());
		int32_t var$5 = var$6 | $LCMSImageLayout::BYTES_SH(2);
		$var($LCMSImageLayout, dstIL, $new($LCMSImageLayout, dst, var$4, var$5, getNumOutComponents() * 2));
		doTransform(srcIL, dstIL);
		return dst;
	} catch ($LCMSImageLayout$ImageLayoutException& e) {
...
$bytes* LCMSTransform::colorConvert($bytes* src, $bytes* dst$renamed) {
	...
	try {
		...
		$var($LCMSImageLayout, srcIL, $new($LCMSImageLayout, src, var$1, var$2, getNumInComponents()));
		int32_t var$4 = $div($nc(dst)->length, getNumOutComponents());
		int32_t var$6 = $LCMSImageLayout::CHANNELS_SH(getNumOutComponents());
		int32_t var$5 = var$6 | $LCMSImageLayout::BYTES_SH(1);
		$var($LCMSImageLayout, dstIL, $new($LCMSImageLayout, dst, var$4, var$5, getNumOutComponents()));
		doTransform(srcIL, dstIL);
		return dst;
	} catch ($LCMSImageLayout$ImageLayoutException& e) {
		$throwNew($CMMException, "Unable to convert data"_s);
...
$Class* LCMSTransform::load$($String* name, bool initialize) {
	...
	$loadClass(LCMSTransform, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LCMSTransform);
	});
	return class$;
}
