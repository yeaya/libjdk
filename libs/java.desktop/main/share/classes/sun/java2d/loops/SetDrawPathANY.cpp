...
void SetDrawPathANY::DrawPath$($SunGraphics2D* sg2d, $SurfaceData* sData, int32_t transx, int32_t transy, $Path2D$Float* p2df) {
	...
	$ProcessPath::drawPath($$new($PixelWriterDrawHandler, sData, pw, $($nc(sg2d)->getCompClip()), $nc(sg2d)->strokeHint), p2df, transx, transy);
}
...
$Class* SetDrawPathANY::load$($String* name, bool initialize) {
	...
	$loadClass(SetDrawPathANY, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SetDrawPathANY);
	});
	return class$;
}
