...
void ProcessPath$FillProcessHandler::processFixedLine(int32_t x1, int32_t y1, int32_t x2, int32_t y2, $ints* pixelInfo, bool checkBounds, bool endSubPath) {
	...
	if (checkBounds) {
		...
		outXMax = $cast(int32_t, (this->dhnd->xMaxf * 1024));
		outYMin = $cast(int32_t, (this->dhnd->yMinf * 1024));
		outYMax = $cast(int32_t, (this->dhnd->yMaxf * 1024));
...
$Class* ProcessPath$FillProcessHandler::load$($String* name, bool initialize) {
	...
	$loadClass(ProcessPath$FillProcessHandler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ProcessPath$FillProcessHandler);
	});
	return class$;
}
