...
void BlitBg::BlitBg$($SurfaceData* src, $SurfaceData* dst, $Composite* comp, $Region* clip, int32_t bgColor, int32_t srcx, int32_t srcy, int32_t dstx, int32_t dsty, int32_t width, int32_t height) {
	$prepareNative(BlitBg, void, $SurfaceData* src, $SurfaceData* dst, $Composite* comp, $Region* clip, int32_t bgColor, int32_t srcx, int32_t srcy, int32_t dstx, int32_t dsty, int32_t width, int32_t height);
...
void BlitBg::clinit$($Class* clazz) {
...
$Class* BlitBg::load$($String* name, bool initialize) {
	...
	$loadClass(BlitBg, name, initialize, &classInfo$$, BlitBg::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(BlitBg);
	});
	return class$;
}
