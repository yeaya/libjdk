...
void DefaultDisposerRecord::invokeNativeDispose(int64_t disposerMethodPointer, int64_t dataPointer) {
	...
	$prepareNativeStatic(invokeNativeDispose, void, int64_t disposerMethodPointer, int64_t dataPointer);
...
$Class* DefaultDisposerRecord::load$($String* name, bool initialize) {
	...
	$loadClass(DefaultDisposerRecord, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DefaultDisposerRecord);
	});
	return class$;
}
