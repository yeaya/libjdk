...
RenderingEngine* RenderingEngine::getInstance() {
	...
	$synchronized(class$) {
		...
		$var($String, reClass, $cast($String, $AccessController::doPrivileged(gpa)));
		if (reClass != nullptr) {
			try {
				$Class* cls = $Class::forName(reClass);
				$assignStatic(RenderingEngine::reImpl, $cast(RenderingEngine, $$nc(cls->getConstructor($$new($ClassArray, 0)))->newInstance($$new($ObjectArray, 0))));
		...
		$var($String, verbose, $cast($String, $AccessController::doPrivileged(gpa)));
		...
		$var($String, reTrace, $cast($String, $AccessController::doPrivileged(gpa)));
		if (reTrace != nullptr) {
...
$Class* RenderingEngine::load$($String* name, bool initialize) {
	...
	$loadClass(RenderingEngine, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(RenderingEngine);
	});
	return class$;
}
