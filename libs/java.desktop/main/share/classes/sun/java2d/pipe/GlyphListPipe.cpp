...
void GlyphListPipe::drawString($SunGraphics2D* sg2d, $String* s, double x, double y) {
	...
	if ($nc(gl)->setFromString(info, s, devx, devy)) {
		...
		gl->dispose();
		$var($Font, var$0, sg2d->getFont());
		$var($TextLayout, tl, $new($TextLayout, s, var$0, $(sg2d->getFontRenderContext())));
...
$Class* GlyphListPipe::load$($String* name, bool initialize) {
	...
	$loadClass(GlyphListPipe, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(GlyphListPipe);
	});
	return class$;
}
