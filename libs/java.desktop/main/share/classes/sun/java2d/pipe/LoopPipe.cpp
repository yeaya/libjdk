...
void LoopPipe::drawRect($SunGraphics2D* sg2d, int32_t x, int32_t y, int32_t width, int32_t height) {
	$nc($nc($nc(sg2d)->loops)->drawRectLoop)->DrawRect$(sg2d, $($nc(sg2d)->getSurfaceData()), x + $nc(sg2d)->transX, y + $nc(sg2d)->transY, width, height);
}
...
void LoopPipe::drawPolyline($SunGraphics2D* sg2d, $ints* xPoints, $ints* yPoints, int32_t nPoints) {
	...
	$nc($nc($nc(sg2d)->loops)->drawPolygonsLoop)->DrawPolygons$(sg2d, $($nc(sg2d)->getSurfaceData()), xPoints, yPoints, nPointsArray, 1, $nc(sg2d)->transX, $nc(sg2d)->transY, false);
}

void LoopPipe::drawPolygon($SunGraphics2D* sg2d, $ints* xPoints, $ints* yPoints, int32_t nPoints) {
	$useLocalObjectStack();
	$var($ints, nPointsArray, $new($ints, {nPoints}));
	$nc($nc($nc(sg2d)->loops)->drawPolygonsLoop)->DrawPolygons$(sg2d, $($nc(sg2d)->getSurfaceData()), xPoints, yPoints, nPointsArray, 1, $nc(sg2d)->transX, $nc(sg2d)->transY, true);
}

void LoopPipe::fillRect($SunGraphics2D* sg2d, int32_t x, int32_t y, int32_t width, int32_t height) {
	$nc($nc($nc(sg2d)->loops)->fillRectLoop)->FillRect$(sg2d, $($nc(sg2d)->getSurfaceData()), x + $nc(sg2d)->transX, y + $nc(sg2d)->transY, width, height);
}
...
void LoopPipe::fill($SunGraphics2D* sg2d, $Shape* s) {
	...
	$var($Throwable, var$0, nullptr);
	try {
		$nc(sr)->setOutputArea($(sg2d->getCompClip()));
		$var($AffineTransform, at, (sg2d->transformState == $SunGraphics2D::TRANSFORM_ISIDENT) ? ($AffineTransform*)nullptr : sg2d->transform$);
...
void LoopPipe::fillSpans($SunGraphics2D* sg2d, $SpanIterator* si$renamed) {
	...
	$var($SurfaceData, sd, sg2d->getSurfaceData());
...
void LoopPipe::clinit$($Class* clazz) {
...
$Class* LoopPipe::load$($String* name, bool initialize) {
	...
	$loadClass(LoopPipe, name, initialize, &classInfo$$, LoopPipe::clinit$, []($Class* clazz) -> $Object* {
