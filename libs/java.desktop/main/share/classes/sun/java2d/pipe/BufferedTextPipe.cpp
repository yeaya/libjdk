...
int32_t BufferedTextPipe::createPackedParams($SunGraphics2D* sg2d, $GlyphList* gl) {
	...
	int32_t var$0 = var$1 | ($sl(gl->isSubPixPos() ? 1 : 0, BufferedTextPipe::OFFSET_SUBPIXPOS));
	return ((var$0 | ($sl(gl->isRGBOrder() ? 1 : 0, BufferedTextPipe::OFFSET_RGBORDER))) | ($sl($nc(sg2d)->lcdTextContrast & 0xff, BufferedTextPipe::OFFSET_CONTRAST)));
}
...
void BufferedTextPipe::enqueueGlyphList($SunGraphics2D* sg2d, $GlyphList* gl) {
	...
	if (totalBytesRequired <= $nc(buf)->capacity()) {
		...
		this->rq->flushAndInvokeNow($$new($BufferedTextPipe$1, this, totalGlyphs, gl, sg2d, glyphListOrigX, glyphListOrigY, images));
	}
}
...
$Class* BufferedTextPipe::load$($String* name, bool initialize) {
	...
	$loadClass(BufferedTextPipe, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(BufferedTextPipe);
	});
	return class$;
}
