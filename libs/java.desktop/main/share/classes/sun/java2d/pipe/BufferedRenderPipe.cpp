...
void BufferedRenderPipe::drawRect($SunGraphics2D* sg2d, int32_t x, int32_t y, int32_t width, int32_t height) {
	...
	try {
		...
		$nc(this->buf)->putInt(y + sg2d->transY);
...
void BufferedRenderPipe::drawPoly($SunGraphics2D* sg2d, $ints* xPoints, $ints* yPoints, int32_t nPoints, bool isClosed) {
	...
	if (nPoints < 2) {
		...
		drawLine(sg2d, xPoints->get(0), $nc(yPoints)->get(0), xPoints->get(1), $nc(yPoints)->get(1));
		return;
	}
	$nc(this->rq)->lock();
	$var($Throwable, var$0, nullptr);
	try {
		...
		if (totalBytesRequired <= $nc(this->buf)->capacity()) {
			...
			$nc(this->buf)->putInt(sg2d->transY);
...
int32_t BufferedRenderPipe::fillSpans($RenderQueue* rq, int64_t buf, int32_t pos, int32_t limit, $SpanIterator* si, int64_t iterator, int32_t transx, int32_t transy) {
	$prepareNative(fillSpans, int32_t, $RenderQueue* rq, int64_t buf, int32_t pos, int32_t limit, $SpanIterator* si, int64_t iterator, int32_t transx, int32_t transy);
	int32_t $ret = $invokeNative(rq, buf, pos, limit, si, iterator, transx, transy);
	$finishNative();
	return $ret;
}
...
void BufferedRenderPipe::fillSpans($SunGraphics2D* sg2d, $SpanIterator* si, int32_t transx, int32_t transy) {
	...
	try {
		...
		int32_t var$3 = this->buf->position();
		int32_t var$4 = this->buf->capacity();
		int32_t newpos = fillSpans(var$1, var$2, var$3, var$4, si, $nc(si)->getNativeIterator(), transx, transy);
...
$Class* BufferedRenderPipe::load$($String* name, bool initialize) {
	...
	$loadClass(BufferedRenderPipe, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
