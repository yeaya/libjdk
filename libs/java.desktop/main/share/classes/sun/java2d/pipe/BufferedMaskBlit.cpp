...
void BufferedMaskBlit::MaskBlit$($SurfaceData* src, $SurfaceData* dst, $Composite* comp$renamed, $Region* clip, int32_t srcx, int32_t srcy, int32_t dstx, int32_t dsty, int32_t width, int32_t height, $bytes* mask, int32_t maskoff, int32_t maskscan) {
	...
	try {
		...
		int32_t newpos = enqueueTile(var$3, var$4, src, $nc(src)->getNativeOps(), this->srcTypeVal, mask, $nc(mask)->length, maskoff, maskscan, srcx, srcy, dstx, dsty, width, height);
		buf->position(newpos);
...
int32_t BufferedMaskBlit::enqueueTile(int64_t buf, int32_t bpos, $SurfaceData* srcData, int64_t pSrcOps, int32_t srcType, $bytes* mask, int32_t masklen, int32_t maskoff, int32_t maskscan, int32_t srcx, int32_t srcy, int32_t dstx, int32_t dsty, int32_t width, int32_t height) {
	$prepareNative(enqueueTile, int32_t, int64_t buf, int32_t bpos, $SurfaceData* srcData, int64_t pSrcOps, int32_t srcType, $bytes* mask, int32_t masklen, int32_t maskoff, int32_t maskscan, int32_t srcx, int32_t srcy, int32_t dstx, int32_t dsty, int32_t width, int32_t height);
	int32_t $ret = $invokeNative(buf, bpos, srcData, pSrcOps, srcType, mask, masklen, maskoff, maskscan, srcx, srcy, dstx, dsty, width, height);
	$finishNative();
	return $ret;
}
...
$Class* BufferedMaskBlit::load$($String* name, bool initialize) {
	...
	$loadClass(BufferedMaskBlit, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(BufferedMaskBlit);
	});
	return class$;
}
