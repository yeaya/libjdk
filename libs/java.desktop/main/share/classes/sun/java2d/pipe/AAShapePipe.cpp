...
void AAShapePipe::fillParallelogram($SunGraphics2D* sg, double ux1, double uy1, double ux2, double uy2, double x, double y, double dx1, double dy1, double dx2, double dy2) {
	$useLocalObjectStack();
	$var($AAShapePipe$TileState, ts, $cast($AAShapePipe$TileState, AAShapePipe::TILE_STATE_PROVIDER->acquire()));
	$var($Throwable, var$0, nullptr);
	try {
		$var($ints, abox, $nc(ts)->abox);
		$var($AATileGenerator, aatg, $nc(AAShapePipe::RDR_ENGINE)->getAATileGenerator(x, y, dx1, dy1, dx2, dy2, 0, 0, $($nc(sg)->getCompClip()), abox));
		if (aatg != nullptr) {
		...
		AAShapePipe::TILE_STATE_PROVIDER->release(ts);
	}
	if (var$0 != nullptr) {
		$throw(var$0);
	}
...
void AAShapePipe::renderPath($SunGraphics2D* sg, $Shape* s, $BasicStroke* bs) {
	...
	bool thin = ($nc(sg)->strokeState <= $SunGraphics2D::STROKE_THINDASHED);
	$var($AAShapePipe$TileState, ts, $cast($AAShapePipe$TileState, AAShapePipe::TILE_STATE_PROVIDER->acquire()));
	$var($Throwable, var$0, nullptr);
	try {
		$var($ints, abox, $nc(ts)->abox);
...
void AAShapePipe::renderTiles($SunGraphics2D* sg, $Shape* s, $AATileGenerator* aatg, $ints* abox, $AAShapePipe$TileState* ts) {
	...
	try {
		...
		$var($bytes, alpha, ts->getAlphaTile(tw * th));
...
void AAShapePipe::clinit$($Class* clazz) {
...
$Class* AAShapePipe::load$($String* name, bool initialize) {
	...
	$loadClass(AAShapePipe, name, initialize, &classInfo$$, AAShapePipe::clinit$, []($Class* clazz) -> $Object* {
