...
void ShapeSpanIterator::appendPoly($ints* xPoints, $ints* yPoints, int32_t nPoints, int32_t xoff, int32_t yoff) {
	$prepareNative(appendPoly, void, $ints* xPoints, $ints* yPoints, int32_t nPoints, int32_t xoff, int32_t yoff);
	$invokeNative(xPoints, yPoints, nPoints, xoff, yoff);
	$finishNative();
}

void ShapeSpanIterator::setNormalize(bool adjust) {
	$prepareNative(setNormalize, void, bool adjust);
	$invokeNative(adjust);
	$finishNative();
}

void ShapeSpanIterator::setOutputAreaXYWH(int32_t x, int32_t y, int32_t w, int32_t h) {
	int32_t var$0 = $Region::dimAdd(x, w);
	setOutputAreaXYXY(x, y, var$0, $Region::dimAdd(y, h));
}

void ShapeSpanIterator::setOutputAreaXYXY(int32_t lox, int32_t loy, int32_t hix, int32_t hiy) {
	$prepareNative(setOutputAreaXYXY, void, int32_t lox, int32_t loy, int32_t hix, int32_t hiy);
	$invokeNative(lox, loy, hix, hiy);
	$finishNative();
}

void ShapeSpanIterator::setOutputArea($Rectangle* r) {
	setOutputAreaXYWH($nc(r)->x, $nc(r)->y, $nc(r)->width, $nc(r)->height);
}
...
void ShapeSpanIterator::setRule(int32_t rule) {
	$prepareNative(setRule, void, int32_t rule);
	$invokeNative(rule);
	$finishNative();
}

void ShapeSpanIterator::addSegment(int32_t type, $floats* coords) {
	$prepareNative(addSegment, void, int32_t type, $floats* coords);
	$invokeNative(type, coords);
	$finishNative();
}

void ShapeSpanIterator::getPathBox($ints* pathbox) {
	$prepareNative(getPathBox, void, $ints* pathbox);
	$invokeNative(pathbox);
	$finishNative();
}

void ShapeSpanIterator::intersectClipBox(int32_t lox, int32_t loy, int32_t hix, int32_t hiy) {
	$prepareNative(intersectClipBox, void, int32_t lox, int32_t loy, int32_t hix, int32_t hiy);
	$invokeNative(lox, loy, hix, hiy);
	$finishNative();
}

bool ShapeSpanIterator::nextSpan($ints* spanbox) {
	$prepareNative(nextSpan, bool, $ints* spanbox);
	bool $ret = $invokeNative(spanbox);
	$finishNative();
	return $ret;
}

void ShapeSpanIterator::skipDownTo(int32_t y) {
	$prepareNative(skipDownTo, void, int32_t y);
	$invokeNative(y);
	$finishNative();
}

int64_t ShapeSpanIterator::getNativeIterator() {
	$prepareNative(getNativeIterator, int64_t);
...
$Class* ShapeSpanIterator::load$($String* name, bool initialize) {
	...
	$loadClass(ShapeSpanIterator, name, initialize, &classInfo$$, ShapeSpanIterator::clinit$, []($Class* clazz) -> $Object* {
