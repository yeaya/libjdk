...
$Object* GeneralCompositePipe::startSequence($SunGraphics2D* sg, $Shape* s, $Rectangle* devR, $ints* abox) {
	...
	$var($Rectangle2D, var$0, $nc(s)->getBounds2D());
	$var($PaintContext, paintContext, $nc(sg->paint)->createContext(model, devR, var$0, $(sg->cloneTransform()), hints));
	$var($CompositeContext, compositeContext, $nc(sg->composite)->createContext($($nc(paintContext)->getColorModel()), model, hints));
	return $new($GeneralCompositePipe$TileContext, this, sg, paintContext, compositeContext, model);
}
...
void GeneralCompositePipe::renderPathTile(Object$* ctx, $bytes* atile, int32_t offset, int32_t tilesize, int32_t x, int32_t y, int32_t w, int32_t h) {
	...
	if ($instanceOf($WritableRaster, dstRaster) && atile == nullptr) {
		...
		$assign(dstOut, dstOut->createWritableChild(x, y, w, h, 0, 0, nullptr));
		$assign(dstIn, dstOut);
	} else {
	...
	if (!$equals(dstRaster, dstOut) && $nc(dstOut)->getParent() != dstRaster) {
		if ($instanceOf($WritableRaster, dstRaster) && atile == nullptr) {
			$cast($WritableRaster, dstRaster)->setDataElements(x, y, dstOut);
		} else {
			$var($ColorModel, cm, sg->getDeviceColorModel());
			$var($BufferedImage, resImg, $new($BufferedImage, cm, dstOut, $nc(cm)->isAlphaPremultiplied(), nullptr));
...
$Class* GeneralCompositePipe::load$($String* name, bool initialize) {
	...
	$loadClass(GeneralCompositePipe, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(GeneralCompositePipe);
	});
	return class$;
}
