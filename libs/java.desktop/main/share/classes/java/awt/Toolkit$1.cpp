...
$Object* Toolkit$1::run() {
	...
	try {
		...
		$nc(this->val$properties)->load(in);
		in->close();
	} catch ($Exception& e) {
	}
	...
	if (magPresent == nullptr) {
		$assign(magPresent, this->val$properties->getProperty("screen_magnifier_present"_s, nullptr));
		if (magPresent != nullptr) {
	...
	if (classNames == nullptr) {
		$assign(classNames, this->val$properties->getProperty("assistive_technologies"_s, nullptr));
		if (classNames != nullptr) {
...
$Class* Toolkit$1::load$($String* name, bool initialize) {
	...
	$loadClass(Toolkit$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Toolkit$1);
	});
	return class$;
}
