...
void EventQueue::init$() {
	...
	$set(this, name, $str({"AWT-EventQueue-"_s, $$str(EventQueue::threadInitNumber->getAndIncrement())}));
	...
	$set(this, pushPopCond, $cast($Condition, this->appContext->get($AppContext::EVENT_QUEUE_COND_KEY)));
}
...
void EventQueue::postEvent($AWTEvent* theEvent, int32_t priority) {
	...
	if ($nc($nc(this->queues)->get(priority))->head == nullptr) {
		...
		$set($nc($nc(this->queues->get(priority))->tail), next, newItem);
		$set($nc(this->queues->get(priority)), tail, newItem);
		if (notifyID) {
			$nc(this->pushPopCond)->signalAll();
		}
	}
...
bool EventQueue::coalesceEvent($AWTEvent* e, int32_t priority) {
	...
	bool var$0 = $$sure($Component, e->getSource())->isCoalescingEnabled();
...
void EventQueue::cacheEQItem($EventQueueItem* entry) {
	...
	if (index != -1 && $instanceOf($Component, $($nc(entry->event)->getSource()))) {
		$var($Component, source, $cast($Component, entry->event->getSource()));
		if ($nc(source)->eventCache == nullptr) {
			$set(source, eventCache, $new($EventQueueItemArray, EventQueue::CACHE_LENGTH));
		}
		$nc(source->eventCache)->set(index, entry);
	}
}
...
void EventQueue::uncacheEQItem($EventQueueItem* entry) {
	...
	if (index != -1 && $instanceOf($Component, $($nc(entry->event)->getSource()))) {
		...
		$nc(source->eventCache)->set(index, nullptr);
	}
}
...
$AWTEvent* EventQueue::getNextEventPrivate() {
	...
	for (int32_t i = EventQueue::NUM_PRIORITIES - 1; i >= 0; --i) {
		if ($nc($nc(this->queues)->get(i))->head != nullptr) {
			$var($EventQueueItem, entry, $nc(this->queues->get(i))->head);
			$set($nc(this->queues->get(i)), head, $nc(entry)->next);
			if (entry->next == nullptr) {
				$set($nc(this->queues->get(i)), tail, nullptr);
			}
			uncacheEQItem(entry);
			return entry->event;
		}
				...
				for (; entry != nullptr; $assign(prev, entry), $assign(entry, entry->next)) {
					if ($nc(entry->event)->getID() == id) {
						...
						if ($nc($nc(this->queues)->get(i))->tail == entry) {
							$set($nc(this->queues->get(i)), tail, prev);
						}
						uncacheEQItem(entry);
...
$AWTEvent* EventQueue::peekEvent() {
	...
	try {
		for (int32_t i = EventQueue::NUM_PRIORITIES - 1; i >= 0; --i) {
			if ($nc($nc(this->queues)->get(i))->head != nullptr) {
				$assign(var$2, $nc(this->queues->get(i))->head->event);
				return$1 = true;
				goto $finally;
			}
		}
...
$AccessControlContext* EventQueue::getAccessControlContextFrom(Object$* src) {
	$init(EventQueue);
	return $instanceOf($Component, src) ? $cast($Component, src)->getAccessControlContext() : $instanceOf($MenuComponent, src) ? $cast($MenuComponent, src)->getAccessControlContext() : $instanceOf($TrayIcon, src) ? $cast($TrayIcon, src)->getAccessControlContext() : ($AccessControlContext*)nullptr;
}
...
void EventQueue::dispatchEventImpl($AWTEvent* event, Object$* src) {
	...
	if ($instanceOf($ActiveEvent, event)) {
		...
		$cast($ActiveEvent, event)->dispatch();
	} else if ($instanceOf($Component, src)) {
		$cast($Component, src)->dispatchEvent(event);
		event->dispatched();
	} else if ($instanceOf($MenuComponent, src)) {
		$cast($MenuComponent, src)->dispatchEvent(event);
	} else if ($instanceOf($TrayIcon, src)) {
		$cast($TrayIcon, src)->dispatchEvent(event);
...
$AWTEvent* EventQueue::getCurrentEventImpl() {
	...
	try {
		if ($equals($Thread::currentThread(), this->dispatchThread) || EventQueue::fxAppThreadIsDispatchThread) {
			$assign(var$2, (this->currentEvent != nullptr) ? $cast($AWTEvent, this->currentEvent->get()) : ($AWTEvent*)nullptr);
			return$1 = true;
			goto $finally;
		}
...
void EventQueue::push(EventQueue* newEventQueue) {
	...
	try {
		...
		if ((topQueue->dispatchThread != nullptr) && (topQueue->dispatchThread->getEventQueue() == this)) {
		...
		while (topQueue->peekEvent() != nullptr) {
			try {
				...
				if ($$nc(getEventLog())->isLoggable($PlatformLogger$Level::FINE)) {
					$$nc(getEventLog())->fine("Interrupted push"_s, ie);
				}
			}
		}
		if (topQueue->dispatchThread != nullptr) {
		...
		$set(topQueue, nextQueue, newEventQueue);
		$init($AppContext);
	...
	try {
		...
		while (topQueue->peekEvent() != nullptr) {
			try {
				...
				if ($$nc(getEventLog())->isLoggable($PlatformLogger$Level::FINE)) {
					$$nc(getEventLog())->fine("Interrupted pop"_s, ie);
				}
			}
		}
		if ((topQueue->dispatchThread != nullptr) && (topQueue->dispatchThread->getEventQueue() == this)) {
...
$SecondaryLoop* EventQueue::createSecondaryLoop($Conditional* cond, $EventFilter* filter, int64_t interval) {
	...
	try {
		if (this->nextQueue != nullptr) {
			$assign(var$2, this->nextQueue->createSecondaryLoop(cond, filter, interval));
			return$1 = true;
			goto $finally;
		}
		if (this->fwDispatcher != nullptr) {
			$assign(var$2, $new($EventQueue$FwSecondaryLoopWrapper, this, $(this->fwDispatcher->createSecondaryLoop()), filter));
			return$1 = true;
			goto $finally;
		}
		if (this->dispatchThread == nullptr) {
...
bool EventQueue::isDispatchThreadImpl() {
	...
	try {
		...
		if ($nc(eq)->fwDispatcher != nullptr) {
			var$2 = eq->fwDispatcher->isDispatchThread();
			return$1 = true;
			goto $finally;
		}
		var$2 = ($equals($Thread::currentThread(), eq->dispatchThread));
		return$1 = true;
		goto $finally;
	} catch ($Throwable& var$3) {
		$assign(var$0, var$3);
...
void EventQueue::initDispatchThread() {
	...
	try {
		...
		if (var$1 && !$nc(this->appContext)->isDisposed()) {
			$set(this, dispatchThread, $cast($EventDispatchThread, $AccessController::doPrivileged($$new($EventQueue$6, this))));
			...
			while (entry != nullptr) {
				if (($equals($nc(entry->event)->getSource(), source)) && (removeAllEvents || !($instanceOf($SequencedEvent, entry->event) || $instanceOf($SentEvent, entry->event) || $instanceOf($FocusEvent, entry->event) || $instanceOf($WindowEvent, entry->event) || $instanceOf($KeyEvent, entry->event) || $instanceOf($InputMethodEvent, entry->event)))) {
					if ($instanceOf($SequencedEvent, entry->event)) {
						$cast($SequencedEvent, entry->event)->dispose();
					}
					if ($instanceOf($SentEvent, entry->event)) {
						$cast($SentEvent, entry->event)->dispose();
					}
					...
					if ($instanceOf($SunDropTargetEvent, entry->event)) {
						$cast($SunDropTargetEvent, entry->event)->dispose();
					}
					if (prev == nullptr) {
						$set($nc($nc(this->queues)->get(i)), head, entry->next);
					} else {
...
void EventQueue::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$beforeCallerSensitive();
	...
	$assignStatic(EventQueue::dummyRunnable, $new($EventQueue$1));
	{
		...
		$AccessController::doPrivileged($$new($EventQueue$3));
	}
...
$Class* EventQueue::load$($String* name, bool initialize) {
	...
	$loadClass(EventQueue, name, initialize, &classInfo$$, EventQueue::clinit$, []($Class* clazz) -> $Object* {
