...
$Class* DragSourceContext$$Lambda$lambda$static$0::load$($String* name, bool initialize) {
	...
	$loadClass(DragSourceContext$$Lambda$lambda$static$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DragSourceContext$$Lambda$lambda$static$0);
	});
	return class$;
}
...
void DragSourceContext::init$($DragGestureEvent* trigger, $Cursor* dragCursor, $Image* dragImage, $Point* offset, $Transferable* t, $DragSourceListener* dsl) {
	...
	if (trigger->getComponent() == nullptr) {
		$throwNew($IllegalArgumentException, "Component"_s);
	}
	if ($$nc(trigger->getSourceAsDragGestureRecognizer())->getSourceActions() == $DnDConstants::ACTION_NONE) {
	...
	this->sourceActions = $$nc(trigger->getSourceAsDragGestureRecognizer())->getSourceActions();
...
void DragSourceContext::removeDragSourceListener($DragSourceListener* dsl) {
	$synchronized(this) {
		if (this->listener != nullptr && this->listener->equals(dsl)) {
	...
	$synchronized(this) {
		...
		$var($Cursor, c, nullptr);
		{
			...
			case DragSourceContext::CHANGED:
				...
				} else if ((ra & $DnDConstants::ACTION_LINK) == $DnDConstants::ACTION_LINK) {
					$init($DragSource);
...
void DragSourceContext::readObject($ObjectInputStream* s) {
	...
	$var($DragGestureEvent, newTrigger, $cast($DragGestureEvent, $nc(f)->get("trigger"_s, nullptr)));
	...
	if (newTrigger->getComponent() == nullptr) {
		$throwNew($InvalidObjectException, "Null trigger component"_s);
	}
	int32_t newSourceActions = f->get("sourceActions"_s, 0) & ($DnDConstants::ACTION_COPY_OR_MOVE | $DnDConstants::ACTION_LINK);
	if (newSourceActions == $DnDConstants::ACTION_NONE) {
		$throwNew($InvalidObjectException, "Invalid source actions"_s);
	}
	int32_t triggerActions = newTrigger->getDragAction();
	...
	$set(this, cursor, $cast($Cursor, f->get("cursor"_s, nullptr)));
...
void DragSourceContext::clinit$($Class* clazz) {
	{
		$AWTAccessor::setDragSourceContextAccessor($$new(DragSourceContext$$Lambda$lambda$static$0));
	}
}
...
$Class* DragSourceContext::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.awt.dnd.DragSourceContext$$Lambda$lambda$static$0")) {
	...
	$loadClass(DragSourceContext, name, initialize, &classInfo$$, DragSourceContext::clinit$, []($Class* clazz) -> $Object* {
