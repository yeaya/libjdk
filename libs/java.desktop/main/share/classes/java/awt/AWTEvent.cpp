...
void AWTEvent::init$($Event* event) {
	AWTEvent::init$($nc(event)->target, $nc(event)->id);
}
...
void AWTEvent::nativeSetSource($ComponentPeer* peer) {
	$prepareNative(nativeSetSource, void, $ComponentPeer* peer);
	$invokeNative(peer);
	$finishNative();
}
...
$String* AWTEvent::toString() {
	...
	if ($instanceOf($Component, this->source)) {
		$assign(srcName, $cast($Component, this->source)->getName());
	} else if ($instanceOf($MenuComponent, this->source)) {
		$assign(srcName, $cast($MenuComponent, this->source)->getName());
	}
	...
	var$0->append("] on "_s);
	var$0->append(srcName != nullptr ? $of(srcName) : this->source);
	return $str(var$0);
}
	...
	int32_t newid = this->id;
	{
		...
		case $KeyEvent::KEY_RELEASED:
			{
				...
				int64_t var$0 = ke->getWhen();
				int32_t var$1 = $Event::getOldEventKey(ke);
				return $new($Event, src, var$0, newid, 0, 0, var$1, (ke->getModifiers() & ~$InputEvent::BUTTON1_MASK));
			}
		case $MouseEvent::MOUSE_PRESSED:
		case $MouseEvent::MOUSE_RELEASED:
		case $MouseEvent::MOUSE_MOVED:
		...
		case $ComponentEvent::COMPONENT_MOVED:
			if ($instanceOf($Frame, src) || $instanceOf($Dialog, src)) {
				...
				return $new($Event, src, 0, $Event::WINDOW_MOVED, $nc(p)->x, $nc(p)->y, 0, 0);
			}
			break;
		case $ActionEvent::ACTION_PERFORMED:
			$assign(ae, $cast($ActionEvent, this));
			if ($instanceOf($Button, src)) {
				$assign(cmd, $cast($Button, src)->getLabel());
...
void AWTEvent::clinit$($Class* clazz) {
	{
...
$Class* AWTEvent::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(Ljava/awt/Event;)V", nullptr, $PUBLIC | $DEPRECATED, $method(AWTEvent, init$, void, $Event*), nullptr, nullptr, init$methodAnnotations$$},
	...
	$loadClass(AWTEvent, name, initialize, &classInfo$$, AWTEvent::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(AWTEvent);
	});
	return class$;
}
