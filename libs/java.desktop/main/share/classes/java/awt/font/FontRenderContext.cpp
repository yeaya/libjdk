...
bool FontRenderContext::equals(FontRenderContext* rhs) {
	...
	if (!$nc(rhs)->defaulting && !this->defaulting) {
		if ($equals(rhs->aaHintValue, this->aaHintValue) && $equals(rhs->fmHintValue, this->fmHintValue)) {
			return this->tx == nullptr ? rhs->tx == nullptr : this->tx->equals(rhs->tx);
		}
		return false;
	} else {
...
int32_t FontRenderContext::hashCode() {
	$useLocalObjectStack();
	int32_t hash = this->tx == nullptr ? 0 : this->tx->hashCode();
	if (this->defaulting) {
		hash += $$nc(getAntiAliasingHint())->hashCode();
		hash += $$nc(getFractionalMetricsHint())->hashCode();
	} else {
		hash += $nc(this->aaHintValue)->hashCode();
		hash += $nc(this->fmHintValue)->hashCode();
	}
	return hash;
}
...
$Class* FontRenderContext::load$($String* name, bool initialize) {
	...
	$loadClass(FontRenderContext, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FontRenderContext);
	});
	return class$;
}
