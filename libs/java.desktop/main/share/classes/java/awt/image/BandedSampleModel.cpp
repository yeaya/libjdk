...
void BandedSampleModel::init$(int32_t dataType, int32_t w, int32_t h, int32_t numBands) {
	$useLocalObjectStack();
	$var($ints, var$0, BandedSampleModel::createIndicesArray(numBands));
	$ComponentSampleModel::init$(dataType, w, h, 1, w, var$0, $(BandedSampleModel::createOffsetArray(numBands)));
}
...
$DataBuffer* BandedSampleModel::createDataBuffer() {
	...
	if (this->numBanks == 1) {
		...
		for (int32_t index = 1; index < this->bandOffsets->length; ++index) {
			size += (this->bandOffsets->get(index) - size) + sizePerBand;
		}
	} else {
		int32_t maxBandOffset = $nc(this->bandOffsets)->get(0);
		for (int32_t index = 1; index < this->bandOffsets->length; ++index) {
			maxBandOffset = $Math::max(maxBandOffset, this->bandOffsets->get(index));
		}
		size += maxBandOffset;
	}
...
void BandedSampleModel::setDataElements(int32_t x, int32_t y, Object$* obj, $DataBuffer* data) {
	...
	int32_t pixelOffset = y * this->scanlineStride + x;
	{
		...
		case $DataBuffer::TYPE_BYTE:
			...
			for (int32_t i = 0; i < numDataElems; ++i) {
				$nc(data)->setElem($nc(this->bankIndices)->get(i), pixelOffset + $nc(this->bandOffsets)->get(i), $nc(barray)->get(i) & 0xff);
			}
			break;
		case $DataBuffer::TYPE_USHORT:
		case $DataBuffer::TYPE_SHORT:
			...
			for (int32_t i = 0; i < numDataElems; ++i) {
				$nc(data)->setElem($nc(this->bankIndices)->get(i), pixelOffset + $nc(this->bandOffsets)->get(i), $nc(sarray)->get(i) & 0xffff);
			}
			break;
		case $DataBuffer::TYPE_INT:
...
$Class* BandedSampleModel::load$($String* name, bool initialize) {
	...
	$loadClass(BandedSampleModel, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(BandedSampleModel);
	});
	return class$;
}
