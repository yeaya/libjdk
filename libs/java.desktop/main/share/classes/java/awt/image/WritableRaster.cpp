...
void WritableRaster::init$($SampleModel* sampleModel, $Point* origin) {
	...
	WritableRaster::init$(sampleModel, var$0, $$new($Rectangle, var$1, var$2, var$3, sampleModel->getHeight()), origin, nullptr);
}
...
WritableRaster* WritableRaster::createWritableChild(int32_t parentX, int32_t parentY, int32_t w, int32_t h, int32_t childMinX, int32_t childMinY, $ints* bandList) {
	...
	$var($Rectangle, var$1, $new($Rectangle, childMinX, childMinY, w, h));
	return $new(WritableRaster, sm, var$0, var$1, $$new($Point, this->sampleModelTranslateX + deltaX, this->sampleModelTranslateY + deltaY), this);
}
...
$Class* WritableRaster::load$($String* name, bool initialize) {
	...
	$loadClass(WritableRaster, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(WritableRaster);
	});
	return class$;
}
