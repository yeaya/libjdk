...
Image* Image::getScaledInstance(int32_t width, int32_t height, int32_t hints) {
	...
	if ((hints & (Image::SCALE_SMOOTH | Image::SCALE_AREA_AVERAGING)) != 0) {
...
$Class* Image::load$($String* name, bool initialize) {
	...
	$loadClass(Image, name, initialize, &classInfo$$, Image::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Image);
	});
	return class$;
}
