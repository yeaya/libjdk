...
void LightweightDispatcher$3::run() {
	...
	$nc(this->val$mouseEvent)->translatePoint($nc(this->val$ptSrcOrigin)->x - $nc(ptDstOrigin)->x, $nc(this->val$ptSrcOrigin)->y - $nc(ptDstOrigin)->y);
	int32_t var$0 = this->val$mouseEvent->getX();
	$var($Component, targetOver, $nc(this->this$0->nativeContainer)->getMouseEventTarget(var$0, this->val$mouseEvent->getY(), $Container::INCLUDE_SELF));
...
$Class* LightweightDispatcher$3::load$($String* name, bool initialize) {
	...
	$loadClass(LightweightDispatcher$3, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LightweightDispatcher$3);
	});
	return class$;
}
