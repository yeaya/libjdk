...
$Class* Toolkit$$Lambda$trim::load$($String* name, bool initialize) {
	...
	$loadClass(Toolkit$$Lambda$trim, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Toolkit$$Lambda$trim);
	});
	return class$;
}
...
$Class* Toolkit$$Lambda$lambda$loadAssistiveTechnologies$0$1::load$($String* name, bool initialize) {
	...
	$loadClass(Toolkit$$Lambda$lambda$loadAssistiveTechnologies$0$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Toolkit$$Lambda$lambda$loadAssistiveTechnologies$0$1);
	});
	return class$;
}
...
$Class* Toolkit$$Lambda$lambda$loadAssistiveTechnologies$1$2::load$($String* name, bool initialize) {
	...
	$loadClass(Toolkit$$Lambda$lambda$loadAssistiveTechnologies$1$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Toolkit$$Lambda$lambda$loadAssistiveTechnologies$1$2);
	});
	return class$;
}
...
$Class* Toolkit$$Lambda$fallbackToLoadClassForAT$3::load$($String* name, bool initialize) {
	...
	$loadClass(Toolkit$$Lambda$fallbackToLoadClassForAT$3, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Toolkit$$Lambda$fallbackToLoadClassForAT$3);
	});
	return class$;
}
...
void Toolkit::initAssistiveTechnologies() {
	...
	$assignStatic(Toolkit::atNames, $cast($String, $AccessController::doPrivileged($$new($Toolkit$1, sep, properties))));
}
...
void Toolkit::fallbackToLoadClassForAT($String* atName) {
	...
	try {
		...
		$$nc(c->getConstructor($$new($ClassArray, 0)))->newInstance($$new($ObjectArray, 0));
...
void Toolkit::loadAssistiveTechnologies() {
	...
	$useLocalObjectStack();
	$beforeCallerSensitive();
	if (Toolkit::atNames != nullptr && !Toolkit::atNames->isBlank()) {
		$var($ClassLoader, cl, $ClassLoader::getSystemClassLoader());
		$var($Set, names, $cast($Set, $$nc($$nc($Arrays::stream($(Toolkit::atNames->split(","_s))))->map($$new(Toolkit$$Lambda$trim)))->collect($($Collectors::toSet()))));
		$var($Map, providers, $new($HashMap));
		$AccessController::doPrivileged($cast($PrivilegedAction, $$new(Toolkit$$Lambda$lambda$loadAssistiveTechnologies$0$1, cl, names, providers)));
		$$nc($$nc($nc(names)->stream())->filter($$new(Toolkit$$Lambda$lambda$loadAssistiveTechnologies$1$2, providers)))->forEach($$new(Toolkit$$Lambda$fallbackToLoadClassForAT$3));
	}
}
...
void Toolkit::loadLibraries() {
	...
	$beforeCallerSensitive();
	if (!Toolkit::loaded) {
		$AccessController::doPrivileged($$new($Toolkit$2));
		Toolkit::loaded = true;
	}
}
...
void Toolkit::initStatic() {
	...
	$AccessController::doPrivileged($$new($Toolkit$4));
	loadLibraries();
...
$String* Toolkit::getProperty($String* key, $String* defaultValue) {
	...
	if (Toolkit::platformResources != nullptr) {
		try {
			return Toolkit::platformResources->getString(key);
		} catch ($MissingResourceException& e) {
		}
	}
	if (Toolkit::resources != nullptr) {
		try {
			return Toolkit::resources->getString(key);
		} catch ($MissingResourceException& e) {
		}
	}
	return defaultValue;
...
$Object* Toolkit::getDesktopProperty($String* propertyName) {
	$synchronized(this) {
		...
		if ($instanceOf($HeadlessToolkit, this)) {
			return $$nc($cast($HeadlessToolkit, this)->getUnderlyingToolkit())->getDesktopProperty(propertyName);
		}
		...
		if ($nc(propertyName)->equals("awt.dynamicLayoutSupported"_s)) {
			return $$nc(getDefaultToolkit())->lazilyLoadDesktopProperty(propertyName);
		}
		$assign(value, this->desktopProperties->get(propertyName));
		if (value == nullptr) {
		...
		if ($instanceOf($RenderingHints, value)) {
			$assign(value, $cast($RenderingHints, value)->clone());
		}
		return value;
	}
}
...
void Toolkit::setDesktopProperty($String* name, Object$* newValue) {
	$useLocalObjectStack();
	if ($instanceOf($HeadlessToolkit, this)) {
		$$nc($cast($HeadlessToolkit, this)->getUnderlyingToolkit())->setDesktopProperty(name, newValue);
		return;
	}
	$var($Object, oldValue, nullptr);
	$synchronized(this) {
		$assign(oldValue, this->desktopProperties->get(name));
...
$AWTEventListener* Toolkit::deProxyAWTEventListener($AWTEventListener* l) {
	...
	if ($instanceOf($AWTEventListenerProxy, l)) {
		$assign(localL, $cast($AWTEventListener, $cast($AWTEventListenerProxy, l)->getListener()));
	}
	return localL;
}
	...
	$synchronized(this) {
		...
		if (selectiveListener != nullptr) {
			...
			for (int32_t i = 0; i < Toolkit::LONG_BITS; ++i) {
				...
				if (!Toolkit::$assertionsDisabled && !(this->calls->get(i) >= 0)) {
					$throwNew($AssertionError, $of("Negative Listeners count"_s));
				}
				if (this->calls->get(i) == 0) {
					Toolkit::enabledOnToolkitMask &= (uint64_t)~($sl((int64_t)1, i));
				}
			}
		}
		$set(this, eventListener, $Toolkit$ToolkitEventMulticaster::remove(this->eventListener, (selectiveListener == nullptr) ? localL : $cast($AWTEventListener, selectiveListener)));
	}
}

bool Toolkit::enabledOnToolkit(int64_t eventMask) {
	$init(Toolkit);
	return (Toolkit::enabledOnToolkitMask & eventMask) != 0;
}
...
$AWTEventListenerArray* Toolkit::getAWTEventListeners(int64_t eventMask) {
	...
	$synchronized(this) {
		...
		for (int32_t i = 0; i < la->length; ++i) {
			$var($Toolkit$SelectiveAWTEventListener, sael, $cast($Toolkit$SelectiveAWTEventListener, la->get(i)));
			if (($nc(sael)->getEventMask() & eventMask) == eventMask) {
				int64_t var$0 = sael->getEventMask();
				list->add($$new($AWTEventListenerProxy, var$0, $(sael->getListener())));
			}
		}
		return $cast($AWTEventListenerArray, list->toArray($$new($AWTEventListenerArray, 0)));
	}
}
...
void Toolkit::notifyAWTEventListeners($AWTEvent* theEvent) {
	$useLocalObjectStack();
	if ($instanceOf($HeadlessToolkit, this)) {
		$$nc($cast($HeadlessToolkit, this)->getUnderlyingToolkit())->notifyAWTEventListeners(theEvent);
		return;
	}
...
void Toolkit::clinit$($Class* clazz) {
...
$Class* Toolkit::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.awt.Toolkit$$Lambda$trim")) {
			return Toolkit$$Lambda$trim::load$(name, initialize);
		}
		if (name->equals("java.awt.Toolkit$$Lambda$lambda$loadAssistiveTechnologies$0$1")) {
			return Toolkit$$Lambda$lambda$loadAssistiveTechnologies$0$1::load$(name, initialize);
		}
		if (name->equals("java.awt.Toolkit$$Lambda$lambda$loadAssistiveTechnologies$1$2")) {
			return Toolkit$$Lambda$lambda$loadAssistiveTechnologies$1$2::load$(name, initialize);
		}
		if (name->equals("java.awt.Toolkit$$Lambda$fallbackToLoadClassForAT$3")) {
	...
	$CompoundAttribute getFontListmethodAnnotations$$[] = {
	...
	$MethodInfo methodInfos$$[] = {
		...
		{"getFontList", "()[Ljava/lang/String;", nullptr, $PUBLIC | $ABSTRACT | $DEPRECATED, $virtualMethod(Toolkit, getFontList, $StringArray*), nullptr, nullptr, getFontListmethodAnnotations$$},
		{"getFontMetrics", "(Ljava/awt/Font;)Ljava/awt/FontMetrics;", nullptr, $PUBLIC | $ABSTRACT | $DEPRECATED, $virtualMethod(Toolkit, getFontMetrics, $FontMetrics*, $Font*), nullptr, nullptr, getFontMetricsmethodAnnotations$$},
		...
		{"getMenuShortcutKeyMask", "()I", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Toolkit, getMenuShortcutKeyMask, int32_t), "java.awt.HeadlessException", nullptr, getMenuShortcutKeyMaskmethodAnnotations$$},
	...
	$loadClass(Toolkit, name, initialize, &classInfo$$, Toolkit::clinit$, []($Class* clazz) -> $Object* {
