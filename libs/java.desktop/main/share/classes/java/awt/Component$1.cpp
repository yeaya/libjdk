...
$Rectangle* Component$1::getBounds($Component* comp) {
	return $new($Rectangle, $nc(comp)->x, $nc(comp)->y, $nc(comp)->width, $nc(comp)->height);
}
...
$Class* Component$1::load$($String* name, bool initialize) {
	...
	$loadClass(Component$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Component$1);
	});
	return class$;
}
