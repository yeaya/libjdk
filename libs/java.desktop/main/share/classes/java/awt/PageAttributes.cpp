...
void PageAttributes::setOrientationRequested(int32_t orientationRequested) {
	...
	case 4:
		...
		break;
	default:
		setOrientationRequested(nullptr);
		break;
	}
}
...
$ints* PageAttributes::getPrinterResolution() {
	...
	copy->set(1, this->printerResolution->get(1));
	copy->set(2, this->printerResolution->get(2));
	return copy;
}

void PageAttributes::setPrinterResolution($ints* printerResolution) {
	if (printerResolution == nullptr || printerResolution->length != 3 || printerResolution->get(0) <= 0 || printerResolution->get(1) <= 0 || (printerResolution->get(2) != 3 && printerResolution->get(2) != 4)) {
...
bool PageAttributes::equals(Object$* obj) {
	...
	return (this->color == $nc(rhs)->color && this->media == rhs->media && this->orientationRequested == rhs->orientationRequested && this->origin == rhs->origin && this->printQuality == rhs->printQuality && $nc(this->printerResolution)->get(0) == $nc(rhs->printerResolution)->get(0) && this->printerResolution->get(1) == rhs->printerResolution->get(1) && this->printerResolution->get(2) == rhs->printerResolution->get(2));
}
...
$String* PageAttributes::toString() {
	...
	var$0->append(",media="_s);
	var$0->append($(getMedia()));
	var$0->append(",orientation-requested="_s);
	var$0->append($(getOrientationRequested()));
	var$0->append(",origin="_s);
	var$0->append($(getOrigin()));
	var$0->append(",print-quality="_s);
	var$0->append($(getPrintQuality()));
	var$0->append(",printer-resolution=["_s);
	var$0->append($nc(this->printerResolution)->get(0));
	var$0->append(","_s);
	var$0->append(this->printerResolution->get(1));
	var$0->append(","_s);
	var$0->append(this->printerResolution->get(2));
	var$0->append("]"_s);
	return $str(var$0);
}
...
$Class* PageAttributes::load$($String* name, bool initialize) {
	...
	$loadClass(PageAttributes, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PageAttributes);
	});
	return class$;
}
