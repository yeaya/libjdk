	...
	$synchronized(class$) {
		if (cm == GradientPaintContext::cachedModel) {
			if (GradientPaintContext::cached != nullptr) {
				$var($Raster, ras, $cast($Raster, GradientPaintContext::cached->get()));
...
void GradientPaintContext::putCachedRaster($ColorModel* cm, $Raster* ras) {
	...
	$synchronized(class$) {
		if (GradientPaintContext::cached != nullptr) {
			$var($Raster, cras, $cast($Raster, GradientPaintContext::cached->get()));
			if (cras != nullptr) {
...
void GradientPaintContext::init$($ColorModel* cm, $Point2D* p1$renamed, $Point2D* p2$renamed, $AffineTransform* xform, $Color* c1$renamed, $Color* c2$renamed, bool cyclic) {
	...
	$var($Point2D, xvec, $new($Point2D$Double, 1, 0));
	$var($Point2D, yvec, $new($Point2D$Double, 0, 1));
	try {
	...
	int32_t a1 = (rgb1 >> 24) & 0xff;
	int32_t r1 = (rgb1 >> 16) & 0xff;
	int32_t g1 = (rgb1 >> 8) & 0xff;
	int32_t b1 = (rgb1) & 0xff;
	int32_t da = ((rgb2 >> 24) & 0xff) - a1;
	int32_t dr = ((rgb2 >> 16) & 0xff) - r1;
	int32_t dg = ((rgb2 >> 8) & 0xff) - g1;
	int32_t db = ((rgb2) & 0xff) - b1;
	if (a1 == 255 && da == 0) {
	...
	for (int32_t i = 0; i <= 256; ++i) {
		...
		this->interp->set(i, rgb);
		if (cyclic) {
			this->interp->set(512 - i, rgb);
		}
	}
}
...
$Raster* GradientPaintContext::getRaster(int32_t x, int32_t y, int32_t w, int32_t h) {
	...
	bool var$0 = rast == nullptr || rast->getWidth() < w;
	if (var$0 || rast->getHeight() < h) {
...
void GradientPaintContext::clinit$($Class* clazz) {
...
$Class* GradientPaintContext::load$($String* name, bool initialize) {
	...
	$loadClass(GradientPaintContext, name, initialize, &classInfo$$, GradientPaintContext::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(GradientPaintContext);
	});
	return class$;
}
