...
int32_t MediaEntry::getStatus(bool doLoad, bool doVerify) {
	$synchronized(this) {
		if (doLoad && ((this->status & MediaEntry::LOADSTARTED) == 0)) {
			this->status = (this->status & ~MediaEntry::ABORTED) | MediaEntry::LOADING;
			startLoad();
		}
		return this->status;
	}
...
$Class* MediaEntry::load$($String* name, bool initialize) {
	...
	$loadClass(MediaEntry, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MediaEntry);
	});
	return class$;
}
