...
void Component::setName($String* name) {
	...
	firePropertyChange("name"_s, oldName, name);
}
...
void Component::setDropTarget($DropTarget* dt) {
	$synchronized(this) {
		$useLocalObjectStack();
		if (dt == this->dropTarget || (this->dropTarget != nullptr && this->dropTarget->equals(dt))) {
			return;
		}
...
bool Component::updateGraphicsData($GraphicsConfiguration* gc) {
	...
	if (oldConfig != gc) {
		firePropertyChange("graphicsConfiguration"_s, oldConfig, gc);
	}
	return ret;
}
...
bool Component::isRecursivelyVisible() {
	return this->visible && (this->parent == nullptr || this->parent->isRecursivelyVisible());
}
...
$Point* Component::pointRelativeToComponent($Point* absolute) {
	...
	return $new($Point, $nc(absolute)->x - $nc(compCoords)->x, $nc(absolute)->y - $nc(compCoords)->y);
}
...
Component* Component::findUnderMouseInWindow($PointerInfo* pi) {
	...
	$var(Component, inTheSameWindow, win->findComponentAt($nc(relativeToWindow)->x, $nc(relativeToWindow)->y, INCLUDE_DISABLED));
...
$Point* Component::getMousePosition() {
	...
	$var($PointerInfo, pi, $cast($PointerInfo, $AccessController::doPrivileged($$new($Component$2, this))));
	$synchronized(getTreeLock()) {
...
bool Component::isShowing() {
	if (this->visible && (this->peer != nullptr)) {
		$var($Container, parent, this->parent);
		return (parent == nullptr) || parent->isShowing();
	}
	return false;
}
...
void Component::enableInputMethods(bool enable) {
	$useLocalObjectStack();
	if (enable) {
		if ((this->eventMask & $AWTEvent::INPUT_METHODS_ENABLED_MASK) != 0) {
			return;
		}
		if (isFocusOwner()) {
...
void Component::show() {
	...
	if (!this->visible) {
		$synchronized(getTreeLock()) {
			...
			if (this->componentListener != nullptr || (this->eventMask & $AWTEvent::COMPONENT_EVENT_MASK) != 0 || $Toolkit::enabledOnToolkit($AWTEvent::COMPONENT_EVENT_MASK)) {
...
$Color* Component::getForeground() {
	...
	$var($Container, parent, this->parent);
	return (parent != nullptr) ? parent->getForeground() : ($Color*)nullptr;
}
...
void Component::setForeground($Color* c$renamed) {
	...
	firePropertyChange("foreground"_s, oldColor, c);
}
...
$Color* Component::getBackground() {
	...
	$var($Container, parent, this->parent);
	return (parent != nullptr) ? parent->getBackground() : ($Color*)nullptr;
}
...
void Component::setBackground($Color* c$renamed) {
	...
	firePropertyChange("background"_s, oldColor, c);
}
...
$Font* Component::getFont_NoClientCode() {
	...
	$var($Container, parent, this->parent);
	return (parent != nullptr) ? parent->getFont_NoClientCode() : ($Font*)nullptr;
}
...
void Component::setFont($Font* f$renamed) {
	...
	firePropertyChange("font"_s, oldFont, newFont);
	if (f != oldFont && (oldFont == nullptr || !oldFont->equals(f))) {
		invalidateIfValid();
	}
}
...
void Component::setLocale($Locale* l) {
	...
	$set(this, locale, l);
	firePropertyChange("locale"_s, oldValue, l);
	invalidateIfValid();
}
	...
	if (peer != nullptr && isShowing()) {
		if ($instanceOf($LightweightPeer, peer)) {
			...
			$var($Point, pt, $nc($nc(host)->peer)->getLocationOnScreen());
			{
				...
				for (; !$equals(c, host); $assign(c, $nc(c)->getContainer())) {
					$nc(pt)->x += $nc(c)->x;
					pt->y += c->y;
				}
			}
...
void Component::reshapeNativePeer(int32_t x, int32_t y, int32_t width, int32_t height, int32_t op) {
	...
	int32_t nativeY = y;
	{
		...
		for (; (c != nullptr) && ($instanceOf($LightweightPeer, c->peer)); $assign(c, c->parent)) {
			nativeX += c->x;
			nativeY += c->y;
		}
	}
...
void Component::notifyNewBounds(bool resized, bool moved) {
	...
	} else if ($instanceOf($Container, this) && $cast($Container, this)->countComponents() > 0) {
		bool enabledOnToolkit = $Toolkit::enabledOnToolkit($AWTEvent::HIERARCHY_BOUNDS_EVENT_MASK);
		if (resized) {
			$cast($Container, this)->createChildHierarchyEvents($HierarchyEvent::ANCESTOR_RESIZED, 0, enabledOnToolkit);
		}
		if (moved) {
			$cast($Container, this)->createChildHierarchyEvents($HierarchyEvent::ANCESTOR_MOVED, 0, enabledOnToolkit);
		}
	}
}
...
void Component::setPreferredSize($Dimension* preferredSize) {
	...
	firePropertyChange("preferredSize"_s, old, preferredSize);
}
...
void Component::setMinimumSize($Dimension* minimumSize) {
	...
	firePropertyChange("minimumSize"_s, old, minimumSize);
}
...
void Component::setMaximumSize($Dimension* maximumSize) {
	...
	firePropertyChange("maximumSize"_s, old, maximumSize);
}
...
$Graphics* Component::getGraphics() {
	...
	if ($instanceOf($LightweightPeer, this->peer)) {
		...
		if ($instanceOf($ConstrainableGraphics, g)) {
			$cast($ConstrainableGraphics, g)->constrain(this->x, this->y, this->width, this->height);
		} else {
		...
		$var($ComponentPeer, peer, this->peer);
		return (peer != nullptr) ? peer->getGraphics() : ($Graphics*)nullptr;
	}
}
...
void Component::paintAll($Graphics* g) {
	...
	if (isShowing()) {
		...
		$$nc($GraphicsCallback$PeerPaintCallback::getInstance())->runOneComponent(this, var$0, g, $($nc(g)->getClip()), $GraphicsCallback::LIGHTWEIGHTS | $GraphicsCallback::HEAVYWEIGHTS);
	}
}
...
void Component::printAll($Graphics* g) {
	...
	if (isShowing()) {
		$var($Rectangle, var$0, $new($Rectangle, 0, 0, this->width, this->height));
		$$nc($GraphicsCallback$PeerPrintCallback::getInstance())->runOneComponent(this, var$0, g, $($nc(g)->getClip()), $GraphicsCallback::LIGHTWEIGHTS | $GraphicsCallback::HEAVYWEIGHTS);
	}
}
...
$Insets* Component::getInsets_NoClientCode() {
	...
	if ($instanceOf($ContainerPeer, peer)) {
		return $cast($Insets, $$nc($cast($ContainerPeer, peer)->getInsets())->clone());
	}
...
$Image* Component::createImage(int32_t width, int32_t height) {
	...
	if ($instanceOf($LightweightPeer, peer)) {
		...
		return (peer != nullptr) ? peer->createImage(width, height) : ($Image*)nullptr;
	}
}
...
$VolatileImage* Component::createVolatileImage(int32_t width, int32_t height) {
	...
	if ($instanceOf($LightweightPeer, peer)) {
		...
		return (peer != nullptr) ? peer->createVolatileImage(width, height) : ($VolatileImage*)nullptr;
	}
}
...
void Component::dispatchEventImpl($AWTEvent* e$renamed) {
	...
	if (compContext != nullptr && !compContext->equals($($AppContext::getAppContext()))) {
		$init($PlatformLogger$Level);
		if ($nc(Component::eventLog)->isLoggable($PlatformLogger$Level::FINE)) {
			Component::eventLog->fine($$str({"Event "_s, e, " is being dispatched on the wrong AppContext"_s}));
		}
	}
	$init($PlatformLogger$Level);
	if ($nc(Component::eventLog)->isLoggable($PlatformLogger$Level::FINEST)) {
		Component::eventLog->finest("{0}"_s, $$new($ObjectArray, {e}));
	}
	if (!($instanceOf($KeyEvent, e))) {
		$EventQueue::setCurrentEventAndMostRecentTime(e);
	}
	if ($instanceOf($SunDropTargetEvent, e)) {
		$cast($SunDropTargetEvent, e)->dispatch();
		return;
	}
	...
	if (($instanceOf($FocusEvent, e)) && $nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
		Component::focusLog->finest($$str({""_s, e}));
	}
	...
	if (areInputMethodsEnabled()) {
		if ((($instanceOf($InputMethodEvent, e)) && !($instanceOf($CompositionArea, this))) || ($instanceOf($InputEvent, e)) || ($instanceOf($FocusEvent, e))) {
			...
			if (inputContext != nullptr) {
				...
				if (e->isConsumed()) {
					if (($instanceOf($FocusEvent, e)) && $nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
						Component::focusLog->finest($$str({"3579: Skipping "_s, e}));
					}
					return;
				}
			}
		...
		if (inputContext != nullptr && $instanceOf($1InputContext, inputContext)) {
			$cast($1InputContext, inputContext)->disableNativeIM();
		}
	}
	{
		...
		case $KeyEvent::KEY_RELEASED:
			$assign(p, $cast($Container, (($instanceOf($Container, this)) ? this : $cast(Component, this->parent))));
			if (p != nullptr) {
				p->preProcessKeyEvent($cast($KeyEvent, e));
				if (e->isConsumed()) {
					if ($nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
						Component::focusLog->finest("Pre-process consumed event"_s);
					}
					return;
				}
			}
	...
	if (this->newEventsOnly) {
		...
		if (olde != nullptr) {
			...
			case $Event::KEY_ACTION_RELEASE:
				if (olde->key != key) {
					$cast($KeyEvent, e)->setKeyChar(olde->getKeyEventChar());
				}
				if (olde->modifiers != modifiers) {
					$cast($KeyEvent, e)->setModifiers(olde->modifiers);
				}
				break;
			default:
				break;
	...
	if ($SunToolkit::isTouchKeyboardAutoShowEnabled() && ($instanceOf($SunToolkit, toolkit)) && (($instanceOf($MouseEvent, e)) || ($instanceOf($FocusEvent, e)))) {
		$cast($SunToolkit, toolkit)->showOrHideTouchKeyboard(this, e);
	}
}
...
bool Component::dispatchMouseWheelToAncestor($MouseWheelEvent* e) {
	...
	$init($PlatformLogger$Level);
	if ($nc(Component::eventLog)->isLoggable($PlatformLogger$Level::FINEST)) {
		Component::eventLog->finest("dispatchMouseWheelToAncestor"_s);
		Component::eventLog->finest($$str({"orig event src is of "_s, $$nc(e->getSource())->getClass()}));
	}
	$synchronized(getTreeLock()) {
		...
		if (Component::eventLog->isLoggable($PlatformLogger$Level::FINEST)) {
			Component::eventLog->finest($$str({"new event src is "_s, $nc($of(anc))->getClass()}));
		}
		if (anc != nullptr && anc->eventEnabled(e)) {
			...
			$assign(newMWE, $new($MouseWheelEvent, anc, var$2, var$3, var$4, newX, newY, var$5, var$6, var$7, var$8, var$9, var$10, var$11, e->getPreciseWheelRotation()));
			$cast($AWTEvent, e)->copyPrivateDataInto(newMWE);
...
bool Component::areInputMethodsEnabled() {
	return ((this->eventMask & $AWTEvent::INPUT_METHODS_ENABLED_MASK) != 0) && ((this->eventMask & $AWTEvent::KEY_EVENT_MASK) != 0 || this->keyListener != nullptr);
}
...
bool Component::eventTypeEnabled(int32_t type) {
	...
	case $ComponentEvent::COMPONENT_HIDDEN:
		if ((this->eventMask & $AWTEvent::COMPONENT_EVENT_MASK) != 0 || this->componentListener != nullptr) {
			return true;
		}
		break;
	case $FocusEvent::FOCUS_GAINED:
	case $FocusEvent::FOCUS_LOST:
		if ((this->eventMask & $AWTEvent::FOCUS_EVENT_MASK) != 0 || this->focusListener != nullptr) {
			return true;
		}
		break;
	case $KeyEvent::KEY_PRESSED:
	...
	case $KeyEvent::KEY_TYPED:
		if ((this->eventMask & $AWTEvent::KEY_EVENT_MASK) != 0 || this->keyListener != nullptr) {
			return true;
		}
		break;
	...
	case $MouseEvent::MOUSE_CLICKED:
		if ((this->eventMask & $AWTEvent::MOUSE_EVENT_MASK) != 0 || this->mouseListener != nullptr) {
			return true;
		}
		break;
	case $MouseEvent::MOUSE_MOVED:
	case $MouseEvent::MOUSE_DRAGGED:
		if ((this->eventMask & $AWTEvent::MOUSE_MOTION_EVENT_MASK) != 0 || this->mouseMotionListener != nullptr) {
			return true;
		}
		break;
	case $MouseEvent::MOUSE_WHEEL:
		if ((this->eventMask & $AWTEvent::MOUSE_WHEEL_EVENT_MASK) != 0 || this->mouseWheelListener != nullptr) {
			return true;
		}
		break;
	case $InputMethodEvent::INPUT_METHOD_TEXT_CHANGED:
	case $InputMethodEvent::CARET_POSITION_CHANGED:
		if ((this->eventMask & $AWTEvent::INPUT_METHOD_EVENT_MASK) != 0 || this->inputMethodListener != nullptr) {
			return true;
		}
		break;
	case $HierarchyEvent::HIERARCHY_CHANGED:
		if ((this->eventMask & $AWTEvent::HIERARCHY_EVENT_MASK) != 0 || this->hierarchyListener != nullptr) {
			return true;
		}
		break;
	case $HierarchyEvent::ANCESTOR_MOVED:
	case $HierarchyEvent::ANCESTOR_RESIZED:
		if ((this->eventMask & $AWTEvent::HIERARCHY_BOUNDS_EVENT_MASK) != 0 || this->hierarchyBoundsListener != nullptr) {
			return true;
		}
		break;
	case $ActionEvent::ACTION_PERFORMED:
		if ((this->eventMask & $AWTEvent::ACTION_EVENT_MASK) != 0) {
			return true;
		}
		break;
	case $TextEvent::TEXT_VALUE_CHANGED:
		if ((this->eventMask & $AWTEvent::TEXT_EVENT_MASK) != 0) {
			return true;
		}
		break;
	case $ItemEvent::ITEM_STATE_CHANGED:
		if ((this->eventMask & $AWTEvent::ITEM_EVENT_MASK) != 0) {
			return true;
		}
		break;
	case $AdjustmentEvent::ADJUSTMENT_VALUE_CHANGED:
		if ((this->eventMask & $AWTEvent::ADJUSTMENT_EVENT_MASK) != 0) {
			return true;
		}
		break;
	default:
...
$ComponentListenerArray* Component::getComponentListeners() {
	$synchronized(this) {
		...
		return $cast($ComponentListenerArray, getListeners($ComponentListener::class$));
	}
}
...
$FocusListenerArray* Component::getFocusListeners() {
	$synchronized(this) {
		$load($FocusListener);
		return $cast($FocusListenerArray, getListeners($FocusListener::class$));
	}
}
...
void Component::addHierarchyListener($HierarchyListener* l) {
	...
	$synchronized(this) {
		notifyAncestors = (this->hierarchyListener == nullptr && (this->eventMask & $AWTEvent::HIERARCHY_EVENT_MASK) == 0);
...
$HierarchyListenerArray* Component::getHierarchyListeners() {
	$synchronized(this) {
		...
		return $cast($HierarchyListenerArray, getListeners($HierarchyListener::class$));
	}
}
...
void Component::addHierarchyBoundsListener($HierarchyBoundsListener* l) {
	...
	bool notifyAncestors = false;
	$synchronized(this) {
		notifyAncestors = (this->hierarchyBoundsListener == nullptr && (this->eventMask & $AWTEvent::HIERARCHY_BOUNDS_EVENT_MASK) == 0);
...
int32_t Component::numListening(int64_t mask) {
	...
	if ($nc(Component::eventLog)->isLoggable($PlatformLogger$Level::FINE)) {
		if ((mask != $AWTEvent::HIERARCHY_EVENT_MASK) && (mask != $AWTEvent::HIERARCHY_BOUNDS_EVENT_MASK)) {
			Component::eventLog->fine("Assertion failed"_s);
		}
	}
	if ((mask == $AWTEvent::HIERARCHY_EVENT_MASK && (this->hierarchyListener != nullptr || (this->eventMask & $AWTEvent::HIERARCHY_EVENT_MASK) != 0)) || (mask == $AWTEvent::HIERARCHY_BOUNDS_EVENT_MASK && (this->hierarchyBoundsListener != nullptr || (this->eventMask & $AWTEvent::HIERARCHY_BOUNDS_EVENT_MASK) != 0))) {
		return 1;
	} else {
		return 0;
	}
...
int32_t Component::createHierarchyEvents(int32_t id, Component* changed, $Container* changedParent, int64_t changeFlags, bool enabledOnToolkit) {
	...
	switch (id) {
	case $HierarchyEvent::HIERARCHY_CHANGED:
		if (this->hierarchyListener != nullptr || (this->eventMask & $AWTEvent::HIERARCHY_EVENT_MASK) != 0 || enabledOnToolkit) {
	...
	case $HierarchyEvent::ANCESTOR_RESIZED:
		...
		if ($nc(Component::eventLog)->isLoggable($PlatformLogger$Level::FINE)) {
			if (changeFlags != 0) {
				Component::eventLog->fine("Assertion (changeFlags == 0) failed"_s);
			}
		}
		if (this->hierarchyBoundsListener != nullptr || (this->eventMask & $AWTEvent::HIERARCHY_BOUNDS_EVENT_MASK) != 0 || enabledOnToolkit) {
	...
	default:
		$init($PlatformLogger$Level);
		if ($nc(Component::eventLog)->isLoggable($PlatformLogger$Level::FINE)) {
			Component::eventLog->fine("This code must never be reached"_s);
		}
		break;
	}
	return 0;
...
$HierarchyBoundsListenerArray* Component::getHierarchyBoundsListeners() {
	$synchronized(this) {
		...
		return $cast($HierarchyBoundsListenerArray, getListeners($HierarchyBoundsListener::class$));
	}
}
...
$KeyListenerArray* Component::getKeyListeners() {
	$synchronized(this) {
		$load($KeyListener);
		return $cast($KeyListenerArray, getListeners($KeyListener::class$));
	}
}
...
$MouseListenerArray* Component::getMouseListeners() {
	$synchronized(this) {
		$load($MouseListener);
		return $cast($MouseListenerArray, getListeners($MouseListener::class$));
	}
}
...
$MouseMotionListenerArray* Component::getMouseMotionListeners() {
	$synchronized(this) {
		...
		return $cast($MouseMotionListenerArray, getListeners($MouseMotionListener::class$));
	}
}
...
$MouseWheelListenerArray* Component::getMouseWheelListeners() {
	$synchronized(this) {
		...
		return $cast($MouseWheelListenerArray, getListeners($MouseWheelListener::class$));
	}
}
...
$InputMethodListenerArray* Component::getInputMethodListeners() {
	$synchronized(this) {
		...
		return $cast($InputMethodListenerArray, getListeners($InputMethodListener::class$));
	}
}
								...
								if (listenerType == $MouseWheelListener::class$) {
									...
									if (listenerType == $InputMethodListener::class$) {
										...
										if (listenerType == $PropertyChangeListener::class$) {
											return $cast($EventListenerArray, getPropertyChangeListeners());
										}
									}
								}
							}
...
void Component::enableEvents(int64_t eventsToEnable) {
	...
	$synchronized(this) {
		if ((eventsToEnable & $AWTEvent::HIERARCHY_EVENT_MASK) != 0 && this->hierarchyListener == nullptr && (this->eventMask & $AWTEvent::HIERARCHY_EVENT_MASK) == 0) {
			notifyAncestors |= $AWTEvent::HIERARCHY_EVENT_MASK;
		}
		if ((eventsToEnable & $AWTEvent::HIERARCHY_BOUNDS_EVENT_MASK) != 0 && this->hierarchyBoundsListener == nullptr && (this->eventMask & $AWTEvent::HIERARCHY_BOUNDS_EVENT_MASK) == 0) {
...
void Component::disableEvents(int64_t eventsToDisable) {
	int64_t notifyAncestors = 0;
	$synchronized(this) {
		if ((eventsToDisable & $AWTEvent::HIERARCHY_EVENT_MASK) != 0 && this->hierarchyListener == nullptr && (this->eventMask & $AWTEvent::HIERARCHY_EVENT_MASK) != 0) {
			notifyAncestors |= $AWTEvent::HIERARCHY_EVENT_MASK;
		}
		if ((eventsToDisable & $AWTEvent::HIERARCHY_BOUNDS_EVENT_MASK) != 0 && this->hierarchyBoundsListener == nullptr && (this->eventMask & $AWTEvent::HIERARCHY_BOUNDS_EVENT_MASK) != 0) {
			notifyAncestors |= $AWTEvent::HIERARCHY_BOUNDS_EVENT_MASK;
		}
...
bool Component::checkCoalescing() {
	...
	$synchronized(Component::coalesceMap) {
		$var($Boolean, value, $cast($Boolean, Component::coalesceMap->get(clazz)));
		if (value != nullptr) {
			return value->booleanValue();
		}
		$var($Boolean, enabled, $cast($Boolean, $AccessController::doPrivileged($$new($Component$3, this, clazz))));
		Component::coalesceMap->put(clazz, enabled);
...
bool Component::isCoalesceEventsOverriden($Class* clazz) {
	...
	if ($nc(superclass)->getClassLoader() != nullptr) {
		$var($Boolean, value, $cast($Boolean, Component::coalesceMap->get(superclass)));
		if (value == nullptr) {
			if (isCoalesceEventsOverriden(superclass)) {
				Component::coalesceMap->put(superclass, $($Boolean::valueOf(true)));
				return true;
			}
...
void Component::addNotify() {
	...
	$synchronized(getTreeLock()) {
		...
		if (peer == nullptr || $instanceOf($LightweightPeer, peer)) {
			...
			if (this->parent != nullptr) {
				int64_t mask = 0;
				if ((this->mouseListener != nullptr) || ((this->eventMask & $AWTEvent::MOUSE_EVENT_MASK) != 0)) {
					mask |= $AWTEvent::MOUSE_EVENT_MASK;
				}
				if ((this->mouseMotionListener != nullptr) || ((this->eventMask & $AWTEvent::MOUSE_MOTION_EVENT_MASK) != 0)) {
					mask |= $AWTEvent::MOUSE_MOTION_EVENT_MASK;
				}
				if ((this->mouseWheelListener != nullptr) || ((this->eventMask & $AWTEvent::MOUSE_WHEEL_EVENT_MASK) != 0)) {
					mask |= $AWTEvent::MOUSE_WHEEL_EVENT_MASK;
				}
				if (this->focusListener != nullptr || (this->eventMask & $AWTEvent::FOCUS_EVENT_MASK) != 0) {
					mask |= $AWTEvent::FOCUS_EVENT_MASK;
				}
				if (this->keyListener != nullptr || (this->eventMask & $AWTEvent::KEY_EVENT_MASK) != 0) {
		...
		invalidate();
		int32_t npopups = (this->popups != nullptr ? this->popups->size() : 0);
		...
		if (this->hierarchyListener != nullptr || (this->eventMask & $AWTEvent::HIERARCHY_EVENT_MASK) != 0 || $Toolkit::enabledOnToolkit($AWTEvent::HIERARCHY_EVENT_MASK)) {
	...
	$synchronized(getTreeLock()) {
		...
		if (p != nullptr) {
			...
			if ($instanceOf($Component$FlipBufferStrategy, this->bufferStrategy)) {
				$cast($Component$FlipBufferStrategy, this->bufferStrategy)->invalidate();
			}
			if (this->dropTarget != nullptr) {
...
void Component::setFocusTraversalKeys_NoIDCheck(int32_t id, $Set* keystrokes) {
	...
	$synchronized(this) {
		...
		this->focusTraversalKeys->set(id, (keystrokes != nullptr) ? $($Collections::unmodifiableSet($$new($HashSet, keystrokes))) : ($Set*)nullptr);
	}
	firePropertyChange(Component::focusTraversalKeyPropertyNames->get(id), oldKeys, keystrokes);
}
...
bool Component::requestFocusHelper(bool temporary, bool focusedWindowChangeAllowed, $FocusEvent$Cause* cause) {
	...
	if ($instanceOf($MouseEvent, currentEvent) && $SunToolkit::isSystemGenerated(currentEvent)) {
		$var(Component, source, $cast($MouseEvent, currentEvent)->getComponent());
		bool var$0 = source == nullptr;
		if (!var$0) {
			var$0 = source->getContainingWindow() == getContainingWindow();
		}
		if (var$0) {
	...
	if (!isRequestFocusAccepted(temporary, focusedWindowChangeAllowed, cause)) {
		$init($PlatformLogger$Level);
		if ($nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
			Component::focusLog->finest("requestFocus is not accepted"_s);
		}
		return false;
	}
	...
	while ((window != nullptr) && !($instanceOf($Window, window))) {
		if (!window->isVisible()) {
			$init($PlatformLogger$Level);
			if ($nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
				Component::focusLog->finest("component is recursively invisible"_s);
			}
			return false;
		}
	...
	$var($ComponentPeer, peer, this->peer);
	$var(Component, heavyweight, ($instanceOf($LightweightPeer, peer)) ? $cast(Component, getNativeContainer()) : this);
	if (heavyweight == nullptr || !heavyweight->isVisible()) {
		$init($PlatformLogger$Level);
		if ($nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
			Component::focusLog->finest("Component is not a part of visible hierarchy"_s);
		}
		return false;
	}
	...
	if (peer == nullptr) {
		$init($PlatformLogger$Level);
		if ($nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
			Component::focusLog->finest("Peer is null"_s);
		}
		return false;
	}
	int64_t time = 0;
	...
	if (!success) {
		...
		$init($PlatformLogger$Level);
		if ($nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
			Component::focusLog->finest("Peer request failed"_s);
		}
	} else {
		$init($PlatformLogger$Level);
		if ($nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
			Component::focusLog->finest($$str({"Pass for "_s, this}));
		}
	}
	return success;
}
...
bool Component::isRequestFocusAccepted(bool temporary, bool focusedWindowChangeAllowed, $FocusEvent$Cause* cause) {
	...
	if (var$0 || !isVisible()) {
		$init($PlatformLogger$Level);
		if ($nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
			Component::focusLog->finest("Not focusable or not visible"_s);
		}
		return false;
	}
	$var($ComponentPeer, peer, this->peer);
	...
	if (window == nullptr || !window->isFocusableWindow()) {
		$init($PlatformLogger$Level);
		if ($nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
			Component::focusLog->finest("Component doesn\'t have toplevel"_s);
		}
		return false;
	}
	...
	if (focusOwner == this || focusOwner == nullptr) {
		$init($PlatformLogger$Level);
		if ($nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
			Component::focusLog->finest("focus owner is null or this"_s);
		}
		return true;
	}
	$init($FocusEvent$Cause);
	if ($FocusEvent$Cause::ACTIVATION == cause) {
		$init($PlatformLogger$Level);
		if ($nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
			Component::focusLog->finest("cause is activation"_s);
		}
		return true;
	}
	...
	$init($PlatformLogger$Level);
	if ($nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
		Component::focusLog->finest("RequestFocusController returns {0}"_s, $$new($ObjectArray, {$($Boolean::valueOf(ret))}));
	}
	return ret;
}
...
bool Component::transferFocus(bool clearOnFailure) {
	...
	if ($nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINER)) {
		Component::focusLog->finer($$str({"clearOnFailure = "_s, $$str(clearOnFailure)}));
	}
	...
	if (clearOnFailure && !res) {
		if (Component::focusLog->isLoggable($PlatformLogger$Level::FINER)) {
			Component::focusLog->finer("clear global focus owner"_s);
		}
		$$nc($KeyboardFocusManager::getCurrentKeyboardFocusManager())->clearGlobalFocusOwnerPriv();
	}
	if (Component::focusLog->isLoggable($PlatformLogger$Level::FINER)) {
		Component::focusLog->finer($$str({"returning result: "_s, $$str(res)}));
	}
	return res;
}
...
Component* Component::getNextFocusCandidate() {
	...
	while (true) {
		...
		}
		{
			...
			$assign(rootAncestor, $nc(comp)->getFocusCycleRootAncestor());
		}
	}
	$init($PlatformLogger$Level);
	if ($nc(Component::focusLog)->isLoggable($PlatformLogger$Level::FINER)) {
		Component::focusLog->finer($$str({"comp = "_s, comp, ", root = "_s, rootAncestor}));
	}
	...
	if (rootAncestor != nullptr) {
		...
		if (Component::focusLog->isLoggable($PlatformLogger$Level::FINER)) {
			Component::focusLog->finer($$str({"component after is "_s, toFocus}));
		}
		if (toFocus == nullptr) {
			$assign(toFocus, policy->getDefaultComponent(rootAncestor));
			if (Component::focusLog->isLoggable($PlatformLogger$Level::FINER)) {
				Component::focusLog->finer($$str({"default component is "_s, toFocus}));
			}
		}
		if (toFocus == nullptr) {
	...
	if (Component::focusLog->isLoggable($PlatformLogger$Level::FINER)) {
		Component::focusLog->finer($$str({"Focus transfer candidate: "_s, candidate}));
	}
	return candidate;
}
...
void Component::transferFocusUpCycle() {
	...
	for ($assign(rootAncestor, getFocusCycleRootAncestor());; $assign(rootAncestor, $nc(rootAncestor)->getFocusCycleRootAncestor())) {
		bool var$0 = rootAncestor != nullptr;
		if (var$0) {
...
void Component::firePropertyChange($String* propertyName, int8_t oldValue, int8_t newValue) {
	...
	$var($Object, var$0, $Byte::valueOf(oldValue));
	firePropertyChange(propertyName, var$0, $($Byte::valueOf(newValue)));
}
...
void Component::firePropertyChange($String* propertyName, char16_t oldValue, char16_t newValue) {
	...
	$var($Object, var$0, $Character::valueOf(oldValue));
	firePropertyChange(propertyName, var$0, $($Character::valueOf(newValue)));
}
...
void Component::firePropertyChange($String* propertyName, int16_t oldValue, int16_t newValue) {
	...
	$var($Object, var$0, $Short::valueOf(oldValue));
	firePropertyChange(propertyName, var$0, $($Short::valueOf(newValue)));
}
...
void Component::firePropertyChange($String* propertyName, int64_t oldValue, int64_t newValue) {
	...
	$var($Object, var$0, $Long::valueOf(oldValue));
	firePropertyChange(propertyName, var$0, $($Long::valueOf(newValue)));
}
...
void Component::firePropertyChange($String* propertyName, float oldValue, float newValue) {
	...
	$var($Object, var$0, $Float::valueOf(oldValue));
	firePropertyChange(propertyName, var$0, $($Float::valueOf(newValue)));
}
...
void Component::firePropertyChange($String* propertyName, double oldValue, double newValue) {
	...
	$var($Object, var$0, $Double::valueOf(oldValue));
	firePropertyChange(propertyName, var$0, $($Double::valueOf(newValue)));
}
...
void Component::readObject($ObjectInputStream* s) {
	...
	if (this->popups != nullptr) {
		int32_t npopups = this->popups->size();
		for (int32_t i = 0; i < npopups; ++i) {
			$var($PopupMenu, popup, $cast($PopupMenu, this->popups->elementAt(i)));
...
void Component::setComponentOrientation($ComponentOrientation* o) {
	...
	firePropertyChange("componentOrientation"_s, oldValue, o);
	invalidateIfValid();
}
...
int32_t Component::getAccessibleIndexInParent() {
	...
	$synchronized(getTreeLock()) {
		...
		for (int32_t i = 0; i < $nc(accContext)->getAccessibleChildrenCount(); ++i) {
...
$AccessibleStateSet* Component::getAccessibleStateSet() {
	...
	$synchronized(getTreeLock()) {
		...
		if ($instanceOf($Accessible, this)) {
			$var($AccessibleContext, ac, $cast($Accessible, this)->getAccessibleContext());
			if (ac != nullptr) {
		...
		if (Component::isInstanceOf(this, "javax.swing.JComponent"_s)) {
			if ($cast($JComponent, this)->isOpaque()) {
				$init($AccessibleState);
...
bool Component::areBoundsValid() {
	...
	bool var$0 = cont == nullptr || cont->isValid();
	return var$0 || cont->getLayout() == nullptr;
}
...
void Component::applyCompoundShape($Region* shape$renamed) {
	...
	if (!areBoundsValid()) {
		...
		if ($nc(Component::mixingLog)->isLoggable($PlatformLogger$Level::FINE)) {
			...
			var$0->append(this);
			var$0->append("; areBoundsValid = "_s);
			var$0->append(areBoundsValid());
			Component::mixingLog->fine($$str(var$0));
		}
		return;
	}
	if (!isLightweight()) {
		...
		if (peer != nullptr) {
			...
			if ($nc(shape)->equals($(getNormalShape()))) {
				...
				if ($nc(Component::mixingLog)->isLoggable($PlatformLogger$Level::FINER)) {
					Component::mixingLog->fine($$str({"this = "_s, this, "; compAbsolute="_s, compAbsolute, "; shape="_s, shape}));
				}
				peer->applyShape($(shape->getTranslatedRegion(-$nc(compAbsolute)->x, -$nc(compAbsolute)->y)));
			}
		}
	}
}
...
$Point* Component::getLocationOnWindow() {
	...
	{
		$var($Container, parent, getContainer());
		for (; parent != nullptr && !($instanceOf($Window, parent)); $assign(parent, parent->getContainer())) {
...
bool Component::isNonOpaqueForMixing() {
	return this->mixingCutoutRegion != nullptr && this->mixingCutoutRegion->isEmpty();
}
...
$Region* Component::calculateCurrentShape() {
	...
	$init($PlatformLogger$Level);
	if ($nc(Component::mixingLog)->isLoggable($PlatformLogger$Level::FINE)) {
		Component::mixingLog->fine($$str({"this = "_s, this, "; normalShape="_s, s}));
	}
	...
	if (Component::mixingLog->isLoggable($PlatformLogger$Level::FINE)) {
		Component::mixingLog->fine($$str({"currentShape="_s, s}));
	}
	return s;
}
...
void Component::applyCurrentShape() {
	...
	$init($PlatformLogger$Level);
	if ($nc(Component::mixingLog)->isLoggable($PlatformLogger$Level::FINE)) {
		Component::mixingLog->fine($$str({"this = "_s, this}));
	}
...
void Component::subtractAndApplyShape($Region* s) {
	...
	$init($PlatformLogger$Level);
	if ($nc(Component::mixingLog)->isLoggable($PlatformLogger$Level::FINE)) {
		Component::mixingLog->fine($$str({"this = "_s, this, "; s="_s, s}));
	}
...
void Component::mixOnHiding(bool isLightweight) {
	...
	$synchronized(getTreeLock()) {
		$init($PlatformLogger$Level);
		if ($nc(Component::mixingLog)->isLoggable($PlatformLogger$Level::FINE)) {
			Component::mixingLog->fine($$str({"this = "_s, this, "; isLightweight = "_s, $$str(isLightweight)}));
		}
		if (!isMixingNeeded()) {
			return;
		}
...
void Component::mixOnZOrderChanging(int32_t oldZorder, int32_t newZorder) {
	...
	$synchronized(getTreeLock()) {
		...
		$init($PlatformLogger$Level);
		if ($nc(Component::mixingLog)->isLoggable($PlatformLogger$Level::FINE)) {
			Component::mixingLog->fine($$str({"this = "_s, this, "; oldZorder="_s, $$str(oldZorder), "; newZorder="_s, $$str(newZorder), "; parent="_s, parent}));
		}
		if (!isMixingNeeded()) {
			return;
		}
...
bool Component::isMixingNeeded() {
	...
	if ($SunToolkit::getSunAwtDisableMixing()) {
		...
		if ($nc(Component::mixingLog)->isLoggable($PlatformLogger$Level::FINEST)) {
			Component::mixingLog->finest($$str({"this = "_s, this, "; Mixing disabled via sun.awt.disableMixing"_s}));
		}
		return false;
	}
	if (!areBoundsValid()) {
	...
	if (window != nullptr) {
		...
		if (var$1 || window->isDisposing()) {
			...
			if ($nc(Component::mixingLog)->isLoggable($PlatformLogger$Level::FINE)) {
				...
				var$3->append("containing window = "_s);
				var$3->append(window);
				var$3->append("; has h/w descendants = "_s);
				var$3->append(window->hasHeavyweightDescendants());
				var$3->append("; has l/w descendants = "_s);
				var$3->append(window->hasLightweightDescendants());
				var$3->append("; disposing = "_s);
				var$3->append(window->isDisposing());
				Component::mixingLog->fine($$str(var$3));
			}
			return false;
		}
	} else {
		$init($PlatformLogger$Level);
		if ($nc(Component::mixingLog)->isLoggable($PlatformLogger$Level::FINE)) {
			Component::mixingLog->fine($$str({"this = "_s, this, "; containing window is null"_s}));
		}
		return false;
	}
	return true;
...
void Component::setMixingCutoutShape($Shape* shape) {
	$var($Region, region, shape == nullptr ? ($Region*)nullptr : $Region::getInstance(shape, nullptr));
	$synchronized(getTreeLock()) {
...
void Component::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(Component::LOCK, $new($Component$AWTTreeLock));
	{
		...
		$var($String, s, $cast($String, $AccessController::doPrivileged($$new($GetPropertyAction, "awt.image.incrementaldraw"_s))));
		Component::isInc = (s == nullptr || s->equals("true"_s));
		$var($String, s2, $cast($String, $AccessController::doPrivileged($$new($GetPropertyAction, "awt.image.redrawrate"_s))));
...
$Class* Component::load$($String* name, bool initialize) {
	...
	$CompoundAttribute actionmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$CompoundAttribute boundsmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	...
	$CompoundAttribute getBackgroundmethodAnnotations$$[] = {
		{"Ljava/beans/Transient;", nullptr},
		{}
	};
	$CompoundAttribute getFontmethodAnnotations$$[] = {
		{"Ljava/beans/Transient;", nullptr},
		{}
	};
	$CompoundAttribute getForegroundmethodAnnotations$$[] = {
		{"Ljava/beans/Transient;", nullptr},
		{}
	};
	$CompoundAttribute gotFocusmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$CompoundAttribute handleEventmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	...
	$CompoundAttribute isFocusTraversablemethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$CompoundAttribute isVisiblemethodAnnotations$$[] = {
		{"Ljava/beans/Transient;", nullptr},
		{}
	};
	$CompoundAttribute keyDownmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$CompoundAttribute keyUpmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	...
	$CompoundAttribute locationmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$CompoundAttribute lostFocusmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	...
	$CompoundAttribute mouseDownmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$CompoundAttribute mouseDragmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$CompoundAttribute mouseEntermethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$CompoundAttribute mouseExitmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$CompoundAttribute mouseMovemethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$CompoundAttribute mouseUpmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	...
	$CompoundAttribute nextFocusmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	...
	$CompoundAttribute resizemethodAnnotations$$$1[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	...
	$CompoundAttribute showmethodAnnotations$$$1[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$CompoundAttribute sizemethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$MethodInfo methodInfos$$[] = {
		...
		{"action", "(Ljava/awt/Event;Ljava/lang/Object;)Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, action, bool, $Event*, Object$*), nullptr, nullptr, actionmethodAnnotations$$},
		...
		{"bounds", "()Ljava/awt/Rectangle;", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, bounds, $Rectangle*), nullptr, nullptr, boundsmethodAnnotations$$},
		...
		{"deliverEvent", "(Ljava/awt/Event;)V", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, deliverEvent, void, $Event*), nullptr, nullptr, deliverEventmethodAnnotations$$},
		{"disable", "()V", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, disable, void), nullptr, nullptr, disablemethodAnnotations$$},
		...
		{"enable", "()V", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, enable, void), nullptr, nullptr, enablemethodAnnotations$$},
		{"enable", "(Z)V", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, enable, void, bool), nullptr, nullptr, enablemethodAnnotations$$$1},
		...
		{"getBackground", "()Ljava/awt/Color;", nullptr, $PUBLIC, $virtualMethod(Component, getBackground, $Color*), nullptr, nullptr, getBackgroundmethodAnnotations$$},
		...
		{"getFont", "()Ljava/awt/Font;", nullptr, $PUBLIC, $virtualMethod(Component, getFont, $Font*), nullptr, nullptr, getFontmethodAnnotations$$},
		...
		{"getForeground", "()Ljava/awt/Color;", nullptr, $PUBLIC, $virtualMethod(Component, getForeground, $Color*), nullptr, nullptr, getForegroundmethodAnnotations$$},
		...
		{"gotFocus", "(Ljava/awt/Event;Ljava/lang/Object;)Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, gotFocus, bool, $Event*, Object$*), nullptr, nullptr, gotFocusmethodAnnotations$$},
		{"handleEvent", "(Ljava/awt/Event;)Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, handleEvent, bool, $Event*), nullptr, nullptr, handleEventmethodAnnotations$$},
		{"hasFocus", "()Z", nullptr, $PUBLIC, $virtualMethod(Component, hasFocus, bool)},
		{"hide", "()V", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, hide, void), nullptr, nullptr, hidemethodAnnotations$$},
		...
		{"inside", "(II)Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, inside, bool, int32_t, int32_t), nullptr, nullptr, insidemethodAnnotations$$},
		...
		{"isFocusTraversable", "()Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, isFocusTraversable, bool), nullptr, nullptr, isFocusTraversablemethodAnnotations$$},
		...
		{"isVisible", "()Z", nullptr, $PUBLIC, $virtualMethod(Component, isVisible, bool), nullptr, nullptr, isVisiblemethodAnnotations$$},
		{"isVisible_NoClientCode", "()Z", nullptr, $FINAL, $method(Component, isVisible_NoClientCode, bool)},
		{"keyDown", "(Ljava/awt/Event;I)Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, keyDown, bool, $Event*, int32_t), nullptr, nullptr, keyDownmethodAnnotations$$},
		{"keyUp", "(Ljava/awt/Event;I)Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, keyUp, bool, $Event*, int32_t), nullptr, nullptr, keyUpmethodAnnotations$$},
		{"layout", "()V", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, layout, void), nullptr, nullptr, layoutmethodAnnotations$$},
		...
		{"locate", "(II)Ljava/awt/Component;", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, locate, Component*, int32_t, int32_t), nullptr, nullptr, locatemethodAnnotations$$},
		{"location", "()Ljava/awt/Point;", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, location, $Point*), nullptr, nullptr, locationmethodAnnotations$$},
		{"location_NoClientCode", "()Ljava/awt/Point;", nullptr, $PRIVATE, $method(Component, location_NoClientCode, $Point*)},
		{"lostFocus", "(Ljava/awt/Event;Ljava/lang/Object;)Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, lostFocus, bool, $Event*, Object$*), nullptr, nullptr, lostFocusmethodAnnotations$$},
		{"minimumSize", "()Ljava/awt/Dimension;", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, minimumSize, $Dimension*), nullptr, nullptr, minimumSizemethodAnnotations$$},
		...
		{"mouseDown", "(Ljava/awt/Event;II)Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, mouseDown, bool, $Event*, int32_t, int32_t), nullptr, nullptr, mouseDownmethodAnnotations$$},
		{"mouseDrag", "(Ljava/awt/Event;II)Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, mouseDrag, bool, $Event*, int32_t, int32_t), nullptr, nullptr, mouseDragmethodAnnotations$$},
		{"mouseEnter", "(Ljava/awt/Event;II)Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, mouseEnter, bool, $Event*, int32_t, int32_t), nullptr, nullptr, mouseEntermethodAnnotations$$},
		{"mouseExit", "(Ljava/awt/Event;II)Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, mouseExit, bool, $Event*, int32_t, int32_t), nullptr, nullptr, mouseExitmethodAnnotations$$},
		{"mouseMove", "(Ljava/awt/Event;II)Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, mouseMove, bool, $Event*, int32_t, int32_t), nullptr, nullptr, mouseMovemethodAnnotations$$},
		{"mouseUp", "(Ljava/awt/Event;II)Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, mouseUp, bool, $Event*, int32_t, int32_t), nullptr, nullptr, mouseUpmethodAnnotations$$},
		{"move", "(II)V", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, move, void, int32_t, int32_t), nullptr, nullptr, movemethodAnnotations$$},
		{"nextFocus", "()V", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, nextFocus, void), nullptr, nullptr, nextFocusmethodAnnotations$$},
		...
		{"postEvent", "(Ljava/awt/Event;)Z", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, postEvent, bool, $Event*), nullptr, nullptr, postEventmethodAnnotations$$},
		{"postsOldMouseEvents", "()Z", nullptr, 0, $virtualMethod(Component, postsOldMouseEvents, bool)},
		{"preferredSize", "()Ljava/awt/Dimension;", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, preferredSize, $Dimension*), nullptr, nullptr, preferredSizemethodAnnotations$$},
		...
		{"reshape", "(IIII)V", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, reshape, void, int32_t, int32_t, int32_t, int32_t), nullptr, nullptr, reshapemethodAnnotations$$},
		...
		{"resize", "(II)V", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, resize, void, int32_t, int32_t), nullptr, nullptr, resizemethodAnnotations$$},
		{"resize", "(Ljava/awt/Dimension;)V", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, resize, void, $Dimension*), nullptr, nullptr, resizemethodAnnotations$$$1},
		...
		{"show", "()V", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, show, void), nullptr, nullptr, showmethodAnnotations$$},
		{"show", "(Z)V", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, show, void, bool), nullptr, nullptr, showmethodAnnotations$$$1},
		{"size", "()Ljava/awt/Dimension;", nullptr, $PUBLIC | $DEPRECATED, $virtualMethod(Component, size, $Dimension*), nullptr, nullptr, sizemethodAnnotations$$},
	...
	$loadClass(Component, name, initialize, &classInfo$$, Component::clinit$, []($Class* clazz) -> $Object* {
