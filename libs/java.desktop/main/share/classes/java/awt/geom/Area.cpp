...
void Area::init$($Shape* s) {
	if ($instanceOf(Area, s)) {
		$set(this, curves, $cast(Area, s)->curves);
	} else {
...
bool Area::isPolygonal() {
	...
	while ($nc(enum_)->hasMoreElements()) {
		if ($$sure($Curve, enum_->nextElement())->getOrder() > 1) {
			return false;
		}
	}
	return true;
...
bool Area::isRectangular() {
	...
	$var($Curve, c1, $cast($Curve, this->curves->get(1)));
	$var($Curve, c2, $cast($Curve, this->curves->get(2)));
	bool var$0 = $nc(c1)->getOrder() != 1;
	if (var$0 || $nc(c2)->getOrder() != 1) {
		return false;
	}
	double var$2 = c1->getXTop();
	...
	double var$5 = c1->getYTop();
	bool var$4 = var$5 != $nc(c2)->getYTop();
	if (!var$4) {
		double var$6 = c1->getYBot();
		var$4 = var$6 != c2->getYBot();
	}
	if (var$4) {
		return false;
	}
...
bool Area::isSingular() {
	...
	$var($Enumeration, enum_, this->curves->elements());
	$nc(enum_)->nextElement();
	while (enum_->hasMoreElements()) {
		if ($$sure($Curve, enum_->nextElement())->getOrder() == 0) {
			return false;
		}
	}
	return true;
...
$Rectangle2D* Area::getCachedBounds() {
	...
	if ($nc(this->curves)->size() > 0) {
		$var($Curve, c, $cast($Curve, this->curves->get(0)));
		double var$0 = $nc(c)->getX0();
		r->setRect(var$0, c->getY0(), 0, 0);
		for (int32_t i = 1; i < $nc(this->curves)->size(); ++i) {
			$$sure($Curve, this->curves->get(i))->enlarge(r);
		}
	}
...
$Class* Area::load$($String* name, bool initialize) {
	...
	$loadClass(Area, name, initialize, &classInfo$$, Area::clinit$, []($Class* clazz) -> $Object* {
