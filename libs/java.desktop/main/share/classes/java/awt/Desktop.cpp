...
$Class* Desktop$$Lambda$lambda$moveToTrash$0::load$($String* name, bool initialize) {
	...
	$loadClass(Desktop$$Lambda$lambda$moveToTrash$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Desktop$$Lambda$lambda$moveToTrash$0);
	});
	return class$;
}
...
void Desktop::init$() {
	...
	if ($instanceOf($SunToolkit, defaultToolkit)) {
		$set(this, peer, $cast($SunToolkit, defaultToolkit)->createDesktopPeer(this));
	}
}
...
bool Desktop::isDesktopSupported() {
	$var($Toolkit, defaultToolkit, $Toolkit::getDefaultToolkit());
	if ($instanceOf($SunToolkit, defaultToolkit)) {
		return $cast($SunToolkit, defaultToolkit)->isDesktopSupported();
	}
	return false;
}
...
void Desktop::browseFileDirectory($File* file$renamed) {
	...
	if (parentFile == nullptr || !parentFile->exists()) {
...
bool Desktop::moveToTrash($File* file$renamed) {
	...
	$AccessController::doPrivileged($cast($PrivilegedAction, $$new(Desktop$$Lambda$lambda$moveToTrash$0, finalFile)));
...
$Class* Desktop::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.awt.Desktop$$Lambda$lambda$moveToTrash$0")) {
	...
	$loadClass(Desktop, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Desktop);
	});
	return class$;
}
