...
void KeyboardFocusManager::setGlobalFocusOwner($Component* focusOwner) {
	...
	if (focusOwner == nullptr || focusOwner->isFocusable()) {
		$synchronized(KeyboardFocusManager::class$) {
			checkKFMSecurity();
			...
			if (var$0) {
				$var($Container, rootAncestor, $nc(focusOwner)->getFocusCycleRootAncestor());
...
void KeyboardFocusManager::clearGlobalFocusOwnerPriv() {
	...
	$AccessController::doPrivileged($$new($KeyboardFocusManager$2, this));
}
...
void KeyboardFocusManager::setNativeFocusOwner($Component* comp) {
	...
	if ($nc(KeyboardFocusManager::focusLog)->isLoggable($PlatformLogger$Level::FINEST)) {
		KeyboardFocusManager::focusLog->finest("Calling peer {0} setCurrentFocusOwner for {1}"_s, $$new($ObjectArray, {
			$($String::valueOf(this->peer)),
			$($String::valueOf(comp))
		}));
	}
...
void KeyboardFocusManager::setGlobalPermanentFocusOwner($Component* permanentFocusOwner) {
	...
	bool shouldFire = false;
	if (permanentFocusOwner == nullptr || permanentFocusOwner->isFocusable()) {
		$synchronized(KeyboardFocusManager::class$) {
			checkKFMSecurity();
...
void KeyboardFocusManager::setGlobalFocusedWindow($Window* focusedWindow) {
	...
	bool shouldFire = false;
	if (focusedWindow == nullptr || focusedWindow->isFocusableWindow()) {
		$synchronized(KeyboardFocusManager::class$) {
			checkKFMSecurity();
...
void KeyboardFocusManager::setGlobalActiveWindow($Window* activeWindow) {
	...
	$synchronized(KeyboardFocusManager::class$) {
		...
		if ($nc(KeyboardFocusManager::focusLog)->isLoggable($PlatformLogger$Level::FINER)) {
			KeyboardFocusManager::focusLog->finer($$str({"Setting global active window to "_s, activeWindow, ", old active "_s, oldActiveWindow}));
		}
		try {
...
void KeyboardFocusManager::setDefaultFocusTraversalKeys(int32_t id, $Set* keystrokes) {
	...
	$synchronized(this) {
		...
		this->defaultFocusTraversalKeys->set(id, $($Collections::unmodifiableSet($$new($HashSet, keystrokes))));
	}
	firePropertyChange(KeyboardFocusManager::defaultFocusTraversalKeyPropertyNames->get(id), oldKeys, keystrokes);
}
...
void KeyboardFocusManager::setGlobalCurrentFocusCycleRootPriv($Container* newFocusCycleRoot) {
	$beforeCallerSensitive();
	$AccessController::doPrivileged($$new($KeyboardFocusManager$3, this, newFocusCycleRoot));
}
...
$List* KeyboardFocusManager::getKeyEventDispatchers() {
	$synchronized(this) {
		return (this->keyEventDispatchers != nullptr) ? $cast($List, this->keyEventDispatchers->clone()) : ($List*)nullptr;
	}
}
...
$List* KeyboardFocusManager::getKeyEventPostProcessors() {
	return (this->keyEventPostProcessors != nullptr) ? $cast($List, this->keyEventPostProcessors->clone()) : ($List*)nullptr;
}
...
$Component* KeyboardFocusManager::getMostRecentFocusOwner($Window* window) {
	...
	$synchronized(class$) {
		...
		return weakValue == nullptr ? ($Component*)nullptr : $cast($Component, weakValue->get());
	}
}
...
bool KeyboardFocusManager::processSynchronousLightweightTransfer($Component* heavyweight, $Component* descendant$renamed, bool temporary, bool focusedWindowChangeAllowed, int64_t time) {
	...
	if (parentWindow == nullptr || !parentWindow->syncLWRequests) {
		return false;
	}
	...
	try {
		...
		$synchronized($Component::LOCK) {
			if (currentFocusOwnerEvent != nullptr && currentFocusOwner != nullptr) {
				$cast($AWTEvent, currentFocusOwnerEvent)->isPosted = true;
			...
			if (newFocusOwnerEvent != nullptr && descendant != nullptr) {
				$cast($AWTEvent, newFocusOwnerEvent)->isPosted = true;
...
int32_t KeyboardFocusManager::shouldNativelyFocusHeavyweight($Component* heavyweight, $Component* descendant$renamed, bool temporary, bool focusedWindowChangeAllowed, int64_t time, $FocusEvent$Cause* cause) {
	...
	if ($nc(KeyboardFocusManager::log)->isLoggable($PlatformLogger$Level::FINE)) {
		...
		if (time == 0) {
			KeyboardFocusManager::log->fine("Assertion (time != 0) failed"_s);
		}
	}
	if (descendant == nullptr) {
		$assign(descendant, heavyweight);
	...
	if ($nc(KeyboardFocusManager::focusLog)->isLoggable($PlatformLogger$Level::FINER)) {
		KeyboardFocusManager::focusLog->finer("SNFH for {0} in {1}"_s, $$new($ObjectArray, {
	...
	if (KeyboardFocusManager::focusLog->isLoggable($PlatformLogger$Level::FINEST)) {
		KeyboardFocusManager::focusLog->finest("0. Current focus owner {0}"_s, $$new($ObjectArray, {$($String::valueOf(currentFocusOwner))}));
		KeyboardFocusManager::focusLog->finest("0. Native focus owner {0}"_s, $$new($ObjectArray, {$($String::valueOf(nativeFocusOwner))}));
		KeyboardFocusManager::focusLog->finest("0. Native focused window {0}"_s, $$new($ObjectArray, {$($String::valueOf(nativeFocusedWindow))}));
	}
	$synchronized(KeyboardFocusManager::heavyweightRequests) {
		$var($KeyboardFocusManager$HeavyweightFocusRequest, hwFocusRequest, getLastHWRequest());
		if (KeyboardFocusManager::focusLog->isLoggable($PlatformLogger$Level::FINEST)) {
			KeyboardFocusManager::focusLog->finest("Request {0}"_s, $$new($ObjectArray, {$($String::valueOf(hwFocusRequest))}));
		}
		if (hwFocusRequest == nullptr && heavyweight == nativeFocusOwner && $nc(heavyweight)->getContainingWindow() == nativeFocusedWindow) {
			if (descendant == currentFocusOwner) {
				if (KeyboardFocusManager::focusLog->isLoggable($PlatformLogger$Level::FINEST)) {
					KeyboardFocusManager::focusLog->finest("1. SNFH_FAILURE for {0}"_s, $$new($ObjectArray, {$($String::valueOf(descendant))}));
				}
			...
			if (KeyboardFocusManager::focusLog->isLoggable($PlatformLogger$Level::FINEST)) {
				KeyboardFocusManager::focusLog->finest("2. SNFH_HANDLED for {0}"_s, $$new($ObjectArray, {$($String::valueOf(descendant))}));
			}
			...
			if (KeyboardFocusManager::focusLog->isLoggable($PlatformLogger$Level::FINEST)) {
				KeyboardFocusManager::focusLog->finest($$str({"3. SNFH_HANDLED for lightweight"_s, descendant, " in "_s, heavyweight}));
			}
			return KeyboardFocusManager::SNFH_SUCCESS_HANDLED;
		} else {
			if (!focusedWindowChangeAllowed) {
				...
				if (hwFocusRequest == $KeyboardFocusManager$HeavyweightFocusRequest::CLEAR_GLOBAL_FOCUS_OWNER) {
					...
					$assign(hwFocusRequest, (size >= 2) ? $cast($KeyboardFocusManager$HeavyweightFocusRequest, KeyboardFocusManager::heavyweightRequests->get(size - 2)) : ($KeyboardFocusManager$HeavyweightFocusRequest*)nullptr);
				}
				if (focusedWindowChanged(heavyweight, (hwFocusRequest != nullptr) ? hwFocusRequest->heavyweight : $cast($Component, nativeFocusedWindow))) {
					if (KeyboardFocusManager::focusLog->isLoggable($PlatformLogger$Level::FINEST)) {
						KeyboardFocusManager::focusLog->finest($$str({"4. SNFH_FAILURE for "_s, descendant}));
					}
					return KeyboardFocusManager::SNFH_FAILURE;
				}
			}
			...
			if (KeyboardFocusManager::focusLog->isLoggable($PlatformLogger$Level::FINEST)) {
				KeyboardFocusManager::focusLog->finest($$str({"5. SNFH_PROCEED for "_s, descendant}));
			}
...
$Window* KeyboardFocusManager::markClearGlobalFocusOwner() {
	...
	$synchronized(KeyboardFocusManager::heavyweightRequests) {
		...
		$var($Component, activeWindow, (hwFocusRequest != nullptr) ? $cast($Component, $SunToolkit::getContainingWindow(hwFocusRequest->heavyweight)) : nativeFocusedWindow);
		while (activeWindow != nullptr && !(($instanceOf($Frame, activeWindow)) || ($instanceOf($Dialog, activeWindow)))) {
			$assign(activeWindow, activeWindow->getParent_NoClientCode());
		}
...
bool KeyboardFocusManager::isAutoFocusTransferEnabled() {
	$init(KeyboardFocusManager);
	$synchronized(KeyboardFocusManager::heavyweightRequests) {
		return (KeyboardFocusManager::heavyweightRequests->size() == 0) && !KeyboardFocusManager::disableRestoreFocus && (nullptr == KeyboardFocusManager::currentLightweightRequests);
	}
}
		...
		if (localLightweightRequests != nullptr) {
			...
			$var($Component, currentFocusOwner, nullptr);
			{
				...
				for (; $nc(iter)->hasNext();) {
					...
					$var($FocusEvent, currentFocusOwnerEvent, nullptr);
					if (currentFocusOwner != nullptr) {
						$assign(currentFocusOwnerEvent, $new($FocusEvent, currentFocusOwner, $FocusEvent::FOCUS_LOST, $nc(lwFocusRequest)->temporary, $nc(lwFocusRequest)->component, $nc(lwFocusRequest)->cause));
					}
					$var($FocusEvent, newFocusOwnerEvent, $new($FocusEvent, $nc(lwFocusRequest)->component, $FocusEvent::FOCUS_GAINED, $nc(lwFocusRequest)->temporary, currentFocusOwner == nullptr ? lastFocusOwner : currentFocusOwner, $nc(lwFocusRequest)->cause));
					if (currentFocusOwner != nullptr) {
						$nc($cast($AWTEvent, currentFocusOwnerEvent))->isPosted = true;
						$assign(caughtEx, dispatchAndCatchException(caughtEx, currentFocusOwner, currentFocusOwnerEvent));
					}
					$cast($AWTEvent, newFocusOwnerEvent)->isPosted = true;
					$assign(caughtEx, dispatchAndCatchException(caughtEx, lwFocusRequest->component, newFocusOwnerEvent));
...
$FocusEvent* KeyboardFocusManager::retargetFocusGained($FocusEvent* fe) {
	...
	$synchronized(KeyboardFocusManager::heavyweightRequests) {
		...
		if (currentFocusOwner != nullptr && $equals(currentFocusOwner->getContainingWindow(), source) && (hwFocusRequest == nullptr || source != hwFocusRequest->heavyweight)) {
			$init($FocusEvent$Cause);
...
$FocusEvent* KeyboardFocusManager::retargetFocusLost($FocusEvent* fe$renamed) {
	...
	$synchronized(KeyboardFocusManager::heavyweightRequests) {
		...
		if (hwFocusRequest == $KeyboardFocusManager$HeavyweightFocusRequest::CLEAR_GLOBAL_FOCUS_OWNER) {
			...
			return $new($FocusEvent, currentFocusOwner, $FocusEvent::FOCUS_LOST, temporary, $nc(lwFocusRequest)->component, $nc(lwFocusRequest)->cause);
...
$AWTEvent* KeyboardFocusManager::retargetFocusEvent($AWTEvent* event$renamed) {
	...
	if ($nc(KeyboardFocusManager::focusLog)->isLoggable($PlatformLogger$Level::FINER)) {
		if ($instanceOf($FocusEvent, event) || $instanceOf($WindowEvent, event)) {
			KeyboardFocusManager::focusLog->finer(">>> {0}"_s, $$new($ObjectArray, {$($String::valueOf(event))}));
		}
		if (KeyboardFocusManager::focusLog->isLoggable($PlatformLogger$Level::FINER) && $instanceOf($KeyEvent, event)) {
			KeyboardFocusManager::focusLog->finer("    focus owner is {0}"_s, $$new($ObjectArray, {$($String::valueOf($($nc(manager)->getGlobalFocusOwner())))}));
			KeyboardFocusManager::focusLog->finer(">>> {0}"_s, $$new($ObjectArray, {$($String::valueOf(event))}));
		}
	}
	$synchronized(KeyboardFocusManager::heavyweightRequests) {
	...
	$synchronized(KeyboardFocusManager::heavyweightRequests) {
		...
		if (hwFocusRequest != nullptr) {
			...
			if (hwFocusRequest->lightweightRequests != nullptr) {
				$var($Iterator, lwIter, hwFocusRequest->lightweightRequests->iterator());
...
$Component* KeyboardFocusManager::getHeavyweight($Component* comp) {
	$init(KeyboardFocusManager);
	if (comp == nullptr || comp->peer == nullptr) {
		return nullptr;
...
$KeyboardFocusManager$HeavyweightFocusRequest* KeyboardFocusManager::getLastHWRequest() {
	$init(KeyboardFocusManager);
	$synchronized(KeyboardFocusManager::heavyweightRequests) {
		return (KeyboardFocusManager::heavyweightRequests->size() > 0) ? $cast($KeyboardFocusManager$HeavyweightFocusRequest, KeyboardFocusManager::heavyweightRequests->getLast()) : ($KeyboardFocusManager$HeavyweightFocusRequest*)nullptr;
	}
}
...
$KeyboardFocusManager$HeavyweightFocusRequest* KeyboardFocusManager::getFirstHWRequest() {
	$init(KeyboardFocusManager);
	$synchronized(KeyboardFocusManager::heavyweightRequests) {
		return (KeyboardFocusManager::heavyweightRequests->size() > 0) ? $cast($KeyboardFocusManager$HeavyweightFocusRequest, KeyboardFocusManager::heavyweightRequests->getFirst()) : ($KeyboardFocusManager$HeavyweightFocusRequest*)nullptr;
	}
}
...
void KeyboardFocusManager::clinit$($Class* clazz) {
...
$Class* KeyboardFocusManager::load$($String* name, bool initialize) {
	...
	$loadClass(KeyboardFocusManager, name, initialize, &classInfo$$, KeyboardFocusManager::clinit$, []($Class* clazz) -> $Object* {
