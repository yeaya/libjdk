...
$Class* MetaData$javax_swing_ToolTipManager_PersistenceDelegate::load$($String* name, bool initialize) {
	...
	$loadClass(MetaData$javax_swing_ToolTipManager_PersistenceDelegate, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MetaData$javax_swing_ToolTipManager_PersistenceDelegate);
	});
	return class$;
}
