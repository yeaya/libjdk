...
$Expression* MetaData$java_util_Collections$SynchronizedSortedMap_PersistenceDelegate::instantiate(Object$* oldInstance, $Encoder* out) {
	...
	$load($Collections);
	return $new($Expression, oldInstance, $Collections::class$, "synchronizedSortedMap"_s, $$new($ObjectArray, {map}));
}
...
$Class* MetaData$java_util_Collections$SynchronizedSortedMap_PersistenceDelegate::load$($String* name, bool initialize) {
	...
	$loadClass(MetaData$java_util_Collections$SynchronizedSortedMap_PersistenceDelegate, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MetaData$java_util_Collections$SynchronizedSortedMap_PersistenceDelegate);
	});
	return class$;
}
