...
$Class* MetaData$java_awt_SystemColor_PersistenceDelegate::load$($String* name, bool initialize) {
	...
	$loadClass(MetaData$java_awt_SystemColor_PersistenceDelegate, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MetaData$java_awt_SystemColor_PersistenceDelegate);
	});
	return class$;
}
