...
$Class* BeanContextServicesSupport$BCSSChild$BCSSCServiceRef::load$($String* name, bool initialize) {
	...
	$loadClass(BeanContextServicesSupport$BCSSChild$BCSSCServiceRef, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(BeanContextServicesSupport$BCSSChild$BCSSCServiceRef);
	});
	return class$;
}
