...
void BeanContextServicesSupport$BCSSProxyServiceProvider::serviceRevoked($BeanContextServiceRevokedEvent* bcsre) {
	...
	while ($nc(i)->hasNext()) {
		...
		$$sure($BeanContextServicesSupport$BCSSChild, i->next())->revokeService(var$0, true, bcsre->isCurrentServiceInvalidNow());
	}
}
...
$Class* BeanContextServicesSupport$BCSSProxyServiceProvider::load$($String* name, bool initialize) {
	...
	$loadClass(BeanContextServicesSupport$BCSSProxyServiceProvider, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
