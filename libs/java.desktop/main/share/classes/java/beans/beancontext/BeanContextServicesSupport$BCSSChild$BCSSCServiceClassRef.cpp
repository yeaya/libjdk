...
void BeanContextServicesSupport$BCSSChild$BCSSCServiceClassRef::addRequestor(Object$* requestor, $BeanContextServiceRevokedListener* bcsrl) {
	...
	if (cbcsrl != nullptr && !cbcsrl->equals(bcsrl)) {
...
$Iterator* BeanContextServicesSupport$BCSSChild$BCSSCServiceClassRef::cloneOfEntries() {
	$useLocalObjectStack();
	return $$nc($$sure($HashMap, $nc(this->requestors)->clone())->entrySet())->iterator();
}
...
$Class* BeanContextServicesSupport$BCSSChild$BCSSCServiceClassRef::load$($String* name, bool initialize) {
	...
	$loadClass(BeanContextServicesSupport$BCSSChild$BCSSCServiceClassRef, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(BeanContextServicesSupport$BCSSChild$BCSSCServiceClassRef);
	});
	return class$;
}
