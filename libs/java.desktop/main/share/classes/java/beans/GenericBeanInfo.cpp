...
void GenericBeanInfo::init$($BeanDescriptor* beanDescriptor, $EventSetDescriptorArray* events, int32_t defaultEvent, $PropertyDescriptorArray* properties, int32_t defaultProperty, $MethodDescriptorArray* methods, $BeanInfo* targetBeanInfo) {
	...
	$set(this, targetBeanInfoRef, (targetBeanInfo != nullptr) ? $cast($Reference, $new($SoftReference, targetBeanInfo)) : ($Reference*)nullptr);
}
...
void GenericBeanInfo::init$(GenericBeanInfo* old) {
	...
	if (old->events != nullptr) {
		int32_t len = old->events->length;
		$set(this, events, $new($EventSetDescriptorArray, len));
		for (int32_t i = 0; i < len; ++i) {
			this->events->set(i, $$new($EventSetDescriptor, old->events->get(i)));
		}
	}
	this->defaultEvent = old->defaultEvent;
	if (old->properties != nullptr) {
		int32_t len = old->properties->length;
		$set(this, properties, $new($PropertyDescriptorArray, len));
		for (int32_t i = 0; i < len; ++i) {
			$var($PropertyDescriptor, oldp, old->properties->get(i));
			if ($instanceOf($IndexedPropertyDescriptor, oldp)) {
				this->properties->set(i, $$new($IndexedPropertyDescriptor, $cast($IndexedPropertyDescriptor, oldp)));
			} else {
				this->properties->set(i, $$new($PropertyDescriptor, oldp));
			}
		}
	}
	this->defaultProperty = old->defaultProperty;
	if (old->methods != nullptr) {
		int32_t len = old->methods->length;
		$set(this, methods, $new($MethodDescriptorArray, len));
		for (int32_t i = 0; i < len; ++i) {
			this->methods->set(i, $$new($MethodDescriptor, old->methods->get(i)));
		}
	}
...
$Class* GenericBeanInfo::load$($String* name, bool initialize) {
	...
	$loadClass(GenericBeanInfo, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(GenericBeanInfo);
	});
	return class$;
}
