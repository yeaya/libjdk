...
void XMLEncoder::flush() {
	...
	if (!this->preambleWritten) {
		if (this->declaration) {
			...
			var$0->append("<?xml version="_s);
			var$0->append($(quote("1.0"_s)));
			var$0->append(" encoding="_s);
			var$0->append($(quote(this->charset)));
			var$0->append("?>"_s);
			writeln($$str(var$0));
		}
		$var($StringBuilder, var$1, $new($StringBuilder));
		var$1->append("<java version="_s);
		var$1->append($(quote($($System::getProperty("java.version"_s)))));
		var$1->append(" class="_s);
		$load($XMLDecoder);
		var$1->append($(quote($($XMLDecoder::class$->getName()))));
		var$1->append(">"_s);
		writeln($$str(var$1));
	...
	while (!$nc(statements)->isEmpty()) {
		...
		if ("writeObject"_s->equals($($nc(s)->getMethodName()))) {
			outputValue($nc($(s->getArguments()))->get(0), this, true);
		} else {
...
$Statement* XMLEncoder::getMissedStatement() {
	...
	for (; $nc(i$)->hasNext();) {
		...
		for (int32_t i = 0; i < $nc(statements)->size(); ++i) {
			$load($Statement);
			if ($Statement::class$ == $$sure($Statement, statements->get(i))->getClass()) {
...
void XMLEncoder::outputValue(Object$* value$renamed, Object$* outer, bool isArgument) {
	...
	if ($instanceOf($Class, value)) {
		writeln($$str({"<class>"_s, $($cast($Class, value)->getName()), "</class>"_s}));
		return;
	}
	$var($XMLEncoder$ValueData, d, getValueData(value));
	if ($nc(d)->exp != nullptr) {
		$var($Object, target, d->exp->getTarget());
		...
		if (isArgument && $instanceOf($Field, target) && $nc(methodName)->equals("get"_s)) {
			...
			if ($Modifier::isStatic(f->getModifiers())) {
				...
				var$0->append($(quote($($nc(f->getDeclaringClass())->getName()))));
				var$0->append(" field="_s);
				var$0->append($(quote($(f->getName()))));
				var$0->append("/>"_s);
				writeln($$str(var$0));
				return;
			}
		...
		bool var$1 = primitiveType != nullptr && $equals(target, $of(value)->getClass());
		if (var$1 && $nc(methodName)->equals("new"_s)) {
			$var($String, primitiveTypeName, primitiveType->getName());
			if (primitiveType == $Character::TYPE) {
				char16_t code = $cast($Character, value)->charValue();
...
$String* XMLEncoder::createString($String* string) {
	...
	while (index < $nc(string)->length()) {
		...
		if (var$0 && $nc(this->encoder)->canEncode($(string->substring(index, index + count)))) {
...
void XMLEncoder::outputStatement($Statement* exp, Object$* outer, bool isArgument) {
	...
	if ($equals(target, outer)) {
		...
		if ($equals(target, $1Array::class$) && $nc(methodName)->equals("newInstance"_s)) {
			...
			$assign(attributes, $str({attributes, " length="_s, $(quote($($nc(args->get(1))->toString())))}));
			$assign(args, $new($ObjectArray, 0));
		} else if ($nc(target)->getClass() == $Class::class$) {
			$assign(attributes, $str({attributes, " class="_s, $(quote($($cast($Class, target)->getName())))}));
		} else {
			$nc(d)->refs = 2;
			if (d->name == nullptr) {
	...
	if (var$0 || (expression && $nc(methodName)->equals("get"_s) && $nc(args)->length == 1 && $instanceOf($Integer, args->get(0)))) {
		$assign(attributes, $str({attributes, " index="_s, $(quote($($nc(args->get(0))->toString())))}));
		$assign(args, (args->length == 1) ? $new($ObjectArray, 0) : $new($ObjectArray, {args->get(1)}));
	} else {
		bool var$1 = !expression && $nc(methodName)->startsWith("set"_s) && $nc(args)->length == 1;
		if (var$1 || (expression && $nc(methodName)->startsWith("get"_s) && $nc(args)->length == 0)) {
			...
			bool var$2 = !$nc(methodName)->equals("new"_s);
			if (var$2 && !methodName->equals("newInstance"_s)) {
...
$Class* XMLEncoder::load$($String* name, bool initialize) {
	...
	$loadClass(XMLEncoder, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
