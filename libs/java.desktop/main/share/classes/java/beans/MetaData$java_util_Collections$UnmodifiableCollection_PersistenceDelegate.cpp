...
$Expression* MetaData$java_util_Collections$UnmodifiableCollection_PersistenceDelegate::instantiate(Object$* oldInstance, $Encoder* out) {
	...
	$load($Collections);
	return $new($Expression, oldInstance, $Collections::class$, "unmodifiableCollection"_s, $$new($ObjectArray, {list}));
}
...
$Class* MetaData$java_util_Collections$UnmodifiableCollection_PersistenceDelegate::load$($String* name, bool initialize) {
	...
	$loadClass(MetaData$java_util_Collections$UnmodifiableCollection_PersistenceDelegate, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MetaData$java_util_Collections$UnmodifiableCollection_PersistenceDelegate);
	});
	return class$;
}
