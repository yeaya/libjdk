...
$Expression* MetaData$java_util_Collections$UnmodifiableRandomAccessList_PersistenceDelegate::instantiate(Object$* oldInstance, $Encoder* out) {
	...
	$load($Collections);
	return $new($Expression, oldInstance, $Collections::class$, "unmodifiableList"_s, $$new($ObjectArray, {list}));
}
...
$Class* MetaData$java_util_Collections$UnmodifiableRandomAccessList_PersistenceDelegate::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, 0, $method(MetaData$java_util_Collections$UnmodifiableRandomAccessList_PersistenceDelegate, init$, void)},
		{"instantiate", "(Ljava/lang/Object;Ljava/beans/Encoder;)Ljava/beans/Expression;", nullptr, $PROTECTED, $virtualMethod(MetaData$java_util_Collections$UnmodifiableRandomAccessList_PersistenceDelegate, instantiate, $Expression*, Object$*, $Encoder*)},
		{}
	};
	...
	$loadClass(MetaData$java_util_Collections$UnmodifiableRandomAccessList_PersistenceDelegate, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MetaData$java_util_Collections$UnmodifiableRandomAccessList_PersistenceDelegate);
	});
	return class$;
}
