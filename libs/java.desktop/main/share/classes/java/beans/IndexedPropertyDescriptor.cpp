...
void IndexedPropertyDescriptor::init$($String* propertyName, $Class* beanClass) {
	$useLocalObjectStack();
	$init($Introspector);
	$var($String, var$0, $str({$Introspector::GET_PREFIX, $($NameGenerator::capitalize(propertyName))}));
	$var($String, var$1, $str({$Introspector::SET_PREFIX, $($NameGenerator::capitalize(propertyName))}));
	$var($String, var$2, $str({$Introspector::GET_PREFIX, $($NameGenerator::capitalize(propertyName))}));
	IndexedPropertyDescriptor::init$(propertyName, beanClass, var$0, var$1, var$2, $$str({$Introspector::SET_PREFIX, $($NameGenerator::capitalize(propertyName))}));
}
...
void IndexedPropertyDescriptor::init$($Map$Entry* entry, bool bound) {
	...
	$set(this, indexedReadMethodRef, $new($MethodRef));
	$set(this, indexedWriteMethodRef, $new($MethodRef));
	$var($PropertyInfo, info, $$sure($PropertyInfo, $nc(entry)->getValue())->getIndexed());
...
$Method* IndexedPropertyDescriptor::getIndexedReadMethod() {
	$synchronized(this) {
		...
		if (indexedReadMethod == nullptr) {
			$Class* cls = getClass0();
			if (cls == nullptr || (this->indexedReadMethodName == nullptr && !this->indexedReadMethodRef->isSet())) {
				return nullptr;
			}
			$init($Introspector);
...
void IndexedPropertyDescriptor::setIndexedReadMethod0($Method* readMethod) {
	...
	$set(this, indexedReadMethodName, readMethod->getName());
	$load($Transient);
...
$Method* IndexedPropertyDescriptor::getIndexedWriteMethod() {
	$synchronized(this) {
		...
		if (indexedWriteMethod == nullptr) {
			$Class* cls = getClass0();
			if (cls == nullptr || (this->indexedWriteMethodName == nullptr && !this->indexedWriteMethodRef->isSet())) {
				return nullptr;
			}
			$Class* type = getIndexedPropertyType0();
			if (type == nullptr) {
			...
			if (indexedWriteMethod != nullptr) {
				if (!$nc(indexedWriteMethod->getReturnType())->equals($Void::TYPE)) {
...
void IndexedPropertyDescriptor::setIndexedWriteMethod0($Method* writeMethod) {
	...
	$set(this, indexedWriteMethodName, writeMethod->getName());
	$load($Transient);
...
$Class* IndexedPropertyDescriptor::getIndexedPropertyType0() {
	return (this->indexedPropertyTypeRef != nullptr) ? $cast($Class, this->indexedPropertyTypeRef->get()) : ($Class*)nullptr;
}
...
$Class* IndexedPropertyDescriptor::findIndexedPropertyType($Method* indexedReadMethod, $Method* indexedWriteMethod) {
	...
	if (indexedWriteMethod != nullptr) {
		...
		if (indexedPropertyType == nullptr || $nc(params->get(1))->isAssignableFrom(indexedPropertyType)) {
...
void IndexedPropertyDescriptor::init$($PropertyDescriptor* x, $PropertyDescriptor* y) {
	...
	if ($instanceOf(IndexedPropertyDescriptor, x)) {
		...
		$assign(tr, ix->getIndexedReadMethod());
	...
	if ($instanceOf(IndexedPropertyDescriptor, y)) {
		...
		$var($Method, yr, iy->getIndexedReadMethod());
...
void IndexedPropertyDescriptor::init$(IndexedPropertyDescriptor* old) {
	...
	$nc(this->indexedWriteMethodRef)->set($($nc(old->indexedWriteMethodRef)->get()));
	$set(this, indexedPropertyTypeRef, old->indexedPropertyTypeRef);
...
int32_t IndexedPropertyDescriptor::hashCode() {
	...
	result = 37 * result + ((this->indexedWriteMethodName == nullptr) ? 0 : this->indexedWriteMethodName->hashCode());
	result = 37 * result + ((this->indexedReadMethodName == nullptr) ? 0 : this->indexedReadMethodName->hashCode());
	result = 37 * result + ((getIndexedPropertyType() == nullptr) ? 0 : $nc(getIndexedPropertyType())->hashCode());
	return result;
}
...
void IndexedPropertyDescriptor::appendTo($StringBuilder* sb) {
	...
	$FeatureDescriptor::appendTo(sb, "indexedReadMethod"_s, $($nc(this->indexedReadMethodRef)->get()));
	$FeatureDescriptor::appendTo(sb, "indexedWriteMethod"_s, $($nc(this->indexedWriteMethodRef)->get()));
}
...
$Class* IndexedPropertyDescriptor::load$($String* name, bool initialize) {
	...
	$loadClass(IndexedPropertyDescriptor, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(IndexedPropertyDescriptor);
	});
	return class$;
}
