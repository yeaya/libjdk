...
int32_t LZWStringTable::addCharString(int16_t index, int8_t b) {
	...
	this->strHsh->set(hshidx, this->numStrings);
...
int32_t LZWStringTable::hash(int16_t index, int8_t lastbyte) {
	return $mod((((int16_t)(lastbyte << 8) ^ index) & 0xffff), LZWStringTable::HASHSIZE);
}
...
void LZWStringTable::dump($PrintStream* out) {
	...
	for (i = 258; i < this->numStrings; ++i) {
		...
		var$0->append(" strNxt["_s);
		var$0->append(i);
		var$0->append("] = "_s);
		var$0->append($nc(this->strNxt)->get(i));
		var$0->append(" strChr "_s);
		var$0->append($($Integer::toHexString($nc(this->strChr)->get(i) & 0xff)));
		var$0->append(" strLen "_s);
		var$0->append($($Integer::toHexString($nc(this->strLen)->get(i))));
		$nc(out)->println($$str(var$0));
	}
}
...
$Class* LZWStringTable::load$($String* name, bool initialize) {
	...
	$loadClass(LZWStringTable, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LZWStringTable);
	});
	return class$;
}
