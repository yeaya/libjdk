...
void GIFImageMetadataFormat::init$() {
	...
	addAttribute("LocalColorTable"_s, "sizeOfLocalColorTable"_s, $IIOMetadataFormat::DATATYPE_INTEGER, true, nullptr, $($Arrays::asList($GIFStreamMetadata::colorTableSizes)));
	...
	addAttribute("GraphicControlExtension"_s, "disposalMethod"_s, $IIOMetadataFormat::DATATYPE_STRING, true, nullptr, $($Arrays::asList($GIFImageMetadata::disposalMethodNames)));
...
void GIFImageMetadataFormat::clinit$($Class* clazz) {
...
$Class* GIFImageMetadataFormat::load$($String* name, bool initialize) {
	...
	$loadClass(GIFImageMetadataFormat, name, initialize, &classInfo$$, GIFImageMetadataFormat::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(GIFImageMetadataFormat);
	});
	return class$;
}
