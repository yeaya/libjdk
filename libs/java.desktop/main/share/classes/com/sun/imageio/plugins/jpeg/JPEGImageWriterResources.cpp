...
$ObjectArray2* JPEGImageWriterResources::getContents() {
	...
	return $new($ObjectArray2, {
		$$new($ObjectArray, {
			...
			"Only Rasters or band subsets may be written with a destination type. Destination type ignored."_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_STREAM_METADATA_IGNORED)),
			"Stream metadata ignored on write"_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_DEST_METADATA_COMP_MISMATCH)),
			"Metadata component ids incompatible with destination type. Metadata modified."_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_DEST_METADATA_JFIF_MISMATCH)),
			"Metadata JFIF settings incompatible with destination type. Metadata modified."_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_DEST_METADATA_ADOBE_MISMATCH)),
			"Metadata Adobe settings incompatible with destination type. Metadata modified."_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_IMAGE_METADATA_JFIF_MISMATCH)),
			"Metadata JFIF settings incompatible with image type. Metadata modified."_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_IMAGE_METADATA_ADOBE_MISMATCH)),
			"Metadata Adobe settings incompatible with image type. Metadata modified."_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_METADATA_NOT_JPEG_FOR_RASTER)),
			"Metadata must be JPEGMetadata when writing a Raster. Metadata ignored."_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_NO_BANDS_ON_INDEXED)),
			"Band subset not allowed for an IndexColorModel image.  Band subset ignored."_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_ILLEGAL_THUMBNAIL)),
			"Thumbnails must be simple (possibly index) RGB or grayscale.  Incompatible thumbnail ignored."_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_IGNORING_THUMBS)),
			"Thumbnails ignored for non-JFIF-compatible image."_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_FORCING_JFIF)),
			"Thumbnails require JFIF marker segment.  Missing node added to metadata."_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_THUMB_CLIPPED)),
			"Thumbnail clipped."_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_METADATA_ADJUSTED_FOR_THUMB)),
			"Metadata adjusted (made JFIF-compatible) for thumbnail."_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_NO_RGB_THUMB_AS_INDEXED)),
			"RGB thumbnail can\'t be written as indexed.  Written as RGB"_s
		}),
		$$new($ObjectArray, {
			$($Integer::toString($JPEGImageWriter::WARNING_NO_GRAY_THUMB_AS_INDEXED)),
			"Grayscale thumbnail can\'t be written as indexed.  Written as JPEG"_s
		})
	});
}
...
$Class* JPEGImageWriterResources::load$($String* name, bool initialize) {
	...
	$loadClass(JPEGImageWriterResources, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JPEGImageWriterResources);
	});
	return class$;
}
