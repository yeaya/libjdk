...
$IIOMetadata* PNGImageWriter::convertImageMetadata($IIOMetadata* inData, $ImageTypeSpecifier* imageType, $ImageWriteParam* param) {
	if ($instanceOf($PNGMetadata, inData)) {
		return $cast($PNGMetadata, $cast($PNGMetadata, inData)->clone());
	} else {
...
void PNGImageWriter::write_IHDR() {
	...
	if ($nc(this->metadata)->IHDR_interlaceMethod < 0 || this->metadata->IHDR_interlaceMethod > 1) {
...
void PNGImageWriter::write_iCCP() {
	if ($nc(this->metadata)->iCCP_present) {
		...
		if ($nc(this->metadata->iCCP_profileName)->length() > 79) {
...
void PNGImageWriter::write_PLTE() {
	...
	if ($nc(this->metadata)->PLTE_present) {
		if (this->metadata->IHDR_colorType == $PNGImageReader::PNG_COLOR_GRAY || this->metadata->IHDR_colorType == $PNGImageReader::PNG_COLOR_GRAY_ALPHA) {
		...
		for (int32_t i = 0; i < numEntries; ++i) {
			palette->set(index++, this->metadata->PLTE_red->get(i));
			palette->set(index++, $nc(this->metadata->PLTE_green)->get(i));
			palette->set(index++, $nc(this->metadata->PLTE_blue)->get(i));
		}
		cs->write(palette);
		cs->finish();
	}
...
void PNGImageWriter::write_hIST() {
	if ($nc(this->metadata)->hIST_present) {
		...
		cs->writeChars(this->metadata->hIST_histogram, 0, $nc(this->metadata->hIST_histogram)->length);
		cs->finish();
	}
}
...
void PNGImageWriter::write_tRNS() {
	if ($nc(this->metadata)->tRNS_present) {
		...
		int32_t colorType = this->metadata->IHDR_colorType;
		int32_t chunkType = this->metadata->tRNS_colorType;
		int32_t chunkRed = this->metadata->tRNS_red;
		int32_t chunkGreen = this->metadata->tRNS_green;
		int32_t chunkBlue = this->metadata->tRNS_blue;
...
void PNGImageWriter::write_bKGD() {
	if ($nc(this->metadata)->bKGD_present) {
		...
		int32_t colorType = this->metadata->IHDR_colorType & 3;
		int32_t chunkType = this->metadata->bKGD_colorType;
		int32_t chunkRed = this->metadata->bKGD_red;
		int32_t chunkGreen = this->metadata->bKGD_green;
		int32_t chunkBlue = this->metadata->bKGD_blue;
		if (colorType == $PNGImageReader::PNG_COLOR_RGB && chunkType == $PNGImageReader::PNG_COLOR_GRAY) {
			chunkType = colorType;
...
void PNGImageWriter::write_sPLT() {
	if ($nc(this->metadata)->sPLT_present) {
		...
		if ($nc(this->metadata->sPLT_paletteName)->length() > 79) {
...
void PNGImageWriter::write_tEXt() {
	...
	$var($Iterator, textIter, $nc(this->metadata->tEXt_text)->iterator());
...
void PNGImageWriter::write_iTXt() {
	...
	$var($Iterator, flagIter, $nc(this->metadata->iTXt_compressionFlag)->iterator());
	$var($Iterator, methodIter, $nc(this->metadata->iTXt_compressionMethod)->iterator());
	$var($Iterator, languageIter, $nc(this->metadata->iTXt_languageTag)->iterator());
	$var($Iterator, translatedKeywordIter, $nc(this->metadata->iTXt_translatedKeyword)->iterator());
	$var($Iterator, textIter, $nc(this->metadata->iTXt_text)->iterator());
	while ($nc(keywordIter)->hasNext()) {
		...
		cs->writeByte($$sure($Integer, $nc(methodIter)->next())->intValue());
		cs->writeBytes($$cast($String, $nc(languageIter)->next()));
		cs->writeByte(0);
		cs->write($($$sure($String, $nc(translatedKeywordIter)->next())->getBytes("UTF8"_s)));
		cs->writeByte(0);
		$var($String, text, $cast($String, $nc(textIter)->next()));
...
void PNGImageWriter::write_zTXt() {
	...
	$var($Iterator, methodIter, $nc(this->metadata->zTXt_compressionMethod)->iterator());
	$var($Iterator, textIter, $nc(this->metadata->zTXt_text)->iterator());
	while ($nc(keywordIter)->hasNext()) {
		...
		cs->writeBytes(keyword);
		cs->writeByte(0);
		int32_t compressionMethod = $$sure($Integer, $nc(methodIter)->next())->intValue();
...
void PNGImageWriter::writeUnknownChunks() {
	...
	$var($Iterator, dataIter, $nc(this->metadata->unknownChunkData)->iterator());
	while (true) {
		...
		}
		{
			...
			$var($bytes, data, $cast($bytes, $nc(dataIter)->next()));
			cs->write(data);
			cs->finish();
		}
	}
...
void PNGImageWriter::encodePass($ImageOutputStream* os, $RenderedImage* image, int32_t xOffset, int32_t yOffset, int32_t xSkip, int32_t ySkip) {
	...
	if (this->metadata->IHDR_colorType == $PNGImageReader::PNG_COLOR_GRAY_ALPHA && $instanceOf($IndexColorModel, $($nc(image)->getColorModel()))) {
		bytesPerRow *= 2;
				...
				if (this->numBands == 1) {
					for (int32_t s = xOffset; s < numSamples; s += xSkip) {
						...
						if (icm_gray_alpha != nullptr) {
							this->currRow->set(count++, this->scale0->get(icm_gray_alpha->getAlpha(0xff & samples->get(s))));
						}
					}
				} else {
					for (int32_t s = xOffset; s < numSamples; s += xSkip) {
			...
			case 16:
				for (int32_t s = xOffset; s < numSamples; s += xSkip) {
					for (int32_t b = 0; b < this->numBands; ++b) {
						...
						this->currRow->set(count++, $nc($nc(this->scalel)->get(b))->get(samples->get(s + b)));
					}
				}
				break;
			}
...
void PNGImageWriter::write_IDAT($RenderedImage* image, int32_t deflaterLevel) {
	$useLocalObjectStack();
	$var($IDATOutputStream, ios, $new($IDATOutputStream, this->stream, 0x00008000, deflaterLevel));
...
void PNGImageWriter::write($IIOMetadata* streamMetadata, $IIOImage* image, $ImageWriteParam* param) {
	...
	$var($RenderedImage, im, image->getRenderedImage());
...
$Class* PNGImageWriter::load$($String* name, bool initialize) {
	...
	$loadClass(PNGImageWriter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PNGImageWriter);
	});
	return class$;
}
