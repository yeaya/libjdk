...
void JPEGImageReader::initStatic() {
	...
	$AccessController::doPrivileged($$new($JPEGImageReader$1));
...
void JPEGImageReader::initReaderIDs($Class* iisClass, $Class* qTableClass, $Class* huffClass) {
	$init(JPEGImageReader);
	$prepareNativeStatic(initReaderIDs, void, $Class* iisClass, $Class* qTableClass, $Class* huffClass);
...
int64_t JPEGImageReader::initJPEGImageReader() {
	$prepareNative(initJPEGImageReader, int64_t);
...
void JPEGImageReader::setSource(int64_t structPointer) {
	$prepareNative(setSource, void, int64_t structPointer);
...
void JPEGImageReader::checkTablesOnly() {
	...
	if (this->debug) {
		...
		$System::out->println($$str({"length is "_s, $$str($nc(this->iis)->length())}));
	}
	...
	if (tablesOnly) {
		if (this->debug) {
			...
			$System::out->println($$str({"pos after return from native is "_s, $$str(pos)}));
		}
	...
	if (this->seekForwardOnly && !$nc(this->imagePositions)->isEmpty()) {
		$var($Long, pos, $cast($Long, this->imagePositions->get(this->imagePositions->size() - 1)));
...
void JPEGImageReader::gotoImage(int32_t imageIndex) {
	...
	if (imageIndex < this->imagePositions->size()) {
		$nc(this->iis)->seek($$sure($Long, this->imagePositions->get(imageIndex))->longValue());
	} else {
		...
		for (int32_t index = $nc(this->imagePositions)->size(); index <= imageIndex; ++index) {
			...
			if (this->seekForwardOnly) {
				$nc(this->iis)->flushBefore(pos->longValue());
			}
			if (index < imageIndex) {
				skipImage();
			}
...
bool JPEGImageReader::readImageHeader(int64_t structPointer, bool clearBuffer, bool reset) {
	$prepareNative(readImageHeader, bool, int64_t structPointer, bool clearBuffer, bool reset);
	bool $ret = $invokeNative(structPointer, clearBuffer, reset);
	$finishNative();
	return $ret;
}
...
void JPEGImageReader::setImageData(int32_t width, int32_t height, int32_t colorSpaceCode, int32_t outColorSpaceCode, int32_t numComponents, $bytes* iccData) {
	...
	if ($instanceOf($ICC_ColorSpace, this->iccCS)) {
		$assign(oldProfile, $cast($ICC_ColorSpace, this->iccCS)->getProfile());
	}
	...
	if (oldData == nullptr || !$Arrays::equals(oldData, newData)) {
		...
		try {
			$var($floats, colors, this->iccCS->fromRGB($$new($floats, {
				1.0f,
				0.0f,
				0.0f
			})));
...
void JPEGImageReader::checkColorConversion($BufferedImage* image, $ImageReadParam* param) {
	...
	case $JPEG::JCS_RGB:
		if (csType == $ColorSpace::TYPE_GRAY) {
			...
			bool var$1 = (this->iccCS == nullptr) && (!cs->isCS_sRGB());
			if (var$1 && (cm->getNumComponents() == this->numComponents)) {
				$init($JPEG);
...
void JPEGImageReader::setOutColorSpace(int64_t structPointer, int32_t id) {
	$prepareNative(setOutColorSpace, void, int64_t structPointer, int32_t id);
...
$Raster* JPEGImageReader::readInternal(int32_t imageIndex, $ImageReadParam* param, bool wantRaster) {
	...
	$var($ints, bandOffs, $JPEG::bandOffsets->get(numRasterBands - 1));
	$set(this, raster, $Raster::createInterleavedRaster(this->buffer, this->destROI->width, 1, lineSize, numRasterBands, bandOffs, nullptr));
	if (wantRaster) {
		$set(this, target, $Raster::createInterleavedRaster($DataBuffer::TYPE_BYTE, $nc(this->destROI)->width, $nc(this->destROI)->height, lineSize, numRasterBands, bandOffs, nullptr));
	} else {
	...
	if (this->debug) {
		...
		$System::out->println($$str({"numRasterBands is "_s, $$str(numRasterBands)}));
		$System::out->print("srcBands:"_s);
		for (int32_t i = 0; i < $nc(srcBands)->length; ++i) {
			$System::out->print($$str({" "_s, $$str(srcBands->get(i))}));
		}
		$System::out->println();
		$System::out->println($$str({"destination bands is "_s, this->destinationBands}));
		if (this->destinationBands != nullptr) {
			for (int32_t i = 0; i < this->destinationBands->length; ++i) {
				$System::out->print($$str({" "_s, $$str(this->destinationBands->get(i))}));
			}
			$System::out->println();
		}
		$System::out->println($$str({"sourceROI is "_s, srcROI}));
		$System::out->println($$str({"destROI is "_s, this->destROI}));
		$System::out->println($$str({"periodX is "_s, $$str(periodX)}));
		$System::out->println($$str({"periodY is "_s, $$str(periodY)}));
		$System::out->println($$str({"minProgressivePass is "_s, $$str(this->minProgressivePass)}));
		$System::out->println($$str({"maxProgressivePass is "_s, $$str(this->maxProgressivePass)}));
		$System::out->println($$str({"callbackUpdates is "_s, $$str(callbackUpdates)}));
	}
				...
				if (this->knownPassCount != JPEGImageReader::UNKNOWN) {
					...
					if ($mod(y, this->progInterval) == 0) {
						...
						if (this->debug) {
							...
							$System::out->print($$str({", y= "_s, $$str(y)}));
							$System::out->print($$str({", progInt= "_s, $$str(this->progInterval)}));
							$System::out->print($$str({", % of pass: "_s, $$str(percentOfPass)}));
							$System::out->print($$str({", rem. passes: "_s, $$str(remainingPasses)}));
							$System::out->print($$str({", prev%: "_s, $$str(this->previousPassPercentage)}));
							$System::out->print($$str({", %ToDate: "_s, $$str(this->percentToDate)}));
							$System::out->print(" "_s);
						}
...
bool JPEGImageReader::readImage(int32_t imageIndex, int64_t structPointer, $bytes* buffer, int32_t numRasterBands, $ints* srcBands, $ints* bandSizes, int32_t sourceXOffset, int32_t sourceYOffset, int32_t sourceWidth, int32_t sourceHeight, int32_t periodX, int32_t periodY, $JPEGQTableArray* abbrevQTables, $JPEGHuffmanTableArray* abbrevDCHuffmanTables, $JPEGHuffmanTableArray* abbrevACHuffmanTables, int32_t minProgressivePass, int32_t maxProgressivePass, bool wantUpdates) {
	$prepareNative(readImage, bool, int32_t imageIndex, int64_t structPointer, $bytes* buffer, int32_t numRasterBands, $ints* srcBands, $ints* bandSizes, int32_t sourceXOffset, int32_t sourceYOffset, int32_t sourceWidth, int32_t sourceHeight, int32_t periodX, int32_t periodY, $JPEGQTableArray* abbrevQTables, $JPEGHuffmanTableArray* abbrevDCHuffmanTables, $JPEGHuffmanTableArray* abbrevACHuffmanTables, int32_t minProgressivePass, int32_t maxProgressivePass, bool wantUpdates);
	bool $ret = $invokeNative(imageIndex, structPointer, buffer, numRasterBands, srcBands, bandSizes, sourceXOffset, sourceYOffset, sourceWidth, sourceHeight, periodX, periodY, abbrevQTables, abbrevDCHuffmanTables, abbrevACHuffmanTables, minProgressivePass, maxProgressivePass, wantUpdates);
	$finishNative();
	return $ret;
}

void JPEGImageReader::clearNativeReadAbortFlag(int64_t structPointer) {
	$prepareNative(clearNativeReadAbortFlag, void, int64_t structPointer);
	$invokeNative(structPointer);
	$finishNative();
}
...
void JPEGImageReader::abortRead(int64_t structPointer) {
	$prepareNative(abortRead, void, int64_t structPointer);
	$invokeNative(structPointer);
	$finishNative();
}

void JPEGImageReader::resetLibraryState(int64_t structPointer) {
	$prepareNative(resetLibraryState, void, int64_t structPointer);
	$invokeNative(structPointer);
	$finishNative();
}
...
void JPEGImageReader::resetReader(int64_t structPointer) {
	$prepareNative(resetReader, void, int64_t structPointer);
	$invokeNative(structPointer);
	$finishNative();
}
...
void JPEGImageReader::disposeReader(int64_t structPointer) {
	$init(JPEGImageReader);
	$prepareNativeStatic(disposeReader, void, int64_t structPointer);
...
void JPEGImageReader::clinit$($Class* clazz) {
	{
...
$Class* JPEGImageReader::load$($String* name, bool initialize) {
	...
	$loadClass(JPEGImageReader, name, initialize, &classInfo$$, JPEGImageReader::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(JPEGImageReader);
	});
	return class$;
}
