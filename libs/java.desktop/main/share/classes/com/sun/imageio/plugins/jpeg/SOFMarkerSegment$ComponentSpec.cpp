...
void SOFMarkerSegment$ComponentSpec::init$($SOFMarkerSegment* this$0, $JPEGBuffer* buffer) {
	$set(this, this$0, this$0);
	this->componentId = $nc($nc(buffer)->buf)->get($nc(buffer)->bufPtr++);
	this->HsamplingFactor = (int32_t)((uint32_t)buffer->buf->get(buffer->bufPtr) >> 4);
	this->VsamplingFactor = buffer->buf->get(buffer->bufPtr++) & 0x0f;
	this->QtableSelector = buffer->buf->get(buffer->bufPtr++);
}
...
void SOFMarkerSegment$ComponentSpec::print() {
	...
	$System::out->print("H sampling factor: "_s);
	$System::out->println(this->HsamplingFactor);
	$System::out->print("V sampling factor: "_s);
	$System::out->println(this->VsamplingFactor);
	$System::out->print("Q table selector: "_s);
...
$Class* SOFMarkerSegment$ComponentSpec::load$($String* name, bool initialize) {
	...
	$loadClass(SOFMarkerSegment$ComponentSpec, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SOFMarkerSegment$ComponentSpec);
	});
	return class$;
}
