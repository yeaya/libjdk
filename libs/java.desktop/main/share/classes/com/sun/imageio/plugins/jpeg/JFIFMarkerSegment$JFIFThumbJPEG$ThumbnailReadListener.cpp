...
$Class* JFIFMarkerSegment$JFIFThumbJPEG$ThumbnailReadListener::load$($String* name, bool initialize) {
	...
	$loadClass(JFIFMarkerSegment$JFIFThumbJPEG$ThumbnailReadListener, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JFIFMarkerSegment$JFIFThumbJPEG$ThumbnailReadListener);
	});
	return class$;
}
