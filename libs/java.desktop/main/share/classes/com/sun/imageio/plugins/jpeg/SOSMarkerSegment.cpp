...
void SOSMarkerSegment::init$(bool willSubsample, $bytes* componentIDs, int32_t numComponents) {
	...
	for (int32_t i = 0; i < numComponents; ++i) {
		...
		this->componentSpecs->set(i, $$new($SOSMarkerSegment$ScanComponentSpec, this, $nc(componentIDs)->get(i), tableSel));
	}
}
...
void SOSMarkerSegment::init$($JPEGBuffer* buffer) {
	...
	int32_t numComponents = $nc($nc(buffer)->buf)->get($nc(buffer)->bufPtr++);
	$set(this, componentSpecs, $new($SOSMarkerSegment$ScanComponentSpecArray, numComponents));
	for (int32_t i = 0; i < numComponents; ++i) {
		this->componentSpecs->set(i, $$new($SOSMarkerSegment$ScanComponentSpec, this, buffer));
	}
	this->startSpectralSelection = buffer->buf->get(buffer->bufPtr++);
	this->endSpectralSelection = buffer->buf->get(buffer->bufPtr++);
	this->approxHigh = buffer->buf->get(buffer->bufPtr) >> 4;
	this->approxLow = buffer->buf->get(buffer->bufPtr++) & 0x0f;
...
$Object* SOSMarkerSegment::clone() {
	...
	if (this->componentSpecs != nullptr) {
		$set($nc(newGuy), componentSpecs, $cast($SOSMarkerSegment$ScanComponentSpecArray, this->componentSpecs->clone()));
		for (int32_t i = 0; i < $nc(this->componentSpecs)->length; ++i) {
			$nc(newGuy->componentSpecs)->set(i, $$cast($SOSMarkerSegment$ScanComponentSpec, $nc(this->componentSpecs->get(i))->clone()));
		}
	}
	return newGuy;
}
...
void SOSMarkerSegment::updateFromNativeNode($Node* node, bool fromScratch) {
	...
	$set(this, componentSpecs, $new($SOSMarkerSegment$ScanComponentSpecArray, numComponents));
	for (int32_t i = 0; i < numComponents; ++i) {
		$nc(this->componentSpecs)->set(i, $$new($SOSMarkerSegment$ScanComponentSpec, this, $(children->item(i))));
	}
}
...
void SOSMarkerSegment::print() {
	...
	$System::out->print("End spectral selection: "_s);
	$System::out->println(this->endSpectralSelection);
	$System::out->print("Approx high: "_s);
	$System::out->println(this->approxHigh);
	$System::out->print("Approx low: "_s);
	$System::out->println(this->approxLow);
	$System::out->print("Num scan components: "_s);
...
$Class* SOSMarkerSegment::load$($String* name, bool initialize) {
	...
	$loadClass(SOSMarkerSegment, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SOSMarkerSegment);
	});
	return class$;
}
