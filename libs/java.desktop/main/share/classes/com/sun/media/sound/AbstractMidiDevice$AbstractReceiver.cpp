...
void AbstractMidiDevice$AbstractReceiver::close() {
	...
	$synchronized(this->this$0->traRecLock) {
		$$nc(this->this$0->getReceiverList())->remove(this);
	}
...
$Class* AbstractMidiDevice$AbstractReceiver::load$($String* name, bool initialize) {
	...
	$loadClass(AbstractMidiDevice$AbstractReceiver, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AbstractMidiDevice$AbstractReceiver);
	});
	return class$;
}
