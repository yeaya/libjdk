...
$AudioFloatInputStream* ModelByteBufferWavetable::openStream() {
	...
	if ($nc(this->buffer)->array() == nullptr) {
		$var($InputStream, var$0, this->buffer->getInputStream());
		$var($AudioFormat, var$1, this->format);
		int64_t var$2 = this->buffer->capacity();
	...
	if (this->buffer8 != nullptr) {
		...
		if (var$3 || $$nc(this->format->getEncoding())->equals($AudioFormat$Encoding::PCM_UNSIGNED)) {
			...
			float var$5 = this->format->getSampleRate();
			int32_t var$6 = this->format->getSampleSizeInBits() + 8;
			int32_t var$7 = this->format->getChannels();
			int32_t var$9 = this->format->getFrameSize();
			int32_t var$8 = var$9 + (1 * this->format->getChannels());
			float var$10 = this->format->getFrameRate();
			$var($AudioFormat, format2, $new($AudioFormat, var$4, var$5, var$6, var$7, var$8, var$10, this->format->isBigEndian()));
			int64_t var$11 = this->buffer->capacity();
	...
	$var($bytes, var$13, this->buffer->array());
	int32_t var$14 = (int32_t)this->buffer->arrayOffset();
	return $AudioFloatInputStream::getInputStream(var$12, var$13, var$14, (int32_t)this->buffer->capacity());
}
...
$Class* ModelByteBufferWavetable::load$($String* name, bool initialize) {
	...
	$loadClass(ModelByteBufferWavetable, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ModelByteBufferWavetable);
	});
	return class$;
}
