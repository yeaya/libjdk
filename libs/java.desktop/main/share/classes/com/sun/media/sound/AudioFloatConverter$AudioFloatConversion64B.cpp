...
$floats* AudioFloatConverter$AudioFloatConversion64B::toFloatArray($bytes* in_buff, int32_t in_offset, $floats* out_buff, int32_t out_offset, int32_t out_len) {
	...
	if (this->double_buff == nullptr || this->double_buff->length < out_len + out_offset) {
...
$bytes* AudioFloatConverter$AudioFloatConversion64B::toByteArray($floats* in_buff, int32_t in_offset, int32_t in_len, $bytes* out_buff, int32_t out_offset) {
	...
	$nc(this->floatbuffer)->position(0);
	$nc(this->bytebuffer)->position(0);
	if (this->double_buff == nullptr || this->double_buff->length < in_offset + in_len) {
...
$Class* AudioFloatConverter$AudioFloatConversion64B::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PRIVATE, $method(AudioFloatConverter$AudioFloatConversion64B, init$, void)},
		{"toByteArray", "([FII[BI)[B", nullptr, $PUBLIC, $virtualMethod(AudioFloatConverter$AudioFloatConversion64B, toByteArray, $bytes*, $floats*, int32_t, int32_t, $bytes*, int32_t)},
		{"toFloatArray", "([BI[FII)[F", nullptr, $PUBLIC, $virtualMethod(AudioFloatConverter$AudioFloatConversion64B, toFloatArray, $floats*, $bytes*, int32_t, $floats*, int32_t, int32_t)},
		{}
	};
	...
	$loadClass(AudioFloatConverter$AudioFloatConversion64B, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AudioFloatConverter$AudioFloatConversion64B);
	});
	return class$;
}
