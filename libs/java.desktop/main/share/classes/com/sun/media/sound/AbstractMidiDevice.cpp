	...
	$synchronized(this->traRecLock) {
		if (this->receiverList != nullptr) {
			for (int32_t i = 0; i < $nc(this->receiverList)->size(); ++i) {
				$$sure($Receiver, this->receiverList->get(i))->close();
			}
...
$List* AbstractMidiDevice::getReceivers() {
	...
	$synchronized(this->traRecLock) {
		if (this->receiverList == nullptr) {
			...
			$assign(recs, $Collections::unmodifiableList($$cast($List, this->receiverList->clone())));
		}
	}
	return recs;
}
...
$List* AbstractMidiDevice::getTransmitters() {
	...
	$synchronized(this->traRecLock) {
		if (this->transmitterList == nullptr || $nc(this->transmitterList->transmitters)->size() == 0) {
			$assign(tras, $Collections::unmodifiableList($$new($ArrayList, 0)));
		} else {
			$assign(tras, $Collections::unmodifiableList($$cast($List, $nc(this->transmitterList->transmitters)->clone())));
		}
	}
	return tras;
}
...
$Class* AbstractMidiDevice::load$($String* name, bool initialize) {
	...
	$loadClass(AbstractMidiDevice, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
