...
int64_t ModelByteBuffer$RandomFileInputStream::skip(int64_t n) {
	...
	int64_t p = this->raf->getFilePointer();
	this->raf->seek(p + n);
	this->left -= n;
	return n;
}
...
$Class* ModelByteBuffer$RandomFileInputStream::load$($String* name, bool initialize) {
	...
	$loadClass(ModelByteBuffer$RandomFileInputStream, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ModelByteBuffer$RandomFileInputStream);
	});
	return class$;
}
