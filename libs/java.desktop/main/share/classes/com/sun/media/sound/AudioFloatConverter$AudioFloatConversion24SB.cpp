...
$Class* AudioFloatConverter$AudioFloatConversion24SB::load$($String* name, bool initialize) {
	...
	$loadClass(AudioFloatConverter$AudioFloatConversion24SB, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AudioFloatConverter$AudioFloatConversion24SB);
	});
	return class$;
}
