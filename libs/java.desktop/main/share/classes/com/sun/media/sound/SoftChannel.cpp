...
void SoftChannel::init$($SoftSynthesizer* synth, int32_t channel) {
	...
	{
		for (int32_t i = 0; i < this->co_midi->length; ++i) {
			this->co_midi->set(i, $$new($SoftChannel$MidiControlObject, this));
		}
	}
...
int32_t SoftChannel::findFreeVoice(int32_t x) {
	...
	for (int32_t i = x; i < $nc(this->voices)->length; ++i) {
		if (!$nc(this->voices->get(i))->active) {
			return i;
		}
	}
	...
	if (vmode == 1) {
		...
		for (int32_t j = 0; j < this->voices->length; ++j) {
			if ($nc(this->voices->get(j))->stealer_channel == nullptr) {
				if (steal_channel == 9) {
					steal_channel = $nc(this->voices->get(j))->channel;
				} else if ($nc(this->voices->get(j))->channel != 9) {
					if ($nc(this->voices->get(j))->channel > steal_channel) {
						steal_channel = $nc(this->voices->get(j))->channel;
					}
				}
			}
		...
		for (int32_t j = 0; j < this->voices->length; ++j) {
			if ($nc(this->voices->get(j))->channel == steal_channel) {
				if ($nc(this->voices->get(j))->stealer_channel == nullptr && !$nc(this->voices->get(j))->on) {
					if (v == nullptr) {
						$assign(v, this->voices->get(j));
						voiceNo = j;
					}
					if ($nc(this->voices->get(j))->voiceID < $nc(v)->voiceID) {
						$assign(v, this->voices->get(j));
						voiceNo = j;
					}
				}
...
void SoftChannel::initVoice($SoftVoice* voice, $SoftPerformer* p, int32_t voiceID, int32_t noteNumber, int32_t velocity, int32_t delay, $ModelConnectionBlockArray* connectionBlocks, $ModelChannelMixer* channelmixer, bool releaseTriggered) {
	if ($nc(voice)->active) {
		...
		for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
			if ($nc(this->voices->get(i))->active && $nc(this->voices->get(i))->voiceID == voice->voiceID) {
				$nc(this->voices->get(i))->soundOff();
			}
		}
		return;
	}
	$set(voice, extendedConnectionBlocks, connectionBlocks);
	...
	voice->objects->put("midi"_s, this->co_midi->get(noteNumber));
	voice->objects->put("midi_cc"_s, this->co_midi_cc);
	voice->objects->put("midi_rpn"_s, this->co_midi_rpn);
	voice->objects->put("midi_nrpn"_s, this->co_midi_nrpn);
	...
	if (this->controller->get(84) != 0) {
		$nc(voice->co_noteon_keynumber)->set(0, ($nc(this->tuning)->getTuning(this->controller->get(84)) / 100.0) * (1.0f / 128.0f));
		voice->portamento = true;
		controlChange(84, 0);
	} else if (this->portamento) {
		if (this->mono) {
			if ($nc(this->portamento_lastnote)->get(0) != -1) {
				$nc(voice->co_noteon_keynumber)->set(0, ($nc(this->tuning)->getTuning(this->portamento_lastnote->get(0)) / 100.0) * (1.0f / 128.0f));
				voice->portamento = true;
				controlChange(84, 0);
			}
...
void SoftChannel::noteOn_internal(int32_t noteNumber, int32_t velocity, int32_t delay) {
	...
	$synchronized(this->control_mutex) {
		if (this->sustain) {
			this->sustain = false;
			for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
				if (($nc(this->voices->get(i))->sustain || $nc(this->voices->get(i))->on) && $nc(this->voices->get(i))->channel == this->channel && $nc(this->voices->get(i))->active && $nc(this->voices->get(i))->note == noteNumber) {
					$nc(this->voices->get(i))->sustain = false;
					$nc(this->voices->get(i))->on = true;
					$nc(this->voices->get(i))->noteOff(0);
				}
			}
			this->sustain = true;
		}
		...
		if (this->mono) {
			if (this->portamento) {
				bool n_found = false;
				for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
					if ($nc(this->voices->get(i))->on && $nc(this->voices->get(i))->channel == this->channel && $nc(this->voices->get(i))->active && $nc(this->voices->get(i))->releaseTriggered == false) {
						$nc(this->voices->get(i))->portamento = true;
						$nc(this->voices->get(i))->setNote(noteNumber);
						n_found = true;
					}
				}
				if (n_found) {
			...
			if (this->controller->get(84) != 0) {
				bool n_found = false;
				for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
					if ($nc(this->voices->get(i))->on && $nc(this->voices->get(i))->channel == this->channel && $nc(this->voices->get(i))->active && $nc(this->voices->get(i))->note == this->controller->get(84) && $nc(this->voices->get(i))->releaseTriggered == false) {
						$nc(this->voices->get(i))->portamento = true;
						$nc(this->voices->get(i))->setNote(noteNumber);
						n_found = true;
					}
		...
		if (this->current_instrument == nullptr) {
			...
			$set(this, current_mixer, $$nc($nc(this->current_instrument)->getSourceInstrument())->getChannelMixer(this, $(this->synthesizer->getFormat())));
			if (this->current_mixer != nullptr) {
		...
		this->play_releasetriggered = false;
		this->lastVelocity->set(noteNumber, velocity);
...
void SoftChannel::noteOff_internal(int32_t noteNumber, int32_t velocity) {
	...
	$synchronized(this->control_mutex) {
		...
		$nc(this->mainmixer)->activity();
		for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
			if ($nc(this->voices->get(i))->on && $nc(this->voices->get(i))->channel == this->channel && $nc(this->voices->get(i))->note == noteNumber && $nc(this->voices->get(i))->releaseTriggered == false) {
				$nc(this->voices->get(i))->noteOff(velocity);
			}
			if ($nc(this->voices->get(i))->stealer_channel == this && $nc(this->voices->get(i))->stealer_noteNumber == noteNumber) {
				$var($SoftVoice, v, this->voices->get(i));
		...
		int32_t tunedKey = (int32_t)($Math::round($nc(this->tuning)->getTuning(noteNumber) / 100.0));
		this->play_noteNumber = noteNumber;
		this->play_velocity = this->lastVelocity->get(noteNumber);
	...
	if (this->firstVoice) {
		...
		if ($nc(p)->exclusiveClass != 0) {
			...
			for (int32_t j = 0; j < $nc(this->voices)->length; ++j) {
				if ($nc(this->voices->get(j))->active && $nc(this->voices->get(j))->channel == this->channel && $nc(this->voices->get(j))->exclusiveClass == x) {
					if (!(p->selfNonExclusive && $nc(this->voices->get(j))->note == noteNumber)) {
						$nc(this->voices->get(j))->shutdown();
					}
				}
			}
		}
...
void SoftChannel::setPolyPressure(int32_t noteNumber, int32_t pressure) {
	...
	$synchronized(this->control_mutex) {
		$nc(this->mainmixer)->activity();
		$nc($($nc(this->co_midi->get(noteNumber))->get(0, "poly_pressure"_s)))->set(0, pressure * (1.0 / 128.0));
		this->polypressure->set(noteNumber, pressure);
		for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
			if ($nc(this->voices->get(i))->active && $nc(this->voices->get(i))->note == noteNumber) {
				$nc(this->voices->get(i))->setPolyPressure(pressure);
			}
		}
	}
}
...
int32_t SoftChannel::getPolyPressure(int32_t noteNumber) {
	$synchronized(this->control_mutex) {
		return this->polypressure->get(noteNumber);
	}
}
...
void SoftChannel::setChannelPressure(int32_t pressure) {
	...
	$synchronized(this->control_mutex) {
		$nc(this->mainmixer)->activity();
		this->co_midi_channel_pressure->set(0, pressure * (1.0 / 128.0));
		this->channelpressure = pressure;
		for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
			if ($nc(this->voices->get(i))->active) {
				$nc(this->voices->get(i))->setChannelPressure(pressure);
			}
		}
	}
}
...
void SoftChannel::applyInstrumentCustomization() {
	...
	for (int32_t i = 0; i < new_performers->length; ++i) {
		...
		$$nc(new_performer->getOscillators())->addAll($(performer->getOscillators()));
		$$nc(new_performer->getConnectionBlocks())->addAll($(performer->getConnectionBlocks()));
		...
		if (this->cds_control_connections != nullptr) {
			...
			while ($nc(iter)->hasNext()) {
				...
				if (sources != nullptr) {
					for (int32_t j = 0; j < sources->length; ++j) {
						...
						if (var$0 && cc->equals($($$nc(src->getIdentifier())->getVariable()))) {
							removeok = true;
						}
					}
				}
			...
			for (int32_t j = 0; j < $nc(this->cds_control_connections)->length; ++j) {
				connblocks->add(this->cds_control_connections->get(j));
			}
		}
		if (this->cds_polypressure_connections != nullptr) {
			...
			while ($nc(iter)->hasNext()) {
				...
				if (sources != nullptr) {
					for (int32_t j = 0; j < sources->length; ++j) {
						...
						if (var$1 && "poly_pressure"_s->equals($($$nc(src->getIdentifier())->getVariable()))) {
							removeok = true;
						}
					}
				}
			...
			for (int32_t j = 0; j < $nc(this->cds_polypressure_connections)->length; ++j) {
				connblocks->add(this->cds_polypressure_connections->get(j));
			}
		}
		if (this->cds_channelpressure_connections != nullptr) {
			...
			while ($nc(iter)->hasNext()) {
				...
				if (sources != nullptr) {
					for (int32_t j = 0; j < sources->length; ++j) {
						...
						if (var$2 && "channel_pressure"_s->equals($(srcid->getVariable()))) {
							removeok = true;
						}
					}
				}
			...
			for (int32_t j = 0; j < $nc(this->cds_channelpressure_connections)->length; ++j) {
				connblocks->add(this->cds_channelpressure_connections->get(j));
			}
		}
	}
...
$ModelConnectionBlockArray* SoftChannel::createModelConnections($ModelIdentifier* sid, $ints* destination, $ints* range) {
	...
	for (int32_t i = 0; i < $nc(destination)->length; ++i) {
		...
		if (d == 0) {
			...
			$var($ModelConnectionBlock, conn, $new($ModelConnectionBlock, var$0, scale, $$new($ModelDestination, $$new($ModelIdentifier, "osc"_s, "pitch"_s))));
			conns->add(conn);
		}
		if (d == 1) {
			...
			if (scale > 0) {
				$var($ModelSource, var$1, $new($ModelSource, sid, $ModelStandardTransform::DIRECTION_MAX2MIN, $ModelStandardTransform::POLARITY_UNIPOLAR, $ModelStandardTransform::TRANSFORM_LINEAR));
				double var$2 = -scale;
				$init($ModelDestination);
				$assign(conn, $new($ModelConnectionBlock, var$1, var$2, $$new($ModelDestination, $ModelDestination::DESTINATION_FILTER_FREQ)));
			} else {
				$var($ModelSource, var$3, $new($ModelSource, sid, $ModelStandardTransform::DIRECTION_MIN2MAX, $ModelStandardTransform::POLARITY_UNIPOLAR, $ModelStandardTransform::TRANSFORM_LINEAR));
				$init($ModelDestination);
				$assign(conn, $new($ModelConnectionBlock, var$3, scale, $$new($ModelDestination, $ModelDestination::DESTINATION_FILTER_FREQ)));
			}
			conns->add(conn);
		}
		if (d == 2) {
			...
			$var($ModelSource, var$4, $new($ModelSource, sid, mt));
			$init($ModelDestination);
			$var($ModelConnectionBlock, conn, $new($ModelConnectionBlock, var$4, -960, $$new($ModelDestination, $ModelDestination::DESTINATION_GAIN)));
			conns->add(conn);
		}
		if (d == 3) {
			double scale = (r / 64.0 - 1.0) * 9600.0;
			$init($ModelSource);
			$var($ModelSource, var$5, $new($ModelSource, $ModelSource::SOURCE_LFO1, $ModelStandardTransform::DIRECTION_MIN2MAX, $ModelStandardTransform::POLARITY_BIPOLAR, $ModelStandardTransform::TRANSFORM_LINEAR));
			$var($ModelSource, var$6, $new($ModelSource, sid, $ModelStandardTransform::DIRECTION_MIN2MAX, $ModelStandardTransform::POLARITY_UNIPOLAR, $ModelStandardTransform::TRANSFORM_LINEAR));
			$init($ModelDestination);
			$var($ModelConnectionBlock, conn, $new($ModelConnectionBlock, var$5, var$6, scale, $$new($ModelDestination, $ModelDestination::DESTINATION_PITCH)));
			conns->add(conn);
		}
		if (d == 4) {
			...
			$init($ModelDestination);
			$var($ModelConnectionBlock, conn, $new($ModelConnectionBlock, var$7, var$8, scale, $$new($ModelDestination, $ModelDestination::DESTINATION_FILTER_FREQ)));
			conns->add(conn);
		}
		if (d == 5) {
			...
			$init($ModelSource);
			$var($ModelSource, var$9, $new($ModelSource, $ModelSource::SOURCE_LFO1, $ModelStandardTransform::DIRECTION_MIN2MAX, $ModelStandardTransform::POLARITY_UNIPOLAR, $ModelStandardTransform::TRANSFORM_LINEAR));
			$var($ModelSource, var$10, $new($ModelSource, sid, mt));
			$init($ModelDestination);
			$var($ModelConnectionBlock, conn, $new($ModelConnectionBlock, var$9, var$10, -960, $$new($ModelDestination, $ModelDestination::DESTINATION_GAIN)));
			conns->add(conn);
		}
	}
	return $cast($ModelConnectionBlockArray, conns->toArray($$new($ModelConnectionBlockArray, conns->size())));
}
...
void SoftChannel::controlChangePerNote(int32_t noteNumber, int32_t controller, int32_t value) {
	...
	if ($nc(this->keybasedcontroller_active)->get(noteNumber) == nullptr) {
		this->keybasedcontroller_active->set(noteNumber, $$new($booleans, 128));
		$Arrays::fill(this->keybasedcontroller_active->get(noteNumber), false);
		$nc(this->keybasedcontroller_value)->set(noteNumber, $$new($doubles, 128));
		$Arrays::fill(this->keybasedcontroller_value->get(noteNumber), 0);
	}
	if (value == -1) {
		$nc(this->keybasedcontroller_active->get(noteNumber))->set(controller, false);
	} else {
		$nc(this->keybasedcontroller_active->get(noteNumber))->set(controller, true);
		$nc($nc(this->keybasedcontroller_value)->get(noteNumber))->set(controller, value / 128.0);
	}
	if (controller < 120) {
		for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
			if ($nc(this->voices->get(i))->active) {
				$nc(this->voices->get(i))->controlChange(controller, -1);
			}
		}
	} else if (controller == 120) {
		for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
			if ($nc(this->voices->get(i))->active) {
				$nc(this->voices->get(i))->rpnChange(1, -1);
			}
		}
	} else if (controller == 121) {
		for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
			if ($nc(this->voices->get(i))->active) {
				$nc(this->voices->get(i))->rpnChange(2, -1);
			}
		}
	}
}
...
int32_t SoftChannel::getControlPerNote(int32_t noteNumber, int32_t controller) {
	...
	if (!$nc(this->keybasedcontroller_active->get(noteNumber))->get(controller)) {
		return -1;
	}
	...
	$synchronized(this->control_mutex) {
		{
			...
			case 64:
				...
				if (this->sustain != on) {
					...
					if (!on) {
						for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
							if ($nc(this->voices->get(i))->active && $nc(this->voices->get(i))->sustain && $nc(this->voices->get(i))->channel == this->channel) {
								$nc(this->voices->get(i))->sustain = false;
								if (!$nc(this->voices->get(i))->on) {
									$nc(this->voices->get(i))->on = true;
									$nc(this->voices->get(i))->noteOff(0);
								}
							}
						...
						for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
							if ($nc(this->voices->get(i))->active && $nc(this->voices->get(i))->channel == this->channel) {
								$nc(this->voices->get(i))->redamp();
							}
						}
					}
				}
			...
			case 66:
				...
				if (on) {
					for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
						if ($nc(this->voices->get(i))->active && $nc(this->voices->get(i))->on && $nc(this->voices->get(i))->channel == this->channel) {
							$nc(this->voices->get(i))->sostenuto = true;
						}
					}
				}
				if (!on) {
					for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
						if ($nc(this->voices->get(i))->active && $nc(this->voices->get(i))->sostenuto && $nc(this->voices->get(i))->channel == this->channel) {
							$nc(this->voices->get(i))->sostenuto = false;
							if (!$nc(this->voices->get(i))->on) {
								$nc(this->voices->get(i))->on = true;
								$nc(this->voices->get(i))->noteOff(0);
							}
				...
				break;
			case 98:
				this->nrpn_control = (this->nrpn_control & (127 << 7)) + value;
				this->rpn_control = SoftChannel::RPN_NULL_VALUE;
				break;
			case 99:
				...
				break;
			case 100:
				this->rpn_control = (this->rpn_control & (127 << 7)) + value;
				this->nrpn_control = SoftChannel::RPN_NULL_VALUE;
				break;
			case 101:
		...
		$nc(this->co_midi_cc_cc->get(controller))->set(0, value * (1.0 / 128.0));
		if (controller == 0) {
			this->bank = (value << 7);
			return;
		}
		if (controller == 32) {
			this->bank = (this->bank & (127 << 7)) + value;
			return;
		}
		$nc(this->controller)->set(controller, value);
		if (controller < 32) {
			this->controller->set(controller + 32, 0);
		}
		for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
			if ($nc(this->voices->get(i))->active) {
				$nc(this->voices->get(i))->controlChange(controller, value);
			}
		}
	}
}
...
int32_t SoftChannel::getController(int32_t controller) {
	$synchronized(this->control_mutex) {
		return $nc(this->controller)->get(controller) & 0x7f;
	}
}
...
void SoftChannel::setPitchBend(int32_t bend) {
	...
	$synchronized(this->control_mutex) {
		$nc(this->mainmixer)->activity();
		this->co_midi_pitch->set(0, bend * (1.0 / 16384.0));
		this->pitchbend = bend;
		for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
			if ($nc(this->voices->get(i))->active) {
				$nc(this->voices->get(i))->setPitchBend(bend);
			}
		}
	}
}
...
void SoftChannel::nrpnChange(int32_t controller, int32_t value) {
	...
	if (val_i == nullptr) {
		...
		this->co_midi_nrpn_nrpn_i->put($($Integer::valueOf(controller)), val_i);
	}
	if (val_d == nullptr) {
		$assign(val_d, $new($doubles, 1));
		this->co_midi_nrpn_nrpn->put($($Integer::valueOf(controller)), val_d);
	}
	$nc(val_i)->set(0, value);
	$nc(val_d)->set(0, val_i->get(0) * (1.0 / 16384.0));
	for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
		if ($nc(this->voices->get(i))->active) {
			$nc(this->voices->get(i))->nrpnChange(controller, val_i->get(0));
		}
	}
}
...
void SoftChannel::rpnChange(int32_t controller, int32_t value) {
	...
	if (val_i == nullptr) {
		$assign(val_i, $new($ints, 1));
		this->co_midi_rpn_rpn_i->put($($Integer::valueOf(controller)), val_i);
	}
	if (val_d == nullptr) {
		$assign(val_d, $new($doubles, 1));
		this->co_midi_rpn_rpn->put($($Integer::valueOf(controller)), val_d);
	}
	$nc(val_i)->set(0, value);
	$nc(val_d)->set(0, val_i->get(0) * (1.0 / 16384.0));
	for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
		if ($nc(this->voices->get(i))->active) {
			$nc(this->voices->get(i))->rpnChange(controller, val_i->get(0));
		}
	}
}
...
void SoftChannel::resetAllControllers(bool allControls) {
	...
	$synchronized(this->control_mutex) {
		...
		setPitchBend(8192);
		for (int32_t i = 0; i < 128; ++i) {
			if (!SoftChannel::dontResetControls->get(i)) {
				controlChange(i, 0);
			}
		}
		controlChange(71, 64);
		...
		if (allControls) {
			...
			controlChange(91, 40);
			{
				...
				for (; $nc(i$)->hasNext();) {
					int32_t controller = $$sure($Integer, i$->next())->intValue();
...
void SoftChannel::allNotesOff() {
	...
	$synchronized(this->control_mutex) {
		for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
			if ($nc(this->voices->get(i))->on && $nc(this->voices->get(i))->channel == this->channel && $nc(this->voices->get(i))->releaseTriggered == false) {
				$nc(this->voices->get(i))->noteOff(0);
			}
		}
	}
...
void SoftChannel::allSoundOff() {
	...
	$synchronized(this->control_mutex) {
		for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
			if ($nc(this->voices->get(i))->on && $nc(this->voices->get(i))->channel == this->channel) {
				$nc(this->voices->get(i))->soundOff();
			}
		}
	}
...
void SoftChannel::setMute(bool mute) {
	...
	$synchronized(this->control_mutex) {
		...
		for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
			if ($nc(this->voices->get(i))->active && $nc(this->voices->get(i))->channel == this->channel) {
				$nc(this->voices->get(i))->setMute(mute);
			}
		}
	}
}
...
void SoftChannel::setSolo(bool soloState) {
	...
	$synchronized(this->control_mutex) {
		...
		if (!soloinuse) {
			$var($SoftChannelArray, arr$, this->synthesizer->channels);
			for (int32_t len$ = $nc(arr$)->length, i$ = 0; i$ < len$; ++i$) {
				$var(SoftChannel, c, arr$->get(i$));
...
void SoftChannel::setSoloMute(bool mute) {
	$synchronized(this->control_mutex) {
		...
		for (int32_t i = 0; i < $nc(this->voices)->length; ++i) {
			if ($nc(this->voices->get(i))->active && $nc(this->voices->get(i))->channel == this->channel) {
				$nc(this->voices->get(i))->setSoloMute(this->solomute);
			}
		}
	}
}
...
void SoftChannel::clinit$($Class* clazz) {
	$assignStatic(SoftChannel::dontResetControls, $new($booleans, 128));
	{
		for (int32_t i = 0; i < SoftChannel::dontResetControls->length; ++i) {
			SoftChannel::dontResetControls->set(i, false);
		}
		SoftChannel::dontResetControls->set(0, true);
		SoftChannel::dontResetControls->set(32, true);
		SoftChannel::dontResetControls->set(7, true);
		SoftChannel::dontResetControls->set(8, true);
		SoftChannel::dontResetControls->set(10, true);
		SoftChannel::dontResetControls->set(11, true);
		SoftChannel::dontResetControls->set(91, true);
		SoftChannel::dontResetControls->set(92, true);
		SoftChannel::dontResetControls->set(93, true);
		SoftChannel::dontResetControls->set(94, true);
		SoftChannel::dontResetControls->set(95, true);
		SoftChannel::dontResetControls->set(70, true);
		SoftChannel::dontResetControls->set(71, true);
		SoftChannel::dontResetControls->set(72, true);
		SoftChannel::dontResetControls->set(73, true);
		SoftChannel::dontResetControls->set(74, true);
		SoftChannel::dontResetControls->set(75, true);
		SoftChannel::dontResetControls->set(76, true);
		SoftChannel::dontResetControls->set(77, true);
		SoftChannel::dontResetControls->set(78, true);
		SoftChannel::dontResetControls->set(79, true);
		SoftChannel::dontResetControls->set(120, true);
		SoftChannel::dontResetControls->set(121, true);
		SoftChannel::dontResetControls->set(122, true);
		SoftChannel::dontResetControls->set(123, true);
		SoftChannel::dontResetControls->set(124, true);
		SoftChannel::dontResetControls->set(125, true);
		SoftChannel::dontResetControls->set(126, true);
		SoftChannel::dontResetControls->set(127, true);
		SoftChannel::dontResetControls->set(6, true);
		SoftChannel::dontResetControls->set(38, true);
		SoftChannel::dontResetControls->set(96, true);
		SoftChannel::dontResetControls->set(97, true);
		SoftChannel::dontResetControls->set(98, true);
		SoftChannel::dontResetControls->set(99, true);
		SoftChannel::dontResetControls->set(100, true);
		SoftChannel::dontResetControls->set(101, true);
	}
}
...
$Class* SoftChannel::load$($String* name, bool initialize) {
	...
	$loadClass(SoftChannel, name, initialize, &classInfo$$, SoftChannel::clinit$, []($Class* clazz) -> $Object* {
