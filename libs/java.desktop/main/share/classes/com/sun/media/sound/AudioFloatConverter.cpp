...
AudioFloatConverter* AudioFloatConverter::getConverter($AudioFormat* format) {
	...
	int32_t var$0 = format->getFrameSize();
	...
	if ($$nc(format->getEncoding())->equals($AudioFormat$Encoding::PCM_SIGNED)) {
		if (format->isBigEndian()) {
			if (format->getSampleSizeInBits() <= 8) {
				$assign(conv, $new($AudioFloatConverter$AudioFloatConversion8S));
			} else {
				bool var$3 = format->getSampleSizeInBits() > 8;
				if (var$3 && format->getSampleSizeInBits() <= 16) {
					$assign(conv, $new($AudioFloatConverter$AudioFloatConversion16SB));
				} else {
					bool var$4 = format->getSampleSizeInBits() > 16;
					if (var$4 && format->getSampleSizeInBits() <= 24) {
						$assign(conv, $new($AudioFloatConverter$AudioFloatConversion24SB));
					} else {
						bool var$5 = format->getSampleSizeInBits() > 24;
						if (var$5 && format->getSampleSizeInBits() <= 32) {
			...
			bool var$6 = format->getSampleSizeInBits() > 8;
			if (var$6 && format->getSampleSizeInBits() <= 16) {
				$assign(conv, $new($AudioFloatConverter$AudioFloatConversion16SL));
			} else {
				bool var$7 = format->getSampleSizeInBits() > 16;
				if (var$7 && format->getSampleSizeInBits() <= 24) {
	...
	} else if ($$nc(format->getEncoding())->equals($AudioFormat$Encoding::PCM_UNSIGNED)) {
		if (format->isBigEndian()) {
			if (format->getSampleSizeInBits() <= 8) {
				$assign(conv, $new($AudioFloatConverter$AudioFloatConversion8U));
			} else {
				bool var$9 = format->getSampleSizeInBits() > 8;
				if (var$9 && format->getSampleSizeInBits() <= 16) {
					$assign(conv, $new($AudioFloatConverter$AudioFloatConversion16UB));
				} else {
					bool var$10 = format->getSampleSizeInBits() > 16;
					if (var$10 && format->getSampleSizeInBits() <= 24) {
						$assign(conv, $new($AudioFloatConverter$AudioFloatConversion24UB));
					} else {
						bool var$11 = format->getSampleSizeInBits() > 24;
						if (var$11 && format->getSampleSizeInBits() <= 32) {
			...
			bool var$12 = format->getSampleSizeInBits() > 8;
			if (var$12 && format->getSampleSizeInBits() <= 16) {
				$assign(conv, $new($AudioFloatConverter$AudioFloatConversion16UL));
			} else {
				bool var$13 = format->getSampleSizeInBits() > 16;
				if (var$13 && format->getSampleSizeInBits() <= 24) {
	...
	} else if ($$nc(format->getEncoding())->equals($AudioFormat$Encoding::PCM_FLOAT)) {
	...
	bool var$16 = $$nc(format->getEncoding())->equals($AudioFormat$Encoding::PCM_SIGNED);
	bool var$15 = var$16 || $$nc(format->getEncoding())->equals($AudioFormat$Encoding::PCM_UNSIGNED);
	if (var$15 && (format->getSampleSizeInBits() % 8 != 0)) {
...
$Class* AudioFloatConverter::load$($String* name, bool initialize) {
	...
	$loadClass(AudioFloatConverter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AudioFloatConverter);
	});
	return class$;
}
