...
void DLSSoundbank::readSoundbank($InputStream* inputstream) {
	...
	}
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			{
				$set($nc($$cast($DLSRegion, $nc(entry)->getKey())), sample, $cast($DLSSample, $nc(this->samples)->get((int32_t)$$sure($Long, $nc(entry)->getValue())->longValue())));
			}
		}
	}
...
bool DLSSoundbank::cdlIsQuerySupported($DLSSoundbank$DLSID* uuid) {
	...
	bool var$6 = var$7 || uuid->equals(DLSSoundbank::DLSID_GSInHardware);
	bool var$5 = var$6 || uuid->equals(DLSSoundbank::DLSID_XGInHardware);
	bool var$4 = var$5 || uuid->equals(DLSSoundbank::DLSID_SupportsDLS1);
	bool var$3 = var$4 || uuid->equals(DLSSoundbank::DLSID_SupportsDLS2);
	bool var$2 = var$3 || uuid->equals(DLSSoundbank::DLSID_SampleMemorySize);
	bool var$1 = var$2 || uuid->equals(DLSSoundbank::DLSID_ManufacturersID);
	bool var$0 = var$1 || uuid->equals(DLSSoundbank::DLSID_ProductID);
	return var$0 || uuid->equals(DLSSoundbank::DLSID_SamplePlaybackRate);
}
...
int64_t DLSSoundbank::cdlQuery($DLSSoundbank$DLSID* uuid) {
	...
	if (uuid->equals(DLSSoundbank::DLSID_GSInHardware)) {
		return 0;
	}
	if (uuid->equals(DLSSoundbank::DLSID_XGInHardware)) {
		return 0;
	}
	if (uuid->equals(DLSSoundbank::DLSID_SupportsDLS1)) {
		return 1;
	}
	if (uuid->equals(DLSSoundbank::DLSID_SupportsDLS2)) {
		return 1;
	}
	if (uuid->equals(DLSSoundbank::DLSID_SampleMemorySize)) {
		return $$nc($Runtime::getRuntime())->totalMemory();
	}
	if (uuid->equals(DLSSoundbank::DLSID_ManufacturersID)) {
		return 0;
	}
	if (uuid->equals(DLSSoundbank::DLSID_ProductID)) {
		return 0;
	}
	if (uuid->equals(DLSSoundbank::DLSID_SamplePlaybackRate)) {
		return 0x0000ac44;
	}
	return 0;
}
...
bool DLSSoundbank::readCdlChunk($RIFFReader* riff) {
	...
	while ($nc(riff)->available() != 0) {
		...
		case DLSSoundbank::DLS_CDL_AND:
			x = $$sure($Long, stack->pop())->longValue();
			y = $$sure($Long, stack->pop())->longValue();
	...
	return $$sure($Long, stack->pop())->longValue() == 1;
}
...
void DLSSoundbank::readInfoChunk($RIFFReader* riff) {
	...
	while ($nc(riff)->hasNextChunk()) {
		...
		if ($nc(format)->equals("INAM"_s)) {
			$set(this->info, name, chunk->readString(chunk->available()));
		} else if (format->equals("ICRD"_s)) {
			$set(this->info, creationDate, chunk->readString(chunk->available()));
		} else if (format->equals("IENG"_s)) {
			$set(this->info, engineers, chunk->readString(chunk->available()));
		} else if (format->equals("IPRD"_s)) {
			$set(this->info, product, chunk->readString(chunk->available()));
		} else if (format->equals("ICOP"_s)) {
			$set(this->info, copyright, chunk->readString(chunk->available()));
		} else if (format->equals("ICMT"_s)) {
			$set(this->info, comments, chunk->readString(chunk->available()));
		} else if (format->equals("ISFT"_s)) {
			$set(this->info, tools, chunk->readString(chunk->available()));
		} else if (format->equals("IARL"_s)) {
			$set(this->info, archival_location, chunk->readString(chunk->available()));
		} else if (format->equals("IART"_s)) {
			$set(this->info, artist, chunk->readString(chunk->available()));
		} else if (format->equals("ICMS"_s)) {
			$set(this->info, commissioned, chunk->readString(chunk->available()));
		} else if (format->equals("IGNR"_s)) {
			$set(this->info, genre, chunk->readString(chunk->available()));
		} else if (format->equals("IKEY"_s)) {
			$set(this->info, keywords, chunk->readString(chunk->available()));
		} else if (format->equals("IMED"_s)) {
			$set(this->info, medium, chunk->readString(chunk->available()));
		} else if (format->equals("ISBJ"_s)) {
			$set(this->info, subject, chunk->readString(chunk->available()));
		} else if (format->equals("ISRC"_s)) {
			$set(this->info, source, chunk->readString(chunk->available()));
		} else if (format->equals("ISRF"_s)) {
			$set(this->info, source_form, chunk->readString(chunk->available()));
		} else if (format->equals("ITCH"_s)) {
			$set(this->info, technician, chunk->readString(chunk->available()));
		}
	}
}
		...
		if ($nc(format)->equals("LIST"_s)) {
			...
			if (format->equals("data"_s)) {
				if (this->largeFormat) {
					...
					sample->setData($$new($ModelByteBuffer, var$0, var$1, chunk->available()));
				} else {
...
void DLSSoundbank::writeSample($RIFFWriter* writer, $DLSSample* sample) {
	...
	if ($$nc(audioformat->getEncoding())->equals($AudioFormat$Encoding::PCM_UNSIGNED)) {
		sampleformat = 1;
	} else if ($$nc(audioformat->getEncoding())->equals($AudioFormat$Encoding::PCM_SIGNED)) {
		sampleformat = 1;
	} else if ($$nc(audioformat->getEncoding())->equals($AudioFormat$Encoding::PCM_FLOAT)) {
		sampleformat = 3;
	}
...
void DLSSoundbank::writeInstrument($RIFFWriter* writer, $DLSInstrument* instrument) {
	...
	insh_chunk->writeUnsignedInt(instrument->bank + (instrument->druminstrument ? (int64_t)0x0000000080000000 : 0));
...
void DLSSoundbank::writeRegion($RIFFWriter* writer, $DLSRegion* region, int32_t version) {
	...
	if (region->sampleoptions != nullptr) {
...
void DLSSoundbank::writeSampleOptions($RIFFWriter* wsmp, $DLSSampleOptions* sampleoptions) {
	...
	wsmp->writeInt($nc(sampleoptions->loops)->size());
	{
		$var($Iterator, i$, $nc(sampleoptions->loops)->iterator());
		for (; $nc(i$)->hasNext();) {
...
void DLSSoundbank::writeInfo($RIFFWriter* writer, $DLSInfo* info) {
	...
	writeInfoStringChunk(writer, "ICRD"_s, info->creationDate);
	writeInfoStringChunk(writer, "IENG"_s, info->engineers);
	writeInfoStringChunk(writer, "IPRD"_s, info->product);
	writeInfoStringChunk(writer, "ICOP"_s, info->copyright);
	writeInfoStringChunk(writer, "ICMT"_s, info->comments);
	writeInfoStringChunk(writer, "ISFT"_s, info->tools);
	writeInfoStringChunk(writer, "IARL"_s, info->archival_location);
	writeInfoStringChunk(writer, "IART"_s, info->artist);
	writeInfoStringChunk(writer, "ICMS"_s, info->commissioned);
	writeInfoStringChunk(writer, "IGNR"_s, info->genre);
	writeInfoStringChunk(writer, "IKEY"_s, info->keywords);
	writeInfoStringChunk(writer, "IMED"_s, info->medium);
	writeInfoStringChunk(writer, "ISBJ"_s, info->subject);
	writeInfoStringChunk(writer, "ISRC"_s, info->source);
	writeInfoStringChunk(writer, "ISRF"_s, info->source_form);
	writeInfoStringChunk(writer, "ITCH"_s, info->technician);
}
...
$InstrumentArray* DLSSoundbank::getInstruments() {
	$useLocalObjectStack();
	$var($DLSInstrumentArray, inslist_array, $cast($DLSInstrumentArray, $nc(this->instruments)->toArray($$new($DLSInstrumentArray, $nc(this->instruments)->size()))));
	$Arrays::sort(inslist_array, $$new($ModelInstrumentComparator));
	return $cast($InstrumentArray, inslist_array);
}

$DLSSampleArray* DLSSoundbank::getSamples() {
	return $cast($DLSSampleArray, $nc(this->samples)->toArray($$new($DLSSampleArray, $nc(this->samples)->size())));
}
...
void DLSSoundbank::clinit$($Class* clazz) {
...
$Class* DLSSoundbank::load$($String* name, bool initialize) {
	...
	$loadClass(DLSSoundbank, name, initialize, &classInfo$$, DLSSoundbank::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(DLSSoundbank);
	});
	return class$;
}
