...
void AbstractMidiDevice$BasicTransmitter::close() {
	...
	if (this->tlist != nullptr) {
		this->tlist->receiverChanged(this, this->receiver, nullptr);
...
$Class* AbstractMidiDevice$BasicTransmitter::load$($String* name, bool initialize) {
	...
	$loadClass(AbstractMidiDevice$BasicTransmitter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AbstractMidiDevice$BasicTransmitter);
	});
	return class$;
}
