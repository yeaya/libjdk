...
void SoftShortMessage::setMessage(int32_t command, int32_t channel, int32_t data1, int32_t data2) {
	...
	$ShortMessage::setMessage(command, channel & 0x0f, data1, data2);
}
...
$Class* SoftShortMessage::load$($String* name, bool initialize) {
	...
	$loadClass(SoftShortMessage, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SoftShortMessage);
	});
	return class$;
}
