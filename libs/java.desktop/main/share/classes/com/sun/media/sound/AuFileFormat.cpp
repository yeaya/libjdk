...
void AuFileFormat::init$($AudioFileFormat$Type* type, int64_t byteLength, $AudioFormat* format, int64_t frameLength) {
	...
	} else if ($nc($AudioFormat$Encoding::ULAW)->equals(encoding)) {
		if (format->getSampleSizeInBits() == 8) {
			this->auType = AuFileFormat::AU_ULAW_8;
		}
	} else if ($nc($AudioFormat$Encoding::PCM_SIGNED)->equals(encoding)) {
		if (format->getSampleSizeInBits() == 8) {
	...
	} else if ($nc($AudioFormat$Encoding::PCM_FLOAT)->equals(encoding)) {
...
$Class* AuFileFormat::load$($String* name, bool initialize) {
	...
	$loadClass(AuFileFormat, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AuFileFormat);
	});
	return class$;
}
