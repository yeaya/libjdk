...
int16_t SF2Region::getShort(int32_t generator) {
	...
	return $$sure($Short, $nc(this->generators)->get($($Integer::valueOf(generator))))->shortValue();
}

void SF2Region::putShort(int32_t generator, int16_t value) {
	$useLocalObjectStack();
	$var($Object, var$0, $Integer::valueOf(generator));
...
$bytes* SF2Region::getBytes(int32_t generator) {
	...
	bytes->set(0, (int8_t)(0xff & val));
	bytes->set(1, (int8_t)((0xff00 & val) >> 8));
	return bytes;
}
...
void SF2Region::putBytes(int32_t generator, $bytes* bytes) {
	$useLocalObjectStack();
	$var($Object, var$0, $Integer::valueOf(generator));
	$nc(this->generators)->put(var$0, $($Short::valueOf((int16_t)($nc(bytes)->get(0) + ($nc(bytes)->get(1) << 8)))));
}
...
$Class* SF2Region::load$($String* name, bool initialize) {
	...
	$loadClass(SF2Region, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SF2Region);
	});
	return class$;
}
