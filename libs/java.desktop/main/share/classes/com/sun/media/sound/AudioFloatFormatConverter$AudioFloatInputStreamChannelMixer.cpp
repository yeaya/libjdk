...
void AudioFloatFormatConverter$AudioFloatInputStreamChannelMixer::init$($AudioFloatInputStream* ais, int32_t targetChannels) {
	...
	int32_t var$3 = ($div(format->getFrameSize(), this->sourceChannels)) * targetChannels;
	float var$4 = format->getFrameRate();
	$set(this, targetFormat, $new($AudioFormat, var$0, var$1, var$2, targetChannels, var$3, var$4, format->isBigEndian()));
}
...
int32_t AudioFloatFormatConverter$AudioFloatInputStreamChannelMixer::read($floats* b, int32_t off, int32_t len) {
	...
	if (this->conversion_buffer == nullptr || this->conversion_buffer->length < len2) {
	...
	if (this->sourceChannels == 1) {
		...
		for (int32_t c = 0; c < minChannels; ++c) {
			for (int32_t i = off + c, ix = c; i < off_len; i += ct, ix += cs) {
...
$Class* AudioFloatFormatConverter$AudioFloatInputStreamChannelMixer::load$($String* name, bool initialize) {
	...
	$loadClass(AudioFloatFormatConverter$AudioFloatInputStreamChannelMixer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AudioFloatFormatConverter$AudioFloatInputStreamChannelMixer);
	});
	return class$;
}
