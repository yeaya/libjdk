	...
	$useLocalObjectStack();
	{
		...
		try {
			try {
				...
				fmt_chunk->writeUnsignedInt($cast(int32_t, format->getSampleRate()));
				int32_t var$1 = $cast(int32_t, format->getFrameRate());
...
int32_t WaveFloatFileWriter::write($AudioInputStream* stream$renamed, $AudioFileFormat$Type* fileType, $OutputStream* out) {
	...
	{
		$var($RIFFWriter, writer, $new($RIFFWriter, $$new($WaveFloatFileWriter$NoCloseOutputStream, out), "WAVE"_s));
		$var($Throwable, var$0, nullptr);
		int32_t var$2 = 0;
		bool return$1 = false;
		try {
...
$Class* WaveFloatFileWriter::load$($String* name, bool initialize) {
	...
	$loadClass(WaveFloatFileWriter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(WaveFloatFileWriter);
	});
	return class$;
}
