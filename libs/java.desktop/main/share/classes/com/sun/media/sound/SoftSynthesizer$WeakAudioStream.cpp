...
int32_t SoftSynthesizer$WeakAudioStream::read($bytes* b, int32_t off, int32_t len) {
	...
	if (local_stream != nullptr) {
		...
		if (this->silentbuffer == nullptr || this->silentbuffer->length < flen) {
...
void SoftSynthesizer$WeakAudioStream::close() {
	$var($AudioInputStream, astream, $cast($AudioInputStream, this->weak_stream_link->get()));
	if (astream != nullptr) {
		astream->close();
	}
}
...
$Class* SoftSynthesizer$WeakAudioStream::load$($String* name, bool initialize) {
	...
	$loadClass(SoftSynthesizer$WeakAudioStream, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SoftSynthesizer$WeakAudioStream);
	});
	return class$;
}
