...
void SoftReverb$AllPass::processReplace($floats* inout) {
	...
	for (int32_t i = 0; i < len; ++i) {
		float delayout = this->delaybuffer->get(rovepos);
		float input = inout->get(i);
		inout->set(i, delayout - input);
		this->delaybuffer->set(rovepos, input + delayout * this->feedback);
...
$Class* SoftReverb$AllPass::load$($String* name, bool initialize) {
	...
	$loadClass(SoftReverb$AllPass, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SoftReverb$AllPass);
	});
	return class$;
}
