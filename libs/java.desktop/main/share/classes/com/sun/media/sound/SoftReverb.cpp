...
void SoftReverb::init(float samplerate, float controlrate) {
	...
	this->combL->set(0, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1116)))));
	this->combR->set(0, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1116 + stereospread)))));
	this->combL->set(1, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1188)))));
	this->combR->set(1, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1188 + stereospread)))));
	this->combL->set(2, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1277)))));
	this->combR->set(2, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1277 + stereospread)))));
	this->combL->set(3, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1356)))));
	this->combR->set(3, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1356 + stereospread)))));
	this->combL->set(4, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1422)))));
	this->combR->set(4, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1422 + stereospread)))));
	this->combL->set(5, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1491)))));
	this->combR->set(5, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1491 + stereospread)))));
	this->combL->set(6, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1557)))));
	this->combR->set(6, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1557 + stereospread)))));
	this->combL->set(7, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1617)))));
	this->combR->set(7, $$new($SoftReverb$Comb, $cast(int32_t, (freqscale * (1617 + stereospread)))));
	$set(this, allpassL, $new($SoftReverb$AllPassArray, 4));
	$set(this, allpassR, $new($SoftReverb$AllPassArray, 4));
	this->allpassL->set(0, $$new($SoftReverb$AllPass, $cast(int32_t, (freqscale * (556)))));
	this->allpassR->set(0, $$new($SoftReverb$AllPass, $cast(int32_t, (freqscale * (556 + stereospread)))));
	this->allpassL->set(1, $$new($SoftReverb$AllPass, $cast(int32_t, (freqscale * (441)))));
	this->allpassR->set(1, $$new($SoftReverb$AllPass, $cast(int32_t, (freqscale * (441 + stereospread)))));
	this->allpassL->set(2, $$new($SoftReverb$AllPass, $cast(int32_t, (freqscale * (341)))));
	this->allpassR->set(2, $$new($SoftReverb$AllPass, $cast(int32_t, (freqscale * (341 + stereospread)))));
	this->allpassL->set(3, $$new($SoftReverb$AllPass, $cast(int32_t, (freqscale * (225)))));
	this->allpassR->set(3, $$new($SoftReverb$AllPass, $cast(int32_t, (freqscale * (225 + stereospread)))));
	for (int32_t i = 0; i < $nc(this->allpassL)->length; ++i) {
		$nc(this->allpassL->get(i))->setFeedBack(0.5f);
...
void SoftReverb::processAudio() {
	...
	if (this->input == nullptr || this->input->length < numsamples) {
	...
	if (this->light && (right != nullptr)) {
		...
		for (int32_t i = 4; i < $nc(this->combL)->length - 2; i += 2) {
			$nc(this->combL->get(i))->processMix(this->input, this->pre1);
		}
		$nc($nc(this->combL)->get(3))->processReplace(this->input, this->pre2);
		for (int32_t i = 5; i < $nc(this->combL)->length - 2; i += 2) {
			$nc(this->combL->get(i))->processMix(this->input, this->pre2);
		}
		if (!this->mix) {
			$Arrays::fill(right, 0);
			$Arrays::fill(left, 0);
		...
		for (int32_t i = $nc(this->combR)->length - 2; i < $nc(this->combR)->length; ++i) {
			$nc(this->combR->get(i))->processMix(this->input, right);
		}
		for (int32_t i = $nc(this->combL)->length - 2; i < $nc(this->combL)->length; ++i) {
			$nc(this->combL->get(i))->processMix(this->input, left);
		}
		for (int32_t i = 0; i < numsamples; ++i) {
		...
		if (this->out == nullptr || this->out->length < numsamples) {
		...
		if (right != nullptr) {
			...
			for (int32_t i = 1; i < $nc(this->allpassR)->length; ++i) {
				$nc(this->allpassR->get(i))->processReplace(this->out);
			}
			for (int32_t i = 0; i < $nc(this->combR)->length; ++i) {
				$nc(this->combR->get(i))->processMix(this->out, right);
			}
		}
		if (!this->mix) {
			$Arrays::fill(left, 0);
		...
		for (int32_t i = 1; i < $nc(this->allpassL)->length; ++i) {
			$nc(this->allpassL->get(i))->processReplace(this->out);
		}
		for (int32_t i = 0; i < $nc(this->combL)->length; ++i) {
			$nc(this->combL->get(i))->processMix(this->out, left);
		}
	}
	if (silent_input) {
		this->silent = true;
		...
		if (slothpath->get(0) == 1 * 128 + 1) {
			if (param == 0) {
				if (value == 0) {
					...
					this->dirty_damp = (5000);
					this->dirty_predelay = (0);
					this->dirty_gain = (4);
					this->dirty = true;
				}
				if (value == 1) {
...
void SoftReverb::setRoomSize(float value) {
	...
	for (int32_t i = 0; i < $nc(this->combL)->length; ++i) {
		$nc(this->combL->get(i))->feedback = this->roomsize;
...
void SoftReverb::setDamp(float value) {
	...
	for (int32_t i = 0; i < $nc(this->combL)->length; ++i) {
		$nc(this->combL->get(i))->setDamp(this->damp);
...
$Class* SoftReverb::load$($String* name, bool initialize) {
	...
	$loadClass(SoftReverb, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SoftReverb);
	});
	return class$;
}
