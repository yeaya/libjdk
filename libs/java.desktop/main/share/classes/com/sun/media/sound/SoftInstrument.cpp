...
void SoftInstrument::initPerformers($ModelPerformerArray* modelperformers) {
	...
	for (int32_t i = 0; i < modelperformers->length; ++i) {
		this->performers->set(i, $$new($SoftPerformer, modelperformers->get(i)));
	}
}
...
$Class* SoftInstrument::load$($String* name, bool initialize) {
	...
	$loadClass(SoftInstrument, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SoftInstrument);
	});
	return class$;
}
