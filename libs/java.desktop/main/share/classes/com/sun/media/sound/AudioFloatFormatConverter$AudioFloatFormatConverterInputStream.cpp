...
int32_t AudioFloatFormatConverter$AudioFloatFormatConverterInputStream::read($bytes* b, int32_t off, int32_t len) {
	...
	if (this->readfloatbuffer == nullptr || this->readfloatbuffer->length < flen) {
...
$Class* AudioFloatFormatConverter$AudioFloatFormatConverterInputStream::load$($String* name, bool initialize) {
	...
	$loadClass(AudioFloatFormatConverter$AudioFloatFormatConverterInputStream, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AudioFloatFormatConverter$AudioFloatFormatConverterInputStream);
	});
	return class$;
}
