...
$MidiFileFormat* StandardMidiFileReader::getMidiFileFormat($URL* url) {
	...
	$var($Throwable, var$0, nullptr);
	try {
		$assign(fileFormat, getMidiFileFormat(bis));
	} catch ($Throwable& var$1) {
		$assign(var$0, var$1);
	} /*finally*/ {
		bis->close();
...
$Sequence* StandardMidiFileReader::getSequence($InputStream* stream) {
	...
	float var$1 = format->getDivisionType();
...
$Class* StandardMidiFileReader::load$($String* name, bool initialize) {
	...
	$loadClass(StandardMidiFileReader, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(StandardMidiFileReader);
	});
	return class$;
}
