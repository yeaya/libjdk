...
$String* SoftPerformer::extractKeys($ModelConnectionBlock* conn) {
	...
	if ($nc(conn)->getSources() != nullptr) {
		...
		for (int32_t i = 0; i < srcs->length; ++i) {
			sb->append($($nc(srcs->get(i))->getIdentifier()));
			sb->append(";"_s);
		}
		sb->append("]"_s);
	}
	sb->append(";"_s);
	if (conn->getDestination() != nullptr) {
		sb->append($($$nc(conn->getDestination())->getIdentifier()));
	}
	sb->append(";"_s);
	return sb->toString();
}
...
void SoftPerformer::processMidiControlSource($ModelSource* src, int32_t ix) {
	...
	if ($nc(this->midi_ctrl_connections)->get(c) == nullptr) {
		this->midi_ctrl_connections->set(c, $$new($ints, {ix}));
	} else {
		$var($ints, olda, this->midi_ctrl_connections->get(c));
		$var($ints, newa, $new($ints, $nc(olda)->length + 1));
		for (int32_t i = 0; i < olda->length; ++i) {
			newa->set(i, olda->get(i));
		}
		newa->set(newa->length - 1, ix);
		this->midi_ctrl_connections->set(c, newa);
	}
}
...
void SoftPerformer::processNoteOnSource($ModelSource* src, int32_t ix) {
	...
	if ($nc(this->midi_connections)->get(c) == nullptr) {
		this->midi_connections->set(c, $$new($ints, {ix}));
	} else {
		$var($ints, olda, this->midi_connections->get(c));
		$var($ints, newa, $new($ints, $nc(olda)->length + 1));
		for (int32_t i = 0; i < olda->length; ++i) {
			newa->set(i, olda->get(i));
		}
		newa->set(newa->length - 1, ix);
		this->midi_connections->set(c, newa);
	}
}
...
void SoftPerformer::processMidiSource($ModelSource* src, int32_t ix) {
	...
	if (v->equals("channel_pressure"_s)) {
		c = 1;
	}
	if (v->equals("poly_pressure"_s)) {
		c = 2;
	}
	if (c == -1) {
		return;
...
void SoftPerformer::processMidiRpnSource($ModelSource* src, int32_t ix) {
	...
	if ($nc(this->midi_rpn_connections)->get($($Integer::valueOf(c))) == nullptr) {
		this->midi_rpn_connections->put($($Integer::valueOf(c)), $$new($ints, {ix}));
	} else {
		$var($ints, olda, $cast($ints, this->midi_rpn_connections->get($($Integer::valueOf(c)))));
		$var($ints, newa, $new($ints, $nc(olda)->length + 1));
		for (int32_t i = 0; i < olda->length; ++i) {
			newa->set(i, olda->get(i));
		}
		newa->set(newa->length - 1, ix);
		this->midi_rpn_connections->put($($Integer::valueOf(c)), newa);
	}
}
...
void SoftPerformer::processMidiNrpnSource($ModelSource* src, int32_t ix) {
	...
	if ($nc(this->midi_nrpn_connections)->get($($Integer::valueOf(c))) == nullptr) {
		this->midi_nrpn_connections->put($($Integer::valueOf(c)), $$new($ints, {ix}));
	} else {
		$var($ints, olda, $cast($ints, this->midi_nrpn_connections->get($($Integer::valueOf(c)))));
		$var($ints, newa, $new($ints, $nc(olda)->length + 1));
		for (int32_t i = 0; i < olda->length; ++i) {
			newa->set(i, olda->get(i));
		}
		newa->set(newa->length - 1, ix);
		this->midi_nrpn_connections->put($($Integer::valueOf(c)), newa);
	}
}
...
void SoftPerformer::init$($ModelPerformer* performer) {
	...
	if (performer->isDefaultConnectionsEnabled()) {
		...
		if (!isModulationWheelConectionFound) {
			...
			$var($ModelConnectionBlock, conn, $new($ModelConnectionBlock, var$0, var$1, 50, $$new($ModelDestination, $ModelDestination::DESTINATION_PITCH)));
				...
				$var($ModelConnectionBlock, connection, $cast($ModelConnectionBlock, i$->next()));
				{
					...
					if (dest != nullptr && sources != nullptr) {
						for (int32_t i = 0; i < sources->length; ++i) {
							...
							if ($$nc(srcid->getObject())->equals("midi"_s)) {
		...
		$assign(connection, $new($ModelConnectionBlock, var$3, 2000, $$new($ModelDestination, $$new($ModelIdentifier, "lfo"_s, "delay2"_s, instance))));
		connmap->put($(extractKeys(connection)), connection);
		double scale = found_vib_connection == nullptr ? 0 : found_vib_connection->getScale();
		...
		$var($ModelSource, var$5, $new($ModelSource, var$6, $$new($SoftPerformer$2, this, scale)));
		...
		$assign(connection, $new($ModelConnectionBlock, var$7, 2400, $$new($ModelDestination, $$new($ModelIdentifier, "lfo"_s, "freq"_s, instance))));
		connmap->put($(extractKeys(connection)), connection);
	}
	if (performer->isDefaultConnectionsEnabled()) {
	...
	for (int32_t i = 0; i < this->midi_ctrl_connections->length; ++i) {
		this->midi_ctrl_connections->set(i, nullptr);
	}
	$set(this, midi_connections, $new($intArray2, 5));
	for (int32_t i = 0; i < this->midi_connections->length; ++i) {
		this->midi_connections->set(i, nullptr);
	}
	int32_t ix = 0;
	...
	$set(this, ctrl_connections, $new($ints, this->ctrl_connections_list->size()));
	for (int32_t i = 0; i < this->ctrl_connections->length; ++i) {
		this->ctrl_connections->set(i, $$sure($Integer, this->ctrl_connections_list->get(i))->intValue());
	}
...
void SoftPerformer::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$assignStatic(SoftPerformer::defaultconnections, $new($ModelConnectionBlockArray, 42));
	{
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$0, 1, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "on"_s, 0))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$1, 1, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "on"_s, 1))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$2, 1, $$new($ModelDestination, $$new($ModelIdentifier, "mixer"_s, "active"_s, 0))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$3, -960, $$new($ModelDestination, $$new($ModelIdentifier, "mixer"_s, "gain"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$4, -960, $$new($ModelDestination, $$new($ModelIdentifier, "mixer"_s, "gain"_s))));
		...
		$var($ModelSource, var$6, $new($ModelSource, var$7, $$new($SoftPerformer$1)));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$5, var$6, $$new($ModelDestination, $$new($ModelIdentifier, "osc"_s, "pitch"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$8, 12800, $$new($ModelDestination, $$new($ModelIdentifier, "osc"_s, "pitch"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$9, -960, $$new($ModelDestination, $$new($ModelIdentifier, "mixer"_s, "gain"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$10, 1000, $$new($ModelDestination, $$new($ModelIdentifier, "mixer"_s, "balance"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$11, 1000, $$new($ModelDestination, $$new($ModelIdentifier, "mixer"_s, "pan"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$12, -960, $$new($ModelDestination, $$new($ModelIdentifier, "mixer"_s, "gain"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$13, 1000, $$new($ModelDestination, $$new($ModelIdentifier, "mixer"_s, "reverb"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$14, 1000, $$new($ModelDestination, $$new($ModelIdentifier, "mixer"_s, "chorus"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$15, 200, $$new($ModelDestination, $$new($ModelIdentifier, "filter"_s, "q"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$16, 9600, $$new($ModelDestination, $$new($ModelIdentifier, "filter"_s, "freq"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$17, 6000, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "release2"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$18, 2000, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "attack2"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$19, 6000, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "decay2"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$20, -50, $$new($ModelDestination, $ModelDestination::DESTINATION_GAIN)));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$21, -2400, $$new($ModelDestination, $ModelDestination::DESTINATION_FILTER_FREQ)));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$22, 100, $$new($ModelDestination, $$new($ModelIdentifier, "osc"_s, "pitch"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$23, 12800, $$new($ModelDestination, $$new($ModelIdentifier, "osc"_s, "pitch"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$24, 100, $$new($ModelDestination, $$new($ModelIdentifier, "osc"_s, "pitch"_s))));
		...
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$25, 12800, $$new($ModelDestination, $$new($ModelIdentifier, "osc"_s, "pitch"_s))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, 13500, $$new($ModelDestination, $$new($ModelIdentifier, "filter"_s, "freq"_s, 0))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, $Float::NEGATIVE_INFINITY, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "delay"_s, 0))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, $Float::NEGATIVE_INFINITY, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "attack"_s, 0))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, $Float::NEGATIVE_INFINITY, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "hold"_s, 0))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, $Float::NEGATIVE_INFINITY, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "decay"_s, 0))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, 1000, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "sustain"_s, 0))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, $Float::NEGATIVE_INFINITY, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "release"_s, 0))));
		double var$27 = 1200.0 * $Math::log(0.015);
		double var$26 = var$27 / $Math::log(2);
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, var$26, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "shutdown"_s, 0))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, $Float::NEGATIVE_INFINITY, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "delay"_s, 1))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, $Float::NEGATIVE_INFINITY, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "attack"_s, 1))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, $Float::NEGATIVE_INFINITY, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "hold"_s, 1))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, $Float::NEGATIVE_INFINITY, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "decay"_s, 1))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, 1000, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "sustain"_s, 1))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, $Float::NEGATIVE_INFINITY, $$new($ModelDestination, $$new($ModelIdentifier, "eg"_s, "release"_s, 1))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, -8.51318, $$new($ModelDestination, $$new($ModelIdentifier, "lfo"_s, "freq"_s, 0))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, $Float::NEGATIVE_INFINITY, $$new($ModelDestination, $$new($ModelIdentifier, "lfo"_s, "delay"_s, 0))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, -8.51318, $$new($ModelDestination, $$new($ModelIdentifier, "lfo"_s, "freq"_s, 1))));
		SoftPerformer::defaultconnections->set(o++, $$new($ModelConnectionBlock, $Float::NEGATIVE_INFINITY, $$new($ModelDestination, $$new($ModelIdentifier, "lfo"_s, "delay"_s, 1))));
	}
...
$Class* SoftPerformer::load$($String* name, bool initialize) {
	...
	$loadClass(SoftPerformer, name, initialize, &classInfo$$, SoftPerformer::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(SoftPerformer);
	});
	return class$;
}
