...
int32_t SMFParser::readUnsigned() {
	return $nc(this->trackData)->get(this->pos++) & 0xff;
}
...
int64_t SMFParser::readVarInt() {
	...
	do {
		currentByte = $nc(this->trackData)->get(this->pos++) & 0xff;
...
$Class* SMFParser::load$($String* name, bool initialize) {
	...
	$loadClass(SMFParser, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SMFParser);
	});
	return class$;
}
