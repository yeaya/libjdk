...
$Object* FieldElementHandler::getContextBean() {
	return (this->type != nullptr) ? $of(this->type) : $AccessorElementHandler::getContextBean();
}
...
$Object* FieldElementHandler::getFieldValue(Object$* bean, $String* name) {
	...
	return $$nc(findField(bean, name))->get(bean);
}
...
$Class* FieldElementHandler::load$($String* name, bool initialize) {
	...
	$loadClass(FieldElementHandler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FieldElementHandler);
	});
	return class$;
}
