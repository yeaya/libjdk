...
$Cache$Ref* Cache$Kind$2::create(Object$* owner, Object$* referent, $ReferenceQueue* queue) {
	return (referent == nullptr) ? $cast($Cache$Ref, $new($Cache$Kind$Strong, owner, referent)) : $cast($Cache$Ref, $new($Cache$Kind$Soft, owner, referent, queue));
}
...
$Class* Cache$Kind$2::load$($String* name, bool initialize) {
	...
	$loadClass(Cache$Kind$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
