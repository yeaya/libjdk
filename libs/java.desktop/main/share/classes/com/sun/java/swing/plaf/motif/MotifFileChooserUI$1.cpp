	...
	if ($nc(prop)->equals($JFileChooser::SELECTED_FILE_CHANGED_PROPERTY)) {
		...
		if (currentDirectory != nullptr) {
			try {
				...
				this->this$0->setDirectoryName($($$sure($File, e->getNewValue())->getAbsolutePath()));
			}
		...
		if (this->this$0->getAccessoryPanel() != nullptr) {
			...
			if (accessory != nullptr) {
				$init($BorderLayout);
				$$nc(this->this$0->getAccessoryPanel())->add(accessory, $BorderLayout::CENTER);
		...
		bool var$4 = prop->equals($JFileChooser::APPROVE_BUTTON_TEXT_CHANGED_PROPERTY);
		bool var$3 = var$4 || prop->equals($JFileChooser::APPROVE_BUTTON_TOOL_TIP_TEXT_CHANGED_PROPERTY);
		if (var$3 || prop->equals($JFileChooser::DIALOG_TYPE_CHANGED_PROPERTY)) {
...
$Class* MotifFileChooserUI$1::load$($String* name, bool initialize) {
	...
	$loadClass(MotifFileChooserUI$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MotifFileChooserUI$1);
	});
	return class$;
}
