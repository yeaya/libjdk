...
$Object* GTKLookAndFeel$1FontLazyValue::createValue($UIDefaults* table) {
	...
	return $nc(style)->getDefaultFont();
}
...
$Class* GTKLookAndFeel$1FontLazyValue::load$($String* name, bool initialize) {
	...
	$loadClass(GTKLookAndFeel$1FontLazyValue, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(GTKLookAndFeel$1FontLazyValue);
	});
	return class$;
}
