	...
	if (this->type == Metacity$Privileged::GET_THEME_DIR) {
		...
		if (themeDir == nullptr) {
			...
			if (url != nullptr) {
				...
				try {
					$assign(themeDir, $new($URL, $$str({$($nc(str)->substring(0, $nc(str)->lastIndexOf(u'/'))), "/"_s})));
				} catch ($MalformedURLException& ex) {
					$assign(themeDir, nullptr);
				}
			}
		...
		return $$new($ImageIcon, $cast($URL, this->arg))->getImage();
	} else {
		return nullptr;
	}
}

void Metacity$Privileged::clinit$($Class* clazz) {
...
$Class* Metacity$Privileged::load$($String* name, bool initialize) {
	...
	$loadClass(Metacity$Privileged, name, initialize, &classInfo$$, Metacity$Privileged::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Metacity$Privileged);
	});
	return class$;
}
