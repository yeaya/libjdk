...
void GTKColorChooserPanel::buildChooser() {
	...
	this->triangle->setName("GTKColorChooserPanel.triangle"_s);
	$set(this, label, $new($GTKColorChooserPanel$OpaqueLabel, this));
	this->label->setName("GTKColorChooserPanel.colorWell"_s);
	...
	this->lastLabel->setName("GTKColorChooserPanel.lastColorWell"_s);
	...
	$var($Component, var$0, $new($JSeparator, $SwingConstants::HORIZONTAL));
	add(var$0, $$new($GridBagConstraints, 1, 3, 4, 1, 1, 0, $GridBagConstraints::LINE_START, $GridBagConstraints::HORIZONTAL, $$new($Insets, 14, 0, 0, 0), 0, 0));
	add(this, "GTKColorChooserPanel.colorName"_s, this->colorNameTF, 0, 4);
	add(this->triangle, $$new($GridBagConstraints, 0, 0, 1, 5, 0, 0, $GridBagConstraints::LINE_START, $GridBagConstraints::NONE, $$new($Insets, 14, 20, 2, 9), 0, 0));
	$var($Box, hBox, $Box::createHorizontalBox());
	$nc(hBox)->add(this->lastLabel);
	hBox->add(this->label);
	add(hBox, $$new($GridBagConstraints, 0, 5, 1, 1, 0, 0, $GridBagConstraints::CENTER, $GridBagConstraints::NONE, $$new($Insets, 0, 0, 0, 0), 0, 0));
	$var($Component, var$1, $new($JSeparator, $SwingConstants::HORIZONTAL));
	add(var$1, $$new($GridBagConstraints, 0, 6, 5, 1, 1, 0, $GridBagConstraints::LINE_START, $GridBagConstraints::HORIZONTAL, $$new($Insets, 12, 0, 0, 0), 0, 0));
}
...
void GTKColorChooserPanel::configureSpinner($JSpinner* spinner, $String* name) {
	...
	if ($instanceOf($JSpinner$DefaultEditor, editor)) {
		$var($JFormattedTextField, ftf, $cast($JSpinner$DefaultEditor, editor)->getTextField());
...
void GTKColorChooserPanel::add($Container* parent, $String* key, $JComponent* widget, int32_t x, int32_t y) {
	$useLocalObjectStack();
	$var($JLabel, label, $new($JLabel, $($UIManager::getString($$str({key, "Text"_s}), $(getLocale())))));
	...
	$var($GridBagConstraints, con, $new($GridBagConstraints, x + 1, y, 1, 1, 0, 0, $GridBagConstraints::FIRST_LINE_END, $GridBagConstraints::NONE, $$new($Insets, 4, 0, 0, 4), 0, 0));
	if (y == 0) {
		$nc(con->insets)->top = 14;
	}
	$nc(parent)->add(label, con);
	++con->gridx;
...
void GTKColorChooserPanel::setHSB(float h, float s, float b) {
	...
	$nc(this->greenSpinner)->setValue($($Integer::valueOf(color->getGreen())));
	$nc(this->blueSpinner)->setValue($($Integer::valueOf(color->getBlue())));
	this->settingColor = false;
}
...
void GTKColorChooserPanel::setColor($Color* color$renamed, bool updateSpinners, bool updateHSB, bool updateModel) {
	...
	$var($String, hexString, $Integer::toHexString(($nc(color)->getRGB() & 0x00ffffff) | 0x01000000));
	$nc(this->colorNameTF)->setText($$str({"#"_s, $($nc(hexString)->substring(1))}));
	if (updateSpinners) {
		$nc(this->redSpinner)->setValue($($Integer::valueOf(color->getRed())));
		$nc(this->greenSpinner)->setValue($($Integer::valueOf(color->getGreen())));
		$nc(this->blueSpinner)->setValue($($Integer::valueOf(color->getBlue())));
		$nc(this->hueSpinner)->setValue($($Integer::valueOf($cast(int32_t, (this->hue * 360)))));
		$nc(this->saturationSpinner)->setValue($($Integer::valueOf($cast(int32_t, (this->saturation * 255)))));
...
void GTKColorChooserPanel::stateChanged($ChangeEvent* e) {
	...
	if ($equals($nc(e)->getSource(), this->hueSpinner)) {
		setHue($$sure($Number, $nc(this->hueSpinner)->getValue())->floatValue() / 360, false);
	} else if ($equals(e->getSource(), this->saturationSpinner)) {
		setSaturation($$sure($Number, $nc(this->saturationSpinner)->getValue())->floatValue() / 255);
	} else if ($equals(e->getSource(), this->valueSpinner)) {
		setBrightness($$sure($Number, $nc(this->valueSpinner)->getValue())->floatValue() / 255);
	} else if ($equals(e->getSource(), this->redSpinner)) {
		setRed($$sure($Number, $nc(this->redSpinner)->getValue())->intValue());
	} else if ($equals(e->getSource(), this->greenSpinner)) {
		setGreen($$sure($Number, $nc(this->greenSpinner)->getValue())->intValue());
	} else if ($equals(e->getSource(), this->blueSpinner)) {
		setBlue($$sure($Number, $nc(this->blueSpinner)->getValue())->intValue());
	}
}
...
void GTKColorChooserPanel::clinit$($Class* clazz) {
	$init($Math);
...
$Class* GTKColorChooserPanel::load$($String* name, bool initialize) {
	...
	$loadClass(GTKColorChooserPanel, name, initialize, &classInfo$$, GTKColorChooserPanel::clinit$, []($Class* clazz) -> $Object* {
