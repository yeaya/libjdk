...
void XRRenderer::validateSurface($SunGraphics2D* sg2d) {
	...
	$nc(xrsd->maskBuffer)->validateCompositeState(sg2d->composite, sg2d->transform$, sg2d->paint, sg2d);
}
...
void XRRenderer::drawLine($SunGraphics2D* sg2d, int32_t x1, int32_t y1, int32_t x2, int32_t y2) {
	...
	try {
		...
		int32_t var$2 = $nc(compClip)->getLoX();
		int32_t var$3 = compClip->getLoY();
		int32_t var$4 = compClip->getHiX();
		$nc(this->lineGen)->rasterizeLine(var$1, transX1, transY1, transX2, transY2, var$2, var$3, var$4, compClip->getHiY(), true, true);
...
void XRRenderer::drawPolyline($SunGraphics2D* sg2d, $ints* xpoints, $ints* ypoints, int32_t npoints) {
	...
	if (npoints > 1) {
		...
		for (int32_t i = 1; i < npoints; ++i) {
			p2d->lineTo((float)xpoints->get(i), (float)ypoints->get(i));
		}
	}
	draw(sg2d, p2d);
}
...
$Class* XRRenderer::load$($String* name, bool initialize) {
	...
	$loadClass(XRRenderer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
