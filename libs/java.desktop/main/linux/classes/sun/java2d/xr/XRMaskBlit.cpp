...
void XRMaskBlit::register$() {
	...
	$var($GraphicsPrimitiveArray, primitives, $new($GraphicsPrimitiveArray, {
		$$new(XRMaskBlit, $XRSurfaceData::IntArgbPreX11, $CompositeType::SrcOver, $XRSurfaceData::IntArgbPreX11),
		$$new(XRMaskBlit, $XRSurfaceData::IntRgbX11, $CompositeType::SrcOver, $XRSurfaceData::IntRgbX11),
		$$new(XRMaskBlit, $XRSurfaceData::IntArgbPreX11, $CompositeType::SrcNoEa, $XRSurfaceData::IntRgbX11),
		$$new(XRMaskBlit, $XRSurfaceData::IntRgbX11, $CompositeType::SrcNoEa, $XRSurfaceData::IntArgbPreX11)
	}));
...
void XRMaskBlit::maskBlit(int64_t srcXsdo, int64_t dstxsdo, int32_t srcx, int32_t srcy, int32_t dstx, int32_t dsty, int32_t w, int32_t h, int32_t maskoff, int32_t maskscan, int32_t masklen, $bytes* mask) {
	$prepareNative(maskBlit, void, int64_t srcXsdo, int64_t dstxsdo, int32_t srcx, int32_t srcy, int32_t dstx, int32_t dsty, int32_t w, int32_t h, int32_t maskoff, int32_t maskscan, int32_t masklen, $bytes* mask);
...
void XRMaskBlit::MaskBlit$($SurfaceData* src, $SurfaceData* dst, $Composite* comp, $Region* clip, int32_t srcx, int32_t srcy, int32_t dstx, int32_t dsty, int32_t width, int32_t height, $bytes* mask, int32_t maskoff, int32_t maskscan) {
	...
	try {
		...
		maskBuffer->XRComposite(var$1, maskPict, x11dst->getPicture(), srcx, srcy, 0, 0, dstx, dsty, width, height);
...
$Class* XRMaskBlit::load$($String* name, bool initialize) {
	...
	$loadClass(XRMaskBlit, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XRMaskBlit);
	});
	return class$;
}
