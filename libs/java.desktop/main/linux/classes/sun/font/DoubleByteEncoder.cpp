		...
		while (sp < sl) {
			...
			if ($Character::isSurrogate(c)) {
				...
				if (outputBytes == nullptr) {
					...
					$nc(da)->set(dp++, outputBytes->get(0));
			...
			if (ncode != 0 && c != u'\0') {
				...
				$nc(da)->set(dp++, (int8_t)((ncode & 0xff00) >> 8));
				da->set(dp++, (int8_t)(ncode & 0xff));
				++sp;
				continue;
			}
			$assign(var$6, $CoderResult::unmappableForLength(1));
		...
		while (src->hasRemaining()) {
			...
			if ($Character::isSurrogate(c)) {
				...
				if (outputBytes == nullptr) {
					...
					mark += 2;
					dst->put(outputBytes->get(0));
					dst->put(outputBytes->get(1));
					continue;
				}
			}
			if (c >= (char16_t)0xfffe) {
...
int32_t DoubleByteEncoder::encodeDouble(char16_t ch) {
	int32_t offset = $nc(this->index1)->get((ch & 0xff00) >> 8) << 8;
	return $nc($nc(this->index2)->get(offset >> 12))->charAt((offset & 0x0fff) + (ch & 0xff));
}
...
$Class* DoubleByteEncoder::load$($String* name, bool initialize) {
	...
	$loadClass(DoubleByteEncoder, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DoubleByteEncoder);
	});
	return class$;
}
