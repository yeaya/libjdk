...
void X11TextRenderer::doDrawGlyphList(int64_t dstData, int64_t xgc, $Region* clip, $GlyphList* gl) {
	$prepareNative(doDrawGlyphList, void, int64_t dstData, int64_t xgc, $Region* clip, $GlyphList* gl);
...
$Class* X11TextRenderer::load$($String* name, bool initialize) {
	...
	$loadClass(X11TextRenderer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(X11TextRenderer);
	});
	return class$;
}
