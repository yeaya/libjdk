...
int32_t NativeStrike::getNativePointSize() {
	...
	if (var$0 && this->desc->devTx->getType() != $AffineTransform::TYPE_TRANSLATION) {
		try {
	...
	if ((ttype & ~$AffineTransform::TYPE_UNIFORM_SCALE) != 0 || $nc(this->fontTx)->getScaleY() <= 0) {
...
void NativeStrike::init$($NativeFont* nativeFont, $FontStrikeDesc* desc) {
	...
	if ($nc(nativeFont)->isBitmapDelegate) {
		...
		if ((ttype & ~$AffineTransform::TYPE_UNIFORM_SCALE) != 0 || $nc(desc->glyphTx)->getScaleX() <= 0) {
			this->numGlyphs = 0;
			return;
		}
	}
	int32_t ptSize = getNativePointSize();
	$var($bytes, nameBytes, nativeFont->getPlatformNameBytes(ptSize));
...
int64_t NativeStrike::createScalerContext($bytes* nameBytes, int32_t ptSize, double scale) {
	$prepareNative(createScalerContext, int64_t, $bytes* nameBytes, int32_t ptSize, double scale);
	int64_t $ret = $invokeNative(nameBytes, ptSize, scale);
	$finishNative();
	return $ret;
}

int32_t NativeStrike::getMaxGlyph(int64_t pScalerContext) {
	$prepareNative(getMaxGlyph, int32_t, int64_t pScalerContext);
	int32_t $ret = $invokeNative(pScalerContext);
	$finishNative();
	return $ret;
}

int64_t NativeStrike::createNullScalerContext() {
	$prepareNative(createNullScalerContext, int64_t);
...
int64_t NativeStrike::getGlyphImagePtr(int32_t glyphCode) {
	...
	if (usingIntGlyphImages()) {
		if ((glyphPtr = $nc(this->intGlyphImages)->get(glyphCode) & $PhysicalStrike::INTMASK) != 0) {
			return glyphPtr;
		} else {
	...
	} else if ((glyphPtr = $nc($(getLongGlyphImages()))->get(glyphCode)) != 0) {
		return glyphPtr;
	} else {
		glyphPtr = $nc(this->nativeFont)->getGlyphImage(this->pScalerContext, glyphCode);
		$synchronized(this) {
...
$Class* NativeStrike::load$($String* name, bool initialize) {
	...
	$loadClass(NativeStrike, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(NativeStrike);
	});
	return class$;
}
