...
void XRGlyphCache::glyphDisposed($ArrayList* glyphPtrList) {
	...
	try {
		...
		{
			$var($Iterator, i$, glyphPtrList->iterator());
			for (; $nc(i$)->hasNext();) {
				int64_t glyphPtr = $$sure($Long, i$->next())->longValue();
				{
...
int32_t XRGlyphCache::getFreeGlyphID() {
	if ($nc(this->freeGlyphIDs)->size() > 0) {
		int32_t newID = $$sure($Integer, this->freeGlyphIDs->remove(this->freeGlyphIDs->size() - 1))->intValue();
		return newID;
	}
	return this->nextID++;
}
...
$XRGlyphCacheEntryArray* XRGlyphCache::cacheGlyphs($GlyphList* glyphList) {
	...
	for (int32_t i = 0; i < glyphList->getNumGlyphs(); ++i) {
		...
		if ($nc(imgPtrs)->get(i) == 0) {
			continue;
		}
		if (($assign(glyph, getEntryForPointer(imgPtrs->get(i)))) == nullptr) {
			$assign(glyph, $new($XRGlyphCacheEntry, imgPtrs->get(i), glyphList));
...
$bytes* XRGlyphCache::generateGlyphImageStream($List* glyphList) {
	$useLocalObjectStack();
	bool isLCDGlyph = $$sure($XRGlyphCacheEntry, $nc(glyphList)->get(0))->getGlyphSet() == this->lcdGlyphSet;
...
$Class* XRGlyphCache::load$($String* name, bool initialize) {
	...
	$loadClass(XRGlyphCache, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XRGlyphCache);
	});
	return class$;
}
