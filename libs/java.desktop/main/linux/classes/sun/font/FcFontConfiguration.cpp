...
$StringArray* FcFontConfiguration::getPlatformFontNames() {
	...
	for (int32_t i = 0; i < $nc(fcCompFonts)->length; ++i) {
		for (int32_t j = 0; j < $nc($nc(fcCompFonts->get(i))->allFonts)->length; ++j) {
			nameSet->add($nc($nc(fcCompFonts->get(i))->allFonts->get(j))->fontFile);
		}
	}
	return $cast($StringArray, nameSet->toArray($$new($StringArray, 0)));
}
...
$FontConfigManager$FontConfigFontArray* FcFontConfiguration::getFcFontList($FontConfigManager$FcCompFontArray* fcFonts, $String* fontname$renamed, int32_t style) {
	...
	for (int32_t i = 0; i < $nc(fcFonts)->length; ++i) {
		if (fontname->equals($nc(fcFonts->get(i))->jdkName) && style == $nc(fcFonts->get(i))->style) {
			return $nc(fcFonts->get(i))->allFonts;
		}
	}
	return $nc(fcFonts->get(0))->allFonts;
}
	...
	for (int32_t fontIndex = 0; fontIndex < $FontConfiguration::NUM_FONTS; ++fontIndex) {
		...
		for (int32_t styleIndex = 0; styleIndex < $FontConfiguration::NUM_STYLES; ++styleIndex) {
			...
			if ($FontConfiguration::installedFallbackFontFiles != nullptr) {
				numFonts += $FontConfiguration::installedFallbackFontFiles->length;
			}
			...
			if ($FontConfiguration::installedFallbackFontFiles != nullptr) {
				$System::arraycopy($FontConfiguration::installedFallbackFontFiles, 0, fileNames, fcFonts->length, $FontConfiguration::installedFallbackFontFiles->length);
			}
...
void FcFontConfiguration::writeFcInfo() {
	...
	if (fcInfo->cacheDirs != nullptr) {
		for (int32_t i = 0; i < fcInfo->cacheDirs->length; ++i) {
			if (fcInfo->cacheDirs->get(i) != nullptr) {
				props->setProperty($$str({"cachedir."_s, $$str(i)}), fcInfo->cacheDirs->get(i));
			}
		}
	}
	for (int32_t i = 0; i < $nc(this->fcCompFonts)->length; ++i) {
		$var($FontConfigManager$FcCompFont, fci, this->fcCompFonts->get(i));
		$var($String, styleKey, $str({$nc(fci)->jdkName, "."_s, $$str($nc(fci)->style)}));
		props->setProperty($$str({styleKey, ".length"_s}), $($Integer::toString($nc(fci->allFonts)->length)));
		for (int32_t j = 0; j < fci->allFonts->length; ++j) {
			props->setProperty($$str({styleKey, "."_s, $$str(j), ".file"_s}), $nc(fci->allFonts->get(j))->fontFile);
			if ($nc(fci->allFonts->get(j))->fullName != nullptr) {
				props->setProperty($$str({styleKey, "."_s, $$str(j), ".fullName"_s}), $nc(fci->allFonts->get(j))->fullName);
			}
		}
	}
	try {
		...
		props->store(fos, "JDK Font Configuration Generated File: *Do Not Edit*"_s);
		fos->close();
...
void FcFontConfiguration::readFcInfo() {
	...
	try {
		...
		if ($FontUtilities::debugFonts()) {
			...
			var$2->append("IOException ("_s);
			var$2->append($(e->getCause()));
			var$2->append(") reading from "_s);
			var$2->append($(fcFile->toString()));
			warning($$str(var$2));
		}
		return;
	}
	$var($String, version, $cast($String, props->get("version"_s)));
	if (version == nullptr || !version->equals(FcFontConfiguration::fileVersion)) {
		if ($FontUtilities::debugFonts()) {
	...
	int64_t lastModified = fcFile->lastModified();
		...
		for (int32_t i = 0; i < namesLen; ++i) {
			for (int32_t s = 0; s < numStyles; ++s) {
				...
				for (int32_t f = 0; f < nfonts; ++f) {
					...
					$set($nc($nc(fci->get(index))->allFonts->get(f)), fullName, fullName);
...
void FcFontConfiguration::clinit$($Class* clazz) {
...
$Class* FcFontConfiguration::load$($String* name, bool initialize) {
	...
	$loadClass(FcFontConfiguration, name, initialize, &classInfo$$, FcFontConfiguration::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(FcFontConfiguration);
	});
	return class$;
}
