...
void XRTextRenderer::drawGlyphList($SunGraphics2D* sg2d, $GlyphList* gl) {
	...
	try {
		...
		for (int32_t i = 0; i < gl->getNumGlyphs(); ++i) {
			...
			int32_t glyphSet = cacheEntry->getGlyphSet();
		...
		int32_t var$8 = $cast(int32_t, gl->getX());
		$nc(this->maskBuffer)->compositeText(x11sd, var$8, $cast(int32_t, gl->getY()), 0, maskFormat, this->eltList);
...
$Class* XRTextRenderer::load$($String* name, bool initialize) {
	...
	$loadClass(XRTextRenderer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XRTextRenderer);
	});
	return class$;
}
