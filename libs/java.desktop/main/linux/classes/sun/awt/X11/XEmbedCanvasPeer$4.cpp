...
void XEmbedCanvasPeer$4::run() {
	...
	if ($nc($XEmbedCanvasPeer::xembedLog)->isLoggable($PlatformLogger$Level::FINE)) {
		$XEmbedCanvasPeer::xembedLog->fine($$str({"Grabbing key: "_s, grab}));
	}
...
$Class* XEmbedCanvasPeer$4::load$($String* name, bool initialize) {
	...
	$loadClass(XEmbedCanvasPeer$4, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XEmbedCanvasPeer$4);
	});
	return class$;
}
