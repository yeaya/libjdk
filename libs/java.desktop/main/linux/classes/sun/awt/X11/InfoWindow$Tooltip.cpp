...
void InfoWindow$Tooltip::init$($Frame* parent, Object$* target, $InfoWindow$Tooltip$LiveArguments* liveArguments) {
	...
	this->textLabel->setBackground(InfoWindow$Tooltip::TOOLTIP_BACKGROUND_COLOR);
	this->textLabel->setFont(InfoWindow$Tooltip::TOOLTIP_TEXT_FONT);
	add(this->textLabel);
}
...
bool InfoWindow$Tooltip::isPointerOverTrayIcon($Rectangle* trayRect) {
	...
	return !($nc(p)->x < $nc(trayRect)->x || p->x > (trayRect->x + trayRect->width) || p->y < trayRect->y || p->y > (trayRect->y + trayRect->height));
}

void InfoWindow$Tooltip::clinit$($Class* clazz) {
...
$Class* InfoWindow$Tooltip::load$($String* name, bool initialize) {
	...
	$loadClass(InfoWindow$Tooltip, name, initialize, &classInfo$$, InfoWindow$Tooltip::clinit$, []($Class* clazz) -> $Object* {
