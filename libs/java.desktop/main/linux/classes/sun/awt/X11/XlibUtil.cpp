...
$Point* XlibUtil::translateCoordinates(int64_t src, int64_t dst, $Point* p, int32_t scale) {
	...
	$var($Throwable, var$0, nullptr);
	try {
		$var($XTranslateCoordinates, xtc, $new($XTranslateCoordinates, src, dst, $nc(p)->x * scale, $nc(p)->y * scale));
...
bool XlibUtil::isToplevelWindow(int64_t window) {
	...
	try {
		$init($XWM);
		$var($WindowPropertyGetter, wpg, $new($WindowPropertyGetter, window, $XWM::XA_WM_STATE, 0, 1, false, $XWM::XA_WM_STATE));
		$var($Throwable, var$3, nullptr);
		bool var$5 = false;
		bool return$4 = false;
		try {
...
void XlibUtil::clinit$($Class* clazz) {
...
$Class* XlibUtil::load$($String* name, bool initialize) {
	...
	$loadClass(XlibUtil, name, initialize, &classInfo$$, XlibUtil::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XlibUtil);
	});
	return class$;
}
