...
$String* XRenderPictFormat::getFieldsAsString() {
	...
	ret->append("direct = "_s)->append($(get_direct()))->append(", "_s);
...
$Class* XRenderPictFormat::load$($String* name, bool initialize) {
	...
	$loadClass(XRenderPictFormat, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XRenderPictFormat);
	});
	return class$;
}
