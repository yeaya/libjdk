...
void InfoWindow$Balloon$Displayer::run() {
	...
	while (true) {
		...
		this->this$0->_display($nc(msg)->caption, $nc(msg)->text, $nc(msg)->messageType);
	}
}
...
$Class* InfoWindow$Balloon$Displayer::load$($String* name, bool initialize) {
	...
	$loadClass(InfoWindow$Balloon$Displayer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(InfoWindow$Balloon$Displayer);
	});
	return class$;
}
