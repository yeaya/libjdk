...
void XDnDDropTargetProtocol::registerEmbedderDropSite(int64_t embedder) {
	...
	int32_t status = 0;
	$init($XDnDConstants);
	$var($WindowPropertyGetter, wpg1, $new($WindowPropertyGetter, embedder, $XDnDConstants::XA_XdndAware, 0, 1, false, $XConstants::AnyPropertyType));
	$var($Throwable, var$0, nullptr);
	try {
	...
	if (overriden && version >= 4) {
		$var($WindowPropertyGetter, wpg2, $new($WindowPropertyGetter, embedder, $XDnDConstants::XA_XdndProxy, 0, 1, false, $XAtom::XA_WINDOW));
		...
		if (proxy != 0) {
			$var($WindowPropertyGetter, wpg3, $new($WindowPropertyGetter, proxy, $XDnDConstants::XA_XdndProxy, 0, 1, false, $XAtom::XA_WINDOW));
			...
			try {
				...
				if (var$7 || $Native::getLong(wpg3->getData()) != proxy) {
					...
					$var($WindowPropertyGetter, wpg4, $new($WindowPropertyGetter, proxy, $XDnDConstants::XA_XdndAware, 0, 1, false, $XConstants::AnyPropertyType));
...
void XDnDDropTargetProtocol::registerEmbeddedDropSite(int64_t embedded) {
	...
	int32_t status = 0;
	$init($XDnDConstants);
	$var($WindowPropertyGetter, wpg1, $new($WindowPropertyGetter, embedded, $XDnDConstants::XA_XdndAware, 0, 1, false, $XConstants::AnyPropertyType));
	$var($Throwable, var$0, nullptr);
	try {
		status = wpg1->execute($($XErrorHandler$IgnoreBadWindowHandler::getInstance()));
		bool var$1 = status == $XConstants::Success && wpg1->getData() != 0;
	...
	if (overriden && version >= 4) {
		$var($WindowPropertyGetter, wpg2, $new($WindowPropertyGetter, embedded, $XDnDConstants::XA_XdndProxy, 0, 1, false, $XAtom::XA_WINDOW));
		$var($Throwable, var$3, nullptr);
		try {
			status = wpg2->execute($($XErrorHandler$IgnoreBadWindowHandler::getInstance()));
			bool var$4 = status == $XConstants::Success && wpg2->getData() != 0;
...
bool XDnDDropTargetProtocol::isProtocolSupported(int64_t window) {
	...
	$init($XDnDConstants);
	$var($WindowPropertyGetter, wpg1, $new($WindowPropertyGetter, window, $XDnDConstants::XA_XdndAware, 0, 1, false, $XConstants::AnyPropertyType));
	$var($Throwable, var$0, nullptr);
	bool var$2 = false;
	bool return$1 = false;
	try {
...
bool XDnDDropTargetProtocol::processXdndEnter($XClientMessageEvent* xclient) {
	...
	if (var$0 && getEmbedderRegistryEntry(xclient->get_window()) == nullptr) {
		return false;
	}
	int64_t var$1 = xclient->get_message_type();
	$init($XDnDConstants);
	if (var$1 != $nc($XDnDConstants::XA_XdndEnter)->getAtom()) {
		return false;
	}
	protocol_version = (int32_t)($sr(xclient->get_data(1) & $XDnDConstants::XDND_PROTOCOL_MASK, $XDnDConstants::XDND_PROTOCOL_SHIFT));
	...
	if (protocol_version < 2) {
		...
		$var($WindowPropertyGetter, wpg, $new($WindowPropertyGetter, source_win, $XDnDConstants::XA_XdndActionList, 0, 0x0000ffff, false, $XAtom::XA_ATOM));
	...
	if ((xclient->get_data(1) & $XDnDConstants::XDND_DATA_TYPES_BIT) != 0) {
		$var($WindowPropertyGetter, wpg, $new($WindowPropertyGetter, source_win, $XDnDConstants::XA_XdndTypeList, 0, 0x0000ffff, false, $XAtom::XA_ATOM));
...
bool XDnDDropTargetProtocol::processXdndPosition($XClientMessageEvent* xclient) {
	...
	x = (int32_t)(xclient->get_data(2) >> 16);
	y = (int32_t)(xclient->get_data(2) & 0xffff);
	if (xwindow != nullptr) {
	...
	if (this->sourceProtocolVersion > 0) {
		time_stamp = xclient->get_data(3) & (int64_t)0xffffffff;
	}
...
int32_t XDnDDropTargetProtocol::getMessageType($XClientMessageEvent* xclient) {
	...
	} else if (messageType == $nc($XDnDConstants::XA_XdndPosition)->getAtom()) {
		return $XDropTargetProtocol::MOTION_MESSAGE;
	} else if (messageType == $nc($XDnDConstants::XA_XdndLeave)->getAtom()) {
		return $XDropTargetProtocol::LEAVE_MESSAGE;
	} else if (messageType == $nc($XDnDConstants::XA_XdndDrop)->getAtom()) {
...
void XDnDDropTargetProtocol::sendEnterMessageToToplevel(int64_t toplevel, $XClientMessageEvent* xclient) {
	...
	if (this->sourceFormats != nullptr && this->sourceFormats->length > 3) {
		data1 |= $XDnDConstants::XDND_DATA_TYPES_BIT;
	}
	int64_t data2 = $nc(this->sourceFormats)->length > 0 ? this->sourceFormats->get(0) : 0;
	int64_t data3 = this->sourceFormats->length > 1 ? this->sourceFormats->get(1) : 0;
	int64_t data4 = this->sourceFormats->length > 2 ? this->sourceFormats->get(2) : 0;
...
$Object* XDnDDropTargetProtocol::getData(int64_t ctxt, int64_t format) {
	...
	if (message_type == $nc($XDnDConstants::XA_XdndPosition)->getAtom()) {
		time_stamp = xclient->get_data(3) & (int64_t)0xffffffff;
	} else if (message_type == $nc($XDnDConstants::XA_XdndDrop)->getAtom()) {
		time_stamp = xclient->get_data(2) & (int64_t)0xffffffff;
	} else {
		$throwNew($IllegalArgumentException);
	}
	return $nc($XDnDConstants::XDnDSelection)->getData(format, time_stamp);
}
...
bool XDnDDropTargetProtocol::sendDropDone(int64_t ctxt, bool success, int32_t dropAction) {
	...
	if (dropAction == $DnDConstants::ACTION_MOVE && success) {
		int64_t time_stamp = xclient->get_data(2) & (int64_t)0xffffffff;
		int64_t xdndSelectionAtom = $$nc($nc($XDnDConstants::XDnDSelection)->getSelectionAtom())->getAtom();
		$XToolkit::awtLock();
		$var($Throwable, var$1, nullptr);
		try {
			int64_t var$2 = $XToolkit::getDisplay();
			int64_t var$3 = $$nc($XAtom::get("DELETE"_s))->getAtom();
			int64_t var$4 = $$nc($XAtom::get("XAWT_SELECTION"_s))->getAtom();
			$XlibWrapper::XConvertSelection(var$2, xdndSelectionAtom, var$3, var$4, $$nc($XWindow::getXAWTRootWindow())->getWindow(), time_stamp);
		} catch ($Throwable& var$5) {
			$assign(var$1, var$5);
		} /*finally*/ {
			$XToolkit::awtUnlock();
	...
	try {
		...
		try {
			...
			$XlibWrapper::XSendEvent(var$8, xclient->get_data(0), false, $XConstants::NoEventMask, msg->pData);
...
void XDnDDropTargetProtocol::adjustEventForForwarding($XClientMessageEvent* xclient, $XDropTargetProtocol$EmbedderRegistryEntry* entry) {
	...
	if (var$0 == $nc($XDnDConstants::XA_XdndEnter)->getAtom()) {
		...
		if ($nc(XDnDDropTargetProtocol::logger)->isLoggable($PlatformLogger$Level::FINEST)) {
			XDnDDropTargetProtocol::logger->finest($$str({"          entryVersion="_s, $$str(version), " sourceProtocolVersion="_s, $$str(this->sourceProtocolVersion), " sourceFormats.length="_s, $$str((this->sourceFormats != nullptr ? this->sourceFormats->length : 0))}));
		}
...
void XDnDDropTargetProtocol::notifyProtocolListener($XWindow* xwindow, int32_t x, int32_t y, int32_t dropAction, $XClientMessageEvent* xclient, int32_t eventID) {
	...
	if (xclient != nullptr) {
		...
		XDnDDropTargetProtocol::unsafe->copyMemory(xclient->pData, nativeCtxt, size);
		int64_t data1 = $sl(this->sourceProtocolVersion, $XDnDConstants::XDND_PROTOCOL_SHIFT);
		if (this->sourceFormats != nullptr && this->sourceFormats->length > 3) {
			data1 |= $XDnDConstants::XDND_DATA_TYPES_BIT;
		}
		$Native::putLong(nativeCtxt + size, data1);
		$Native::putLong(nativeCtxt + size + $Native::getLongSize(), $nc(this->sourceFormats)->length > 0 ? this->sourceFormats->get(0) : 0);
		$Native::putLong(nativeCtxt + size + 2 * $Native::getLongSize(), $nc(this->sourceFormats)->length > 1 ? this->sourceFormats->get(1) : 0);
		$Native::putLong(nativeCtxt + size + 3 * $Native::getLongSize(), $nc(this->sourceFormats)->length > 2 ? this->sourceFormats->get(2) : 0);
	}
...
bool XDnDDropTargetProtocol::forwardEventToEmbedded(int64_t embedded, int64_t ctxt, int32_t eventID) {
	...
	if ($nc(XDnDDropTargetProtocol::logger)->isLoggable($PlatformLogger$Level::FINEST)) {
		...
		var$0->append(ctxt != 0 ? getMessageType($$new($XClientMessageEvent, ctxt)) : 0);
		var$0->append(" prevCtxt="_s);
		var$0->append(this->prevCtxt);
		var$0->append(" prevType="_s);
		var$0->append(this->prevCtxt != 0 ? getMessageType($$new($XClientMessageEvent, this->prevCtxt)) : 0);
		XDnDDropTargetProtocol::logger->finest($$str(var$0));
	}
	...
	if (ctxt != 0) {
		...
		if (!this->overXEmbedClient) {
			...
			if (XDnDDropTargetProtocol::logger->isLoggable($PlatformLogger$Level::FINEST)) {
				XDnDDropTargetProtocol::logger->finest($$str({"         1  embedded="_s, $$str(embedded), " source="_s, $$str(xclient->get_data(0)), " data1="_s, $$str(data1), " data2="_s, $$str(data2), " data3="_s, $$str(data3), " data4="_s, $$str(data4)}));
			}
			if ((data1 & $XDnDConstants::XDND_DATA_TYPES_BIT) != 0) {
				$init($XDnDConstants);
				$var($WindowPropertyGetter, wpg, $new($WindowPropertyGetter, xclient->get_data(0), $XDnDConstants::XA_XdndTypeList, 0, 0x0000ffff, false, $XAtom::XA_ATOM));
				$var($Throwable, var$2, nullptr);
				try {
					wpg->execute($($XErrorHandler$IgnoreBadWindowHandler::getInstance()));
					bool var$3 = wpg->getActualType() == $XAtom::XA_ATOM;
					if (var$3 && wpg->getActualFormat() == 32) {
						...
						try {
							...
							$XErrorHandlerUtil::RESTORE_XERROR_HANDLER();
							if (($XErrorHandlerUtil::saved_error != nullptr) && ($nc($XErrorHandlerUtil::saved_error)->get_error_code() != $XConstants::Success)) {
								if (XDnDDropTargetProtocol::logger->isLoggable($PlatformLogger$Level::WARNING)) {
									XDnDDropTargetProtocol::logger->warning("Cannot set XdndTypeList on the proxy window"_s);
								}
							}
					...
					} else if (XDnDDropTargetProtocol::logger->isLoggable($PlatformLogger$Level::WARNING)) {
						XDnDDropTargetProtocol::logger->warning("Cannot read XdndTypeList from the source window"_s);
					}
				} catch ($Throwable& var$8) {
					$assign(var$2, var$8);
				} /*finally*/ {
		...
		if (XDnDDropTargetProtocol::logger->isLoggable($PlatformLogger$Level::FINEST)) {
			XDnDDropTargetProtocol::logger->finest($$str({"         2  embedded="_s, $$str(embedded), " xclient="_s, xclient}));
		}
		{
	...
	if (ctxt != 0 && this->overXEmbedClient) {
		...
		XDnDDropTargetProtocol::unsafe->copyMemory(ctxt, this->prevCtxt, size + 4 * $Native::getLongSize());
	}
	return true;
}
...
void XDnDDropTargetProtocol::clinit$($Class* clazz) {
...
$Class* XDnDDropTargetProtocol::load$($String* name, bool initialize) {
	...
	$loadClass(XDnDDropTargetProtocol, name, initialize, &classInfo$$, XDnDDropTargetProtocol::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XDnDDropTargetProtocol);
	});
	return class$;
}
