...
int64_t XAtom::get32Property(int64_t window, int64_t property_type) {
	...
	checkWindow(window);
	$var($WindowPropertyGetter, getter, $new($WindowPropertyGetter, window, this, 0, 1, false, property_type));
	$var($Throwable, var$0, nullptr);
	int64_t var$2 = 0;
	bool return$1 = false;
	try {
...
bool XAtom::getAtomData(int64_t window, int64_t data_ptr, int32_t length) {
	...
	checkWindow(window);
	$var($WindowPropertyGetter, getter, $new($WindowPropertyGetter, window, this, 0, (int64_t)length, false, this));
	$var($Throwable, var$0, nullptr);
	bool var$2 = false;
	bool return$1 = false;
	try {
		...
		int64_t var$5 = getter->getData();
		$XlibWrapper::memcpy(data_ptr, var$5, length * getAtomSize());
		var$2 = true;
		return$1 = true;
		goto $finally;
...
bool XAtom::getAtomData(int64_t window, int64_t type, int64_t data_ptr, int32_t length) {
	...
	checkWindow(window);
	$var($WindowPropertyGetter, getter, $new($WindowPropertyGetter, window, this, 0, (int64_t)length, false, type));
	$var($Throwable, var$0, nullptr);
	bool var$2 = false;
	bool return$1 = false;
	try {
...
void XAtom::DeleteProperty($XBaseWindow* window) {
	...
	try {
		...
		$XlibWrapper::XDeleteProperty(var$1, window->getWindow(), this->atom);
...
$bytes* XAtom::getByteArrayProperty(int64_t window, int64_t property_type) {
	...
	checkWindow(window);
	$var($WindowPropertyGetter, getter, $new($WindowPropertyGetter, window, this, 0, 0x0000ffff, false, property_type));
	$var($Throwable, var$0, nullptr);
...
$XAtomArray* XAtom::getAtomListProperty(int64_t window) {
	...
	checkWindow(window);
	$var($WindowPropertyGetter, getter, $new($WindowPropertyGetter, window, this, 0, 0x0000ffff, false, XAtom::XA_ATOM));
	$var($Throwable, var$0, nullptr);
...
int64_t XAtom::toData($XAtomArray* atoms) {
	...
	for (int32_t i = 0; i < atoms->length; ++i) {
...
int32_t XAtom::hashCode() {
	return (int32_t)((this->atom ^ this->display) & (int64_t)0xffff);
}
...
int64_t XAtom::getWindowProperty(int64_t window) {
	...
	checkWindow(window);
	$var($WindowPropertyGetter, getter, $new($WindowPropertyGetter, window, this, 0, 1, false, XAtom::XA_WINDOW));
	$var($Throwable, var$0, nullptr);
	int64_t var$2 = 0;
	bool return$1 = false;
	try {
...
$Class* XAtom::load$($String* name, bool initialize) {
	...
	$loadClass(XAtom, name, initialize, &classInfo$$, XAtom::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XAtom);
	});
	return class$;
}
