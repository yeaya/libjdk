...
void XNETProtocol::setState($XWindowPeer* window, int32_t state) {
	...
	if ($nc(XNETProtocol::log)->isLoggable($PlatformLogger$Level::FINE)) {
		XNETProtocol::log->fine($$str({"Setting state of "_s, window, " to "_s, $$str(state)}));
	}
...
void XNETProtocol::setInitialState($XWindowPeer* window, int32_t state) {
	...
	$init($PlatformLogger$Level);
	if ($nc(XNETProtocol::log)->isLoggable($PlatformLogger$Level::FINE)) {
		XNETProtocol::log->fine("Current state of the window {0} is {1}"_s, $$new($ObjectArray, {
			window,
			old_state
		}));
	}
	...
	if (XNETProtocol::log->isLoggable($PlatformLogger$Level::FINE)) {
		XNETProtocol::log->fine("Setting initial state of the window {0} to {1}"_s, $$new($ObjectArray, {
			window,
			old_state
		}));
	}
...
void XNETProtocol::requestState($XWindowPeer* window, int32_t state) {
	...
	int32_t max_changed = (state ^ old_net_state) & ($Frame::MAXIMIZED_BOTH);
	...
	try {
		...
		$init($PlatformLogger$Level);
		if ($nc(XNETProtocol::log)->isLoggable($PlatformLogger$Level::FINE)) {
			XNETProtocol::log->fine($$str({"Requesting state on "_s, window, " for "_s, $$str(state)}));
		}
		...
		try {
			...
			$XlibWrapper::XSendEvent(var$3, $XlibWrapper::RootWindow(var$4, window->getScreenNumber()), false, $XConstants::SubstructureRedirectMask | $XConstants::SubstructureNotifyMask, req->pData);
...
bool XNETProtocol::isStateChange($XPropertyEvent* e) {
	...
	bool res = var$0;
	if (res) {
		$var($XWindowPeer, wpeer, $cast($XWindowPeer, $XToolkit::windowToXWindow($nc(e)->get_window())));
...
void XNETProtocol::requestState($XWindow* window, $XAtom* state, bool isAdd) {
	...
	try {
		...
		$init($PlatformLogger$Level);
		if ($nc(XNETProtocol::log)->isLoggable($PlatformLogger$Level::FINE)) {
			XNETProtocol::log->fine("Setting _NET_STATE atom {0} on {1} for {2}"_s, $$new($ObjectArray, {
				state,
				window,
		...
		try {
			...
			int64_t var$3 = $XToolkit::getDisplay();
			$XlibWrapper::XSendEvent(var$2, $XlibWrapper::RootWindow(var$3, window->getScreenNumber()), false, $XConstants::SubstructureRedirectMask | $XConstants::SubstructureNotifyMask, req->pData);
...
void XNETProtocol::setStateHelper($XWindowPeer* window, $XAtom* state, bool set) {
	...
	if ($nc(XNETProtocol::log)->isLoggable($PlatformLogger$Level::FINER)) {
		XNETProtocol::log->finer("Window visibility is: withdrawn={0}, visible={1}, mapped={2} showing={3}"_s, $$new($ObjectArray, {
			$($Boolean::valueOf($nc(window)->isWithdrawn())),
			$($Boolean::valueOf($nc(window)->isVisible())),
			$($Boolean::valueOf($nc(window)->isMapped())),
			$($Boolean::valueOf($nc(window)->isShowing()))
		}));
	}
	if ($nc(window)->isShowing()) {
		...
		if (XNETProtocol::log->isLoggable($PlatformLogger$Level::FINER)) {
			XNETProtocol::log->finer("Current state on {0} is {1}"_s, $$new($ObjectArray, {
				window,
				net_wm_state
			}));
		}
		...
		if (XNETProtocol::log->isLoggable($PlatformLogger$Level::FINE)) {
			XNETProtocol::log->fine("Setting states on {0} to {1}"_s, $$new($ObjectArray, {
				window,
				net_wm_state
			}));
		}
...
void XNETProtocol::detect() {
	...
	$init($PlatformLogger$Level);
	if ($nc(XNETProtocol::log)->isLoggable($PlatformLogger$Level::FINE)) {
		XNETProtocol::log->fine($$str({"### "_s, this, " is active: "_s, $$str((this->NetWindow != 0))}));
	}
}
...
bool XNETProtocol::isWMName($String* name) {
	...
	$init($PlatformLogger$Level);
	if ($nc(XNETProtocol::log)->isLoggable($PlatformLogger$Level::FINE)) {
		XNETProtocol::log->fine($$str({"### WM_NAME = "_s, net_wm_name_string}));
	}
...
void XNETProtocol::setWMIcons($XWindowPeer* window, $List* icons) {
	...
	if (icons == nullptr) {
		$nc(iconsAtom)->DeleteProperty(window);
		return;
	}
	int32_t length = 0;
	{
	...
	if (bufferSize != 0) {
		...
		try {
			...
			$nc(iconsAtom)->setAtomData(var$1, $XAtom::XA_CARDINAL, buffer, $div(bufferSize, $Native::getCard32Size()));
...
void XNETProtocol::clinit$($Class* clazz) {
...
$Class* XNETProtocol::load$($String* name, bool initialize) {
	...
	$loadClass(XNETProtocol, name, initialize, &classInfo$$, XNETProtocol::clinit$, []($Class* clazz) -> $Object* {
