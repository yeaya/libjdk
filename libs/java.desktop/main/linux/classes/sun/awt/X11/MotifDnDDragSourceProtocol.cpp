...
bool MotifDnDDragSourceProtocol::processClientMessage($XClientMessageEvent* xclient) {
	...
	int8_t reason = (int8_t)($nc(MotifDnDDragSourceProtocol::unsafe)->getByte(data) & $MotifDnDConstants::MOTIF_MESSAGE_REASON_MASK);
	int8_t origin = (int8_t)(MotifDnDDragSourceProtocol::unsafe->getByte(data) & $MotifDnDConstants::MOTIF_MESSAGE_SENDER_MASK);
	int8_t byteOrder = MotifDnDDragSourceProtocol::unsafe->getByte(data + 1);
	...
	int32_t t = MotifDnDDragSourceProtocol::unsafe->getInt(data + 4);
	if (swapNeeded) {
		t = $MotifDnDConstants$Swapper::swap(t);
	}
	int64_t time = t & (int64_t)0xffffffff;
	...
	if (reason != $MotifDnDConstants::DROP_SITE_LEAVE) {
		int16_t flags = MotifDnDDragSourceProtocol::unsafe->getShort(data + 2);
		if (swapNeeded) {
			flags = $MotifDnDConstants$Swapper::swap(flags);
		}
		int8_t status = (int8_t)($sr(flags & $MotifDnDConstants::MOTIF_DND_STATUS_MASK, $MotifDnDConstants::MOTIF_DND_STATUS_SHIFT));
		int8_t motif_action = (int8_t)($sr(flags & $MotifDnDConstants::MOTIF_DND_ACTION_MASK, $MotifDnDConstants::MOTIF_DND_ACTION_SHIFT));
		...
		int16_t tx = MotifDnDDragSourceProtocol::unsafe->getShort(data + 8);
		int16_t ty = MotifDnDDragSourceProtocol::unsafe->getShort(data + 10);
		if (swapNeeded) {
...
$XDragSourceProtocol$TargetWindowInfo* MotifDnDDragSourceProtocol::getTargetWindowInfo(int64_t window) {
	...
	try {
		...
		if (var$3 && wpg->getNumberOfItems() >= $MotifDnDConstants::MOTIF_RECEIVER_INFO_SIZE) {
			...
			int8_t dragProtocolStyle = MotifDnDDragSourceProtocol::unsafe->getByte(data + 2);
			{
				int32_t proxy = 0;
				...
				case $MotifDnDConstants::MOTIF_PREFER_RECEIVER_STYLE:
					proxy = MotifDnDDragSourceProtocol::unsafe->getInt(data + 4);
					if (byteOrderByte != $MotifDnDConstants::getByteOrderByte()) {
						proxy = $MotifDnDConstants$Swapper::swap(proxy);
					}
					protocolVersion = MotifDnDDragSourceProtocol::unsafe->getByte(data + 1);
...
void MotifDnDDragSourceProtocol::sendEnterMessage($longs* formats, int32_t sourceAction, int32_t sourceActions, int64_t time) {
	...
	try {
		...
		MotifDnDDragSourceProtocol::unsafe->putByte(data + 1, $MotifDnDConstants::getByteOrderByte());
		MotifDnDDragSourceProtocol::unsafe->putShort(data + 2, (int16_t)flags);
		MotifDnDDragSourceProtocol::unsafe->putInt(data + 4, (int32_t)time);
		MotifDnDDragSourceProtocol::unsafe->putInt(data + 8, (int32_t)$XDragSourceProtocol::getDragSourceWindow());
		MotifDnDDragSourceProtocol::unsafe->putInt(data + 12, (int32_t)$nc($MotifDnDConstants::XA_MOTIF_ATOM_0)->getAtom());
...
void MotifDnDDragSourceProtocol::sendMoveMessage(int32_t xRoot, int32_t yRoot, int32_t sourceAction, int32_t sourceActions, int64_t time) {
	...
	try {
		...
		MotifDnDDragSourceProtocol::unsafe->putShort(data + 2, (int16_t)flags);
		MotifDnDDragSourceProtocol::unsafe->putInt(data + 4, (int32_t)time);
		MotifDnDDragSourceProtocol::unsafe->putShort(data + 8, (int16_t)xRoot);
		MotifDnDDragSourceProtocol::unsafe->putShort(data + 10, (int16_t)yRoot);
		int64_t var$2 = $XToolkit::getDisplay();
		$XlibWrapper::XSendEvent(var$2, getTargetProxyWindow(), false, $XConstants::NoEventMask, msg->pData);
	} catch ($Throwable& var$3) {
		$assign(var$0, var$3);
...
void MotifDnDDragSourceProtocol::sendLeaveMessage(int64_t time) {
	...
	try {
		...
		MotifDnDDragSourceProtocol::unsafe->putByte(data + 1, $MotifDnDConstants::getByteOrderByte());
		MotifDnDDragSourceProtocol::unsafe->putShort(data + 2, (int16_t)0);
		MotifDnDDragSourceProtocol::unsafe->putInt(data + 4, (int32_t)time);
		MotifDnDDragSourceProtocol::unsafe->putInt(data + 8, (int32_t)$XDragSourceProtocol::getDragSourceWindow());
...
void MotifDnDDragSourceProtocol::sendDropMessage(int32_t xRoot, int32_t yRoot, int32_t sourceAction, int32_t sourceActions, int64_t time) {
	...
	try {
		...
		MotifDnDDragSourceProtocol::unsafe->putShort(data + 10, (int16_t)yRoot);
		MotifDnDDragSourceProtocol::unsafe->putInt(data + 12, (int32_t)$nc($MotifDnDConstants::XA_MOTIF_ATOM_0)->getAtom());
		MotifDnDDragSourceProtocol::unsafe->putInt(data + 16, (int32_t)$XDragSourceProtocol::getDragSourceWindow());
		int64_t var$2 = $XToolkit::getDisplay();
		$XlibWrapper::XSendEvent(var$2, getTargetProxyWindow(), false, $XConstants::NoEventMask, msg->pData);
	} catch ($Throwable& var$3) {
		$assign(var$0, var$3);
...
void MotifDnDDragSourceProtocol::dispatchEvent($XEvent* ev) {
	...
	case $XConstants::SelectionRequest:
		...
		if (atom == $nc($MotifDnDConstants::XA_MOTIF_ATOM_0)->getAtom()) {
			...
			} else if (target == $nc($MotifDnDConstants::XA_XmTRANSFER_FAILURE)->getAtom()) {
...
void MotifDnDDragSourceProtocol::clinit$($Class* clazz) {
...
$Class* MotifDnDDragSourceProtocol::load$($String* name, bool initialize) {
	...
	$loadClass(MotifDnDDragSourceProtocol, name, initialize, &classInfo$$, MotifDnDDragSourceProtocol::clinit$, []($Class* clazz) -> $Object* {
