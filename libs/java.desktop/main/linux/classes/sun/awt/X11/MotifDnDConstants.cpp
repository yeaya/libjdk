...
int64_t MotifDnDConstants::readMotifWindow() {
	...
	$var($WindowPropertyGetter, wpg, $new($WindowPropertyGetter, defaultRootWindow, MotifDnDConstants::XA_MOTIF_DRAG_WINDOW, 0, 1, false, $XConstants::AnyPropertyType));
	...
	try {
		...
		try {
			...
			try {
				...
				$XlibWrapper::XChangeProperty(var$9, defaultRootWindow, $nc(MotifDnDConstants::XA_MOTIF_DRAG_WINDOW)->getAtom(), $XAtom::XA_WINDOW, 32, $XConstants::PropModeReplace, data, 1);
...
$longArray2* MotifDnDConstants::getTargetListTable(int64_t motifWindow) {
	$init(MotifDnDConstants);
	$useLocalObjectStack();
	$var($WindowPropertyGetter, wpg, $new($WindowPropertyGetter, motifWindow, MotifDnDConstants::XA_MOTIF_DRAG_TARGETS, 0, 100000, false, $nc(MotifDnDConstants::XA_MOTIF_DRAG_TARGETS)->getAtom()));
	$var($Throwable, var$0, nullptr);
	...
	try {
		...
		if (!var$4) {
			...
			var$4 = var$5 != MotifDnDConstants::XA_MOTIF_DRAG_TARGETS->getAtom();
		}
		bool var$3 = var$4;
		...
		int8_t var$6 = MotifDnDConstants::unsafe->getByte(data + 0);
		bool swapNeeded = var$6 != getByteOrderByte();
		int16_t numTargetLists = MotifDnDConstants::unsafe->getShort(data + 2);
		if (swapNeeded) {
		...
		for (int16_t i = 0; i < numTargetLists; ++i) {
			int16_t numTargets = MotifDnDConstants::unsafe->getShort(bufptr);
			bufptr += 2;
			if (swapNeeded) {
				numTargets = $MotifDnDConstants$Swapper::swap(numTargets);
			}
			table->set(i, $$new($longs, numTargets));
			for (int16_t j = 0; j < numTargets; ++j) {
				int32_t target = 0;
				if (byteOrder == $ByteOrder::LITTLE_ENDIAN) {
					for (int32_t idx = 0; idx < 4; ++idx) {
						target |= ($sl(MotifDnDConstants::unsafe->getByte(bufptr + idx), 8 * idx)) & ($sl(255, 8 * idx));
					}
				} else {
					for (int32_t idx = 0; idx < 4; ++idx) {
						target |= ($sl(MotifDnDConstants::unsafe->getByte(bufptr + idx), 8 * (3 - idx))) & ($sl(255, 8 * (3 - idx)));
					}
				}
...
void MotifDnDConstants::putTargetListTable(int64_t motifWindow, $longArray2* table) {
	...
	$var($Throwable, var$0, nullptr);
	try {
		MotifDnDConstants::unsafe->putByte(data + 0, getByteOrderByte());
		MotifDnDConstants::unsafe->putByte(data + 1, MotifDnDConstants::MOTIF_DND_PROTOCOL_VERSION);
		MotifDnDConstants::unsafe->putShort(data + 2, (int16_t)table->length);
		MotifDnDConstants::unsafe->putInt(data + 4, tableSize);
		int64_t bufptr = data + 8;
		for (int32_t i = 0; i < table->length; ++i) {
			MotifDnDConstants::unsafe->putShort(bufptr, (int16_t)$nc(table->get(i))->length);
			bufptr += 2;
			for (int32_t j = 0; j < $nc(table->get(i))->length; ++j) {
				...
				if ($ByteOrder::nativeOrder() == $ByteOrder::LITTLE_ENDIAN) {
					for (int32_t idx = 0; idx < 4; ++idx) {
						int8_t b = (int8_t)($sr(target & ($sl(255, 8 * idx)), 8 * idx));
						MotifDnDConstants::unsafe->putByte(bufptr + idx, b);
					}
				} else {
					for (int32_t idx = 0; idx < 4; ++idx) {
						int8_t b = (int8_t)($sr(target & ($sl(255, 8 * idx)), 8 * idx));
						MotifDnDConstants::unsafe->putByte(bufptr + (3 - idx), b);
					}
				}
				bufptr += 4;
			}
		...
		int64_t var$2 = $nc(MotifDnDConstants::XA_MOTIF_DRAG_TARGETS)->getAtom();
		$XlibWrapper::XChangeProperty(var$1, motifWindow, var$2, MotifDnDConstants::XA_MOTIF_DRAG_TARGETS->getAtom(), 8, $XConstants::PropModeReplace, data, tableSize);
		$XErrorHandlerUtil::RESTORE_XERROR_HANDLER();
		if (($XErrorHandlerUtil::saved_error != nullptr) && ($nc($XErrorHandlerUtil::saved_error)->get_error_code() != $XConstants::Success)) {
			motifWindow = createMotifWindow();
			$XErrorHandlerUtil::WITH_XERROR_HANDLER($($XErrorHandler$VerifyChangePropertyHandler::getInstance()));
			int64_t var$3 = $XToolkit::getDisplay();
			int64_t var$4 = MotifDnDConstants::XA_MOTIF_DRAG_TARGETS->getAtom();
			$XlibWrapper::XChangeProperty(var$3, motifWindow, var$4, MotifDnDConstants::XA_MOTIF_DRAG_TARGETS->getAtom(), 8, $XConstants::PropModeReplace, data, tableSize);
			$XErrorHandlerUtil::RESTORE_XERROR_HANDLER();
			if (($XErrorHandlerUtil::saved_error != nullptr) && ($nc($XErrorHandlerUtil::saved_error)->get_error_code() != $XConstants::Success)) {
		...
		MotifDnDConstants::unsafe->freeMemory(data);
	}
	if (var$0 != nullptr) {
		$throw(var$0);
	}
...
int32_t MotifDnDConstants::getIndexForTargetList($longs* formats$renamed) {
	...
	try {
		...
		if (table != nullptr) {
			for (int32_t i = 0; i < table->length; ++i) {
				bool equals = true;
				if ($nc(table->get(i))->length == formats->length) {
					for (int32_t j = 0; j < $nc(table->get(i))->length; ++j) {
...
void MotifDnDConstants::writeDragInitiatorInfoStruct(int64_t window, int32_t index) {
	...
	$var($Throwable, var$0, nullptr);
	try {
		MotifDnDConstants::unsafe->putByte(structData, getByteOrderByte());
		MotifDnDConstants::unsafe->putByte(structData + 1, MotifDnDConstants::MOTIF_DND_PROTOCOL_VERSION);
		MotifDnDConstants::unsafe->putShort(structData + 2, (int16_t)index);
		MotifDnDConstants::unsafe->putInt(structData + 4, (int32_t)$nc(MotifDnDConstants::XA_MOTIF_ATOM_0)->getAtom());
		$XErrorHandlerUtil::WITH_XERROR_HANDLER($($XErrorHandler$VerifyChangePropertyHandler::getInstance()));
		int64_t var$1 = $XToolkit::getDisplay();
		int64_t var$2 = MotifDnDConstants::XA_MOTIF_ATOM_0->getAtom();
		$XlibWrapper::XChangeProperty(var$1, window, var$2, $nc(MotifDnDConstants::XA_MOTIF_DRAG_INITIATOR_INFO)->getAtom(), 8, $XConstants::PropModeReplace, structData, MotifDnDConstants::MOTIF_INITIATOR_INFO_SIZE);
		$XErrorHandlerUtil::RESTORE_XERROR_HANDLER();
		if (($XErrorHandlerUtil::saved_error != nullptr) && ($nc($XErrorHandlerUtil::saved_error)->get_error_code() != $XConstants::Success)) {
		...
		MotifDnDConstants::unsafe->freeMemory(structData);
	}
	if (var$0 != nullptr) {
		$throw(var$0);
	}
...
void MotifDnDConstants::writeDragReceiverInfoStruct(int64_t window) {
	...
	$var($Throwable, var$0, nullptr);
	try {
		MotifDnDConstants::unsafe->putByte(data, MotifDnDConstants::getByteOrderByte());
		MotifDnDConstants::unsafe->putByte(data + 1, MotifDnDConstants::MOTIF_DND_PROTOCOL_VERSION);
		MotifDnDConstants::unsafe->putByte(data + 2, (int8_t)MotifDnDConstants::MOTIF_DYNAMIC_STYLE);
		MotifDnDConstants::unsafe->putByte(data + 3, (int8_t)0);
		MotifDnDConstants::unsafe->putInt(data + 4, (int32_t)window);
		MotifDnDConstants::unsafe->putShort(data + 8, (int16_t)0);
		MotifDnDConstants::unsafe->putShort(data + 10, (int16_t)0);
		MotifDnDConstants::unsafe->putInt(data + 12, dataSize);
		$XErrorHandlerUtil::WITH_XERROR_HANDLER($($XErrorHandler$VerifyChangePropertyHandler::getInstance()));
		int64_t var$1 = $XToolkit::getDisplay();
		int64_t var$2 = $nc(MotifDnDConstants::XA_MOTIF_DRAG_RECEIVER_INFO)->getAtom();
		$XlibWrapper::XChangeProperty(var$1, window, var$2, MotifDnDConstants::XA_MOTIF_DRAG_RECEIVER_INFO->getAtom(), 8, $XConstants::PropModeReplace, data, dataSize);
		$XErrorHandlerUtil::RESTORE_XERROR_HANDLER();
		if (($XErrorHandlerUtil::saved_error != nullptr) && ($nc($XErrorHandlerUtil::saved_error)->get_error_code() != $XConstants::Success)) {
...
int32_t MotifDnDConstants::getJavaActionsForMotifActions(int32_t motifActions) {
	...
	if ((motifActions & MotifDnDConstants::MOTIF_DND_MOVE) != 0) {
		javaActions |= $DnDConstants::ACTION_MOVE;
	}
	if ((motifActions & MotifDnDConstants::MOTIF_DND_COPY) != 0) {
		javaActions |= $DnDConstants::ACTION_COPY;
	}
	if ((motifActions & MotifDnDConstants::MOTIF_DND_LINK) != 0) {
		javaActions |= $DnDConstants::ACTION_LINK;
	}
	return javaActions;
}

void MotifDnDConstants::clinit$($Class* clazz) {
...
$Class* MotifDnDConstants::load$($String* name, bool initialize) {
	...
	$loadClass(MotifDnDConstants, name, initialize, &classInfo$$, MotifDnDConstants::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(MotifDnDConstants);
	});
	return class$;
}
