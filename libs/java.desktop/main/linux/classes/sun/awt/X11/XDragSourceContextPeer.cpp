...
XDragSourceContextPeer* XDragSourceContextPeer::createDragSourceContextPeer($DragGestureEvent* dge) {
	$init(XDragSourceContextPeer);
	XDragSourceContextPeer::theInstance->setTrigger(dge);
	return XDragSourceContextPeer::theInstance;
}
...
void XDragSourceContextPeer::setProxyModeSourceWindow(int64_t window) {
	$init(XDragSourceContextPeer);
	XDragSourceContextPeer::theInstance->setProxyModeSourceWindowImpl(window);
}
...
void XDragSourceContextPeer::updateTargetWindow($XMotionEvent* xmotion) {
	...
	int32_t y = scaleDown(xmotion->get_y_root());
	int64_t time = xmotion->get_time();
...
void XDragSourceContextPeer::processMouseMove($XMotionEvent* xmotion) {
	...
	int32_t motionYRoot = scaleDown(xmotion->get_y_root());
	if (this->xRoot != motionXRoot || this->yRoot != motionYRoot) {
		this->xRoot = motionXRoot;
		this->yRoot = motionYRoot;
		postDragSourceDragEvent(this->targetAction, $XWindow::getModifiers(xmotion->get_state(), 0, 0), this->xRoot, this->yRoot, $SunDragSourceContextPeer::DISPATCH_MOUSE_MOVED);
	}
	...
	if (this->dragProtocol != nullptr) {
		int32_t var$0 = xmotion->get_x_root();
		int32_t var$1 = xmotion->get_y_root();
		int32_t var$2 = this->sourceAction;
		int32_t var$3 = this->sourceActions;
		this->dragProtocol->sendMoveMessage(var$0, var$1, var$2, var$3, xmotion->get_time());
	}
}
...
bool XDragSourceContextPeer::processProxyModeEvent($XEvent* ev) {
	...
	if ($nc(XDragSourceContextPeer::logger)->isLoggable($PlatformLogger$Level::FINEST)) {
		XDragSourceContextPeer::logger->finest($$str({"        proxyModeSourceWindow="_s, $$str(getProxyModeSourceWindow()), " ev="_s, ev}));
	}
	...
	case $XConstants::DestroyNotify:
		{
			...
			if (var$0) {
				...
				var$0 = var$1 == this->dragProtocol->getTargetWindow();
			}
			if (var$0) {
...
case$4:
				// (int)XKeySymConstants.XK_Shift_L
				{
					...
					$XlibWrapper::XQueryPointer(var$4, xkey->get_root(), $XlibWrapper::larg1, $XlibWrapper::larg2, $XlibWrapper::larg3, $XlibWrapper::larg4, $XlibWrapper::larg5, $XlibWrapper::larg6, $XlibWrapper::larg7);
	...
	case $XConstants::ButtonRelease:
		{
			...
			$init($XConstants);
			bool var$9 = xbutton->get_button() == $nc($XConstants::buttons)->get(0);
			if (var$9 || xbutton->get_button() == $XConstants::buttons->get(1)) {
...
bool XDragSourceContextPeer::processEvent($XEvent* ev) {
	...
	try {
		try {
			var$2 = XDragSourceContextPeer::theInstance->doProcessEvent(ev);
			return$1 = true;
			goto $finally;
		} catch ($XException& e) {
			e->printStackTrace();
...
void XDragSourceContextPeer::clinit$($Class* clazz) {
...
$Class* XDragSourceContextPeer::load$($String* name, bool initialize) {
	...
	$loadClass(XDragSourceContextPeer, name, initialize, &classInfo$$, XDragSourceContextPeer::clinit$, []($Class* clazz) -> $Object* {
