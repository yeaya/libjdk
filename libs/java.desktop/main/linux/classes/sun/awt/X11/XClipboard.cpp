...
int64_t XClipboard::getID() {
	return $$nc(this->selection->getSelectionAtom())->getAtom();
}
...
int32_t XClipboard::getPollInterval() {
	...
	$synchronized(XClipboard::classLock) {
		if (XClipboard::pollInterval <= 0) {
			XClipboard::pollInterval = $$sure($Integer, $AccessController::doPrivileged($$new($GetIntegerAction, "awt.datatransfer.clipboard.poll.interval"_s, XClipboard::defaultPollInterval)))->intValue();
			if (XClipboard::pollInterval <= 0) {
...
$XAtom* XClipboard::getTargetsPropertyAtom() {
	...
	if (nullptr == this->targetsPropertyAtom) {
		$set(this, targetsPropertyAtom, $XAtom::get($$str({"XAWT_TARGETS_OF_SELECTION:"_s, $($$nc(this->selection->getSelectionAtom())->getName())})));
	}
...
void XClipboard::registerClipboardViewerChecked() {
	...
	try {
		$synchronized(XClipboard::classLock) {
			...
			XClipboard::targetsAtom2Clipboard->put($($Long::valueOf($$nc(getTargetsPropertyAtom())->getAtom())), this);
			if (mustSchedule) {
		...
		if (mustSchedule) {
			$var($Runnable, var$2, $new($XClipboard$CheckChangeTimerTask));
...
void XClipboard::getTargetsDelayed() {
	...
	try {
		...
		if (this->isSelectionNotifyProcessed || curTime >= (this->convertSelectionTime + $UNIXToolkit::getDatatransferTimeout())) {
			...
			int64_t var$2 = $$nc(this->selection->getSelectionAtom())->getAtom();
...
void XClipboard::checkChange($XSelectionEvent* xse) {
	...
	if (propertyAtom == $XConstants::None) {
		...
		$var($WindowPropertyGetter, targetsGetter, $new($WindowPropertyGetter, var$0, $($XAtom::get(propertyAtom)), 0, $XSelection::MAX_LENGTH, true, $XConstants::AnyPropertyType));
...
void XClipboard::clinit$($Class* clazz) {
...
$Class* XClipboard::load$($String* name, bool initialize) {
	...
	$loadClass(XClipboard, name, initialize, &classInfo$$, XClipboard::clinit$, []($Class* clazz) -> $Object* {
