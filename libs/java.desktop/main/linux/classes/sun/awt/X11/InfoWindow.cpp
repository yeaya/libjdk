...
$Component* InfoWindow::add($Component* c) {
	$init($BorderLayout);
	$nc(this->container)->add(c, $BorderLayout::CENTER);
	return c;
}
...
void InfoWindow::show($Point* corner, int32_t indent) {
	...
	if ($nc(corner)->x < $nc(scrSize)->x + $nc(scrSize)->width / 2 && corner->y < scrSize->y + scrSize->height / 2) {
		...
		setLocation(corner->x - indent - $nc(size)->width, corner->y - indent - $nc(size)->height);
	}
	$Window::show();
...
void InfoWindow::clinit$($Class* clazz) {
...
$Class* InfoWindow::load$($String* name, bool initialize) {
	...
	$loadClass(InfoWindow, name, initialize, &classInfo$$, InfoWindow::clinit$, []($Class* clazz) -> $Object* {
