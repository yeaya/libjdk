	...
	try {
		...
		if (this->firstCheck) {
			...
			if ($nc(XProtocol::log)->isLoggable($PlatformLogger$Level::FINE)) {
				XProtocol::log->fine("{0}:{1} supports {2}"_s, $$new($ObjectArray, {
					this,
					listName,
					protocols
				}));
...
void XProtocol::clinit$($Class* clazz) {
...
$Class* XProtocol::load$($String* name, bool initialize) {
	...
	$loadClass(XProtocol, name, initialize, &classInfo$$, XProtocol::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XProtocol);
	});
	return class$;
}
