...
void InfoWindow$Tooltip$2::run() {
	...
	$var($String, tooltipString, this->this$0->liveArguments->getTooltipString());
	if (tooltipString == nullptr) {
		return;
	} else if (tooltipString->length() > 64) {
		$nc(this->this$0->textLabel)->setText($(tooltipString->substring(0, 64)));
	} else {
		$nc(this->this$0->textLabel)->setText(tooltipString);
	}
	$var($Point, pointer, $cast($Point, $AccessController::doPrivileged($$new($InfoWindow$Tooltip$2$1, this))));
	if (pointer == nullptr) {
		return;
	}
	this->this$0->show($$new($Point, $nc(pointer)->x, $nc(pointer)->y), 5);
}
...
$Class* InfoWindow$Tooltip$2::load$($String* name, bool initialize) {
	...
	$loadClass(InfoWindow$Tooltip$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(InfoWindow$Tooltip$2);
	});
	return class$;
}
