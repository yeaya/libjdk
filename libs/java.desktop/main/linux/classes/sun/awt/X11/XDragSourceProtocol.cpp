...
void XDragSourceProtocol::clinit$($Class* clazz) {
...
$Class* XDragSourceProtocol::load$($String* name, bool initialize) {
	...
	$loadClass(XDragSourceProtocol, name, initialize, &classInfo$$, XDragSourceProtocol::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XDragSourceProtocol);
	});
	return class$;
}
