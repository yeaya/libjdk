		...
		for (int32_t i = 0; i < gdslen; ++i) {
			...
			if (pointerFound) {
				...
				if ($instanceOf($X11GraphicsDevice, device)) {
					int32_t scale = $cast($X11GraphicsDevice, device)->getScaleFactor();
...
void XMouseInfoPeer::clinit$($Class* clazz) {
...
$Class* XMouseInfoPeer::load$($String* name, bool initialize) {
	...
	$loadClass(XMouseInfoPeer, name, initialize, &classInfo$$, XMouseInfoPeer::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMouseInfoPeer);
	});
	return class$;
}
