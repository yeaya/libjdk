...
bool XEmbedCanvasPeer$XEmbedServer::processXEmbedInfo() {
	...
	try {
		...
		bool new_mapped = (this->flags & $XEmbedHelper::XEMBED_MAPPED) != 0;
		...
		if (new_mapped != currently_mapped) {
			...
			if ($nc($XEmbedCanvasPeer::xembedLog)->isLoggable($PlatformLogger$Level::FINER)) {
				$XEmbedCanvasPeer::xembedLog->finer($$str({"Mapping state of the client has changed, old state: "_s, $$str(currently_mapped), ", new state: "_s, $$str(new_mapped)}));
			}
			if (new_mapped) {
				$XToolkit::awtLock();
			...
			$init($PlatformLogger$Level);
			if ($nc($XEmbedCanvasPeer::xembedLog)->isLoggable($PlatformLogger$Level::FINER)) {
				$XEmbedCanvasPeer::xembedLog->finer($$str({"Mapping state didn\'t change, mapped: "_s, $$str(currently_mapped)}));
			}
		}
		var$2 = true;
		return$1 = true;
...
void XEmbedCanvasPeer$XEmbedServer::handlePropertyNotify($XEvent* xev) {
	...
	if (this->this$0->isXEmbedActive()) {
		...
		$init($PlatformLogger$Level);
		if ($nc($XEmbedCanvasPeer::xembedLog)->isLoggable($PlatformLogger$Level::FINER)) {
			$XEmbedCanvasPeer::xembedLog->finer($$str({"Property change on client: "_s, ev}));
		}
		if ($nc(ev)->get_atom() == $XAtom::XA_WM_NORMAL_HINTS) {
			this->this$0->childResized();
		} else {
			int64_t var$0 = ev->get_atom();
			$init($XEmbedHelper);
			if (var$0 == $nc($XEmbedHelper::XEmbedInfo)->getAtom()) {
				processXEmbedInfo();
			} else {
				int64_t var$1 = ev->get_atom();
				$init($XDnDConstants);
				if (var$1 == $nc($XDnDConstants::XA_XdndAware)->getAtom()) {
...
void XEmbedCanvasPeer$XEmbedServer::handleConfigureNotify($XEvent* xev) {
	...
	if (this->this$0->isXEmbedActive()) {
		...
		$init($PlatformLogger$Level);
		if ($nc($XEmbedCanvasPeer::xembedLog)->isLoggable($PlatformLogger$Level::FINER)) {
			$XEmbedCanvasPeer::xembedLog->finer($$str({"Bounds change on client: "_s, ev}));
		}
...
$Class* XEmbedCanvasPeer$XEmbedServer::load$($String* name, bool initialize) {
	...
	$loadClass(XEmbedCanvasPeer$XEmbedServer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
