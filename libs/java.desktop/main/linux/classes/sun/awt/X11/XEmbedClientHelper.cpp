...
void XEmbedClientHelper::setClient($XEmbeddedFramePeer* client) {
	...
	if ($nc(XEmbedClientHelper::xembedLog)->isLoggable($PlatformLogger$Level::FINE)) {
		XEmbedClientHelper::xembedLog->fine($$str({"XEmbed client: "_s, client}));
	}
...
void XEmbedClientHelper::install() {
	...
	$init($PlatformLogger$Level);
	if ($nc(XEmbedClientHelper::xembedLog)->isLoggable($PlatformLogger$Level::FINE)) {
		XEmbedClientHelper::xembedLog->fine($$str({"Installing xembedder on "_s, this->embedded}));
	}
	$var($longs, info, $new($longs, {
		$XEmbedHelper::XEMBED_VERSION,
		$XEmbedHelper::XEMBED_MAPPED
	}));
...
void XEmbedClientHelper::handleClientMessage($XEvent* xev) {
	...
	$init($PlatformLogger$Level);
	if ($nc(XEmbedClientHelper::xembedLog)->isLoggable($PlatformLogger$Level::FINE)) {
		XEmbedClientHelper::xembedLog->fine($($nc(msg)->toString()));
	}
	int64_t var$0 = $nc(msg)->get_message_type();
	$init($XEmbedHelper);
	if (var$0 == $nc($XEmbedHelper::XEmbed)->getAtom()) {
		if (XEmbedClientHelper::xembedLog->isLoggable($PlatformLogger$Level::FINE)) {
			XEmbedClientHelper::xembedLog->fine($$str({"Embedded message: "_s, $(msgidToString((int32_t)msg->get_data(1)))}));
		}
...
void XEmbedClientHelper::clinit$($Class* clazz) {
...
$Class* XEmbedClientHelper::load$($String* name, bool initialize) {
	...
	$loadClass(XEmbedClientHelper, name, initialize, &classInfo$$, XEmbedClientHelper::clinit$, []($Class* clazz) -> $Object* {
