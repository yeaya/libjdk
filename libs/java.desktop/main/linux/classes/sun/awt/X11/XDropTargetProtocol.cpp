...
bool XDropTargetProtocol::forwardClientMessageToToplevel(int64_t toplevel, $XClientMessageEvent* xclient) {
	...
	if ($nc(XDropTargetProtocol::logger)->isLoggable($PlatformLogger$Level::FINEST)) {
		XDropTargetProtocol::logger->finest("        entry={0}"_s, $$new($ObjectArray, {entry}));
	}
	if (entry == nullptr) {
		return false;
	}
	if (XDropTargetProtocol::logger->isLoggable($PlatformLogger$Level::FINEST)) {
		XDropTargetProtocol::logger->finest("        entry.isOverriden()={0}"_s, $$new($ObjectArray, {$($Boolean::valueOf($nc(entry)->isOverriden()))}));
	}
	if (!$nc(entry)->isOverriden()) {
		return false;
	}
	adjustEventForForwarding(xclient, entry);
	int64_t proxy = entry->getProxy();
	if (XDropTargetProtocol::logger->isLoggable($PlatformLogger$Level::FINEST)) {
		XDropTargetProtocol::logger->finest("        proxy={0} toplevel={1}"_s, $$new($ObjectArray, {
			$($Long::valueOf(proxy)),
			$($Long::valueOf(toplevel))
		}));
	}
	if (proxy == 0) {
		proxy = toplevel;
...
void XDropTargetProtocol::putEmbedderRegistryEntry(int64_t embedder, bool overriden, int32_t version, int64_t proxy) {
	$useLocalObjectStack();
	$synchronized(this) {
		$var($Object, var$0, $Long::valueOf(embedder));
		this->embedderRegistry->put(var$0, $$new($XDropTargetProtocol$EmbedderRegistryEntry, overriden, version, proxy));
	}
}

$XDropTargetProtocol$EmbedderRegistryEntry* XDropTargetProtocol::getEmbedderRegistryEntry(int64_t embedder) {
	$synchronized(this) {
		return $cast($XDropTargetProtocol$EmbedderRegistryEntry, this->embedderRegistry->get($($Long::valueOf(embedder))));
	}
}

void XDropTargetProtocol::removeEmbedderRegistryEntry(int64_t embedder) {
	$synchronized(this) {
		this->embedderRegistry->remove($($Long::valueOf(embedder)));
	}
}

void XDropTargetProtocol::clinit$($Class* clazz) {
...
$Class* XDropTargetProtocol::load$($String* name, bool initialize) {
	...
	$loadClass(XDropTargetProtocol, name, initialize, &classInfo$$, XDropTargetProtocol::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XDropTargetProtocol);
	});
	return class$;
}
