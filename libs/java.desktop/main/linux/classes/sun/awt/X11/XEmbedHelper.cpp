...
void XEmbedHelper::init$() {
	...
	if (XEmbedHelper::XEmbed == nullptr) {
		...
		if ($nc(XEmbedHelper::xembedLog)->isLoggable($PlatformLogger$Level::FINER)) {
			XEmbedHelper::xembedLog->finer($$str({"Created atom "_s, $($nc(XEmbedHelper::XEmbed)->toString())}));
		}
	}
	if (XEmbedHelper::XEmbedInfo == nullptr) {
		...
		$init($PlatformLogger$Level);
		if ($nc(XEmbedHelper::xembedLog)->isLoggable($PlatformLogger$Level::FINER)) {
			XEmbedHelper::xembedLog->finer($$str({"Created atom "_s, $($nc(XEmbedHelper::XEmbedInfo)->toString())}));
		}
	}
}
...
void XEmbedHelper::sendMessage(int64_t window, int32_t message, int64_t detail, int64_t data1, int64_t data2) {
	...
	try {
		...
		if ($nc(XEmbedHelper::xembedLog)->isLoggable($PlatformLogger$Level::FINE)) {
			XEmbedHelper::xembedLog->fine($$str({"Sending "_s, $(XEmbedMessageToString(msg))}));
		}
...
$String* XEmbedHelper::XEmbedMessageToString($XClientMessageEvent* msg) {
	...
	var$0->append("XEmbed message to "_s);
	var$0->append($($Long::toHexString($nc(msg)->get_window())));
	var$0->append(": "_s);
	var$0->append($(msgidToString((int32_t)msg->get_data(1))));
	var$0->append(", detail: "_s);
	var$0->append(msg->get_data(2));
	var$0->append(", data:["_s);
	var$0->append(msg->get_data(3));
	var$0->append(","_s);
	var$0->append(msg->get_data(4));
	var$0->append("]"_s);
	return ($str(var$0));
}
...
int32_t XEmbedHelper::getModifiers(int32_t state) {
	int32_t mods = 0;
	if ((state & XEmbedHelper::XEMBED_MODIFIER_SHIFT) != 0) {
		mods |= $InputEvent::SHIFT_DOWN_MASK;
	}
	if ((state & XEmbedHelper::XEMBED_MODIFIER_CONTROL) != 0) {
		mods |= $InputEvent::CTRL_DOWN_MASK;
	}
	if ((state & XEmbedHelper::XEMBED_MODIFIER_ALT) != 0) {
		mods |= $InputEvent::ALT_DOWN_MASK;
	}
	if ((state & XEmbedHelper::XEMBED_MODIFIER_SUPER) != 0) {
		mods |= $InputEvent::ALT_DOWN_MASK;
	}
	return mods;
}
...
void XEmbedHelper::clinit$($Class* clazz) {
...
$Class* XEmbedHelper::load$($String* name, bool initialize) {
	...
	$loadClass(XEmbedHelper, name, initialize, &classInfo$$, XEmbedHelper::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XEmbedHelper);
	});
	return class$;
}
