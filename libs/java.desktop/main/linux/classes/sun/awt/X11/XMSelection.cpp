		...
		try {
			try {
				...
				$synchronized(this) {
					...
					setAtom(atom, screen);
					int64_t owner = $XlibWrapper::XGetSelectionOwner(display, atom->getAtom());
					if (owner != 0) {
						setOwner(owner, screen);
						if ($nc(XMSelection::log)->isLoggable($PlatformLogger$Level::FINE)) {
...
bool XMSelection::processClientMessage($XEvent* xev, int32_t screen) {
	...
	if (var$0 == $nc(XMSelection::XA_MANAGER)->getAtom()) {
		...
		int64_t timestamp = xce->get_data(0) & (int64_t)0xffffffff;
...
$Collection* XMSelection::getListeners() {
	$synchronized(this) {
		return $cast($AbstractCollection, this->listeners);
	}
}
...
void XMSelection::dispatchSelectionChanged($XPropertyEvent* ev, int32_t screen) {
	$synchronized(this) {
		...
		if (this->listeners != nullptr) {
			$var($Iterator, iter, this->listeners->iterator());
...
void XMSelection::clinit$($Class* clazz) {
...
$Class* XMSelection::load$($String* name, bool initialize) {
	...
	$loadClass(XMSelection, name, initialize, &classInfo$$, XMSelection::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XMSelection);
	});
	return class$;
}
