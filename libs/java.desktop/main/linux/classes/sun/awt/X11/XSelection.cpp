...
XSelection* XSelection::getSelection($XAtom* atom) {
	$init(XSelection);
	return $cast(XSelection, XSelection::table->get(atom));
}
...
void XSelection::init$($XAtom* atom) {
	...
	XSelection::table->put(this->selectionAtom, this);
}
...
$longs* XSelection::getTargets(int64_t time) {
	...
	$synchronized(XSelection::lock) {
		$var($WindowPropertyGetter, targetsGetter, $new($WindowPropertyGetter, $$nc($XWindow::getXAWTRootWindow())->getWindow(), XSelection::selectionPropertyAtom, 0, XSelection::MAX_LENGTH, true, $XConstants::AnyPropertyType));
		$var($Throwable, var$0, nullptr);
...
$longs* XSelection::getFormats($WindowPropertyGetter* targetsGetter) {
	...
	if (var$1) {
		...
		if (!var$3) {
			int64_t var$4 = targetsGetter->getActualType();
			$init($XDataTransferer);
...
$bytes* XSelection::getData(int64_t format, int64_t time) {
	...
	$synchronized(XSelection::lock) {
		$var($WindowPropertyGetter, dataGetter, $new($WindowPropertyGetter, $$nc($XWindow::getXAWTRootWindow())->getWindow(), XSelection::selectionPropertyAtom, 0, XSelection::MAX_LENGTH, false, $XConstants::AnyPropertyType));
		$var($Throwable, var$0, nullptr);
		...
		try {
			...
			try {
				...
				int64_t var$6 = $XToolkit::getDisplay();
				int64_t var$7 = $$nc(getSelectionAtom())->getAtom();
				int64_t var$8 = $nc(XSelection::selectionPropertyAtom)->getAtom();
				$XlibWrapper::XConvertSelection(var$6, var$7, format, var$8, $$nc($XWindow::getXAWTRootWindow())->getWindow(), time);
				$var($Throwable, var$9, nullptr);
				$var($bytes, var$11, nullptr);
				bool return$10 = false;
				try {
					try {
						waitForSelectionNotify(dataGetter);
					} catch ($InterruptedException& ie) {
						$assign(var$11, $new($bytes, 0));
						return$10 = true;
						goto $finally2;
					}
			...
			int64_t var$14 = dataGetter->getActualType();
			$init($XDataTransferer);
			if (var$14 == $nc($XDataTransferer::INCR_ATOM)->getAtom()) {
				...
				while (true) {
					$var($WindowPropertyGetter, incrDataGetter, $new($WindowPropertyGetter, $$nc($XWindow::getXAWTRootWindow())->getWindow(), XSelection::selectionPropertyAtom, 0, XSelection::MAX_LENGTH, false, $XConstants::AnyPropertyType));
					...
					try {
						...
						try {
							try {
								int64_t var$20 = $XToolkit::getDisplay();
								int64_t var$21 = $$nc($XWindow::getXAWTRootWindow())->getWindow();
								$XlibWrapper::XDeleteProperty(var$20, var$21, $nc(XSelection::selectionPropertyAtom)->getAtom());
				...
				$XToolkit::awtLock();
				$var($Throwable, var$24, nullptr);
				try {
					int64_t var$25 = $XToolkit::getDisplay();
					int64_t var$26 = $$nc($XWindow::getXAWTRootWindow())->getWindow();
					$XlibWrapper::XDeleteProperty(var$25, var$26, $nc(XSelection::selectionPropertyAtom)->getAtom());
				} catch ($Throwable& var$27) {
					$assign(var$24, var$27);
				} /*finally*/ {
					$XToolkit::awtUnlock();
				}
				if (var$24 != nullptr) {
...
bool XSelection::handleMultipleRequest(int64_t requestor, int64_t property) {
	...
	bool conversionSucceeded = false;
	$var($WindowPropertyGetter, wpg, $new($WindowPropertyGetter, requestor, $($XAtom::get(property)), 0, XSelection::MAX_LENGTH, false, $XConstants::AnyPropertyType));
	$var($Throwable, var$0, nullptr);
	try {
		wpg->execute();
		...
		if (var$1 && (wpg->getNumberOfItems() % 2) == 0) {
			...
			if (writeBack) {
				...
				try {
					...
					int64_t var$4 = wpg->getActualType();
					int32_t var$5 = wpg->getActualFormat();
					int64_t var$6 = wpg->getData();
					$XlibWrapper::XChangeProperty(var$3, requestor, property, var$4, var$5, $XConstants::PropModeReplace, var$6, wpg->getNumberOfItems());
...
void XSelection::clinit$($Class* clazz) {
	$useLocalObjectStack();
...
$Class* XSelection::load$($String* name, bool initialize) {
	...
	$loadClass(XSelection, name, initialize, &classInfo$$, XSelection::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(XSelection);
	});
	return class$;
}
