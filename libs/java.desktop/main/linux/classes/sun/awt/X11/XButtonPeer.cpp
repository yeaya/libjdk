...
void XButtonPeer::paintPeer($Graphics* g) {
	...
	if (!this->disposed) {
		...
		paintBorder(g, $nc(this->borderInsets)->left, $nc(this->borderInsets)->top, size->width - ($nc(this->borderInsets)->left + $nc(this->borderInsets)->right), size->height - ($nc(this->borderInsets)->top + $nc(this->borderInsets)->bottom));
		...
		viewRect->width = size->width - ($nc(this->contentAreaInsets)->left + $nc(this->contentAreaInsets)->right);
		...
		viewRect->y = this->contentAreaInsets->top;
...
void XButtonPeer::paintButtonPressed($Graphics* g, $Component* b) {
	...
	g->fillRect($nc(this->contentAreaInsets)->left, $nc(this->contentAreaInsets)->top, $nc(size)->width - ($nc(this->contentAreaInsets)->left + $nc(this->contentAreaInsets)->right), $nc(size)->height - ($nc(this->contentAreaInsets)->top + $nc(this->contentAreaInsets)->bottom));
}
...
void XButtonPeer::clinit$($Class* clazz) {
...
$Class* XButtonPeer::load$($String* name, bool initialize) {
	...
	$loadClass(XButtonPeer, name, initialize, &classInfo$$, XButtonPeer::clinit$, []($Class* clazz) -> $Object* {
