...
void XAWTXSettings::initPerScreenXSettings() {
	...
	$$sure($XToolkit, $Toolkit::getDefaultToolkit())->parseXSettings(0, updatedSettings);
}
...
$Map* XAWTXSettings::getUpdatedSettings(int64_t owner) {
	...
	try {
		$var($WindowPropertyGetter, getter, $new($WindowPropertyGetter, owner, this->xSettingsPropertyAtom, 0, XAWTXSettings::MAX_LENGTH, false, $nc(this->xSettingsPropertyAtom)->getAtom()));
		$var($Throwable, var$0, nullptr);
		try {
...
void XAWTXSettings::clinit$($Class* clazz) {
...
$Class* XAWTXSettings::load$($String* name, bool initialize) {
	...
	$loadClass(XAWTXSettings, name, initialize, &classInfo$$, XAWTXSettings::clinit$, []($Class* clazz) -> $Object* {
