...
void XCheckboxMenuItemPeer::doToggleState(int64_t when) {
	...
	$var($Object, var$0, getTargetLabel());
	$var($ItemEvent, e, $new($ItemEvent, cb, $ItemEvent::ITEM_STATE_CHANGED, var$0, getTargetState() ? $ItemEvent::SELECTED : $ItemEvent::DESELECTED));
...
$Class* XCheckboxMenuItemPeer::load$($String* name, bool initialize) {
	...
	$loadClass(XCheckboxMenuItemPeer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
