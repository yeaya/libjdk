...
void XTextAreaPeer$JavaMouseEventHandler::dispatch($MouseEvent* event) {
	...
	$var($Point, p, nullptr);
	switch ($nc($XTextAreaPeer$1::$SwitchMap$sun$awt$X11$XTextAreaPeer$JavaMouseEventHandler$Pointer$Type)->get(($$nc(this->current->getType()))->ordinal())) {
	case 1:
		{
			$var($JViewport, var$0, $nc($nc(this->outer)->textPane)->getViewport());
			$assign(point, toViewportChildLocalSpace(var$0, $($nc(event)->getPoint())));
			$assign(jtext, this->outer->jtext);
	...
	case 2:
	case 3:
		$assign(c, this->current->getBar());
		$assign(p, toLocalSpace(c, $($nc(event)->getPoint())));
		if (this->current->getType() == $XTextAreaPeer$JavaMouseEventHandler$Pointer$Type::BUTTON) {
			$assign(c, this->current->getButton());
...
$MouseEvent* XTextAreaPeer$JavaMouseEventHandler::newMouseEvent($Component* source, $Point* point, $MouseEvent* template$) {
	...
	int32_t var$3 = e->getModifiersEx();
	int32_t var$2 = var$3 | e->getModifiers();
	int32_t var$4 = $nc(point)->x;
	int32_t var$5 = point->y;
	int32_t var$6 = e->getXOnScreen();
	int32_t var$7 = e->getYOnScreen();
	int32_t var$8 = e->getClickCount();
	bool var$9 = e->isPopupTrigger();
	$var($MouseEvent, nme, $new($MouseEvent, source, var$0, var$1, var$2, var$4, var$5, var$6, var$7, var$8, var$9, e->getButton()));
...
void XTextAreaPeer$JavaMouseEventHandler::setCursor() {
	...
	if (this->current->getType() == $XTextAreaPeer$JavaMouseEventHandler$Pointer$Type::TEXT) {
...
void XTextAreaPeer$JavaMouseEventHandler::grabbed_update($MouseEvent* event) {
	...
	this->grabbed = (($nc(event)->getModifiersEx() & allButtonsMask) != 0);
}
...
$Point* XTextAreaPeer$JavaMouseEventHandler::toLocalSpace($Component* local, $Point* inParentSpace) {
	...
	return $new($Point, $nc(p)->x - $nc(l)->x, $nc(p)->y - $nc(l)->y);
}
...
void XTextAreaPeer$JavaMouseEventHandler::setPointerToUnderPoint($Point* point) {
	...
	} else if (!setPointerIfPointOverScrollbar($(this->outer->textPane->getVerticalScrollBar()), point)) {
		if (!setPointerIfPointOverScrollbar($(this->outer->textPane->getHorizontalScrollBar()), point)) {
...
bool XTextAreaPeer$JavaMouseEventHandler::setPointerIfPointOverScrollbar($JScrollBar* bar, $Point* point) {
	...
	$var($XTextAreaPeer$XAWTScrollBarUI, ui, $cast($XTextAreaPeer$XAWTScrollBarUI, $cast($ScrollBarUI, bar->getUI())));
...
$Class* XTextAreaPeer$JavaMouseEventHandler::load$($String* name, bool initialize) {
	...
	$loadClass(XTextAreaPeer$JavaMouseEventHandler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(XTextAreaPeer$JavaMouseEventHandler);
	});
	return class$;
}
