...
bool X11GraphicsEnvironment::initGLX() {
	...
	$prepareNativeStatic(initGLX, bool);
...
bool X11GraphicsEnvironment::initXRender(bool verbose, bool ignoreLinuxVersion) {
	$init(X11GraphicsEnvironment);
	$prepareNativeStatic(initXRender, bool, bool verbose, bool ignoreLinuxVersion);
	bool $ret = $invokeNativeStatic(verbose, ignoreLinuxVersion);
	$finishNativeStatic();
	return $ret;
}
...
int32_t X11GraphicsEnvironment::checkShmExt() {
	$init(X11GraphicsEnvironment);
	$prepareNativeStatic(checkShmExt, int32_t);
...
$String* X11GraphicsEnvironment::getDisplayString() {
	$init(X11GraphicsEnvironment);
	$prepareNativeStatic(getDisplayString, $String*);
...
void X11GraphicsEnvironment::initDisplay(bool glxRequested) {
	$init(X11GraphicsEnvironment);
	$prepareNativeStatic(initDisplay, void, bool glxRequested);
	$invokeNativeStatic(glxRequested);
	$finishNativeStatic();
}

int32_t X11GraphicsEnvironment::getNumScreens() {
	$prepareNative(getNumScreens, int32_t);
...
int32_t X11GraphicsEnvironment::getDefaultScreenNum() {
	$prepareNative(getDefaultScreenNum, int32_t);
	int32_t $ret = $invokeNative();
	$finishNative();
	return $ret;
}
...
void X11GraphicsEnvironment::initNativeData() {
	$init(X11GraphicsEnvironment);
	$prepareNativeStatic(initNativeData, void);
	$invokeNativeStatic();
	$finishNativeStatic();
}
...
void X11GraphicsEnvironment::initDevices() {
	$synchronized(this) {
		...
		for (int32_t id = 0; id < numScreens; ++id) {
			$var($Object, var$0, $Integer::valueOf(id));
			this->devices->put(var$0, old->containsKey($($Integer::valueOf(id))) ? $$cast($X11GraphicsDevice, old->remove($($Integer::valueOf(id)))) : $$new($X11GraphicsDevice, id));
		}
		{
		...
		}
		{
			...
			for (; $nc(it)->hasNext();) {
				$var($X11GraphicsDevice, gd, $cast($X11GraphicsDevice, $$sure($WeakReference, it->next())->get()));
				if (gd != nullptr) {
					gd->invalidate($$cast($X11GraphicsDevice, this->devices->get($($Integer::valueOf(this->mainScreen)))));
					gd->displayChanged();
				} else {
					it->remove();
				}
...
$GraphicsDevice* X11GraphicsEnvironment::getDefaultScreenDevice() {
	$synchronized(this) {
		return $cast($GraphicsDevice, this->devices->get($($Integer::valueOf(this->mainScreen))));
	}
}
...
$GraphicsDeviceArray* X11GraphicsEnvironment::getScreenDevices() {
	$synchronized(this) {
		$useLocalObjectStack();
		return $cast($GraphicsDeviceArray, $$nc(this->devices->values())->toArray($$new($X11GraphicsDeviceArray, 0)));
	}
}

$GraphicsDevice* X11GraphicsEnvironment::getScreenDevice(int32_t screen) {
	$synchronized(this) {
		return $cast($GraphicsDevice, this->devices->get($($Integer::valueOf(screen))));
	}
}
...
bool X11GraphicsEnvironment::_isDisplayLocal() {
	...
	$var($String, isRemote, $cast($String, $AccessController::doPrivileged($$new($GetPropertyAction, "sun.java2d.remote"_s))));
	if (isRemote != nullptr) {
	...
	int32_t ind = $nc(display)->indexOf(u':');
	$var($String, hostName, display->substring(0, ind));
	if (ind <= 0) {
		return true;
	}
	$var($Boolean, result, $cast($Boolean, $AccessController::doPrivileged($$new($X11GraphicsEnvironment$2, hostName))));
...
bool X11GraphicsEnvironment::pRunningXinerama() {
	$init(X11GraphicsEnvironment);
	$prepareNativeStatic(pRunningXinerama, bool);
	bool $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}
...
void X11GraphicsEnvironment::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$beforeCallerSensitive();
	{
		$AccessController::doPrivileged($$new($X11GraphicsEnvironment$1));
...
$Class* X11GraphicsEnvironment::load$($String* name, bool initialize) {
	...
	$loadClass(X11GraphicsEnvironment, name, initialize, &classInfo$$, X11GraphicsEnvironment::clinit$, []($Class* clazz) -> $Object* {
