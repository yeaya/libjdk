...
void TransparencyTest::createAndShowGUI() {
	...
	TransparencyTest::frame->setSize(TransparencyTest::WIDTH, TransparencyTest::HEIGHT);
	$nc(TransparencyTest::frame)->setLocation(100, 300);
	$assignStatic(TransparencyTest::dialog, $new($JDialog, TransparencyTest::frame, false));
	TransparencyTest::dialog->setSize(250, 250);
	...
	$assignStatic(TransparencyTest::backgroundDialog, $new($JDialog, TransparencyTest::frame, false));
	TransparencyTest::backgroundDialog->setSize(250, 250);
...
void TransparencyTest::clinit$($Class* clazz) {
...
$Class* TransparencyTest::load$($String* name, bool initialize) {
	...
	$loadClass(TransparencyTest, name, initialize, &classInfo$$, TransparencyTest::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(TransparencyTest);
	});
	return class$;
}
