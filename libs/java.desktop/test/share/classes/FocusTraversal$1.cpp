...
void FocusTraversal$1::run() {
	...
	$FocusTraversal::frame->setDefaultCloseOperation($JFrame::DISPOSE_ON_CLOSE);
	...
	panel->add($FocusTraversal::prev);
	panel->add($FocusTraversal::a);
	panel->add($FocusTraversal::b);
	panel->add($FocusTraversal::c);
	panel->add($FocusTraversal::d);
	$assignStatic($FocusTraversal::next, $new($JTextField, "text"_s));
	panel->add($FocusTraversal::next);
	$var($JPanel, root, $new($JPanel));
	root->setLayout($$new($BorderLayout));
	root->add(panel, $BorderLayout::CENTER);
	root->add($$new($JButton, "OK"_s), $BorderLayout::SOUTH);
	$nc($FocusTraversal::frame)->add(root);
...
$Class* FocusTraversal$1::load$($String* name, bool initialize) {
	...
	$loadClass(FocusTraversal$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FocusTraversal$1);
	});
	return class$;
}
