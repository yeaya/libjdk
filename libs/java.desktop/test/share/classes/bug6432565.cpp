...
void bug6432565::main($StringArray* args) {
	...
	swingWorker->execute();
	while (!bug6432565::isDone->get()) {
		$Thread::sleep(100);
	}
	if ($instanceOf($ArrayStoreException, $(bug6432565::throwable->get()))) {
		$throwNew($RuntimeException, "Test failed"_s);
	}
}

void bug6432565::clinit$($Class* clazz) {
...
$Class* bug6432565::load$($String* name, bool initialize) {
	...
	$loadClass(bug6432565, name, initialize, &classInfo$$, bug6432565::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(bug6432565);
	});
	return class$;
}
