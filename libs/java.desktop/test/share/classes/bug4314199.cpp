...
void bug4314199::createAndShowMessage($String* message) {
	$useLocalObjectStack();
	$$nc(getContentPane())->add($$new($JLabel, message));
}
...
void bug4314199::createAndShowGUI() {
	...
	n2->add(n4);
	$var($JTree, tree, $new($JTree, $$new($DefaultTreeModel, n1)));
...
$Class* bug4314199::load$($String* name, bool initialize) {
	...
	$loadClass(bug4314199, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
