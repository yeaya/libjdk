...
void RepaintOnFrameIconifiedStateChangeTest::main($StringArray* args) {
	...
	if (var$0 || !toolkit->isFrameStateSupported($JFrame::NORMAL)) {
			...
			for (int32_t i = 0; i < $nc(RepaintOnFrameIconifiedStateChangeTest::compRedrawn)->length; ++i) {
				if (!$nc(RepaintOnFrameIconifiedStateChangeTest::compRedrawn)->get(i)) {
					...
					$nc($System::out)->println($($String::format("Not redrawn component #%d: \'%s\'"_s, $$new($ObjectArray, {
						$($Integer::valueOf(i)),
						$nc(RepaintOnFrameIconifiedStateChangeTest::comps)->get(i)
					}))));
				}
			}
			if (notRedrawnCompsCount > 0) {
				$throwNew($RuntimeException, $($String::format("\'%d\' components were not redrawn."_s, $$new($ObjectArray, {$($Integer::valueOf(notRedrawnCompsCount))}))));
			}
...
void RepaintOnFrameIconifiedStateChangeTest::createGUI() {
	...
	if (!($instanceOf($MetalLookAndFeel, $($UIManager::getLookAndFeel())))) {
		try {
			...
			$throwNew($RuntimeException, ulafe);
		}
	}
	...
	$nc(RepaintOnFrameIconifiedStateChangeTest::comps)->set(RepaintOnFrameIconifiedStateChangeTest::lblIndex, $assignStatic(RepaintOnFrameIconifiedStateChangeTest::label, $new($RepaintOnFrameIconifiedStateChangeTest$6, $nc(RepaintOnFrameIconifiedStateChangeTest::strsForComps->get(RepaintOnFrameIconifiedStateChangeTest::lblIndex))->get(0))));
	$nc(RepaintOnFrameIconifiedStateChangeTest::label)->setPreferredSize($$new($Dimension, 150, 50));
	content->add(RepaintOnFrameIconifiedStateChangeTest::label);
	$nc(RepaintOnFrameIconifiedStateChangeTest::comps)->set(RepaintOnFrameIconifiedStateChangeTest::btnIndex, $assignStatic(RepaintOnFrameIconifiedStateChangeTest::button, $new($RepaintOnFrameIconifiedStateChangeTest$7, $nc(RepaintOnFrameIconifiedStateChangeTest::strsForComps->get(RepaintOnFrameIconifiedStateChangeTest::btnIndex))->get(0))));
	...
	content->add(RepaintOnFrameIconifiedStateChangeTest::button);
	$nc(RepaintOnFrameIconifiedStateChangeTest::frame)->pack();
	$nc(RepaintOnFrameIconifiedStateChangeTest::frame)->setVisible(true);
}

void RepaintOnFrameIconifiedStateChangeTest::clinit$($Class* clazz) {
	$useLocalObjectStack();
...
$Class* RepaintOnFrameIconifiedStateChangeTest::load$($String* name, bool initialize) {
	...
	$loadClass(RepaintOnFrameIconifiedStateChangeTest, name, initialize, &classInfo$$, RepaintOnFrameIconifiedStateChangeTest::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(RepaintOnFrameIconifiedStateChangeTest);
	});
	return class$;
}
