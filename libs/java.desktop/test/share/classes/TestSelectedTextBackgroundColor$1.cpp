...
void TestSelectedTextBackgroundColor$1::run() {
	...
	$init($BorderLayout);
	panel->add($TestSelectedTextBackgroundColor::listModelSpinner, $BorderLayout::CENTER);
	$assignStatic($TestSelectedTextBackgroundColor::frame, $new($JFrame, "TestSelectedTextBackgroundColor"_s));
	$TestSelectedTextBackgroundColor::frame->add(panel);
...
$Class* TestSelectedTextBackgroundColor$1::load$($String* name, bool initialize) {
	...
	$loadClass(TestSelectedTextBackgroundColor$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TestSelectedTextBackgroundColor$1);
	});
	return class$;
}
