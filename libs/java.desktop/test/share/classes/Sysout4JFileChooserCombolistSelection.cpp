...
void Sysout4JFileChooserCombolistSelection::createDialogWithInstructions($StringArray* instructions) {
	...
	Sysout4JFileChooserCombolistSelection::dialog->printInstructions(instructions);
...
$Class* Sysout4JFileChooserCombolistSelection::load$($String* name, bool initialize) {
	...
	$loadClass(Sysout4JFileChooserCombolistSelection, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Sysout4JFileChooserCombolistSelection);
	});
	return class$;
}
