...
void HeadlessJDesktopPane::main($StringArray* args) {
	...
	ch->getMinimumSize();
	ch->contains(1, 2);
	$var($Component, c1, ch->add($$new($HeadlessJDesktopPane$2)));
	$var($Component, c2, ch->add($$new($HeadlessJDesktopPane$3)));
	$var($Component, c3, ch->add($$new($HeadlessJDesktopPane$4)));
...
$Class* HeadlessJDesktopPane::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PUBLIC, $method(HeadlessJDesktopPane, init$, void)},
		{"main", "([Ljava/lang/String;)V", nullptr, $PUBLIC | $STATIC, $staticMethod(HeadlessJDesktopPane, main, void, $StringArray*)},
		{}
	};
	...
	$loadClass(HeadlessJDesktopPane, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(HeadlessJDesktopPane);
	});
	return class$;
}
