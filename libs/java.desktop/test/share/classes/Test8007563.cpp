...
void Test8007563::main($StringArray* args) {
	...
	Test8007563::LATCH->await();
	if (!Test8007563::LIST->isEmpty()) {
		$throwNew($Error, $(Test8007563::LIST->toString()));
	}
}
...
void Test8007563::addOpaqueError(bool opaque) {
	$init(Test8007563);
	$useLocalObjectStack();
	Test8007563::LIST->add($$str({$($$nc($UIManager::getLookAndFeel())->getName()), " opaque="_s, $$str(opaque)}));
}

bool Test8007563::updateLookAndFeel() {
	$init(Test8007563);
	$useLocalObjectStack();
	int32_t index = (int32_t)Test8007563::LATCH->getCount() - 1;
	if (index >= 0) {
		try {
...
void Test8007563::run() {
	...
	if (this->frame == nullptr) {
		...
		$$nc(this->frame->getContentPane())->setBackground($Color::BLUE);
...
void Test8007563::clinit$($Class* clazz) {
...
$Class* Test8007563::load$($String* name, bool initialize) {
	...
	$loadClass(Test8007563, name, initialize, &classInfo$$, Test8007563::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Test8007563);
	});
	return class$;
}
