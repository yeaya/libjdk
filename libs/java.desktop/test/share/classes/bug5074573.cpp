...
bool bug5074573::test($Class* textComponentClass) {
	...
	robot->keyRelease(getCtrlKey());
	robot->waitForIdle();
	return bug5074573::resultString->equals($(getText()));
}
...
void bug5074573::initialize($Class* textComponentClass) {
	...
	try {
		...
		frame->add(bug5074573::textComponent);
		frame->pack();
...
void bug5074573::clinit$($Class* clazz) {
...
$Class* bug5074573::load$($String* name, bool initialize) {
	...
	$loadClass(bug5074573, name, initialize, &classInfo$$, bug5074573::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(bug5074573);
	});
	return class$;
}
