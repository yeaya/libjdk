...
void bug7088744::processEvent(int32_t eventType, $MouseEvent* e) {
	$init(bug7088744);
	$useLocalObjectStack();
	if (bug7088744::eventCount >= bug7088744::BUTTON_EVENTS_SEQUENCE->length) {
		$throwNew($RuntimeException, $$str({"Unexpected event "_s, e}));
	}
	$var($ints, arr, bug7088744::BUTTON_EVENTS_SEQUENCE->get(bug7088744::eventCount));
...
void bug7088744::clinit$($Class* clazz) {
	$useLocalObjectStack();
...
$Class* bug7088744::load$($String* name, bool initialize) {
	...
	$loadClass(bug7088744, name, initialize, &classInfo$$, bug7088744::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(bug7088744);
	});
	return class$;
}
