...
int32_t bug8067346::testMargin() {
	...
	for (int32_t inx = 0; inx < $nc(this->menuClasses)->length; ++inx) {
		$var($Insets, margin, $cast($Insets, $UIManager::get($$str({this->menuClasses->get(inx), this->MARGIN}))));
...
int32_t bug8067346::testCheckIconOffset() {
	$useLocalObjectStack();
	for (int32_t inx = 0; inx < $nc(this->menuClasses)->length; ++inx) {
		$var($Object, checkIconOffset, $UIManager::get($$str({this->menuClasses->get(inx), this->CHECKICONOFFSET})));
		if (checkIconOffset != nullptr && $cast($Integer, checkIconOffset)->intValue() == 0) {
			return inx + 1;
		}
	}
	return 0;
...
void bug8067346::clinit$($Class* clazz) {
...
$Class* bug8067346::load$($String* name, bool initialize) {
	...
	$loadClass(bug8067346, name, initialize, &classInfo$$, bug8067346::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(bug8067346);
	});
	return class$;
}
