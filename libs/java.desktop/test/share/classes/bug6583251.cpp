...
void bug6583251::createGui() {
	...
	bug6583251::frame->setDefaultCloseOperation($JFrame::EXIT_ON_CLOSE);
	$var($JPanel, panel, $new($JPanel));
	$assignStatic(bug6583251::menu, $new($JPopupMenu));
	bug6583251::menu->add($$new($JMenuItem, "item"_s));
	panel->setComponentPopupMenu(bug6583251::menu);
	$nc(bug6583251::frame)->add(panel);
...
void bug6583251::main($StringArray* args) {
	...
	if ($SystemTray::isSupported()) {
		...
		$var($Component, var$0, $new($JButton));
...
$Class* bug6583251::load$($String* name, bool initialize) {
	...
	$loadClass(bug6583251, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(bug6583251);
	});
	return class$;
}
