...
void HeadlessBox::main($StringArray* args) {
	...
	for (; i$ < len$; ++i$) {
		$var($Box, b, arr$->get(i$));
		{
			...
			b->getMinimumSize();
			b->contains(1, 2);
			$var($Component, c1, b->add($$new($HeadlessBox$1)));
			$var($Component, c2, b->add($$new($HeadlessBox$2)));
			$var($Component, c3, b->add($$new($HeadlessBox$3)));
			b->getComponentCount();
			b->countComponents();
			b->getComponent(1);
			b->getComponent(2);
...
$Class* HeadlessBox::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PUBLIC, $method(HeadlessBox, init$, void)},
		{"main", "([Ljava/lang/String;)V", nullptr, $PUBLIC | $STATIC, $staticMethod(HeadlessBox, main, void, $StringArray*)},
		{}
	};
	...
	$loadClass(HeadlessBox, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(HeadlessBox);
	});
	return class$;
}
