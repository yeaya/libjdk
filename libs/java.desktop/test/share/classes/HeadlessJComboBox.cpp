...
void HeadlessJComboBox::main($StringArray* args) {
	...
	ch->getMinimumSize();
	ch->contains(1, 2);
	$var($Component, c1, ch->add($$new($HeadlessJComboBox$1)));
	$var($Component, c2, ch->add($$new($HeadlessJComboBox$2)));
	$var($Component, c3, ch->add($$new($HeadlessJComboBox$3)));
...
$Class* HeadlessJComboBox::load$($String* name, bool initialize) {
	...
	$loadClass(HeadlessJComboBox, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(HeadlessJComboBox);
	});
	return class$;
}
