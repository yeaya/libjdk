...
void bug6866751::setup($JFrame* frame) {
	...
	$$nc($nc(frame)->getContentPane())->add($$new($JScrollPane, bug6866751::area));
...
$Class* bug6866751::load$($String* name, bool initialize) {
	...
	$loadClass(bug6866751, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(bug6866751);
	});
	return class$;
}
