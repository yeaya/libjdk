...
void bug8081411$CenteredSynthIcon::paintIcon($SynthContext* syntContext, $Graphics* g, int32_t x, int32_t y, int32_t w, int32_t h) {
	...
	int32_t dh = this->icon->getIconHeight(syntContext);
	int32_t dx = this->width - dw;
	int32_t dy = this->height - dh;
	this->icon->paintIcon(syntContext, g, x + dx / 2, y + dy / 2, dw + 2, dh + 2);
}
...
$Class* bug8081411$CenteredSynthIcon::load$($String* name, bool initialize) {
	...
	$loadClass(bug8081411$CenteredSynthIcon, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(bug8081411$CenteredSynthIcon);
	});
	return class$;
}
