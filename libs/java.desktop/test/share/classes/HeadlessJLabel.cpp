...
void HeadlessJLabel::main($StringArray* args) {
	...
	l->getMinimumSize();
	l->contains(1, 2);
	$var($Component, c1, l->add($$new($HeadlessJLabel$1)));
	$var($Component, c2, l->add($$new($HeadlessJLabel$2)));
	$var($Component, c3, l->add($$new($HeadlessJLabel$3)));
...
$Class* HeadlessJLabel::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PUBLIC, $method(HeadlessJLabel, init$, void)},
		{"main", "([Ljava/lang/String;)V", nullptr, $PUBLIC | $STATIC, $staticMethod(HeadlessJLabel, main, void, $StringArray*)},
		{}
	};
	...
	$loadClass(HeadlessJLabel, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(HeadlessJLabel);
	});
	return class$;
}
