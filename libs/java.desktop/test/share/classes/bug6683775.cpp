...
$Class* bug6683775$$Lambda$lambda$main$0::load$($String* name, bool initialize) {
	...
	$loadClass(bug6683775$$Lambda$lambda$main$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(bug6683775$$Lambda$lambda$main$0);
	});
	return class$;
}
...
$Class* bug6683775$$Lambda$dispose$1::load$($String* name, bool initialize) {
	...
	$loadClass(bug6683775$$Lambda$dispose$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(bug6683775$$Lambda$dispose$1);
	});
	return class$;
}
...
void bug6683775::main($StringArray* args) {
	...
	if (gc == nullptr || !$$nc(gc->getDevice())->isWindowTranslucencySupported($GraphicsDevice$WindowTranslucency::PERPIXEL_TRANSLUCENT)) {
		return;
	}
	$var($Robot, robot, $new($Robot));
	$var($JFrame, testFrame, $new($JFrame, gc));
	$SwingUtilities::invokeAndWait($$new(bug6683775$$Lambda$lambda$main$0, testFrame));
	robot->waitForIdle();
	$Thread::sleep(1500);
	...
	$SwingUtilities::invokeAndWait($$new(bug6683775$$Lambda$dispose$1, testFrame));
	$init($Color);
...
$Class* bug6683775::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("bug6683775$$Lambda$lambda$main$0")) {
			return bug6683775$$Lambda$lambda$main$0::load$(name, initialize);
		}
		if (name->equals("bug6683775$$Lambda$dispose$1")) {
	...
	$loadClass(bug6683775, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(bug6683775);
	});
	return class$;
}
