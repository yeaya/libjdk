...
bool bug8057791::findColor($Color* colorCheck, $Rectangle* bounds) {
	$init(bug8057791);
	$useLocalObjectStack();
	$var($BufferedImage, img, $new($BufferedImage, $nc(bounds)->width, $nc(bounds)->height, $BufferedImage::TYPE_INT_ARGB));
...
void bug8057791::clinit$($Class* clazz) {
...
$Class* bug8057791::load$($String* name, bool initialize) {
	...
	$loadClass(bug8057791, name, initialize, &classInfo$$, bug8057791::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(bug8057791);
	});
	return class$;
}
