...
$Class* JFileChooserTest$$Lambda$lambda$main$0::load$($String* name, bool initialize) {
	...
	$loadClass(JFileChooserTest$$Lambda$lambda$main$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JFileChooserTest$$Lambda$lambda$main$0);
	});
	return class$;
}
...
$Class* JFileChooserTest$$Lambda$lambda$main$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(JFileChooserTest$$Lambda$lambda$main$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JFileChooserTest$$Lambda$lambda$main$1$1);
	});
	return class$;
}
...
void JFileChooserTest::main($StringArray* args) {
	...
	$SwingUtilities::invokeAndWait($$new(JFileChooserTest$$Lambda$lambda$main$0, test));
	$init($TimeUnit);
	bool status = latch->await(5, $TimeUnit::MINUTES);
	if (!status) {
		$nc($System::out)->println("Test timed out."_s);
	}
	$SwingUtilities::invokeAndWait($$new(JFileChooserTest$$Lambda$lambda$main$1$1, test));
...
$Class* JFileChooserTest::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("JFileChooserTest$$Lambda$lambda$main$0")) {
			return JFileChooserTest$$Lambda$lambda$main$0::load$(name, initialize);
		}
		if (name->equals("JFileChooserTest$$Lambda$lambda$main$1$1")) {
	...
	$loadClass(JFileChooserTest, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JFileChooserTest);
	});
	return class$;
}
